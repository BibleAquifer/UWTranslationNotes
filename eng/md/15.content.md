# Translation Notes (unfoldingWord)

**unfoldingWord® Translation Notes** © 2022 unfoldingWord. Released under CC BY\-SA 4\.0 license. *unfoldingWord® Translation Notes* has been adapted in the following languages: Tok Pisin, Arabic (عربي), French (Français), Hindi (हिंदी), Indonesian (Bahasa Indonesia), Portuguese (Português), Russian (Русский), Spanish (Español), Swahili (Kiswahili), and Simplified Chinese (简体中文) from *unfoldingWord® Translation Notes* © 2022 unfoldingWord. Released under CC BY\-SA 4\.0 license by Mission Mutual



--------------------------------


## Ezra - Introduction (id: 718613)

Introduction to Ezra
====================

Part 1: General Introduction
----------------------------

### Outline of Ezra

1. The first Jewish exiles return to Jerusalem from Persia (1:1–2:70\)
2. The people rebuild and dedicate the temple in Jerusalem (3:1–6:22\)
3. More exiles return; Ezra teaches the Law of Yahweh (7:1–8:36\)
4. The problem of the people marrying foreigners, and how it is solved (9:1–10:44\)

### What is the Book of Ezra about?

The book of Ezra is about how the people of Israel returned from Babylon and tried to worship Yahweh again as the law required. To do this, they needed to rebuild their temple so that they could sacrifice to Yahweh. (See: Temple)

### How should the title of this book be translated?

The book of Ezra is named for the priest Ezra who led the first group of Jews out of exile and back to Judah. Translators can use the traditional title “Ezra.” Or they might choose a clearer title, such as “The Book about Ezra.”

Part 2: Important Religious and Cultural Concepts
-------------------------------------------------

### Why were Israelites not allowed to marry people from other nations?

Foreigners worshiped many false gods. Yahweh did not allow his people to marry foreigners because he knew that this would cause the people of Israel to worship false gods. (See: False God)

### Did all of the people of Israel return to their homeland?

Many of the Jews remained in Babylon instead of returning to the land of Israel. Many of them were successful in Babylon and desired to remain there. However, this meant that they were unable to worship Yahweh in Jerusalem as their ancestors had done.

Part 3: Important Translation Issues
------------------------------------

### How does the Book of Ezra use the term “Israel”?

The book of Ezra uses the term “Israel” to refer to the kingdom of Judah. It was mostly made up of the tribes of Judah and Benjamin. The other ten tribes had ended their loyalty to any kings descended from David. God allowed the Assyrians to conquer the other ten tribes and take them into exile. As a result, they mixed with other people groups and did not return to the land of Israel. (See: Israel)

### Are the events in the Book of Ezra told in the order that they actually happened?

Some of the events in the Book of Ezra are not told in the order that they happened, but are grouped according to theme. Translators should pay attention to notes that signal when events are probably out of chronological order.

* **Associated Passages:** Ezra 0:1
* **Associated Articles:** False God (ID: `26586@UWTranslationWords`); Israel (ID: `26737@UWTranslationWords`); Temple (ID: `27142@UWTranslationWords`)


## Ezra 1:1 (#1) (id: 462462)

וּ⁠בִ⁠שְׁנַ֣ת

**"Now in year"**

The word **now** introduces a new event in the story. Use the normal method for introducing the beginning of a true story in your language.

See: Introduction of a New Event

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#2) (id: 462463)

וּ⁠בִ⁠שְׁנַ֣ת אַחַ֗ת לְ⁠כ֨וֹרֶשׁ֙ מֶ֣לֶךְ פָּרַ֔ס

**"Now in year one of Cyrus, the king of Persia"**

The original Jewish readers of this book would have known that this is not a reference to the year when Cyrus first became king of the Persians. Rather, it is a reference to the later year when, by conquering Babylon, he became king over the Jews, since Babylon is the nation that had previously conquered the Jews and taken them into exile. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “During the first year after Cyrus, the king of Persia, conquered Babylon and became ruler over the Jews”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#3) (id: 462464)

וּ⁠בִ⁠שְׁנַ֣ת אַחַ֗ת לְ⁠כ֨וֹרֶשׁ֙ מֶ֣לֶךְ פָּרַ֔ס

**"Now in year one of Cyrus, the king of Persia"**

This expression could mean that sometime during the first year that he ruled over the Jews, Cyrus issued the decree that this verse describes. However, it could also mean that he issued this decree as soon as he became their ruler. Alternate translation: “As soon as Cyrus, the king of Persia, conquered Babylon and became the ruler of the Jews”

* **Associated Passages:** Ezra 1:1

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#4) (id: 462465)

וּ⁠בִ⁠שְׁנַ֣ת אַחַ֗ת

**"Now in year one"**

The Hebrew uses a cardinal number here, **one**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, “first,” in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of years, you can do that here in your translation. Alternate translation: “In the first year”

See: Ordinal Numbers

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#5) (id: 462466)

לְ⁠כ֨וֹרֶשׁ֙

**"of Cyrus"**

**Cyrus** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#6) (id: 462467)

פָּרַ֔ס

**"Persia"**

**Persia** is the name of an empire.

See: How to Translate Names

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#7) (id: 462468)

דְּבַר־יְהוָ֖ה

**"the word of Yahweh"**

Here, **word** refers to the thing that Yahweh spoke about. Alternate translation: “the promise that Yahweh had made”

See: Metonymy

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#8) (id: 462469)

דְּבַר־יְהוָ֖ה

**"the word of Yahweh"**

What Yahweh had said specifically was that, after 70 years in exile, the Jews would be allowed to return to their homeland. If it would be helpful to your readers, you could say that explicitly, or you could put this information in a footnote. Alternate translation: “the promise that Yahweh had made that after 70 years in exile, the Jews would be allowed to return to their homeland”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#9) (id: 462470)

יְהוָ֖ה

**"Yahweh"**

**Yahweh** is the name of God that he revealed to his people in the Old Testament. It occurs many times in the book, and it will be helpful to your readers if you translate it consistently each time.

See: How to Translate Names

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#10) (id: 462471)

מִ⁠פִּ֣י יִרְמְיָ֑ה

**"from the mouth of Jeremiah"**

Here, **mouth** represents speaking. Alternate translation: “which Jeremiah spoke about” or “which Jeremiah announced”

See: Metonymy

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#11) (id: 462472)

הֵעִ֣יר יְהוָ֗ה אֶת־ר֨וּחַ֙ כֹּ֣רֶשׁ

**"Yahweh stirred up the spirit of Cyrus"**

Here the book uses one aspect of Cyrus, his spirit, to represent all of him. Alternate translation: “Yahweh put a desire to do something into Cyrus”

See: Synecdoche

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#12) (id: 462473)

הֵעִ֣יר יְהוָ֗ה אֶת־ר֨וּחַ֙ כֹּ֣רֶשׁ

**"Yahweh stirred up the spirit of Cyrus"**

Here, the book speaks of Yahweh causing Cyrus to act by saying that he **stirred up** his spirit in the way that winds might stir up calm waters and make them move around. The meaning is that Yahweh directly influenced the heart and will of Cyrus to get him to do something. Alternate translation: “Yahweh put a desire to do something into Cyrus”

See: Metaphor

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#13) (id: 462474)

וַ⁠יַּֽעֲבֶר

**"So he caused"** \- **"to pass"**

The word **so** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result, he made … to be sent”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#14) (id: 462475)

וַ⁠יַּֽעֲבֶר־קוֹל֙ בְּ⁠כָל־מַלְכוּת֔⁠וֹ

**"So he caused a sound to pass throughout all his kingdom"**

Here, **he** means Cyrus. In this context, the **sound** is a voice speaking a message, and the voice represents the message that it speaks. But since the message could not travel by itself, ultimately the reference is to the messengers who delivered it. Alternate translation: “Cyrus sent messengers out to proclaim a decree everywhere in his empire”

See: Metonymy

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:1 (#15) (id: 462476)

וְ⁠גַם־בְּ⁠מִכְתָּ֖ב

**"and also in writing"**

Here the book leaves out some of the words that a sentence would ordinarily need in order to be complete. The meaning is: Cyrus also put this decree in writing. You could say that explicitly if it would be helpful to your readers. Alternate translation: “and Cyrus also sent out written copies of the decree” or “and Cyrus also had his scribes write down the decree in his official records”

See: Ellipsis

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Cyrus (ID: `person:Cyrus`); Word of the Lord (ID: `keyterm:WordOfTheLord`); Jeremiah (ID: `person:Jeremiah.6`); LORD (ID: `deity:Lord`)

## Ezra 1:2 (#1) (id: 462477)

יְהוָ֖ה אֱלֹהֵ֣י הַ⁠שָּׁמָ֑יִם

**"Yahweh, the God of heaven"**

Here Cyrus provides some background information to inform his subjects who Yahweh is. Alternate translation: “Yahweh, the God who is above all” or “Yahweh, the God who rules in heaven”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 1:2
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:2 (#2) (id: 462478)

כֹּ֚ל מַמְלְכ֣וֹת הָ⁠אָ֔רֶץ

**"all the kingdoms of the earth"**

**All** is an exaggeration for emphasis. There were still some kingdoms on earth that Cyrus did not rule. However, he had conquered every other large empire in his part of the world that might have been a threat to him. Alternate translation: “to be unchallenged ruler over this entire land”

See: Hyperbole

* **Associated Passages:** Ezra 1:2
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:2 (#3) (id: 462479)

נָ֣תַן לִ֔⁠י

**"has given to me"**

This is an idiom. Alternate translation: “has made me the king over”

See: Idiom

* **Associated Passages:** Ezra 1:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:2 (#4) (id: 462480)

לִ⁠בְנֽוֹת־ל֣⁠וֹ בַ֔יִת

**"to build for him a house"**

Cyrus is not going to do the building personally. Rather, he means that he is giving the Jews, the people who worship Yahweh, permission and support to rebuild his temple, which the Babylonians had destroyed. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “to make it possible for the Jews, the people who worship him, to rebuild his temple”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:2 (#5) (id: 462481)

בַ֔יִת

**"a house"**

Here, **house** means a temple. Cyrus speaks of this temple as if it would be a house in which God lived, since God’s presence would be there. Alternate translation: “a temple”

See: Metaphor

* **Associated Passages:** Ezra 1:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:2 (#6) (id: 462482)

בִּ⁠ירוּשָׁלִַ֖ם אֲשֶׁ֥ר בִּֽ⁠יהוּדָֽה

**"in Jerusalem, which is in Judah"**

Here, Cyrus provides some background information about the city where he wants the Jews to rebuild the temple of Yahweh, since many of the recipients of his message might not have known where Jerusalem was. Alternate translation: “in Jerusalem, the capital city of the province of Judah”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 1:2
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:2 (#7) (id: 462483)

בִּ⁠ירוּשָׁלִַ֖ם אֲשֶׁ֥ר בִּֽ⁠יהוּדָֽה

**"in Jerusalem, which is in Judah"**

**Jerusalem** is the name of a city, and **Judah** is the name of the province in which it was located. These names occur many times in the book, and it will be helpful to your readers if you translate them consistently each time.

See: How to Translate Names

* **Associated Passages:** Ezra 1:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:3 (#1) (id: 462484)

מִֽי־בָ⁠כֶ֣ם מִ⁠כָּל־עַמּ֗⁠וֹ

**"Whoever among you is from all his people"**

Here, **all** probably indicates that Cyrus is thinking of not only the people whom the Babylonians had taken into exile from the southern kingdom of Judah some decades before, and their descendants, but also any of the people whose ancestors the Assyrians had taken into exile from the northern kingdom of Israel nearly two centuries earlier. Cyrus now ruled over the territories to which both groups had been exiled. If any in the second group still had awareness and proof of their identity, and if they still wanted to honor and worship Yahweh, they could also return to Jerusalem and help rebuild the temple. (However, as [1:5](https://ref.ly/Ezra1:5) indicates, it was essentially Israelites from the tribes of Judah and Benjamin who actually did return.) To make this clear, you could include this information. Alternate translation: “Which of you is an Israelite from any tribe”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:3 (#2) (id: 462485)

יְהִ֤י אֱלֹהָי⁠ו֙ עִמּ֔⁠וֹ

**"may his God be with him"**

Cyrus wishing that **God** will **be with** these Israelites is an idiom that expresses his wish that God would make their journey and the rebuilding project successful. If it is more natural in your language, you could say this at the end of the verse. Alternate translation: “May his God make him successful”

See: Idiom

* **Associated Passages:** Ezra 1:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:3 (#3) (id: 462486)

וְ⁠יַ֕עַל לִ⁠ירוּשָׁלִַ֖ם

**"and may he go up to Jerusalem"**

Cyrus says **go up** because the Jews would have to travel from a river valley up into the mountains in order to return from their places of exile to Jerusalem. Alternate translation: “and let him return to Jerusalem”

See: Idiom

* **Associated Passages:** Ezra 1:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:3 (#4) (id: 462487)

לִ⁠ירוּשָׁלִַ֖ם אֲשֶׁ֣ר בִּ⁠יהוּדָ֑ה

**"to Jerusalem, which is in Judah"**

Cyrus once again provides background information about the city. Alternate translation: “to Jerusalem, in the province of Judah”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 1:3
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:3 (#5) (id: 462488)

אֶת־בֵּ֤ית יְהוָה֙

**"the house of Yahweh"**

Here, **house** means a temple. Cyrus continues to speak of this temple as if it would be a house in which Yahweh lived, since Yahweh’s presence would be there. Alternate translation: “a temple for Yahweh”

See: Metaphor

* **Associated Passages:** Ezra 1:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:3 (#6) (id: 462489)

יְהוָה֙ אֱלֹהֵ֣י יִשְׂרָאֵ֔ל ה֥וּא הָ⁠אֱלֹהִ֖ים אֲשֶׁ֥ר בִּ⁠ירוּשָׁלִָֽם

**"Yahweh, the God of Israel. He is the God who is in Jerusalem"**

Here Cyrus provides further background information to inform his subjects who Yahweh is. Alternate translation: “Yahweh, the God whom the people of Israel worship, who should have a temple in Jerusalem”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 1:3
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:3 (#7) (id: 462490)

אֱלֹהֵ֣י יִשְׂרָאֵ֔ל

**"the God of Israel"**

The phrase **the God of Israel** does not mean that God belongs to Israel, but that this is the God whom Israel worships. Alternate translation: “the God whom Israel worships”

See: Possession

* **Associated Passages:** Ezra 1:3
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:4 (#1) (id: 462491)

וְ⁠כָל־הַ⁠נִּשְׁאָ֗ר מִֽ⁠כָּל־הַ⁠מְּקֹמוֹת֮ אֲשֶׁ֣ר ה֣וּא גָֽר־שָׁם֒ יְנַשְּׂא֨וּ⁠הוּ֙ אַנְשֵׁ֣י מְקֹמ֔⁠וֹ

**"And every one who is remaining in all of the places where he is sojourning there, let the men of his place lift him"**

The structure of this sentence may present difficulties for translation because the long phrase at the beginning actually describes who will receive the action, rather than who will do the action. If it would be helpful in your language, you could say first who will do the action. Alternate translation: “Let the people who live in any place where Jewish survivors are in exile help them”

* **Associated Passages:** Ezra 1:4

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:4 (#2) (id: 462492)

וְ⁠כָל־הַ⁠נִּשְׁאָ֗ר

**"And every one who is remaining"**

This phrase refers to any Israelite who is a surviving member of the group that was taken into exile, or who is a descendant of someone in that group. The word **remaining** refers to people who are remaining or left over from a larger group. In this context, that larger group is all of the Israelites who lived in the land of Judah before the Babylonians conquered it. Alternate translation: “Any survivor of the Jews who were taken from their land”

See: Idiom

* **Associated Passages:** Ezra 1:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:4 (#3) (id: 462493)

מִֽ⁠כָּל־הַ⁠מְּקֹמוֹת֮ אֲשֶׁ֣ר ה֣וּא גָֽר־שָׁם֒

**"in all of the places where he is sojourning there"**

**Sojourning** means living somewhere other than in one’s native land. Here, **he** refers to any Jew such as is described in the previous phrase. Alternate translation: “in any of the places where a Jew may be living in exile”

See: Idiom

* **Associated Passages:** Ezra 1:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:4 (#4) (id: 462494)

אֲשֶׁ֣ר ה֣וּא גָֽר־שָׁם֒

**"where he is sojourning there"**

Saying **where** and **there** in the same phrase like this is a characteristic Hebrew construction, but it might represent an unnecessary duplication in your language. If it would be helpful in your language, you could omit any translation of the word “there.” Alternate translation: “in which he is living”

* **Associated Passages:** Ezra 1:4

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:4 (#5) (id: 462495)

יְנַשְּׂא֨וּ⁠הוּ֙ אַנְשֵׁ֣י מְקֹמ֔⁠וֹ

**"let the men of his place lift him"**

It is likely that both women and men would have helped to gather the supplies listed in the rest of this verse to support the returning Jews. So the term **men** here probably includes both groups. Alternate translation: “the people of that place should help him”

See: When Masculine Words Include Women

* **Associated Passages:** Ezra 1:4
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:4 (#6) (id: 462496)

יְנַשְּׂא֨וּ⁠הוּ֙

**"let"** \- **"lift him"**

**Lift** here is an idiom that means help. Alternate translation: “let … help him by providing him”

See: Idiom

* **Associated Passages:** Ezra 1:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:4 (#7) (id: 462497)

הַ֨⁠נְּדָבָ֔ה

**"the freewill offering"**

The book expects readers to know that these would be extra gifts, beyond the necessities already listed. They might include money to help rebuild the temple and vessels to be used in the temple, such as the ones listed in [1:7–11](https://ref.ly/Ezra1:7-Ezra1:11). Alternate translation: “any extra gifts that they want to give”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:4 (#8) (id: 462498)

לְ⁠בֵ֥ית הָ⁠אֱלֹהִ֖ים אֲשֶׁ֥ר בִּ⁠ירוּשָׁלִָֽם

**"for the house of God, which is in Jerusalem"**

**House** means a temple. Cyrus continues to speak of this temple as if it would be a house in which God lived, since God’s presence would be there. The book repeatedly uses the expressions **house**, **house of God**, and “house of Yahweh” to mean the temple in Jerusalem. It will be helpful to your readers if you translate these expressions consistently every time. Alternate translation: “for the temple that the Jews will rebuild for God in Jerusalem”

See: Metaphor

* **Associated Passages:** Ezra 1:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:5 (#1) (id: 462499)

וַ⁠יָּק֜וּמוּ

**"Then"** \- **"arose"**

The word **then** indicates that the sentence it introduces explains the results of what the previous verses have described. Alternate translation: “In response to this decree … prepared”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 1:5
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:5 (#2) (id: 462500)

וַ⁠יָּק֜וּמוּ

**"Then"** \- **"arose"**

In this context, the term **arose** means that these leaders took action to get an enterprise under way. It does not indicate that these leaders had been sitting or lying down and that they stood up. Alternate translation: “Then … started making preparations”

See: Idiom

* **Associated Passages:** Ezra 1:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:5 (#3) (id: 462501)

רָאשֵׁ֣י הָ⁠אָב֗וֹת

**"the heads of the fathers"**

This is an abbreviated way of saying the heads of the father’s houses. Alternate translation: “the clan leaders”

See: Ellipsis

* **Associated Passages:** Ezra 1:5
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:5 (#4) (id: 462502)

רָאשֵׁ֣י הָ⁠אָב֗וֹת

**"the heads of the fathers"**

Among the Israelites, the expression “father’s house” or “house of the father” originally described an extended\-family group. It later came to be used more generally to refer to a larger clan within a tribe. In this expression, the word “house” (which does not appear in the abbreviated version here) describes all the people descended from a particular person. The term views all of those descendants as if they were one household living together. Alternate translation: “the clan leaders”

* **Associated Passages:** Ezra 1:5

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:5 (#5) (id: 462503)

רָאשֵׁ֣י

**"the heads of"**

Here, **heads** is a figurative way of saying leaders. Alternate translation: “leaders”

See: Metaphor

* **Associated Passages:** Ezra 1:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:5 (#6) (id: 462504)

לִֽ⁠יהוּדָה֙ וּ⁠בִנְיָמִ֔ן

**"of Judah and Benjamin"**

These are the names of two of the tribes of Israel. Alternate translation: “of the tribes of Judah and Benjamin”

See: How to Translate Names

* **Associated Passages:** Ezra 1:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:5 (#7) (id: 462505)

לְ⁠כֹ֨ל הֵעִ֤יר הָ⁠אֱלֹהִים֙ אֶת־רוּח֔⁠וֹ

**"that is, all whose spirit God had stirred up"**

Here the book again speaks of God causing people to act by saying that he **stirred up** their spirits the way winds might stir up calm waters and get them to move around. The meaning is that God directly influenced the hearts and wills of these clan leaders to get them to do something. Alternate translation: “all those whom God had led to act”

See: Metaphor

* **Associated Passages:** Ezra 1:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:5 (#8) (id: 462506)

לַ⁠עֲל֣וֹת

**"to go up"**

As in [1:3](https://ref.ly/Ezra1:3), the book says **go up** because the Jews would have to travel from a river valley up into the mountains in order to return from their places of exile to Jerusalem. Alternate translation: “to return to Jerusalem”

See: Idiom

* **Associated Passages:** Ezra 1:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Benjamin (ID: `person:Benjamin`); Tribe of Judah (ID: `group:Judah`); Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:6 (#1) (id: 462507)

וְ⁠כָל־סְבִיבֹֽתֵי⁠הֶם֙

**"And all those around them"**

**All** is an exaggeration for emphasis. Every person who lived near a returning Jew did not necessarily provide support. But the expression indicates that the Jews received very generous support from many of their neighbors. Alternate translation: “And the people of their communities”

See: Hyperbole

* **Associated Passages:** Ezra 1:6
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:6 (#2) (id: 462508)

חִזְּק֣וּ בִֽ⁠ידֵי⁠הֶ֔ם

**"strengthened their hands"**

Here, **hands** represents strength and power. This expression means that the neighbors of the Jews gave them greater capacity to act and fulfill their project by supplying them with the items listed. Alternate translation: “enabled them by giving them”

See: Metaphor

* **Associated Passages:** Ezra 1:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:6 (#3) (id: 462509)

לְ⁠בַ֖ד עַל־כָּל־הִתְנַדֵּֽב

**"apart from all that was freely offered"**

This is a reference to the freewill offerings that are also mentioned in [1:4](https://ref.ly/Ezra1:4). Alternate translation: “in addition, the people freely gave extra gifts”

* **Associated Passages:** Ezra 1:6

* **Associated ACAI Concepts:** Cattle (ID: `fauna:Bull`); Money (ID: `realia:Money`)

## Ezra 1:7 (#1) (id: 462510)

וְ⁠הַ⁠מֶּ֣לֶךְ כּ֔וֹרֶשׁ הוֹצִ֖יא

**"And the king Cyrus brought out"**

**Cyrus** did not do this work himself. He ordered others to do it. If that is not understood in your language, you can say this explicitly, as in the UST.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:7 (#2) (id: 462511)

אֶת־כְּלֵ֣י בֵית־יְהוָ֑ה אֲשֶׁ֨ר הוֹצִ֤יא נְבֽוּכַדְנֶצַּר֙ מִ⁠יר֣וּשָׁלִַ֔ם וַֽ⁠יִּתְּנֵ֖⁠ם בְּ⁠בֵ֥ית אֱלֹהָֽי⁠ו

**"the vessels of the house of Yahweh that Nebuchadnezzar had brought out from Jerusalem and had put in the house of his gods"**

**Vessels** refers to the bowls, basins, and other objects that are listed in [1:9](https://ref.ly/Ezra1:9) and [1:10](https://ref.ly/Ezra1:10). The book assumes that readers will know that this verse is describing how Nebuchadnezzar, the king of Babylon, took these items from the temple in Jerusalem and then put them, as trophies of conquest, in a temple devoted to his own gods. If it would be helpful to your readers, you could say this explicitly in the text or in a footnote. Alternate translation: “the objects from the temple of Yahweh that Nebuchadnezzar, the king of Babylon, had taken away from Jerusalem when he captured that city and then put in the temple of his own gods”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:7 (#3) (id: 462512)

נְבֽוּכַדְנֶצַּר֙

**"Nebuchadnezzar"**

**Nebuchadnezzar** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 1:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 1:8 (#1) (id: 462513)

וַ⁠יּֽוֹצִיאֵ֗⁠ם כּ֚וֹרֶשׁ מֶ֣לֶךְ פָּרַ֔ס עַל־יַ֖ד מִתְרְדָ֣ת הַ⁠גִּזְבָּ֑ר

**"And Cyrus, the king of Persia, brought them out by the hand of Mithredath the treasurer"**

Here, **by the hand of** is a metaphor that represents control and action. The expression means that Mithredath did this at the command of Cyrus and on his behalf. Mithredath would have commanded others to do the work; he would not have carried the things in his own hands. Alternate translation: “King Cyrus of Persia ordered his treasurer, Mithredath, to bring them out”

See: Metaphor

* **Associated Passages:** Ezra 1:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Cyrus (ID: `person:Cyrus`)

## Ezra 1:8 (#2) (id: 462514)

וַ⁠יּֽוֹצִיאֵ֗⁠ם כּ֚וֹרֶשׁ מֶ֣לֶךְ פָּרַ֔ס עַל־יַ֖ד מִתְרְדָ֣ת

**"And Cyrus, the king of Persia, brought them out by the hand of Mithredath"**

The implication is that these vessels had remained in the temple where Nebuchadnezzar had put them, and so **Mithredath brought them out** from there. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “King Cyrus of Persia had his treasurer, Mithredath, bring these vessels out from the temple where Nebuchadnezzar had put them”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 1:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Cyrus (ID: `person:Cyrus`)

## Ezra 1:8 (#3) (id: 462515)

מִתְרְדָ֣ת

**"Mithredath"**

**Mithredath** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 1:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Cyrus (ID: `person:Cyrus`)

## Ezra 1:8 (#4) (id: 462516)

הַ⁠גִּזְבָּ֑ר

**"the treasurer"**

This term describes the office of a person responsible for all of the valuable possessions of a kingdom and for the places where they are stored safely.

See: Translate Unknowns

* **Associated Passages:** Ezra 1:8
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Cyrus (ID: `person:Cyrus`)

## Ezra 1:8 (#5) (id: 462517)

וַֽ⁠יִּסְפְּרֵ⁠ם֙ לְ⁠שֵׁשְׁבַּצַּ֔ר

**"And he counted them out to Sheshbazzar"**

Here, **he** refers to Mithredath, and **them** refers to the objects for the temple. The expression **counted them out** probably does not mean that Mithredath handed Sheshbazzar the objects one at a time while announcing the number of each one. It probably means that Mithredath gave Sheshbazzar the objects along with a document that listed how many there were of each kind, as recorded in [1:9](https://ref.ly/Ezra1:9) and [1:10](https://ref.ly/Ezra1:10). Alternate translation: “Mithredath turned them over to Sheshbazzar along with a detailed list of them”

See: Idiom

* **Associated Passages:** Ezra 1:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Cyrus (ID: `person:Cyrus`)

## Ezra 1:8 (#6) (id: 462518)

לְ⁠שֵׁשְׁבַּצַּ֔ר

**"to Sheshbazzar"**

This leader of the first group of exiles to return to Judah, who is called **Sheshbazzar** here and in [1:11](https://ref.ly/Ezra1:11), [5:14](https://ref.ly/Ezra5:14), and [5:16](https://ref.ly/Ezra5:16), seems to be the same person who is called Zerubbabel in the rest of the book. The reason for the difference in names is not clear. Some interpreters suggest that he was known as Sheshbazzar in the Persian court and that he used the name Zerubbabel as governor of Judah. There are examples in the Bible of Jews having both a Persian name and a Hebrew name. It would probably be best simply to translate both names consistently where they appear without calling attention to the difference, but you could add this possible explanation in a footnote.

See: How to Translate Names

* **Associated Passages:** Ezra 1:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Cyrus (ID: `person:Cyrus`)

## Ezra 1:8 (#7) (id: 462519)

הַ⁠נָּשִׂ֖יא לִ⁠יהוּדָֽה

**"the ruler of Judah"**

The book says in [5:14](https://ref.ly/Ezra5:14) that Cyrus appointed Sheshbazzar to be the governor of the province of Judah. If it would be helpful to your readers, you could use that phrase to describe him here. Alternate translation: “whom Cyrus had appointed to be the governor of the province of Judah”

* **Associated Passages:** Ezra 1:8

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Cyrus (ID: `person:Cyrus`)

## Ezra 1:9 (#1) (id: 462520)

וְ⁠אֵ֖לֶּה מִסְפָּרָ֑⁠ם

**"And this was their number"**

This expression means that what follows is the number of each kind of utensil that Cyrus is returning

See: Idiom

* **Associated Passages:** Ezra 1:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Money (ID: `realia:Money`)

## Ezra 1:9 (#2) (id: 462521)

מַחֲלָפִ֖ים

**"knives"**

This word occurs only here in the Hebrew Bible, and there is some disagreement about its meaning. It is probably a borrowed term from a related language that means “knife,” but the ancient Greek translation took its meaning to be “replacements,” that is, basins to replace others that became unusable. You may see translations that follow the ancient Greek, but we recommend following the Hebrew.

* **Associated Passages:** Ezra 1:9

* **Associated ACAI Concepts:** Money (ID: `realia:Money`)

## Ezra 1:10 (#1) (id: 462522)

כְּפ֤וֹרֵי כֶ֨סֶף֙ מִשְׁנִ֔ים

**"bowls of silver of a second kind"**

This means that these bowls were a different type of bowl than the gold ones just mentioned. This could also mean that they were second in rank of status or honor behind the gold bowls. It does not mean that the silver was of a different type than the silver in the basins described in [1:9](https://ref.ly/Ezra1:9).

* **Associated Passages:** Ezra 1:10

* **Associated ACAI Concepts:** Money (ID: `realia:Money`)

## Ezra 1:11 (#1) (id: 462523)

כָּל־כֵּלִים֙ לַ⁠זָּהָ֣ב וְ⁠לַ⁠כֶּ֔סֶף חֲמֵ֥שֶׁת אֲלָפִ֖ים וְ⁠אַרְבַּ֣ע מֵא֑וֹת

**"All the vessels of gold and of silver were 5400"**

This expression means that the total number of these gold and silver objects was **5,400**. Verses [1:9](https://ref.ly/Ezra1:9) and [1:10](https://ref.ly/Ezra1:10) actually list only 2,499 items, and it is not clear why the numbers are different. The explanation does not seem to be that the total here includes other miscellaneous items because the last item on the list itself is other vessels. It would probably be best simply to report this total without calling attention to the difference and trying to explain it, since there is no clear reason for it.

* **Associated Passages:** Ezra 1:11

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); Money (ID: `realia:Money`)

## Ezra 1:11 (#2) (id: 462524)

הַ⁠כֹּ֞ל הֶעֱלָ֣ה שֵׁשְׁבַּצַּ֗ר עִ֚ם הֵעָל֣וֹת הַ⁠גּוֹלָ֔ה מִ⁠בָּבֶ֖ל לִ⁠ירוּשָׁלִָֽם

**"Sheshbazzar brought up all this with the going up of the exiles from Babylon to Jerusalem"**

Like the similar expression in [1:3](https://ref.ly/Ezra1:3), here the book says **brought up** and **going up** because the Jews had to travel from a river valley up into the mountains to return from exile to Jerusalem. Alternate translation: “Sheshbazzar brought all these vessels along when he traveled to Jerusalem with the group of Jews who had been taken away from their homeland but who were now returning from Babylon to Jerusalem”

See: Idiom

* **Associated Passages:** Ezra 1:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); Money (ID: `realia:Money`)

## Ezra 1:11 (#3) (id: 462525)

הַ⁠גּוֹלָ֔ה

**"the exiles"**

The abstract noun **exiles** refers, in this context, to the community of Jews who were living in Babylon because the Babylonians had relocated them away from their homeland when they conquered Jerusalem. If it would be helpful in your language, you could translate the idea behind this term with an equivalent phrase. Alternate translation: “the group of Jews who had been taken away from their homeland”

See: Abstract Nouns

* **Associated Passages:** Ezra 1:11
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); Money (ID: `realia:Money`)

## Ezra 1:11 (#4) (id: 462526)

מִ⁠בָּבֶ֖ל

**"from Babylon"**

**Babylon** is the name of a city. The book also uses this name for the empire that was once ruled from that city, and for the region that had been at the heart of this empire that became a province in the Persian Empire. The name occurs many times in the book, and it will be helpful to your readers if you translate it consistently each time.

See: How to Translate Names

* **Associated Passages:** Ezra 1:11
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); Money (ID: `realia:Money`)

## Ezra 2:1 (#1) (id: 462527)

בְּנֵ֣י הַ⁠מְּדִינָ֗ה

**"the sons of the province"**

Here, **the sons of the province** is an idiom that means that these people originated in the province of Judah. Alternate translation: “the people from the province of Judah”

See: Idiom

* **Associated Passages:** Ezra 2:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 2:1 (#2) (id: 462528)

הַ⁠מְּדִינָ֗ה

**"the province"**

**The province** means the province of Judah. The book is referring to Judah by something associated with it, its status as a province.

See: Metonymy

* **Associated Passages:** Ezra 2:1
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 2:1 (#3) (id: 462529)

הָֽ⁠עֹלִים֙

**"the ones who went up"**

**Went up** means traveled from Babylon back to Judah, since that involves going from a river valley up into the mountains. Alternate translation: “those who returned to Judah from Babylon”

See: Idiom

* **Associated Passages:** Ezra 2:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 2:1 (#4) (id: 462530)

מִ⁠שְּׁבִ֣י הַ⁠גּוֹלָ֔ה אֲשֶׁ֥ר הֶגְלָ֛ה נְבוּכַדְנֶצַּ֥ר מֶֽלֶךְ־בָּבֶ֖ל לְ⁠בָבֶ֑ל

**"from the captivity of the exiles whom Nebuchadnezzar, the king of Babylon, had exiled to Babylon"**

The abstract nouns **captivity** and **exiles** refer to the way Nebuchadnezzar took these people prisoner and transported them away from their homeland. If it would be helpful in your language, you could translate the idea behind these terms with verbs. Alternate translation: “after Nebuchadnezzar, the king of Babylon, had captured their ancestors, transported them to Babylon, and held them there in a foreign land”

See: Abstract Nouns

* **Associated Passages:** Ezra 2:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 2:1 (#5) (id: 462531)

אִ֥ישׁ לְ⁠עִירֽ⁠וֹ

**"a man to his city"**

Here, **a man** means each one or each person. Alternate translation: “returning to live in the same towns where their families had lived before”

See: Idiom

* **Associated Passages:** Ezra 2:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 2:2 (#1) (id: 462532)

אֲשֶׁר־בָּ֣אוּ עִם

**"who came with"**

This phrase introduces a list of the men who led this group back to Judah. Alternate translation: “traveling with”

* **Associated Passages:** Ezra 2:2

* **Associated ACAI Concepts:** Baanah (ID: `person:Baanah.3`); Bigvai (ID: `person:Bigvai`); Reelaiah (ID: `person:Reelaiah`); Mordecai (ID: `person:Mordecai`); Seraiah (ID: `person:Seraiah.6`); Nehemiah (ID: `person:Nehemiah`); Mispar (ID: `person:Mispar`); Rehum (ID: `person:Rehum`); Bilshan (ID: `person:Bilshan`); Zerubbabel (ID: `person:Zerubbabel.2`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`)

## Ezra 2:2 (#2) (id: 462533)

זְרֻבָּבֶ֗ל יֵשׁ֡וּעַ נְ֠חֶמְיָה שְׂרָיָ֨ה רְֽעֵלָיָ֜ה מָרְדֳּכַ֥י בִּלְשָׁ֛ן מִסְפָּ֥ר בִּגְוַ֖י רְח֣וּם בַּעֲנָ֑ה

**"Zerubbabel, Jeshua, Nehemiah, Seraiah, Reelaiah, Mordecai, Bilshan, Mispar, Bigvai, Rehum, and Baanah"**

These are the names of eleven men.

See: How to Translate Names

* **Associated Passages:** Ezra 2:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Baanah (ID: `person:Baanah.3`); Bigvai (ID: `person:Bigvai`); Reelaiah (ID: `person:Reelaiah`); Mordecai (ID: `person:Mordecai`); Seraiah (ID: `person:Seraiah.6`); Nehemiah (ID: `person:Nehemiah`); Mispar (ID: `person:Mispar`); Rehum (ID: `person:Rehum`); Bilshan (ID: `person:Bilshan`); Zerubbabel (ID: `person:Zerubbabel.2`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`)

## Ezra 2:2 (#3) (id: 462534)

יֵשׁ֡וּעַ

**"Jeshua"**

This is the high priest, who has the same name as the Levite who is introduced in [2:40](https://ref.ly/Ezra2:40). The spelling of this name varies throughout the Bible. To be clear which man is being referred to, the UST will spell this name “Joshua” for the high priest and “Jeshua” for the Levite. You may want to consider whether doing something similar would be helpful for your readers.

See: How to Translate Names

* **Associated Passages:** Ezra 2:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Baanah (ID: `person:Baanah.3`); Bigvai (ID: `person:Bigvai`); Reelaiah (ID: `person:Reelaiah`); Mordecai (ID: `person:Mordecai`); Seraiah (ID: `person:Seraiah.6`); Nehemiah (ID: `person:Nehemiah`); Mispar (ID: `person:Mispar`); Rehum (ID: `person:Rehum`); Bilshan (ID: `person:Bilshan`); Zerubbabel (ID: `person:Zerubbabel.2`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`)

## Ezra 2:2 (#4) (id: 462535)

מִסְפַּ֕ר אַנְשֵׁ֖י עַ֥ם יִשְׂרָאֵֽל

**"This is the number of the men of the people of Israel"**

In keeping with the practices of the time, the totals in the list that follows likely include just the men and not also the women and children. Alternate translation: “This is how many men came back from each Israelite clan and town”

* **Associated Passages:** Ezra 2:2

* **Associated ACAI Concepts:** Baanah (ID: `person:Baanah.3`); Bigvai (ID: `person:Bigvai`); Reelaiah (ID: `person:Reelaiah`); Mordecai (ID: `person:Mordecai`); Seraiah (ID: `person:Seraiah.6`); Nehemiah (ID: `person:Nehemiah`); Mispar (ID: `person:Mispar`); Rehum (ID: `person:Rehum`); Bilshan (ID: `person:Bilshan`); Zerubbabel (ID: `person:Zerubbabel.2`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`)

## Ezra 2:3 (#1) (id: 462536)

בְּנֵ֣י פַרְעֹ֔שׁ אַלְפַּ֕יִם מֵאָ֖ה שִׁבְעִ֥ים וּ⁠שְׁנָֽיִם

**"The sons of Parosh were 2, 172"**

This means that from the descendants **of Parosh**, **2,172** returned. To help make this clear for your readers, you could say something like “returned” throughout [2:3–42](https://ref.ly/Ezra2:3-Ezra2:42), after the name of each group and the number that is given.

* **Associated Passages:** Ezra 2:3

* **Associated ACAI Concepts:** Parosh (ID: `person:Parosh`)

## Ezra 2:3 (#2) (id: 462537)

בְּנֵ֣י פַרְעֹ֔שׁ

**"The sons of Parosh"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Parosh”

See: Metaphor

* **Associated Passages:** Ezra 2:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Parosh (ID: `person:Parosh`)

## Ezra 2:3 (#3) (id: 462538)

פַרְעֹ֔שׁ

**"Parosh"**

**Parosh** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Parosh (ID: `person:Parosh`)

## Ezra 2:4 (#1) (id: 462539)

בְּנֵ֣י שְׁפַטְיָ֔ה

**"The sons of Shephatiah"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Shephatiah”

See: Metaphor

* **Associated Passages:** Ezra 2:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Shephatiah (ID: `person:Shephatiah.6`)

## Ezra 2:4 (#2) (id: 462540)

שְׁפַטְיָ֔ה

**"Shephatiah"**

**Shephatiah** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:4
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Shephatiah (ID: `person:Shephatiah.6`)

## Ezra 2:5 (#1) (id: 462541)

בְּנֵ֣י אָרַ֔ח

**"The sons of Arah"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Arah”

See: Metaphor

* **Associated Passages:** Ezra 2:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Arah (ID: `person:Arah.2`)

## Ezra 2:5 (#2) (id: 462542)

אָרַ֔ח

**"Arah"**

**Arah** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Arah (ID: `person:Arah.2`)

## Ezra 2:6 (#1) (id: 462543)

בְּנֵֽי־פַחַ֥ת מוֹאָ֛ב לִ⁠בְנֵ֥י יֵשׁ֖וּעַ יוֹאָ֑ב

**"The sons of Pahath\-Moab, of the sons of Jeshua and Joab"**

Here, **sons** means descendants.

See: Metaphor

* **Associated Passages:** Ezra 2:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.3`); Joab (ID: `person:Joab.3`); Pahath-moab (ID: `person:Pahath-moab`)

## Ezra 2:6 (#2) (id: 462544)

פַחַ֥ת מוֹאָ֛ב \& יֵשׁ֖וּעַ

**"Pahath\-Moab,"** \- **"Jeshua"**

**Pahath\-Moab** is the name of a man, and **Jeshua** and **Joab** are the names of two of his male descendants.

See: How to Translate Names

* **Associated Passages:** Ezra 2:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.3`); Joab (ID: `person:Joab.3`); Pahath-moab (ID: `person:Pahath-moab`)

## Ezra 2:7 (#1) (id: 462545)

בְּנֵ֣י עֵילָ֔ם

**"The sons of Elam"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Elam”

See: Metaphor

* **Associated Passages:** Ezra 2:7
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Elam (ID: `person:Elam`)

## Ezra 2:7 (#2) (id: 462546)

עֵילָ֔ם

**"Elam"**

**Elam** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Elam (ID: `person:Elam`)

## Ezra 2:8 (#1) (id: 462547)

בְּנֵ֣י זַתּ֔וּא

**"The sons of Zattu"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Zattu”

See: Metaphor

* **Associated Passages:** Ezra 2:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zattu (ID: `person:Zattu.2`)

## Ezra 2:8 (#2) (id: 462548)

זַתּ֔וּא

**"Zattu"**

**Zattu** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zattu (ID: `person:Zattu.2`)

## Ezra 2:9 (#1) (id: 462549)

בְּנֵ֣י

**"The sons of"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Zakkai”

See: Metaphor

* **Associated Passages:** Ezra 2:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zaccai (ID: `person:Zaccai.2`)

## Ezra 2:9 (#2) (id: 462550)

זַכָּ֔י

**"Zakkai"**

**Zakkai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zaccai (ID: `person:Zaccai.2`)

## Ezra 2:10 (#1) (id: 462551)

בְּנֵ֣י בָנִ֔י

**"The sons of Bani"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Bani”

See: Metaphor

* **Associated Passages:** Ezra 2:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bani (ID: `person:Bani`)

## Ezra 2:10 (#2) (id: 462552)

בָנִ֔י

**"Bani"**

**Bani** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:10
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bani (ID: `person:Bani`)

## Ezra 2:11 (#1) (id: 462553)

בְּנֵ֣י בֵבָ֔י

**"The sons of Bebai"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Bebai”

See: Metaphor

* **Associated Passages:** Ezra 2:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bebai (ID: `person:Bebai`)

## Ezra 2:11 (#2) (id: 462554)

בֵבָ֔י

**"Bebai"**

**Bebai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:11
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bebai (ID: `person:Bebai`)

## Ezra 2:12 (#1) (id: 462555)

בְּנֵ֣י עַזְגָּ֔ד

**"The sons of Azgad"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Azgad”

See: Metaphor

* **Associated Passages:** Ezra 2:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Azgad (ID: `person:Azgad`)

## Ezra 2:12 (#2) (id: 462556)

עַזְגָּ֔ד

**"Azgad"**

**Azgad** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:12
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Azgad (ID: `person:Azgad`)

## Ezra 2:13 (#1) (id: 462557)

בְּנֵי֙ אֲדֹ֣נִיקָ֔ם

**"The sons of Adonikam"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Adonikam”

See: Metaphor

* **Associated Passages:** Ezra 2:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Adonikam (ID: `person:Adonikam`)

## Ezra 2:13 (#2) (id: 462558)

אֲדֹ֣נִיקָ֔ם

**"Adonikam"**

**Adonikam** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:13
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Adonikam (ID: `person:Adonikam`)

## Ezra 2:14 (#1) (id: 462559)

בְּנֵ֣י בִגְוָ֔י

**"The sons of Bigvai"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Bigvai”

See: Metaphor

* **Associated Passages:** Ezra 2:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigvai (ID: `person:Bigvai.2`)

## Ezra 2:14 (#2) (id: 462560)

בִגְוָ֔י

**"Bigvai"**

**Bigvai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:14
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigvai (ID: `person:Bigvai.2`)

## Ezra 2:15 (#1) (id: 462561)

בְּנֵ֣י עָדִ֔ין

**"The sons of Adin"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Adin”

See: Metaphor

* **Associated Passages:** Ezra 2:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Adin (ID: `person:Adin`)

## Ezra 2:15 (#2) (id: 462562)

עָדִ֔ין

**"Adin"**

**Adin** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:15
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Adin (ID: `person:Adin`)

## Ezra 2:16 (#1) (id: 462563)

בְּנֵֽי־אָטֵ֥ר

**"The sons of Ater"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Ater who were descendants of Hezekiah”

See: Metaphor

* **Associated Passages:** Ezra 2:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hezekiah (ID: `person:Hezekiah.2`); Ater (ID: `person:Ater`)

## Ezra 2:16 (#2) (id: 462564)

אָטֵ֥ר לִֽ⁠יחִזְקִיָּ֖ה

**"Ater, of Hezekiah"**

**Ater** is the name of a man, and **Hezekiah** is the name of one of his descendants.

See: How to Translate Names

* **Associated Passages:** Ezra 2:16
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hezekiah (ID: `person:Hezekiah.2`); Ater (ID: `person:Ater`)

## Ezra 2:17 (#1) (id: 462565)

בְּנֵ֣י בֵצָ֔י

**"The sons of Bezai"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Bezai”

See: Metaphor

* **Associated Passages:** Ezra 2:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bezai (ID: `person:Bezai`)

## Ezra 2:17 (#2) (id: 462566)

בֵצָ֔י

**"Bezai"**

**Bezai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:17
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bezai (ID: `person:Bezai`)

## Ezra 2:18 (#1) (id: 462567)

בְּנֵ֣י יוֹרָ֔ה

**"The sons of Jorah"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Jorah”

See: Metaphor

* **Associated Passages:** Ezra 2:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jorah (ID: `person:Jorah`)

## Ezra 2:18 (#2) (id: 462568)

יוֹרָ֔ה

**"Jorah"**

**Jorah** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:18
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jorah (ID: `person:Jorah`)

## Ezra 2:19 (#1) (id: 462569)

בְּנֵ֣י חָשֻׁ֔ם

**"The sons of Hashum"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Hashum”

See: Metaphor

* **Associated Passages:** Ezra 2:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashum (ID: `person:Hashum`)

## Ezra 2:19 (#2) (id: 462570)

חָשֻׁ֔ם

**"Hashum"**

**Hashum** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:19
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashum (ID: `person:Hashum`)

## Ezra 2:20 (#1) (id: 462571)

בְּנֵ֥י גִבָּ֖ר

**"The sons of Gibbar"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Gibbar”

See: Metaphor

* **Associated Passages:** Ezra 2:20
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Gibeon (ID: `place:Gibeon`)

## Ezra 2:20 (#2) (id: 462572)

גִבָּ֖ר

**"Gibbar"**

**Gibbar** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:20
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Gibeon (ID: `place:Gibeon`)

## Ezra 2:21 (#1) (id: 462573)

בְּנֵ֣י בֵֽית־לָ֔חֶם מֵאָ֖ה עֶשְׂרִ֥ים וּ⁠שְׁלֹשָֽׁה

**"The sons of Bethlehem were 123"**

The list speaks here of the **sons of Bethlehem**. This is an idiom that means that these men were from families that had originally lived in that town. Alternate translation: “From the town of Bethlehem, 123 returned”

See: Idiom

* **Associated Passages:** Ezra 2:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Bethlehem (ID: `place:Bethlehem`)

## Ezra 2:21 (#2) (id: 462574)

בֵֽית־לָ֔חֶם

**"Bethlehem"**

**Bethlehem** is the name of a town.

See: How to Translate Names

* **Associated Passages:** Ezra 2:21
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bethlehem (ID: `place:Bethlehem`)

## Ezra 2:22 (#1) (id: 462575)

אַנְשֵׁ֥י נְטֹפָ֖ה חֲמִשִּׁ֥ים וְ⁠שִׁשָּֽׁה

**"The men of Netophah were 56"**

**Netophah** is the name of a town. Alternate translation: “From the town of Netophah, 56 returned.”

See: How to Translate Names

* **Associated Passages:** Ezra 2:22
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Netophah (ID: `place:Netophah`)

## Ezra 2:23 (#1) (id: 462576)

אַנְשֵׁ֣י עֲנָת֔וֹת מֵאָ֖ה עֶשְׂרִ֥ים וּ⁠שְׁמֹנָֽה

**"The men of Anathoth were 128"**

**Anathoth** is the name of a town. Alternate translation: “From the town of Anathoth, 128 returned”

See: How to Translate Names

* **Associated Passages:** Ezra 2:23
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Anathoth (ID: `place:Anathoth`)

## Ezra 2:24 (#1) (id: 462577)

בְּנֵ֥י עַזְמָ֖וֶת

**"The sons of Azmaveth"**

The list speaks here of the **sons of Azmaveth**. This is an idiom that means that these men were from families that had originally lived in that town. Alternate translation: “From the town of Azmaveth, 42 returned”

See: Idiom

* **Associated Passages:** Ezra 2:24
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Beth-azmaveth (ID: `place:Beth-azmaveth`)

## Ezra 2:24 (#2) (id: 462578)

עַזְמָ֖וֶת

**"Azmaveth"**

**Azmaveth** is the name of a town.

See: How to Translate Names

* **Associated Passages:** Ezra 2:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Beth-azmaveth (ID: `place:Beth-azmaveth`)

## Ezra 2:25 (#1) (id: 462579)

בְּנֵ֨י קִרְיַ֤ת עָרִים֙ כְּפִירָ֣ה וּ⁠בְאֵר֔וֹת

**"The sons of Kirjath\-Arim, Kephirah, and Beeroth"**

The list speaks here of the **sons of Kirjath\-Arim, Kephirah, and Beeroth**. This is an idiom that means that these men were from families that had originally lived in those towns. Alternate translation: “From the towns of Kirjath\-Arim, Kephirah, and Beeroth”

See: Idiom

* **Associated Passages:** Ezra 2:25
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Chephirah (ID: `place:Chephirah`); Beeroth (ID: `place:Beeroth`); Kiriath-jearim (ID: `place:Kiriath-jearim`)

## Ezra 2:25 (#2) (id: 462580)

קִרְיַ֤ת עָרִים֙ כְּפִירָ֣ה וּ⁠בְאֵר֔וֹת

**"Kirjath\-Arim, Kephirah, and Beeroth"**

**Kirjath\-Arim, Kephirah**, and **Beeroth** are the names of towns.

See: How to Translate Names

* **Associated Passages:** Ezra 2:25
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Chephirah (ID: `place:Chephirah`); Beeroth (ID: `place:Beeroth`); Kiriath-jearim (ID: `place:Kiriath-jearim`)

## Ezra 2:26 (#1) (id: 462581)

בְּנֵ֤י הָ⁠רָמָה֙ וָ⁠גָ֔בַע

**"The sons of the Ramah and Geba"**

The list speaks here of the **sons of Ramah and Geba**. This is an idiom that means that these men were from families that had originally lived in those towns. Alternate translation: “From the towns of Ramah and Geba”

See: Idiom

* **Associated Passages:** Ezra 2:26
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Geba (ID: `place:Geba`); Ramah (ID: `place:Ramah.2`)

## Ezra 2:26 (#2) (id: 462582)

הָ⁠רָמָה֙ וָ⁠גָ֔בַע

**"the Ramah and Geba"**

**Ramah** and **Geba** are the names of towns.

See: How to Translate Names

* **Associated Passages:** Ezra 2:26
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Geba (ID: `place:Geba`); Ramah (ID: `place:Ramah.2`)

## Ezra 2:27 (#1) (id: 462583)

אַנְשֵׁ֣י מִכְמָ֔ס מֵאָ֖ה עֶשְׂרִ֥ים וּ⁠שְׁנָֽיִם

**"The men of Michmas were 122"**

**Michmas** is the name of a town. Alternate translation: “From the town of Michmas, 122 returned”

See: How to Translate Names

* **Associated Passages:** Ezra 2:27
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Michmash (ID: `place:Michmash`)

## Ezra 2:28 (#1) (id: 462584)

אַנְשֵׁ֤י בֵֽית־אֵל֙ וְ⁠הָ⁠עָ֔י מָאתַ֖יִם עֶשְׂרִ֥ים וּ⁠שְׁלֹשָֽׁה

**"The men of Bethel and the Ai were 223"**

**Bethel** and **Ai** are the names of towns. Alternate translation: “From the towns of Bethel and Ai, 223 returned”

See: How to Translate Names

* **Associated Passages:** Ezra 2:28
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bethel (ID: `place:Bethel`); Ai (ID: `place:Ai`)

## Ezra 2:29 (#1) (id: 462585)

בְּנֵ֥י נְב֖וֹ

**"The sons of Nebo"**

The list speaks here of the **sons of Nebo**. This is an idiom that means that these men were from families that had originally lived in this town. Alternate translation: “From the town of Nebo”

See: Idiom

* **Associated Passages:** Ezra 2:29
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebo (ID: `place:Nebo.2`)

## Ezra 2:29 (#2) (id: 462586)

נְב֖וֹ

**"Nebo"**

**Nebo** is the name of a town.

See: How to Translate Names

* **Associated Passages:** Ezra 2:29
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebo (ID: `place:Nebo.2`)

## Ezra 2:30 (#1) (id: 462587)

בְּנֵ֣י מַגְבִּ֔ישׁ

**"The sons of Magbish"**

The list speaks here of the **sons of Magbish**. This is an idiom that means that these men were from families that had originally lived in those towns. Alternate translation: “From the town of Magbish”

See: Idiom

* **Associated Passages:** Ezra 2:30
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Magbish (ID: `place:Magbish`)

## Ezra 2:30 (#2) (id: 462588)

מַגְבִּ֔ישׁ

**"Magbish"**

**Magbish** is the name of a town.

See: How to Translate Names

* **Associated Passages:** Ezra 2:30
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Magbish (ID: `place:Magbish`)

## Ezra 2:31 (#1) (id: 462589)

בְּנֵי֙ עֵילָ֣ם אַחֵ֔ר

**"The sons of the other Elam"**

The list speaks here of the **sons of Elam**. This is an idiom that means that these men were from families that had originally lived in this town. (“The other Elam” indicates that this is the town of that name, not the person of that name mentioned in [2:7](https://ref.ly/Ezra2:7).) Alternate translation: “From the town of Elam”

See: Idiom

* **Associated Passages:** Ezra 2:31
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Elam (ID: `person:Elam`)

## Ezra 2:31 (#2) (id: 462590)

עֵילָ֣ם

**"Elam"**

Here, **Elam** is the name of a town.

See: How to Translate Names

* **Associated Passages:** Ezra 2:31
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Elam (ID: `person:Elam`)

## Ezra 2:32 (#1) (id: 462591)

בְּנֵ֣י חָרִ֔ם

**"The sons of Harim"**

The list speaks here of the **sons of Harim**. This is an idiom that means that these men were from families that had originally lived in this town. Alternate translation: “From the town of Harim”

See: Idiom

* **Associated Passages:** Ezra 2:32
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Harim (ID: `person:Harim.2`)

## Ezra 2:32 (#2) (id: 462592)

חָרִ֔ם

**"Harim"**

**Harim** is the name of a town.

See: How to Translate Names

* **Associated Passages:** Ezra 2:32
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Harim (ID: `person:Harim.2`)

## Ezra 2:33 (#1) (id: 462593)

בְּנֵי־לֹד֙ חָדִ֣יד וְ⁠אוֹנ֔וֹ

**"The sons of Lod, Hadid, and Ono"**

The list speaks here of the **sons of Lod, Hadid, and Ono**. This is an idiom that means that these men were from families that had originally lived in these towns. Alternate translation: “From the towns of Lod, Hadid, and Ono”

See: Idiom

* **Associated Passages:** Ezra 2:33
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hadid (ID: `place:Hadid`); Lod (ID: `place:Lod`); Ono (ID: `place:Ono`)

## Ezra 2:33 (#2) (id: 462594)

לֹד֙ חָדִ֣יד וְ⁠אוֹנ֔וֹ

**"Lod, Hadid, and Ono"**

**Lod, Hadid, and Ono** are the names of towns.

See: How to Translate Names

* **Associated Passages:** Ezra 2:33
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hadid (ID: `place:Hadid`); Lod (ID: `place:Lod`); Ono (ID: `place:Ono`)

## Ezra 2:34 (#1) (id: 462595)

בְּנֵ֣י יְרֵח֔וֹ

**"The sons of Jericho"**

Here, the list speaks of the **sons of Jericho**. This is an idiom that means that these men were from families that had originally lived in this city. Alternate translation: “From the city of Jericho”

See: Idiom

* **Associated Passages:** Ezra 2:34
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jericho (ID: `place:Jericho`)

## Ezra 2:35 (#1) (id: 462596)

בְּנֵ֣י סְנָאָ֔ה

**"The sons of Senaah"**

The list speaks here of the **sons of Senaah**. This is an idiom that means that these men were from families that had originally lived in this town. Alternate translation: “From the town of Senaah”

See: Idiom

* **Associated Passages:** Ezra 2:35
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Senaah (ID: `place:Senaah`)

## Ezra 2:35 (#2) (id: 462597)

סְנָאָ֔ה

**"Senaah"**

**Senaah** is the name of a town.

See: How to Translate Names

* **Associated Passages:** Ezra 2:35
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Senaah (ID: `place:Senaah`)

## Ezra 2:36 (#1) (id: 462598)

הַֽ⁠כֹּהֲנִ֑ים

**"The priests"**

**The priests** were men chosen to offer sacrifices to God on behalf of God’s people, and to perform other duties and functions to represent God to the people and to represent the people to God. Alternate translation: “This is how many men returned from each family of priests”

* **Associated Passages:** Ezra 2:36

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua`); Jedaiah (ID: `person:Jedaiah.4`)

## Ezra 2:36 (#2) (id: 462599)

בְּנֵ֤י יְדַֽעְיָה֙ לְ⁠בֵ֣ית יֵשׁ֔וּעַ תְּשַׁ֥ע מֵא֖וֹת שִׁבְעִ֥ים וּ⁠שְׁלֹשָֽׁה

**"the sons of Jedaiah of the house of Jeshua were 973"**

Here, **sons** means descendants. Alternate translation: “from the descendants of Jedaiah who were descendants of Jeshua, 973 returned”

See: Metaphor

* **Associated Passages:** Ezra 2:36
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua`); Jedaiah (ID: `person:Jedaiah.4`)

## Ezra 2:36 (#3) (id: 462600)

יְדַֽעְיָה֙ \& יֵשׁ֔וּעַ

**"Jedaiah"** \- **"Jeshua"**

**Jedaiah** is the name of a man, and **Jeshua** is the name of one of his male descendants.

See: How to Translate Names

* **Associated Passages:** Ezra 2:36
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua`); Jedaiah (ID: `person:Jedaiah.4`)

## Ezra 2:36 (#4) (id: 462601)

לְ⁠בֵ֣ית יֵשׁ֔וּעַ

**"of the house of Jeshua"**

Here, **house** describes all the people descended from a particular person. The book is describing all of the descendants **of Jeshua** as if they were one household living together. Alternate translation: “who were descendants of Jeshua”

See: Metonymy

* **Associated Passages:** Ezra 2:36
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua`); Jedaiah (ID: `person:Jedaiah.4`)

## Ezra 2:37 (#1) (id: 462602)

בְּנֵ֣י אִמֵּ֔ר

**"The sons of Immer"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Immer”

See: Metaphor

* **Associated Passages:** Ezra 2:37
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Immer (ID: `person:Immer.2`)

## Ezra 2:37 (#2) (id: 462603)

אִמֵּ֔ר

**"Immer"**

**Immer** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:37
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Immer (ID: `person:Immer.2`)

## Ezra 2:38 (#1) (id: 462604)

בְּנֵ֣י פַשְׁח֔וּר

**"The sons of Pashhur"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Pashhur”

See: Metaphor

* **Associated Passages:** Ezra 2:38
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Pashhur (ID: `person:Pashhur`)

## Ezra 2:38 (#2) (id: 462605)

פַשְׁח֔וּר

**"Pashhur"**

**Pashhur** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 2:38
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Pashhur (ID: `person:Pashhur`)

## Ezra 2:39 (#1) (id: 462606)

בְּנֵ֣י חָרִ֔ם

**"The sons of Harim"**

Here, **sons** means descendants. Alternate translation: “From the descendants of Harim”

See: Metaphor

* **Associated Passages:** Ezra 2:39
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Harim (ID: `person:Harim`)

## Ezra 2:39 (#2) (id: 462607)

חָרִ֔ם

**"Harim"**

**Harim** is the name of a man. Alternate translation: “From the descendants of Harim”

See: How to Translate Names

* **Associated Passages:** Ezra 2:39
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Harim (ID: `person:Harim`)

## Ezra 2:40 (#1) (id: 462608)

הַ⁠לְוִיִּ֑ם

**"The Levites"**

**The Levites** were descendants of Levi. They had the special assignment of helping the priests. Alternate translation: “Some Levites also returned” or “Some of the descendants of Levi also returned”

See: Translate Unknowns

* **Associated Passages:** Ezra 2:40
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Hodaviah (ID: `person:Hodaviah.4`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Kadmiel (ID: `person:Kadmiel`); Levi (ID: `person:Levi.3`)

## Ezra 2:40 (#2) (id: 462609)

בְּנֵי־יֵשׁ֧וּעַ וְ⁠קַדְמִיאֵ֛ל לִ⁠בְנֵ֥י הוֹדַוְיָ֖ה

**"the sons of Jeshua and Kadmiel, of the sons of Hodaviah"**

Here, **sons** means descendants. Alternate translation: “from the descendants of Jeshua and Kadmiel, who were descended from Hodaviah”

See: Metaphor

* **Associated Passages:** Ezra 2:40
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hodaviah (ID: `person:Hodaviah.4`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Kadmiel (ID: `person:Kadmiel`); Levi (ID: `person:Levi.3`)

## Ezra 2:40 (#3) (id: 462610)

יֵשׁ֧וּעַ וְ⁠קַדְמִיאֵ֛ל \& הוֹדַוְיָ֖ה

**"Jeshua and Kadmiel,"** \- **"Hodaviah"**

**Jeshua and Kadmiel** are men’s names, and **Hodaviah** is the name of their male ancestor.

See: How to Translate Names

* **Associated Passages:** Ezra 2:40
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hodaviah (ID: `person:Hodaviah.4`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Kadmiel (ID: `person:Kadmiel`); Levi (ID: `person:Levi.3`)

## Ezra 2:41 (#1) (id: 462611)

הַֽ⁠מְשֹׁרְרִ֑ים

**"The"**

**The ones who sang** refers to vocal musicians who led in worship, in processions, and ceremonies, producing music and chants that emphasized and enhanced the occasion.

See: Translate Unknowns

* **Associated Passages:** Ezra 2:41
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`)

## Ezra 2:41 (#2) (id: 462612)

בְּנֵ֣י אָסָ֔ף

**"the sons of Asaph"**

Here, **sons** means descendants. Alternate translation: “who were descendants of Asaph”

See: Metaphor

* **Associated Passages:** Ezra 2:41
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`)

## Ezra 2:41 (#3) (id: 462613)

אָסָ֔ף

**"Asaph"**

**Asaph** is a man’s name.

See: How to Translate Names

* **Associated Passages:** Ezra 2:41
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`)

## Ezra 2:42 (#1) (id: 462614)

בְּנֵ֣י הַ⁠שֹּֽׁעֲרִ֗ים

**"The sons of the gatekeepers"**

Here, **sons** means descendants. Alternate translation: “Some of the descendants of the gatekeepers also returned”

See: Metaphor

* **Associated Passages:** Ezra 2:42
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Shobai (ID: `person:Shobai`); Talmon (ID: `person:Talmon`); Ater (ID: `person:Ater.2`); Hatita (ID: `person:Hatita`); Shallum (ID: `person:Shallum.8`); Akkub (ID: `person:Akkub.2`)

## Ezra 2:42 (#2) (id: 462615)

הַ⁠שֹּֽׁעֲרִ֗ים

**"the gatekeepers"**

Here, **the gatekeepers** refers to people who had been assigned to the gates of the city of Jerusalem and to the gates of the temple within the city. They were responsible for controlling access to these places. They would open and close the gates at times and for reasons set by the authorities.

See: Translate Unknowns

* **Associated Passages:** Ezra 2:42
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Shobai (ID: `person:Shobai`); Talmon (ID: `person:Talmon`); Ater (ID: `person:Ater.2`); Hatita (ID: `person:Hatita`); Shallum (ID: `person:Shallum.8`); Akkub (ID: `person:Akkub.2`)

## Ezra 2:42 (#3) (id: 462616)

בְּנֵי־שַׁלּ֤וּם בְּנֵֽי־אָטֵר֙ בְּנֵי־טַלְמ֣וֹן בְּנֵי־עַקּ֔וּב בְּנֵ֥י חֲטִיטָ֖א בְּנֵ֣י שֹׁבָ֑י

**"the sons of Shallum, the sons of Ater, the sons of Talmon, the sons of Akkub, the sons of Hatita, and the sons of Shobai"**

Here, **sons** means descendants. Alternate translation: “from the descendants of Shallum, Ater, Talmon, Akkub, Hatita, and Shobai”

See: Metaphor

* **Associated Passages:** Ezra 2:42
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Shobai (ID: `person:Shobai`); Talmon (ID: `person:Talmon`); Ater (ID: `person:Ater.2`); Hatita (ID: `person:Hatita`); Shallum (ID: `person:Shallum.8`); Akkub (ID: `person:Akkub.2`)

## Ezra 2:42 (#4) (id: 462617)

שַׁלּ֤וּם \& אָטֵר֙ \& טַלְמ֣וֹן \& עַקּ֔וּב \& חֲטִיטָ֖א \& שֹׁבָ֑י

**"Shallum,"** \- **"Ater,"** \- **"Talmon,"** \- **"Akkub,"** \- **"Hatita,"** \- **"Shobai"**

These are the names of six men.

See: How to Translate Names

* **Associated Passages:** Ezra 2:42
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Shobai (ID: `person:Shobai`); Talmon (ID: `person:Talmon`); Ater (ID: `person:Ater.2`); Hatita (ID: `person:Hatita`); Shallum (ID: `person:Shallum.8`); Akkub (ID: `person:Akkub.2`)

## Ezra 2:43 (#1) (id: 462618)

הַ⁠נְּתִינִ֑ים

**"The temple servants"**

The term **Nethinim** describes servants who worked in the temple. Alternate translation: “The temple servants”

See: Translate Unknowns

* **Associated Passages:** Ezra 2:43
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Hasupha (ID: `person:Hasupha`); Ziha (ID: `person:Ziha`); Tabbaoth (ID: `person:Tabbaoth`)

## Ezra 2:43 (#2) (id: 462619)

הַ⁠נְּתִינִ֑ים

**"The temple servants"**

Alternate translation: “Some of the descendants of the temple servants also returned”

* **Associated Passages:** Ezra 2:43

* **Associated ACAI Concepts:** Hasupha (ID: `person:Hasupha`); Ziha (ID: `person:Ziha`); Tabbaoth (ID: `person:Tabbaoth`)

## Ezra 2:43 (#3) (id: 462620)

בְּנֵי־צִיחָ֥א בְנֵי־חֲשׂוּפָ֖א בְּנֵ֥י טַבָּעֽוֹת

**"the sons of Ziha, the sons of Hasupha, the sons of Tabbaoth"**

Here, **sons** means descendants. Alternate translation: “they were from the descendants of Ziha, Hasupha, Tabbaoth” beginning a series that will continue through [2:54](https://ref.ly/Ezra2:54).

See: Metaphor

* **Associated Passages:** Ezra 2:43
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hasupha (ID: `person:Hasupha`); Ziha (ID: `person:Ziha`); Tabbaoth (ID: `person:Tabbaoth`)

## Ezra 2:43 (#4) (id: 462621)

צִיחָ֥א \& חֲשׂוּפָ֖א \& טַבָּעֽוֹת

**"Ziha,"** \- **"Hasupha,"** \- **"Tabbaoth"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 2:43
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hasupha (ID: `person:Hasupha`); Ziha (ID: `person:Ziha`); Tabbaoth (ID: `person:Tabbaoth`)

## Ezra 2:44 (#1) (id: 462622)

בְּנֵי־קֵרֹ֥ס בְּֽנֵי־סִֽיעֲהָ֖א בְּנֵ֥י פָדֽוֹן

**"the sons of Keros, the sons of Siaha, the sons of Padon"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Keros, Siaha, Padon,”

See: Metaphor

* **Associated Passages:** Ezra 2:44
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Siaha (ID: `person:Siaha`); Keros (ID: `person:Keros`); Padon (ID: `person:Padon`)

## Ezra 2:45 (#1) (id: 462623)

בְּנֵי־לְבָנָ֥ה בְנֵי־חֲגָבָ֖ה בְּנֵ֥י עַקּֽוּב

**"the sons of Lebanah, the sons of Hagabah, the sons of Akkub"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Lebanah, Hagabah, Akkub,”

See: Metaphor

* **Associated Passages:** Ezra 2:45
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Lebanah (ID: `person:Lebanah`); Akkub (ID: `person:Akkub.3`); Hagabah (ID: `person:Hagabah`)

## Ezra 2:46 (#1) (id: 462624)

בְּנֵי־חָגָ֥ב בְּנֵי־שַׁלְמַ֖י בְּנֵ֥י חָנָֽן

**"the sons of Hagab, the sons of Shalmai, and the sons of Hanan"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Hagab, Shalmai, Hanan,”

See: How to Translate Names

* **Associated Passages:** Ezra 2:46
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hagab (ID: `person:Hagab`); Hanan (ID: `person:Hanan.4`); Shalmai (ID: `person:Shalmai`)

## Ezra 2:47 (#1) (id: 462625)

בְּנֵי־גִדֵּ֥ל בְּנֵי־גַ֖חַר בְּנֵ֥י רְאָיָֽה

**"the sons of Giddel, the sons of Gahar, the sons of Reaiah"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Giddel, Gahar, Reaiah,”

See: Metaphor

* **Associated Passages:** Ezra 2:47
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Gahar (ID: `person:Gahar`); Giddel (ID: `person:Giddel`); Reaiah (ID: `person:Reaiah.3`)

## Ezra 2:48 (#1) (id: 462626)

בְּנֵי־רְצִ֥ין בְּנֵי־נְקוֹדָ֖א בְּנֵ֥י גַזָּֽם

**"the sons of Rezin, the sons of Nekoda, the sons of Gazzam"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Rezin, Nekoda, Gazzam,”

See: Metaphor

* **Associated Passages:** Ezra 2:48
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Gazzam (ID: `person:Gazzam`); Nekoda (ID: `person:Nekoda`); Rezin (ID: `person:Rezin.2`)

## Ezra 2:49 (#1) (id: 462627)

בְּנֵי־עֻזָּ֥א בְנֵי־פָסֵ֖חַ בְּנֵ֥י בֵסָֽי

**"the sons of Uzza, the sons of Paseah, the sons of Besai"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Uzza, Paseah, Besai,”

See: Metaphor

* **Associated Passages:** Ezra 2:49
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Paseah (ID: `person:Paseah.2`); Uzza (ID: `person:Uzza.2`); Besai (ID: `person:Besai`)

## Ezra 2:50 (#1) (id: 462628)

בְּנֵי־אַסְנָ֥ה בְנֵי־מְעוּנִ֖ים בְּנֵ֥י נפיסים

**"the sons of Asnah, the sons of Meunim, the sons of Nephusim"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Asnah, Meunim, Nephusim,”

See: Metaphor

* **Associated Passages:** Ezra 2:50
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Meunim (ID: `group:Meunim`); Nephushesim (ID: `group:Nephushesim`); Asnah (ID: `person:Asnah`)

## Ezra 2:51 (#1) (id: 462629)

בְּנֵי־בַקְבּ֥וּק בְּנֵי־חֲקוּפָ֖א בְּנֵ֥י חַרְחֽוּר

**"the sons of Bakbuk, the sons of Hakupha, the sons of Harhur"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Bakbuk, Hakupha, Harhur,”

See: Metaphor

* **Associated Passages:** Ezra 2:51
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hakupha (ID: `person:Hakupha`); Harhur (ID: `person:Harhur`); Bakbuk (ID: `person:Bakbuk`)

## Ezra 2:52 (#1) (id: 462630)

בְּנֵי־בַצְל֥וּת בְּנֵי־מְחִידָ֖א בְּנֵ֥י חַרְשָֽׁא

**"the sons of Bazluth, the sons of Mehida, the sons of Harsha"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Bazluth, Mehida, Harsha,”

See: Metaphor

* **Associated Passages:** Ezra 2:52
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mehida (ID: `person:Mehida`); Harsha (ID: `person:Harsha`); Bazluth (ID: `person:Bazluth`)

## Ezra 2:53 (#1) (id: 462631)

בְּנֵי־בַרְק֥וֹס בְּֽנֵי־סִֽיסְרָ֖א בְּנֵי־תָֽמַח

**"the sons of Barkos, the sons of Sisera, the sons of Temah"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these three men. Alternate translation: “Barkos, Sisera, Temah,”

See: Metaphor

* **Associated Passages:** Ezra 2:53
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Barkos (ID: `person:Barkos`); Sisera (ID: `person:Sisera.2`); Temah (ID: `person:Temah`)

## Ezra 2:54 (#1) (id: 462632)

בְּנֵ֥י נְצִ֖יחַ בְּנֵ֥י חֲטִיפָֽא

**"the sons of Neziah, and the sons of Hatipha"**

Here, **sons** means descendants. If you continue the sentence from [2:43](https://ref.ly/Ezra2:43), you can just list the names of these two men, and end the series in this verse. Alternate translation: “Neziah, and Hatipha”

See: Metaphor

* **Associated Passages:** Ezra 2:54
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Neziah (ID: `person:Neziah`); Hatipha (ID: `person:Hatipha`)

## Ezra 2:55 (#1) (id: 462633)

בְּנֵ֖י עַבְדֵ֣י שְׁלֹמֹ֑ה

**"The sons of the servants of Solomon"**

Here, **sons** means descendants. Alternate translation: “Some of the descendants of the laborers who had worked for the kingdom also returned”

See: Metaphor

* **Associated Passages:** Ezra 2:55
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Sotai (ID: `person:Sotai`); Peruda (ID: `person:Peruda`); Sophereth (ID: `person:Sophereth`); Solomon (ID: `person:Solomon`)

## Ezra 2:55 (#2) (id: 462634)

עַבְדֵ֣י שְׁלֹמֹ֑ה

**"the servants of Solomon"**

By **servants of Solomon**, this list does not mean officials who served in Solomon’s court. Rather, this phrase refers to people whom Solomon first conscripted as laborers. They were descendants of the groups that were living in the land of Canaan before the Israelites occupied it. They and their descendants remained conscripted laborers (slaves) under later kings. Alternate translation: “the laborers that King Solomon first conscripted” or “the laborers who had worked for the kingdom under Solomon”

See: Translate Unknowns

* **Associated Passages:** Ezra 2:55
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Sotai (ID: `person:Sotai`); Peruda (ID: `person:Peruda`); Sophereth (ID: `person:Sophereth`); Solomon (ID: `person:Solomon`)

## Ezra 2:55 (#3) (id: 462635)

בְּנֵי־סֹטַ֥י בְּנֵי־הַ⁠סֹּפֶ֖רֶת בְּנֵ֥י פְרוּדָֽא

**"the sons of Sotai, the sons of Hassophereth, the sons of Peruda"**

Here, **sons** means descendants. Alternate translation: “they were from the descendants of Sotai, Hassophereth, Peruda,” beginning a series that will continue through [2:57](https://ref.ly/Ezra2:57).

See: Metaphor

* **Associated Passages:** Ezra 2:55
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Sotai (ID: `person:Sotai`); Peruda (ID: `person:Peruda`); Sophereth (ID: `person:Sophereth`); Solomon (ID: `person:Solomon`)

## Ezra 2:55 (#4) (id: 462636)

סֹטַ֥י \& הַ⁠סֹּפֶ֖רֶת \& פְרוּדָֽא

**"Sotai,"** \- **"Hassophereth,"** \- **"Peruda"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 2:55
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Sotai (ID: `person:Sotai`); Peruda (ID: `person:Peruda`); Sophereth (ID: `person:Sophereth`); Solomon (ID: `person:Solomon`)

## Ezra 2:56 (#1) (id: 462637)

בְּנֵי־יַעְלָ֥ה בְנֵי־דַרְק֖וֹן בְּנֵ֥י גִדֵּֽל

**"the sons of Jaalah, the sons of Darkon, the sons of Giddel"**

Here, **sons** means descendants. If you continue the sentence from [2:55](https://ref.ly/Ezra2:55), you can just list the names of these three men. Alternate translation: “Jaalah, Darkon, Giddel,”

See: Metaphor

* **Associated Passages:** Ezra 2:56
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Darkon (ID: `person:Darkon`); Giddel (ID: `person:Giddel.2`); Jaalah (ID: `person:Jaalah`)

## Ezra 2:57 (#1) (id: 462638)

בְּנֵ֧י שְׁפַטְיָ֣ה בְנֵֽי־חַטִּ֗יל בְּנֵ֛י פֹּכֶ֥רֶת הַצְּבָיִ֖ים בְּנֵ֥י אָמִֽי

**"the sons of Shephatiah, the sons of Hattil, the sons of Pochereth Hazzebaim, and the sons of Ami"**

Here, **sons** means descendants. If you continue the sentence from [2:55](https://ref.ly/Ezra2:55), you can just list the names of these four men, and end the series in this verse. Alternate translation: “Shephatiah, Hattil, Pochereth Hazzebaim, and Ami”

See: Metaphor

* **Associated Passages:** Ezra 2:57
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Pochereth-hazzebaim (ID: `person:Pochereth-hazzebaim`); Shephatiah (ID: `person:Shephatiah.7`); Hattil (ID: `person:Hattil`); Ami (ID: `person:Ami`)

## Ezra 2:58 (#1) (id: 462639)

כָּ֨ל־הַ⁠נְּתִינִ֔ים וּ⁠בְנֵ֖י עַבְדֵ֣י שְׁלֹמֹ֑ה שְׁלֹ֥שׁ מֵא֖וֹת תִּשְׁעִ֥ים וּ⁠שְׁנָֽיִם

**"All the temple servants and the sons of the servants of Solomon were 392"**

Alternate translation: “Altogether, 392 men returned who were descendants of temple servants or of laborers who had worked for the kingdom of Solomon”

* **Associated Passages:** Ezra 2:58

* **Associated ACAI Concepts:** Solomon (ID: `person:Solomon`)

## Ezra 2:59 (#1) (id: 462640)

וְ⁠אֵ֗לֶּה הָֽ⁠עֹלִים֙

**"And these were the ones who went up"**

As in [2:1](https://ref.ly/Ezra2:1), **went up** means traveled from Babylon back to Judah, since that involved going from a river valley up into the mountains. Alternate translation: “Some others returned to Judah”

See: Idiom

* **Associated Passages:** Ezra 2:59
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tel-harsha (ID: `place:Tel-harsha`); Cherub (ID: `place:Cherub`); Tel-melah (ID: `place:Tel-melah`); Immer (ID: `place:Immer`); Addan (ID: `place:Addan`); Israel (ID: `person:Israel`)

## Ezra 2:59 (#2) (id: 462641)

מִ⁠תֵּ֥ל מֶ֨לַח֙ תֵּ֣ל חַרְשָׁ֔א כְּר֥וּב אַדָּ֖ן אִמֵּ֑ר

**"from Tel Melah, Tel Harsha, Kerub, Addon, and Immer"**

These are the names of five towns in Babylonia. Alternate translation: “who had been living in the towns of Tel Melah, Tel Harsha, Kerub, Addon, and Immer”

See: How to Translate Names

* **Associated Passages:** Ezra 2:59
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tel-harsha (ID: `place:Tel-harsha`); Cherub (ID: `place:Cherub`); Tel-melah (ID: `place:Tel-melah`); Immer (ID: `place:Immer`); Addan (ID: `place:Addan`); Israel (ID: `person:Israel`)

## Ezra 2:59 (#3) (id: 462642)

וְ⁠לֹ֣א יָֽכְל֗וּ לְ⁠הַגִּ֤יד

**"but they were not able to tell"**

Alternate translation: “but they had no records to prove”

* **Associated Passages:** Ezra 2:59

* **Associated ACAI Concepts:** Tel-harsha (ID: `place:Tel-harsha`); Cherub (ID: `place:Cherub`); Tel-melah (ID: `place:Tel-melah`); Immer (ID: `place:Immer`); Addan (ID: `place:Addan`); Israel (ID: `person:Israel`)

## Ezra 2:59 (#4) (id: 462643)

בֵּית־אֲבוֹתָ⁠ם֙ וְ⁠זַרְעָ֔⁠ם אִ֥ם מִ⁠יִּשְׂרָאֵ֖ל הֵֽם

**"the house of their fathers or their seed, whether they were from Israel"**

All three of these phrases, **the house of their fathers**, **their seed**, and **whether they were from Israel**, mean very similar things. They are used together to give clarity and emphasis. The first phrase refers to a person being able to name a well\-known ancestor. The second phrase refers to a person being able to list his ancestors going back to the one that is well\-known. The third phrase summarizes the other two: if a person cannot name an ancestor or recite his ancestry, then he cannot claim to be from Israel. If there would be no distinction between these phrases in your language, you could combine them into one phrase. Alternate translations: “that their ancestors had been Israelites” or “what clan they were from or who their ancestors were, whether they were really Israelites”

See: Parallelism

* **Associated Passages:** Ezra 2:59
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Tel-harsha (ID: `place:Tel-harsha`); Cherub (ID: `place:Cherub`); Tel-melah (ID: `place:Tel-melah`); Immer (ID: `place:Immer`); Addan (ID: `place:Addan`); Israel (ID: `person:Israel`)

## Ezra 2:59 (#5) (id: 462644)

בֵּית־אֲבוֹתָ⁠ם֙

**"the house of their fathers"**

Among the Israelites, the expressions father’s house or **house of their fathers** originally described an extended\-family group. It later came to be used more generally to refer to a larger clan within a tribe. In this expression, the word “house” describes all the people descended from a particular person. The term views all of those descendants as if they were one household living together. Alternate translation: “what clan they were from”

See: Metaphor

* **Associated Passages:** Ezra 2:59
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tel-harsha (ID: `place:Tel-harsha`); Cherub (ID: `place:Cherub`); Tel-melah (ID: `place:Tel-melah`); Immer (ID: `place:Immer`); Addan (ID: `place:Addan`); Israel (ID: `person:Israel`)

## Ezra 2:59 (#6) (id: 462645)

וְ⁠זַרְעָ֔⁠ם

**"or their seed"**

"Here, **seed** means “ancestors.” In the Bible, the term often describes a person’s descendants. But here, ""their seed"" means “whose seed they were,” so the term is describing the ancestors of these people. Alternate translation: “or who their ancestors were”"

See: Idiom

* **Associated Passages:** Ezra 2:59
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tel-harsha (ID: `place:Tel-harsha`); Cherub (ID: `place:Cherub`); Tel-melah (ID: `place:Tel-melah`); Immer (ID: `place:Immer`); Addan (ID: `place:Addan`); Israel (ID: `person:Israel`)

## Ezra 2:60 (#1) (id: 462646)

בְּנֵי־דְלָיָ֥ה בְנֵי־טוֹבִיָּ֖ה בְּנֵ֣י נְקוֹדָ֑א שֵׁ֥שׁ מֵא֖וֹת חֲמִשִּׁ֥ים וּ⁠שְׁנָֽיִם

**"The sons of Delaiah, the sons of Tobiah, and the sons of Nekoda were 652"**

Alternate translation: “In this group were 652 men from the descendants of Delaiah, Tobiah, and Nekoda”

* **Associated Passages:** Ezra 2:60

* **Associated ACAI Concepts:** Nekoda (ID: `person:Nekoda.2`); Delaiah (ID: `person:Delaiah.3`); Tobiah (ID: `person:Tobiah`)

## Ezra 2:60 (#2) (id: 462647)

בְּנֵי־דְלָיָ֥ה בְנֵי־טוֹבִיָּ֖ה בְּנֵ֣י נְקוֹדָ֑א

**"The sons of Delaiah, the sons of Tobiah, and the sons of Nekoda"**

Here, **sons** means descendants. Either the men named here were not recognized clan heads of Israel, or the 652 people could not prove that they were descendants of these men. Alternate translation: “The descendants of Delaiah, Tobiah, and Nekoda”

See: Metaphor

* **Associated Passages:** Ezra 2:60
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Nekoda (ID: `person:Nekoda.2`); Delaiah (ID: `person:Delaiah.3`); Tobiah (ID: `person:Tobiah`)

## Ezra 2:60 (#3) (id: 462648)

דְלָיָ֥ה \& טוֹבִיָּ֖ה \& נְקוֹדָ֑א

**"Delaiah,"** \- **"Tobiah,"** \- **"Nekoda"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 2:60
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Nekoda (ID: `person:Nekoda.2`); Delaiah (ID: `person:Delaiah.3`); Tobiah (ID: `person:Tobiah`)

## Ezra 2:61 (#1) (id: 462649)

וּ⁠מִ⁠בְּנֵי֙ הַ⁠כֹּ֣הֲנִ֔ים

**"And from the sons of the priests"**

Alternate translation: “Some of the men who returned from those towns were descendants of the priests”

* **Associated Passages:** Ezra 2:61

* **Associated ACAI Concepts:** Habaiah (ID: `person:Habaiah`); Hakkoz (ID: `person:Hakkoz.2`); Barzillai (ID: `person:Barzillai.3`); Gileadites (ID: `group:Gilead`); Gilead (ID: `place:Gilead`); Barzillai (ID: `person:Barzillai`)

## Ezra 2:61 (#2) (id: 462650)

וּ⁠מִ⁠בְּנֵי֙ הַ⁠כֹּ֣הֲנִ֔ים

**"And from the sons of the priests"**

Here, **sons** means descendants. Alternate translation: “And descendants of the priests”

See: Metaphor

* **Associated Passages:** Ezra 2:61
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Habaiah (ID: `person:Habaiah`); Hakkoz (ID: `person:Hakkoz.2`); Barzillai (ID: `person:Barzillai.3`); Gileadites (ID: `group:Gilead`); Gilead (ID: `place:Gilead`); Barzillai (ID: `person:Barzillai`)

## Ezra 2:61 (#3) (id: 462651)

בְּנֵ֥י חֳבַיָּ֖ה בְּנֵ֣י הַקּ֑וֹץ בְּנֵ֣י בַרְזִלַּ֗י

**"the sons of Habaiah; the sons of Hakkoz; and the sons of Barzillai"**

Here, **sons** means descendants. Alternate translation: “they were from the descendants of Habaiah, Hakkoz, and Barzillai”

See: Metaphor

* **Associated Passages:** Ezra 2:61
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Habaiah (ID: `person:Habaiah`); Hakkoz (ID: `person:Hakkoz.2`); Barzillai (ID: `person:Barzillai.3`); Gileadites (ID: `group:Gilead`); Gilead (ID: `place:Gilead`); Barzillai (ID: `person:Barzillai`)

## Ezra 2:61 (#4) (id: 462652)

חֳבַיָּ֖ה \& הַקּ֑וֹץ \& בַרְזִלַּ֗י

**"Habaiah;"** \- **"Hakkoz;"** \- **"Barzillai"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 2:61
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Habaiah (ID: `person:Habaiah`); Hakkoz (ID: `person:Hakkoz.2`); Barzillai (ID: `person:Barzillai.3`); Gileadites (ID: `group:Gilead`); Gilead (ID: `place:Gilead`); Barzillai (ID: `person:Barzillai`)

## Ezra 2:61 (#5) (id: 462653)

אֲשֶׁ֣ר לָ֠קַח מִ⁠בְּנ֞וֹת בַּרְזִלַּ֤י הַ⁠גִּלְעָדִי֙ אִשָּׁ֔ה

**"who took a wife from the daughters of Barzillai the Gileadite"**

You could start a new sentence here. Alternate translation: “Barzillai married a woman who was one of the descendants of Barzillai the Gileadite”

* **Associated Passages:** Ezra 2:61

* **Associated ACAI Concepts:** Habaiah (ID: `person:Habaiah`); Hakkoz (ID: `person:Hakkoz.2`); Barzillai (ID: `person:Barzillai.3`); Gileadites (ID: `group:Gilead`); Gilead (ID: `place:Gilead`); Barzillai (ID: `person:Barzillai`)

## Ezra 2:61 (#6) (id: 462654)

מִ⁠בְּנ֞וֹת בַּרְזִלַּ֤י הַ⁠גִּלְעָדִי֙

**"from the daughters of Barzillai the Gileadite"**

Here, **daughters** means female descendants. Alternate translation: “from the descendants of Barzillai the Gileadite”

See: Metaphor

* **Associated Passages:** Ezra 2:61
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Habaiah (ID: `person:Habaiah`); Hakkoz (ID: `person:Hakkoz.2`); Barzillai (ID: `person:Barzillai.3`); Gileadites (ID: `group:Gilead`); Gilead (ID: `place:Gilead`); Barzillai (ID: `person:Barzillai`)

## Ezra 2:61 (#7) (id: 462655)

וַ⁠יִּקָּרֵ֖א עַל־שְׁמָֽ⁠ם

**"so he was called by their name"**

Alternate translation: “and he took the name of her clan as his own name”

* **Associated Passages:** Ezra 2:61

* **Associated ACAI Concepts:** Habaiah (ID: `person:Habaiah`); Hakkoz (ID: `person:Hakkoz.2`); Barzillai (ID: `person:Barzillai.3`); Gileadites (ID: `group:Gilead`); Gilead (ID: `place:Gilead`); Barzillai (ID: `person:Barzillai`)

## Ezra 2:62 (#1) (id: 462656)

אֵ֗לֶּה בִּקְשׁ֧וּ כְתָבָ֛⁠ם הַ⁠מִּתְיַחְשִׂ֖ים

**"These ones sought their record among the ones who were enrolled by genealogy"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “They searched for a mention of their names in the lists of people who were descendants of the priests”

See: Active or Passive

* **Associated Passages:** Ezra 2:62
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Priesthood (ID: `keyterm:Priesthood`); Defile (ID: `keyterm:Stain`)

## Ezra 2:62 (#2) (id: 462657)

אֵ֗לֶּה בִּקְשׁ֧וּ כְתָבָ֛⁠ם הַ⁠מִּתְיַחְשִׂ֖ים וְ⁠לֹ֣א נִמְצָ֑אוּ

**"These ones sought their record among the ones who were enrolled by genealogy, but they were not found"**

There are two possible meanings for what these people **sought** in the **record**. (1\) They were seeking their own names from among a list of descendants from Israelite clan leaders. See the UST. (2\) They were seeking the names of the clan leaders listed in verse 61 from among a list of names of Israelite clan leaders. Alternate translation: “The people in that group searched in the documents that had the names of the ancestors of all the clans, but they did not find the names of these men”

See: Active or Passive

* **Associated Passages:** Ezra 2:62
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Priesthood (ID: `keyterm:Priesthood`); Defile (ID: `keyterm:Stain`)

## Ezra 2:62 (#3) (id: 462658)

וְ⁠לֹ֣א נִמְצָ֑אוּ

**"but they were not found"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “but they were not able to find any mention of their names there”

See: Active or Passive

* **Associated Passages:** Ezra 2:62
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Priesthood (ID: `keyterm:Priesthood`); Defile (ID: `keyterm:Stain`)

## Ezra 2:62 (#4) (id: 462659)

וַֽ⁠יְגֹאֲל֖וּ מִן־הַ⁠כְּהֻנָּֽה

**"And they were desecrated from the priesthood"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “So the Jewish officials did not allow them to be priests”

See: Active or Passive

* **Associated Passages:** Ezra 2:62
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Priesthood (ID: `keyterm:Priesthood`); Defile (ID: `keyterm:Stain`)

## Ezra 2:62 (#5) (id: 462660)

וַֽ⁠יְגֹאֲל֖וּ מִן־הַ⁠כְּהֻנָּֽה

**"And they were desecrated from the priesthood"**

The abstract noun **priesthood** refers to the work that a priest does and the status that a priest holds as a representative of God to the people. If it would be helpful in your language, you could translate the idea behind this term with an equivalent phrase. Alternate translation: “And so they were not allowed to serve as priests”

See: Abstract Nouns

* **Associated Passages:** Ezra 2:62
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Priesthood (ID: `keyterm:Priesthood`); Defile (ID: `keyterm:Stain`)

## Ezra 2:63 (#1) (id: 462661)

הַ⁠תִּרְשָׁ֨תָא֙

**"the Tirshatha"**

**Tirshatha** was the formal Persian title for the governor of a province, so this would be referring to Sheshbazzar, who may be the same person as Zerubbabel. If it would be helpful in your language, you could translate the title with a word like “governor,” or you could state the title and then explain it. Alternate translation: “the Tirshatha, that is, the governor”

See: Translate Unknowns

* **Associated Passages:** Ezra 2:63
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)


## Ezra 2:63 (#2) (id: 462662)

וַ⁠יֹּ֤אמֶר \& לָ⁠הֶ֔ם אֲשֶׁ֥ר לֹא־יֹאכְל֖וּ מִ⁠קֹּ֣דֶשׁ הַ⁠קֳּדָשִׁ֑ים

**"And"** \- **"said to them that they must not eat from the holiest holy food"**

The phrase **the holiest holy food** refers to the most sacred part of the food offerings, which was reserved for only the priests to eat. Alternate translation: “told them that they would not be allowed to eat the most sacred food”

See: Idiom

* **Associated Passages:** Ezra 2:63
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 2:63 (#3) (id: 462663)

עַ֛ד עֲמֹ֥ד כֹּהֵ֖ן

**"until a priest stood"**

Here, **stood** is a figurative way of saying that a person had assumed the duties of his office. Here, **priest** means specifically the high priest, since he was the one who would have possession of the objects that this verse describes. Alternate translation: “until the high priest was once again on duty”

See: Metaphor

* **Associated Passages:** Ezra 2:63
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## Ezra 2:63 (#4) (id: 462664)

לְ⁠אוּרִ֥ים וּ⁠לְ⁠תֻמִּֽים

**"with Urim and with Thummim"**

This was a pair of objects that the high priest kept in the breastpiece of his robe. It is unknown exactly what type of objects they were, but they were used to answer yes\-or\-no questions and to decide between two alternatives. If the priest, without looking, pulled out the Urim, that meant one answer, while if he pulled out the Thummim, that meant the other answer. If it would be helpful in your language, you could describe these objects by their function rather than by their names. Alternate translation: “and could use the sacred objects to decide their cases”

See: Translate Unknowns

* **Associated Passages:** Ezra 2:63
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)


## Ezra 2:64 (#1) (id: 462665)

כָּל־הַ⁠קָּהָ֖ל כְּ⁠אֶחָ֑ד אַרְבַּ֣ע רִבּ֔וֹא אַלְפַּ֖יִם שְׁלֹשׁ־מֵא֥וֹת שִׁשִּֽׁים

**"All the assembly, as one, was 42, 360"**

The expression **as one** means all together. Alternate translation: “The total number of men who returned was 42,360”

See: Idiom

* **Associated Passages:** Ezra 2:64
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 2:65 (#1) (id: 462666)

מִ֠⁠לְּ⁠בַד עַבְדֵי⁠הֶ֤ם וְ⁠אַמְהֹֽתֵי⁠הֶם֙ אֵ֔לֶּה שִׁבְעַ֣ת אֲלָפִ֔ים שְׁלֹ֥שׁ מֵא֖וֹת שְׁלֹשִׁ֣ים וְ⁠שִׁבְעָ֑ה

**"apart from their male servants and their female servants, these were 7, 337"**

Alternate translation: “not counting their 7,337 male and female servants”

* **Associated Passages:** Ezra 2:65


## Ezra 2:65 (#2) (id: 462667)

וְ⁠לָ⁠הֶ֛ם מְשֹׁרְרִ֥ים וּֽ⁠מְשֹׁרְר֖וֹת מָאתָֽיִם

**"and for them were 200 men who sang and women who sang"**

This group of those **who sang** is a different group from those in [2:41](https://ref.ly/Ezra2:41). Those were Levites who sang in connection with Israel’s worship. The group here is a class of servants who were employed to provide music for public and private events. Alternate translation: “and the 200 male and female singers whom they employed”

See: Translate Unknowns

* **Associated Passages:** Ezra 2:65
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)


## Ezra 2:66 (#1) (id: 462668)

סוּסֵי⁠הֶ֕ם שְׁבַ֥ע מֵא֖וֹת שְׁלֹשִׁ֣ים וְ⁠שִׁשָּׁ֑ה פִּרְדֵי⁠הֶ֕ם מָאתַ֖יִם אַרְבָּעִ֥ים וַ⁠חֲמִשָּֽׁה

**"Their horses were 736\. Their mules were 245"**

Alternate translation, beginning a sentence that will continue through the next verse: “The group also brought back with them 736 horses, 245 mules”

* **Associated Passages:** Ezra 2:66

* **Associated ACAI Concepts:** Mule (ID: `fauna:Mule`); Horse (ID: `fauna:Horse`)

## Ezra 2:67 (#1) (id: 462669)

גְּמַ֨לֵּי⁠הֶ֔ם אַרְבַּ֥ע מֵא֖וֹת שְׁלֹשִׁ֣ים וַ⁠חֲמִשָּׁ֑ה חֲמֹרִ֕ים שֵׁ֣שֶׁת אֲלָפִ֔ים שְׁבַ֥ע מֵא֖וֹת וְ⁠עֶשְׂרִֽים

**"Their camels were 435\. Their donkeys were 6, 720"**

Alternate translation, concluding the sentence from the previous verse: “435 camels, and 6,720 donkeys”

* **Associated Passages:** Ezra 2:67

* **Associated ACAI Concepts:** Camel (ID: `fauna:Camel`); Donkey (ID: `fauna:Donkey`)

## Ezra 2:68 (#1) (id: 462670)

וּ⁠מֵ⁠רָאשֵׁי֙

**"And"** \- **"some from the heads of"**

The word **and** introduces a new event in the story. Use the most natural way in your language to introduce this new event.

See: Introduction of a New Event

* **Associated Passages:** Ezra 2:68
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 2:68 (#2) (id: 462671)

וּ⁠מֵ⁠רָאשֵׁי֙ הָֽ⁠אָב֔וֹת

**"And"** \- **"some from the heads of the fathers"**

Here, **heads of the fathers** is an abbreviated way of saying the heads of father’s houses. The full expression “house of their fathers” was used in [2:59](https://ref.ly/Ezra2:59). See how you translated it there, and review the note there if that would be helpful. Alternate translation: “some of the clan leaders”

See: Ellipsis

* **Associated Passages:** Ezra 2:68
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 2:68 (#3) (id: 462672)

וּ⁠מֵ⁠רָאשֵׁי֙

**"And"** \- **"some from the heads of"**

Here, **heads** is a figurative way of saying leaders. Alternate translation: “And … leaders of”

See: Metaphor

* **Associated Passages:** Ezra 2:68
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 2:68 (#4) (id: 462673)

לְ⁠בֵ֥ית יְהוָ֖ה אֲשֶׁ֣ר בִּ⁠ירוּשָׁלִָ֑ם \& לְ⁠בֵ֣ית הָֽ⁠אֱלֹהִ֔ים

**"to the house of Yahweh which is in Jerusalem,"** \- **"for the house of God"**

As in [1:3–4](https://ref.ly/Ezra1:3-Ezra1:4), the expressions **house of Yahweh** and **house of God** both refer to the same temple. The book speaks of this temple as if it would be a house in which God lived, since God’s presence would be there. Alternate translation: “the temple of Yahweh in Jerusalem … the temple of God”

See: Metaphor

* **Associated Passages:** Ezra 2:68
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 2:68 (#5) (id: 462674)

לְ⁠הַעֲמִיד֖⁠וֹ עַל־מְכוֹנֽ⁠וֹ

**"to cause it to stand upon its place"**

This expression indicates that these gifts were given towards the costs of rebuilding the temple on its former site. The expression envisions the rebuilt temple as like a living thing that would **stand** in that location. Alternate translation: “to rebuild it on its former site”

See: Personification

* **Associated Passages:** Ezra 2:68
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 2:69 (#1) (id: 462675)

לְ⁠אוֹצַ֣ר הַ⁠מְּלָאכָה֒

**"to the treasury of the work"**

**The work** means the project of rebuilding the temple. The **treasury** was where all of the money would be kept safely until it was needed. Alternate translation: “to the fund for rebuilding the temple”

* **Associated Passages:** Ezra 2:69

* **Associated ACAI Concepts:** Daric (ID: `realia:Daric`); Strength (ID: `keyterm:Strength`); Tunic (ID: `realia:Tunic`); Mina (ID: `realia:Mina`); Money (ID: `realia:Money`)

## Ezra 2:69 (#2) (id: 462676)

זָהָ֗ב דַּרְכְּמוֹנִים֙ שֵׁשׁ־רִבֹּ֣אות וָ⁠אֶ֔לֶף

**"61000 gold darics"**

In ancient times, **gold darics** each weighed about 8 or 8\.5 grams, or about a quarter of an ounce. You could try to express this in terms of modern money values, but if you did, that could cause your Bible translation to become outdated and inaccurate, since those values can change over time. Instead, you might say something general like “61,000 gold coins,” or give the equivalent weight, or use the biblical term in the text and give the weight in a note.

See: Biblical Money

* **Associated Passages:** Ezra 2:69
* **Associated Articles:** Biblical Money (ID: `33372@UWTranslationManual`)

* **Associated ACAI Concepts:** Daric (ID: `realia:Daric`); Strength (ID: `keyterm:Strength`); Tunic (ID: `realia:Tunic`); Mina (ID: `realia:Mina`); Money (ID: `realia:Money`)

## Ezra 2:69 (#3) (id: 462677)

וְ⁠כֶ֕סֶף מָנִ֖ים חֲמֵ֣שֶׁת אֲלָפִ֑ים

**"and 5000 silver minas"**

In ancient times, **silver minas** each weighed about half a kilogram, or about 1\.25 pounds. However, as in the case of darics, it would probably be best to say something general like “5,000 silver bars,” or give the equivalent weight, or use the biblical term and give the equivalent weight in a footnote.

See: Biblical Money

* **Associated Passages:** Ezra 2:69
* **Associated Articles:** Biblical Money (ID: `33372@UWTranslationManual`)

* **Associated ACAI Concepts:** Daric (ID: `realia:Daric`); Strength (ID: `keyterm:Strength`); Tunic (ID: `realia:Tunic`); Mina (ID: `realia:Mina`); Money (ID: `realia:Money`)

## Ezra 2:69 (#4) (id: 462678)

וְ⁠כָתְנֹ֥ת כֹּהֲנִ֖ים מֵאָֽה

**"and 100 tunics of the priests"**

**Tunics** were the special garments that priests would wear while performing their duties in the temple. Alternate translation: “100 special garments for the priests to wear”

See: Translate Unknowns

* **Associated Passages:** Ezra 2:69
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Daric (ID: `realia:Daric`); Strength (ID: `keyterm:Strength`); Tunic (ID: `realia:Tunic`); Mina (ID: `realia:Mina`); Money (ID: `realia:Money`)

## Ezra 2:70 (#1) (id: 462679)

וַ⁠יֵּשְׁב֣וּ הַ⁠כֹּהֲנִ֣ים וְ֠⁠הַ⁠לְוִיִּם וּֽ⁠מִן־הָ⁠עָ֞ם וְ⁠הַ⁠מְשֹׁרְרִ֧ים וְ⁠הַ⁠שּׁוֹעֲרִ֛ים וְ⁠הַ⁠נְּתִינִ֖ים בְּ⁠עָרֵי⁠הֶ֑ם וְ⁠כָל־יִשְׂרָאֵ֖ל בְּ⁠עָרֵי⁠הֶֽם

**"So the priests, and the Levites, and some from the people, and the singers, and the gatekeepers, and the temple servants dwelt in their cities. So all Israel was in their cities"**

This is a summary conclusion to the list. See how you translated each of these terms within the list: priests in v. [36](https://ref.ly/Ezra2:36), Levites in v. [40](https://ref.ly/Ezra2:40), singers in v. [41](https://ref.ly/Ezra2:41), gatekeepers in v. [42](https://ref.ly/Ezra2:42), and temple servants in v. [43](https://ref.ly/Ezra2:43). Review the notes to each of these verses if that would be helpful. Alternate translation: “So the priests, Levites, other Israelites, singers, gatekeepers, and temple servants returned to Judah and settled in the places where their ancestors had formerly lived”

* **Associated Passages:** Ezra 2:70

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 2:70 (#2) (id: 462680)

וְ⁠כָל־יִשְׂרָאֵ֖ל בְּ⁠עָרֵי⁠הֶֽם

**"So all Israel was in their cities"**

This sentence summarizes the previous sentence, which was itself a summary of the chapter. It shows the result of the journey, that the return of the Israelites was accomplished successfully. You could include this meaning with the previous sentence if stating it separately might be confusing for your readers because of the repetition. Its meaning is included in the alternate translation suggested for the previous sentence.

See: Parallelism

* **Associated Passages:** Ezra 2:70
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 3:1 (#1) (id: 462681)

וַ⁠יִּגַּע֙

**"Then"** \- **"came"**

The word **then** indicates that the events the story will now relate came after the event it has just described. If it would be helpful in your language, you could show this relationship by using a fuller phrase. Alternate translation: “After this group had returned to Judah”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:1 (#2) (id: 462682)

וַ⁠יִּגַּע֙ הַ⁠חֹ֣דֶשׁ הַ⁠שְּׁבִיעִ֔י

**"Then the seventh month came"**

This phrase refers to the beginning of the seventh month in the Jewish religious calendar, which is the first month in the Jewish civic calendar. The phrase is actually giving the reason why the people gathered in Jerusalem at this time. It was the start of a new year, and the leadership had decided to resume community worship, including daily sacrifices, as of the beginning of that year, as [3:6](https://ref.ly/Ezra3:6) indicates. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “At the start of the seventh month, because it was the beginning of a new civic year”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:1 (#3) (id: 462683)

הַ⁠חֹ֣דֶשׁ הַ⁠שְּׁבִיעִ֔י

**"the seventh month"**

Alternate translation: “month seven”

See: Ordinal Numbers

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:1 (#4) (id: 462684)

הַ⁠חֹ֣דֶשׁ הַ⁠שְּׁבִיעִ֔י

**"the seventh month"**

You could convert the Hebrew **month** into an equivalent on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the equivalency will be different every year and the translation will not be entirely accurate. So you may just want to use the number of the Hebrew month.

See: Hebrew Months

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:1 (#5) (id: 462685)

וּ⁠בְנֵ֥י יִשְׂרָאֵ֖ל בֶּ⁠עָרִ֑ים \& וַ⁠יֵּאָסְפ֥וּ הָ⁠עָ֛ם \& אֶל־יְרוּשָׁלִָֽם

**"and the sons of Israel were in their cities. And the people were gathered together"** \- **"to Jerusalem"**

Here the book repeats some background information to remind readers why the Israelites were not all in Jerusalem already and would have had to come there from various places. Alternate translation: “the Israelites came from the different places where they had settled and they gathered … in Jerusalem”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:1 (#6) (id: 462686)

וּ⁠בְנֵ֥י יִשְׂרָאֵ֖ל

**"and the sons of Israel"**

Here, **sons** means descendants. The book is envisioning all of the Israelites as descendants of the patriarch Jacob, who was also known as Israel. Alternate translation: “the Israelites”

See: Metaphor

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:1 (#7) (id: 462687)

וַ⁠יֵּאָסְפ֥וּ הָ⁠עָ֛ם

**"And the people were gathered together"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “And they gathered together”

See: Active or Passive

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:1 (#8) (id: 462688)

כְּ⁠אִ֥ישׁ אֶחָ֖ד

**"as one man"**

**As one man** is an idiom that means they gathered as if they were a single person, that is, all in one place at the same time for the same purpose. Alternate translation: “as if they were a single person”

See: Idiom

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 3:2 (#1) (id: 462689)

וַ⁠יָּקָם֩

**"Then arose"**

The word **then** indicates that the events the story will now relate came after the event it has just described. If it would be helpful in your language, you could show this relationship by using a fuller phrase. Alternate translation: “Once everyone had gathered, arose”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#2) (id: 462690)

וַ⁠יָּקָם֩

**"Then arose"**

In this context, the word **arose** means they took action to get an enterprise under way. It does not indicate that these leaders had been sitting or lying down and that they stood up. Alternate translation: “Then took action” or “Then carried out their plans”

See: Idiom

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#3) (id: 462691)

יֵשׁ֨וּעַ בֶּן־יֽוֹצָדָ֜ק

**"Jeshua the son of Jozadak"**

**Jeshua** is the name of a man. See how you translated it in [2:2](https://ref.ly/Ezra2:2). **Jozadak** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#4) (id: 462692)

וְ⁠אֶחָ֣י⁠ו הַ⁠כֹּהֲנִ֗ים

**"and his brothers the priests"**

Here, **brothers** is likely a figurative way of saying fellow priests, although it is possible that some of the biological brothers of Jeshua were included in this group. Jeshua himself was the high priest at this time. Alternate translation: “his fellow priests”

See: Metaphor

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#5) (id: 462693)

וּ⁠זְרֻבָּבֶ֤ל בֶּן־שְׁאַלְתִּיאֵל֙

**"and Zerubbabel the son of Shealtiel"**

**Zerubbabel** is the name of a man. See how you translated it in [2:2](https://ref.ly/Ezra2:2). **Shealtiel** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#6) (id: 462694)

וְ⁠אֶחָ֣י⁠ו

**"and his brothers"**

Here, **brothers** appears to be a figurative way of saying fellow leaders. The other seven men listed in [2:2](https://ref.ly/Ezra2:2) as leaders of the group that returned from exile may be particularly in view. Alternate translation: “and his fellow leaders”

See: Metaphor

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#7) (id: 462695)

וַ⁠יִּבְנ֕וּ אֶת־מִזְבַּ֖ח אֱלֹהֵ֣י יִשְׂרָאֵ֑ל לְ⁠הַעֲל֤וֹת עָלָי⁠ו֙ עֹל֔וֹת

**"and they built the altar of the God of Israel to offer up burnt up offerings on it"**

This was a way of resuming regular community worship even before construction began on the new temple. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “and they built a new altar to the God of Israel so that the priests could start offering sacrifices right away on behalf of the community”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#8) (id: 462696)

עֹל֔וֹת

**"burnt up offerings"**

The book assumes that readers will know that this means whole burnt offerings, that is, sacrifices that were burned up completely on the altar. These were offered to express a desire to be in good standing with God through the complete sacrifice of something valuable and also through the creation of an aroma, the smell of roasting meat, that was considered to be pleasing to God. If your language has a special term for such offerings, you can use it in your translation. Alternate translation: “whole burnt offerings”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#9) (id: 462697)

כַּ⁠כָּת֕וּב בְּ⁠תוֹרַ֖ת מֹשֶׁ֥ה

**"as is written in the law of Moses"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “as God had commanded the people of Israel to do through Moses”

See: Active or Passive

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#10) (id: 462698)

מֹשֶׁ֥ה אִישׁ־הָ⁠אֱלֹהִֽים

**"Moses, the man of God"**

Here the book repeats some background information to remind readers who Moses was. Alternate translation: “Moses, that godly man” or “Moses, that man who knew God well”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:2 (#11) (id: 462699)

מֹשֶׁ֥ה

**"Moses"**

**Moses** is the name of a man. It occurs many times throughout the book, and it will be helpful to your readers if you translate it consistently each time.

See: How to Translate Names

* **Associated Passages:** Ezra 3:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jehozadak (ID: `person:Jehozadak`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Offering (ID: `keyterm:Offering`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:3 (#1) (id: 462700)

וַ⁠יָּכִ֤ינוּ הַ⁠מִּזְבֵּ֨חַ֙ עַל־מְכ֣וֹנֹתָ֔י⁠ו כִּ֚י בְּ⁠אֵימָ֣ה עֲלֵי⁠הֶ֔ם מֵ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"And they set up the altar on its foundation, for dread was on them because of the peoples of the lands"**

If it would be helpful in your language, you could reverse the order of these phrases since the second phrase gives the reason for the action that the first phrase describes. Alternate translation: “Because they were afraid of the people from other groups who lived around them, thinking they might attack them, they wanted to ask God for help and protection, so they made rebuilding the altar their first priority” or “Because they were afraid of the people from other groups who lived around them, thinking they might try to stop them from rebuilding the temple, they wanted to get started on rebuilding as quickly as possible, and so they began right away with the altar”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 3:3
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Terror (ID: `keyterm:Terror`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:3 (#2) (id: 462701)

וַ⁠יָּכִ֤ינוּ הַ⁠מִּזְבֵּ֨חַ֙ עַל־מְכ֣וֹנֹתָ֔י⁠ו

**"And they set up the altar on its foundation"**

This phrase likely means that the priests and leaders built this new altar right at the place where the altar had been located within the former temple. If it would be helpful in your language, you could say that as an alternate translation.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Terror (ID: `keyterm:Terror`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:3 (#3) (id: 462702)

כִּ֚י בְּ⁠אֵימָ֣ה עֲלֵי⁠הֶ֔ם מֵ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"for dread was on them because of the peoples of the lands"**

The book does not say specifically why the Israelites were afraid of the people from other groups who lived around them. Two possibilities are that they were afraid these people might attack them or that they might try to keep them from rebuilding the temple, in both cases to stop them from re\-establishing themselves in their former homeland. If it would be helpful to your readers, you could suggest one of these explanations explicitly. Alternate translation: “because they were afraid that the people from other groups who lived around them might attack them, they wanted to ask God for help and protection” or “because they were afraid that the people from other groups who lived around them might try to stop them from rebuilding the temple, they wanted to get started on rebuilding as quickly as possible”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Terror (ID: `keyterm:Terror`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:3 (#4) (id: 462703)

בְּ⁠אֵימָ֣ה עֲלֵי⁠הֶ֔ם

**"dread was on them"**

Here the book uses a spatial metaphor of **dread** or fear landing **on** or resting **on** the Israelites to express that the Israelites felt that emotion. Alternate translation: “they were very afraid”

See: Metaphor

* **Associated Passages:** Ezra 3:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Terror (ID: `keyterm:Terror`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:3 (#5) (id: 462704)

מֵ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"because of the peoples of the lands"**

This was the expression that the Jews used to refer to members of other people groups, including some that the Assyrians had brought into the area who were originally from other places (**lands**) but who were now living in and around the province of Judah. Alternate translation: “people from other groups”

See: Idiom

* **Associated Passages:** Ezra 3:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Terror (ID: `keyterm:Terror`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:3 (#6) (id: 462705)

וַיַּעֲל֨וּ עָלָ֤י⁠ו עֹלוֹת֙ לַֽ⁠יהוָ֔ה עֹל֖וֹת לַ⁠בֹּ֥קֶר וְ⁠לָ⁠עָֽרֶב

**"Then they offered up on it burnt up offerings to Yahweh, burnt up offerings at the morning and at the evening"**

The word **then** at the beginning of this sentence indicates that the activity it describes took place after the events the story has been describing. Specifically, this was a continuous activity, not something the leaders did just on that occasion. If it would be helpful in your language, you could show this relationship by using a phrase such as “from that time on.” Alternate translation: “From that time on, the priests offered sacrifices to Yahweh on this altar every morning and every evening”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:3
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Terror (ID: `keyterm:Terror`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:3 (#7) (id: 462706)

וַיַּעֲל֨וּ עָלָ֤י⁠ו עֹלוֹת֙ לַֽ⁠יהוָ֔ה עֹל֖וֹת לַ⁠בֹּ֥קֶר וְ⁠לָ⁠עָֽרֶב

**"Then they offered up on it burnt up offerings to Yahweh, burnt up offerings at the morning and at the evening"**

As [3:2](https://ref.ly/Ezra3:2) notes, God had commanded these daily **offerings** in the law of Moses. By resuming them, the leaders and priests were restoring the regular worship life of the community. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “From that time on, the priests offered sacrifices to Yahweh on this altar every morning and every evening. By doing that, they restored the regular worship life of the community”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Terror (ID: `keyterm:Terror`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Altar (ID: `realia:Altar`)

## Ezra 3:4 (#1) (id: 462707)

וַֽ⁠יַּעֲשׂ֛וּ

**"Then they performed"**

The word **Then** indicates that the events the story will now recount came after the event it has just related. If it would be helpful in your language, you could show this relationship by using an equivalent phrase. The next event is specifically the Festival of Tabernacles, which the law of Moses said to observe from the fifteenth through the twenty\-second days of the seventh month. So that event would have taken place about two weeks after the Israelites first gathered in Jerusalem. Alternate translation: “Later that month”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:4
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Feast of Tabernacles (ID: `keyterm:FeastOfTabernacles`); Offering (ID: `keyterm:Offering`)

## Ezra 3:4 (#2) (id: 462708)

וַֽ⁠יַּעֲשׂ֛וּ אֶת־חַ֥ג הַ⁠סֻּכּ֖וֹת

**"Then they performed the Festival of Huts"**

This phrase describes the Israelites celebrating an observance that is also known as the Festival of Tabernacles. The purpose of this festival was to remind the Israelites how Yahweh had cared for their ancestors when they lived in temporary shelters as they traveled through the wilderness from Egypt to Canaan. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “The Israelites celebrated the Festival of Tabernacles to remember how God had taken care of their ancestors when they had only temporary shelters to live in as they traveled through the wilderness from Egypt to Canaan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Feast of Tabernacles (ID: `keyterm:FeastOfTabernacles`); Offering (ID: `keyterm:Offering`)

## Ezra 3:4 (#3) (id: 462709)

כַּ⁠כָּת֑וּב

**"as is written"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “as God had commanded the people of Israel to do through Moses”

See: Active or Passive

* **Associated Passages:** Ezra 3:4
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Feast of Tabernacles (ID: `keyterm:FeastOfTabernacles`); Offering (ID: `keyterm:Offering`)

## Ezra 3:4 (#4) (id: 462710)

וְ⁠עֹלַ֨ת י֤וֹם בְּ⁠יוֹם֙ בְּ⁠מִסְפָּ֔ר כְּ⁠מִשְׁפַּ֖ט דְּבַר־י֥וֹם בְּ⁠יוֹמֽ⁠וֹ

**"with a burnt up offering day by day, by number according to the ordinance of the matter of the day, on its day"**

These expressions refer to the way the law of Moses commanded the Israelites to offer a different number of sacrifices on each of the eight days of this festival. Alternate translation: “they offered burnt offerings every day, and on each day they offered the number of sacrifices that the law specified for that day”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Feast of Tabernacles (ID: `keyterm:FeastOfTabernacles`); Offering (ID: `keyterm:Offering`)

## Ezra 3:5 (#1) (id: 462711)

וְ⁠אַחֲרֵי כֵ֞ן

**"And after this was"**

This phrase indicates that the activity it describes took place after the one it has just described. If it would be helpful in your language, you could show this relationship by using a phrase such as “And from that time on.”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:5
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Appointed Feast (ID: `keyterm:AppointedFeast`); New Moon Festival (ID: `keyterm:NewMoonFestival`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`)

## Ezra 3:5 (#2) (id: 462712)

עֹלַ֤ת תָּמִיד֙ וְ⁠לֶ֣⁠חֳדָשִׁ֔ים וּ⁠לְ⁠כָל־מוֹעֲדֵ֥י יְהוָ֖ה הַ⁠מְקֻדָּשִׁ֑ים וּ⁠לְ⁠כֹ֛ל מִתְנַדֵּ֥ב נְדָבָ֖ה לַ⁠יהוָֽה

**"a continual burnt up offering, and for the new moons, and for all the consecrated appointed times of Yahweh, and for everyone who freely offered a freewill offering to Yahweh"**

Here the book leaves out some of the words that a sentence would ordinarily need in order to be complete. The meaning seems to be that once the daily morning and evening sacrifices had been re\-instituted, the less frequent sacrifices were resumed as well, including those for the new moon, for the annual festivals, and on the occasion of freewill offerings. You could say that explicitly if it would be helpful to your readers. Alternate translation: “the priests offered sacrifices every morning and evening, and they also offered sacrifices for the new moon festivals, for the festivals that Yahweh had commanded them to observe each year, and whenever anyone freely offered an animal to Yahweh”

See: Ellipsis

* **Associated Passages:** Ezra 3:5
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Appointed Feast (ID: `keyterm:AppointedFeast`); New Moon Festival (ID: `keyterm:NewMoonFestival`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`)

## Ezra 3:6 (#1) (id: 462713)

מִ⁠יּ֤וֹם אֶחָד֙

**"From day one"**

The Hebrew uses a cardinal number here, **one**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, **first**, in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of days, you can do that here in your translation. Alternate translation: “Starting on the first day”

See: Ordinal Numbers

* **Associated Passages:** Ezra 3:6
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 3:6 (#2) (id: 462714)

הֵחֵ֕לּוּ לְ⁠הַעֲל֥וֹת עֹל֖וֹת לַ⁠יהוָ֑ה וְ⁠הֵיכַ֥ל יְהוָ֖ה לֹ֥א יֻסָּֽד

**"they began to offer up burnt up offerings to Yahweh, but the temple of Yahweh had not been founded"**

This sentence draws a contrast between the conditions under which readers would have expected sacrifices to resume and the conditions under which they actually did resume. You could use a connecting phrase such as “even though” between the two clauses to indicate this contrast. Alternate translation: “they resumed offering sacrifices to Yahweh, even though they had not yet rebuilt the temple of Yahweh”

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 3:6
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 3:6 (#3) (id: 462715)

וְ⁠הֵיכַ֥ל יְהוָ֖ה לֹ֥א יֻסָּֽד

**"but the temple of Yahweh had not been founded"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “the Jewish leaders had not yet rebuilt the temple of Yahweh”

See: Active or Passive

* **Associated Passages:** Ezra 3:6
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 3:6 (#4) (id: 462716)

וְ⁠הֵיכַ֥ל יְהוָ֖ה לֹ֥א יֻסָּֽד

**"but the temple of Yahweh had not been founded"**

This phrase likely means specifically that no one had laid a foundation yet for the new temple. That would have been the first step in construction. However, this seems to be a figurative way of describing the entire process of building the temple by referring to one part of that process, its first step. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “the Jewish leaders had not yet rebuilt the temple of Yahweh”

See: Synecdoche

* **Associated Passages:** Ezra 3:6
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 3:7 (#1) (id: 462717)

וַ⁠יִּ֨תְּנוּ

**"And they gave"**

Here, the word **and** indicates that the sentence it introduces will explain what people did as a result of the situation that the previous sentence described. Alternate translation: “And so they gave”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#2) (id: 462718)

וַ⁠יִּ֨תְּנוּ־כֶ֔סֶף לַ⁠חֹצְבִ֖ים וְ⁠לֶ⁠חָרָשִׁ֑ים

**"And they gave silver to the masons and to the carpenters"**

The implication is that the Jewish leaders did this, and the rest of the things the verse describes, in order to start construction on the new temple. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “In order to start building a new temple, the Jewish leaders hired masons and carpenters”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#3) (id: 462719)

וַ⁠יִּ֨תְּנוּ־כֶ֔סֶף לַ⁠חֹצְבִ֖ים וְ⁠לֶ⁠חָרָשִׁ֑ים

**"And they gave silver to the masons and to the carpenters"**

**Gave** is an idiom that means the Jewish leaders paid money to these workers in exchange for their expected labor. The money was not a gift. Alternate translation: “And they hired masons and carpenters”

See: Idiom

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#4) (id: 462720)

וַ⁠יִּ֨תְּנוּ־כֶ֔סֶף

**"And they gave silver"**

Here, **silver** means money. Though the payment may have been in the form of silver bars, such as are described in [2:69](https://ref.ly/Ezra2:69), it may also have been in the form of some other type of money. The book is describing a payment by reference to something that was the most common type of money. Alternate translation: “And they paid money”

See: Metonymy

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#5) (id: 462721)

לַ⁠חֹצְבִ֖ים

**"to the masons"**

The term **masons** describes workers who build things from stone. Alternate translation: “stoneworkers”

See: Translate Unknowns

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#6) (id: 462722)

וְ⁠לֶ⁠חָרָשִׁ֑ים

**"and to the carpenters"**

The term **carpenters** describes workers who build things from wood. Alternate translation: “woodworkers”

See: Translate Unknowns

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#7) (id: 462723)

וּ⁠מַאֲכָ֨ל וּ⁠מִשְׁתֶּ֜ה וָ⁠שֶׁ֗מֶן לַ⁠צִּֽדֹנִים֙ וְ⁠לַ⁠צֹּרִ֔ים

**"and food, and drink, and oil to the Sidonians and to the Tyrians"**

This phrase describes a further step that the Jewish leaders took to rebuild the temple. The words **they gave**, earlier in the sentence, apply to this phrase as well. In this case, **gave** would now have the sense of “sent,” since the leaders sent the **food**, **drink, and oil** to people in distant cities. Alternate translation: “and they also sent grain and wine and olive oil to people who lived in the cities of Sidon and Tyre”

See: Idiom

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#8) (id: 462724)

וּ⁠מַאֲכָ֨ל וּ⁠מִשְׁתֶּ֜ה וָ⁠שֶׁ֗מֶן

**"and food, and drink, and oil"**

While these are all general terms that describe categories of things, the book is likely using them to refer to specific items within each category. While **food** means anything to eat, here it probably means grain, which the Israelites grew in their land and which they could transport relatively easily. Similarly, **drink** means anything to drink, but it probably means wine, and **oil** probably means olive oil, because these were both things that the Israelites also produced and could transport. Alternate translation: “and grain and wine and olive oil”

See: Synecdoche

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#9) (id: 462725)

לַ⁠צִּֽדֹנִים֙ וְ⁠לַ⁠צֹּרִ֔ים

**"to the Sidonians and to the Tyrians"**

These are the names of two people groups. The terms describe the residents of the cities of Sidon and Tyre, which were located on the seacoast north of Israel.

See: How to Translate Names

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#10) (id: 462726)

לְ⁠הָבִיא֩

**"to bring"**

This phrase describes the purpose for which the Jewish leaders took the actions just described. If it would be helpful in your language, you could use a term such as “so that” to indicate this. Alternate translation: “so that they would bring”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#11) (id: 462727)

עֲצֵ֨י אֲרָזִ֤ים

**"trees of cedar"**

The term **cedar** refers to large fir trees with strong, beautiful, aromatic wood that were valued for construction. Such trees grew plentifully and grew to great heights in the region of Lebanon. Alternate translation: “cedar trees”

See: Translate Unknowns

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#12) (id: 462728)

מִן־הַ⁠לְּבָנוֹן֙ אֶל־יָ֣ם יָפ֔וֹא

**"from Lebanon to the sea of Joppa"**

The **sea of Joppa** is not the name of a body of water. Instead, that term refers to the coast of the Mediterranean Sea at the port city of Joppa. This full phrase means that the people of Tyre and Sidon were to cut cedar trees in Lebanon and transport them to Israel by floating them along the seacoast to the port of Joppa. Alternate translation: “by sea from Lebanon to Joppa”

See: Idiom

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#13) (id: 462729)

הַ⁠לְּבָנוֹן֙

**"Lebanon"**

**Lebanon** is the name of a place. It was a mountainous region located along the coast of the Mediterranean Sea, north of Israel.

See: How to Translate Names

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#14) (id: 462730)

יָפ֔וֹא

**"Joppa"**

**Joppa** is the name of a city.

See: How to Translate Names

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#15) (id: 462731)

כְּ⁠רִשְׁי֛וֹן כּ֥וֹרֶשׁ מֶֽלֶךְ־פָּרַ֖ס עֲלֵי⁠הֶֽם

**"according to the permission of Cyrus, the king of Persia, given to them"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “for King Cyrus of Persia had authorized them to do this”

See: Active or Passive

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#16) (id: 462732)

כְּ⁠רִשְׁי֛וֹן כּ֥וֹרֶשׁ מֶֽלֶךְ־פָּרַ֖ס עֲלֵי⁠הֶֽם

**"according to the permission of Cyrus, the king of Persia, given to them"**

If it would be helpful in your language, you could translate the abstract noun **permission** with a verb. Alternate translation: “since King Cyrus of Persia had authorized them to do this”

See: Abstract Nouns

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:7 (#17) (id: 462733)

כְּ⁠רִשְׁי֛וֹן כּ֥וֹרֶשׁ מֶֽלֶךְ־פָּרַ֖ס עֲלֵי⁠הֶֽם

**"according to the permission of Cyrus, the king of Persia, given to them"**

If it would be helpful in your language, you could put this sentence earlier or even first in the verse since it gives the reason for the actions that the rest of the verse describes. You could also show the connection by beginning the next sentence with a phrase such as “and so.” Alternate translation: “King Cyrus of Persia had authorized the Jewish leaders to get the materials they needed to rebuild the temple, and so”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 3:7
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Tyrians (ID: `group:Tyre`); Persia (ID: `place:Persia`); Joppa (ID: `place:Joppa`); Lebanon (ID: `place:Lebanon`); Cyrus (ID: `person:Cyrus`); Tyre (ID: `place:Tyre`); Sidon (ID: `place:Sidon`); Sidonians (ID: `group:Sidon`); Anointing Oil (ID: `realia:AnointingOil`); Cedar (ID: `flora:Cedar`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`); Craftsman (ID: `realia:Craftsman`)

## Ezra 3:8 (#1) (id: 462734)

וּ⁠בַ⁠שָּׁנָ֣ה הַ⁠שֵּׁנִ֗ית לְ⁠בוֹאָ֞⁠ם

**"Then in the second year of their coming"**

The word **Then** at the beginning of this phrase indicates that the events the story will now relate came after the events it has just described. Alternate translation: “Then in the second year after they came”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#2) (id: 462735)

וּ⁠בַ⁠שָּׁנָ֣ה הַ⁠שֵּׁנִ֗ית לְ⁠בוֹאָ֞⁠ם אֶל־בֵּ֤ית הָֽ⁠אֱלֹהִים֙ לִ⁠יר֣וּשָׁלִַ֔ם בַּ⁠חֹ֖דֶשׁ הַ⁠שֵּׁנִ֑י

**"Then in the second year of their coming to the house of God in Jerusalem, in the second month"**

“Then in month two of year two after arriving at the house of God in Jerusalem” or “Then in the springtime of the second year since they came to the house of God in Jerusalem” or “Seven months after they had come to the house of God in Jerusalem”

See: Ordinal Numbers

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#3) (id: 462736)

וּ⁠בַ⁠שָּׁנָ֣ה הַ⁠שֵּׁנִ֗ית לְ⁠בוֹאָ֞⁠ם אֶל־בֵּ֤ית הָֽ⁠אֱלֹהִים֙ לִ⁠יר֣וּשָׁלִַ֔ם

**"Then in the second year of their coming to the house of God in Jerusalem"**

At this time, there was no temple in Jerusalem. So this phrase means one of two things: (1\) The book could be referring implicitly to the place where God's temple belonged. It seems that the people thought of the temple site as God's temple, even though it was in ruins. Alternate translation: “In the second year after they returned from exile to the site of God's temple in Jerusalem.” or (2\) The book could be referring implicitly to the purpose for which they came. Alternate translation: “In the second year after they returned to Jerusalem in order to build a temple there for God.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#4) (id: 462737)

בֵּ֤ית הָֽ⁠אֱלֹהִים֙

**"the house of God"**

“the temple”

See: Metaphor

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#5) (id: 462738)

בַּ⁠חֹ֖דֶשׁ הַ⁠שֵּׁנִ֑י

**"in the second month"**

The book does not say explicitly why the Jewish leaders started the actual construction of the new temple at this time. One possibility is that once they had ordered the necessary materials, as [3:7](https://ref.ly/Ezra3:7) describes, it took seven months for enough materials to be delivered for construction to begin. Another possibility is that the leaders waited until spring because winter would have been a bad time to start building. Yet another possibility is that they wanted to lay the foundation of this new temple in the second month of the year for ceremonial reasons, because that was the month in which King Solomon had laid the foundation of the original temple. If it would be helpful to your readers, you could put these explanations in a footnote.

* **Associated Passages:** Ezra 3:8

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#6) (id: 462739)

זְרֻבָּבֶ֣ל בֶּן־שְׁ֠אַלְתִּיאֵל

**"Zerubbabel the son of Shealtiel"**

**Zerubbabel** is the name of a man, and **Shealtiel** is the name of his father. See how you translated these names in [3:2](https://ref.ly/Ezra3:2).

See: How to Translate Names

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#7) (id: 462740)

וְ⁠יֵשׁ֨וּעַ בֶּן־יֽוֹצָדָ֜ק

**"and Jeshua the son of Jozadak"**

**Jeshua** is the name of a man, and **Jozadak** is the name of his father. See how you translated these names in [3:2](https://ref.ly/Ezra3:2).

See: How to Translate Names

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#8) (id: 462741)

וּ⁠שְׁאָ֥ר אֲחֵי⁠הֶ֣ם ׀ הַ⁠כֹּהֲנִ֣ים וְ⁠הַ⁠לְוִיִּ֗ם

**"and the rest of their brothers the priests and the Levites"**

Here, **brothers** seems to be a figurative way of saying fellow leaders, since the priests and Levites were also leaders in the community like Zerubbabel, the governor, and Jeshua, the high priest. Alternate translation: “and their fellow leaders, the priests and Levites”

See: Metaphor

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#9) (id: 462742)

וְ⁠כָל־הַ⁠בָּאִים֙ מֵ⁠הַ⁠שְּׁבִ֣י יְרֽוּשָׁלִַ֔ם

**"and all the ones who came from captivity to Jerusalem"**

The abstract noun **captivity** refers to the way the Babylonians had transported many of the Jews away from their homeland when they conquered Jerusalem. (These included many of the parents or grandparents of the people here, but also some of these people themselves, as [3:12](https://ref.ly/Ezra3:12) indicates.) If it would be helpful in your language, you could translate the idea behind this term with an equivalent phrase. Alternate translation: “and the rest of the people who had returned to Jerusalem from where their enemies had taken them”

See: Abstract Nouns

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#10) (id: 462743)

הֵחֵ֡לּוּ

**"began"**

Here the book leaves out some of the words that a sentence would ordinarily need in order to be complete because they were understood by the original readers. If it would be helpful in your language, you could supply them from the context, or you could use a term such as “so” to indicate this. Alternate translation: “so they appointed”

See: Ellipsis

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#11) (id: 462744)

אֶת־הַ⁠לְוִיִּ֗ם מִ⁠בֶּ֨ן עֶשְׂרִ֤ים שָׁנָה֙ וָ⁠מַ֔עְלָ⁠ה

**"the Levites from a son of 20 years and above"**

Here, **son of 20 years and above** is an idiom that indicates a person’s age. Alternate translation: “the Levites who were at least 20 years old”

See: Idiom

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#12) (id: 462745)

לְ⁠נַצֵּ֖חַ עַל־מְלֶ֥אכֶת בֵּית־יְהוָֽה

**"to act as overseers for the work of the house of Yahweh"**

In this context, **the work of the house** is an abbreviated way of referring to the project of constructing a temple. Alternate translation: “to supervise the construction of the temple for Yahweh”

See: Ellipsis

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:8 (#13) (id: 462746)

בֵּית־יְהוָֽה

**"the house of Yahweh"**

See how you translated this expression in [1:3](https://ref.ly/Ezra1:3). Alternate translation: “the temple for Yahweh”

See: Metaphor

* **Associated Passages:** Ezra 3:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Tribe of Levi (ID: `group:Levi`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Jehozadak (ID: `person:Jehozadak`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#1) (id: 462747)

וַ⁠יַּעֲמֹ֣ד

**"And"** \- **"stood"**

Here, the word **and** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “So … helped”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#2) (id: 462748)

יֵשׁ֡וּעַ

**"Jeshua"**

**Jeshua** is the name of a man, but he is not the high priest mentioned in the previous verse. Rather, he is the Levite clan leader listed in [2:40](https://ref.ly/Ezra2:40) as one of the exiles who returned to Judah. See how you translated his name there.

See: How to Translate Names

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#3) (id: 462749)

בָּנָ֣י⁠ו וְ֠⁠אֶחָי⁠ו

**"his sons and his brothers"**

The terms **sons** and **brothers** do not seem to be figurative here. Rather, they would describe the biological sons and brothers of Jeshua as men who worked closely with him in supervising the construction of the new temple. So it would be appropriate to use the usual terms in your language for these close relationships.

* **Associated Passages:** Ezra 3:9

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#4) (id: 462750)

קַדְמִיאֵ֨ל

**"Kadmiel"**

**Kadmiel** is the name of a man. He is another Levite clan leader listed in [2:40](https://ref.ly/Ezra2:40). See how you translated his name there.

See: How to Translate Names

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#5) (id: 462751)

וּ⁠בָנָ֤י⁠ו

**"and his sons"**

Here as well, the term **sons** does not seem to be figurative. Rather, it describes the biological sons of Kadmiel as men who helped supervise the temple construction.

* **Associated Passages:** Ezra 3:9

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#6) (id: 462752)

בְּנֵֽי־יְהוּדָה֙

**"the sons of Judah"**

Here, **sons** does mean “descendants.”

See: Metaphor

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#7) (id: 462753)

בְּנֵֽי־יְהוּדָה֙

**"the sons of Judah"**

**Judah** is the name of a man. According to [2:40](https://ref.ly/Ezra2:40), Jeshua and Kadmiel were the leaders of two Levite clans that were both descended from a man named Hodaviah. It is possible that this is an alternate spelling of that same name. Alternate translation: “who were all descendants of Hodaviah”

See: How to Translate Names

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#8) (id: 462754)

וַ⁠יַּעֲמֹ֣ד \& כְּ⁠אֶחָ֔ד לְ⁠נַצֵּ֛חַ עַל־עֹשֵׂ֥ה הַ⁠מְּלָאכָ֖ה

**"And"** \- **"stood as one to act as overseers for the ones doing the work"**

As in [2:63](https://ref.ly/Ezra2:63), **stood** is a figurative way of saying that a person assumed the duties of his office. Alternate translation: “joined together to supervise the workers”

See: Metaphor

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#9) (id: 462755)

כְּ⁠אֶחָ֔ד

**"as one"**

**As one** is an idiom that means these men all behaved as if they were a single person. That is, they all did the same thing; they worked unitedly towards a common goal. Alternate translation: “together”

See: Idiom

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#10) (id: 462756)

**"for the ones doing the work at the house of God"**

As in [2:69](https://ref.ly/Ezra2:69) and [3:8](https://ref.ly/Ezra3:8), **the work** means the project of rebuilding the temple. Alternate translation: “of the workers who were rebuilding the temple”

* **Associated Passages:** Ezra 3:9

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#11) (id: 462757)

בְּנֵי֙ חֵֽנָדָ֔ד בְּנֵי⁠הֶ֥ם וַ⁠אֲחֵי⁠הֶ֖ם הַ⁠לְוִיִּֽם

**"the sons of Henadad, their sons, and their brothers the Levites"**

The relationship of this list of names to the rest of the verse is not entirely clear. There are 2 possibilities. (1\) This is a list of the people who did the actual construction work on the new temple under the supervision of Jeshua, Kadmiel, and their relatives. Alternate translation: “Those workers were the sons and grandsons of a man named Henadad and all the other Levites” (2\) The people listed here helped Jeshua, Kadmiel, and their relatives supervise other people who did the construction work. Alternate translation: “The sons and grandsons of a man named Henadad and all the other Levites helped them supervise the workers”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#12) (id: 462758)

וַ⁠אֲחֵי⁠הֶ֖ם הַ⁠לְוִיִּֽם

**"and their brothers the Levites"**

There are two possibilities for the referent of **their** here. (1\) It could refer to Jeshua and Kadmiel, in which case **their brothers the Levites** would be a figurative way of saying “their fellow Levites,” as opposed to their biological brothers mentioned earlier in the verse. Alternate translation: “and all the other Levites who had returned from exile” (2\) However, **their** could refer to the sons of Henadad. It would still mean “fellow Levites.” The only difference would be that it would also mean that Henadad and his sons and grandsons were also Levites themselves. Alternate translation: “and the others who were Levites as they were”

See: Metaphor

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:9 (#13) (id: 462759)

בְּנֵי֙ חֵֽנָדָ֔ד בְּנֵי⁠הֶ֥ם

**"the sons of Henadad, their sons"**

The **sons of Henadad** seems to mean the actual sons of a man named Henadad. Likewise, **their sons** seems to mean the actual sons of those men. The book does not mention Henadad anywhere else and it does not provide any further information about him, so it would probably be best simply to state his name. Alternate translation: “the sons and grandsons of a man named Henadad”

See: How to Translate Names

* **Associated Passages:** Ezra 3:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Henadad (ID: `person:Henadad`); Hodaviah (ID: `person:Hodaviah.4`); Kadmiel (ID: `person:Kadmiel`); Jeshua (ID: `person:Jeshua.4`); Tribe of Levi (ID: `group:Levi`); Hodiah (ID: `person:Hodiah.2`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:10 (#1) (id: 462760)

וְ⁠יִסְּד֥וּ הַ⁠בֹּנִ֖ים אֶת־הֵיכַ֣ל יְהוָ֑ה

**"Then the ones who were building laid the foundation of the temple of Yahweh"**

The word **Then** at the beginning of this sentence indicates that it will describe something that took place after the event the story has just related. (Specifically, now that all the necessary supervisors were in place, the construction work could begin.)

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#2) (id: 462761)

וְ⁠יִסְּד֥וּ הַ⁠בֹּנִ֖ים אֶת־הֵיכַ֣ל יְהוָ֑ה

**"Then the ones who were building laid the foundation of the temple of Yahweh"**

There are two possibilities for the meaning of **laid the foundation**. (1\) This means that the workers had finished the foundation and perhaps set the entire temple floor in stone. The ceremony with music would be to mark the completion of this important step. Alternate translation: “Then the builders completed the floor of the temple of Yahweh” or (2\) It means that they started to lay the foundation, and the ceremony with the music is to mark the start of the building. Alternate translation: “Then the builders started to lay the foundation of the temple of Yahweh”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#3) (id: 462762)

וַ⁠יַּעֲמִידוּ֩ הַ⁠כֹּהֲנִ֨ים מְלֻבָּשִׁ֜ים בַּ⁠חֲצֹֽצְר֗וֹת

**"And they caused to stand the clothed priests with the trumpets"**

Unlike in [2:63](https://ref.ly/Ezra2:63) and [3:9](https://ref.ly/Ezra3:9), here **stand** means that the **priests** and **Levites** literally stood. They took up standing positions around the foundation and played their instruments. Alternate translation: “And they had the priests, wearing their sacred garments, come to the temple site and play their trumpets”

* **Associated Passages:** Ezra 3:10

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#4) (id: 462763)

מְלֻבָּשִׁ֜ים

**"clothed"**

If it would be helpful in your language, you could express the idea behind this passive verb form with a phrase that uses an active form. Alternate translation: “wearing their sacred garments”

See: Active or Passive

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#5) (id: 462764)

וְ⁠הַ⁠לְוִיִּ֤ם בְּנֵֽי־אָסָף֙ בַּֽ⁠מְצִלְתַּ֔יִם

**"and the Levites, the sons of Asaph, with the cymbals"**

The book expects readers to know that **the Levites** of this clan were vocal musicians who led the community in worship, particularly on ceremonial occasions such as this one. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “and they had the Levites who were descendants of Asaph sing and play their cymbals”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#6) (id: 462765)

וְ⁠הַ⁠לְוִיִּ֤ם בְּנֵֽי־אָסָף֙

**"and the Levites, the sons of Asaph"**

Here, **sons** means descendants. Alternate translation: “and the Levites who were descendants of Asaph”

See: Metaphor

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#7) (id: 462766)

**"Asaph"**

**Asaph** is a man’s name. See how you translated it in [2:41](https://ref.ly/Ezra2:41).

See: How to Translate Names

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#8) (id: 462767)

בַּֽ⁠מְצִלְתַּ֔יִם

**"with the cymbals"**

The word **cymbals** refers to a set of two thin, round metal plates that are hit together to make a loud sound.

See: Translate Unknowns

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#9) (id: 462768)

לְ⁠הַלֵּל֙ אֶת־יְהוָ֔ה עַל־יְדֵ֖י דָּוִ֥יד מֶֽלֶךְ־יִשְׂרָאֵֽל

**"to praise Yahweh in accordance with the hands of David, the king of Israel"**

You could place this phrase at the beginning of this sentence, before the one that describes how the priests and Levites were positioned, since it explains why the leaders put them in those positions. You could then show the connection by using a word like “so” to introduce the information about the priests and Levites. Alternate translation: “To celebrate this occasion, the leaders wanted to worship Yahweh in the way that King David of Israel had commanded, so”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#10) (id: 462769)

עַל־יְדֵ֖י דָּוִ֥יד מֶֽלֶךְ־יִשְׂרָאֵֽל

**"in accordance with the hands of David, the king of Israel"**

Here, the phrase **the hands of David, the king** is used to represent his authority to give commands. Alternate translation: “as David, king of Israel, had commanded”

See: Metonymy

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:10 (#11) (id: 462770)

דָּוִ֥יד מֶֽלֶךְ־יִשְׂרָאֵֽל

**"David, the king of Israel"**

Here the book provides some background information to describe more fully who David was. Alternate translation: “King David of Israel”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 3:10
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Asaph (ID: `person:Asaph.2`); Tribe of Levi (ID: `group:Levi`); David (ID: `person:David`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); Cymbals (ID: `realia:Cymbals`); Trumpet (ID: `realia:Trumpet`)

## Ezra 3:11 (#1) (id: 462771)

וַֽ֠⁠יַּעֲנוּ

**"Then they sang in response"**

The word **then** indicates that the sentence it introduces it will describe something that took place after the event the story has just related. If it would be helpful in your language, you could show this relationship by using an equivalent phrase. Alternate translation: “once all the musicians were in place, they sang back and forth”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 3:11
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:11 (#2) (id: 462772)

וַֽ֠⁠יַּעֲנוּ

**"Then they sang in response"**

Here, **they** refers to the Levite singers. **In response** probably means that first one group of them sang something, and then a second group of them sang something in reply, possibly playing instruments as well. The groups likely did this repeatedly over the course of a given song. Alternate translations: “Then the Levites sang responsively” or “Then the Levites sang antiphonally”

* **Associated Passages:** Ezra 3:11

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:11 (#3) (id: 462773)

כִּ֣י ט֔וֹב כִּֽי־לְ⁠עוֹלָ֥ם חַסְדּ֖⁠וֹ עַל־יִשְׂרָאֵ֑ל

**"For he is good, for his covenant faithfulness to Israel is to eternity"**

This is at least part of the song that the Levites sang on this occasion. Based on their other occurrences in the Bible, these may be the words to a refrain that the second group sang in response to verses that the first group sang. Or this may be a song that was sung for this celebration. Either way, it may be helpful to your readers to indicate that these are the words of the singers by setting off them with quotation marks or with whatever other punctuation or convention your language uses to indicate a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 3:11
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:11 (#4) (id: 462774)

כִּֽי־לְ⁠עוֹלָ֥ם חַסְדּ֖⁠וֹ עַל־יִשְׂרָאֵ֑ל

**"for his covenant faithfulness to Israel is to eternity"**

The abstract noun **faithfulness** describes the quality of a person who will dependably keep his word and fulfill his responsibilities. **Covenant faithfulness** refers specifically to Yahweh keeping all of the promises that he has made to the people of Israel. If it would be helpful in your language, you could translate the idea behind this word with an expression such as “keep promises.” Alternate translation: “because Yahweh always keeps the promises he made to Israel”

See: Abstract Nouns

* **Associated Passages:** Ezra 3:11
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:11 (#5) (id: 462775)

עַל־יִשְׂרָאֵ֑ל

**"to Israel"**

Here, the name **Israel** stands for the Israelite people. Alternate translation: “to the people of Israel”

See: Metonymy

* **Associated Passages:** Ezra 3:11
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:11 (#6) (id: 462776)

עַ֖ל הוּסַ֥ד בֵּית־יְהוָֽה

**"because the foundation of the house of Yahweh had been laid"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translations: “because the builders had laid the foundation of Yahweh’s temple” or “because the builders had set the temple floor in stone”

See: Active or Passive

* **Associated Passages:** Ezra 3:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:11 (#7) (id: 462777)

עַ֖ל הוּסַ֥ד בֵּית־יְהוָֽה

**"because the foundation of the house of Yahweh had been laid"**

As in [2:10](https://ref.ly/Ezra2:10), here there are the same two possibilities for the meaning of **the foundation … had been laid**. See how you translated it there. (1\) This means that the workers had finished the foundation. Alternate translation: “because the builders had completed the floor of the temple of Yahweh” or (2\) It means that they had started to lay the foundation, which was the first step in building the temple. Alternate translation: “because the builders had started to lay the foundation of the temple of Yahweh”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 3:12 (#1) (id: 462778)

וְ⁠רַבִּ֡ים

**"But many"**

The word **But** indicates that the sentence it introduces will draw a contrast with what came just before it. That contrast is specifically between the joyful way that most of the Israelites were responding on this occasion and the sorrowful way that some of them responded. You could use a word such as “however” to indicate this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#2) (id: 462779)

וְ⁠רָאשֵׁ֨י הָ⁠אָב֜וֹת

**"and the heads of the fathers"**

As in [1:5](https://ref.ly/Ezra1:5), this is an abbreviated way of saying “the heads of the father's houses.” Review the explanatory note to this phrase in [1:5](https://ref.ly/Ezra1:5) if that would be helpful. Alternate translation: “and the clan leaders”

See: Ellipsis

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#3) (id: 462780)

הַ⁠זְּקֵנִ֗ים אֲשֶׁ֨ר רָא֜וּ אֶת־הַ⁠בַּ֤יִת

**"the old men who had seen the"** \- **"house"**

This phrase provides background information to identify these specific priests, Levites, and clan leaders further. Alternate translation: “who were old enough that they had seen the first temple”

See: Background Information

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#4) (id: 462781)

בֹּכִ֖ים בְּ⁠ק֣וֹל גָּד֑וֹל

**"were weeping with a loud voice"**

The book does not say specifically why these older leaders who had seen the first temple were weeping. There are two possibilities. (1\) They felt grief and loss because they remembered the first temple in all of its glory and they could not imagine that this replacement would ever be as splendid. Alternate translation: “wept loudly because they thought this new temple could never be as glorious as the first one” or (2\) Seeing the community begin to restore its temple brought back memories of the terrible days in which the original temple had been destroyed and the people had had to leave their homes and go into exile. Alternate translation: “wept loudly because they remembered how the first temple had been destroyed” or just “wept loudly, remembering the past”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#5) (id: 462782)

בְּ⁠יָסְד֔⁠וֹ זֶ֤ה הַ⁠בַּ֨יִת֙ בְּ⁠עֵ֣ינֵי⁠הֶ֔ם

**"at the founding of this house in their eyes"**

As in [3:6](https://ref.ly/Ezra3:6), **founding** means to do the first step of constructing a building. See how you translated it there. Since it is a reference to the first step, laying a foundation, it may simply refer to starting the project generally. It could mean that the workers had begun to lay the foundation or that they had finished the foundation. Alternate translation: “when they saw the foundation that the builders laid for this temple”

* **Associated Passages:** Ezra 3:12

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#6) (id: 462783)

בְּ⁠יָסְד֔⁠וֹ זֶ֤ה הַ⁠בַּ֨יִת֙ בְּ⁠עֵ֣ינֵי⁠הֶ֔ם

**"at the founding of this house in their eyes"**

Here, **eyes** represent sight. So this phrase means “when construction began on the temple in their sight.” Alternate translation: “when they saw the foundation that the builders laid for this temple”

See: Metonymy

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#7) (id: 462784)

אֶת־הַ⁠בַּ֤יִת הָֽ⁠רִאשׁוֹן֙ \& זֶ֤ה הַ⁠בַּ֨יִת֙

**"the first house,"** \- **"this house"**

As often in this book, **house** is a metaphor for **temple**. Alternate translation: “the first temple … this temple”

See: Metaphor

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#8) (id: 462785)

וְ⁠רַבִּ֛ים

**"Yet many"**

The word **yet** indicates a contrast between what the people just described were doing and what the people who will be described next were doing. Alternate translation: “But many others”

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#9) (id: 462786)

וְ⁠רַבִּ֛ים בִּ⁠תְרוּעָ֥ה בְ⁠שִׂמְחָ֖ה לְ⁠הָרִ֥ים קֽוֹל

**"Yet many with a shout, with joy, were lifting up a voice"**

To be **lifting up a voice** is an idiom that means to be speaking, shouting, or singing loudly. Alternate translation: “But many others were giving loud, happy shouts.”

See: Idiom

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:12 (#10) (id: 462787)

וְ⁠רַבִּ֛ים בִּ⁠תְרוּעָ֥ה בְ⁠שִׂמְחָ֖ה לְ⁠הָרִ֥ים קֽוֹל

**"Yet many with a shout, with joy, were lifting up a voice"**

The idea behind the abstract noun **joy** can be expressed with an adjective such as happy. Alternate translation: “But many others were giving loud, happy shouts”

See: Abstract Nouns

* **Associated Passages:** Ezra 3:12
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 3:13 (#1) (id: 462788)

וְ⁠אֵ֣ין הָ⁠עָ֗ם מַכִּירִים֙

**"So the people could not differentiate"**

The word **So** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result the people could not differentiate”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 3:13
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)


## Ezra 3:13 (#2) (id: 462789)

וְ⁠אֵ֣ין הָ⁠עָ֗ם מַכִּירִים֙ ק֚וֹל תְּרוּעַ֣ת הַ⁠שִּׂמְחָ֔ה לְ⁠ק֖וֹל בְּכִ֣י הָ⁠עָ֑ם כִּ֣י הָ⁠עָ֗ם מְרִיעִים֙ תְּרוּעָ֣ה גְדוֹלָ֔ה

**"So the people could not differentiate the sound of the shout of joy from the sound of the weeping of the people. For the people were shouting a great shout"**

If it would be helpful in your language, you could reverse the order of these phrases, since the second phrase gives the reason for the action that the first phrase describes. Alternate translation: “Because the people who were happy were shouting so loudly, no one could distinguish their cries from the sounds of weeping of the other people”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 3:13
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)


## Ezra 3:13 (#3) (id: 462790)

וְ⁠אֵ֣ין הָ⁠עָ֗ם מַכִּירִים֙ ק֚וֹל תְּרוּעַ֣ת הַ⁠שִּׂמְחָ֔ה לְ⁠ק֖וֹל בְּכִ֣י הָ⁠עָ֑ם כִּ֣י הָ⁠עָ֗ם מְרִיעִים֙ תְּרוּעָ֣ה גְדוֹלָ֔ה

**"So the people could not differentiate the sound of the shout of joy from the sound of the weeping of the people. For the people were shouting a great shout"**

In its first occurrence here, the phrase **the people** means all the Israelites who had gathered for this ceremony. In its second occurrence, the phrase refers to one group of them, the older leaders who had seen the first temple and were weeping. In its third occurrence, it refers to another group of them, the younger Israelites who were shouting for joy. Alternate translation: “Because the people who were happy were shouting so loudly, no one at the ceremony could distinguish their cries from the sounds of the people who were weeping”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 3:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 3:13 (#4) (id: 462791)

וְ⁠הַ⁠קּ֥וֹל נִשְׁמַ֖ע עַד־לְ⁠מֵ⁠רָחֽוֹק

**"and the sound was heard as far as from a remote place"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “the combined sound was so loud that those who lived far away from Jerusalem could hear it”

See: Active or Passive

* **Associated Passages:** Ezra 3:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 4:1 (#1) (id: 462792)

וַֽ⁠יִּשְׁמְע֔וּ

**"Now"** \- **"heard"**

The word **Now** indicates that the sentence it introduces will provide background information that will be needed to understand what the book describes next. You can translate it with the word or phrase in your language that is most similar in meaning and significance.

See: Background Information

* **Associated Passages:** Ezra 4:1
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 4:1 (#2) (id: 462793)

יְהוּדָ֖ה וּ⁠בִנְיָמִ֑ן

**"Judah and Benjamin"**

Here the book is referring to the people who came from the tribe of Judah as if they were a single person, their ancestor Judah. It is referring similarly to the people of the tribe of Benjamin as if they were a single person, their ancestor Benjamin. Alternate translation: “the people of the tribes of Judah and Benjamin” or see the next note for a further possibility.

See: Personification

* **Associated Passages:** Ezra 4:1
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 4:1 (#3) (id: 462794)

יְהוּדָ֖ה וּ⁠בִנְיָמִ֑ן

**"Judah and Benjamin"**

Here the book may be expressing a single idea by using two words connected with **and**. Together the two words may be a way of referring to all of the Israelites, since at this point the community consisted essentially of people from the tribes of Judah and Benjamin. Alternate translation: “the Israelites”

See: Hendiadys

* **Associated Passages:** Ezra 4:1
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 4:1 (#4) (id: 462795)

וַֽ⁠יִּשְׁמְע֔וּ \& כִּֽי־בְנֵ֤י הַ⁠גּוֹלָה֙ בּוֹנִ֣ים הֵיכָ֔ל

**"Now"** \- **"heard that the sons of the exile were building a temple"**

**Heard** means that these enemies learned by some means that the Jews were doing this. The book is using hearing, one means of discovering things, to describe the enemies learning this. Alternate translation: “Now … learned that the Jews who had returned from exile were building a temple”

See: Synecdoche

* **Associated Passages:** Ezra 4:1
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 4:1 (#5) (id: 462796)

בְנֵ֤י הַ⁠גּוֹלָה֙

**"the sons of the exile"**

As indicated in [2:1](https://ref.ly/Ezra2:1) and [8:35](https://ref.ly/Ezra8:35), the phrase **the sons of the exile** refers specifically to the group of Jewish people who returned to the land of Judah from Babylon. Babylon was the country where King Nebuchadnezzar had taken many Jews as captives after he conquered Jerusalem. Alternate translations: “the Jews who had returned from exile” or “the Jews who had returned to their homeland”

See: Idiom

* **Associated Passages:** Ezra 4:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 4:1 (#6) (id: 462797)

לַ⁠יהוָ֖ה אֱלֹהֵ֥י יִשְׂרָאֵֽל

**"for Yahweh, the God of Israel"**

This phrase provides further background information about Yahweh and it likely expresses the perspective of the enemies on the situation. Alternate translations: “Yahweh, the God they worshiped” or “Yahweh, the God whom the people of Israel worship”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 4:1
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#1) (id: 462798)

וַ⁠יִּגְּשׁ֨וּ

**"So they came"**

The word **So** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result, they went”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#2) (id: 462799)

וַ⁠יִּגְּשׁ֨וּ

**"So they came"**

Use the motion verb that is most natural in your language. Alternate translation: “So they went”

See: Go and Come

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** Go and Come (ID: `33278@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#3) (id: 462800)

**"Zerubbabel"**

**Zerubbabel** is the name of a man. See how you translated it in [2:2](https://ref.ly/Ezra2:2).

See: How to Translate Names

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#4) (id: 462801)

רָאשֵׁ֣י הָֽ⁠אָב֗וֹת

**"the heads of the fathers"**

As in [1:5](https://ref.ly/Ezra1:5) and elsewhere in the book, this is an abbreviated way of saying “**the heads of** father’s houses.” Alternate translation: “the clan leaders”

See: Ellipsis

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#5) (id: 462802)

וַ⁠יֹּאמְר֤וּ לָ⁠הֶם֙ נִבְנֶ֣ה עִמָּ⁠כֶ֔ם

**"and they said to them, “Let us build with you"**

Since the book says in the previous verse that these people were enemies of the Jews, the implication is that they were not sincere when they claimed that they wanted to help build the temple. Instead, they were probably looking for a way to hinder the building project from within. The Jewish leaders refused their offer, and that suggests that they themselves suspected that these people were insincere. If it would be helpful to your readers, you could say something like that explicitly. Alternate translation: “and attempting to deceive the Jews, they said to them, ‘We would like to help you build the temple’”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#6) (id: 462803)

נִדְר֖וֹשׁ לֵֽ⁠אלֹהֵי⁠כֶ֑ם

**"we seek your God"**

Here, **seek** is an idiom that means to worship and pray to God or a god. Alternate translation: “we worship your God”

See: Idiom

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#7) (id: 462804)

מִ⁠ימֵי֙ אֵסַ֤ר חַדֹּן֙

**"since the days of Esarhaddon"**

The term **days** is used here to refer a particular period of time. Alternate translation: “since the time of Esarhaddon”

See: Idiom

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#8) (id: 462805)

אֵסַ֤ר חַדֹּן֙ מֶ֣לֶךְ אַשּׁ֔וּר

**"Esarhaddon, the king of Assyria"**

**Esarhaddon** is the name of a man, and **Assyria** is the name of his kingdom.

See: How to Translate Names

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:2 (#9) (id: 462806)

הַ⁠מַּעֲלֶ֥ה אֹתָ֖⁠נוּ פֹּֽה

**"the one who caused us to come up here"**

These men say that Esarhaddon brought them **up** to this land because it is mountainous and, as [4:9](https://ref.ly/Ezra4:9) indicates, their ancestors formerly lived in lowland areas. Alternate translation: “the one who brought us here”

See: Idiom

* **Associated Passages:** Ezra 4:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esarhaddon (ID: `person:Esarhaddon`); Zerubbabel (ID: `person:Zerubbabel.2`); Assyria (ID: `place:Assyria`); LORD (ID: `deity:Lord`)

## Ezra 4:3 (#1) (id: 462807)

וַ⁠יֹּאמֶר֩

**"But"** \- **"said"**

The word **But** indicates that the sentence it introduces draws a contrast between what the men asked and what the Jewish leaders answered. You could begin the sentence with a word like “however” or “nevertheless” to indicate this contrast, or use another method that is natural in your language.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#2) (id: 462808)

**"Zerubbabel, and Jeshua"**

These are the names of two men. See how you translated them in [2:2](https://ref.ly/Ezra2:2).

See: How to Translate Names

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#3) (id: 462809)

רָאשֵׁ֤י הָֽ⁠אָבוֹת֙

**"the heads of the fathers"**

This is an abbreviated way of saying “**the heads of** father’s houses.” See how you translated this expression in [1:5](https://ref.ly/Ezra1:5). Review the note there if that would be helpful. Alternate translation: “the clan leaders”

See: Ellipsis

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#4) (id: 462810)

לֹֽא־לָ֣⁠כֶם וָ⁠לָ֔⁠נוּ

**"It is not for you and for us"**

This expression is an idiom that means “it is for us alone.” Alternate translation: “You may not help us”

See: Idiom

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#5) (id: 462811)

בַּ֖יִת לֵ⁠אלֹהֵ֑י⁠נוּ

**"a house for our God"**

Alternate translation: “a temple for our God”

See: Metaphor

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#6) (id: 462812)

לֵ⁠אלֹהֵ֑י⁠נוּ

**"for our God"**

Here, **our** is exclusive, excluding the non\-Jewish hearers.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#7) (id: 462813)

לֵ⁠אלֹהֵ֑י⁠נוּ

**"for our God"**

The Israelites could say that Yahweh is **our God** not in the sense that they possess or control him, but in the sense that they worship him exclusively. Alternate translation: “for the God whom we worship”

See: Possession

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#8) (id: 462814)

כִּי֩

**"but"**

The word **but** indicates that the clause it introduces draws a contrast between what the men asked and what the Jewish leaders were prepared to allow. To indicate this contrast, you could begin the sentence with a word or phrase such as “no,” “rather,” or “on the contrary.”

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#9) (id: 462815)

אֲנַ֨חְנוּ יַ֜חַד נִבְנֶ֗ה לַֽ⁠יהוָה֙ אֱלֹהֵ֣י יִשְׂרָאֵ֔ל

**"we ourselves together will build for Yahweh, the God of Israel"**

**We ourselves** here does not include the addressees. If your language marks that distinction, be sure that this is clear in your translation. **Together** does not mean the Jewish people and these foreign peoples together. It means the whole Jewish community together. Alternate translation: “it is all of us Israelites who will build a temple for Yahweh, the God of Israel”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#10) (id: 462816)

**"the God of Israel"**

See the note to [4:1](https://ref.ly/Ezra4:1) about the significance of this phrase.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:3 (#11) (id: 462817)

כַּ⁠אֲשֶׁ֣ר צִוָּ֔⁠נוּ הַ⁠מֶּ֖לֶךְ כּ֥וֹרֶשׁ מֶֽלֶךְ־פָּרָֽס

**"just as the king Cyrus, the king of Persia, has commanded us"**

It is implicit in the story that the Jewish leaders know that they cannot trust these foreign leaders, whom the story describes as their enemies. The Jewish leaders suggest indirectly that they do not accept their claim that they worship Yahweh. They say, “It is not for you … to build a house for our God,” and they describe Yahweh as the God of Israel exclusively. But apparently they cannot contradict the claim directly, perhaps for reasons of etiquette and diplomacy. Instead, it seems that they find convenient grounds to exclude foreigners from the temple project in the literal wording of the proclamation that Cyrus issued: “Whoever among you is from all his people … may he build the house of Yahweh, the God of Israel” ([1:3](https://ref.ly/Ezra1:3)). It may be possible in your translation to translate this phrase in such a way that your readers will implicitly recognize it as the convenient excuse that it apparently is. Alternate translation: “for that is what King Cyrus of Persia commanded”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Relatives (ID: `group:Relatives`); Persia (ID: `place:Persia`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Joshua (ID: `person:Joshua.5`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 4:4 (#1) (id: 462818)

וַ⁠יְהִי֙

**"And it happened that"**

The phrase **and it happened that** introduces a new series of events in the story. Use a natural way in your language for introducing something new or different.

See: Introduction of a New Event

* **Associated Passages:** Ezra 4:4
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Judah (ID: `person:Judah.4`)

## Ezra 4:4 (#2) (id: 462819)

עַם־הָ⁠אָ֔רֶץ

**"the people of the land"**

This expression refers to the non\-Israelite people groups, listed in [4:9](https://ref.ly/Ezra4:9), whom the Assyrian kings Esarhaddon and Ashurbanipal had brought to live in the area just north of Judah in the century before the Jews were taken away into exile themselves. When the Jews returned to their homeland, those foreign groups were still living nearby. Alternate translation: “the foreign people groups living nearby”

See: Idiom

* **Associated Passages:** Ezra 4:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Judah (ID: `person:Judah.4`)

## Ezra 4:4 (#3) (id: 462820)

וַ⁠יְהִי֙ \& מְרַפִּ֖ים יְדֵ֣י עַם־יְהוּדָ֑ה וּֽמְבַהֲלִ֥ים אוֹתָ֖⁠ם לִ⁠בְנֽוֹת

**"And it happened that"** \- **"were making the hands of the people of Judah weak, and making them afraid to build"**

These two phrases mean similar things. If it would be helpful in your language, you could combine them. Alternate translation: “making the people of Judah so intimidated that they stopped working on the temple” However, there is a slight distinction between the phrases. The first one describes how the people felt inwardly, and the second one explains what effect this had on their outward actions. So you could also translate the phrases separately. Alternate translation: “were intimidating the people of Judah so that they were afraid to keep working on the temple”

See: Parallelism

* **Associated Passages:** Ezra 4:4
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Judah (ID: `person:Judah.4`)

## Ezra 4:4 (#4) (id: 462821)

וַ⁠יְהִי֙ \& מְרַפִּ֖ים יְדֵ֣י עַם־יְהוּדָ֑ה

**"And it happened that"** \- **"were making the hands of the people of Judah weak"**

Here, **hands** represents power and action, so **making the hands weak** means to make someone so discouraged or intimidated that they do not use their power to act. Alternate translation: “were intimidating the people of Judah”

See: Metaphor

* **Associated Passages:** Ezra 4:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Judah (ID: `person:Judah.4`)

## Ezra 4:4 (#5) (id: 462822)

עַם־יְהוּדָ֑ה

**"the people of Judah"**

In this context, this expression does not refer to all of the inhabitants of the province of Judah, whatever their nationality, but rather it specifically refers to the Jews who returned from exile and settled there. In this book, **the people of Judah** and “the people of Israel” both refer to the Jewish people and mean the same thing. Alternate translation: “the Jews who had returned from exile and settled in the province of Judah”

* **Associated Passages:** Ezra 4:4

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Judah (ID: `person:Judah.4`)

## Ezra 4:5 (#1) (id: 462823)

וְ⁠סֹכְרִ֧ים עֲלֵי⁠הֶ֛ם יוֹעֲצִ֖ים

**"and hiring counselors against them"**

Here, **counselors** means royal advisors in the Persian court. The enemies of the Jews were bribing the counselors to try to get them to influence the king against the Jews. Alternate translation: “and bribing royal advisors to work against the Jews in the Persian court”

See: Translate Unknowns

* **Associated Passages:** Ezra 4:5
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Counsel (ID: `keyterm:Counsel`)

## Ezra 4:5 (#2) (id: 462824)

לְ⁠הָפֵ֣ר עֲצָתָ֑⁠ם

**"to frustrate their plans"**

Alternate translation: “to keep them from successfully rebuilding the temple”

* **Associated Passages:** Ezra 4:5

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Counsel (ID: `keyterm:Counsel`)

## Ezra 4:5 (#3) (id: 462825)

כָּל־יְמֵ֗י כּ֚וֹרֶשׁ מֶ֣לֶךְ פָּרַ֔ס

**"all the days of Cyrus, the king of Persia"**

**Days** is used here to refer to a particular period of time. In this context, it would mean specifically the nine years from 538 BC, when Cyrus conquered Babylon and became ruler of the Jews, to 529 BC, when he died. Alternate translation: “throughout the rest of the reign of Cyrus as king of Persia”

See: Idiom

* **Associated Passages:** Ezra 4:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Counsel (ID: `keyterm:Counsel`)

## Ezra 4:5 (#4) (id: 462826)

וְ⁠עַד־מַלְכ֖וּת דָּרְיָ֥וֶשׁ מֶֽלֶךְ־פָּרָֽס

**"and until the reign of Darius, the king of Persia"**

Darius did not succeed Cyrus directly, and so this means that the enemy opposition continued throughout the reigns of the successors of Cyrus until Darius became king. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “and throughout the reign of his successors until Darius became king of Persia”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Counsel (ID: `keyterm:Counsel`)

## Ezra 4:5 (#5) (id: 462827)

דָּרְיָ֥וֶשׁ

**"Darius"**

**Darius** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 4:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Cyrus (ID: `person:Cyrus`); Persia (ID: `place:Persia`); Counsel (ID: `keyterm:Counsel`)

## Ezra 4:6 (#1) (id: 462828)

וּ⁠בְ⁠מַלְכוּת֙

**"Now in the reign of"**

The word **Now** indicates that the sentence it introduces will provide some background information. You can translate it with the word or phrase in your language that is most similar in meaning and significance.

See: Background Information

* **Associated Passages:** Ezra 4:6
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 4:6 (#2) (id: 462829)

וּ⁠בְ⁠מַלְכוּת֙ אֲחַשְׁוֵר֔וֹשׁ בִּ⁠תְחִלַּ֖ת מַלְכוּת֑⁠וֹ

**"Now in the reign of Ahasuerus,in the beginning of his reign"**

Ahasuerus ruled the Persian Empire after Darius, who was his father. So here the book is presenting some events out of chronological order. That is, it is telling about some things that happened later, before finishing the story of what happened at this time. In [4:24](https://ref.ly/Ezra4:24) the book will bring readers back to where it left off in [4:5](https://ref.ly/Ezra4:5), and then, in [5:1](https://ref.ly/Ezra5:1), it will describe how work on rebuilding the temple did resume under the reign of Darius. But first it gives accounts here of how the enemies of the Jews continued to oppose them under two later kings, Ahasuerus (described in this verse) and Artaxerxes (described in verses [7–23](https://ref.ly/Ezra4:7-Ezra4:23)). The purpose may be to show that the Jews were wise to refuse the help that leaders from other nations offered, as described in [4:1–3](https://ref.ly/Ezra4:1-Ezra4:3). Since those leaders were actually enemies of the Jews, as they demonstrated by opposing them under one king after another, their offer was not sincere and they could not be trusted. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “In fact, they continued to oppose them under later kings. As soon as Ahasuerus succeeded his father Darius as king”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 4:6 (#3) (id: 462830)

אֲחַשְׁוֵר֔וֹשׁ

**"Ahasuerus"**

**Ahasuerus** is the name of a man. He is better known as Xerxes, so you could use that name for him in your translation, if it would be helpful in your language. Use the same name here that you use in the book of Esther.

See: How to Translate Names

* **Associated Passages:** Ezra 4:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 4:6 (#4) (id: 462831)

כָּתְב֣וּ שִׂטְנָ֔ה עַל־יֹשְׁבֵ֥י יְהוּדָ֖ה וִ⁠ירוּשָׁלִָֽם

**"they wrote an accusation against those who dwelled in Judah and Jerusalem"**

If it would be helpful to your readers, you could say explicitly what the enemies of the Jews accused them of doing. (The book provides the text of their letter in [4:11–16](https://ref.ly/Ezra4:11-Ezra4:16), but it might be helpful to provide a summary in advance.) Alternate translation: “they wrote a letter in which they accused the Jews who had returned to Judah and Jerusalem of plotting to rebel against the king”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 4:6 (#5) (id: 462832)

כָּתְב֣וּ שִׂטְנָ֔ה

**"they wrote an accusation"**

The abstract noun **accusation** refers to what the enemies of the Jews said about them in the letter that they sent to the king. If it would be helpful in your language, you could translate the idea behind this word with a verb such as “accuse” or “complain.” Alternate translation: “they wrote a letter to complain”

See: Abstract Nouns

* **Associated Passages:** Ezra 4:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 4:6 (#6) (id: 462833)

כָּתְב֣וּ שִׂטְנָ֔ה עַל

**"they wrote an accusation against"**

The noun **accusation** might be referring to the actual letter itself by substituting a description of the content of the letter. If it would be helpful in your language, you could use a concrete term that explains the meaning of the figurative expression. Alternate translation: “they wrote a letter, accusing”

See: Metonymy

* **Associated Passages:** Ezra 4:6
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 4:6 (#7) (id: 462834)

יֹשְׁבֵ֥י יְהוּדָ֖ה וִ⁠ירוּשָׁלִָֽם

**"those who dwelled in Judah and Jerusalem"**

In this context, this expression does not mean all the inhabitants of the province of Judah and the city of Jerusalem, whatever their nationality, but rather the Jews who returned from exile and settled in those places. Alternate translation: “the Jews who had returned from exile and settled in Judah and Jerusalem”

* **Associated Passages:** Ezra 4:6

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 4:7 (#1) (id: 462835)

וּ⁠בִ⁠ימֵ֣י

**"And in the days of"**

Here the word **And** introduces another event in the sequence of actions by the enemies of the Jews. Use the form in your language that is most natural for marking events in a sequence.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#2) (id: 462836)

וּ⁠בִ⁠ימֵ֣י אַרְתַּחְשַׁ֗שְׂתָּא

**"And in the days of Artaxerxes"**

The term **days** is used here to refer to a particular period of time. In this context, it means during the reign of the next Persian king, Artaxerxes, the son of Ahasuerus (Xerxes). Alternate translation: “during the reign of Artaxerxes”

See: Idiom

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#3) (id: 462837)

וּ⁠בִ⁠ימֵ֣י אַרְתַּחְשַׁ֗שְׂתָּא

**"And in the days of Artaxerxes"**

If it would be helpful to your readers, you could explain that Artaxerxes was the next king of Persia. Alternate translation: “during the reign of the next Persian king, Artaxerxes”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#4) (id: 462838)

אַרְתַּחְשַׁ֗שְׂתָּא

**"Artaxerxes"**

**Artaxerxes** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#5) (id: 462839)

בִּשְׁלָ֜ם מִתְרְדָ֤ת טָֽבְאֵל֙

**"Bishlam, Mithredath, Tabeel"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#6) (id: 462840)

וּ⁠שְׁאָ֣ר כנותו

**"and the rest of his companions"**

The term **companions** here describes people who hold a similar position. Alternate translation: “and his other associates”

* **Associated Passages:** Ezra 4:7

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#7) (id: 462841)

וּ⁠כְתָב֙ הַֽ⁠נִּשְׁתְּוָ֔ן כָּת֥וּב אֲרָמִ֖ית וּ⁠מְתֻרְגָּ֥ם אֲרָמִֽית

**"and the script of the letter was written in Aramaic and was interpreted in Aramaic"**

This is background information that helps readers understand how these officials went about writing to the king. Aramaic was the official language for government correspondence within the Persian empire. These officials apparently spoke Samaritan, a language similar to Hebrew. So they had someone express their message for them in the Aramaic language. The term **interpreted** does not necessarily indicate that the officials first wrote out their letter in Samaritan, and then someone translated it into Aramaic for them. Instead, they may have told someone what they wanted to say, and that person composed the letter in Aramaic. Since Aramaic also used differently shaped letters from the ones that Hebrew and Samaritan used, that person wrote the letter using those characters. Alternate translation: “these officials arranged for someone to write their letter for them in the Aramaic script and language”

See: Background Information

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#8) (id: 462842)

וּ⁠כְתָב֙ הַֽ⁠נִּשְׁתְּוָ֔ן כָּת֥וּב אֲרָמִ֖ית וּ⁠מְתֻרְגָּ֥ם אֲרָמִֽית

**"and the script of the letter was written in Aramaic and was interpreted in Aramaic"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “and these officials hired a scribe who wrote the letter in Aramaic”

See: Active or Passive

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#9) (id: 462843)

וּ⁠כְתָב֙ הַֽ⁠נִּשְׁתְּוָ֔ן כָּת֥וּב אֲרָמִ֖ית וּ⁠מְתֻרְגָּ֥ם אֲרָמִֽית

**"and the script of the letter was written in Aramaic and was interpreted in Aramaic"**

If it would be helpful in your language, you could simply say that the letter was written in Aramaic, rather than mentioning both the script and the language. Alternate translation: “and these officials hired a scribe who wrote the letter in Aramaic”

* **Associated Passages:** Ezra 4:7

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:7 (#10) (id: 462844)

וּ⁠מְתֻרְגָּ֥ם אֲרָמִֽית

**"and was interpreted in Aramaic"**

We might expect that, logically, this phrase would come before the previous one, written in the **Aramaic** script, since the letter had to be composed before it was written down. But in its present position at the end of this sentence, this phrase actually serves a larger purpose within the book. It signals readers that what follows, through [6:18](https://ref.ly/Ezra6:18), is itself written in Aramaic. Presumably the original readers, who were clearly expected to understand both languages, would have been alert to such a signal. (A long Aramaic section in the book of Daniel is introduced similarly: “Then the wise men spoke to the king in Aramaic,” Daniel 2:4\.) Alternate translation: “and was translated into Aramaic such as the following:”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mithredath (ID: `person:Mithredath.2`); Bishlam (ID: `person:Bishlam`); Tabeel (ID: `person:Tabeel`); Persia (ID: `place:Persia`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:8 (#1) (id: 462845)

רְח֣וּם

**"Rehum"**

**Rehum** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 4:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jerusalem (ID: `place:Jerusalem`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:8 (#2) (id: 462846)

בְּעֵל־טְעֵ֗ם

**"a lord of decree"**

This was Rehum’s official title. The expression likely means that he had the authority in that area to issue decrees on behalf of the king. The king actually tells him to do this in [4:21](https://ref.ly/Ezra4:21). Use the title for a government official with authority over a local area. Alternate translation: “the royal deputy”

See: Idiom

* **Associated Passages:** Ezra 4:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jerusalem (ID: `place:Jerusalem`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:8 (#3) (id: 462847)

וְ⁠שִׁמְשַׁי֙

**"and Shimshai"**

**Shimshai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 4:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jerusalem (ID: `place:Jerusalem`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:8 (#4) (id: 462848)

סָֽפְרָ֔⁠א

**"the scribe"**

This was Shimshai’s official title. It does not mean that he merely handled correspondence and record\-keeping for Rehum. Rather, historical sources suggest that a **scribe** such as Shimshai had important administrative responsibilities and authority of his own. Alternate translation: “the state secretary”

See: Translate Unknowns

* **Associated Passages:** Ezra 4:8
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jerusalem (ID: `place:Jerusalem`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:8 (#5) (id: 462849)

אִגְּרָ֥ה חֲדָ֖ה עַל־יְרוּשְׁלֶ֑ם

**"one letter against Jerusalem"**

Here the book uses the term **Jerusalem** to refer figuratively, by association, to the Jews who lived in that city and, by extension, to their fellow Jews in the rest of the province of Judah. Alternate translation: “a letter against the Jews who had returned from exile”

See: Metonymy

* **Associated Passages:** Ezra 4:8
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jerusalem (ID: `place:Jerusalem`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:8 (#6) (id: 462850)

כְּנֵֽמָא

**"as follows"**

The phrase **as follows** would ordinarily indicate that a copy of the text of the letter comes next in the book. If that is the case, then [4:9](https://ref.ly/Ezra4:9) and [4:10](https://ref.ly/Ezra4:10) are a preamble to the letter that lists the names of the senders. Alternate translation: “and here is what they wrote to the king”

See: Metonymy

* **Associated Passages:** Ezra 4:8
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jerusalem (ID: `place:Jerusalem`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:9 (#1) (id: 462851)

אֱדַ֜יִן

**"Then"**

The term translated as **Then** often introduces a new episode in a story. But the book is not continuing the story here. Instead, the book is quoting a document that begins with a list of names. So it seems that here the term **Then** is introducing the document that follows. Use the term in your language that would customarily introduce a list of the senders of a letter or other document. Alternate translation: “From”

* **Associated Passages:** Ezra 4:9

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#2) (id: 462852)

אֱדַ֜יִן

**"Then"**

The word **then** here appears to be introducing a quotation from a document. If this is not how you would introduce a quotation in your language, then use quotation marks or whatever other punctuation or convention your language commonly uses for that purpose.

See: Quote Markings

* **Associated Passages:** Ezra 4:9
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#3) (id: 462853)

רְח֣וּם בְּעֵל־טְעֵ֗ם

**"Rehum, a lord of decree"**

**Rehum** is the name of a man and **a lord of decree** is his title. See how you translated them in [4:8](https://ref.ly/Ezra4:8). Alternate translation: “Rehum, the royal deputy”

See: How to Translate Names

* **Associated Passages:** Ezra 4:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#4) (id: 462854)

וְ⁠שִׁמְשַׁי֙ סָֽפְרָ֔⁠א

**"and Shimshai the scribe"**

**Shimshai** is the name of a man and **the scribe** is his title. See how you translated them in [4:8](https://ref.ly/Ezra4:8). Alternate translation: “Shimshai the state secretary”

See: How to Translate Names

* **Associated Passages:** Ezra 4:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#5) (id: 462855)

וּ⁠שְׁאָ֖ר כְּנָוָתְ⁠ה֑וֹן דִּ֠ינָיֵ⁠א וַ⁠אֲפַרְסַתְכָיֵ֞⁠א טַרְפְּלָיֵ֣⁠א

**"and the rest of their companions, the judges and the rulers, the officials"**

As in [4:7](https://ref.ly/Ezra4:7), the term **companions** here indicates people who hold a similar position. The other three terms describe the various positions that the other people held who joined in sending this letter. Alternate translation: “and their associates, who were judges, rulers, and officials”

See: Idiom

* **Associated Passages:** Ezra 4:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#6) (id: 462856)

וּ⁠שְׁאָ֖ר כְּנָוָתְ⁠ה֑וֹן דִּ֠ינָיֵ⁠א וַ⁠אֲפַרְסַתְכָיֵ֞⁠א טַרְפְּלָיֵ֣⁠א

**"and the rest of their companions, the judges and the rulers, the officials"**

Rehum and Shimshai speak of themselves here in the third person by saying **their companions**. This was normal in their culture. If this is confusing in your language, you can use the first person. Alternate translation: “and our colleagues, who are judges, rulers, and officials”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:9
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#7) (id: 462857)

וַ⁠אֲפַרְסַתְכָיֵ֞⁠א טַרְפְּלָיֵ֣⁠א

**"and the rulers, the officials"**

The difference between these two terms is uncertain. If it would be helpful in your language, you could combine them, as in the UST.

* **Associated Passages:** Ezra 4:9

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#8) (id: 462858)

אֲפָֽרְסָיֵ֗⁠א אַרְכְּוָיֵ֤א בָבְלָיֵ⁠א֙ שֽׁוּשַׁנְכָיֵ֔⁠א ד⁠הוא עֵלְמָיֵֽ⁠א

**"the Persians, the Erechites, the Babylonians, the Susaites (that is, the Elamites"**

These are the names of four people groups and an alternate name by which the last group was known.

See: How to Translate Names

* **Associated Passages:** Ezra 4:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:9 (#9) (id: 462859)

אֲפָֽרְסָיֵ֗⁠א אַרְכְּוָיֵ֤א בָבְלָיֵ⁠א֙ שֽׁוּשַׁנְכָיֵ֔⁠א ד⁠הוא עֵלְמָיֵֽ⁠א

**"the Persians, the Erechites, the Babylonians, the Susaites (that is, the Elamites"**

These are apparently the people groups that the officials who wrote the letter belonged to. The writers seem to feel that they can speak on behalf of their entire people groups because they would agree with the concerns that they are expressing about the Jews. You could say this explicitly if it would be helpful to your readers. Alternate translation: “on behalf of the Persians, Erechites, Babylonians, and Susaites (that is, Elamites)”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Erech (ID: `place:Erech`); Babylonians (ID: `group:Babylon`); Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Dinaite (ID: `person:Dinaite`); Persia (ID: `place:Persia`); Babylon (ID: `place:Babylon`); Persians (ID: `group:Persia`); Elamites (ID: `group:Elam`); Susa (ID: `place:Susa`); Elam (ID: `place:Elam`)

## Ezra 4:10 (#1) (id: 462860)

וּ⁠שְׁאָ֣ר אֻמַּיָּ֗⁠א דִּ֤י הַגְלִי֙ אָסְנַפַּר֙ רַבָּ֣⁠א וְ⁠יַקִּירָ֔⁠א וְ⁠הוֹתֵ֣ב הִמּ֔וֹ בְּ⁠קִרְיָ֖ה דִּ֣י שָׁמְרָ֑יִן וּ⁠שְׁאָ֥ר עֲבַֽר־נַהֲרָ֖⁠ה

**"and the rest of the nations whom Ashurbanipal the great and the noble caused to be exiled and caused them to dwell in the cities of Samaria, and the rest of Beyond\-the\-River"**

This could mean one of two things. (1\) It could be referring to two different sets of people groups: (a) **the rest of the nations**, which would be some further groups that, like the four listed in [4:9](https://ref.ly/Ezra4:9), Ashurbanipal exiled to the region of Samaria, and (b) **the rest of Beyond\-the\-River**, which would be all the other groups that were then living in the province of Beyond\-the\-River, however they came to live there. This is the reading of ULT. or (2\) It could be referring to one set of people groups made up of all the other groups that, like the four listed in [4:9](https://ref.ly/Ezra4:9), Ashurbanipal moved to this area and resettled in the cities of Samaria and in other parts of the province. This is the reading of UST. Either way, the officials writing this letter feel that they can speak on behalf of those other groups in addition to the groups that they belong to personally.

* **Associated Passages:** Ezra 4:10

* **Associated ACAI Concepts:** Osnappar (ID: `person:Osnappar`); Samaria (ID: `place:Samaria`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:10 (#2) (id: 462861)

אָסְנַפַּר֙

**"Ashurbanipal"**

**Ashurbanipal** is the name of a man. His name in Aramaic was Osnappar. He was king of Assyria from 668 to 627 B.C.

See: How to Translate Names

* **Associated Passages:** Ezra 4:10
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Osnappar (ID: `person:Osnappar`); Samaria (ID: `place:Samaria`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:10 (#3) (id: 462862)

עֲבַֽר־נַהֲרָ֖⁠ה

**"Beyond\-the\-River"**

**Beyond\-the\-River** is the name of the province of the Persian Empire that contained the lands of Israel and Judah. It occurs many times throughout the book, and it will be helpful to your readers if you translate it consistently each time.

See: How to Translate Names

* **Associated Passages:** Ezra 4:10
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Osnappar (ID: `person:Osnappar`); Samaria (ID: `place:Samaria`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:10 (#4) (id: 462863)

וּ⁠כְעֶֽנֶת

**"And now"**

The phrase translated **And now** is an Aramaic expression that was used in the letters of this time to introduce the main business of the letter. In [4:11](https://ref.ly/Ezra4:11), [4:17](https://ref.ly/Ezra4:17), and [7:12](https://ref.ly/Ezra7:12), it comes at the beginning of a letter, right after opening conventions such as the names of the sender and recipient and greetings. In [5:17](https://ref.ly/Ezra5:17), it comes near the end of a letter to introduce a request that the writers finally make in light of extensive background information that they provide up to that point. But here it introduces the letter itself after a long preamble that lists the names of the senders and the groups on whose behalf they feel they can speak. If your language has a comparable expression that it uses for this same purpose, you can use that in your translation. If your language would not put such an expression here, then you could leave it out.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 4:10
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Osnappar (ID: `person:Osnappar`); Samaria (ID: `place:Samaria`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:11 (#1) (id: 462864)

דְּנָה֙ פַּרְשֶׁ֣גֶן אִגַּרְתָּ֔⁠א דִּ֚י שְׁלַ֣חוּ עֲל֔וֹ⁠הִי

**"This is a copy of the letter that they sent to him"**

**They** means the men listed in [4:9](https://ref.ly/Ezra4:9). The book is interrupting its quotation after the preamble to make sure that readers know that the text of the letter itself will now follow.

See: Aside

* **Associated Passages:** Ezra 4:11
* **Associated Articles:** Aside (ID: `33260@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:11 (#2) (id: 462865)

דְּנָה֙ פַּרְשֶׁ֣גֶן אִגַּרְתָּ֔⁠א דִּ֚י שְׁלַ֣חוּ עֲל֔וֹ⁠הִי

**"This is a copy of the letter that they sent to him"**

If you used quotation marks or some other punctuation or convention to indicate the beginning of the quotation at the start of [4:9](https://ref.ly/Ezra4:9), you should use the same means to indicate that there is a break in the quotation at the end of [4:10](https://ref.ly/Ezra4:10), and then show that the quotation resumes after this sentence.

See: Quote Markings

* **Associated Passages:** Ezra 4:11
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:11 (#3) (id: 462866)

**"Artaxerxes"**

**Artaxerxes** is the name of a man. See how you translated it in [4:7](https://ref.ly/Ezra4:7).

See: How to Translate Names

* **Associated Passages:** Ezra 4:11
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:11 (#4) (id: 462867)

עַבְדָ֛י⁠ךְ אֱנָ֥שׁ עֲבַֽר־נַהֲרָ֖⁠ה

**"your servants, men of Beyond\-the\-River"**

Here the officials identify themselves as the senders of this letter, and they refer to themselves in the third person as **your servants** in order to address the king as their superior with humility and respect. Alternate translation: “from your officials in the province of Beyond\-the\-River”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:11
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:11 (#5) (id: 462868)

עַבְדָ֛י⁠ךְ אֱנָ֥שׁ עֲבַֽר־נַהֲרָ֖⁠ה

**"your servants, men of Beyond\-the\-River"**

In this context, **men** means people, so this phrase means people who live in Beyond\-the\-River province. But together with the preceding phrase, **your servants**, it means specifically those who work for the king there, that is, his royal officials. Alternate translation: “from your officials in the province of Beyond\-the\-River”

See: Idiom

* **Associated Passages:** Ezra 4:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:11 (#6) (id: 462869)

וּ⁠כְעֶֽנֶת

**"and now"**

As in [4:10](https://ref.ly/Ezra4:10), the phrase translated **and now** is an Aramaic expression that introduces the main business of a letter. If your language has a comparable expression that it uses for this same purpose, you can use that in your translation. Otherwise, you do not need to represent this expression.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 4:11
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes.2`)

## Ezra 4:12 (#1) (id: 462870)

יְדִ֨יעַ֙ לֶהֱוֵ֣א לְ⁠מַלְכָּ֔⁠א

**"let it be known to the king"**

These officials address the king in the third person as a sign of respect. If it would be helpful in your language, you could indicate this respect by using an expression such as “O king.” Alternate translation: “we would like you to know, O king”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:12
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:12 (#2) (id: 462871)

יְדִ֨יעַ֙ לֶהֱוֵ֣א לְ⁠מַלְכָּ֔⁠א

**"let it be known to the king"**

If it would be helpful in your language, you could say this with an active form, and you could say who is doing the action. Alternate translation: “we would like you to know, O king”

See: Active or Passive

* **Associated Passages:** Ezra 4:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:12 (#3) (id: 462872)

יְהוּדָיֵ֗⁠א דִּ֤י סְלִ֨קוּ֙ מִן־לְוָתָ֔⁠ךְ

**"the Jews who went up from near you"**

Here, as in several instances earlier in the book, **went up** means “traveled from Babylon to Judah,” since that involves going from a river valley up into the mountains. The officials describe Babylon to the king as **near you** because it was relatively much closer to the Persian royal court and more distant from Samaria. Alternate translation: “the Jews who returned to Judah from Babylon”

See: Idiom

* **Associated Passages:** Ezra 4:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:12 (#4) (id: 462873)

סְלִ֨קוּ֙

**"went up"**

If it is more natural in your language, you could say “came” rather than **went**.

See: Go and Come

* **Associated Passages:** Ezra 4:12
* **Associated Articles:** Go and Come (ID: `33278@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:12 (#5) (id: 462874)

עֲלֶ֥י⁠נָא אֲת֖וֹ לִ⁠ירוּשְׁלֶ֑ם

**"have come to us at Jerusalem"**

Alternate translation: “have settled near us in Jerusalem”

* **Associated Passages:** Ezra 4:12

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:12 (#6) (id: 462875)

קִרְיְתָ֨⁠א מָֽרָדְתָּ֤⁠א ו⁠באישת⁠א

**"the rebellious and evil city"**

Here the officials are describing the inhabitants of Jerusalem by reference to something associated with them, the **city** where they live. The officials are not saying that the walls and buildings would be rebellious if they were rebuilt. Rather, they are saying that the people who have lived in this city have continually revolted against their foreign rulers. Alternate translation: “that city whose people are constantly rebelling”

See: Metonymy

* **Associated Passages:** Ezra 4:12
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:12 (#7) (id: 462876)

מָֽרָדְתָּ֤⁠א ו⁠באישת⁠א

**"rebellious and evil"**

**Rebellious and evil** mean similar things. The officials use them together to emphasize their certainty that, because the people of Jerusalem are so evil or wicked (in this context meaning rebellious against authority), they would revolt against Artaxerxes if they were able to rebuild and fortify their city. If it would be helpful in your language, you could combine these words into a single expression. Alternate translation: “constantly rebelling”

See: Doublet

* **Associated Passages:** Ezra 4:12
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:12 (#8) (id: 462877)

וְשׁוּרַיָּ֣א שַׁכְלִ֔ילוּ וְ⁠אֻשַּׁיָּ֖⁠א יַחִֽיטוּ

**"they are completing the walls and repairing the foundations"**

Here, **completing** is a way of saying “building.” Several times in this section of Ezra, the text uses the two words **repairing** and **completing** together to mean “building,” or it uses them as poetic parallels. (For example, “which a great king of Israel built and completed,” [5:11](https://ref.ly/Ezra5:11), and “to build this house and to complete this structure,” [5:3](https://ref.ly/Ezra5:3).) The term **completing** by itself here seems similarly to have the meaning of “building.” **Repairing** here could also mean “digging out.” Either way, this is a reference to preparing foundations so that buildings can be set up on them. So the officials are describing the project of restoring the city by referring to the work on two parallel parts of it, its walls and its buildings. If it would be helpful in your language, you could say this without using a similar figure of speech. Alternate translations: “they are working throughout the city to restore it” or “they are rebuilding the walls and preparing foundations for buildings”

See: Parallelism

* **Associated Passages:** Ezra 4:12
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Jerusalem (ID: `place:Jerusalem`); Foundation (ID: `realia:Foundation`)

## Ezra 4:13 (#1) (id: 462878)

כְּעַ֗ן

**"Now"**

The word **now** is similar to the expression “and now” in [4:10](https://ref.ly/Ezra4:10) and [4:11](https://ref.ly/Ezra4:11). Here, **now** introduces an important point within the letter. If your language has a comparable expression that it uses for this same purpose, you can use that in your translation.

* **Associated Passages:** Ezra 4:13


## Ezra 4:13 (#2) (id: 462879)

יְדִ֨יעַ֙ לֶהֱוֵ֣א לְ⁠מַלְכָּ֔⁠א

**"let it be known to the king"**

As in [4:12](https://ref.ly/Ezra4:12), the officials address the king here in the third person as a sign of respect. If it would be helpful in your language, you could indicate this respect by using an expression such as “O king.” Alternate translation: “we would like you to know, O king”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:13
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Ezra 4:13 (#3) (id: 462880)

יְדִ֨יעַ֙ לֶהֱוֵ֣א לְ⁠מַלְכָּ֔⁠א

**"let it be known to the king"**

If it would be helpful in your language, you could say this with an active form, and you could say who is doing the action. Alternate translation: “we would like you to know, O king”

See: Active or Passive

* **Associated Passages:** Ezra 4:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 4:13 (#4) (id: 462881)

הֵ֣ן קִרְיְתָ֥⁠א דָךְ֙ תִּתְבְּנֵ֔א וְ⁠שׁוּרַיָּ֖⁠ה יִֽשְׁתַּכְלְל֑וּן

**"if that city is built and the walls are completed"**

These two phrases mean similar things. (As noted in [4:12](https://ref.ly/Ezra4:12), **completed** is another way of saying **built** when the terms are paired like this.) Likely for emphasis, the officials say essentially the same thing twice to create an ominous sense of threat. You do not need to repeat both phrases in your translation if that might be confusing for your readers. Alternate translation: “if they are able to rebuild that city” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase is actually stronger and more threatening, because the city walls could prevent Artaxerxes from reasserting his sovereignty by force. Alternate translation: “if they rebuild the city, and especially if they restore its protective walls”

See: Parallelism

* **Associated Passages:** Ezra 4:13
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)


## Ezra 4:13 (#5) (id: 462882)

הֵ֣ן קִרְיְתָ֥⁠א דָךְ֙ תִּתְבְּנֵ֔א וְ⁠שׁוּרַיָּ֖⁠ה יִֽשְׁתַּכְלְל֑וּן

**"if that city is built and the walls are completed"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “if the Jews are able to rebuild the city and restore its walls”

See: Active or Passive

* **Associated Passages:** Ezra 4:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 4:13 (#6) (id: 462883)

מִנְדָּֽה־בְל֤וֹ וַ⁠הֲלָךְ֙ לָ֣א יִנְתְּנ֔וּן

**"they will not give tax, tribute, or custom"**

**Tax**, **tribute**, and **custom** mean similar things. The officials use the three terms together to emphasize that the king could lose all of his income from the Jews living in Jerusalem and the province of Judah. However, there is a slight difference between the three terms. While their exact meaning is uncertain, they may refer to these three kinds of taxes: taxes based on a percentage of income, tribute money that subjects would pay on a per\-person basis, and duty that they would pay on articles purchased for use or transported for sale. Alternate translation: “they will no longer pay taxes, tribute, or duty” But if you do not have three different words for different kinds of taxes in your language, you could just use one word, as in the UST. (A doublet can involve the use of more than two words.)

See: Doublet

* **Associated Passages:** Ezra 4:13
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)


## Ezra 4:13 (#7) (id: 462884)

וְ⁠אַפְּתֹ֥ם מַלְכִ֖ים תְּהַנְזִֽק

**"and the revenue of the kings will suffer harm"**

The officials are writing to a king who is the sole ruler of his empire, but they may be using the plural **kings** here because previous kings had accumulated much of the royal treasures. The officials may also be suggesting implicitly that not only Artaxerxes, but also his successors, will get no further revenue from the Jews if Jerusalem is rebuilt and fortified. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “and this will reduce the income of kings”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 4:14 (#1) (id: 462885)

כְּעַ֗ן

**"Now"**

As in [4:13](https://ref.ly/Ezra4:13), the word **now** is similar to the expression “and now” found in [4:10](https://ref.ly/Ezra4:10) and [4:11](https://ref.ly/Ezra4:11). Here it introduces another important point within the letter. If your language has a comparable expression that it uses for this same purpose, you can use that in your translation.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 4:14
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)


## Ezra 4:14 (#2) (id: 462886)

מְלַ֤ח הֵֽיכְלָ⁠א֙ מְלַ֔חְנָא

**"we have eaten the salt of the palace"**

This expression means that these officials are paid from the royal treasury. Alternate translation: “we are paid from the royal treasury”

See: Idiom

* **Associated Passages:** Ezra 4:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 4:14 (#3) (id: 462887)

כָּ⁠ל־קֳבֵל֙ דִּֽי־מְלַ֤ח הֵֽיכְלָ⁠א֙ מְלַ֔חְנָא

**"because we have eaten the salt of the palace"**

The implication is that, because these officials are paid from the royal treasury, they feel a loyalty and a sense of obligation to the king. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “because we owe our own livelihoods to you”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 4:14 (#4) (id: 462888)

וְ⁠עַרְוַ֣ת מַלְכָּ֔⁠א לָ֥א אֲֽרִֽיךְ לַ֖⁠נָא לְ⁠מֶֽחֱזֵ֑א

**"and it is not appropriate for us to see the nakedness of the king"**

The phrase **to see the nakedness of the king** is an idiom that means “to allow the king to be dishonored.” If your language has a similar idiom you can use that, or translate the plain meaning, as in the UST.

See: Idiom

* **Associated Passages:** Ezra 4:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 4:14 (#5) (id: 462889)

עַ֨ל־דְּנָ֔ה שְׁלַ֖חְנָא וְ⁠הוֹדַ֥עְנָא לְ⁠מַלְכָּֽ⁠א

**"on account of this we have sent and made known to the king"**

The officials continue to address the king in the third person as a sign of respect. If it would be helpful in your language, you could indicate this respect by using an expression such as “O king.” Alternate translation: “and that is why we have written to you, O king, to let you know about this”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:14
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Ezra 4:15 (#1) (id: 462890)

דִּ֡י יְבַקַּר֩

**"so that he may search"**

Here again the officials are addressing the king in the third person as a sign of respect. They are also making their suggestion very diplomatically so that they do not appear to be telling the king what to do. Alternate translation: “and may we suggest that you search”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:15 (#2) (id: 462891)

בִּֽ⁠סְפַר־דָּכְרָ֨נַיָּ֜⁠א דִּ֣י אֲבָהָתָ֗⁠ךְ

**"in the book of the records of your fathers"**

Although **book** is singular, this expression actually refers to the collection of documents that Artaxerxes possessed that recorded events and decrees from the reigns of earlier kings. He responds in [4:19](https://ref.ly/Ezra4:19) that he had his court officials search in this collection. Verses [6:1–2](https://ref.ly/Ezra6:1-Ezra6:2) describe how Darius earlier made a similar search. Alternate translation: “in the royal chronicles” or “in the archives of the previous kings”

See: Idiom

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:15 (#3) (id: 462892)

בִּֽ⁠סְפַר־דָּכְרָ֨נַיָּ֜⁠א דִּ֣י אֲבָהָתָ֗⁠ךְ

**"in the book of the records of your fathers"**

Here, **fathers** means “predecessors.” This would include some who were direct ancestors of Artaxerxes, including his father Ahasuerus (Xerxes) and grandfather Darius. But it also seems to include the Assyrian and Babylonian kings to whose empires the Persian kings were now the heirs. The revolts of Jerusalem that this search discovered, as reported in [4:19](https://ref.ly/Ezra4:19), were against the Babylonians. Indeed, Artaxerxes says that his officials investigated the matter back to ancient days, presumably back to the times of earlier empires. Alternate translation: “the chronicles of previous kings”

See: Metaphor

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:15 (#4) (id: 462893)

וּ֠⁠תְהַשְׁכַּח בִּ⁠סְפַ֣ר דָּכְרָנַיָּ⁠א֮ וְ⁠תִנְדַּע֒

**"And you will discover in the book of the records and learn"**

The words **discover** and **learn** mean similar things. The officials use the two words together to emphasize how certain it is that the chronicles will show that Jerusalem has always been rebellious. If it would be helpful in your language, you could use only one word instead of two. Alternate translation: “The chronicles will certainly confirm for you”

See: Doublet

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:15 (#5) (id: 462894)

דִּי֩ קִרְיְתָ֨⁠א דָ֜ךְ קִרְיָ֣א מָֽרָדָ֗א וּֽ⁠מְהַנְזְקַ֤ת מַלְכִין֙ וּ⁠מְדִנָ֔ן וְ⁠אֶשְׁתַּדּוּר֙ עָבְדִ֣ין בְּ⁠גַוַּ֔⁠הּ מִן־יוֹמָ֖ת עָלְמָ֑⁠א

**"that that city is a rebellious city and one that has caused harm to kings and provinces, and they have made revolt in its midst since the ancient days"**

These two phrases mean similar things. The officials say essentially the same thing twice for emphasis. You do not need to repeat both phrases in your translation if that might be confusing for your readers. Alternate translation: “that the people living in Jerusalem have constantly rebelled against their kings and governors and refused to pay tribute” However, there is a slight difference in meaning. The second phrase intensifies the first phrase, and you could also choose to bring that out in your translation. Alternate translation: “that the people of Jerusalem have rebelled against emperors and governors and refused to pay tribute, long being a center of revolt”

See: Parallelism

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:15 (#6) (id: 462895)

וּֽ⁠מְהַנְזְקַ֤ת

**"and one that has caused harm to"**

In light of the similar expression in [4:13](https://ref.ly/Ezra4:13), “and the revenue of the kings will suffer harm,” this likely means that the people of Jerusalem have refused to pay tribute to foreign emperors. Alternate translation: “and refused to pay tribute to”

See: Idiom

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:15 (#7) (id: 462896)

וּ⁠מְדִנָ֔ן

**"and provinces"**

Here, **provinces** refers to the rulers of provinces, that is, governors. The officials are describing them by reference to something associated with them, the jurisdictions that they rule. Alternate translation: “and governors”

See: Metonymy

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:15 (#8) (id: 462897)

עַ֨ל־דְּנָ֔ה קִרְיְתָ֥⁠א דָ֖ךְ הָֽחָרְבַֽת

**"On account of this, that city was destroyed"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “Because the people of Jerusalem kept rebelling against them, the Babylonians finally came and destroyed that city”

See: Active or Passive

* **Associated Passages:** Ezra 4:15
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Ezra 4:16 (#1) (id: 462898)

מְהוֹדְעִ֤ין אֲנַ֨חְנָה֙ לְ⁠מַלְכָּ֔⁠א

**"We are making known to the king"**

These officials address the king in the third person as a sign of respect. If it would be helpful in your language, you could indicate this respect by using an expression such as “O king.” Alternate translation: “We would like you to know, O king”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:16
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`)

## Ezra 4:16 (#2) (id: 462899)

הֵ֣ן קִרְיְתָ֥⁠א דָךְ֙ תִּתְבְּנֵ֔א וְ⁠שׁוּרַיָּ֖⁠ה יִֽשְׁתַּכְלְל֑וּן

**"if that city is built and the walls are completed"**

See how you translated this expression in [4:13](https://ref.ly/Ezra4:13). (It may be helpful to review the two notes there that discuss the parallelism and the passive form.)

* **Associated Passages:** Ezra 4:16

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`)

## Ezra 4:16 (#3) (id: 462900)

לָ⁠קֳבֵ֣ל דְּנָ֔ה חֲלָק֙ בַּ⁠עֲבַ֣ר נַהֲרָ֔⁠א לָ֥א אִיתַ֖י לָֽ⁠ךְ

**"because of this there will be no share for you in Beyond\-the\-River"**

The implication is that if Jerusalem became a stronghold, not only would its people rebel against Artaxerxes, they would lead all of the surrounding peoples to do the same, and the Persian Empire would lose all of its territories west of the Euphrates. The expression **no share for you** likely also has the implication that the king would receive no further tribute or other revenue from the entire province. Alternate translation: “then those people will lead the entire province of Beyond\-the\-River to revolt against you and to stop paying tribute.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`)

## Ezra 4:17 (#1) (id: 462901)

רְח֤וּם בְּעֵל־טְעֵם֙

**"Rehum, a lord of decree"**

**Rehum** is the name of a man and **a lord of decree** is his title. See how you translated them in [4:8](https://ref.ly/Ezra4:8). Alternate translation: “Rehum, the royal deputy”

See: How to Translate Names

* **Associated Passages:** Ezra 4:17
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Samaria (ID: `place:Samaria`); Shimshai (ID: `person:Shimshai`); Euphrates River (ID: `place:Euphrates`); Rehum (ID: `person:Rehum.2`)

## Ezra 4:17 (#2) (id: 462902)

וְ⁠שִׁמְשַׁ֣י סָֽפְרָ֔⁠א

**"and Shimshai the scribe"**

**Shimshai** is the name of a man and **scribe** is his title. See how you translated them in [4:8](https://ref.ly/Ezra4:8). Alternate translation: “Shimshai the state secretary”

See: How to Translate Names

* **Associated Passages:** Ezra 4:17
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Samaria (ID: `place:Samaria`); Shimshai (ID: `person:Shimshai`); Euphrates River (ID: `place:Euphrates`); Rehum (ID: `person:Rehum.2`)

## Ezra 4:17 (#3) (id: 462903)

וּ⁠שְׁאָר֙ כְּנָוָ֣תְ⁠ה֔וֹן דִּ֥י יָתְבִ֖ין בְּ⁠שָֽׁמְרָ֑יִן וּ⁠שְׁאָ֧ר עֲבַֽר־נַהֲרָ֛⁠ה

**"and the rest of their companions who dwell in Samaria, and the rest of Beyond the River"**

As in [4:10](https://ref.ly/Ezra4:10), this could mean one of two things. (1\) It could be referring to two groups: (a) the fellow officials of Rehum and Shimshai in the region of Samaria, and (b) the people groups living in the rest of the province of Beyond\-the\-River, however they came to live there. This is the reading of ULT. Alternate translation: “and the rest of their fellow officials, and the people groups living in Beyond\-the\-River” or (2\) It could be referring to one group, the fellow officials of Rehum and Shimshai who live in the region of Samaria and in other parts of the province. This is the reading of UST. Alternate translation: “the rest of their fellow officials in Samaria and in the rest of Beyond\-the\-River” It would probably be clearest for your readers if you followed the same reading here as you did in [4:10](https://ref.ly/Ezra4:10), whether that of ULT or UST.

See: How to Translate Names

* **Associated Passages:** Ezra 4:17
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Samaria (ID: `place:Samaria`); Shimshai (ID: `person:Shimshai`); Euphrates River (ID: `place:Euphrates`); Rehum (ID: `person:Rehum.2`)

## Ezra 4:17 (#4) (id: 462904)

וּ⁠שְׁאָר֙ כְּנָוָ֣תְ⁠ה֔וֹן

**"and the rest of their companions"**

As in [4:7](https://ref.ly/Ezra4:7) and [4:9](https://ref.ly/Ezra4:9), the term **companions** here indicates people who hold similar positions. Alternate translation: “and the rest of their associates” or “and the rest of their fellow officials”

See: Idiom

* **Associated Passages:** Ezra 4:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Samaria (ID: `place:Samaria`); Shimshai (ID: `person:Shimshai`); Euphrates River (ID: `place:Euphrates`); Rehum (ID: `person:Rehum.2`)

## Ezra 4:17 (#5) (id: 462905)

שְׁלָ֖ם

**"Peace"**

**Peace** was a conventional greeting or good wish that senders often included at the beginning of a letter at this time. (It is the Aramaic term that corresponds to the Hebrew expression “Shalom.”) If your language has a similar expression that it uses for the same purpose, you can use it here. Alternate translations: “greetings” or “I hope all is well with you”

* **Associated Passages:** Ezra 4:17

* **Associated ACAI Concepts:** Samaria (ID: `place:Samaria`); Shimshai (ID: `person:Shimshai`); Euphrates River (ID: `place:Euphrates`); Rehum (ID: `person:Rehum.2`)

## Ezra 4:17 (#6) (id: 462906)

וּ⁠כְעֶֽת

**"And now"**

As in [4:10](https://ref.ly/Ezra4:10) and [4:11](https://ref.ly/Ezra4:11), this phrase translates an Aramaic expression that introduces the main business of a letter. If your language has a comparable expression, then use it here. If it would not use such an expression here, then you do not need to represent it in your translation.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 4:17
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Samaria (ID: `place:Samaria`); Shimshai (ID: `person:Shimshai`); Euphrates River (ID: `place:Euphrates`); Rehum (ID: `person:Rehum.2`)

## Ezra 4:18 (#1) (id: 462907)

נִשְׁתְּוָנָ֕⁠א דִּ֥י שְׁלַחְתּ֖וּן עֲלֶ֑י⁠נָא

**"The letter that you sent to us"**

The implication of the plural **us** is that the king and his royal officials have received this letter. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “the letter that you sent to the king and his royal officials”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 4:18 (#2) (id: 462908)

מְפָרַ֥שׁ קֱרִ֖י קָדָמָֽ⁠י

**"has been carefully read aloud before me"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “my officials have carefully read aloud in my presence”

See: Active or Passive

* **Associated Passages:** Ezra 4:18
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 4:18 (#3) (id: 462909)

מְפָרַ֥שׁ קֱרִ֖י קָדָמָֽ⁠י

**"has been carefully read aloud before me"**

The implication is not that the king was unable to read or unable to see, and so he needed someone to read things to him. Rather, in ancient times, writing was considered a way of conveying speech at a distance. In effect, the court officials who read this letter aloud to the king were doing so on behalf of the Samaritan officials who had sent the letter, as if they had been present themselves. So be sure that your translation does not convey or suggest the idea that the king was unable to read. Alternate translation: “my officials have carefully read out loud in my presence”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 4:19 (#1) (id: 462910)

וּ⁠מִנִּ⁠י֮

**"So from me"**

The word **so** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result, from me”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)


## Ezra 4:19 (#2) (id: 462911)

וּ⁠מִנִּ⁠י֮ שִׂ֣ים טְעֵם֒ וּ⁠בַקַּ֣רוּ

**"So from me was set a decree, and they searched"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “So I commanded my officials, and they searched”

See: Active or Passive

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 4:19 (#3) (id: 462912)

וּ⁠מִנִּ⁠י֮ שִׂ֣ים טְעֵם֒ וּ⁠בַקַּ֣רוּ

**"So from me was set a decree, and they searched"**

The implication is that the king ordered his officials to search in the royal chronicles to investigate the charges that Rehum, Shimshai, and the others had made against Jerusalem. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “I commanded my officials to search in the royal chronicles, and they investigated your claims”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 4:19 (#4) (id: 462913)

דִּ֚י קִרְיְתָ֣⁠א דָ֔ךְ \& עַל־מַלְכִ֖ין מִֽתְנַשְּׂאָ֑ה וּ⁠מְרַ֥ד וְ⁠אֶשְׁתַּדּ֖וּר מִתְעֲבֶד־בַּֽ⁠הּ

**"that that city"** \- **"has risen up against kings and rebellion and revolt have been made in it"**

These two phrases mean similar things. Artaxerxes says the same thing twice, in similar ways, for emphasis. You do not need to repeat both phrases in your translation if that might be confusing for your readers. Instead, If it would be helpful in your language, you could combine them. Alternate translation: “Jerusalem has continually been at the center of revolts against rulers” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase intensifies the first by portraying Jerusalem not just as a city that has rebelled, but as a place where rebellions have repeatedly originated. Alternate translation: “Jerusalem … has rebelled against the emperors that ruled it and, in fact, that city has repeatedly been a base for revolts”

See: Parallelism

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)


## Ezra 4:19 (#5) (id: 462914)

קִרְיְתָ֣⁠א דָ֔ךְ \& עַל־מַלְכִ֖ין מִֽתְנַשְּׂאָ֑ה

**"that city"** \- **"has risen up against kings"**

**That city** means Jerusalem. In this context, the expression **risen up against** means to reject an authority and claim authority for oneself. Alternate translation: “the people of Jerusalem … have rebelled against the emperors who ruled them”

See: Idiom

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 4:19 (#6) (id: 462915)

קִרְיְתָ֣⁠א דָ֔ךְ \& עַל־מַלְכִ֖ין מִֽתְנַשְּׂאָ֑ה

**"that city"** \- **"has risen up against kings"**

Here, Artaxerxes speaks of the people of Jerusalem by reference to something associated with them, the city where they live. Alternate translation: “the people of Jerusalem … have rebelled against the emperors who ruled them”

See: Metonymy

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)


## Ezra 4:19 (#7) (id: 462916)

וּ⁠מְרַ֥ד וְ⁠אֶשְׁתַּדּ֖וּר מִתְעֲבֶד־בַּֽ⁠הּ

**"and rebellion and revolt have been made in it"**

**Rebellion** and **revolt** mean similar things. Artaxerxes uses the two terms together for emphasis, perhaps to acknowledge that the records have indeed confirmed what the officials alleged. If it would be helpful in your language, you could combine these terms. Alternate translation: “and the city has repeatedly been a base for revolts”

See: Doublet

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)


## Ezra 4:19 (#8) (id: 462917)

וּ⁠מְרַ֥ד וְ⁠אֶשְׁתַּדּ֖וּר מִתְעֲבֶד־בַּֽ⁠הּ

**"and rebellion and revolt have been made in it"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “and the people there have repeatedly rebelled against their rulers”

See: Active or Passive

* **Associated Passages:** Ezra 4:19
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 4:20 (#1) (id: 462918)

וּ⁠מַלְכִ֣ין תַּקִּיפִ֗ין הֲווֹ֙ עַל־יְר֣וּשְׁלֶ֔ם

**"And mighty kings were over Jerusalem"**

These kings are described here in a spatial metaphor as being higher than or above Jerusalem to indicate that this was the place in which and from which they ruled. Alternate translation: “And powerful kings ruled from Jerusalem”

See: Metaphor

* **Associated Passages:** Ezra 4:20
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:20 (#2) (id: 462919)

וְ⁠שַׁ֨לִּיטִ֔ין

**"even rulers"**

This word **even** introduces further information. Alternate translation: “in fact, they ruled”

See: Background Information

* **Associated Passages:** Ezra 4:20
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:20 (#3) (id: 462920)

וְ⁠שַׁ֨לִּיטִ֔ין בְּ⁠כֹ֖ל עֲבַ֣ר נַהֲרָ֑⁠ה

**"even rulers over all of Beyond the River"**

Alternate translation: “in fact, they ruled over the whole area that is Beyond\-the\-River province”

* **Associated Passages:** Ezra 4:20

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:20 (#4) (id: 462921)

וּ⁠מִדָּ֥ה בְל֛וֹ וַ⁠הֲלָ֖ךְ מִתְיְהֵ֥ב לְ⁠הֽוֹן

**"and tax, tribute, and custom were paid to them"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “these kings conquered other nations and made them pay tribute” or “these kings conquered other nations and made them pay taxes, tribute, and duty”

See: Active or Passive

* **Associated Passages:** Ezra 4:20
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:20 (#5) (id: 462922)

וּ⁠מִדָּ֥ה בְל֛וֹ וַ⁠הֲלָ֖ךְ

**"and tax, tribute, and custom"**

See how you translated this phrase in [4:13](https://ref.ly/Ezra4:13). (A doublet can involve the use of more than two words.)

See: Doublet

* **Associated Passages:** Ezra 4:20
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Euphrates River (ID: `place:Euphrates`)

## Ezra 4:21 (#1) (id: 462923)

**"Now"**

The word **Now** is similar to the expression “and now” in [4:10](https://ref.ly/Ezra4:10), [4:11](https://ref.ly/Ezra4:11), and [4:17](https://ref.ly/Ezra4:17). As in [4:13](https://ref.ly/Ezra4:13) and [4:14](https://ref.ly/Ezra4:14), it introduces an important point within a letter. If your language has a comparable expression that it uses for this same purpose, you can use that in your translation.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 4:21
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)


## Ezra 4:21 (#2) (id: 462924)

שִׂ֣ימוּ טְּעֵ֔ם לְ⁠בַטָּלָ֖א גֻּבְרַיָּ֣⁠א אִלֵּ֑ךְ

**"set a decree to stop these men"**

**These men** means the Jews. This is a reference back to the letter from the Samaritan officials in which they speak of “the Jews who went up from near you” and who “have come to us at Jerusalem” ([4:12](https://ref.ly/Ezra4:12)). The implication is that the Jews are to stop rebuilding the walls and buildings in Jerusalem. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “issue a decree to make those Jews who returned to Jerusalem from exile stop rebuilding the walls and buildings in Jerusalem”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:21
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 4:21 (#3) (id: 462925)

וְ⁠קִרְיְתָ֥⁠א דָךְ֙ לָ֣א תִתְבְּנֵ֔א עַד־מִנִּ֖⁠י טַעְמָ֥⁠א יִתְּשָֽׂם

**"so that city may not be rebuilt until the decree is set from me"**

The phrase **so that** indicates that the clause that follows describes the purpose for which Artaxerxes wants the officials to issue a decree. It is to make the Jews stop working on the walls and houses. He does not want there to be any rebuilding in Jerusalem unless he authorizes it personally. Alternate translation: “because I want this decree to prevent all rebuilding unless I issue a decree permitting it”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** Ezra 4:21
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)


## Ezra 4:22 (#1) (id: 462926)

וּ⁠זְהִירִ֥ין הֱו֛וֹ שָׁל֖וּ לְ⁠מֶעְבַּ֣ד עַל־דְּנָ֑ה

**"And be careful of negligence of action concerning this"**

This means “Be sure to act concerning this.” This is a figure of speech that expresses a strong positive meaning by using two negative expressions: **be careful of**, that is, “avoid,” together with an expression that is the opposite of what is desired: **negligence of action**. Artaxerxes wants the officials to take action. Alternate translation: “Be sure to take action in response to this situation”

See: Litotes

* **Associated Passages:** Ezra 4:22
* **Associated Articles:** Litotes (ID: `33290@UWTranslationManual`)


## Ezra 4:22 (#2) (id: 462927)

לְ⁠מָה֙ יִשְׂגֵּ֣א חֲבָלָ֔⁠א לְ⁠הַנְזָקַ֖ת מַלְכִֽין

**"Why should the damage increase to the harm of the kings"**

The implication is that if the Jews were able to rebuild and fortify Jerusalem and they revolted, then the interests of the king would be damaged because he would lose honor, as the officials suggest in their letter ([4:14](https://ref.ly/Ezra4:14)), and he would also lose revenue ([4:13](https://ref.ly/Ezra4:13)). If it would be helpful to your readers, you could say this explicitly. Alternate translation: “Why should I and other kings suffer dishonor or lose tribute money”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:22
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 4:22 (#3) (id: 462928)

לְ⁠מָה֙ יִשְׂגֵּ֣א חֲבָלָ֔⁠א לְ⁠הַנְזָקַ֖ת מַלְכִֽין

**"Why should the damage increase to the harm of the kings"**

Artaxerxes is making a statement, not asking a question. He does not expect the Samaritan officials to tell him why he should experience greater damage. Instead, he is using the question form to emphasize how important it is for the officials to get the Jews to stop rebuilding Jerusalem. If it would be helpful in your language, you could translate these words as a statement. Alternate translation: “I should not suffer dishonor or lose tribute money.”

See: Rhetorical Question

* **Associated Passages:** Ezra 4:22
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)


## Ezra 4:22 (#4) (id: 462929)

לְ⁠מָה֙ יִשְׂגֵּ֣א חֲבָלָ֔⁠א לְ⁠הַנְזָקַ֖ת מַלְכִֽין

**"Why should the damage increase to the harm of the kings"**

As in [4:13](https://ref.ly/Ezra4:13), the plural **kings** here may envision that not just Artaxerxes but also his successors would lose revenue if Jerusalem leads a revolt that spreads throughout Beyond\-the\-River, as the Samaritan officials suggest it would. But Artaxerxes is speaking primarily of himself, in the third person, as kings sometimes did, just as their subjects addressed them in the third person to show humility and respect. (Compare, for example, the way Cyrus speaks of himself in the third person in [6:4](https://ref.ly/Ezra6:4), and Darius speaks of himself that way in [6:8](https://ref.ly/Ezra6:8) and [6:10](https://ref.ly/Ezra6:10)). Alternate translation: “Why should kings suffer dishonor or lose tribute money”

See: First, Second or Third Person

* **Associated Passages:** Ezra 4:22
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Ezra 4:23 (#1) (id: 462930)

אֱדַ֗יִן

**"Then"**

The word **Then** indicates that the events the story will now relate came after the events it has just described. In your translation, you can use the expression in your language that would best indicate this relationship.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#2) (id: 462931)

מִן־דִּ֞י פַּרְשֶׁ֤גֶן נִשְׁתְּוָנָ⁠א֙ דִּ֚י ארתחששתא מַלְכָּ֔⁠א קֱרִ֧י קֳדָם־רְח֛וּם וְ⁠שִׁמְשַׁ֥י סָפְרָ֖⁠א וּ⁠כְנָוָתְ⁠ה֑וֹן

**"from when the copy of the letter of Artaxerxes the king was read aloud before Rehum, and Shimshai the scribe, and their companions"**

The implication is that a messenger brought a copy of the letter from King Artaxerxes to these officials and read it aloud to them. (See the note to [4:18](https://ref.ly/Ezra4:18) for background to the practice of reading correspondence out loud.) If it would be helpful to your readers, you could say that explicitly. Alternate translation: “as soon as messengers from King Artaxerxes brought a copy of his letter to Rehum, Shimshai the state secretary, and their associates, and read it aloud to them”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#3) (id: 462932)

פַּרְשֶׁ֤גֶן נִשְׁתְּוָנָ⁠א֙ דִּ֚י ארתחששתא מַלְכָּ֔⁠א קֱרִ֧י

**"the copy of the letter of Artaxerxes the king was read aloud"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “messengers from King Artaxerxes read a copy of his letter aloud”

See: Active or Passive

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#4) (id: 462933)

**"Artaxerxes"**

**Artaxerxes** is the name of a man. See how you translated it in [4:7](https://ref.ly/Ezra4:7).

See: How to Translate Names

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#5) (id: 462934)

**"Rehum"**

Rehum is the name of a man. See how you translated it in [4:8](https://ref.ly/Ezra4:8).

See: How to Translate Names

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#6) (id: 462935)

וְ⁠שִׁמְשַׁ֥י סָפְרָ֖⁠א

**"and Shimshai the scribe"**

**Shimshai** is the name of a man and **scribe** is his title. See how you translated them in [4:8](https://ref.ly/Ezra4:8). Alternate translation: “Shimshai the state secretary”

See: How to Translate Names

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#7) (id: 462936)

**"and their companions"**

As in [4:7](https://ref.ly/Ezra4:7), [4:9](https://ref.ly/Ezra4:9) and [4:17](https://ref.ly/Ezra4:17), the term **companions** here indicates people who hold similar positions. Alternate translation: “and their associates,”

See: Idiom

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#8) (id: 462937)

אֲזַ֨לוּ בִ⁠בְהִיל֤וּ לִ⁠ירֽוּשְׁלֶם֙ עַל־יְה֣וּדָיֵ֔⁠א

**"they went in a hurry to Jerusalem against the Jews"**

Alternate translation: “they hurried to Jerusalem to confront the Jews”

* **Associated Passages:** Ezra 4:23

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#9) (id: 462938)

וּ⁠בַטִּ֥לוּ הִמּ֖וֹ בְּ⁠אֶדְרָ֥ע וְ⁠חָֽיִל

**"and they stopped them by an arm and strength"**

The phrase **an arm and strength** expresses a single idea by using two words connected with “and.” The word **strength** tells what kind of **arm** or influence these officials used: a strong arm, that is, a forcibly coercive one. If it would be helpful in your language, you could express the meaning of the two words in a single phrase. Alternate translation: “and they compelled them to stop”

See: Hendiadys

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#10) (id: 462939)

וּ⁠בַטִּ֥לוּ הִמּ֖וֹ בְּ⁠אֶדְרָ֥ע וְ⁠חָֽיִל

**"and they stopped them by an arm and strength"**

Here, **arm** is a figurative way of referring to a person’s power, influence, and capacity for action. Alternate translation: “and they compelled them to stop”

See: Metaphor

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:23 (#11) (id: 462940)

וּ⁠בַטִּ֥לוּ הִמּ֖וֹ בְּ⁠אֶדְרָ֥ע וְ⁠חָֽיִל

**"and they stopped them by an arm and strength"**

The implication is that these officials made the Jews stop rebuilding the walls and houses in Jerusalem. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “and they compelled them to stop rebuilding the walls and houses in Jerusalem”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:23
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Rehum (ID: `person:Rehum.2`); Shimshai (ID: `person:Shimshai`); Jews (ID: `group:Jews`); Artaxerxes (ID: `person:Artaxerxes.2`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 4:24 (#1) (id: 462941)

בֵּ⁠אדַ֗יִן בְּטֵלַת֙ עֲבִידַ֣ת בֵּית־אֱלָהָ֔⁠א דִּ֖י בִּ⁠ירוּשְׁלֶ֑ם

**"At that time the work of the house of God which is in Jerusalem stopped"**

The book now returns to the point in the story where it left off at the end of [4:5](https://ref.ly/Ezra4:5). (If it would be helpful, review the note to [4:6](https://ref.ly/Ezra4:6). That note explains how the book tells of later events in [4:6–23](https://ref.ly/Ezra4:6-Ezra4:23) to show that the Jews were wise not to trust the Samaritans when they offered to help rebuild the temple during the reign of Cyrus, since they demonstrated by their repeated future opposition that they really were the enemies of the Jews.) It might be useful to include a heading before this verse to alert the reader that there is a change of timeframe at this point, such as, “Building the Temple in the Time of Darius.” Also, if it would be helpful to your readers, you could say something explicitly here that would show how the book is returning to the time of Cyrus and his immediate successors. Alternate translation: “This is how the enemies of the Jews tried, under later kings, to stop them from rebuilding Jerusalem. They attempted the same thing under King Cyrus and his successors and they were able to stop the work on the temple for a time”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 4:24
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 4:24 (#2) (id: 462942)

וַ⁠הֲוָת֙ בָּֽטְלָ֔א

**"and it was stopped"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “and the Jews did not start rebuilding again”

See: Active or Passive

* **Associated Passages:** Ezra 4:24
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 4:24 (#3) (id: 462943)

**"Darius"**

**Darius** is the name of a man. See how you translated it in [4:5](https://ref.ly/Ezra4:5).

See: How to Translate Names

* **Associated Passages:** Ezra 4:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:1 (#1) (id: 462944)

וְ⁠הִתְנַבִּ֞י

**"Then"** \- **"prophesied"**

Here, the word **Then** introduces a new event in the story. Use a natural method in your language to introduce a new event here.

See: Introduction of a New Event

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:1 (#2) (id: 462945)

וְ⁠הִתְנַבִּ֞י

**"Then"** \- **"prophesied"**

The word **Then** indicates that the event the story will now relate took place at the time just named in [4:24](https://ref.ly/Ezra4:24), that is, in the second year of the reign of Darius as king of Persia. Alternate translation: “At that time … prophesied”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:1 (#3) (id: 462946)

וּ⁠זְכַרְיָ֤ה בַר־עִדּוֹא֙

**"and Zechariah, the son of Iddo"**

**Zechariah** is the name of a man, and **Iddo** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:1 (#4) (id: 462947)

חַגַּ֣י

**"Haggai"**

**Haggai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:1 (#5) (id: 462948)

וְ⁠הִתְנַבִּ֞י \& עַל־יְה֣וּדָיֵ֔⁠א דִּ֥י בִ⁠יה֖וּד וּ⁠בִ⁠ירוּשְׁלֶ֑ם

**"Then"** \- **"prophesied to the Jews who were in Judah and in Jerusalem"**

The implication, based on how the Jewish leaders respond in the next verse, is that Haggai and Zechariah said specifically that the Jews should resume work on rebuilding the temple. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “told the Jews in Judah and Jerusalem that they should start rebuilding the temple again”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:1 (#6) (id: 462949)

בְּ⁠שֻׁ֛ם אֱלָ֥הּ יִשְׂרָאֵ֖ל

**"in the name of the God of Israel"**

This expression means that Haggai and Zechariah made clear that they were speaking to the Jews on behalf of God, as if God were speaking through them. Alternate translation: “as messengers of the God of Israel”

See: Idiom

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:1 (#7) (id: 462950)

אֱלָ֥הּ יִשְׂרָאֵ֖ל

**"the God of Israel"**

As in [4:1](https://ref.ly/Ezra4:1), this phrase provides further background information about Yahweh. Alternate translation: “the God whom the people of Israel worshiped”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:1 (#8) (id: 462951)

עֲלֵי⁠הֽוֹן

**"who was over them"**

God is described here in a spatial metaphor as being higher than or above the Jews to indicate that they acknowledged his authority and they served and obeyed him. Alternate translation: “whom they obeyed” or “who was their king”

See: Metaphor

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haggai (ID: `person:Haggai`); Zechariah (ID: `person:Zechariah.17`); Iddo (ID: `person:Iddo.6`); Jews (ID: `group:Jews`); Judah (ID: `person:Judah.4`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`)

## Ezra 5:2 (#1) (id: 462952)

בֵּ⁠אדַ֡יִן

**"Then"**

Here, **Then** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 5:2
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Jehozadak (ID: `person:Jehozadak`); House (ID: `realia:House`)

## Ezra 5:2 (#2) (id: 462953)

זְרֻבָּבֶ֤ל בַּר־שְׁאַלְתִּיאֵל֙

**"Zerubbabel, the son of Shealtiel"**

**Zerubbabel** is the name of a man, and **Shealtiel** is the name of his father. See how you translated these names in [3:2](https://ref.ly/Ezra3:2).

See: How to Translate Names

* **Associated Passages:** Ezra 5:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Jehozadak (ID: `person:Jehozadak`); House (ID: `realia:House`)

## Ezra 5:2 (#3) (id: 462954)

וְ⁠יֵשׁ֣וּעַ בַּר־יֽוֹצָדָ֔ק

**"and Jeshua, the son of Jozadak"**

**Jeshua** is the name of a man, and **Jozadak** is the name of his father. See how you translated these names in [3:2](https://ref.ly/Ezra3:2).

See: How to Translate Names

* **Associated Passages:** Ezra 5:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Jehozadak (ID: `person:Jehozadak`); House (ID: `realia:House`)

## Ezra 5:2 (#4) (id: 462955)

קָ֠מוּ

**"arose"**

In this context, the word **arose** means that they took action to get a project under way. It does not indicate that these leaders had been sitting or lying down and that they stood up. Alternate translation: “took action”

See: Idiom

* **Associated Passages:** Ezra 5:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Jehozadak (ID: `person:Jehozadak`); House (ID: `realia:House`)

## Ezra 5:2 (#5) (id: 462956)

וְ⁠שָׁרִ֣יו לְ⁠מִבְנֵ֔א בֵּ֥ית אֱלָהָ֖⁠א

**"and began to build the house of God"**

Alternate translation: “and resumed work on rebuilding the temple”

See: Metaphor

* **Associated Passages:** Ezra 5:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Joshua (ID: `person:Joshua.5`); Shealtiel (ID: `person:Shealtiel.2`); Jerusalem (ID: `place:Jerusalem`); Jehozadak (ID: `person:Jehozadak`); House (ID: `realia:House`)

## Ezra 5:3 (#1) (id: 462957)

בֵּ⁠הּ־זִמְנָ⁠א֩

**"At that time"**

The phrase **at that time** indicates that this event took place at the same time as the event that the story has just related. If it would be helpful in your language, you could show this relationship in this case by using a phrase such as “when they did this”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#2) (id: 462958)

תַּ֠תְּנַי

**"Tattenai"**

**Tattenai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#3) (id: 462959)

עֲבַֽר־נַהֲרָ֛⁠ה

**"Beyond\-the\-River"**

**Beyond\-the\-River** is the name of the province of the Persian Empire that included Judah and Jerusalem. See how you translated it in [4:10](https://ref.ly/Ezra4:10).

See: How to Translate Names

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#4) (id: 462960)

וּ⁠שְׁתַ֥ר בּוֹזְנַ֖י

**"and Shethar\-Bozenai"**

**Shethar\-Bozenai** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#5) (id: 462961)

וּ⁠כְנָוָתְ⁠ה֑וֹן

**"and their companions"**

As often in the book, the term **companions** here indicates people who hold similar positions. Alternate translation: “and their associates”

See: Idiom

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#6) (id: 462962)

אֲתָ֨א

**"came"**

Use the motion verb that is most natural in your language. Alternate translation: “went” or “traveled”

See: Go and Come

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** Go and Come (ID: `33278@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#7) (id: 462963)

**"Who set for you a decree"**

See how you translated this word for **decree** in [4:19](https://ref.ly/Ezra4:19) and [4:21](https://ref.ly/Ezra4:21). Alternate translation: “Who gave you permission” or “Who authorized you”

* **Associated Passages:** Ezra 5:3

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#8) (id: 462964)

בַּיְתָ֤⁠א דְנָה֙ לִ⁠בְּנֵ֔א וְ⁠אֻשַּׁרְנָ֥⁠א דְנָ֖ה לְ⁠שַׁכְלָלָֽה

**"to build this house and to complete this structure"**

These two phrases mean essentially the same thing. As noted in [4:12](https://ref.ly/Ezra4:12), **complete** is another way of saying **build** when the terms are paired like this. The officials say essentially the same thing twice for emphasis. If that would be confusing for your readers, you could say it once and provide emphasis in a different way. Alternate translation: “to build a large temple such as this”

See: Parallelism

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:3 (#9) (id: 462965)

בַּיְתָ֤⁠א דְנָה֙

**"this house"**

“this temple”

See: Metaphor

* **Associated Passages:** Ezra 5:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 5:4 (#1) (id: 462966)

אֱדַ֥יִן

**"Then"**

Here, the word **Then** is indicating that the enemies asked the question in this verse right after the question they asked in the previous verse. (The word is not indicating that the two questions were asked on separate occasions.) Alternate translation: “In addition”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 5:4
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)


## Ezra 5:4 (#2) (id: 462967)

אֲמַ֣רְנָא לְּ⁠הֹ֑ם מַן־אִנּוּן֙ שְׁמָהָ֣ת גֻּבְרַיָּ֔⁠א

**"we said to them, “What are the names of the men"**

Here, the Aramaic text reads **we said**. The pronoun does not seem to fit the context here, because of the sudden, unexplained shift from third person “they” in verse 3 to first person “we” here. In [5:10](https://ref.ly/Ezra5:10), in their letter to Darius, Tattenai and his associates use “we” when they report that they were the ones who asked this question. So the word “we” could have been mistakenly copied into this verse from that one. Other ancient versions say “they,” and it will likely be clearest for your readers if you translate the phrase in that way. Alternate translation: “they said to them, ‘What are the men's names’” or “they said to them, ‘Who are the men’” It is also possible that the verse is not reporting a question in direct speech, but is simply describing what was said as a statement. In that case, **we** would refer to the Jews. Alternate translation: “we told them who the men were” or “we told them the names of the men \[who were building this building.”]

See: Textual Variants

* **Associated Passages:** Ezra 5:4
* **Associated Articles:** Textual Variants (ID: `33405@UWTranslationManual`)


## Ezra 5:4 (#3) (id: 462968)

אֲמַ֣רְנָא לְּ⁠הֹ֑ם מַן־אִנּוּן֙ שְׁמָהָ֣ת גֻּבְרַיָּ֔⁠א דִּֽי־דְנָ֥ה בִנְיָנָ֖⁠א בָּנַֽיִן

**"we said to them, “What are the names of the men who are building this building"**

If it would be more natural in your language, you could present this sentence as an indirect quotation, as in the UST.

See: Direct and Indirect Quotations

* **Associated Passages:** Ezra 5:4
* **Associated Articles:** Direct and Indirect Quotations (ID: `33305@UWTranslationManual`)


## Ezra 5:5 (#1) (id: 462969)

וְ⁠עֵ֣ין

**"But the eye of"**

Here, the word **But** indicates that the sentence it introduces draws a contrast between what Tattenai and his associates were trying to accomplish, which was an immediate end to the rebuilding of the temple, and what actually happened. You could begin the sentence with a word such as “however” to indicate this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 5:5
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jews (ID: `group:Jews`)

## Ezra 5:5 (#2) (id: 462970)

וְ⁠עֵ֣ין אֱלָהֲ⁠הֹ֗ם הֲוָת֙ עַל־שָׂבֵ֣י יְהוּדָיֵ֔⁠א

**"But the eye of God was on the elders of the Jews"**

Here, **eye** stands for seeing, and in this context, seeing means giving care, protection, and favor. Alternate translation: “God was making sure that the Jewish leaders would be alright”

See: Metaphor

* **Associated Passages:** Ezra 5:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jews (ID: `group:Jews`)

## Ezra 5:5 (#3) (id: 462971)

וְ⁠לָא

**"and"** \- **"not"**

Here the word **and** indicates that the clause it introduces explains the results of what the previous sentence described. Alternate translation: “as a result … not”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 5:5
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jews (ID: `group:Jews`)

## Ezra 5:5 (#4) (id: 462972)

וְ⁠לָא־בַטִּ֣לוּ הִמּ֔וֹ עַד־טַעְמָ֖⁠א לְ⁠דָרְיָ֣וֶשׁ יְהָ֑ךְ

**"and they did not stop them until the report came to Darius"**

Here the word **they** refers to Tattenai and his associates. The implication is that they did not make the Jewish leaders stop rebuilding the temple right away. Rather, they chose to wait until they could send a report about the rebuilding to Darius, to see what he would say about it. Alternate translation: “and Tattenai and his associates did not make the Jews stop rebuilding the temple right away, but they decided instead to report the matter to King Darius”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 5:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jews (ID: `group:Jews`)

## Ezra 5:5 (#5) (id: 462973)

וֶ⁠אֱדַ֛יִן

**"and then"**

This phrase indicates that this event would take place after the event the story has just described.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 5:5
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jews (ID: `group:Jews`)

## Ezra 5:5 (#6) (id: 462974)

יְתִיב֥וּן נִשְׁתְּוָנָ֖⁠א עַל־דְּנָֽה

**"they returned the letter concerning this matter"**

Here the word **they** primarily refers to Darius. As in [4:18](https://ref.ly/Ezra4:18), the plural may be used to indicate the king and his officials, who would consider the matter openly in the royal court. If this is confusing in your language, you can use the singular. Alternate translation: “he sent back a letter to tell them what to do about this”

* **Associated Passages:** Ezra 5:5

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jews (ID: `group:Jews`)

## Ezra 5:6 (#1) (id: 462975)

**"Tattenai,"** \- **"and Shethar\-Bozenai"**

These are the names of two men. See how you translated them in [5:3](https://ref.ly/Ezra5:3).

See: How to Translate Names

* **Associated Passages:** Ezra 5:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Darius (ID: `person:Darius.2`); Euphrates River (ID: `place:Euphrates`)

## Ezra 5:6 (#2) (id: 462976)

וּ⁠כְנָ֣וָתֵ֔⁠הּ

**"and his companions"**

Alternate translation: “and their colleagues”

* **Associated Passages:** Ezra 5:6

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Darius (ID: `person:Darius.2`); Euphrates River (ID: `place:Euphrates`)

## Ezra 5:7 (#1) (id: 462977)

לְ⁠דָרְיָ֥וֶשׁ מַלְכָּ֖⁠א שְׁלָמָ֥⁠א כֹֽלָּ⁠א

**"To Darius the king: All peace"**

Here the book begins to quote the letter that Tattenai and his associates sent to King Darius. It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 5:7
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`)

## Ezra 5:7 (#2) (id: 462978)

**"To Darius"**

**Darius** is the name of a man. See how you translated it in [4:5](https://ref.ly/Ezra4:5).

* **Associated Passages:** Ezra 5:7

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`)

## Ezra 5:7 (#3) (id: 462979)

שְׁלָמָ֥⁠א כֹֽלָּ⁠א

**"All peace"**

As in [4:17](https://ref.ly/Ezra4:17), this is a conventional greeting or good wish that senders at this time often included at the beginning of a letter. If your language has a similar expression that it uses for the same purpose, you can use it here. Alternate translation: “Greetings” or “We hope all is well with you”

* **Associated Passages:** Ezra 5:7

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`)

## Ezra 5:8 (#1) (id: 462980)

יְדִ֣יעַ ׀ לֶהֱוֵ֣א לְ⁠מַלְכָּ֗⁠א

**"Let it be known to the king"**

These officials address the king in the third person as a sign of respect. If it would be helpful in your language, you could indicate this respect by using an expression such as “O king.” Alternate translation: “We would like you to know, O king”

See: First, Second or Third Person

* **Associated Passages:** Ezra 5:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Hewn Stone (ID: `realia:HewnStone`); House (ID: `realia:House`)

## Ezra 5:8 (#2) (id: 462981)

יְדִ֣יעַ ׀ לֶהֱוֵ֣א לְ⁠מַלְכָּ֗⁠א

**"Let it be known to the king"**

If it would be helpful in your language, you could say this with an active form, and you could say who is doing the action. Alternate translation: “We would like you to know, O king”

See: Active or Passive

* **Associated Passages:** Ezra 5:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Hewn Stone (ID: `realia:HewnStone`); House (ID: `realia:House`)

## Ezra 5:8 (#3) (id: 462982)

אֲזַ֜לְנָא לִ⁠יה֤וּד מְדִֽינְתָּ⁠א֙

**"we went to Judah, the province"**

Alternate translation: “we went to the province of Judah”

* **Associated Passages:** Ezra 5:8

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Hewn Stone (ID: `realia:HewnStone`); House (ID: `realia:House`)

## Ezra 5:8 (#4) (id: 462983)

לְ⁠בֵית֙ אֱלָהָ֣⁠א רַבָּ֔⁠א

**"to the house of the great God"**

Here, **the house** refers to the temple. The officials speak of it as if it were a place in which God would live. (They likely say **the great God** because the Jews told them, as they report in [5:11](https://ref.ly/Ezra5:11), that it would be a temple for the God who made heaven and earth.) Alternate translation: “to the temple of the mighty God”

See: Metaphor

* **Associated Passages:** Ezra 5:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Hewn Stone (ID: `realia:HewnStone`); House (ID: `realia:House`)

## Ezra 5:8 (#5) (id: 462984)

וְ⁠ה֤וּא מִתְבְּנֵא֙ אֶ֣בֶן גְּלָ֔ל וְ⁠אָ֖ע מִתְּשָׂ֣ם בְּ⁠כֻתְלַיָּ֑⁠א

**"and it is being built of large stones, and timber is being placed in the walls"**

If it would be helpful in your language, you could say these things with active forms, and you could say who was doing these actions. Alternate translation: “the Jews are building the temple out of large stones and setting timber beams in the walls”

See: Active or Passive

* **Associated Passages:** Ezra 5:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Hewn Stone (ID: `realia:HewnStone`); House (ID: `realia:House`)

## Ezra 5:8 (#6) (id: 462985)

וַ⁠עֲבִ֥ידְתָּ⁠א דָ֛ךְ אָסְפַּ֥רְנָא מִתְעַבְדָ֖א וּ⁠מַצְלַ֥ח בְּ⁠יֶדְ⁠הֹֽם

**"And this work is being done diligently and is succeeding in their hand"**

You could say this with an active form, and you could say who was doing the action. The term **diligently**, used here and several more times in the book, means carefully, exactly, and efficiently. Alternate translation: “And they are doing the work carefully and efficiently and successfully”

See: Active or Passive

* **Associated Passages:** Ezra 5:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Hewn Stone (ID: `realia:HewnStone`); House (ID: `realia:House`)

## Ezra 5:8 (#7) (id: 462986)

וּ⁠מַצְלַ֥ח בְּ⁠יֶדְ⁠הֹֽם

**"and is succeeding in their hand"**

Here, **hand** represents control and action. Alternate translation: “and it is succeeding at their initiative” or “and they have been able to advance the work successfully”

See: Metaphor

* **Associated Passages:** Ezra 5:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judah (ID: `person:Judah.4`); Hewn Stone (ID: `realia:HewnStone`); House (ID: `realia:House`)

## Ezra 5:9 (#1) (id: 462987)

מַן־שָׂ֨ם לְ⁠כֹ֜ם טְעֵ֗ם בַּיְתָ֤⁠א דְנָה֙ לְ⁠מִבְנְיָ֔ה וְ⁠אֻשַּׁרְנָ֥⁠א דְנָ֖ה לְ⁠שַׁכְלָלָֽה

**"Who set a decree for you to build this house and to complete this structure"**

This is a quotation within a quotation. That is, the book is quoting from the letter that Tattenai and his associates sent to King Darius, and within that letter, they are quoting what they asked the Jewish elders. It may be helpful to your readers to indicate this by setting off these words within secondary quotation marks or with whatever other punctuation or convention your language uses to indicate a quotation within a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 5:9
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 5:9 (#2) (id: 462988)

מַן־שָׂ֨ם לְ⁠כֹ֜ם טְעֵ֗ם בַּיְתָ֤⁠א דְנָה֙ לְ⁠מִבְנְיָ֔ה וְ⁠אֻשַּׁרְנָ֥⁠א דְנָ֖ה לְ⁠שַׁכְלָלָֽה

**"Who set a decree for you to build this house and to complete this structure"**

See how you translated this question in [5:3](https://ref.ly/Ezra5:3). The phrases **build this house** and **complete this structure** refer to the same action, not different actions. The officials say essentially the same thing twice for emphasis. If that would be confusing for your readers, you could say it once and provide emphasis in a different way. Alternate translation: “Who said that you could build a large temple such as this”

See: Parallelism

* **Associated Passages:** Ezra 5:9
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 5:10 (#1) (id: 462989)

וְ⁠אַ֧ף שְׁמָהָתְ⁠הֹ֛ם שְׁאֵ֥לְנָא לְּ⁠הֹ֖ם לְ⁠הוֹדָעוּתָ֑⁠ךְ דִּ֛י נִכְתֻּ֥ב שֻׁם־גֻּבְרַיָּ֖⁠א דִּ֥י בְ⁠רָאשֵׁי⁠הֹֽם

**"And also we asked of them their names, to let you know, so that we could write down the names of the men who were at their head"**

If it would be helpful in your language, you could put the reason before the result. (We do not know if Tattenai and his associates succeeded in writing down the list of names.) Alternate translation: “We wanted to write down the names of their leaders and to be prepared to let you know who they were, so we also asked them their names”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 5:10
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)


## Ezra 5:10 (#2) (id: 462990)

גֻּבְרַיָּ֖⁠א דִּ֥י בְ⁠רָאשֵׁי⁠הֹֽם

**"the men who were at their head"**

Here, **head** is a figurative way of referring to a leader. Alternate translation: “the men who were their leaders”

See: Metaphor

* **Associated Passages:** Ezra 5:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## Ezra 5:11 (#1) (id: 462992)

אֲנַ֣חְנָא הִמּ֡וֹ עַבְדוֹ⁠הִי֩ דִֽי־אֱלָ֨הּ שְׁמַיָּ֜⁠א וְ⁠אַרְעָ֗⁠א

**"We are servants of him who is the God of heaven and earth"**

The Jewish leaders describe themselves as **servants** of God because they are speaking of a superior with humility and respect. However, in this context, the expression is also an idiom that indicates that this is the God whom they worship. Alternate translation: “We worship the God who created heaven and earth”

See: Idiom

* **Associated Passages:** Ezra 5:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); House (ID: `realia:House`)

## Ezra 5:11 (#2) (id: 462993)

אֱלָ֨הּ שְׁמַיָּ֜⁠א וְ⁠אַרְעָ֗⁠א

**"is the God of heaven and earth"**

The implications of this phrase are that God created heaven and earth and therefore rightfully rules over them. Alternate translation: “is the God who created heaven and earth and rules over them.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 5:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); House (ID: `realia:House`)

## Ezra 5:11 (#3) (id: 462994)

וּ⁠בָנַ֤יִן בַּיְתָ⁠א֙

**"and we are building the house"**

**House** means the temple. This seems to be an abbreviated way of saying the “house of God,” a figurative description of the temple as the place where God lived. Alternate translation: “and we are rebuilding the temple”

See: Metaphor

* **Associated Passages:** Ezra 5:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); House (ID: `realia:House`)

## Ezra 5:11 (#4) (id: 462995)

וּ⁠מֶ֤לֶךְ לְ⁠יִשְׂרָאֵל֙ רַ֔ב בְּנָ֖⁠הִי וְ⁠שַׁכְלְלֵֽ⁠הּ

**"and a great king of Israel built it and completed it"**

In this section of the book, **completed** is another way of saying **built** when the two words are used together. (Review the note about this at [4:12](https://ref.ly/Ezra4:12) if that would be helpful.) So the Jewish elders are expressing a single idea by using two words connected with **and**. If it would be helpful in your language, you could express the meaning with a single word. Alternate translation: “which a great king of Israel constructed”

See: Hendiadys

* **Associated Passages:** Ezra 5:11
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); House (ID: `realia:House`)

## Ezra 5:11–16 (#1) (id: 462991)

אֲנַ֣חְנָא הִמּ֡וֹ עַבְדוֹ⁠הִי֩ דִֽי־אֱלָ֨הּ שְׁמַיָּ֜⁠א וְ⁠אַרְעָ֗⁠א

**"We are servants of him who is the God of heaven and earth"**

Starting here, and through [5:16](https://ref.ly/Ezra5:16), there is another quotation within a quotation. The book is quoting from the letter that Tattenai and his associates sent to King Darius, and within that letter, they are quoting what the Jewish elders told them in response to their question. It may be helpful to your readers to indicate this with an opening secondary quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation within a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 5:11
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`); Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Israel (ID: `person:Israel`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); Foundation (ID: `realia:Foundation`); Money (ID: `realia:Money`)

## Ezra 5:12 (#1) (id: 462996)

לָהֵ֗ן

**"However"**

The word **However** indicates that there will be a contrast between the sentence that it introduces and the previous one. This sentence expresses the opposite of what someone would hope and expect to happen after a great king built a temple for God, so you could begin the sentence with a word such as “unfortunately” to indicate this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:12 (#2) (id: 462997)

מִן־דִּ֨י הַרְגִּ֤זוּ אֲבָהֳתַ֨⁠נָא֙ לֶ⁠אֱלָ֣הּ שְׁמַיָּ֔⁠א

**"because of this, that our fathers caused the God of heaven to be angry"**

Here, **fathers** means ancestors. Alternate translation: “because our ancestors angered the God who rules in heaven”

See: Metaphor

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:12 (#3) (id: 462998)

יְהַ֣ב הִמּ֔וֹ בְּ⁠יַ֛ד נְבוּכַדְנֶצַּ֥ר

**"he gave them into the hand of Nebuchadnezzar"**

Here, **hand** represents power and control. Alternate translation: “God allowed Nebuchadnezzar to conquer them”

See: Metaphor

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:12 (#4) (id: 462999)

יְהַ֣ב הִמּ֔וֹ בְּ⁠יַ֛ד נְבוּכַדְנֶצַּ֥ר

**"he gave them into the hand of Nebuchadnezzar"**

Nebuchadnezzar alone did not conquer the kingdom of Judah. Rather, the elders are describing his armies by reference to something associated with them, the emperor who commanded them. Alternate translation: “God allowed them to be conquered by the armies of Nebuchadnezzar”

See: Metonymy

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:12 (#5) (id: 463000)

נְבוּכַדְנֶצַּ֥ר מֶֽלֶךְ־בָּבֶ֖ל

**"Nebuchadnezzar, the king of Babylon"**

**Nebuchadnezzar** is the name of a man, and **Babylon** is the name of the empire that he ruled.

See: How to Translate Names

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:12 (#6) (id: 463001)

כַּסְדָּאָ֑ה

**"the Chaldean"**

Here the elders provide some further background information about who Nebuchadnezzar was. Even though he was the ruler of the Babylonian Empire, he was not from the Babylonian people group himself, but rather from the Chaldean people group. Alternate translation: “who was from the Chaldean people group”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:12 (#7) (id: 463002)

כַּסְדָּאָ֑ה

**"the Chaldean"**

**Chaldean** is the name of a people group.

See: How to Translate Names

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:12 (#8) (id: 463003)

וּ⁠בַיְתָ֤⁠ה דְנָה֙ סַתְרֵ֔⁠הּ

**"and he destroyed this house"**

As in [5:11](https://ref.ly/Ezra5:11), **house** seems to be an abbreviated way of saying “house of God” or “temple.” Alternate translation: “and Nebuchadnezzar destroyed the original temple”

See: Metaphor

* **Associated Passages:** Ezra 5:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Chaldeans (ID: `group:Chaldea`); Chaldea (ID: `place:Chaldea`); House (ID: `realia:House`)

## Ezra 5:13 (#1) (id: 463004)

בְּרַם֙

**"But"**

Here, the word **But** indicates that the sentence it introduces draws a contrast between the way Nebuchadnezzar destroyed the temple and the way Cyrus commanded it to be rebuilt. Use a natural way in your language to indicate this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 5:13
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); House (ID: `realia:House`)

## Ezra 5:13 (#2) (id: 463005)

בִּ⁠שְׁנַ֣ת חֲדָ֔ה לְ⁠כ֥וֹרֶשׁ

**"in year one of Cyrus"**

The original text uses a cardinal number here, **one**, but there is not a significant difference in meaning between that and the way it uses an ordinal number, “first,” in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of years, you can do that here in your translation. Alternate translation: “in the first year of Cyrus”

See: Ordinal Numbers

* **Associated Passages:** Ezra 5:13
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); House (ID: `realia:House`)

## Ezra 5:13 (#3) (id: 463006)

**"of Cyrus"**

**Cyrus** is the name of a man. See how you translated it in [1:1](https://ref.ly/Ezra1:1).

See: How to Translate Names

* **Associated Passages:** Ezra 5:13
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); House (ID: `realia:House`)

## Ezra 5:13 (#4) (id: 463007)

מַלְכָּ֖⁠א דִּ֣י בָבֶ֑ל

**"the king of Babylon"**

Cyrus was the king of the Persian Empire, but since under his leadership the Persians had conquered the Babylonians, he also had the right to use the title **king of Babylon**. The implication may be that as the heir and successor to the throne of Babylon, Cyrus had the same power over the temple as Nebuchadnezzar, and so he could order it to be rebuilt. Alternate translation: “who conquered Babylon.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 5:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); House (ID: `realia:House`)

## Ezra 5:14 (#1) (id: 463008)

וְ֠⁠אַף מָאנַיָּ֣⁠א דִֽי־בֵית־אֱלָהָ⁠א֮ דִּ֣י דַהֲבָ֣⁠ה וְ⁠כַסְפָּ⁠א֒ דִּ֣י נְבוּכַדְנֶצַּ֗ר הַנְפֵּק֙ מִן־הֵֽיכְלָ⁠א֙ דִּ֣י בִֽ⁠ירוּשְׁלֶ֔ם וְ⁠הֵיבֵ֣ל הִמּ֔וֹ לְ⁠הֵיכְלָ֖⁠א דִּ֣י בָבֶ֑ל הַנְפֵּ֨ק הִמּ֜וֹ כּ֣וֹרֶשׁ מַלְכָּ֗⁠א מִן־הֵֽיכְלָ⁠א֙ דִּ֣י בָבֶ֔ל וִ⁠יהִ֨יבוּ֙ לְ⁠שֵׁשְׁבַּצַּ֣ר

**"And also the vessels of the house of God, of gold and silver, that Nebuchadnezzar had taken out from the temple that was in Jerusalem and had brought them to the temple of Babylon—Cyrus the king took them out from the temple of Babylon and they were given to"** \- **"Sheshbazzar"**

This sentence may be hard for readers to follow because, in it, the direct object (that is, the thing that receives the action) comes first, and it consists of a very long phrase. If it would be helpful in your language, you could break this sentence into two sentences. Alternate translation: “Nebuchadnezzar had taken the gold and silver objects that were used in worship out of the temple in Jerusalem and brought them to the temple in Babylon. King Cyrus took them out of the temple in Babylon and gave them to … Sheshbazzar” See also the UST.

* **Associated Passages:** Ezra 5:14

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 5:14 (#2) (id: 463009)

מָאנַיָּ֣⁠א דִֽי־בֵית־אֱלָהָ⁠א֮

**"the vessels of the house of God"**

This means specifically the bowls, basins, and other objects listed in [1:9–10](https://ref.ly/Ezra1:9-Ezra1:10). These **vessels** were used during worship in the temple. Alternate translation: “the objects that were used in worship in the temple”

See: Translate Unknowns

* **Associated Passages:** Ezra 5:14
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 5:14 (#3) (id: 463010)

וְ⁠הֵיבֵ֣ל

**"and had brought"**

If it is more natural in your language, rather than **had brought**, you could say “had taken”

See: Go and Come

* **Associated Passages:** Ezra 5:14
* **Associated Articles:** Go and Come (ID: `33278@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 5:14 (#4) (id: 463011)

וִ⁠יהִ֨יבוּ֙

**"and they were given"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “and Cyrus presented them”

See: Active or Passive

* **Associated Passages:** Ezra 5:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 5:14 (#5) (id: 463012)

**"to"** \- **"Sheshbazzar"**

**Sheshbazzar** is the name of a man. See how you translated it in [1:8](https://ref.ly/Ezra1:8).

See: How to Translate Names

* **Associated Passages:** Ezra 5:14
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerubbabel (ID: `person:Zerubbabel.2`); Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 5:15 (#1) (id: 463013)

וַ⁠אֲמַר

**"Then he said"**

Here, **then** indicates that the event that the elders will now describe came after the events that they have just described. In your translation, you can use the expression in your language that would best indicate this relationship.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 5:15
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:15 (#2) (id: 463014)

וַ⁠אֲמַר־לֵ֓⁠הּ

**"Then he said to him"**

This means “Then Cyrus said to Sheshbazzar.”

See: Pronouns — When to Use Them

* **Associated Passages:** Ezra 5:15
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:15 (#3) (id: 463015)

אֵ֚ל מָֽאנַיָּ֔⁠א שֵׂ֚א אֵֽזֶל־אֲחֵ֣ת הִמּ֔וֹ בְּ⁠הֵיכְלָ֖⁠א דִּ֣י בִ⁠ירוּשְׁלֶ֑ם וּ⁠בֵ֥ית אֱלָהָ֖⁠א יִתְבְּנֵ֥א עַל־אַתְרֵֽ⁠הּ

**"Take away these vessels. Go deposit them in the temple that is in Jerusalem, and let the house of God be built on its place"**

This is a quotation within a quotation within a quotation. The book is quoting from the letter that Tattenai and his associates sent to King Darius. Within that letter, they are quoting what the Jewish elders told them in response to their question. And within that response, the elders are quoting what King Cyrus told Sheshbazzar to do. It may be helpful to your readers to indicate this by setting off this part of the verse within tertiary quotation marks or with whatever other punctuation or convention your language uses to indicate a quotation within a quotation within a quotation. Another option would be to present this as an indirect quotation, as in the UST.

See: Quotes within Quotes

* **Associated Passages:** Ezra 5:15
* **Associated Articles:** Quotes within Quotes (ID: `33307@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:15 (#4) (id: 463016)

וּ⁠בֵ֥ית אֱלָהָ֖⁠א יִתְבְּנֵ֥א עַל־אַתְרֵֽ⁠הּ

**"and let the house of God be built on its place"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “I authorize you to rebuild the temple on its original site”

See: Active or Passive

* **Associated Passages:** Ezra 5:15
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:16 (#1) (id: 463017)

אֱדַ֨יִן֙

**"Then"**

The elders use the word **then** to indicate that the event they will now describe came after the events they have just described. In your translation, you can use the expression in your language that would best indicate this relationship.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 5:16
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); House (ID: `realia:House`); Foundation (ID: `realia:Foundation`)

## Ezra 5:16 (#2) (id: 463018)

שֵׁשְׁבַּצַּ֣ר דֵּ֔ךְ אֲתָ֗א

**"that Sheshbazzar came"**

The implication is that he came here, that is, from the perspective of the elders who are speaking, he came to Jerusalem. Alternate translation: “that man named Sheshbazzar came here”

* **Associated Passages:** Ezra 5:16

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); House (ID: `realia:House`); Foundation (ID: `realia:Foundation`)

## Ezra 5:16 (#3) (id: 463019)

יְהַ֧ב אֻשַּׁיָּ֛⁠א

**"he laid the foundation"**

Sheshbazzar did not do the work himself. If it would be helpful in your language, you could refer to others who did the work, as in the UST.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 5:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); House (ID: `realia:House`); Foundation (ID: `realia:Foundation`)

## Ezra 5:16 (#4) (id: 463020)

וּ⁠מִן־אֱדַ֧יִן וְ⁠עַד־כְּעַ֛ן מִתְבְּנֵ֖א וְ⁠לָ֥א שְׁלִֽם

**"And from then even until now it is being built, but it is not complete"**

In several places the book uses the terms **built and complete** together to mean “constructed.” (Review the note about this at [5:11](https://ref.ly/Ezra5:11) if that would be helpful.) But this expression means something different here. Because there is a **not** before the second verb, here the expression means that the temple was only partially rebuilt, and then the work was interrupted. Be sure that your translation shows this difference. Alternate translation: “And ever since then, we have been trying to rebuild the temple, but we still have much work to do on it”

* **Associated Passages:** Ezra 5:16

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); House (ID: `realia:House`); Foundation (ID: `realia:Foundation`)

## Ezra 5:16 (#5) (id: 463021)

מִתְבְּנֵ֖א וְ⁠לָ֥א שְׁלִֽם

**"it is being built, but it is not complete"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “we have been trying to rebuild the temple, but we still have much work to do on it”

See: Active or Passive

* **Associated Passages:** Ezra 5:16
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); House (ID: `realia:House`); Foundation (ID: `realia:Foundation`)

## Ezra 5:16 (#6) (id: 463022)

וְ⁠לָ֥א שְׁלִֽם

**"but it is not complete"**

Here the letter ends its quotation of what the Jewish elders told Tattenai and his associates. If you decided in [5:11](https://ref.ly/Ezra5:11) to mark their words as a secondary quotation, you should indicate that ending here with a closing secondary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation within a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 5:16
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Zerubbabel (ID: `person:Zerubbabel.2`); House (ID: `realia:House`); Foundation (ID: `realia:Foundation`)

## Ezra 5:17 (#1) (id: 463023)

וּ⁠כְעַ֞ן

**"And now"**

As in [4:10](https://ref.ly/Ezra4:10) and [4:11](https://ref.ly/Ezra4:11), **and now** translates an Aramaic expression that introduces the main business of a letter. In this case, it comes near the end of the letter. If your language has a comparable expression that it uses for this same purpose, you can use that in your translation.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:17 (#2) (id: 463024)

הֵ֧ן עַל־מַלְכָּ֣⁠א טָ֗ב

**"if it is good to the king"**

This is an idiom that means “if the king thinks that this is a good idea” or “if this advice is acceptable to the king.” Alternate translation: “if it seems good to you, O king”

See: Idiom

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:17 (#3) (id: 463025)

הֵ֧ן עַל־מַלְכָּ֣⁠א טָ֗ב

**"if it is good to the king"**

Tattenai and his associates address the king in third person as a form of respect. If this is confusing in your language, you can use the second person and indicate respect in another way that is natural in your language. Alternate translation: “if it seems good to you, O king”

See: First, Second or Third Person

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:17 (#4) (id: 463026)

יִ֠תְבַּקַּר

**"let a search be made"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “have your officials search”

See: Active or Passive

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:17 (#5) (id: 463027)

בְּ⁠בֵ֨ית גִּנְזַיָּ֜⁠א דִּי־מַלְכָּ֣⁠א

**"in the house of the treasures of the king"**

As [6:1](https://ref.ly/Ezra6:1) explains more specifically, this was the house of the books that told where the treasures had been deposited. That is, it is the building where valuable documents such as royal chronicles were stored and kept safe. It is called a house metaphorically as if these documents lived there. Alternate translation: “in the building where the royal archives are kept”

See: Metaphor

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:17 (#6) (id: 463028)

הֵ֣ן אִיתַ֗י דִּֽי־מִן־כּ֤וֹרֶשׁ מַלְכָּ⁠א֙ שִׂ֣ים טְעֵ֔ם לְ⁠מִבְנֵ֛א בֵּית־אֱלָהָ֥⁠א דֵ֖ךְ בִּ⁠ירוּשְׁלֶ֑ם

**"if it is that a decree was set by Cyrus the king to build this house of God in Jerusalem"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “whether King Cyrus issued a decree to rebuild the temple in Jerusalem”

See: Active or Passive

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:17 (#7) (id: 463029)

וּ⁠רְע֥וּת מַלְכָּ֛⁠א עַל־דְּנָ֖ה יִשְׁלַ֥ח עֲלֶֽי⁠נָא

**"And let him send to us the will of the king concerning this"**

Tattenai and his associates continue to address the king in third person as a form of respect. If this is confusing in your language, you can use the second person and indicate respect in another way that is natural in your language. Alternate translation: “Please tell us, O king, what you would like us to do about this”

See: First, Second or Third Person

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 5:17 (#8) (id: 463030)

וּ⁠רְע֥וּת מַלְכָּ֛⁠א עַל־דְּנָ֖ה יִשְׁלַ֥ח עֲלֶֽי⁠נָא

**"And let him send to us the will of the king concerning this"**

Here the book ends its quotation of the letter that Tattenai and his associates sent to King Darius. If you decided in [5:7](https://ref.ly/Ezra5:7) to mark their words as a quotation, you should indicate that by ending the quotation here with a closing quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 5:17
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:1 (#1) (id: 463031)

בֵּ⁠אדַ֛יִן

**"Then"**

Here, **Then** indicates that the sentence it introduces explains the results of what the previous sentences described. Alternate translation: “As a result”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:1
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Babylon (ID: `place:Babylon`); House (ID: `realia:House`); Scroll (ID: `realia:Scroll`)

## Ezra 6:1 (#2) (id: 463032)

דָּרְיָ֥וֶשׁ מַלְכָּ֖⁠א שָׂ֣ם טְעֵ֑ם וּ⁠בַקַּ֣רוּ

**"Darius the king set a decree and they searched"**

The implication is that King Darius ordered his officials to search in the royal archives to investigate the claims that the Jewish elders had made in response to the questions that Tattenai and his associates asked them. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “King Darius commanded his officials to investigate whether King Cyrus had issued a decree to rebuild the temple in Jerusalem by searching”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Babylon (ID: `place:Babylon`); House (ID: `realia:House`); Scroll (ID: `realia:Scroll`)

## Ezra 6:1 (#3) (id: 463033)

**"Darius"**

**Darius** is the name of a man. See how you translated it in [4:5](https://ref.ly/Ezra4:5).

See: How to Translate Names

* **Associated Passages:** Ezra 6:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Babylon (ID: `place:Babylon`); House (ID: `realia:House`); Scroll (ID: `realia:Scroll`)

## Ezra 6:1 (#4) (id: 463034)

בְּ⁠בֵ֣ית סִפְרַיָּ֗⁠א דִּ֧י גִנְזַיָּ֛⁠א מְהַחֲתִ֥ין

**"in the house of the books where the treasures had been deposited"**

This means the building or buildings where valuable documents such as royal chronicles were stored and kept safe. This is called a house as if those documents lived there. Alternate translation: “in the places where the kings stored their chronicles and other valuable items”

See: Metaphor

* **Associated Passages:** Ezra 6:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Babylon (ID: `place:Babylon`); House (ID: `realia:House`); Scroll (ID: `realia:Scroll`)

## Ezra 6:1 (#5) (id: 463035)

בְּ⁠בֵ֣ית סִפְרַיָּ֗⁠א דִּ֧י גִנְזַיָּ֛⁠א מְהַחֲתִ֥ין

**"in the house of the books where the treasures had been deposited"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “in the places where the kings stored their chronicles and other valuable items”

See: Active or Passive

* **Associated Passages:** Ezra 6:1
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Babylon (ID: `place:Babylon`); House (ID: `realia:House`); Scroll (ID: `realia:Scroll`)

## Ezra 6:1 (#6) (id: 463036)

תַּמָּ֖ה בְּ⁠בָבֶֽל

**"there in Babylon"**

This cannot mean that the search was limited to the archives in the city of Babylon or even to the region of Babylon that had become a province in the Persian Empire because a scroll describing the decree of Cyrus was finally found in the neighboring province of Media. Rather, **Babylon** must be a general description for the whole area that was the center of power for the Babylonian and Persian empires. Alternate translation: “there in Babylonia”

See: Idiom

* **Associated Passages:** Ezra 6:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Babylon (ID: `place:Babylon`); House (ID: `realia:House`); Scroll (ID: `realia:Scroll`)

## Ezra 6:2 (#1) (id: 463037)

וְ⁠הִשְׁתְּכַ֣ח \& מְגִלָּ֣ה חֲדָ֑ה

**"And one scroll was found"**

Here, **And** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result they found one scroll”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:2
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Ecbatana (ID: `place:Ecbatana`); Media (ID: `place:Media`); Scroll (ID: `realia:Scroll`)

## Ezra 6:2 (#2) (id: 463038)

וְ⁠הִשְׁתְּכַ֣ח \& מְגִלָּ֣ה חֲדָ֑ה

**"And one scroll was found"**

It is implied that the officials searched before they found the scroll. If it would be helpful in your language, you could say that explicitly. Alternate translation: “After searching in the archives, they found one scroll”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ecbatana (ID: `place:Ecbatana`); Media (ID: `place:Media`); Scroll (ID: `realia:Scroll`)

## Ezra 6:2 (#3) (id: 463039)

וְ⁠הִשְׁתְּכַ֣ח \& מְגִלָּ֣ה חֲדָ֑ה

**"And one scroll was found"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “As a result, the officials found one scroll”

See: Active or Passive

* **Associated Passages:** Ezra 6:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Ecbatana (ID: `place:Ecbatana`); Media (ID: `place:Media`); Scroll (ID: `realia:Scroll`)

## Ezra 6:2 (#4) (id: 463040)

בְּ⁠אַחְמְתָ֗א \& דִּ֛י בְּ⁠מָדַ֥י מְדִינְתָּ֖⁠ה

**"at Ecbatana that is in the province of Media"**

**Ecbatana** is the name of a city, and **Media** is the name of the province in which it was located.

See: How to Translate Names

* **Associated Passages:** Ezra 6:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ecbatana (ID: `place:Ecbatana`); Media (ID: `place:Media`); Scroll (ID: `realia:Scroll`)

## Ezra 6:2 (#5) (id: 463041)

וְ⁠כֵן־כְּתִ֥יב בְּ⁠גַוַּ֖⁠הּ דִּכְרוֹנָֽ⁠ה

**"and the record was written within it thusly"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “and the writing in the scroll said this” or “and the message in the scroll was”

See: Active or Passive

* **Associated Passages:** Ezra 6:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Ecbatana (ID: `place:Ecbatana`); Media (ID: `place:Media`); Scroll (ID: `realia:Scroll`)

## Ezra 6:3 (#1) (id: 463042)

בִּ⁠שְׁנַ֨ת חֲדָ֜ה לְ⁠כ֣וֹרֶשׁ מַלְכָּ֗⁠א

**"In year one of Cyrus the king"**

Here the book begins to quote what was said on the scroll that the officials found in Ecbatana. It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:3 (#2) (id: 463043)

בִּ⁠שְׁנַ֨ת חֲדָ֜ה לְ⁠כ֣וֹרֶשׁ מַלְכָּ֗⁠א

**"In year one of Cyrus the king"**

The Hebrew uses a cardinal number here, **one**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, “first,” in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of years, you can do that here in your translation. Alternate translation: “In the first year of the reign of Cyrus as king”

See: Ordinal Numbers

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:3 (#3) (id: 463044)

**"of Cyrus"**

**Cyrus** is the name of a man. See how you translated it in [1:1](https://ref.ly/Ezra1:1).

See: How to Translate Names

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:3 (#4) (id: 463045)

בַּיְתָ֣⁠א יִתְבְּנֵ֔א

**"Let the house be built"**

Starting here, and through [6:5](https://ref.ly/Ezra6:5), there is a quotation within a quotation. The book is quoting from the scroll that the officials found in Ecbatana, and within that scroll, the royal chronicles quote the decree of Cyrus. It may be helpful to your readers to indicate this with an opening secondary quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation within a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:3 (#5) (id: 463046)

בַּיְתָ֣⁠א יִתְבְּנֵ֔א \& וְ⁠אֻשּׁ֖וֹ⁠הִי מְסֽוֹבְלִ֑ין

**"Let the house be built,"** \- **"and let its foundations be raised"**

The words **built** and **raised** mean similar things. They are a poetic parallel, as in Isaiah 58:12, “Your ancient ruins will be built; you will raise up the foundations of many generations.” Cyrus says basically the same thing twice for emphasis and clarity. If it would be helpful in your language, you could combine these phrases. Alternate translation: “I authorize the Jews to raise up a new temple.” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase is more specific in that it implicitly indicates that the new temple is to be built on the site of the former one. Alternate translation: “I authorize the Jews to rebuild the temple … on the site of the former temple”

See: Parallelism

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:3 (#6) (id: 463047)

בַּיְתָ֣⁠א יִתְבְּנֵ֔א \& וְ⁠אֻשּׁ֖וֹ⁠הִי מְסֽוֹבְלִ֑ין

**"Let the house be built,"** \- **"and let its foundations be raised"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “I authorize the Jews to rebuild the temple … on the site of the former temple”

See: Active or Passive

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:3 (#7) (id: 463048)

בֵּית

**"the house"**

Alternate translation: “the temple”

See: Metaphor

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:3 (#8) (id: 463049)

רוּמֵ⁠הּ֙ אַמִּ֣ין שִׁתִּ֔ין פְּתָיֵ֖⁠הּ אַמִּ֥ין שִׁתִּֽין

**"Its height will be 60 cubits. Its width will be 60 cubits"**

If it would be helpful in your language, you could convert these measures into modern units. Alternate translation: “The temple is to be 90 feet high and 90 feet wide”

See: Biblical Distance

* **Associated Passages:** Ezra 6:3
* **Associated Articles:** Biblical Distance (ID: `33369@UWTranslationManual`)

* **Associated ACAI Concepts:** Cyrus (ID: `person:Cyrus`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Cubit (ID: `realia:Cubit`); Foundation (ID: `realia:Foundation`)

## Ezra 6:4 (#1) (id: 463050)

נִדְבָּכִ֞ין דִּי־אֶ֤בֶן גְּלָל֙ תְּלָתָ֔א וְ⁠נִדְבָּ֖ךְ דִּי־אָ֣ע חֲדַ֑ת

**"with three layers of large stone and a layer of new timber"**

It is no longer clear exactly what these building instructions mean. They could mean that the walls of the temple should be three layers of stone thick, with a facing of wood on the inside. They could also mean that the walls should be built with one layer of wood on top of every three layers of stone, or that the temple was to have four stories, the lower three of stone and the top one of wood. Whatever the specifics, the implication is that the temple is to be rebuilt in the same way that Solomon originally built it, since 1 Kings 6:36 says that he built its inner court, at least, “with three courses of hewn stone, and a course of cedar beams.” If it would be helpful to your readers, you could say that explicitly. Alternate translation: “and it is to be built of wood and stone just like the original one”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`); Hewn Stone (ID: `realia:HewnStone`)

## Ezra 6:4 (#2) (id: 463051)

וְ⁠נִ֨פְקְתָ֔⁠א מִן־בֵּ֥ית מַלְכָּ֖⁠א תִּתְיְהִֽב

**"And let the cost be given from the house of the king"**

Here, **house** is a metaphor for property, meaning everything that a person keeps in his house, and by extension, everything that he owns. So **house** in this context represents the wealth and income of the king. Alternate translation: “I will pay the expenses from the royal revenue”

See: Metaphor

* **Associated Passages:** Ezra 6:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`); Hewn Stone (ID: `realia:HewnStone`)

## Ezra 6:4 (#3) (id: 463052)

וְ⁠נִ֨פְקְתָ֔⁠א מִן־בֵּ֥ית מַלְכָּ֖⁠א תִּתְיְהִֽב

**"And let the cost be given from the house of the king"**

If it would be helpful in your language, you could say this with an active form, and you could say who will do the action. Alternate translation: “I will pay the expenses from the royal revenue”

See: Active or Passive

* **Associated Passages:** Ezra 6:4
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`); Hewn Stone (ID: `realia:HewnStone`)

## Ezra 6:4 (#4) (id: 463053)

וְ⁠נִ֨פְקְתָ֔⁠א מִן־בֵּ֥ית מַלְכָּ֖⁠א תִּתְיְהִֽב

**"And let the cost be given from the house of the king"**

Cyrus speaks of himself here in the third person. If this is not clear in your language, you can use the first person. Alternate translation: “I will pay the expenses from the royal revenue”

See: First, Second or Third Person

* **Associated Passages:** Ezra 6:4
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`); Hewn Stone (ID: `realia:HewnStone`)

## Ezra 6:5 (#1) (id: 463054)

מָאנֵ֣י בֵית־אֱלָהָ⁠א֮ דִּ֣י דַהֲבָ֣⁠ה וְ⁠כַסְפָּ⁠א֒ דִּ֣י נְבֽוּכַדְנֶצַּ֗ר הַנְפֵּ֛ק מִן־הֵיכְלָ֥⁠א דִי־בִ⁠ירוּשְׁלֶ֖ם וְ⁠הֵיבֵ֣ל לְ⁠בָבֶ֑ל יַהֲתִיב֗וּן

**"the vessels of the house of God, of gold and silver, that Nebuchadnezzar had taken out from the temple that was in Jerusalem and had brought to Babylon, must be returned"**

See how you translated the very similar sentence in [5:14](https://ref.ly/Ezra5:14). If it would be helpful in your language, you could put the verb first in this sentence. Alternate translation: “I hereby return to the Jews the gold and silver objects which Nebuchadnezzar took out of the temple in Jerusalem and brought to Babylon”

* **Associated Passages:** Ezra 6:5

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 6:5 (#2) (id: 463055)

מָאנֵ֣י \& יַהֲתִיב֗וּן

**"the vessels of"** \- **"must be returned"**

If it would be helpful in your language, you could say this with an active form, as an imperative. Alternate translation: “return to the Jews the objects of”

See: Active or Passive

* **Associated Passages:** Ezra 6:5
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 6:5 (#3) (id: 463056)

מָאנֵ֣י בֵית־אֱלָהָ⁠א֮ דִּ֣י דַהֲבָ֣⁠ה וְ⁠כַסְפָּ⁠א֒

**"the vessels of the house of God, of gold and silver"**

**Vessels** specifically means the bowls, basins, and other objects listed in [1:9–10](https://ref.ly/Ezra1:9-Ezra1:10). These were used during worship in the temple. Alternate translation: “the gold and silver objects that were used in worship in the temple”

See: Translate Unknowns

* **Associated Passages:** Ezra 6:5
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 6:5 (#4) (id: 463057)

וִ֠⁠יהָךְ לְ⁠הֵיכְלָ֤⁠א דִי־בִ⁠ירֽוּשְׁלֶם֙ לְ⁠אַתְרֵ֔⁠הּ וְ⁠תַחֵ֖ת בְּ⁠בֵ֥ית אֱלָהָֽ⁠א

**"And let each go to the temple that is in Jerusalem, to its place. So you must put them in the house of God"**

These two phrases mean similar things. Cyrus is saying basically the same thing twice to emphasize how important it is for his officials to carry out this command. If it is confusing for your readers to repeat both phrases, you can combine them and show the emphasis in a different way. Alternate translation: “Make sure that you put these objects back in the temple, right where they belong in the house of God in Jerusalem”

See: Parallelism

* **Associated Passages:** Ezra 6:5
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 6:5 (#5) (id: 463058)

וְ⁠תַחֵ֖ת בְּ⁠בֵ֥ית אֱלָהָֽ⁠א

**"So you must put them in the house of God"**

It may be that the sudden change to second person singular **you** here is because the document is quoting Cyrus's words, which may have been spoken to Sheshbazzar (See [5:14](https://ref.ly/Ezra5:14)). If this is confusing in your language, you can use the third person. Alternate translation: “So these things must be returned to the house of God”

See: First, Second or Third Person

* **Associated Passages:** Ezra 6:5
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 6:5 (#6) (id: 463059)

וְ⁠תַחֵ֖ת בְּ⁠בֵ֥ית אֱלָהָֽ⁠א

**"So you must put them in the house of God"**

Here the scroll ends its quotation from the decree of Cyrus. If you decided in the middle of [6:3](https://ref.ly/Ezra6:3) to mark these words as a quotation within a quotation, you should indicate that ending here with a closing secondary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation within a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 6:5
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 6:5 (#7) (id: 463060)

וְ⁠תַחֵ֖ת בְּ⁠בֵ֥ית אֱלָהָֽ⁠א

**"So you must put them in the house of God"**

Here, the text also ends its quotation from the scroll. If you decided at the start of [6:3](https://ref.ly/Ezra6:3) to mark its words as a quotation, you should indicate that ending here with a closing primary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 6:5
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 6:6 (#1) (id: 463061)

כְּעַ֡ן תַּ֠תְּנַי

**"Now Tattenai"**

Here the book leaves out some of the material that a story would ordinarily need in order to be complete. Rather than repeating the same information from the scroll, but now as a letter to the officials, it jumps right from its quotation from the scroll that was discovered at Ecbatana and into the letter that King Darius wrote to Tattenai and his associates in response to what the scroll said. You could say this explicitly if it would be helpful to your readers. Alternate translation: “When King Darius learned from the scroll that Cyrus had ordered the temple to be rebuilt, he sent Tattenai and his associates a letter in answer to their inquiry. He told them what he had learned from the scroll and then warned them, ‘Now Tattenai’”

See: Ellipsis

* **Associated Passages:** Ezra 6:6
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`)

## Ezra 6:6 (#2) (id: 463062)

כְּעַ֡ן

**"Now"**

Since the book here begins to quote the letter that King Darius sent in reply to Tattenai and his associates, it may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 6:6
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`)

## Ezra 6:6 (#3) (id: 463063)

**"Now"**

As in [4:13](https://ref.ly/Ezra4:13), [4:14](https://ref.ly/Ezra4:14), and [4:21](https://ref.ly/Ezra4:21), **now** introduces an important point within a letter. (It is similar to the expression “and now” in [4:10](https://ref.ly/Ezra4:10), [4:11](https://ref.ly/Ezra4:11), [4:17](https://ref.ly/Ezra4:17), and [5:17](https://ref.ly/Ezra5:17).) If your language has a comparable expression that it uses for this same purpose, you can use that in your translation.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 6:6
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`)

## Ezra 6:6 (#4) (id: 463064)

תַּ֠תְּנַי פַּחַ֨ת עֲבַֽר־נַהֲרָ֜⁠ה שְׁתַ֤ר בּוֹזְנַי֙ וּ⁠כְנָוָ֣תְ⁠ה֔וֹן אֲפַרְסְכָיֵ֔⁠א דִּ֖י בַּ⁠עֲבַ֣ר נַהֲרָ֑⁠ה רַחִיקִ֥ין הֲו֖וֹ מִן־תַּמָּֽה

**"Tattenai, the governor of Beyond\-the\-River, Shethar\-Bozenai, and their companions, the officials who are in Beyond\-the\-River: be far away from there"**

Since Darius addresses these men directly at the end of the sentence, instead of **their companions**, in the third person, we would expect him to say “your companions,” in the second person. So it seems that the book is compressing the letter as it quotes it. Based on the other letters that the book quotes in [4:11–16](https://ref.ly/Ezra4:11-Ezra4:16), [4:17–22](https://ref.ly/Ezra4:17-Ezra4:22), and [5:7–17](https://ref.ly/Ezra5:7-Ezra5:17), the full letter probably said something like “To Tattenai, the governor of Beyond\-the\-River, Shethar\-Bozenai, and their companions, the officials who are in Beyond\-the\-River. Peace. And now,” followed by an explanation that the court officials had discovered a scroll that recorded the decree of Cyrus. Then would follow instructions to these men, beginning “Be far away from there!” But since the quotation from this letter in the book jumps from the list of the recipients’ names right to these instructions, If it would be helpful in your language, you could use the second person throughout. Alternate translation: “Tattenai, the governor of Beyond\-the\-River, Shethar\-Bozenai, and your associates, you officials who are in Beyond\-the\-River: Be far away from there”

See: First, Second or Third Person

* **Associated Passages:** Ezra 6:6
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`)

## Ezra 6:6 (#5) (id: 463065)

**"Tattenai,"** \- **"Shethar\-Bozenai"**

These are the names of two men. See how you translated them in [5:3](https://ref.ly/Ezra5:3).

See: How to Translate Names

* **Associated Passages:** Ezra 6:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`)

## Ezra 6:6 (#6) (id: 463066)

וּ⁠כְנָוָ֣תְ⁠ה֔וֹן

**"and their companions"**

Alternate translation: “and their associates” or “and your associates”

* **Associated Passages:** Ezra 6:6

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`)

## Ezra 6:6 (#7) (id: 463067)

רַחִיקִ֥ין הֲו֖וֹ מִן־תַּמָּֽה

**"be far away from there"**

King Darius uses a spatial metaphor to indicate that he wants Tattenai and his associates not to interfere with what the Jews are doing in Jerusalem. The meaning is not simply that they are to stay away physically from that area, since they could still plot against the Jews from a distance (for example, by bribing officials in the royal court, as [4:5](https://ref.ly/Ezra4:5) describes). Alternate translation: “do not interfere with what is happening in Jerusalem”

See: Metaphor

* **Associated Passages:** Ezra 6:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tattenai (ID: `person:Tattenai`); Shethar-bozenai (ID: `person:Shethar-bozenai`); Euphrates River (ID: `place:Euphrates`)

## Ezra 6:7 (#1) (id: 463068)

שְׁבֻ֕קוּ לַ⁠עֲבִידַ֖ת בֵּית־אֱלָהָ֣⁠א דֵ֑ךְ

**"Leave alone the work of that house of God"**

This sentence means basically the same thing as the last sentence in [6:6](https://ref.ly/Ezra6:6). Darius says essentially the same thing twice for emphasis. If that would be confusing for your readers, you could combine the two sentences and express the emphasis in a different way. Alternate translation for both sentences: “do not interfere with the rebuilding of the temple in Jerusalem in any way” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase is more specific. It says precisely what Darius wants these Samaritan officials to leave alone. Alternate translation: “Do not interfere with what is happening in Jerusalem. Do not disturb the work on the temple there!”

See: Parallelism

* **Associated Passages:** Ezra 6:7
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); House (ID: `realia:House`)

## Ezra 6:7 (#2) (id: 463069)

בֵּית־אֱלָהָ֥⁠א דֵ֖ךְ יִבְנ֥וֹן עַל־אַתְרֵֽ⁠הּ

**"Let"** \- **"build that house of God on its place"**

As in [5:15](https://ref.ly/Ezra5:15), this expression means “Let … rebuild the temple on its original site.” You could say that as an alternate translation.

* **Associated Passages:** Ezra 6:7

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); House (ID: `realia:House`)

## Ezra 6:8 (#1) (id: 463070)

וּ⁠מִנִּ⁠י֮ שִׂ֣ים טְעֵם֒ לְ⁠מָ֣א דִֽי־תַֽעַבְד֗וּן עִם־שָׂבֵ֤י יְהוּדָיֵ⁠א֙ אִלֵּ֔ךְ לְ⁠מִבְנֵ֖א בֵּית־אֱלָהָ֣⁠א דֵ֑ךְ

**"And from me is set a decree for what it is that you shall do with these elders of the Jews to build that house of God"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “This is what I am commanding you to do to help the elders of the Jews rebuild that temple”

See: Active or Passive

* **Associated Passages:** Ezra 6:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 6:8 (#2) (id: 463071)

וּ⁠מִ⁠נִּכְסֵ֣י מַלְכָּ֗⁠א דִּ֚י מִדַּת֙ עֲבַ֣ר נַהֲרָ֔⁠ה אָסְפַּ֗רְנָא נִפְקְתָ֛⁠א תֶּהֱוֵ֧א מִֽתְיַהֲבָ֛א לְ⁠גֻבְרַיָּ֥⁠א אִלֵּ֖ךְ

**"And from the treasures of the king (that is, the tribute of Beyond\-the\-River) let the expense be given to those men diligently"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “I want you officials to carefully pay the full costs of the rebuilding from the royal treasury from the tribute that you receive in Beyond\-the\-River province”

See: Active or Passive

* **Associated Passages:** Ezra 6:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 6:8 (#3) (id: 463072)

וּ⁠מִ⁠נִּכְסֵ֣י מַלְכָּ֗⁠א

**"And from the treasures of the king"**

Darius speaks of himself here in the third person. If that is confusing in your language, you can use the first person. Alternate translation: “And from my royal revenue”

See: First, Second or Third Person

* **Associated Passages:** Ezra 6:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 6:8 (#4) (id: 463073)

דִּי־לָ֥א לְ⁠בַטָּלָֽא

**"so that it does not stop"**

Here, **it** means the work of rebuilding the temple. Darius is using a figure of speech that expresses a strong positive meaning by using a negative word, **not**, together with a word, **stop**, that is the opposite of the intended meaning. If it would be helpful in your language, you could say this plainly. Alternate translation: “in order to make sure that the rebuilding is completed successfully”

See: Litotes

* **Associated Passages:** Ezra 6:8
* **Associated Articles:** Litotes (ID: `33290@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 6:8 (#5) (id: 463074)

**"so that it does not stop"**

However, since the book uses the term **stop** when describing the opposition to the temple rebuilding in [4:21](https://ref.ly/Ezra4:21), [4:23](https://ref.ly/Ezra4:23), [4:24](https://ref.ly/Ezra4:24), and [5:5](https://ref.ly/Ezra5:5), you could also choose to use that term to show the connection. Alternate translation: “because I do not want anything to stop the Jews from rebuilding that temple”

* **Associated Passages:** Ezra 6:8

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Euphrates River (ID: `place:Euphrates`); House (ID: `realia:House`)

## Ezra 6:9 (#1) (id: 463075)

וּ⁠מָ֣ה חַשְׁחָ֡ן וּ⁠בְנֵ֣י תוֹרִ֣ין וְ⁠דִכְרִ֣ין וְ⁠אִמְּרִ֣ין ׀ לַ⁠עֲלָוָ֣ן ׀ לֶ⁠אֱלָ֪הּ שְׁמַיָּ֟⁠א חִנְטִ֞ין מְלַ֣ח ׀ חֲמַ֣ר וּ⁠מְשַׁ֗ח כְּ⁠מֵאמַ֨ר כָּהֲנַיָּ֤⁠א דִי־בִ⁠ירֽוּשְׁלֶם֙ לֶהֱוֵ֨א מִתְיְהֵ֥ב לְ⁠הֹ֛ם י֥וֹם ׀ בְּ⁠י֖וֹם דִּי־לָ֥א שָׁלֽוּ

**"And whatever is needed (whether sons of bulls, or rams, or lambs for burnt up offerings to the God of heaven, wheat, salt, wine, or oil, according to the command of the priests who are in Jerusalem), let it be given to them day by day (that is, without neglect"**

This sentence, like the one in [5:14](https://ref.ly/Ezra5:14), may be hard for readers to follow because the thing that receives the action comes first, and it consists of a very long phrase. This sentence continues into the next verse. If it would be helpful in your language, you could break it up into four sentences, three in this verse and the last one in verse [10](https://ref.ly/Ezra6:10). You could also use active forms instead of the two passive forms and say who would do the action in each case. Alternate translation: “The priests who are in Jerusalem will tell you what they need. This may include young bulls, rams, and lambs for burnt offerings to the God of heaven, and wheat, salt, wine, and oil. I want you to give them everything that they need every single day”

See: Active or Passive

* **Associated Passages:** Ezra 6:9
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Sheep (ID: `fauna:Lamb`); Olive Oil (ID: `realia:OliveOil`)

## Ezra 6:9 (#2) (id: 463076)

וּ⁠בְנֵ֣י תוֹרִ֣ין

**"whether sons of bulls"**

This expression refers to young bulls.

See: Idiom

* **Associated Passages:** Ezra 6:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Sheep (ID: `fauna:Lamb`); Olive Oil (ID: `realia:OliveOil`)

## Ezra 6:9 (#3) (id: 463077)

לַ⁠עֲלָוָ֣ן

**"for burnt up offerings"**

See how you translated this in [3:2](https://ref.ly/Ezra3:2). Review the note there if that would be helpful. Alternate translation: “whole burnt offerings”

* **Associated Passages:** Ezra 6:9

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Sheep (ID: `fauna:Lamb`); Olive Oil (ID: `realia:OliveOil`)

## Ezra 6:9 (#4) (id: 463078)

לֶ⁠אֱלָ֪הּ שְׁמַיָּ֟⁠א

**"to the God of heaven"**

See how you translated this expression in [5:11](https://ref.ly/Ezra5:11). Alternate translation: “the God who rules in heaven”

* **Associated Passages:** Ezra 6:9

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Sheep (ID: `fauna:Lamb`); Olive Oil (ID: `realia:OliveOil`)

## Ezra 6:9 (#5) (id: 463079)

י֥וֹם ׀ בְּ⁠י֖וֹם

**"day by day"**

**Day by day** is an idiom that means every day or every single day. Alternate translation: “every single day”

See: Idiom

* **Associated Passages:** Ezra 6:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Sheep (ID: `fauna:Lamb`); Olive Oil (ID: `realia:OliveOil`)

## Ezra 6:9 (#6) (id: 463080)

דִּי־לָ֥א שָׁלֽוּ

**"that is, without neglect"**

Here the king uses a figure of speech that expresses a strong positive meaning by using a negative word together with a word that is the opposite of the intended meaning. Alternate translation: “make sure that you do this”

See: Litotes

* **Associated Passages:** Ezra 6:9
* **Associated Articles:** Litotes (ID: `33290@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Sheep (ID: `fauna:Lamb`); Olive Oil (ID: `realia:OliveOil`)

## Ezra 6:10 (#1) (id: 463081)

דִּֽי־לֶהֱוֺ֧ן מְהַקְרְבִ֛ין נִיחוֹחִ֖ין לֶ⁠אֱלָ֣הּ שְׁמַיָּ֑⁠א וּ⁠מְצַלַּ֕יִן לְ⁠חַיֵּ֥י מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"so that they may be offering sweet\-smelling sacrifices to the God of heaven and praying for the life of the king and his sons"**

In this phrase, Darius explains the purpose for which he is commanding the actions described in the previous part of this sentence, in [6:9](https://ref.ly/Ezra6:9). If you decided to break that verse into three sentences, you can make this verse a fourth sentence of its own. If it would be natural in your language, introduce this sentence with a term to indicate that Darius is explaining his purpose here. Alternate translation: “that way, the priests can continually offer sweet\-smelling sacrifices to the God who rules in heaven and pray that God will preserve my life and the life of my sons.”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** Ezra 6:10
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)


## Ezra 6:10 (#2) (id: 463082)

נִיחוֹחִ֖ין

**"sweet\-smelling sacrifices"**

The primary idea is that these sacrifices will be pleasing to God, regardless of how they actually smell. Alternate translation: “pleasing sacrifices”

See: Metaphor

* **Associated Passages:** Ezra 6:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## Ezra 6:10 (#3) (id: 463083)

לֶ⁠אֱלָ֣הּ שְׁמַיָּ֑⁠א

**"to the God of heaven"**

See how you translated this expression in [5:11](https://ref.ly/Ezra5:11). Alternate translation: “the God who rules in heaven”

* **Associated Passages:** Ezra 6:10


## Ezra 6:10 (#4) (id: 463084)

לְ⁠חַיֵּ֥י מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"for the life of the king and his sons"**

Darius speaks of himself here in the third person. If that would be confusing in your language, you could use the first person. Alternate translation: “that God will preserve my life and the life of my sons”

See: First, Second or Third Person

* **Associated Passages:** Ezra 6:10
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Ezra 6:10 (#5) (id: 463085)

לְ⁠חַיֵּ֥י מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"for the life of the king and his sons"**

The king is speaking of the quality of his life and that of his sons, not just that they will live. Alternate translation: “that all will be well for me and my sons”

See: Synecdoche

* **Associated Passages:** Ezra 6:10
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)


## Ezra 6:10 (#6) (id: 463086)

וּ⁠בְנֽוֹ⁠הִי

**"and his sons"**

Here, **sons** could mean one of two things. (1\) It could be literal and mean the biological sons of King Darius. (2\) It could be figurative and mean descendants, specifically those whom Darius hoped would succeed him on the throne. As the note to [4:15](https://ref.ly/Ezra4:15) explains, the Persian kings spoke of predecessor kings as their “fathers,” so they may also have spoken of successor kings as their “sons.” Alternate translation: “his successors”

See: Metaphor

* **Associated Passages:** Ezra 6:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## Ezra 6:11 (#1) (id: 463087)

וּ⁠מִנִּ⁠י֮ שִׂ֣ים טְעֵם֒ דִּ֣י

**"And from me is set a decree that"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “and I am commanding that”

See: Active or Passive

* **Associated Passages:** Ezra 6:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 6:11 (#2) (id: 463088)

כָל־אֱנָ֗שׁ דִּ֤י יְהַשְׁנֵא֙ פִּתְגָמָ֣⁠א דְנָ֔ה

**"for any man who changes this edict"**

The phrase **any man who changes this edict** could mean giving different instructions from what the king had commanded, but it could also mean doing something against what his edict commands. [6:12](https://ref.ly/Ezra6:12) suggests that changing the edict means destroying the temple rather than ensuring that it was rebuilt and maintained. Alternate translation: “if anyone disobeys this edict”

See: Idiom

* **Associated Passages:** Ezra 6:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 6:11 (#3) (id: 463089)

יִתְנְסַ֥ח אָע֙ מִן־בַּיְתֵ֔⁠הּ וּ⁠זְקִ֖יף יִתְמְחֵ֣א עֲלֹ֑⁠הִי

**"a beam shall be pulled from his house, and, being erect, he shall be impaled on it"**

This is an implicit reference to one form of capital punishment that the Persians used. Darius assumes that the officials know what he means. Anyone who violates his order is to be lifted up and impaled on a beam of wood from his own house. That is, one end of the beam is to be sharpened so that it will pass through the body of the offender, the other end is to be planted in the ground, and he is to be impaled on the beam. Alternate translation: “pull a beam out of his house, sharpen one end, and lift him up and impale him on it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 6:11 (#4) (id: 463090)

וּ⁠בַיְתֵ֛⁠הּ נְוָל֥וּ יִתְעֲבֵ֖ד עַל־דְּנָֽה

**"And his house shall be made a rubbish heap on account of this"**

The expression **rubbish heap** means that the house would be so thoroughly destroyed that it would look like a garbage dump. The former site of the house would not necessarily be used as an actual dump. Alternate translation: “and as a further punishment for violating my order, I command you to demolish his house”

See: Metaphor

* **Associated Passages:** Ezra 6:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 6:11 (#5) (id: 463091)

יִתְנְסַ֥ח אָע֙ מִן־בַּיְתֵ֔⁠הּ

**"a beam shall be pulled from his house"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “you must pull a beam out of his house”

See: Active or Passive

* **Associated Passages:** Ezra 6:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 6:11 (#6) (id: 463092)

וּ⁠זְקִ֖יף יִתְמְחֵ֣א

**"and, being erect, he shall be impaled"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “then you must set the beam upright and impale him”

See: Active or Passive

* **Associated Passages:** Ezra 6:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 6:11 (#7) (id: 463093)

וּ⁠בַיְתֵ֛⁠הּ נְוָל֥וּ יִתְעֲבֵ֖ד

**"And his house shall be made a rubbish heap"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “And you must turn his house into a rubbish heap”

See: Active or Passive

* **Associated Passages:** Ezra 6:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 6:12 (#1) (id: 463094)

וֵֽ⁠אלָהָ֞⁠א דִּ֣י שַׁכִּ֧ן שְׁמֵ֣⁠הּ תַּמָּ֗ה יְמַגַּ֞ר

**"And may the God who has caused his name to dwell there overthrow"**

This is a curse formula. In addition to the threat of severe punishment in the previous verse, here Darius adds a curse from God to anyone who would disobey his order. Use a curse formula that is natural in your language.

* **Associated Passages:** Ezra 6:12

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#2) (id: 463095)

וֵֽ⁠אלָהָ֞⁠א דִּ֣י שַׁכִּ֧ן שְׁמֵ֣⁠הּ תַּמָּ֗ה יְמַגַּ֞ר

**"And may the God who has caused his name to dwell there overthrow"**

Darius speaks of God’s **name** here as if it were capable of living in a place. The phrase echoes Jewish usage, which Darius may be following deliberately to show his respect for the God whose temple he wants to be rebuilt. The phrase indicates that Jerusalem is the place from which God chose to start making himself known throughout the world. Alternate translation: “And may the God who began to make himself known from Jerusalem destroy”

See: Personification

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#3) (id: 463096)

וֵֽ⁠אלָהָ֞⁠א דִּ֣י שַׁכִּ֧ן שְׁמֵ֣⁠הּ תַּמָּ֗ה יְמַגַּ֞ר

**"And may the God who has caused his name to dwell there overthrow"**

Here, **name** is a figurative way of referring to the fame or reputation of a person. In this figure, a person's fame is described by something associated with it, which is how well known their name is and how people react to hearing it. Alternate translation: “And may the God who began to make himself known from Jerusalem destroy”

See: Metonymy

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#4) (id: 463097)

יְמַגַּ֞ר

**"may"** \- **"overthrow"**

This is an idiom that means “destroy.” Alternate translation: “may … destroy”

See: Idiom

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#5) (id: 463098)

כָּל־מֶ֤לֶךְ וְ⁠עַם֙ דִּ֣י ׀ יִשְׁלַ֣ח יְדֵ֗⁠הּ

**"any king or people who stretches out his hand"**

Here the expression “who stretches out his hand” means to seek to do harm. Alternate translation: “any king or people who, with harmful intent, attempts”

See: Idiom

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#6) (id: 463099)

לְ⁠הַשְׁנָיָ֛ה לְ⁠חַבָּלָ֛ה בֵּית־אֱלָהָ֥⁠א דֵ֖ךְ

**"to change, to destroy that house of God"**

This could mean one of two things. (1\) It could mean that no one is to try to **change** the decree of Darius, in the sense of disobeying or defying, as in the previous verse, so that they can **destroy** the temple rather than ensure that it is rebuilt. In that case, this letter would be leaving out some of the words that a sentence would ordinarily need in order to be complete. Alternate translation: “to disobey my decree and destroy that temple” or (2\) It could mean that no one is to alter or destroy the temple. Alternate translation: “to alter the design of that temple or destroy it”

See: Ellipsis

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#7) (id: 463100)

דִּ֣י בִ⁠ירוּשְׁלֶ֑ם

**"which is in Jerusalem"**

Here Darius repeats some background information in order to be very explicit about what temple this decree applies to. Alternate translation: “in Jerusalem”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#8) (id: 463101)

אֲנָ֤ה דָרְיָ֨וֶשׁ֙ שָׂ֣מֶת טְעֵ֔ם אָסְפַּ֖רְנָא יִתְעֲבִֽד

**"I, Darius, have set a decree. Let it be done diligently"**

If it would be helpful in your language, you could say this with an active form, and you could say who is to do the action. Alternate translation: “I, King Darius, am issuing this decree. I command you to carry it out exactly and efficiently”

See: Active or Passive

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#9) (id: 463102)

**"Darius"**

**Darius** is the name of a man. See how you translated it in [4:5](https://ref.ly/Ezra4:5).

See: How to Translate Names

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:12 (#10) (id: 463103)

אָסְפַּ֖רְנָא יִתְעֲבִֽד

**"Let it be done diligently"**

Here the text ends its quotation from the letter that King Darius sent in reply to Tattenai and his associates. If you decided in [6:6](https://ref.ly/Ezra6:6) to mark his words as a quotation, you should indicate that ending here with a closing primary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 6:12
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 6:13 (#1) (id: 463104)

אֱ֠דַיִן

**"Then"**

Here the word **Then** indicates that the sentence it introduces explains the result of what the previous sentences described. Alternate translation: “In response,”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:13
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Tattenai (ID: `person:Tattenai`); Euphrates River (ID: `place:Euphrates`); Shethar-bozenai (ID: `person:Shethar-bozenai`)

## Ezra 6:13 (#2) (id: 463105)

**"Tattenai,"** \- **"Shethar\-Bozenai"**

These are the names of two men. See how you translated them in [5:3](https://ref.ly/Ezra5:3).

See: How to Translate Names

* **Associated Passages:** Ezra 6:13
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Tattenai (ID: `person:Tattenai`); Euphrates River (ID: `place:Euphrates`); Shethar-bozenai (ID: `person:Shethar-bozenai`)

## Ezra 6:13 (#3) (id: 463106)

וּ⁠כְנָוָתְ⁠ה֑וֹן

**"and their companions"**

Alternate translation: “and their colleagues”

* **Associated Passages:** Ezra 6:13

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Tattenai (ID: `person:Tattenai`); Euphrates River (ID: `place:Euphrates`); Shethar-bozenai (ID: `person:Shethar-bozenai`)

## Ezra 6:13 (#4) (id: 463107)

לָ⁠קֳבֵ֗ל דִּֽי־שְׁלַ֞ח דָּרְיָ֧וֶשׁ מַלְכָּ֛⁠א כְּנֵ֖מָא אָסְפַּ֥רְנָא עֲבַֽדוּ

**"did thus diligently, according to what Darius the king had sent"**

The implication is that these men received the king’s response to their letter, and once they knew what he had commanded, they carried out his orders. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “received the response of King Darius to their letter and immediately carried out exactly what he had ordered”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); Tattenai (ID: `person:Tattenai`); Euphrates River (ID: `place:Euphrates`); Shethar-bozenai (ID: `person:Shethar-bozenai`)

## Ezra 6:14 (#1) (id: 463108)

וְ⁠שָׂבֵ֤י

**"And the elders of"**

Here the word **And** indicates that the sentence it introduces explains the result of what the previous sentence described. Alternate translation: “As a result … the leaders”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#2) (id: 463109)

וְ⁠שָׂבֵ֤י יְהוּדָיֵ⁠א֙ בָּנַ֣יִן וּ⁠מַצְלְחִ֔ין

**"And the elders of the Jews were building and prospering"**

The phrase **building and prospering** expresses a single idea by using two words connected with **and**. The word **building** tells what the elders prospered or succeeded in. If it would be helpful in your language, you could express the meaning with a single phrase. Alternate translation: “And the Jewish leaders were rebuilding the temple successfully”

See: Hendiadys

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#3) (id: 463110)

בִּ⁠נְבוּאַת֙ חַגַּ֣י נביא⁠ה וּ⁠זְכַרְיָ֖ה בַּר־עִדּ֑וֹא

**"by the prophesying of Haggai the prophet and Zechariah the son of Iddo"**

As in [5:1](https://ref.ly/Ezra5:1), the implication is that these two men, as God’s messengers, encouraged the Jewish leaders to persevere in the project of rebuilding the temple. Alternate translation: “thanks to the encouragement that Haggai the prophet and Zechariah the son of Iddo gave them in messages from God”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#4) (id: 463111)

**"Haggai"**

**Haggai** is the name of a man. See how you translated it in [5:1](https://ref.ly/Ezra5:1).

See: How to Translate Names

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#5) (id: 463112)

וּ⁠זְכַרְיָ֖ה בַּר־עִדּ֑וֹא

**"and Zechariah the son of Iddo"**

**Zechariah** is the name of a man, and **Iddo** is the name of his father. See how you translated their names in [5:1](https://ref.ly/Ezra5:1).

See: How to Translate Names

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#6) (id: 463113)

וּ⁠בְנ֣וֹ וְ⁠שַׁכְלִ֗לוּ

**"And they built and completed"**

As in [5:11](https://ref.ly/Ezra5:11), the words **built** and **completed** connected with **and** express a single idea. In this section of the book, when the two words are used together, “completed” is another way of saying “built.” (Review the note about this at [4:12](https://ref.ly/Ezra4:12) if that would be helpful.) If it would be helpful in your language, you could express the meaning with a single word. Alternate translation: “And they were able to construct the temple”

See: Hendiadys

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#7) (id: 463114)

וּ⁠בְנ֣וֹ וְ⁠שַׁכְלִ֗לוּ

**"And they built and completed"**

The implication is that what the Jewish leaders **built and completed**, that is, constructed, was the temple. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “And they were able to construct the temple”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#8) (id: 463115)

מִן־טַ֨עַם֙ אֱלָ֣הּ יִשְׂרָאֵ֔ל

**"by the decree of the God of Israel"**

Here the book speaks of the messages that God had sent through the prophets Haggai and Zechariah as a **decree** or command that God had issued to the Jewish leaders to rebuild the temple, much like the decrees that the Persian kings had issued. If it would be helpful in your language, you could describe these messages plainly. Alternate translation: “just as the God of Israel had commanded them to do through the prophets”

See: Metaphor

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#9) (id: 463116)

וּ⁠מִ⁠טְּעֵם֙ כּ֣וֹרֶשׁ וְ⁠דָרְיָ֔וֶשׁ וְ⁠אַרְתַּחְשַׁ֖שְׂתְּא מֶ֥לֶךְ פָּרָֽס

**"and by the decree of Cyrus, and Darius, and Artaxerxes, the king of Persia"**

Here, the book assumes some knowledge on the part of readers, because by this point in time, only King Cyrus ([1:2–4](https://ref.ly/Ezra1:2-Ezra1:4)) and King Darius ([6:6–12](https://ref.ly/Ezra6:6-Ezra6:12)) had issued decrees for the Jerusalem temple to be rebuilt. Artaxerxes was the grandson of Darius, and he would not become king for another 50 years. Moreover, while the book anticipated the reign of Artaxerxes in [4:7–23](https://ref.ly/Ezra4:7-Ezra4:23), there it recounted how he ordered the temple rebuilding to stop. But the book is assuming that readers will know that Artaxerxes eventually did reverse himself and issue a decree for the maintenance of the Jerusalem temple. The book describes this in [7:12–26](https://ref.ly/Ezra7:12-Ezra7:26). If it would be helpful to your readers, you could say that explicitly. Alternate translation: “and just as King Cyrus and King Darius of Persia had decreed they should do. King Artaxerxes of Persia also later decreed that they should finish the temple”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#10) (id: 463117)

כּ֣וֹרֶשׁ וְ⁠דָרְיָ֔וֶשׁ וְ⁠אַרְתַּחְשַׁ֖שְׂתְּא מֶ֥לֶךְ פָּרָֽס

**"Cyrus, and Darius, and Artaxerxes, the king of Persia"**

Here the book repeats some background information to remind readers who these men were. The phrase **the king of Persia** applies to each of them. Alternate translation: “King Cyrus and King Darius of Persia and King Artaxerxes of Persia”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:14 (#11) (id: 463118)

**"Cyrus, and Darius, and Artaxerxes"**

These are the names of men. See how you translated them in [1:1](https://ref.ly/Ezra1:1), [4:5](https://ref.ly/Ezra4:5), and [4:7](https://ref.ly/Ezra4:7).

See: How to Translate Names

* **Associated Passages:** Ezra 6:14
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.17`); Darius (ID: `person:Darius.2`); Persia (ID: `place:Persia`); Jews (ID: `group:Jews`); Haggai (ID: `person:Haggai`); Cyrus (ID: `person:Cyrus`); Israel (ID: `person:Israel`); Iddo (ID: `person:Iddo.6`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 6:15 (#1) (id: 463119)

וְ⁠שֵׁיצִיא֙

**"And"** \- **"was done"**

Here the word **And** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result … they finished building”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:15
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); House (ID: `realia:House`)

## Ezra 6:15 (#2) (id: 463120)

וְ⁠שֵׁיצִיא֙ בַּיְתָ֣⁠ה דְנָ֔ה

**"And this house was done"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “The Jewish leaders finished rebuilding the temple”

See: Active or Passive

* **Associated Passages:** Ezra 6:15
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); House (ID: `realia:House`)

## Ezra 6:15 (#3) (id: 463121)

בַּיְתָ֣⁠ה דְנָ֔ה

**"this house"**

As in [5:3](https://ref.ly/Ezra5:3), **house** here is a figurative way of saying temple. Alternate translation: “this temple”

See: Metaphor

* **Associated Passages:** Ezra 6:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); House (ID: `realia:House`)

## Ezra 6:15 (#4) (id: 463122)

עַ֛ד י֥וֹם תְּלָתָ֖ה לִ⁠ירַ֣ח אֲדָ֑ר

**"by day three of the month of Adar"**

The Hebrew uses a cardinal number here, **three**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, **third**, in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of days, you can do that here in your translation. Alternate translation: “by the third day of the month of Adar”

See: Ordinal Numbers

* **Associated Passages:** Ezra 6:15
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); House (ID: `realia:House`)

## Ezra 6:15 (#5) (id: 463123)

עַ֛ד י֥וֹם תְּלָתָ֖ה לִ⁠ירַ֣ח אֲדָ֑ר

**"by day three of the month of Adar"**

You could convert the Hebrew day and month into an approximate date on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the date will be different every year and the translation will not be entirely accurate. So you may just want to give the number of the day and the name of the month on the Hebrew calendar. Alternate translation: “by the third day of the month of Adar”

See: Hebrew Months

* **Associated Passages:** Ezra 6:15
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); House (ID: `realia:House`)

## Ezra 6:15 (#6) (id: 463124)

שְׁנַת־שֵׁ֔ת לְ⁠מַלְכ֖וּת דָּרְיָ֥וֶשׁ מַלְכָּֽ⁠א

**"year six of the reign of Darius the king"**

The Hebrew uses a cardinal number here, **six**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, **sixth**, in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of years, you can do that here in your translation. Alternate translation: “in the sixth year of the reign of Darius as king of Persia”

See: Ordinal Numbers

* **Associated Passages:** Ezra 6:15
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); House (ID: `realia:House`)

## Ezra 6:15 (#7) (id: 463125)

שְׁנַת־שֵׁ֔ת לְ⁠מַלְכ֖וּת דָּרְיָ֥וֶשׁ מַלְכָּֽ⁠א

**"year six of the reign of Darius the king"**

If it would be helpful in your language, you could include the name of Darius's kingdom. Alternate translation: “in the sixth year of the reign of Darius as king of Persia”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Darius (ID: `person:Darius.2`); House (ID: `realia:House`)

## Ezra 6:16 (#1) (id: 463126)

וַ⁠עֲבַ֣דוּ

**"And"** \- **"performed"**

Here, the word **And** indicates that the event the story will now relate came after the events it has just described. If it would be helpful in your language, you could show this relationship by using a word such as “Then.”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 6:16
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Offering (ID: `keyterm:Offering`); House (ID: `realia:House`)

## Ezra 6:16 (#2) (id: 463127)

בְנֵֽי־יִ֠שְׂרָאֵל

**"the sons of Israel"**

Here, **sons** means “descendants.” Here the book envisions all of the Israelites as descendants of the patriarch Jacob, who was also known as Israel. The expression comprises the three groups that are listed next, the priests, Levites, and other Jews. Alternate translation: “the Israelites”

See: Metaphor

* **Associated Passages:** Ezra 6:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Offering (ID: `keyterm:Offering`); House (ID: `realia:House`)

## Ezra 6:16 (#3) (id: 463128)

וּ⁠שְׁאָ֣ר בְּנֵי־גָלוּתָ֗⁠א

**"and the rest of the sons of the exile"**

As indicated in [2:1](https://ref.ly/Ezra2:1) and [8:35](https://ref.ly/Ezra8:35), the phrase **the sons of the exile** refers specifically to the group of Jewish people who returned to the land of Judah from Babylon after King Nebuchadnezzar had conquered Jerusalem and had taken many Jews as captives to Babylon. Alternate translation: “and the Jews who had returned from exile” or “and the Jews who had returned to their homeland”

See: Idiom

* **Associated Passages:** Ezra 6:16
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Offering (ID: `keyterm:Offering`); House (ID: `realia:House`)

## Ezra 6:16 (#4) (id: 463129)

וַ⁠עֲבַ֣דוּ \& חֲנֻכַּ֛ת בֵּית־אֱלָהָ֥⁠א דְנָ֖ה בְּ⁠חֶדְוָֽה

**"And"** \- **"performed the dedication of this house of God with joy"**

The abstract noun **dedication** refers to a special ceremony the Israelites conducted to set the temple apart as a place for the worship of Yahweh. If it would be helpful in your language, you could translate the idea behind this word with a verb such as “consecrate.” Alternate translation: “joyfully consecrated this temple for the worship of Yahweh”

See: Abstract Nouns

* **Associated Passages:** Ezra 6:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Offering (ID: `keyterm:Offering`); House (ID: `realia:House`)

## Ezra 6:16 (#5) (id: 463130)

וַ⁠עֲבַ֣דוּ \& חֲנֻכַּ֛ת בֵּית־אֱלָהָ֥⁠א דְנָ֖ה בְּ⁠חֶדְוָֽה

**"And"** \- **"performed the dedication of this house of God with joy"**

If your language does not use abstract nouns such as **joy**, you can translate the idea behind it with an adverb such as “joyfully.” Alternate translation: “joyfully dedicated this temple to God”

See: Abstract Nouns

* **Associated Passages:** Ezra 6:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Offering (ID: `keyterm:Offering`); House (ID: `realia:House`)

## Ezra 6:17 (#1) (id: 463131)

וְ⁠הַקְרִ֗בוּ לַ⁠חֲנֻכַּת֮ בֵּית־אֱלָהָ֣⁠א דְנָה֒

**"And they offered for the dedication of this house of God"**

As in the previous sentence, the abstract noun **dedication** refers to a special ceremony that the Israelites conducted. Once again you could translate the idea behind this word with a verb such as “dedicate” or “consecrate” Alternate translation: “As they dedicated this temple, they offered”

See: Abstract Nouns

* **Associated Passages:** Ezra 6:17
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); Flock (ID: `fauna:Flock`); House (ID: `realia:House`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`)

## Ezra 6:17 (#2) (id: 463132)

וְ⁠הַקְרִ֗בוּ \& תּוֹרִ֣ין מְאָ֔ה דִּכְרִ֣ין מָאתַ֔יִן אִמְּרִ֖ין אַרְבַּ֣ע מְאָ֑ה וּ⁠צְפִירֵ֨י עִזִּ֜ין ל⁠חטיא \& תְּרֵֽי־עֲשַׂ֔ר

**"And they offered"** \- **"100 bulls, 200 rams, 400 lambs, and 12 bucks of goats for a sin offering"**

The implication, as [6:9](https://ref.ly/Ezra6:9) indicates explicitly, is that the bulls, rams, and lambs were used for whole burnt offerings. If it would be helpful, review the note to [3:2](https://ref.ly/Ezra3:2) about what whole burnt offerings were and why they were offered. Alternate translation: “they offered 100 bulls, 200 rams, and 400 lambs as whole burnt offerings and 12 male goats as a sin offering”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); Flock (ID: `fauna:Flock`); House (ID: `realia:House`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`)

## Ezra 6:17 (#3) (id: 463133)

וּ⁠צְפִירֵ֨י עִזִּ֜ין

**"and"** \- **"bucks of goats"**

A male goat is called a buck. So this expression, **bucks of goats**, means the same thing as male goats in [8:35](https://ref.ly/Ezra8:35)

See: Idiom

* **Associated Passages:** Ezra 6:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); Flock (ID: `fauna:Flock`); House (ID: `realia:House`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`)

## Ezra 6:17 (#4) (id: 463134)

וּ⁠צְפִירֵ֨י עִזִּ֜ין \& תְּרֵֽי־עֲשַׂ֔ר לְ⁠מִנְיָ֖ן שִׁבְטֵ֥י יִשְׂרָאֵֽל

**"and 12 bucks of goats"** \- **"according to the number of the tribes of Israel"**

The book assumes that readers will know that there were twelve tribes in Israel, so the twelve goats could symbolically represent the entire nation. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “12 male goats … one for each of the 12 tribes of Israel”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); Flock (ID: `fauna:Flock`); House (ID: `realia:House`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`)

## Ezra 6:18 (#1) (id: 463135)

וַ⁠הֲקִ֨ימוּ

**"And they caused"** \- **"to stand"**

Here the word **And** indicates that the event the story will now relate came after the event it has just described. If it would be helpful in your language, you could show this relationship by using a word such as “Then.”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 6:18
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Moses (ID: `person:Moses`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Scroll (ID: `realia:Scroll`)

## Ezra 6:18 (#2) (id: 463136)

וַ⁠הֲקִ֨ימוּ כָהֲנַיָּ֜⁠א \& וְ⁠לֵוָיֵ⁠א֙

**"And they caused the priests to stand"** \- **"and the Levites"**

Here, **stand** is a figurative way of saying that a person has assumed the duties of their office. So to cause someone to stand is to appoint them to those duties and install them in that office. Alternate translation: “And they appointed the priests … and the Levites”

See: Metaphor

* **Associated Passages:** Ezra 6:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Moses (ID: `person:Moses`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Scroll (ID: `realia:Scroll`)

## Ezra 6:18 (#3) (id: 463137)

וַ⁠הֲקִ֨ימוּ כָהֲנַיָּ֜⁠א

**"And they caused the priests to stand"**

Here, **they** refers to the leaders of the Jews. Alternate translation: “And the leaders of the Jews appointed the priests” or “Then the Jewish elders established the priests”

See: Pronouns — When to Use Them

* **Associated Passages:** Ezra 6:18
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Moses (ID: `person:Moses`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Scroll (ID: `realia:Scroll`)

## Ezra 6:18 (#4) (id: 463138)

כָהֲנַיָּ֜⁠א בִּ⁠פְלֻגָּתְ⁠ה֗וֹן וְ⁠לֵוָיֵ⁠א֙ בְּ⁠מַחְלְקָ֣תְ⁠ה֔וֹן

**"the priests"** \- **"in their divisions, and the Levites in their sections"**

The book assumes that readers will know that these **divisions** and **sections** were groups of priests and Levites, respectively, that served in the temple for a week at a time. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “They appointed the priests and the Levites to groups that would serve in the temple for a week at a time”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Moses (ID: `person:Moses`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Scroll (ID: `realia:Scroll`)

## Ezra 6:18 (#5) (id: 463139)

עַל־עֲבִידַ֥ת אֱלָהָ֖⁠א דִּ֣י בִ⁠ירוּשְׁלֶ֑ם

**"for the service of the God who is in Jerusalem"**

Here the book describes God in a spatial metaphor as if he lived in the city of Jerusalem. This is a figurative reference to the way God’s presence was in the temple in Jerusalem. Alternate translation: “to lead the worship of God in the temple in Jerusalem”

See: Metaphor

* **Associated Passages:** Ezra 6:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Moses (ID: `person:Moses`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Scroll (ID: `realia:Scroll`)

## Ezra 6:18 (#6) (id: 463140)

כִּ⁠כְתָ֖ב סְפַ֥ר מֹשֶֽׁה

**"according to the writing of the book of Moses"**

Alternate translation: “as it is written in the book of Moses” or “as God commanded in the law of Moses”

* **Associated Passages:** Ezra 6:18

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Moses (ID: `person:Moses`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Scroll (ID: `realia:Scroll`)

## Ezra 6:19 (#1) (id: 463141)

וַ⁠יַּעֲשׂ֥וּ בְנֵי־הַ⁠גּוֹלָ֖ה אֶת־הַ⁠פָּ֑סַח

**"So the sons of the exile performed the Passover"**

The phrase at the beginning of this sentence indicates that it explains the results of what the previous sentence described. (As [6:20](https://ref.ly/Ezra6:20) explains, because the priests and Levites had been assigned to their duties, festivals like this could be observed again.) Alternate translation: “As a result, the Jews who had returned from exile were able to celebrate Passover”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:19
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`)

## Ezra 6:19 (#2) (id: 463142)

וַ⁠יַּעֲשׂ֥וּ

**"So"** \- **"performed"**

The word **and** introduces a new event in the story. You do not need to represent it in your translation unless your language has a similar expression that it characteristically uses.

See: Introduction of a New Event

* **Associated Passages:** Ezra 6:19
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`)

## Ezra 6:19 (#3) (id: 463143)

בְנֵי־הַ⁠גּוֹלָ֖ה

**"the sons of the exile"**

See how you translated this expression in [6:16](https://ref.ly/Ezra6:16). Alternate translation: “the Jews who had returned to their homeland”

See: Idiom

* **Associated Passages:** Ezra 6:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`)

## Ezra 6:19 (#4) (id: 463144)

אֶת־הַ⁠פָּ֑סַח

**"the Passover"**

**Passover** is the name of a religious festival that the law of Moses commanded the Jews to celebrate every year to remember how God had rescued their ancestors from slavery in Egypt.

See: Translate Unknowns

* **Associated Passages:** Ezra 6:19
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`)

## Ezra 6:19 (#5) (id: 463145)

בְּ⁠אַרְבָּעָ֥ה עָשָׂ֖ר לַ⁠חֹ֥דֶשׁ הָ⁠רִאשֽׁוֹן

**"on day 14 of the first month"**

The Hebrew uses a cardinal number here, **14**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, fourteenth, in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of days, you can do that here in your translation. Alternate translation: “on the fourteenth day of the first month”

See: Ordinal Numbers

* **Associated Passages:** Ezra 6:19
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`)

## Ezra 6:19 (#6) (id: 463146)

בְּ⁠אַרְבָּעָ֥ה עָשָׂ֖ר לַ⁠חֹ֥דֶשׁ הָ⁠רִאשֽׁוֹן

**"on day 14 of the first month"**

You could convert the Hebrew day and month into an approximate date on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the date will be different every year and the translation will not be entirely accurate. So you may just want to use the numbers of the Hebrew day and month. Alternate translation: “on the fourteenth day of the first month”

See: Hebrew Months

* **Associated Passages:** Ezra 6:19
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`)

## Ezra 6:19 (#7) (id: 463147)

בְּ⁠אַרְבָּעָ֥ה עָשָׂ֖ר לַ⁠חֹ֥דֶשׁ הָ⁠רִאשֽׁוֹן

**"on day 14 of the first month"**

This means the first month of the following year. Since Adar is the last month in the Jewish calendar, this was only a few weeks after the temple was finished. The implication is that the Jewish leaders had been able to complete that work and install the priests and Levites in time to celebrate this festival. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “only a few weeks later, on the fourteenth day of the first month”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#1) (id: 463148)

כִּ֣י

**"For"**

The word **for** indicates that the sentence it introduces explains the reason why the action described in the previous sentence was possible. Alternate translation: “They were able to do this because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#2) (id: 463149)

הִֽטַּהֲר֞וּ הַ⁠כֹּהֲנִ֧ים וְ⁠הַ⁠לְוִיִּ֛ם כְּ⁠אֶחָ֖ד כֻּלָּ֣⁠ם טְהוֹרִ֑ים

**"the priests and the Levites had purified themselves as one, all of them were pure"**

As in [3:9](https://ref.ly/Ezra3:9), **as one** is an idiom that means that these priests and Levites all behaved as if they were a single person, that is, they all did the same thing. Alternate translation: “Every single one of the priests and Levites had purified himself”

See: Idiom

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#3) (id: 463150)

הִֽטַּהֲר֞וּ \& כְּ⁠אֶחָ֖ד כֻּלָּ֣⁠ם טְהוֹרִ֑ים

**"had purified themselves as one, all of them were pure"**

These two phrases mean the same thing. The book uses the repetition for emphasis and clarity. You do not need to repeat both phrases in your translation if that might be confusing for your readers. Alternate translation: “every single one had purified himself”

See: Parallelism

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#4) (id: 463151)

הִֽטַּהֲר֞וּ

**"had purified themselves"**

Being pure represents being acceptable to God. Alternate translation: “performed a ceremony to show that they wanted to be acceptable to God”

See: Metaphor

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#5) (id: 463152)

וַ⁠יִּשְׁחֲט֤וּ הַ⁠פֶּ֨סַח֙

**"And they slaughtered the Passover"**

The book assumes that readers will know that the **Passover** celebration included a special meal of lamb. Following God's instructions, the Israelites had slaughtered lambs to eat on the night before they left Egypt, and they had put the blood of the lambs on their doorframes so that God would pass over their houses and everyone inside would be safe. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “And they slaughtered lambs for the special Passover meal”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#6) (id: 463153)

**"the Passover"**

See how you translated the name of this festival in [6:19](https://ref.ly/Ezra6:19).

See: How to Translate Names

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#7) (id: 463154)

בְּנֵ֣י הַ⁠גּוֹלָ֔ה

**"the sons of the exile"**

See how you translated this expression in [6:16](https://ref.ly/Ezra6:16). Alternate translation: “the Jews who had returned to their homeland”

See: Idiom

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:20 (#8) (id: 463155)

וְ⁠לַ⁠אֲחֵי⁠הֶ֥ם הַ⁠כֹּהֲנִ֖ים

**"and for their brothers, the priests"**

Here, **brothers** is a figurative way of saying fellow priests, although it is possible that this group included some of the biological brothers of the priests who slaughtered the lambs. Alternate translation: “and for their fellow priests”

See: Metaphor

* **Associated Passages:** Ezra 6:20
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); Passover (ID: `keyterm:Passover`)

## Ezra 6:21 (#1) (id: 463156)

וַ⁠יֹּאכְל֣וּ בְנֵֽי־יִשְׂרָאֵ֗ל

**"And the sons of Israel ate"**

**Sons** here means descendants. The book is envisioning all of the Israelites as descendants of the patriarch Jacob, who was also known as Israel. Alternate translation: “And the Israelites ate”

See: Metaphor

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:21 (#2) (id: 463157)

וַ⁠יֹּאכְל֣וּ בְנֵֽי־יִשְׂרָאֵ֗ל

**"And the sons of Israel ate"**

Here the book leaves out some of the words that a sentence would ordinarily need in order to be complete. Alternate translation: “And the Israelites ate the Passover meal”

See: Ellipsis

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:21 (#3) (id: 463158)

וְ⁠כֹ֗ל הַ⁠נִּבְדָּ֛ל מִ⁠טֻּמְאַ֥ת גּוֹיֵֽ־הָ⁠אָ֖רֶץ אֲלֵ⁠הֶ֑ם לִ⁠דְרֹ֕שׁ לַֽ⁠יהוָ֖ה

**"and every one who separated himself to them from the uncleanness of the nations of the land in order to seek Yahweh"**

This could mean one of two possibilities. (1\) It could be referring to people from other nations who had converted and become Jews. Alternate translation: “and anyone from another nation who had abandoned their former practices and become a Jew and would now worship and obey Yahweh” or (2\) It could be referring to Israelites who had not been taken into exile but who had adopted the customs and practices of the other people groups that had come to live in the former Jewish homeland. Alternate translation: “as well as Israelites who had not gone into exile, who had adopted foreign practices, but who would now join the returned exiles in worshiping and obeying Yahweh”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:21 (#4) (id: 463159)

וְ⁠כֹ֗ל הַ⁠נִּבְדָּ֛ל מִ⁠טֻּמְאַ֥ת גּוֹיֵֽ־הָ⁠אָ֖רֶץ

**"and every one who separated himself"** \- **"from the uncleanness of the nations of the land"**

Being **separated** from uncleanness represents refusing to do things that make people unclean. Alternate translation: “and everyone who refused to do the things that made them unclean that the other people groups living in the land did”

See: Metaphor

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:21 (#5) (id: 463160)

מִ⁠טֻּמְאַ֥ת גּוֹיֵֽ־הָ⁠אָ֖רֶץ

**"from the uncleanness of the nations of the land"**

Here, **uncleanness** represents being unacceptable to God. Alternate translation: “from the things that made the nations around them unacceptable to God”

See: Metaphor

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:21 (#6) (id: 463161)

גּוֹיֵֽ־הָ⁠אָ֖רֶץ

**"the nations of the land"**

Like **the people of the land** in [4:4](https://ref.ly/Ezra4:4), this expression refers to the non\-Israelite people groups who were living in this area. (Review the note at [4:4](https://ref.ly/Ezra4:4) if that would be helpful.) Alternate translation: “the foreign people groups living nearby”

See: Idiom

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:21 (#7) (id: 463162)

לִ⁠דְרֹ֕שׁ לַֽ⁠יהוָ֖ה

**"in order to seek Yahweh"**

To **seek** Yahweh means to choose to know, worship, and obey him. Alternate translation: “to worship and obey Yahweh”

See: Metaphor

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:21 (#8) (id: 463163)

אֱלֹהֵ֥י יִשְׂרָאֵֽל

**"the God of Israel"**

As in [4:1](https://ref.ly/Ezra4:1), this phrase provides further background information about who Yahweh is. In context, it helps distinguish Yahweh from the gods of the nations of the land. Alternate translation: “Yahweh, the God whom the people of Israel worshiped”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 6:21
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Gentiles (ID: `group:Gentile`); Defilement (ID: `keyterm:Defilement`); LORD (ID: `deity:Lord`)

## Ezra 6:22 (#1) (id: 463164)

וַ⁠יַּֽעֲשׂ֧וּ

**"Then they performed"**

This phrase indicates that the event the story will now relate came after the event it has just described. If it would be helpful in your language, you could show this relationship by using a phrase such as “After that.”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 6:22
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Assyria (ID: `place:Assyria`); House (ID: `realia:House`); Unleavened Bread (ID: `realia:UnleavenedBread`)

## Ezra 6:22 (#2) (id: 463165)

וַ⁠יַּֽעֲשׂ֧וּ חַג־מַצּ֛וֹת שִׁבְעַ֥ת יָמִ֖ים בְּ⁠שִׂמְחָ֑ה

**"Then they performed the Festival of Unleavened Bread seven days with joy"**

**Festival of Unleavened Bread** is the name of another religious festival that the law of Moses commanded the Jews to celebrate every year to remember how God had rescued their ancestors from slavery in Egypt. The term **Unleavened Bread** refers to bread that is made without yeast or other leavening. When God freed the Israelites from slavery in Egypt, he told them to flee quickly without waiting for their bread to rise. Unleavened bread is served in the yearly Passover meal in remembrance of that time. Then for the next week after Passover, the Jews are still not to eat any leavened bread. The beginning and end of that week are marked by special ceremonies. Alternate translation: “For the next seven days, they joyfully celebrated the Festival of Unleavened Bread”

See: How to Translate Names

* **Associated Passages:** Ezra 6:22
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Assyria (ID: `place:Assyria`); House (ID: `realia:House`); Unleavened Bread (ID: `realia:UnleavenedBread`)

## Ezra 6:22 (#3) (id: 463166)

וַ⁠יַּֽעֲשׂ֧וּ חַג־מַצּ֛וֹת שִׁבְעַ֥ת יָמִ֖ים בְּ⁠שִׂמְחָ֑ה כִּ֣י ׀ שִׂמְּחָ֣⁠ם יְהוָ֗ה וְֽ⁠הֵסֵ֞ב לֵ֤ב מֶֽלֶךְ־אַשּׁוּר֙ עֲלֵי⁠הֶ֔ם לְ⁠חַזֵּ֣ק יְדֵי⁠הֶ֔ם בִּ⁠מְלֶ֥אכֶת בֵּית־הָ⁠אֱלֹהִ֖ים אֱלֹהֵ֥י יִשְׂרָאֵֽל

**"Then they performed the Festival of Unleavened Bread seven days with joy, because Yahweh had caused them to be joyful, and had caused the heart of the king of Assyria to turn toward them, to strengthen their hands in the work of the house of God, the God of Israel"**

This sentence has three main parts, each one giving the reason for what comes before it. If it would be helpful in your language, you could break the sentence into three sentences, and put the earlier part about the festival at the end. In that way, each sentence gives the result of the sentence before it. Alternate translation: “Yahweh had made the king of Assyria to feel favorable toward the Jews so that he supported the rebuilding of the temple. This had made the people very happy. As a result, they performed the Festival of Unleavened Bread seven days with joy”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 6:22
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Assyria (ID: `place:Assyria`); House (ID: `realia:House`); Unleavened Bread (ID: `realia:UnleavenedBread`)

## Ezra 6:22 (#4) (id: 463167)

וְֽ⁠הֵסֵ֞ב לֵ֤ב מֶֽלֶךְ־אַשּׁוּר֙ עֲלֵי⁠הֶ֔ם

**"and had caused the heart of the king of Assyria to turn toward them"**

Here, **the heart** represents the thoughts and the will. Turning the king’s heart towards the Jews means that Yahweh made him think and feel differently about the Jews. Alternate translation: “and made King Darius of Persia favorable toward the Jews”

See: Metaphor

* **Associated Passages:** Ezra 6:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Assyria (ID: `place:Assyria`); House (ID: `realia:House`); Unleavened Bread (ID: `realia:UnleavenedBread`)

## Ezra 6:22 (#5) (id: 463168)

מֶֽלֶךְ־אַשּׁוּר֙

**"the king of Assyria"**

This phrase actually refers to Darius, the king of Persia. But since the Persian Empire was the heir to the Assyrian Empire, Darius could be addressed by this title as well. (See the note to [4:15](https://ref.ly/Ezra4:15) that explains that the Persian kings considered their “fathers” or predecessor kings to include the Assyrian and Babylonian kings whose empires they had absorbed.) Alternate translation: “King Darius of Persia”

* **Associated Passages:** Ezra 6:22

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Assyria (ID: `place:Assyria`); House (ID: `realia:House`); Unleavened Bread (ID: `realia:UnleavenedBread`)

## Ezra 6:22 (#6) (id: 463169)

אֱלֹהֵ֥י יִשְׂרָאֵֽל

**"the God of Israel"**

As in [4:1](https://ref.ly/Ezra4:1), this phrase provides further background information about who Yahweh is. In context, it helps distinguish Yahweh from the gods of the nations of the land. Alternate translation: “Yahweh, the God whom the people of Israel worshiped”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 6:22
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Assyria (ID: `place:Assyria`); House (ID: `realia:House`); Unleavened Bread (ID: `realia:UnleavenedBread`)

## Ezra 7:1 (#1) (id: 463170)

וְ⁠אַחַר֙ הַ⁠דְּבָרִ֣ים הָ⁠אֵ֔לֶּה

**"Now after these things"**

This phrase, common in Hebrew storytelling, indicates that the book will now relate events that occurred some time after the events that it had been describing. The book had been describing the time when the Jews finished rebuilding the temple, [6:15](https://ref.ly/Ezra6:15), which was the sixth year of the reign of Darius. Now it will describe events beginning during the seventh year of the reign of Artaxerxes, who was the grandson of Darius. So nearly sixty years went by between the end of chapter 6 and the beginning of chapter 7\. If your language has a similar phrase that can indicate the passage of time like this, you can use that in your translation.

See: Introduction of a New Event

* **Associated Passages:** Ezra 7:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Seraiah (ID: `person:Seraiah.2`); Azariah (ID: `person:Azariah.7`); Persia (ID: `place:Persia`); Hilkiah (ID: `person:Hilkiah.2`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:1 (#2) (id: 463171)

**"Artaxerxes"**

**Artaxerxes** is the name of a man. See how you translated it in [4:7](https://ref.ly/Ezra4:7).

See: How to Translate Names

* **Associated Passages:** Ezra 7:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Seraiah (ID: `person:Seraiah.2`); Azariah (ID: `person:Azariah.7`); Persia (ID: `place:Persia`); Hilkiah (ID: `person:Hilkiah.2`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:1 (#3) (id: 463172)

עֶזְרָא֙

**"Ezra"**

Here the book introduces a new, important person. Use a natural way in your language to do this. Also, since the sentence that begins here extends through verse 6, it may be helpful to your readers to make this first part of verse 1 into a complete sentence and begin another sentence with the list of Ezra's ancestors, as in the UST.

See: Introduction of New and Old Participants

* **Associated Passages:** Ezra 7:1
* **Associated Articles:** Introduction of New and Old Participants (ID: `33421@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Seraiah (ID: `person:Seraiah.2`); Azariah (ID: `person:Azariah.7`); Persia (ID: `place:Persia`); Hilkiah (ID: `person:Hilkiah.2`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:1 (#4) (id: 463173)

בֶּן־שְׂרָיָ֔ה בֶּן־עֲזַרְיָ֖ה בֶּן־חִלְקִיָּֽה

**"the son of Seraiah, the son of Azariah, the son of Hilkiah"**

In general, in the list in [7:1–5](https://ref.ly/Ezra7:1-Ezra7:5), **son** means descendant. However, in many cases a man actually is the biological son of the next man on the list. So for your translation, you will need to decide whether to use the figurative expression **son**, which can also be literally true in many cases, or the plain expression “descendant,” which is true in every case, or to say “son” for actual sons and “descendant” for descendants who are not actual sons. Choosing that last option would help to show that the men on the list have been selected to make a certain number and arrangement, as explained in the chapter introduction. Alternative translations will illustrate this last option for each verse. Here, Alternate translation: “the descendant of Seraiah, the son of Azariah, the son of Hilkiah”

See: Metaphor

* **Associated Passages:** Ezra 7:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Seraiah (ID: `person:Seraiah.2`); Azariah (ID: `person:Azariah.7`); Persia (ID: `place:Persia`); Hilkiah (ID: `person:Hilkiah.2`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:1 (#5) (id: 463174)

שְׂרָיָ֔ה \& עֲזַרְיָ֖ה \& חִלְקִיָּֽה

**"Seraiah,"** \- **"Azariah,"** \- **"Hilkiah"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 7:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Seraiah (ID: `person:Seraiah.2`); Azariah (ID: `person:Azariah.7`); Persia (ID: `place:Persia`); Hilkiah (ID: `person:Hilkiah.2`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:2 (#1) (id: 463175)

בֶּן־שַׁלּ֥וּם בֶּן־צָד֖וֹק בֶּן־אֲחִיטֽוּב

**"the son of Shallum, the son of Zadok, the son of Ahitub"**

Alternate translation: “the son of Shallum, the son of Zadok, the descendant of Ahitub”

* **Associated Passages:** Ezra 7:2

* **Associated ACAI Concepts:** Ahitub (ID: `person:Ahitub.3`); Shallum (ID: `person:Shallum.7`); Zadok (ID: `person:Zadok.4`)

## Ezra 7:2 (#2) (id: 463176)

שַׁלּ֥וּם \& צָד֖וֹק \& אֲחִיטֽוּב

**"Shallum,"** \- **"Zadok,"** \- **"Ahitub"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 7:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahitub (ID: `person:Ahitub.3`); Shallum (ID: `person:Shallum.7`); Zadok (ID: `person:Zadok.4`)

## Ezra 7:3 (#1) (id: 463177)

בֶּן־אֲמַרְיָ֥ה בֶן־עֲזַרְיָ֖ה בֶּן־מְרָיֽוֹת

**"the son of Amariah, the son of Azariah, the son of Meraioth"**

Alternate translation: “the descendant of Amariah, the son of Azariah, the descendant of Meraioth”

* **Associated Passages:** Ezra 7:3

* **Associated ACAI Concepts:** Azariah (ID: `person:Azariah.6`); Amariah (ID: `person:Amariah.2`); Meraioth (ID: `person:Meraioth`)

## Ezra 7:3 (#2) (id: 463178)

אֲמַרְיָ֥ה \& עֲזַרְיָ֖ה \& מְרָיֽוֹת

**"Amariah,"** \- **"Azariah,"** \- **"Meraioth"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 7:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Azariah (ID: `person:Azariah.6`); Amariah (ID: `person:Amariah.2`); Meraioth (ID: `person:Meraioth`)

## Ezra 7:4 (#1) (id: 463179)

בֶּן־זְרַֽחְיָ֥ה בֶן־עֻזִּ֖י בֶּן־בֻּקִּֽי

**"the son of Zerahiah, the son of Uzzi, the son of Bukki"**

Even if you have decided to use plain terminology, unless you have chosen to use the term “descendant” throughout this list, you can say “son” in each case here because each of these men was the actual father of the man whose name precedes his. Alternate translation: “the son of Zerahiah, the son of Uzzi, the son of Bukki”

* **Associated Passages:** Ezra 7:4

* **Associated ACAI Concepts:** Uzzi (ID: `person:Uzzi`); Bukki (ID: `person:Bukki.2`); Zerahiah (ID: `person:Zerahiah`)

## Ezra 7:4 (#2) (id: 463180)

זְרַֽחְיָ֥ה \& עֻזִּ֖י \& בֻּקִּֽי

**"Zerahiah,"** \- **"Uzzi,"** \- **"Bukki"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 7:4
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Uzzi (ID: `person:Uzzi`); Bukki (ID: `person:Bukki.2`); Zerahiah (ID: `person:Zerahiah`)

## Ezra 7:5 (#1) (id: 463181)

בֶּן־אֲבִישׁ֗וּעַ בֶּן־פִּֽינְחָס֙ בֶּן־אֶלְעָזָ֔ר בֶּן־אַהֲרֹ֥ן

**"the son of Abishua, the son of Phinehas, the son of Eleazar, the son of Aaron"**

Here again, unless you have chosen to use the term “descendant” throughout this list, you can say “son” in each case here because each of these men was the actual father of the man whose name precedes his. Alternate translation: “the son of Abishua, the son of Phinehas, the son of Eleazar, the son of Aaron”

* **Associated Passages:** Ezra 7:5

* **Associated ACAI Concepts:** Abishua (ID: `person:Abishua`); Phinehas (ID: `person:Phinehas`); Eleazar (ID: `person:Eleazar.2`); Aaron (ID: `person:Aaron`)

## Ezra 7:5 (#2) (id: 463182)

אֲבִישׁ֗וּעַ \& פִּֽינְחָס֙ \& אֶלְעָזָ֔ר \& אַהֲרֹ֥ן

**"Abishua,"** \- **"Phinehas,"** \- **"Eleazar,"** \- **"Aaron"**

These are the names of four men.

See: How to Translate Names

* **Associated Passages:** Ezra 7:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Abishua (ID: `person:Abishua`); Phinehas (ID: `person:Phinehas`); Eleazar (ID: `person:Eleazar.2`); Aaron (ID: `person:Aaron`)

## Ezra 7:5 (#3) (id: 463183)

הַ⁠כֹּהֵ֖ן הָ⁠רֹֽאשׁ

**"the head priest"**

Here the book provides some background information to remind readers of who Aaron was. Alternate translation: “the high priest”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 7:5
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Abishua (ID: `person:Abishua`); Phinehas (ID: `person:Phinehas`); Eleazar (ID: `person:Eleazar.2`); Aaron (ID: `person:Aaron`)

## Ezra 7:5 (#4) (id: 463184)

הַ⁠כֹּהֵ֖ן הָ⁠רֹֽאשׁ

**"the head priest"**

Here, **head** is a figurative way of saying “leader.” Specifically, it means that Aaron was the leader of the priests, or the high priest. Alternate translation: “the high priest”

See: Metaphor

* **Associated Passages:** Ezra 7:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Abishua (ID: `person:Abishua`); Phinehas (ID: `person:Phinehas`); Eleazar (ID: `person:Eleazar.2`); Aaron (ID: `person:Aaron`)

## Ezra 7:5 (#5) (id: 463185)

הַ⁠כֹּהֵ֖ן הָ⁠רֹֽאשׁ

**"the head priest"**

The book assumes that readers will know that Aaron was the first high priest to serve in the tabernacle when God gave the law to Moses, his brother. Alternate translation: “the first high priest”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Abishua (ID: `person:Abishua`); Phinehas (ID: `person:Phinehas`); Eleazar (ID: `person:Eleazar.2`); Aaron (ID: `person:Aaron`)

## Ezra 7:6 (#1) (id: 463186)

ה֤וּא עֶזְרָא֙ עָלָ֣ה מִ⁠בָּבֶ֔ל

**"this Ezra went up from Babylon"**

The book says that Ezra **went up** because he needed to travel from a river valley up into the mountains in order to go from exile in Babylon to Jerusalem. Alternate translation: “this Ezra went from Babylon to Jerusalem”

See: Idiom

* **Associated Passages:** Ezra 7:6
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Babylon (ID: `place:Babylon`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:6 (#2) (id: 463187)

ה֤וּא עֶזְרָא֙ עָלָ֣ה מִ⁠בָּבֶ֔ל

**"this Ezra went up from Babylon"**

If it is more natural in your language, you can say “came” instead of **went**. Alternate translation: “this Ezra came from Babylon to Jerusalem”

See: Go and Come

* **Associated Passages:** Ezra 7:6
* **Associated Articles:** Go and Come (ID: `33278@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Babylon (ID: `place:Babylon`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:6 (#3) (id: 463188)

וְ⁠הֽוּא־סֹפֵ֤ר מָהִיר֙ בְּ⁠תוֹרַ֣ת מֹשֶׁ֔ה אֲשֶׁר־נָתַ֥ן יְהוָ֖ה אֱלֹהֵ֣י יִשְׂרָאֵ֑ל

**"and he was a skilled scribe in the law of Moses that Yahweh, the God of Israel, had given"**

This is background information that helps identify Ezra further. Alternate translation: “and he had carefully studied the law that Yahweh, the God of Israel, had given through Moses”

See: Background Information

* **Associated Passages:** Ezra 7:6
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Babylon (ID: `place:Babylon`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:6 (#4) (id: 463189)

וַ⁠יִּתֶּן־ל֣⁠וֹ הַ⁠מֶּ֗לֶךְ כְּ⁠יַד־יְהוָ֤ה אֱלֹהָי⁠ו֙ עָלָ֔י⁠ו כֹּ֖ל בַּקָּשָׁתֽ⁠וֹ

**"And the king granted to him all his request according to the hand of Yahweh his God upon him"**

Here, **hand** represents action, and in this context the expression **the hand of Yahweh his God upon him** indicates that Ezra enjoyed Yahweh’s care, protection, and favor. (The expression has a similar sense to the one in [5:5](https://ref.ly/Ezra5:5), **the eye of God was on the elders of the Jews.**) Alternate translations: “And King Artaxerxes gave Ezra everything he asked for because Yahweh, his God, was helping him” or “And King Artaxerxes gave Ezra everything he asked for because Yahweh, his God, was showing him favor”

See: Metaphor

* **Associated Passages:** Ezra 7:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Babylon (ID: `place:Babylon`); Israel (ID: `person:Israel`); Moses (ID: `person:Moses`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:7 (#1) (id: 463190)

וַ⁠יַּֽעֲל֣וּ

**"And"** \- **"went up"**

Here, **And** indicates that the event this sentence describes took place at the same time as the event the story has just related. If it would be helpful in your language, you could show this relationship in this case by using a phrase such as “At the same time … went up”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Ezra 7:7
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#2) (id: 463191)

מִ⁠בְּנֵֽי־יִ֠שְׂרָאֵל

**"some from the sons of Israel"**

Here, **sons** means descendants. Here the book envisions all of the Israelites as descendants of the patriarch Jacob, who was also known as Israel. Alternate translation: “from among the Israelites”

See: Metaphor

* **Associated Passages:** Ezra 7:7
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#3) (id: 463192)

וְ⁠הַ⁠לְוִיִּ֜ם

**"and the Levites"**

See how you translated this term in [2:40](https://ref.ly/Ezra2:40). Review the explanation in the note there if that would be helpful.

See: How to Translate Names

* **Associated Passages:** Ezra 7:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#4) (id: 463193)

וְ⁠הַ⁠מְשֹׁרְרִ֧ים

**"and the ones who sang"**

This is the same occupational group as in [2:41](https://ref.ly/Ezra2:41). Review the explanation in the note there if that would be helpful.

* **Associated Passages:** Ezra 7:7

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#5) (id: 463194)

וְ⁠הַ⁠שֹּׁעֲרִ֛ים

**"and the gatekeepers"**

This is the same occupational group as in [2:42](https://ref.ly/Ezra2:42). Review the explanation in the note there if that would be helpful.

* **Associated Passages:** Ezra 7:7

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#6) (id: 463195)

וְ⁠הַ⁠נְּתִינִ֖ים

**"and the temple servants"**

See how you translated this term in [2:43](https://ref.ly/Ezra2:43). Review the explanation in the note there if that would be helpful. These men would not have worked in the temple themselves, but would be descendants of men who had worked in the temple. If it would be helpful in your language, you could say this explicitly. Alternate translation: “and descendants of the temple servants”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#7) (id: 463196)

וַ⁠יַּֽעֲל֣וּ \& אֶל־יְרוּשָׁלִָ֑ם

**"And"** \- **"went up to Jerusalem"**

The book says **went up** because these people had to travel from a river valley up into the mountains in order to return from their places of exile to Jerusalem. Alternate translation: “traveled to Jerusalem”

See: Idiom

* **Associated Passages:** Ezra 7:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#8) (id: 463197)

וַ⁠יַּֽעֲל֣וּ \& אֶל־יְרוּשָׁלִָ֑ם

**"And"** \- **"went up to Jerusalem"**

If it is more natural in your language, you can say “came” instead of **went**. Alternate translation: “came to Jerusalem”

See: Go and Come

* **Associated Passages:** Ezra 7:7
* **Associated Articles:** Go and Come (ID: `33278@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:7 (#9) (id: 463198)

בִּ⁠שְׁנַת־שֶׁ֖בַע לְ⁠אַרְתַּחְשַׁ֥סְתְּא הַ⁠מֶּֽלֶךְ

**"in year seven of Artaxerxes the king"**

The Hebrew uses a cardinal number here, **seven**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, **seventh**, in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of years, you can do that here in your translation. Alternate translation: “in the seventh year of the reign of Artaxerxes as king of Persia”

See: Ordinal Numbers

* **Associated Passages:** Ezra 7:7
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:8 (#1) (id: 463199)

וַ⁠יָּבֹ֥א יְרוּשָׁלִַ֖ם

**"And he came to Jerusalem"**

Here, **he** refers to Ezra, but the statement also applies to all the others described in [7:7](https://ref.ly/Ezra7:7) who were traveling with him. Alternate translation: “And they all arrived in Jerusalem”

See: Pronouns — When to Use Them

* **Associated Passages:** Ezra 7:8
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Ezra 7:8 (#2) (id: 463200)

וַ⁠יָּבֹ֥א יְרוּשָׁלִַ֖ם בַּ⁠חֹ֣דֶשׁ הַ⁠חֲמִישִׁ֑י

**"And he came to Jerusalem in the fifth month"**

The **fifth month** means the fifth month in the Jewish calendar. You could convert the Hebrew month into an equivalent on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the equivalency will be different every year and the translation will not be entirely accurate. So you may just want to use the name or number of the Hebrew month. Alternate translation: “And they all arrived in Jerusalem during the fifth month of the year”

See: Hebrew Months

* **Associated Passages:** Ezra 7:8
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Ezra 7:8 (#3) (id: 463201)

בַּ⁠חֹ֣דֶשׁ הַ⁠חֲמִישִׁ֑י

**"in the fifth month"**

Alternate translation: “in month five”

See: Ordinal Numbers

* **Associated Passages:** Ezra 7:8
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Ezra 7:8 (#4) (id: 463202)

הִ֛יא שְׁנַ֥ת הַ⁠שְּׁבִיעִ֖ית לַ⁠מֶּֽלֶךְ

**"which was in the seventh year of the king"**

Alternate translation: “in year seven of the reign of Artaxerxes” or since the year was mentioned at the end of the previous verse, “of that year”

See: Ordinal Numbers

* **Associated Passages:** Ezra 7:8
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Ezra 7:9 (#1) (id: 463203)

כִּ֗י

**"For"**

The word **for** indicates that the sentence it introduces will provide background information that will help readers appreciate what the book describes next. You can translate it with the word or phrase in your language that is most similar in meaning and significance. Alternate translation: “Now”

See: Background Information

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:9 (#2) (id: 463204)

בְּ⁠אֶחָד֙ לַ⁠חֹ֣דֶשׁ הָ⁠רִאשׁ֔וֹן ה֣וּא יְסֻ֔ד הַֽ⁠מַּעֲלָ֖ה מִ⁠בָּבֶ֑ל

**"on the first of the first month was the beginning of the ascent from Babylon"**

The book says **ascent** to characterize the journey once again as involving a significant climb in elevation. Alternate translation: “the group began its uphill trip from Babylon on the first day of the first month”

See: Idiom

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:9 (#3) (id: 463205)

בְּ⁠אֶחָד֙ לַ⁠חֹ֣דֶשׁ הָ⁠רִאשׁ֔וֹן

**"on the first of the first month"**

The **first month** means the first month in the Jewish calendar. You could convert the Hebrew month into an equivalent on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the equivalency will be different every year and the translation will not be entirely accurate. So you may just want to use the name or number of the Hebrew month. Alternate translation: “on the first day of the first month of that year”

See: Hebrew Months

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:9 (#4) (id: 463206)

בְּ⁠אֶחָד֙ לַ⁠חֹ֣דֶשׁ הָ⁠רִאשׁ֔וֹן

**"on the first of the first month"**

Alternate translation: “on day one of month one”

See: Ordinal Numbers

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:9 (#5) (id: 463207)

וּ⁠בְ⁠אֶחָ֞ד

**"And on the first"**

This word **and** might indicate that the sentence it introduces draws a contrast between how long this journey would be expect to take and how quickly Ezra and his companions reached Jerusalem. You could begin the sentence with a word such as “but” to indicate this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:9 (#6) (id: 463208)

וּ⁠בְ⁠אֶחָ֞ד לַ⁠חֹ֣דֶשׁ הַ⁠חֲמִישִׁ֗י בָּ֚א אֶל־יְר֣וּשָׁלִַ֔ם כְּ⁠יַד־אֱלֹהָ֖י⁠ו הַ⁠טּוֹבָ֥ה עָלָֽי⁠ו

**"And on the first of the fifth month he came to Jerusalem, according to the good hand of his God upon him"**

The implication is that the journey was accomplished quickly and safely, and that this was further evidence of God’s favor towards Ezra. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “It only took them four months to reach Jerusalem because God was helping them”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:9 (#7) (id: 463209)

וּ⁠בְ⁠אֶחָ֞ד לַ⁠חֹ֣דֶשׁ הַ⁠חֲמִישִׁ֗י בָּ֚א אֶל־יְר֣וּשָׁלִַ֔ם כְּ⁠יַד־אֱלֹהָ֖י⁠ו הַ⁠טּוֹבָ֥ה עָלָֽי⁠ו

**"And on the first of the fifth month he came to Jerusalem, according to the good hand of his God upon him"**

If it would be helpful in your language, you could reverse the order of these phrases, since the second phrase gives the reason for the results that the first phrase describes. Alternate translation: “Because God was helping them, it only took them four months to reach Jerusalem”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:9 (#8) (id: 463210)

כְּ⁠יַד־אֱלֹהָ֖י⁠ו הַ⁠טּוֹבָ֥ה עָלָֽי⁠ו

**"according to the good hand of his God upon him"**

As in [7:6](https://ref.ly/Ezra7:6), **hand** represents action, and the expression **the good hand of his God upon him** indicates that God acted with care, protection, and favor toward Ezra throughout this journey. Alternate translation: “because God was helping them”

See: Metaphor

* **Associated Passages:** Ezra 7:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 7:10 (#1) (id: 463211)

כִּ֤י

**"For"**

The word **For** indicates that the sentence it introduces explains the reasons for what the previous sentence described. If it would be helpful to your readers, you could express this in a fuller phrase. Alternate translation: “The reason why God blessed them was that”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:10
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Decree (ID: `keyterm:Decree`); Israel (ID: `person:Israel`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:10 (#2) (id: 463212)

עֶזְרָא֙ הֵכִ֣ין לְבָב֔⁠וֹ

**"Ezra had made his heart firm"**

Here, the **heart** represents the thoughts and the will of a person. Alternate translation: “Ezra had firmly determined”

See: Metaphor

* **Associated Passages:** Ezra 7:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Decree (ID: `keyterm:Decree`); Israel (ID: `person:Israel`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:10 (#3) (id: 463213)

לִ⁠דְר֛וֹשׁ אֶת־תּוֹרַ֥ת יְהוָ֖ה וְ⁠לַ⁠עֲשֹׂ֑ת

**"to seek the law of Yahweh, and to do it"**

Just as in [6:21](https://ref.ly/Ezra6:21) to **seek** Yahweh means choosing to know, worship, and obey him, so here **to seek the law of Yahweh and to do it** means choosing to learn his law thoroughly in order to obey it, as a way of being loyal and obedient to Yahweh himself. Alternate translation: “to learn the law of Yahweh thoroughly and to obey it”

See: Metaphor

* **Associated Passages:** Ezra 7:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Decree (ID: `keyterm:Decree`); Israel (ID: `person:Israel`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:10 (#4) (id: 463214)

וּ⁠לְ⁠לַמֵּ֥ד בְּ⁠יִשְׂרָאֵ֖ל חֹ֥ק וּ⁠מִשְׁפָּֽט

**"and to teach statute and ordinance in Israel"**

Here, the book expresses a single idea by using two words connected with **and**. The words **statute** and **ordinance** both refer to commandments in the law of Moses. The book uses the two words together to refer comprehensively to everything in the law. Alternate translation: “and to teach the people of Israel everything that the law commanded”

See: Doublet

* **Associated Passages:** Ezra 7:10
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Decree (ID: `keyterm:Decree`); Israel (ID: `person:Israel`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:10 (#5) (id: 463215)

בְּ⁠יִשְׂרָאֵ֖ל

**"in Israel"**

Here, the story refers to all of the Israelites by reference to their homeland, the country of Israel. Alternate translation: “to the people of Israel”

See: Metonymy

* **Associated Passages:** Ezra 7:10
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Decree (ID: `keyterm:Decree`); Israel (ID: `person:Israel`); Instruction (ID: `keyterm:Instruction`); LORD (ID: `deity:Lord`)

## Ezra 7:11 (#1) (id: 463216)

וְ⁠זֶ֣ה

**"Now this is"**

The word **Now** introduces a new event in the story. Use an expression in your language that would be natural for introducing a new event or a change to something different.

See: Introduction of a New Event

* **Associated Passages:** Ezra 7:11
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Decree (ID: `keyterm:Decree`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Artaxerxes (ID: `person:Artaxerxes`); LORD (ID: `deity:Lord`)

## Ezra 7:11 (#2) (id: 463217)

**"the king Artaxerxes"**

Artaxerxes is the name of a man. It also occurs in [7:12](https://ref.ly/Ezra7:12) and [7:21](https://ref.ly/Ezra7:21) in this chapter. See how you translated it in [4:7](https://ref.ly/Ezra4:7). Alternate translation: “King Artaxerxes of Persia”

See: How to Translate Names

* **Associated Passages:** Ezra 7:11
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Decree (ID: `keyterm:Decree`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Artaxerxes (ID: `person:Artaxerxes`); LORD (ID: `deity:Lord`)

## Ezra 7:11 (#3) (id: 463218)

לְ⁠עֶזְרָ֥א הַ⁠כֹּהֵ֖ן הַ⁠סֹּפֵ֑ר סֹפֵ֞ר דִּבְרֵ֧י מִצְוֺת־יְהוָ֛ה וְ⁠חֻקָּ֖י⁠ו עַל־יִשְׂרָאֵֽל

**"to Ezra the priest, the scribe, a scribe of the words of the commandments of Yahweh and his statutes for Israel"**

Here, the book repeats some background information to remind readers who Ezra was. Alternate translation: “to Ezra, who was a priest and a scribe, and who had carefully studied everything that Yahweh, in the law, had commanded the people of Israel to do”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 7:11
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Decree (ID: `keyterm:Decree`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Artaxerxes (ID: `person:Artaxerxes`); LORD (ID: `deity:Lord`)

## Ezra 7:11 (#4) (id: 463219)

מִצְוֺת־יְהוָ֛ה וְ⁠חֻקָּ֖י⁠ו עַל־יִשְׂרָאֵֽל

**"the commandments of Yahweh and his statutes for Israel"**

These two phrases mean similar things. The book uses them together to refer comprehensively to everything in the law. If it would be helpful in your language, you could combine the phrases in your translation. Alternate translation: “everything that Yahweh, in the law, had commanded the people of Israel to do”

See: Doublet

* **Associated Passages:** Ezra 7:11
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Decree (ID: `keyterm:Decree`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Artaxerxes (ID: `person:Artaxerxes`); LORD (ID: `deity:Lord`)

## Ezra 7:11 (#5) (id: 463220)

יִשְׂרָאֵֽל

**"Israel"**

The story refers here to all of the Israelites as if they were a single person, their ancestor, Israel. Alternate translation: “the people of Israel”

See: Personification

* **Associated Passages:** Ezra 7:11
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Decree (ID: `keyterm:Decree`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Artaxerxes (ID: `person:Artaxerxes`); LORD (ID: `deity:Lord`)

## Ezra 7:12 (#1) (id: 463221)

אַ֨רְתַּחְשַׁ֔סְתְּא מֶ֖לֶךְ מַלְכַיָּ֑⁠א לְ⁠עֶזְרָ֣א כָ֠הֲנָ⁠א

**"Artaxerxes, the king of kings, to Ezra the priest"**

Here, the book begins to quote a letter of introduction and authorization that Artaxerxes gave to Ezra. It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 7:12
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:12 (#2) (id: 463222)

אַ֨רְתַּחְשַׁ֔סְתְּא מֶ֖לֶךְ מַלְכַיָּ֑⁠א

**"Artaxerxes, the king of kings"**

Following the conventions of letter writing at the time, Artaxerxes gives his name first, as the sender. **The king of kings** was a title, meaning that he was the greatest of kings, the king that other kings obeyed. Alternate translation: “From the Great King Artaxerxes” or “From Artaxerxes, the greatest king”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:12 (#3) (id: 463223)

לְ⁠עֶזְרָ֣א כָ֠הֲנָ⁠א סָפַ֨ר דָּתָ֜⁠א דִּֽי־אֱלָ֧הּ שְׁמַיָּ֛⁠א

**"to Ezra the priest, the scribe of the law of the God of heaven"**

As was also conventional, Artaxerxes gives Ezra’s name next, as the recipient. He also includes some background information that helps identify Ezra further. Alternate translation: “Ezra, a priest of the God who rules in heaven, who has studied his law carefully”

See: Background Information

* **Associated Passages:** Ezra 7:12
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:12 (#4) (id: 463224)

אֱלָ֧הּ שְׁמַיָּ֛⁠א

**"the God of heaven"**

See how you translated this expression in [5:11](https://ref.ly/Ezra5:11). Alternate translation: “the God who rules in heaven”

* **Associated Passages:** Ezra 7:12

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:12 (#5) (id: 463225)

גְּמִ֖יר

**"Peace"**

As in [4:17](https://ref.ly/Ezra4:17) and [5:7](https://ref.ly/Ezra5:7), this is a conventional greeting or good wish that senders at this time often included at the beginning of a letter. If your language has a similar expression that it uses for the same purpose, you can use it here. Alternate translation: “Greetings” or “We hope all is well with you”

* **Associated Passages:** Ezra 7:12

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:12 (#6) (id: 463226)

וּ⁠כְעֶֽנֶת

**"And now"**

**And now** is an Aramaic expression that was used in the letters of this time to introduce the main business of the letter. If it would be helpful, review the note about this expression at [4:10](https://ref.ly/Ezra4:10). If your language has a comparable expression that it uses for this same purpose, you could use that in your translation.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 7:12
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:13 (#1) (id: 463227)

מִנִּ⁠י֮ שִׂ֣ים טְעֵם֒ דִּ֣י

**"from me is set a decree that"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “I am issuing a decree that”

See: Active or Passive

* **Associated Passages:** Ezra 7:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 7:13 (#2) (id: 463228)

עִמָּ֖⁠ךְ

**"with you"**

The king is addressing Ezra, so **you** is singular and refers to Ezra here, and **you** and **your** similarly refer to him in all of their other occurrences through [7:20](https://ref.ly/Ezra7:20). If your language distinguishes between forms of you, the form for a superior addressing a respected individual would be appropriate.

See: Forms of You

* **Associated Passages:** Ezra 7:13
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 7:13 (#3) (id: 463229)

מִן־עַמָּ֨⁠ה יִשְׂרָאֵ֜ל

**"from the people ‘Israel"**

Here Artaxerxes specifies that **Israel** is the name of Ezra’s people group, perhaps because he does not expect everyone who sees this letter to be familiar with them already. Alternate translation: “from the people known as ‘Israel’” or “from your people who are called ‘Israel’”

* **Associated Passages:** Ezra 7:13

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 7:14 (#1) (id: 463230)

כָּ⁠ל־קֳבֵ֗ל דִּי֩ מִן־קֳדָ֨ם מַלְכָּ֜⁠א וְ⁠שִׁבְעַ֤ת יָעֲטֹ֨⁠הִי֙ שְׁלִ֔יחַ לְ⁠בַקָּרָ֥א עַל־יְה֖וּד וְ⁠לִֽ⁠ירוּשְׁלֶ֑ם בְּ⁠דָ֥ת אֱלָהָ֖⁠ךְ דִּ֥י בִ⁠ידָֽ⁠ךְ

**"Because it was sent from before the king and his seven counselors to inquire about Judah and about Jerusalem by the law of your God which is in your hand"**

Connecting Statement:\\n\\nVerses [7:14–17](https://ref.ly/Ezra7:14-Ezra7:17) are one long sentence in Aramaic. The first part ([7:14–16](https://ref.ly/Ezra7:14-Ezra7:16)) provides the reasons for the second part (in [7:17](https://ref.ly/Ezra7:17)). If it would be helpful to your readers, you could break up that sentence into several smaller sentences. To begin with, you could make this verse a sentence of its own. To do that, you could leave off the word **Because** and express the meaning later in verse 17 with a word such as “so” or “therefore.” Alternate translation: “The king and his seven counselors are sending you to inquire about Judah and about Jerusalem by the law of your God which is in your hand.”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#2) (id: 463231)

כָּ⁠ל־קֳבֵ֗ל דִּי֩

**"Because"**

Connecting Statement:\\n\\nThe word **Because** indicates that in the long sentence that follows, the next several phrases ([7:14–16](https://ref.ly/Ezra7:14-Ezra7:16)) will provide the reasons for what the final two phrases say (in [7:17](https://ref.ly/Ezra7:17)). If you have decided to keep the long sentence, you can translate this first word with the word or phrase in your language that introduces a reason. However, if it would be helpful in your language, also consider breaking up this long sentence. Alternate translation: “Since” or “In view of the fact that”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#3) (id: 463232)

מִן־קֳדָ֨ם מַלְכָּ֜⁠א וְ⁠שִׁבְעַ֤ת יָעֲטֹ֨⁠הִי֙ שְׁלִ֔יחַ

**"it was sent from before the king and his seven counselors"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “I and my seven counselors are sending you”

See: Active or Passive

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#4) (id: 463233)

מִן־קֳדָ֨ם מַלְכָּ֜⁠א וְ⁠שִׁבְעַ֤ת יָעֲטֹ֨⁠הִי֙ שְׁלִ֔יחַ

**"it was sent from before the king and his seven counselors"**

Artaxerxes speaks of himself here in the third person. If it would be helpful in your language, you could use the first person. Alternate translation: “I and my seven counselors are sending you”

See: First, Second or Third Person

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#5) (id: 463234)

מִן־קֳדָ֨ם מַלְכָּ֜⁠א וְ⁠שִׁבְעַ֤ת יָעֲטֹ֨⁠הִי֙ שְׁלִ֔יחַ

**"it was sent from before the king and his seven counselors"**

This could mean literally that Ezra has been in the king’s presence, that is, in his court, and that the king is sending him out from there. This phrase would show anyone who read the letter that Ezra was an important figure in the royal court. The phrase could also be a spatial metaphor indicating that Ezra is going out on the king’s behalf and with his authority. Alternate translation: “I and my seven counselors are sending you from my court” or “I and my seven counselors are sending you with my authority”

See: Metaphor

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#6) (id: 463235)

וְ⁠שִׁבְעַ֤ת יָעֲטֹ֨⁠הִי֙

**"and his seven counselors"**

As in [4:5](https://ref.ly/Ezra4:5), **counselors** means royal advisors in the Persian court. The seven mentioned here appear to have been the king’s closest and most important advisors. Alternate translation: “and his seven chief royal advisors”

See: Translate Unknowns

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#7) (id: 463236)

לְ⁠בַקָּרָ֥א עַל־יְה֖וּד וְ⁠לִֽ⁠ירוּשְׁלֶ֑ם בְּ⁠דָ֥ת אֱלָהָ֖⁠ךְ

**"to inquire about Judah and about Jerusalem by the law of your God"**

Artaxerxes is describing the Jews who are in the province of Judah, and especially its capital city of Jerusalem, figuratively, by reference to things that are associated with them, the province and city where they live. Alternate translation: “to see how carefully the Jews living in the province of Judah and the city of Jerusalem are following the law of your God”

See: Metonymy

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#8) (id: 463237)

לְ⁠בַקָּרָ֥א עַל־יְה֖וּד וְ⁠לִֽ⁠ירוּשְׁלֶ֑ם בְּ⁠דָ֥ת אֱלָהָ֖⁠ךְ

**"to inquire about Judah and about Jerusalem by the law of your God"**

The implication, particularly in light of the powers that the king gives Ezra at the end of the letter ([7:25–26](https://ref.ly/Ezra7:25-Ezra7:26)), is that **inquire** means more than just to find out about. It also implies to do something about. Alternate translation: “to make sure that the Jews living in the province of Judah and the city of Jerusalem follow the law of your God”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:14 (#9) (id: 463238)

דִּ֥י בִ⁠ידָֽ⁠ךְ

**"which is in your hand"**

Most likely, the word **hand** here means possession, indicating that Ezra was going to bring a written copy of the law of God with him. The phrase **in your hand** could mean that Ezra literally carried a scroll of the law in his hands or that he personally owned a written copy of the law. However, the phrase **in your hand** could also imply that Ezra will use the law as he performs his responsibilities in Judah and Jerusalem. Alternate translation: “that you carry with you” or “that you have with you” or “that you will use”

See: Metaphor

* **Associated Passages:** Ezra 7:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 7:15 (#1) (id: 463239)

וּ⁠לְ⁠הֵיבָלָ֖ה כְּסַ֣ף וּ⁠דְהַ֑ב דִּֽי־מַלְכָּ֣⁠א וְ⁠יָעֲט֗וֹ⁠הִי הִתְנַדַּ֨בוּ֙ לֶ⁠אֱלָ֣הּ יִשְׂרָאֵ֔ל דִּ֥י בִֽ⁠ירוּשְׁלֶ֖ם מִשְׁכְּנֵֽ⁠הּ

**"and to bring silver and gold that the king and his counselors have freely offered to the God of Israel whose dwelling is in Jerusalem"**

Verses [7:14–17](https://ref.ly/Ezra7:14-Ezra7:17) are one long sentence in Aramaic. If it would be helpful to your readers, you could break up that sentence into several parts. You could make [7:14](https://ref.ly/Ezra7:14) a sentence of its own, and you could divide this verse into two sentences, with the reason first and the result second. Alternate translation: “My counselors and I have freely given silver and gold for the God of Israel who lives in Jerusalem. So I am also sending you to deliver those gifts to his temple there.”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:15
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Money (ID: `realia:Money`)

## Ezra 7:15 (#2) (id: 463240)

מַלְכָּ֣⁠א וְ⁠יָעֲט֗וֹ⁠הִי

**"the king and his counselors"**

As in [7:14](https://ref.ly/Ezra7:14), Artaxerxes speaks of himself here in the third person. Alternate translation: “I and my counselors”

See: First, Second or Third Person

* **Associated Passages:** Ezra 7:15
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Money (ID: `realia:Money`)

## Ezra 7:15 (#3) (id: 463241)

דִּ֥י בִֽ⁠ירוּשְׁלֶ֖ם מִשְׁכְּנֵֽ⁠הּ

**"whose dwelling is in Jerusalem"**

Artaxerxes speaks of the God of Israel as if he actually lived in Jerusalem. Like Darius in [6:12](https://ref.ly/Ezra6:12), he seems to be echoing Jewish usage, likely to show his respect for the God of Israel. The phrase indicates that Jerusalem is the place from which God chose to start making himself known throughout the world. In this context it seems to refer specifically to the temple since, in the next verse, Artaxerxes associates the silver and gold that he and his counselors have given with further gifts that others may give for the house of God that is in Jerusalem. Alternate translation: “whose temple is in Jerusalem”

See: Personification

* **Associated Passages:** Ezra 7:15
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Money (ID: `realia:Money`)

## Ezra 7:16 (#1) (id: 463242)

וְ⁠כֹל֙ כְּסַ֣ף וּ⁠דְהַ֔ב דִּ֣י תְהַשְׁכַּ֔ח בְּ⁠כֹ֖ל מְדִינַ֣ת בָּבֶ֑ל

**"with all the silver and the gold that you find in all the province of Babylon"**

If it would be helpful to your readers, you could also divide this verse into two sentences. This phrase could be the first sentence. Here, **find** is an idiom that in this context means “can obtain.” Artaxerxes is giving Ezra permission to invite everyone in the province of Babylon to contribute. Alternate translation: “I also give you permission to ask everyone living in the province of Babylon to contribute silver and gold.”

See: Idiom

* **Associated Passages:** Ezra 7:16
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 7:16 (#2) (id: 463243)

וְ⁠כֹל֙ כְּסַ֣ף וּ⁠דְהַ֔ב דִּ֣י תְהַשְׁכַּ֔ח בְּ⁠כֹ֖ל מְדִינַ֣ת בָּבֶ֑ל

**"with all the silver and the gold that you find in all the province of Babylon"**

The implication is that these would be contributions towards the expenses of the worship of Yahweh in the temple in Jerusalem, just like the gifts described in the rest of the verse. The further implication is that, just as the king and his counselors were sympathetic to the Jews in Jerusalem and eager to please the “God of heaven” ([7:23](https://ref.ly/Ezra7:23)), others in the province might be eager to do so as well. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “I also authorize you to ask everyone living in the province of Babylon to contribute silver and gold towards the expenses of the worship of Yahweh in the temple in Jerusalem”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 7:16 (#3) (id: 463244)

בְּ⁠כֹ֖ל מְדִינַ֣ת בָּבֶ֑ל

**"in all the province of Babylon"**

Artaxerxes is describing the people who live in the province of Babylon figuratively, by reference to something associated with them, the province where they live. Alternate translation: “among everyone living in the province of Babylon”

See: Metonymy

* **Associated Passages:** Ezra 7:16
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 7:16 (#4) (id: 463245)

בָּבֶ֑ל

**"Babylon"**

**Babylon** (also called “Babylonia”) is the name of one of the provinces in the Persian Empire. See how you translated it in [1:11](https://ref.ly/Ezra1:11). Review the note there if that would be helpful.

See: How to Translate Names

* **Associated Passages:** Ezra 7:16
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 7:16 (#5) (id: 463246)

עִם֩ הִתְנַדָּב֨וּת עַמָּ֤⁠א וְ⁠כָֽהֲנַיָּ⁠א֙ מִֽתְנַדְּבִ֔ין לְ⁠בֵ֥ית אֱלָהֲ⁠הֹ֖ם דִּ֥י בִ⁠ירוּשְׁלֶֽם

**"along with the freewill offering of the people and the priests who freely give for the house of God, which is in Jerusalem"**

If you have chosen to divide this verse into two sentences, this can be the second sentence. Here, **the people** means the people of Israel, as in [7:13](https://ref.ly/Ezra7:13). Alternate translation: “I also authorize you to collect any offerings that the people of Israel and their priests want to give freely for the temple in Jerusalem.”

* **Associated Passages:** Ezra 7:16

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 7:16 (#6) (id: 463247)

דִּ֥י בִ⁠ירוּשְׁלֶֽם

**"which is in Jerusalem"**

Like Darius in [6:12](https://ref.ly/Ezra6:12), here Artaxerxes repeats some background information in order to be very explicit about which temple his command applies to. Alternate translation: “in Jerusalem”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 7:16
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 7:17 (#1) (id: 463248)

כָּ⁠ל־קֳבֵ֣ל דְּנָה֩

**"therefore"**

Connecting Statement:\\n\\nThe word **therefore** introduces the final part of the long sentence in [7:14–17](https://ref.ly/Ezra7:14-Ezra7:17). It indicates the results of the reasons that the sentence has given so far. You can translate it with the word or phrase in your language that introduces the result of a list of reasons. However, if you have followed the suggestion to break up this long sentence into several shorter sentences, you may not need to represent that meaning here.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:17
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Sheep (ID: `fauna:Lamb`); Altar (ID: `realia:Altar`); Money (ID: `realia:Money`)

## Ezra 7:17 (#2) (id: 463249)

בְּ⁠כַסְפָּ֣⁠א דְנָ֗ה

**"with this money"**

Connecting Statement:\\n\\nThe implication is that this means not just the gifts from the priests and Israelites, but also the gifts from the king, his counselors, and the people of the province of Babylon. If it would be helpful to your readers, you could indicate this more explicitly. Alternate translation: “with all of this money”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Sheep (ID: `fauna:Lamb`); Altar (ID: `realia:Altar`); Money (ID: `realia:Money`)

## Ezra 7:17 (#3) (id: 463250)

אָסְפַּ֨רְנָא תִקְנֵ֜א \& תּוֹרִ֤ין ׀ דִּכְרִין֙ אִמְּרִ֔ין וּ⁠מִנְחָתְ⁠ה֖וֹן וְ⁠נִסְכֵּי⁠ה֑וֹן

**"diligently you shall buy"** \- **"bulls, rams, lambs, and their grain offerings and their drink offerings"**

The implication, as [6:9](https://ref.ly/Ezra6:9) indicates explicitly, is that the bulls, rams, and lambs were to be used for whole burnt offerings. If it would be helpful, review the note to [3:2](https://ref.ly/Ezra3:2) about what whole burnt offerings were and why they were offered. The book also expects readers to know that the law of Moses said that two further things should be offered together with the animals that were sacrificed in whole burnt offerings. One was a certain amount of flour mixed with oil, considered a **grain offering**. The other was a certain amount of wine, considered a **drink offering**. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “be sure to buy bulls, rams, and lambs to sacrifice as whole burnt offerings, and grain, oil, and wine for the grain offerings and drink offerings that the law says must accompany them”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Sheep (ID: `fauna:Lamb`); Altar (ID: `realia:Altar`); Money (ID: `realia:Money`)

## Ezra 7:17 (#4) (id: 463251)

אָסְפַּ֨רְנָא

**"diligently"**

In their decrees, as several examples in this book show, the Persian kings used this term **diligently** to mean carefully, exactly, and efficiently. Review the note to [5:8](https://ref.ly/Ezra5:8) about this term if that would be helpful. Alternate translation: “be sure that”

* **Associated Passages:** Ezra 7:17

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Sheep (ID: `fauna:Lamb`); Altar (ID: `realia:Altar`); Money (ID: `realia:Money`)

## Ezra 7:17 (#5) (id: 463252)

תִקְנֵ֜א בְּ⁠כַסְפָּ֣⁠א דְנָ֗ה

**"you shall buy with this money"**

The word **you** here is singular, referring to Ezra. However, Ezra may not have made these purchases himself. Alternate translation: “make sure that your people use this money to buy”

See: Forms of You

* **Associated Passages:** Ezra 7:17
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Sheep (ID: `fauna:Lamb`); Altar (ID: `realia:Altar`); Money (ID: `realia:Money`)

## Ezra 7:17 (#6) (id: 463253)

וּ⁠תְקָרֵ֣ב

**"and you shall offer"**

The word **you** here is singular, referring to Ezra. However, Ezra probably did not offer these sacrifices himself. Alternate translation: “and make sure that the priests offer”

See: Forms of You

* **Associated Passages:** Ezra 7:17
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Sheep (ID: `fauna:Lamb`); Altar (ID: `realia:Altar`); Money (ID: `realia:Money`)

## Ezra 7:17 (#7) (id: 463254)

אֱלָהֲ⁠כֹ֖ם

**"your God"**

The word **your** here is plural, referring to all of the Jews.

See: Forms of You

* **Associated Passages:** Ezra 7:17
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`); Tabernacle Altar (ID: `realia:TabernacleAltar`); Stone Altar (ID: `realia:StoneAltar`); Temple Altar (ID: `realia:TempleAltar`); Sheep (ID: `fauna:Lamb`); Altar (ID: `realia:Altar`); Money (ID: `realia:Money`)

## Ezra 7:18 (#1) (id: 463255)

וּ⁠מָ֣ה דִי֩ עלי⁠ך וְ⁠עַל־אחי⁠ך יֵיטַ֗ב בִּ⁠שְׁאָ֛ר כַּסְפָּ֥⁠א וְ⁠דַהֲבָ֖⁠ה לְ⁠מֶעְבַּ֑ד כִּ⁠רְע֥וּת אֱלָהֲ⁠כֹ֖ם תַּעַבְדֽוּן

**"And you may do whatever seems good to you and to your brothers to do with the rest of the silver and the gold, according to the will of your God"**

Here, **brother** is a figurative way of saying “fellow Israelites,” and in context it likely refers to the other Jewish leaders who would decide with Ezra what to do with the extra money. Alternate translation: “You and your fellow Jewish leaders can decide what you think your God would like you to do with any silver and gold that is left over.”

See: Metaphor

* **Associated Passages:** Ezra 7:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Money (ID: `realia:Money`)

## Ezra 7:18 (#2) (id: 463256)

תַּעַבְדֽוּן

**"you may do"**

The word **you** here is plural, referring to Ezra and the other Jewish leaders.

See: Forms of You

* **Associated Passages:** Ezra 7:18
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Money (ID: `realia:Money`)

## Ezra 7:18 (#3) (id: 463257)

עלי⁠ך וְ⁠עַל־אחי⁠ך

**"to you and to your brothers"**

The words **you** and **your** here are singular, referring to Ezra.

See: Forms of You

* **Associated Passages:** Ezra 7:18
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Money (ID: `realia:Money`)

## Ezra 7:18 (#4) (id: 463258)

אֱלָהֲ⁠כֹ֖ם

**"your God"**

The word **your** here is plural, referring to all of the Jews.

See: Forms of You

* **Associated Passages:** Ezra 7:18
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Money (ID: `realia:Money`)

## Ezra 7:19 (#1) (id: 463259)

וּ⁠מָֽאנַיָּ⁠א֙

**"And the vessels"**

Connecting Statement:\\n\\nThis sentence may be introducing a contrast between what Ezra and the other Jewish leaders could do with gold and silver that people contributed in the form of money and what they had to do with gold and silver that people contributed in the form of bowls and other objects that could be used in the temple. You could begin the sentence with a word such as “however” to indicate this contrast.

See: Connecting Words and Phrases

* **Associated Passages:** Ezra 7:19
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 7:19 (#2) (id: 463260)

וּ⁠מָֽאנַיָּ⁠א֙ דִּֽי־מִתְיַהֲבִ֣ין לָ֔⁠ךְ לְ⁠פָלְחָ֖ן בֵּ֣ית אֱלָהָ֑⁠ךְ הַשְׁלֵ֕ם קֳדָ֖ם אֱלָ֥הּ יְרוּשְׁלֶֽם

**"And the vessels that were given to you for the service of the house of your God, deliver in full before the God of Jerusalem"**

As in [1:7](https://ref.ly/Ezra1:7), **vessels** refers to bowls, basins, and other objects that could be used in worship in the temple. The ones that people contributed on this occasion are described in more detail in [8:26–27](https://ref.ly/Ezra8:26-Ezra8:27). The implication in what the king says here is that Ezra and the Jewish leaders are not to sell or melt down these objects and use the proceeds to pay for temple expenses. Rather, they must use these objects only for temple worship. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “Do not use objects that people contribute for use in the temple of your God to pay for the temple expenses. Make sure that you deliver every one of them to Jerusalem, where they can be used in God’s presence”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 7:19 (#3) (id: 463261)

לָ֔⁠ךְ \& אֱלָהָ֑⁠ךְ

**"to you"** \- **"your God"**

Here, **you** and **your** are singular, referring to Ezra.

See: Forms of You

* **Associated Passages:** Ezra 7:19
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 7:19 (#4) (id: 463262)

לְ⁠פָלְחָ֖ן בֵּ֣ית אֱלָהָ֑⁠ךְ

**"for the service of the house of your God"**

The idea behind the abstract noun **service** can be expressed with a verb such as “use.” Alternate translation: “to use in the temple of your God”

See: Abstract Nouns

* **Associated Passages:** Ezra 7:19
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 7:19 (#5) (id: 463263)

הַשְׁלֵ֕ם

**"deliver in full"**

The Aramaic expression translated here as **deliver in full** means to “make complete.” It is the same one that the Jewish leaders use in [5:16](https://ref.ly/Ezra5:16) when they say that the temple is not “finished” yet. In context it means that Ezra must be sure to deliver every single one of the contributed objects to the temple, so that the complete set that was donated will be reassembled there. (We see him doing this carefully in [8:26–27](https://ref.ly/Ezra8:26-Ezra8:27)). Alternate translation: “make sure that you deliver every one of them”

See: Idiom

* **Associated Passages:** Ezra 7:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 7:19 (#6) (id: 463264)

קֳדָ֖ם אֱלָ֥הּ יְרוּשְׁלֶֽם

**"before the God of Jerusalem"**

Using a spatial metaphor, Artaxerxes says that Ezra must place the objects **before** God, meaning in God’s presence. Alternate translation: “to Jerusalem, where they can be used in God’s presence”

See: Metaphor

* **Associated Passages:** Ezra 7:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 7:19 (#7) (id: 463265)

קֳדָ֖ם אֱלָ֥הּ יְרוּשְׁלֶֽם

**"before the God of Jerusalem"**

As in [6:18](https://ref.ly/Ezra6:18), in another spatial metaphor, Artaxerxes describes God as if he lived in the city of Jerusalem. This is a figurative reference to the way God’s presence was in the temple in Jerusalem. Alternate translation: “to Jerusalem, where they can be used in God’s presence”

See: Metaphor

* **Associated Passages:** Ezra 7:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`); House (ID: `realia:House`)

## Ezra 7:20 (#1) (id: 463266)

וּ⁠שְׁאָ֗ר חַשְׁחוּת֙ בֵּ֣ית אֱלָהָ֔⁠ךְ דִּ֥י יִפֶּל־לָ֖⁠ךְ לְ⁠מִנְתַּ֑ן

**"And the rest of what is needed for the house of your God that falls to you to give"**

The phrase **that falls to you to give** is an idiom that means “that you have responsibility to give,” in other words, “that you have to provide.” Alternate translation: “And anything else that you need to provide for the temple of your God”

See: Idiom

* **Associated Passages:** Ezra 7:20
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:20 (#2) (id: 463267)

חַשְׁחוּת֙

**"what is needed for"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “whatever you might need for”

See: Active or Passive

* **Associated Passages:** Ezra 7:20
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:20 (#3) (id: 463268)

אֱלָהָ֔⁠ךְ \& לָ֖⁠ךְ \& תִּנְתֵּ֕ן

**"your God"** \- **"to you"** \- **"you may give"**

All of the forms of **you** in this verse are singular, referring to Ezra.

See: Forms of You

* **Associated Passages:** Ezra 7:20
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:20 (#4) (id: 463269)

תִּנְתֵּ֕ן מִן־בֵּ֖ית גִּנְזֵ֥י מַלְכָּֽ⁠א

**"you may give from the house of the treasures of the king"**

This does not mean, as in [5:17](https://ref.ly/Ezra5:17), the place where valuable royal documents were stored. Here, **house** likely represents all of the wealth of the king figuratively. It is a metaphor for “property,” meaning everything that a person keeps in his house, and by extension everything that he owns. So it is not necessarily a physical building, but it refers in general to all the financial resources that belong to the king, as with the shorter expression **the treasures of the king** in [6:8](https://ref.ly/Ezra6:8). Alternate translation: “you may pay for from my royal treasury”

See: Synecdoche

* **Associated Passages:** Ezra 7:20
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:20 (#5) (id: 463270)

בֵּ֖ית גִּנְזֵ֥י מַלְכָּֽ⁠א

**"the house of the treasures of the king"**

Artaxerxes speaks of himself here in the third person. If it would be helpful in your language, you could use the first person. Alternate translation: “my royal treasury”

See: First, Second or Third Person

* **Associated Passages:** Ezra 7:20
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:21 (#1) (id: 463271)

וּ֠⁠מִנִּ⁠י

**"And from me"**

Connecting Statement:\\n\\nThis phrase indicates that the sentence it introduces explains the results of what the previous sentence described. Specifically, Artaxerxes has just said that Ezra may pay for additional expenses out of the royal treasury. Consequently, regarding providing funds to Ezra, he will now give instructions to the officials responsible for administering the royal revenue in the province where Jerusalem is located. Alternate translation: “Consequently, from me”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:21
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:21 (#2) (id: 463272)

וּ֠⁠מִנִּ⁠י אֲנָ֞ה אַרְתַּחְשַׁ֤סְתְּא מַלְכָּ⁠א֙ שִׂ֣ים טְעֵ֔ם

**"And from me myself, Artaxerxes the king, is set a decree"**

Connecting Statement:\\n\\nUp to this point in the letter, Artaxerxes has been addressing Ezra, and he has spoken of himself mostly in the third person. But now as he turns to address a new audience, the royal treasurers, he speaks of himself in the first person, and he uses the emphatic form **me myself** and repeats his name and title, as in [7:12](https://ref.ly/Ezra7:12). If you have been translating the king’s use of the third person for himself in this letter with the first person in your language, it may be helpful to your readers if you use some emphatic form or extended phrase here to show this transition. Alternate translation: “And I, King Artaxerxes, am personally commanding”

See: First, Second or Third Person

* **Associated Passages:** Ezra 7:21
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:21 (#3) (id: 463273)

וּ֠⁠מִנִּ⁠י אֲנָ֞ה אַרְתַּחְשַׁ֤סְתְּא מַלְכָּ⁠א֙ שִׂ֣ים טְעֵ֔ם לְ⁠כֹל֙ \& דִּ֣י כָל־דִּ֣י יִ֠שְׁאֲלֶנְ⁠כוֹן עֶזְרָ֨א \& אָסְפַּ֖רְנָא יִתְעֲבִֽד

**"And from me myself, Artaxerxes the king, is set a decree for all"** \- **"that all that Ezra ("** \- **"may ask of you, let it be done diligently"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “And I, King Artaxerxes, am personally commanding all of you who are … to give Ezra … anything that he asks from you, and to do it exactly and promptly”

See: Active or Passive

* **Associated Passages:** Ezra 7:21
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:21 (#4) (id: 463274)

יִ֠שְׁאֲלֶנְ⁠כוֹן

**"may ask of you"**

The king is now addressing the treasurers, so **you** is plural and refers to the treasurers here and in [7:24](https://ref.ly/Ezra7:24). If your language distinguishes between forms of you, the form for a superior addressing a group of people would be appropriate.

See: Forms of You

* **Associated Passages:** Ezra 7:21
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:21 (#5) (id: 463275)

גִּזַּֽבְרַיָּ֔⁠א

**"the treasurers"**

These were royal officials responsible for administering the funds of the empire in their province.

See: Translate Unknowns

* **Associated Passages:** Ezra 7:21
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:21 (#6) (id: 463276)

בַּ⁠עֲבַ֣ר נַהֲרָ֑⁠ה

**"are in Beyond\-the\-River"**

**Beyond\-the\-River** is the name of one of the provinces of the Persian Empire. See how you translated it in [4:10](https://ref.ly/Ezra4:10).

See: How to Translate Names

* **Associated Passages:** Ezra 7:21
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:21 (#7) (id: 463277)

עֶזְרָ֨א כָהֲנָ֜⁠ה סָפַ֤ר דָּתָ⁠א֙ דִּֽי־אֱלָ֣הּ שְׁמַיָּ֔⁠א

**"Ezra (the priest, the scribe of the law of the God of heaven"**

Here, the king provides some background information to remind or inform the treasurers who Ezra was. Alternate translation: “Ezra, who is a priest of the God who rules in heaven, and who has carefully studied his law”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 7:21
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 7:22 (#1) (id: 463278)

עַד־כְּסַף֮ כַּכְּרִ֣ין מְאָה֒ וְ⁠עַד־חִנְטִין֙ כֹּרִ֣ין מְאָ֔ה וְ⁠עַד־חֲמַר֙ בַּתִּ֣ין מְאָ֔ה וְ⁠עַד־בַּתִּ֥ין מְשַׁ֖ח מְאָ֑ה וּ⁠מְלַ֖ח דִּי־לָ֥א כְתָֽב

**"as much as 100 silver kikkars, and as much as 100 cors of wheat, and as much as 100 baths of wine, and as much as 100 baths of oil, and salt which is not written"**

Here, the king is speaking in an abbreviated, official way, and the connection to the previous verse may not be entirely clear for all readers. He is describing how much he is authorizing Ezra to ask for from the treasury in money and supplies. If it would be helpful to your readers, you could say that explicitly, and you could also make this verse a sentence of its own. Alternate translation: “I am authorizing Ezra to ask you treasurers for up to 100 kikkars of silver, 100 cors of wheat, 100 baths of wine, 100 baths of oil, and an unlimited amount of salt.”

See: Ellipsis

* **Associated Passages:** Ezra 7:22
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Kor (ID: `realia:Kor`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`)

## Ezra 7:22 (#2) (id: 463279)

כְּסַף֮ כַּכְּרִ֣ין מְאָה֒

**"100 silver kikkars"**

You could try to express the modern value of this amount of silver. But since prices vary over time, that could cause your Bible translation to become outdated and inaccurate. So you could express the amount instead using the ancient measurement, the kikkar, and explain in a note that this was equivalent to about 30 kilograms. Alternatively, you could specify the weight of the silver in the text, as UST does.

See: Biblical Money

* **Associated Passages:** Ezra 7:22
* **Associated Articles:** Biblical Money (ID: `33372@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Kor (ID: `realia:Kor`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`)

## Ezra 7:22 (#3) (id: 463280)

חִנְטִין֙ כֹּרִ֣ין מְאָ֔ה

**"100 cors of wheat"**

A cor was equivalent to about 220 liters. If it would be helpful to your readers, you could express the quantity in modern measurements, as UST does, “500 bushels of wheat.” Alternatively, to help your readers recognize that the biblical writings come from a long time ago when people used different measures, you could express the amount using the ancient measurement, the cor, and explain the equivalent in modern measurements in a footnote.

See: Biblical Volume

* **Associated Passages:** Ezra 7:22
* **Associated Articles:** Biblical Volume (ID: `33373@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Kor (ID: `realia:Kor`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`)

## Ezra 7:22 (#4) (id: 463281)

וְ⁠עַד־חֲמַר֙ בַּתִּ֣ין מְאָ֔ה וְ⁠עַד־בַּתִּ֥ין מְשַׁ֖ח מְאָ֑ה

**"and as much as 100 baths of wine, and as much as 100 baths of oil"**

According to ancient measurements, **baths** were equivalent to about 22 liters. Once again, if it would be helpful to your readers, you could express the quantity in modern measurements, or you could express the amount using the ancient measurement and explain its modern equivalent in a footnote. Alternate translation: “and as much as 2,200 liters of wine, and as much as 2,200 liters of oil” or “and as much as 600 gallons of wine, and as much as 600 gallons of oil”

See: Biblical Volume

* **Associated Passages:** Ezra 7:22
* **Associated Articles:** Biblical Volume (ID: `33373@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Kor (ID: `realia:Kor`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`)

## Ezra 7:22 (#5) (id: 463282)

וּ⁠מְלַ֖ח דִּי־לָ֥א כְתָֽב

**"and salt which is not written"**

This expression means that the king is not specifying in writing a maximum amount of salt that Ezra can request, as he did for the silver, wheat, wine, and oil. Alternate translation: “and an unlimited amount of salt”

See: Idiom

* **Associated Passages:** Ezra 7:22
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Kor (ID: `realia:Kor`); Anointing Oil (ID: `realia:AnointingOil`); Wheat (ID: `flora:Wheat`); Olive Oil (ID: `realia:OliveOil`); Money (ID: `realia:Money`)

## Ezra 7:23 (#1) (id: 463283)

כָּל־דִּ֗י מִן־טַ֨עַם֙ אֱלָ֣הּ שְׁמַיָּ֔⁠א יִתְעֲבֵד֙ אַדְרַזְדָּ֔א לְ⁠בֵ֖ית אֱלָ֣הּ שְׁמַיָּ֑⁠א

**"Everything that is from the decree of the God of heaven shall be done exactly for the house of the God of heaven"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “I want you to make sure that the Jewish leaders have the funds they need to do everything that the God who rules in heaven has commanded regarding worship in his temple”

See: Active or Passive

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#2) (id: 463284)

כָּל־דִּ֗י מִן־טַ֨עַם֙ אֱלָ֣הּ שְׁמַיָּ֔⁠א

**"Everything that is from the decree of the God of heaven"**

Here, by **decree,** Artaxerxes is likely referring to what he calls the law of the God of heaven in [7:12](https://ref.ly/Ezra7:12). He means the law of Moses, and specifically the commandments in the law for how community worship was to be conducted. Artaxerxes speaks of the law as if it were a royal decree that God had issued. Alternate translation: “Everything that the God who rules in heaven has commanded”

See: Metaphor

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#3) (id: 463285)

**"for the house of the God of heaven"**

See how you translated the expressions **the house of God** in [1:4](https://ref.ly/Ezra1:4) and **the God of heaven** in [5:11](https://ref.ly/Ezra5:11). Alternate translation: “for the temple of the God who rules in heaven”

See: Metaphor

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#4) (id: 463286)

דִּֽי־לְ⁠מָ֤ה לֶֽהֱוֵא֙ קְצַ֔ף עַל־מַלְכ֥וּת מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"For why should there be wrath against the kingdom of the king and his sons"**

The king is making a statement, not asking a real question. He does not expect the treasurers to explain to him why there should be wrath against the kingdom. Instead, he is using the question form to emphasize how important it is for the treasurers to ensure that sufficient funds are provided so that worship in the temple can be conducted just as God has commanded, so that God will not be angry. If it would be helpful in your language, you could translate his words as a statement. Alternate translation: “Because I do not want God to be angry with me or with any of my descendants who rule after me”

See: Rhetorical Question

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#5) (id: 463287)

דִּֽי־לְ⁠מָ֤ה לֶֽהֱוֵא֙ קְצַ֔ף עַל־מַלְכ֥וּת מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"For why should there be wrath against the kingdom of the king and his sons"**

The implication is that Artaxerxes wants to have God’s favor, and he is concerned that God will be displeased with him instead if he does not provide for God’s temple in Jerusalem. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “I do not want God to be angry with me or with any of my descendants who rule after me because I neglected his temple.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#6) (id: 463288)

דִּֽי־לְ⁠מָ֤ה לֶֽהֱוֵא֙ קְצַ֔ף עַל־מַלְכ֥וּת מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"For why should there be wrath against the kingdom of the king and his sons"**

God’s wrath represents God’s punishment. Alternate translation: “I do not want God to punish me or any of my descendants who rule after me because I neglected his temple”

See: Metonymy

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#7) (id: 463289)

מַלְכ֥וּת מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"the kingdom of the king and his sons"**

God would not actually be angry with the realm over which Artaxerxes has authority. Rather, he uses the term **kingdom** to describe himself and his descendants by reference to something associated with them. Alternate translation: “me or any of my descendants who rule after me”

See: Metonymy

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#8) (id: 463290)

מַלְכ֥וּת מַלְכָּ֖⁠א וּ⁠בְנֽוֹ⁠הִי

**"the kingdom of the king and his sons"**

Artaxerxes speaks of himself here in the third person. If it would be helpful in your language, you could use the first person. Alternate translation: “my kingdom and that of my descendants who rule after me”

See: First, Second or Third Person

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:23 (#9) (id: 463291)

וּ⁠בְנֽוֹ⁠הִי

**"and his sons"**

This could possibly mean the biological sons of King Artaxerxes, but since he speaks of the kingdom as belonging to them, it is more likely that this is a figurative reference to his descendants, and specifically the line of direct descendants who would rule Persia as his successors (his son, his grandson, etc.). (If it would be helpful, also review the notes to [4:15](https://ref.ly/Ezra4:15) and [6:10](https://ref.ly/Ezra6:10) about how Persian kings considered their predecessors their “fathers” and their successors their **sons**.) Alternate translation: “and my descendants who rule after me”

See: Metaphor

* **Associated Passages:** Ezra 7:23
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Ezra 7:24 (#1) (id: 463292)

וּ⁠לְ⁠כֹ֣ם מְהוֹדְעִ֗ין

**"And to you is made known"**

If it would be helpful in your language, you could say this with an active form, and you could say who is doing the action. Alternate translation: “I am also declaring to you”

See: Active or Passive

* **Associated Passages:** Ezra 7:24
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 7:24 (#2) (id: 463293)

כָל־כָּהֲנַיָּ֣⁠א וְ֠⁠לֵוָיֵ⁠א זַמָּ֨רַיָּ֤⁠א תָרָֽעַיָּ⁠א֙ נְתִ֣ינַיָּ֔⁠א וּ⁠פָ֣לְחֵ֔י בֵּ֖ית אֱלָהָ֣⁠א דְנָ֑ה מִנְדָּ֤ה בְלוֹ֙ וַ⁠הֲלָ֔ךְ לָ֥א שַׁלִּ֖יט לְ⁠מִרְמֵ֥א עֲלֵי⁠הֹֽם

**"there is no authority to set tax, tribute, or custom upon all the priests, and the Levites, the singers, the gatekeepers, the temple servants, and the servants of that house of God"**

The last phrase, **the servants of that house of God**, describes what the people in the five groups listed all have in common. They all work in the temple of God. In this way, the phrase gives the reason why the treasurers may not impose taxes on anyone in any of these groups. Likely as a further way of supporting the temple, and perhaps also as a gesture of respect for God, Artaxerxes is providing an exemption from taxes for all temple personnel. If it would be helpful in your language, you could state this reason before the results that the rest of the sentence describes. Alternate translation: “because the priests, Levites, singers, gatekeepers, and temple servants all work in the temple of this God, you have no authority to set any tax, tribute, or custom upon them”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 7:24
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 7:24 (#3) (id: 463294)

**"tax, tribute, or custom"**

See how you translated this phrase in [4:13](https://ref.ly/Ezra4:13).

* **Associated Passages:** Ezra 7:24

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 7:24 (#4) (id: 463295)

וְ֠⁠לֵוָיֵ⁠א

**"and the Levites"**

See how you translated this term in [2:40](https://ref.ly/Ezra2:40). Review the explanation in the note there if that would be helpful.

See: How to Translate Names

* **Associated Passages:** Ezra 7:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 7:24 (#5) (id: 463296)

זַמָּ֨רַיָּ֤⁠א

**"the singers"**

This is the same occupational group as in [2:41](https://ref.ly/Ezra2:41). Review the explanation in the note there if that would be helpful.

* **Associated Passages:** Ezra 7:24

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 7:24 (#6) (id: 463297)

תָרָֽעַיָּ⁠א֙

**"the gatekeepers"**

This is the same occupational group as in [2:42](https://ref.ly/Ezra2:42). Review the explanation in the note there if that would be helpful.

* **Associated Passages:** Ezra 7:24

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 7:24 (#7) (id: 463298)

נְתִ֣ינַיָּ֔⁠א

**"the temple servants"**

See how you translated this term in [2:43](https://ref.ly/Ezra2:43). Review the explanation in the note there if that would be helpful.

See: How to Translate Names

* **Associated Passages:** Ezra 7:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`); House (ID: `realia:House`)

## Ezra 7:25 (#1) (id: 463299)

וְ⁠אַ֣נְתְּ עֶזְרָ֗א

**"And as for you, Ezra"**

Connecting Statement:\\n\\nThe king now returns to address Ezra personally, so **you** refers to him here, and is singular, The three instances of **your** in this verse are also singular, as is the verb **appoint**. Only the phrase **you shall teach** is plural, referring to Ezra and to those whom he will appoint. If your language distinguishes between levels of social status, the form for a superior addressing a respected individual would be appropriate.

See: Forms of You

* **Associated Passages:** Ezra 7:25
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`)

## Ezra 7:25 (#2) (id: 463300)

כְּ⁠חָכְמַ֨ת אֱלָהָ֤⁠ךְ דִּֽי־בִ⁠ידָ⁠ךְ֙

**"according to the wisdom of your God that is in your hand"**

If your language does not use abstract nouns such as **wisdom**, you can express that idea in a different way. Alternate translation: “because the law of your God has made you wise” or “guided by the wise teachings of your God that you know so well”

See: Abstract Nouns

* **Associated Passages:** Ezra 7:25
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`)

## Ezra 7:25 (#3) (id: 463301)

דִּֽי־בִ⁠ידָ⁠ךְ֙

**"that is in your hand"**

Most likely, the phrase **that is in your hand** here means the same thing as in [7:14](https://ref.ly/Ezra7:14), where the law of God is described as being in Ezra's hand. Review the note there if that would be helpful. As there, the phrase **in your hand** here could mean: (1\) that Ezra knows it and uses it with great ability. Alternate translation: “that you know and use well” or (2\) that Ezra personally owns a scroll of God's wisdom. Alternate translation: “that you have with you”

See: Metaphor

* **Associated Passages:** Ezra 7:25
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`)

## Ezra 7:25 (#4) (id: 463302)

מֶ֣נִּי שָׁפְטִ֞ין וְ⁠דַיָּנִ֗ין דִּי־לֶהֱוֺ֤ן דאנין

**"appoint magistrates and judges who may judge"**

The terms **magistrates** and **judges** mean very similar things. There are two possibilities here. (1\) Artaxerxes could be using these two terms to refer to people in slightly different roles. The first term may refer to officials who decide disputes between people, and the second term may refer to officials who interpret the law and apply it to specific situations. Alternate translation: “appoint officials who can decide disputes between people and officials who can interpret and apply the law” or (2\) Artaxerxes may be using the two words together to emphasize the authority that Ezra has to ensure that the Jews in Judah and Jerusalem follow God’s commandments. If you have only one term for these functions, you can just use that one term here. Alternate translation: “appoint judges so that they can judge”

See: Doublet

* **Associated Passages:** Ezra 7:25
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`)

## Ezra 7:25 (#5) (id: 463303)

לְ⁠כָל־עַמָּ⁠ה֙ דִּ֚י בַּ⁠עֲבַ֣ר נַהֲרָ֔⁠ה לְ⁠כָל־יָדְעֵ֖י דָּתֵ֣י אֱלָהָ֑⁠ךְ

**"all the people who are in Beyond\-the\-River, all who know the laws of your God"**

The second phrase clarifies the meaning of the first phrase, showing that Artaxerxes is referring specifically to the Jews who live in this province. He is allowing the Jews there to have their own legal system, based on the law of Moses. If it would be helpful in your language, you could say that explicitly. Alternate translation: “all the Jews living in Beyond\-the\-River province, who know the laws of your God”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:25
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`)

## Ezra 7:25 (#6) (id: 463304)

וְ⁠דִ֧י לָ֦א יָדַ֖ע תְּהוֹדְעֽוּן

**"And you shall teach those who do not know"**

The word **you** here is plural. If your language distinguishes between forms of you, the form for a superior addressing a group of people would be appropriate.

See: Forms of You

* **Associated Passages:** Ezra 7:25
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`)

## Ezra 7:25 (#7) (id: 463305)

וְ⁠דִ֧י לָ֦א יָדַ֖ע תְּהוֹדְעֽוּן

**"And you shall teach those who do not know"**

The implication, in light of the plural **you**, is that Ezra and the judges he appoints are to teach the law of God to any Jews living in the province who are not familiar with it, so that they can follow it. Alternate translation: “And I want you and the judges to teach the law of God to those Jews in the province who do not know it.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:25
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Euphrates River (ID: `place:Euphrates`)

## Ezra 7:26 (#1) (id: 463306)

וְ⁠כָל־דִּי־לָא֩ לֶהֱוֵ֨א עָבֵ֜ד דָּתָ֣⁠א דִֽי־אֱלָהָ֗⁠ךְ וְ⁠דָתָ⁠א֙ דִּ֣י מַלְכָּ֔⁠א אָסְפַּ֕רְנָא דִּינָ֕⁠ה לֶהֱוֵ֥א מִתְעֲבֵ֖ד מִנֵּ֑⁠הּ

**"And everyone who will not do the law of your God and the law of the king, judgment shall be exacted from him diligently"**

The implication is that the Jews in Judah and Jerusalem will still be governed by Persian law in addition to the law of Moses, but Artaxerxes is giving Ezra the authority to appoint judges who will administer and enforce both sets of laws. In this way, members of the Jewish community will have Jewish judges. Alternate translation: “The judges you appoint are diligently to punish anyone who refuses to obey the law of your God or my decrees”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 7:26
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 7:26 (#2) (id: 463307)

וְ⁠כָל־דִּי־לָא֩ לֶהֱוֵ֨א עָבֵ֜ד דָּתָ֣⁠א דִֽי־אֱלָהָ֗⁠ךְ וְ⁠דָתָ⁠א֙ דִּ֣י מַלְכָּ֔⁠א אָסְפַּ֕רְנָא דִּינָ֕⁠ה לֶהֱוֵ֥א מִתְעֲבֵ֖ד מִנֵּ֑⁠הּ

**"And everyone who will not do the law of your God and the law of the king, judgment shall be exacted from him diligently"**

If it would be helpful in your language, you could say this with an active form, and you could say who is to do the action. Alternate translation: “The judges you appoint are diligently to punish anyone who refuses to obey the law of your God or my decrees.”

See: Active or Passive

* **Associated Passages:** Ezra 7:26
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 7:26 (#3) (id: 463308)

וְ⁠דָתָ⁠א֙ דִּ֣י מַלְכָּ֔⁠א

**"and the law of the king"**

Artaxerxes speaks of himself in the third person. If it would be helpful in your language, you could use the first person. Alternate translation: “or obey my decrees”

See: First, Second or Third Person

* **Associated Passages:** Ezra 7:26
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Ezra 7:26 (#4) (id: 463309)

הֵ֤ן לְ⁠מוֹת֙ הֵ֣ן ל⁠שרשו הֵן־לַ⁠עֲנָ֥שׁ נִכְסִ֖ין וְ⁠לֶ⁠אֱסוּרִֽין

**"whether for death, whether for banishment, whether for confiscation of goods, or for imprisonment"**

The abstract nouns **death**, **banishment**, **confiscation**, and **imprisonment** can be translated with verbs. Alternate translation: “as your judges see fit, they may execute offenders, or make them leave the community, or seize the things that they own, or put them in prison”

See: Abstract Nouns

* **Associated Passages:** Ezra 7:26
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## Ezra 7:26 (#5) (id: 463310)

וְ⁠לֶ⁠אֱסוּרִֽין

**"or for imprisonment"**

Here, the book ends its quotation of the letter of introduction and authorization that King Artaxerxes gave to Ezra. If you decided in [7:12](https://ref.ly/Ezra7:12) to mark his words as a quotation, you should indicate that ending here with a closing primary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 7:26
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)


## Ezra 7:27 (#1) (id: 463311)

בָּר֥וּךְ יְהוָ֖ה אֱלֹהֵ֣י אֲבוֹתֵ֑י⁠נוּ

**"Blessed be Yahweh, the God of our fathers"**

Connecting Statement:\\n\\nPreviously in the book, Ezra has been a character in a story that someone else has been telling. Here he becomes the person telling the story, and he continues to tell it through to the end of chapter 9\. In effect, the letter of introduction that King Artaxerxes wrote for him has been his introduction to us as well. The book does not say specifically that Ezra will now be the narrator, although one indication is that when Ezra is telling the story, he refers to himself as “I” and to the Jewish community as “us,” and another indication is that the book is now written in Hebrew again. But if it would be helpful to your readers, you could add a phrase to indicate this. Alternate translation: “When I, Ezra, received this letter from the king, I prayed, ‘Thank you, Yahweh, the God of our ancestors’”

See: Introduction of New and Old Participants

* **Associated Passages:** Ezra 7:27
* **Associated Articles:** Introduction of New and Old Participants (ID: `33421@UWTranslationManual`)

* **Associated ACAI Concepts:** Bless (ID: `keyterm:Bless`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 7:27 (#2) (id: 463312)

בָּר֥וּךְ יְהוָ֖ה אֱלֹהֵ֣י אֲבוֹתֵ֑י⁠נוּ

**"Blessed be Yahweh, the God of our fathers"**

Connecting Statement:\\n\\nThe first thing that Ezra does when he begins to tell his own story is pray to God, rather than address the audience that will hear his story. If you decided to add a phrase to introduce him as the new narrator, you could use that same phrase to introduce this prayer as a direct quotation. Alternate translation: “When I, Ezra, received this letter from the king, I prayed, ‘Thank you, Yahweh, the God of our ancestors’” Otherwise, you could show that this prayer is distinct from the story by setting it off with quotation marks or with whatever other punctuation or convention your language uses to indicate a quotation. The quotation should end in the middle of [7:28](https://ref.ly/Ezra7:28), after “the mighty officials of the king.”

See: Aside

* **Associated Passages:** Ezra 7:27
* **Associated Articles:** Aside (ID: `33260@UWTranslationManual`)

* **Associated ACAI Concepts:** Bless (ID: `keyterm:Bless`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 7:27 (#3) (id: 463313)

בָּר֥וּךְ יְהוָ֖ה

**"Blessed be Yahweh"**

Connecting Statement:\\n\\nAlthough Ezra speaks of Yahweh in the third person here, he is actually praying a prayer of thanksgiving to Yahweh. If it would be helpful in your language, you could translate his words in the second person. Alternate translation: “Thank you, Yahweh”

See: First, Second or Third Person

* **Associated Passages:** Ezra 7:27
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Bless (ID: `keyterm:Bless`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 7:27 (#4) (id: 463314)

בָּר֥וּךְ יְהוָ֖ה

**"Blessed be Yahweh"**

Connecting Statement:\\n\\nIf it is not natural or not possible in your language to speak of a human blessing God, then you can express the idea here in a different way. Alternate translation: “I praise Yahweh” or “I thank Yahweh”

* **Associated Passages:** Ezra 7:27

* **Associated ACAI Concepts:** Bless (ID: `keyterm:Bless`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 7:27 (#5) (id: 463315)

אֱלֹהֵ֣י אֲבוֹתֵ֑י⁠נוּ

**"the God of our fathers"**

Connecting Statement:\\n\\n**Fathers** here means “ancestors.” Alternate translation: “the God of our ancestors”

See: Metaphor

* **Associated Passages:** Ezra 7:27
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bless (ID: `keyterm:Bless`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 7:27 (#6) (id: 463316)

נָתַ֤ן כָּ⁠זֹאת֙ בְּ⁠לֵ֣ב הַ⁠מֶּ֔לֶךְ לְ⁠פָאֵ֕ר אֶת־בֵּ֥ית יְהוָ֖ה אֲשֶׁ֥ר בִּ⁠ירוּשָׁלִָֽם

**"gave according to this in the heart of the king, to glorify the house of Yahweh, which is in Jerusalem"**

Giving something in the king’s heart, that is, putting something in it, means leading him to have certain thoughts and desires. Alternate translation: “led the king to want to glorify the temple of Yahweh in Jerusalem”

See: Metaphor

* **Associated Passages:** Ezra 7:27
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bless (ID: `keyterm:Bless`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 7:27 (#7) (id: 463317)

לְ⁠פָאֵ֕ר אֶת־בֵּ֥ית יְהוָ֖ה

**"to glorify the house of Yahweh"**

This expression means to make something bright, glorious, or beautiful. Artaxerxes was not doing this literally, the way that Cyrus and Darius did by supporting the physical rebuilding of the temple. Rather, Artaxerxes was making sure that the temple would always have everything it needed to keep operating. In that way he was making sure that the temple would not languish and be disrespected, but rather, always be a vibrant place that was held in honor. If it would be helpful in your language, you could express the meaning plainly. Alternate translation: “to honor the temple of Yahweh” or “to make sure that the temple of Yahweh would always be a glorious place”

See: Metaphor

* **Associated Passages:** Ezra 7:27
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bless (ID: `keyterm:Bless`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 7:28 (#1) (id: 463318)

וְ⁠עָלַ֣⁠י הִטָּה־חֶ֗סֶד לִ⁠פְנֵ֤י הַ⁠מֶּ֨לֶךְ֙ וְ⁠יֽוֹעֲצָ֔י⁠ו וּ⁠לְ⁠כָל־שָׂרֵ֥י הַ⁠מֶּ֖לֶךְ הַ⁠גִּבֹּרִ֑ים

**"and caused covenant faithfulness to extend to me before the face of the king, and his counselors, and before all the mighty officials of the king"**

Here, the abstract noun **faithfulness** refers to a person wanting to do everything that they can to help another person. As in [3:11](https://ref.ly/Ezra3:11), **covenant faithfulness** refers specifically to Yahweh doing that for the people of Israel in order to keep the promises that he had made to them. If it would be helpful in your language, you could translate the idea behind this word with a verb such as **help**. Alternate translation: “and helped me get everything I needed from the king, as I was in his presence and in the presence of his counselors and mighty officials”

See: Abstract Nouns

* **Associated Passages:** Ezra 7:28
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 7:28 (#2) (id: 463319)

וְ⁠עָלַ֣⁠י הִטָּה־חֶ֗סֶד לִ⁠פְנֵ֤י הַ⁠מֶּ֨לֶךְ֙ וְ⁠יֽוֹעֲצָ֔י⁠ו וּ⁠לְ⁠כָל־שָׂרֵ֥י הַ⁠מֶּ֖לֶךְ הַ⁠גִּבֹּרִ֑ים

**"and caused covenant faithfulness to extend to me before the face of the king, and his counselors, and before all the mighty officials of the king"**

Here, **face** represents the presence of a person. The expression means that, as Ezra stood before the king and his counselors and officials, Yahweh helped him get everything he needed from them. Alternate translation: “and helped me get everything I needed from the king, as I was in his presence and in the presence of his counselors and mighty officials”

See: Metonymy

* **Associated Passages:** Ezra 7:28
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 7:28 (#3) (id: 463320)

וְ⁠עָלַ֣⁠י הִטָּה־חֶ֗סֶד

**"and caused covenant faithfulness to extend to me"**

Ezra uses a spatial metaphor to depict help stretching out to him from Yahweh. Alternate translation: “and helped me”

See: Metaphor

* **Associated Passages:** Ezra 7:28
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 7:28 (#4) (id: 463321)

שָׂרֵ֥י הַ⁠מֶּ֖לֶךְ הַ⁠גִּבֹּרִ֑ים

**"the mighty officials of the king"**

This is the end of the prayer that Ezra prays as he begins to tell his own story. If you indicated the beginning of this prayer in [7:27](https://ref.ly/Ezra7:27) with an opening quotation mark or with the corresponding punctuation or convention in your language, you should use the same means to indicate the end of a quotation here.

See: Quote Markings

* **Associated Passages:** Ezra 7:28
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 7:28 (#5) (id: 463322)

הִתְחַזַּ֗קְתִּי כְּ⁠יַד־יְהוָ֤ה אֱלֹהַ⁠י֙ עָלַ֔⁠י

**"I gained strength according to the hand of Yahweh my God upon me"**

As in [7:6](https://ref.ly/Ezra7:6), **hand** represents power and control, and the expression **the hand of Yahweh my God upon me** indicates that Ezra enjoyed Yahweh’s care, protection, and favor. Alternate translation: “I was encouraged because I recognized that Yahweh my God was helping me”

See: Metonymy

* **Associated Passages:** Ezra 7:28
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 7:28 (#6) (id: 463323)

מִ⁠יִּשְׂרָאֵ֛ל רָאשִׁ֖ים

**"from Israel heads"**

As [8:1](https://ref.ly/Ezra8:1) shows, **heads** here is an abbreviated way of saying **heads of father’s houses**. Alternate translation: “some Israelite clan leaders”

See: Ellipsis

* **Associated Passages:** Ezra 7:28
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 7:28 (#7) (id: 463324)

לַ⁠עֲל֥וֹת עִמִּֽ⁠י

**"to go up with me"**

Ezra says **go up** because he and the group he was leading would have to travel from a river valley up into the mountains in order to return from their places of exile to Jerusalem. Alternate translation: “to return to Jerusalem with me”

See: Idiom

* **Associated Passages:** Ezra 7:28
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 8:1 (#1) (id: 463325)

וְ⁠אֵ֛לֶּה

**"Now these are"**

The word **Now** introduces a new event in the story. Use a natural method in your language to introduce a new event here.

See: Introduction of a New Event

* **Associated Passages:** Ezra 8:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 8:1 (#2) (id: 463326)

וְ⁠אֵ֛לֶּה רָאשֵׁ֥י אֲבֹתֵי⁠הֶ֖ם וְ⁠הִתְיַחְשָׂ֑⁠ם הָ⁠עֹלִ֣ים עִמִּ֗⁠י

**"Now these are the heads of their fathers and their enrollment by genealogy, the ones who went up with me"**

As in [4:3](https://ref.ly/Ezra4:3), **heads of their fathers** is an abbreviated way of saying “heads of their father’s houses.” Review the note there if that would be helpful. Alternate translation: “These are the names of the clan leaders, along with the names of their clans, of those who traveled with me”

See: Ellipsis

* **Associated Passages:** Ezra 8:1
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 8:1 (#3) (id: 463327)

הָ⁠עֹלִ֣ים עִמִּ֗⁠י \& מִ⁠בָּבֶֽל

**"the ones who went up with me from Babylon"**

As in [7:28](https://ref.ly/Ezra7:28), Ezra says **go up** because the trip from Babylon to Jerusalem would involve a significant climb in elevation. Alternate translation: “who returned from Babylon with me”

See: Idiom

* **Associated Passages:** Ezra 8:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 8:1 (#4) (id: 463328)

בְּ⁠מַלְכ֛וּת אַרְתַּחְשַׁ֥סְתְּא הַ⁠מֶּ֖לֶךְ

**"in the reign of Artaxerxes the king"**

As [7:8–9](https://ref.ly/Ezra7:8-Ezra7:9) indicates, this was specifically in the seventh year of the reign of Artaxerxes. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “during the seventh year of the reign of Artaxerxes as king of Persia.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 8:1 (#5) (id: 463329)

בְּ⁠מַלְכ֛וּת אַרְתַּחְשַׁ֥סְתְּא הַ⁠מֶּ֖לֶךְ

**"in the reign of Artaxerxes the king"**

If you choose to make explicit the information of which year this is of the king's reign, but your language does not use ordinal numbers, you can say “during year seven of the reign of Artaxerxes as king of Persia.”

See: Ordinal Numbers

* **Associated Passages:** Ezra 8:1
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Babylon (ID: `place:Babylon`); Artaxerxes (ID: `person:Artaxerxes`)

## Ezra 8:2 (#1) (id: 463330)

מִ⁠בְּנֵ֤י פִֽינְחָס֙ גֵּֽרְשֹׁ֔ם

**"From the sons of Phinehas: Gershom"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Phinehas, Gershom”

See: Metaphor

* **Associated Passages:** Ezra 8:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Daniel (ID: `person:Daniel.2`); Gershom (ID: `person:Gershom.2`); Hattush (ID: `person:Hattush.2`); David (ID: `person:David`); Phinehas (ID: `person:Phinehas`); Ithamar (ID: `person:Ithamar`)

## Ezra 8:2 (#2) (id: 463331)

פִֽינְחָס֙ גֵּֽרְשֹׁ֔ם

**"Phinehas: Gershom"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Daniel (ID: `person:Daniel.2`); Gershom (ID: `person:Gershom.2`); Hattush (ID: `person:Hattush.2`); David (ID: `person:David`); Phinehas (ID: `person:Phinehas`); Ithamar (ID: `person:Ithamar`)

## Ezra 8:2 (#3) (id: 463332)

מִ⁠בְּנֵ֥י אִיתָמָ֖ר דָּנִיֵּ֑אל

**"From the sons of Ithamar: Daniel"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Ithamar, Daniel”

See: Metaphor

* **Associated Passages:** Ezra 8:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Daniel (ID: `person:Daniel.2`); Gershom (ID: `person:Gershom.2`); Hattush (ID: `person:Hattush.2`); David (ID: `person:David`); Phinehas (ID: `person:Phinehas`); Ithamar (ID: `person:Ithamar`)

## Ezra 8:2 (#4) (id: 463333)

אִיתָמָ֖ר דָּנִיֵּ֑אל

**"Ithamar: Daniel"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Daniel (ID: `person:Daniel.2`); Gershom (ID: `person:Gershom.2`); Hattush (ID: `person:Hattush.2`); David (ID: `person:David`); Phinehas (ID: `person:Phinehas`); Ithamar (ID: `person:Ithamar`)

## Ezra 8:2 (#5) (id: 463334)

מִ⁠בְּנֵ֥י דָוִ֖יד חַטּֽוּשׁ

**"From the sons of David: Hattush"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of David, Hattush”

See: Metaphor

* **Associated Passages:** Ezra 8:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Daniel (ID: `person:Daniel.2`); Gershom (ID: `person:Gershom.2`); Hattush (ID: `person:Hattush.2`); David (ID: `person:David`); Phinehas (ID: `person:Phinehas`); Ithamar (ID: `person:Ithamar`)

## Ezra 8:2 (#6) (id: 463335)

דָוִ֖יד חַטּֽוּשׁ

**"David: Hattush"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Daniel (ID: `person:Daniel.2`); Gershom (ID: `person:Gershom.2`); Hattush (ID: `person:Hattush.2`); David (ID: `person:David`); Phinehas (ID: `person:Phinehas`); Ithamar (ID: `person:Ithamar`)

## Ezra 8:3 (#1) (id: 463336)

מִ⁠בְּנֵ֣י שְׁכַנְיָ֔ה \& מִ⁠בְּנֵ֥י פַרְעֹ֖שׁ זְכַרְיָ֑ה

**""from the sons of Shecaniah. From the sons of Parosh: Zechariah""**

This could mean one of two things. (1\) The phrase **from the sons of Shecaniah** probably applies to the end of the previous verse. 1 Chronicles 3:19–22 shows that Hattush was the grandson of Shecaniah, and that Shecaniah was either the grandson or a more distant descendant of Zerubbabel, who was a descendant of King David. (As several of the following verses show, further information like this may be provided about the clan leaders on this list.) So the end of [8:2](https://ref.ly/Ezra8:2) and the beginning of [8:3](https://ref.ly/Ezra8:3) could read: “From the descendants of King David, Hattush, one of the descendants of Shecaniah” or (2\) It could mean that Zechariah was the clan leader of the descendants of Shecaniah, and that they were all descendants of a more remote ancestor named Parosh. Alternate translation: "From the descendants of Shecaniah, who were descendants of Parosh, Zechariah"

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.18`); Shecaniah (ID: `person:Shecaniah.5`); Parosh (ID: `person:Parosh.2`)

## Ezra 8:3 (#2) (id: 463337)

מִ⁠בְּנֵ֣י שְׁכַנְיָ֔ה \& מִ⁠בְּנֵ֥י פַרְעֹ֖שׁ זְכַרְיָ֑ה

**""from the sons of Shecaniah. From the sons of Parosh: Zechariah""**

Here, **sons** means “descendants.” Alternate translation: (1\) “one of the descendants of Shecaniah. From the descendants of Parosh, Zechariah” or, if you have decided that **from the sons of Shecaniah** applies to **Zechariah**, (2\) “From the descendants of Shecaniah, who were descendants of Parosh, Zechariah”

See: Metaphor

* **Associated Passages:** Ezra 8:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.18`); Shecaniah (ID: `person:Shecaniah.5`); Parosh (ID: `person:Parosh.2`)

## Ezra 8:3 (#3) (id: 463338)

שְׁכַנְיָ֔ה \& פַרְעֹ֖שׁ זְכַרְיָ֑ה

**""Shecaniah."** \- **"Parosh: Zechariah""**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.18`); Shecaniah (ID: `person:Shecaniah.5`); Parosh (ID: `person:Parosh.2`)

## Ezra 8:3 (#4) (id: 463339)

וְ⁠עִמּ֛⁠וֹ הִתְיַחֵ֥שׂ לִ⁠זְכָרִ֖ים מֵאָ֥ה וַ⁠חֲמִשִּֽׁים

**"and with him were 150 males enrolled by genealogy"**

The implication is that these 150 males belonged to the same clan as Zechariah. Alternate translation: “along with 150 other men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zechariah (ID: `person:Zechariah.18`); Shecaniah (ID: `person:Shecaniah.5`); Parosh (ID: `person:Parosh.2`)

## Ezra 8:4 (#1) (id: 463340)

מִ⁠בְּנֵי֙ פַּחַ֣ת מוֹאָ֔ב אֶלְיְהֽוֹעֵינַ֖י בֶּן־זְרַֽחְיָ֑ה

**"From the sons of Pahath\-Moab: Eliehoenai, the son of Zerahiah"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Pahath\-Moab, Eliehoenai the son of Zerahiah”

See: Metaphor

* **Associated Passages:** Ezra 8:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerahiah (ID: `person:Zerahiah.2`); Pahath-moab (ID: `person:Pahath-moab`); Eliehoenai (ID: `person:Eliehoenai.2`)

## Ezra 8:4 (#2) (id: 463341)

פַּחַ֣ת מוֹאָ֔ב אֶלְיְהֽוֹעֵינַ֖י \& זְרַֽחְיָ֑ה

**"Pahath\-Moab: Eliehoenai,"** \- **"Zerahiah"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:4
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerahiah (ID: `person:Zerahiah.2`); Pahath-moab (ID: `person:Pahath-moab`); Eliehoenai (ID: `person:Eliehoenai.2`)

## Ezra 8:4 (#3) (id: 463342)

וְ⁠עִמּ֖⁠וֹ מָאתַ֥יִם הַ⁠זְּכָרִֽים

**"and with him were 200 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 200 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zerahiah (ID: `person:Zerahiah.2`); Pahath-moab (ID: `person:Pahath-moab`); Eliehoenai (ID: `person:Eliehoenai.2`)

## Ezra 8:5 (#1) (id: 463343)

מִ⁠בְּנֵ֥י שְׁכַנְיָ֖ה בֶּן־יַחֲזִיאֵ֑ל

**"From the sons of … Shecaniah the son of Jahaziel"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Shecaniah, the son of Jahaziel”

See: Metaphor

* **Associated Passages:** Ezra 8:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.6`); Jahaziel (ID: `person:Jahaziel.5`)

## Ezra 8:5 (#2) (id: 463344)

מִ⁠בְּנֵ֥י שְׁכַנְיָ֖ה בֶּן־יַחֲזִיאֵ֑ל

**"From the sons of … Shecaniah the son of Jahaziel"**

Since this list has been providing the name of each clan leader, it would be unusual for it to omit the leader’s name here and state only the name of his father. Most scholars believe that the name of the clan leader was accidentally left out at one point in the copying process. Other ancient versions of the Old Testament read, “from the sons of Zattu, Shecaniah the son of Jahaziel.” If it would be helpful to your readers, you could follow those versions and include the following footnote: “The name Zattu appears in the ancient Greek and other ancient versions.” Alternate translation: “From the descendants of Zattu, Shecaniah the son of Jahaziel” If you want to translate only what is in the Hebrew, you could follow the ULT or translate, “From the descendants of Shecaniah, the son of Jahaziel”

See: Textual Variants

* **Associated Passages:** Ezra 8:5
* **Associated Articles:** Textual Variants (ID: `33405@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.6`); Jahaziel (ID: `person:Jahaziel.5`)

## Ezra 8:5 (#3) (id: 463345)

שְׁכַנְיָ֖ה \& יַחֲזִיאֵ֑ל

**"Shecaniah"** \- **"Jahaziel"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.6`); Jahaziel (ID: `person:Jahaziel.5`)

## Ezra 8:5 (#4) (id: 463346)

וְ⁠עִמּ֕⁠וֹ שְׁלֹ֥שׁ מֵא֖וֹת הַ⁠זְּכָרִֽים

**"and with him were 300 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 300 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.6`); Jahaziel (ID: `person:Jahaziel.5`)

## Ezra 8:6 (#1) (id: 463347)

וּ⁠מִ⁠בְּנֵ֣י עָדִ֔ין עֶ֖בֶד בֶּן־יוֹנָתָ֑ן

**"And from the sons of Adin: Ebed, the son of Jonathan"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Adin, Ebed the son of Jonathan”

See: Metaphor

* **Associated Passages:** Ezra 8:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.9`); Adin (ID: `person:Adin`); Ebed (ID: `person:Ebed.2`)

## Ezra 8:6 (#2) (id: 463348)

עָדִ֔ין עֶ֖בֶד \& יוֹנָתָ֑ן

**"Adin: Ebed,"** \- **"Jonathan"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.9`); Adin (ID: `person:Adin`); Ebed (ID: `person:Ebed.2`)

## Ezra 8:6 (#3) (id: 463349)

וְ⁠עִמּ֖⁠וֹ חֲמִשִּׁ֥ים הַ⁠זְּכָרִֽים

**"and with him were 50 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 50 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.9`); Adin (ID: `person:Adin`); Ebed (ID: `person:Ebed.2`)

## Ezra 8:7 (#1) (id: 463350)

וּ⁠מִ⁠בְּנֵ֣י עֵילָ֔ם יְשַֽׁעְיָ֖ה בֶּן־עֲתַלְיָ֑ה

**"And from the sons of Elam: Jeshaiah, the son of Athaliah"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Elam, Jeshaiah the son of Athaliah”

See: Metaphor

* **Associated Passages:** Ezra 8:7
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Elam (ID: `person:Elam`); Jeshaiah (ID: `person:Jeshaiah.4`); Athaliah (ID: `person:Athaliah.3`)

## Ezra 8:7 (#2) (id: 463351)

עֵילָ֔ם יְשַֽׁעְיָ֖ה \& עֲתַלְיָ֑ה

**"Elam: Jeshaiah,"** \- **"Athaliah"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Elam (ID: `person:Elam`); Jeshaiah (ID: `person:Jeshaiah.4`); Athaliah (ID: `person:Athaliah.3`)

## Ezra 8:7 (#3) (id: 463352)

וְ⁠עִמּ֖⁠וֹ שִׁבְעִ֥ים הַ⁠זְּכָרִֽים

**"and with him were 70 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 70 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Elam (ID: `person:Elam`); Jeshaiah (ID: `person:Jeshaiah.4`); Athaliah (ID: `person:Athaliah.3`)

## Ezra 8:8 (#1) (id: 463353)

וּ⁠מִ⁠בְּנֵ֣י שְׁפַטְיָ֔ה זְבַדְיָ֖ה בֶּן־מִֽיכָאֵ֑ל

**"And from the sons of Shephatiah: Zebadiah, the son of Michael"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Shephatiah, Zebadiah the son of Michael”

See: Metaphor

* **Associated Passages:** Ezra 8:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Michael (ID: `person:Michael.10`); Zebadiah (ID: `person:Zebadiah.8`); Shephatiah (ID: `person:Shephatiah.6`)

## Ezra 8:8 (#2) (id: 463354)

שְׁפַטְיָ֔ה זְבַדְיָ֖ה \& מִֽיכָאֵ֑ל

**"Shephatiah: Zebadiah,"** \- **"Michael"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Michael (ID: `person:Michael.10`); Zebadiah (ID: `person:Zebadiah.8`); Shephatiah (ID: `person:Shephatiah.6`)

## Ezra 8:8 (#3) (id: 463355)

וְ⁠עִמּ֖⁠וֹ שְׁמֹנִ֥ים הַ⁠זְּכָרִֽים

**"and with him were 80 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 80 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Michael (ID: `person:Michael.10`); Zebadiah (ID: `person:Zebadiah.8`); Shephatiah (ID: `person:Shephatiah.6`)

## Ezra 8:9 (#1) (id: 463356)

מִ⁠בְּנֵ֣י יוֹאָ֔ב עֹבַדְיָ֖ה בֶּן־יְחִיאֵ֑ל

**"From the sons of Joab: Obadiah, the son of Jehiel"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Joab, Obadiah the son of Jehiel”

See: Metaphor

* **Associated Passages:** Ezra 8:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Obadiah (ID: `person:Obadiah.10`); Joab (ID: `person:Joab.4`); Jehiel (ID: `person:Jehiel.7`)

## Ezra 8:9 (#2) (id: 463357)

יוֹאָ֔ב עֹבַדְיָ֖ה \& יְחִיאֵ֑ל

**"Joab: Obadiah,"** \- **"Jehiel"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Obadiah (ID: `person:Obadiah.10`); Joab (ID: `person:Joab.4`); Jehiel (ID: `person:Jehiel.7`)

## Ezra 8:9 (#3) (id: 463358)

וְ⁠עִמּ֕⁠וֹ מָאתַ֛יִם וּ⁠שְׁמֹנָ֥ה עָשָׂ֖ר הַ⁠זְּכָרִֽים

**"and with him were 218 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 218 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Obadiah (ID: `person:Obadiah.10`); Joab (ID: `person:Joab.4`); Jehiel (ID: `person:Jehiel.7`)

## Ezra 8:10 (#1) (id: 463359)

וּ⁠מִ⁠בְּנֵ֥י שְׁלוֹמִ֖ית בֶּן־יוֹסִפְיָ֑ה

**"And from the sons of … Shelomith, the son of Josiphiah"**

Here, **sons** means “descendants”, but **son** most likely means a biological son. Alternate translation: “From the descendants of Shelomith, the son of Josiphiah”

See: Metaphor

* **Associated Passages:** Ezra 8:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Josiphiah (ID: `person:Josiphiah`); Shelomith (ID: `person:Shelomith.4`)

## Ezra 8:10 (#2) (id: 463360)

וּ⁠מִ⁠בְּנֵ֥י שְׁלוֹמִ֖ית בֶּן־יוֹסִפְיָ֑ה

**"And from the sons of … Shelomith, the son of Josiphiah"**

As in [8:5](https://ref.ly/Ezra8:5), it appears that the name of the clan was accidentally left out at one point in the copying process for this verse. Since this list has been providing the name of each clan leader, it would be unusual for it to omit the leader’s name here and state only the name of his father. Other ancient versions of the Old Testament in Greek say here, “from the descendants of Bani, Shelomith the son of Josiphiah.” If it would be helpful to your readers, you could follow those versions and include the following footnote: “The name Bani appears in ancient Greek versions.” Alternate translation: “From the descendants of Bani, Shelomith the son of Josiphiah” If you want to translate only what is in the Hebrew, you could follow the ULT or translate, “And from the descendants of Shelomith, the son of Josiphiah”

See: Textual Variants

* **Associated Passages:** Ezra 8:10
* **Associated Articles:** Textual Variants (ID: `33405@UWTranslationManual`)

* **Associated ACAI Concepts:** Josiphiah (ID: `person:Josiphiah`); Shelomith (ID: `person:Shelomith.4`)

## Ezra 8:10 (#3) (id: 463361)

שְׁלוֹמִ֖ית \& יוֹסִפְיָ֑ה

**"Shelomith,"** \- **"Josiphiah"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:10
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Josiphiah (ID: `person:Josiphiah`); Shelomith (ID: `person:Shelomith.4`)

## Ezra 8:10 (#4) (id: 463362)

וְ⁠עִמּ֕⁠וֹ מֵאָ֥ה וְ⁠שִׁשִּׁ֖ים הַ⁠זְּכָרִֽים

**"and with him were 160 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 160 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Josiphiah (ID: `person:Josiphiah`); Shelomith (ID: `person:Shelomith.4`)

## Ezra 8:11 (#1) (id: 463363)

וּ⁠מִ⁠בְּנֵ֣י בֵבַ֔י זְכַרְיָ֖ה בֶּן־בֵּבָ֑י

**"And from the sons of Bebai: Zechariah, the son of Bebai"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Bebai, Zechariah the son of Bebai”

See: Metaphor

* **Associated Passages:** Ezra 8:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bebai (ID: `person:Bebai`); Zechariah (ID: `person:Zechariah.19`); Bebai (ID: `person:Bebai.2`)

## Ezra 8:11 (#2) (id: 463364)

בֵבַ֔י זְכַרְיָ֖ה \& בֵּבָ֑י

**"Bebai: Zechariah,"** \- **"Bebai"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:11
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bebai (ID: `person:Bebai`); Zechariah (ID: `person:Zechariah.19`); Bebai (ID: `person:Bebai.2`)

## Ezra 8:11 (#3) (id: 463365)

וְ⁠עִמּ֕⁠וֹ עֶשְׂרִ֥ים וּ⁠שְׁמֹנָ֖ה הַ⁠זְּכָרִֽים

**"and with him were 28 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 28 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Bebai (ID: `person:Bebai`); Zechariah (ID: `person:Zechariah.19`); Bebai (ID: `person:Bebai.2`)

## Ezra 8:12 (#1) (id: 463366)

וּ⁠מִ⁠בְּנֵ֣י עַזְגָּ֔ד יוֹחָנָ֖ן בֶּן־הַ⁠קָּטָ֑ן

**"And from the sons of Azgad: Johanan, the son of Hakkatan"**

Here, **sons** means “descendants,” but **son** most likely means a biological son. Alternate translation: “From the descendants of Azgad, Johanan the son of Hakkatan”

See: Metaphor

* **Associated Passages:** Ezra 8:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Johanan (ID: `person:Johanan.8`); Azgad (ID: `person:Azgad`); Hakkatan (ID: `person:Hakkatan`)

## Ezra 8:12 (#2) (id: 463367)

עַזְגָּ֔ד יוֹחָנָ֖ן \& הַ⁠קָּטָ֑ן

**"Azgad: Johanan,"** \- **"Hakkatan"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:12
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Johanan (ID: `person:Johanan.8`); Azgad (ID: `person:Azgad`); Hakkatan (ID: `person:Hakkatan`)

## Ezra 8:12 (#3) (id: 463368)

וְ⁠עִמּ֕⁠וֹ מֵאָ֥ה וַ⁠עֲשָׂרָ֖ה הַ⁠זְּכָרִֽים

**"and with him were 110 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 110 men from his clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Johanan (ID: `person:Johanan.8`); Azgad (ID: `person:Azgad`); Hakkatan (ID: `person:Hakkatan`)

## Ezra 8:13 (#1) (id: 463369)

וּ⁠מִ⁠בְּנֵ֣י אֲדֹנִיקָם֮ אַחֲרֹנִים֒ וְ⁠אֵ֣לֶּה שְׁמוֹתָ֔⁠ם אֱלִיפֶ֖לֶט יְעִיאֵ֣ל וּֽ⁠שְׁמַעְיָ֑ה

**"And from the last sons of Adonikam, and these were their names: Eliphelet, Jeuel, and Shemaiah"**

The implication is that some descendants of Adonikam had still remained in Babylon even after a large number of his descendants returned to Judah with Zerubbabel some time before this, as [2:13](https://ref.ly/Ezra2:13) describes. The term **last** here indicates that these who had remained in Babylon now came with Ezra. Alternate translation: “And from the descendants of Adonikam who had remained in Babylon, the family leaders Eliphelet, Jeuel, and Shemaiah”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeuel (ID: `person:Jeuel.3`); Adonikam (ID: `person:Adonikam`); Eliphelet (ID: `person:Eliphelet.5`); Shemaiah (ID: `person:Shemaiah.14`)

## Ezra 8:13 (#2) (id: 463370)

אֲדֹנִיקָם֮ \& אֱלִיפֶ֖לֶט יְעִיאֵ֣ל וּֽ⁠שְׁמַעְיָ֑ה

**"Adonikam,"** \- **"Eliphelet, Jeuel, and Shemaiah"**

These are the names of four men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:13
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeuel (ID: `person:Jeuel.3`); Adonikam (ID: `person:Adonikam`); Eliphelet (ID: `person:Eliphelet.5`); Shemaiah (ID: `person:Shemaiah.14`)

## Ezra 8:13 (#3) (id: 463371)

וְ⁠עִמָּ⁠הֶ֖ם שִׁשִּׁ֥ים הַ⁠זְּכָרִֽים

**"and with them were 60 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Alternate translation: “along with 60 men from their clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeuel (ID: `person:Jeuel.3`); Adonikam (ID: `person:Adonikam`); Eliphelet (ID: `person:Eliphelet.5`); Shemaiah (ID: `person:Shemaiah.14`)

## Ezra 8:14 (#1) (id: 463372)

וּ⁠מִ⁠בְּנֵ֥י בִגְוַ֖י עוּתַ֣י וְזַכּ֑וּר

**"And from the sons of Bigvai: Uthai and Zaccur"**

**Sons** here means “descendants.” Alternate translation: “From the descendants of Bigvai, Uthai and Zaccur”

See: Metaphor

* **Associated Passages:** Ezra 8:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigvai (ID: `person:Bigvai.2`); Zaccur (ID: `person:Zaccur.5`); Uthai (ID: `person:Uthai.2`)

## Ezra 8:14 (#2) (id: 463373)

וְ⁠עִמּ֖⁠וֹ שִׁבְעִ֥ים הַ⁠זְּכָרִֽים

**"and with him were 70 males"**

See the note to [8:3](https://ref.ly/Ezra8:3) about this expression. Here, **him** actually refers to both Uthai and Zaccur, so you can use a plural pronoun in your translation, if it would be helpful in your language. Alternate translation: “along with 70 men from their clan”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigvai (ID: `person:Bigvai.2`); Zaccur (ID: `person:Zaccur.5`); Uthai (ID: `person:Uthai.2`)

## Ezra 8:15 (#1) (id: 463374)

וָֽ⁠אֶקְבְּצֵ֗⁠ם אֶל־הַ⁠נָּהָר֙ הַ⁠בָּ֣א אֶֽל־אַהֲוָ֔א וַ⁠נַּחֲנֶ֥ה שָׁ֖ם יָמִ֣ים שְׁלֹשָׁ֑ה

**"And I gathered them to the river, the one that goes to Ahava, and we encamped there three days"**

**Them** means everyone listed in [8:2–14](https://ref.ly/Ezra8:2-Ezra8:14), everyone who would be traveling with Ezra. The implication is that they all packed for the trip and set out from their homes, but then assembled at a location outside of the city of Babylon where Ezra could make sure the group was ready for the trip. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “I had everyone who was going to be traveling with me assemble on the banks of the river that flows from Babylon to Ahava. We camped there for three days so that I could make sure that we had enough temple personnel with us and that we could transport the gold and silver safely”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Perceive (ID: `keyterm:Perceive`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:15 (#2) (id: 463375)

הַ⁠נָּהָר֙ הַ⁠בָּ֣א אֶֽל־אַהֲוָ֔א

**"the river, the one that goes to Ahava"**

The exact location of this river on whose banks the group assembled can no longer be identified. In [8:21](https://ref.ly/Ezra8:21) and [8:31](https://ref.ly/Ezra8:31), Ezra calls it the Ahava River, and you could use that same name here if that would be helpful to your readers. Alternate translation: “the Ahava River”

See: How to Translate Names

* **Associated Passages:** Ezra 8:15
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Perceive (ID: `keyterm:Perceive`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:15 (#3) (id: 463376)

וַ⁠נַּחֲנֶ֥ה

**"and we encamped"**

As Ezra tells his story, he uses **we** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:15
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Perceive (ID: `keyterm:Perceive`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:15 (#4) (id: 463377)

וָ⁠אָבִ֤ינָ⁠ה

**"And I examined"**

The word **And** at the beginning of this phrase indicates that this event took place at the same time as the event the story has just related. If it would be helpful in your language, you could show this relationship in this case by using a phrase such as “while we were there.”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Ezra 8:15
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Perceive (ID: `keyterm:Perceive`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:15 (#5) (id: 463378)

וָ⁠אָבִ֤ינָ⁠ה בָ⁠עָם֙ וּ⁠בַ⁠כֹּ֣הֲנִ֔ים וּ⁠מִ⁠בְּנֵ֥י לֵוִ֖י לֹא־מָצָ֥אתִי שָֽׁם

**"And I examined the people and the priests, but I did not find there any from the sons of Levi"**

Here, we learn that one purpose of this preliminary encampment was for Ezra to see whether he had all the temple personnel that he needed. The implication behind this statement is that Ezra looked over the whole group to make sure of this. If it would be helpful to your readers, you could say that explicitly. (We learn in [8:25](https://ref.ly/Ezra8:25) that a further purpose of the encampment was to make arrangements for the safe transportation of the money and objects that had been donated for the temple.) Alternate translation: “I looked over the whole group and discovered that it consisted of lay people and priests, but there were no Levites in our group”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Perceive (ID: `keyterm:Perceive`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:15 (#6) (id: 463379)

וּ⁠מִ⁠בְּנֵ֥י לֵוִ֖י

**"but"** \- **"any from the sons of Levi"**

Here, **sons** means “descendants.” The Levites were descendants of Levi. Alternate translation: “Levites”

See: Metaphor

* **Associated Passages:** Ezra 8:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Perceive (ID: `keyterm:Perceive`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:16 (#1) (id: 463380)

וָ⁠אֶשְׁלְחָ֡⁠ה

**"Then I sent"**

Here the word **then** indicates that the sentence it introduces explains what Ezra did as a result of what the previous sentence described. Alternate translation: “As a result”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:16
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Elnathan (ID: `person:Elnathan.2`); Joiarib (ID: `person:Joiarib.2`); Elnathan (ID: `person:Elnathan.4`); Ariel (ID: `person:Ariel`); Nathan (ID: `person:Nathan.5`); Elnathan (ID: `person:Elnathan.3`); Shemaiah (ID: `person:Shemaiah.15`); Zechariah (ID: `person:Zechariah.20`); Jarib (ID: `person:Jarib.2`); Meshullam (ID: `person:Meshullam.9`); Eliezer (ID: `person:Eliezer.8`); Perceive (ID: `keyterm:Perceive`)

## Ezra 8:16 (#2) (id: 463381)

וָ⁠אֶשְׁלְחָ֡⁠ה לֶ⁠אֱלִיעֶ֡זֶר לַ⁠אֲרִיאֵ֡ל לִֽ֠⁠שְׁמַעְיָה וּ⁠לְ⁠אֶלְנָתָ֨ן וּ⁠לְ⁠יָרִ֜יב וּ⁠לְ⁠אֶלְנָתָ֧ן וּ⁠לְ⁠נָתָ֛ן וְ⁠לִ⁠זְכַרְיָ֥ה וְ⁠לִ⁠מְשֻׁלָּ֖ם

**"Then I sent for Eliezer, for Ariel, for Shemaiah, and for Elnathan, and for Jarib, and for Elnathan and for Nathan, and for Zechariah, and for Meshullam"**

Here, Ezra lists the names of nine men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:16
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Elnathan (ID: `person:Elnathan.2`); Joiarib (ID: `person:Joiarib.2`); Elnathan (ID: `person:Elnathan.4`); Ariel (ID: `person:Ariel`); Nathan (ID: `person:Nathan.5`); Elnathan (ID: `person:Elnathan.3`); Shemaiah (ID: `person:Shemaiah.15`); Zechariah (ID: `person:Zechariah.20`); Jarib (ID: `person:Jarib.2`); Meshullam (ID: `person:Meshullam.9`); Eliezer (ID: `person:Eliezer.8`); Perceive (ID: `keyterm:Perceive`)

## Ezra 8:16 (#3) (id: 463382)

רָאשִׁ֑ים

**"the heads"**

Unlike in [7:28](https://ref.ly/Ezra7:28), here this expression does not mean heads of father’s houses, that is, clan leaders. Rather, **heads** here has a more general meaning and is simply a figurative way of saying “leaders.” None of these nine men, with the possible exception of Zechariah, are among the clan leaders listed in [8:2–14](https://ref.ly/Ezra8:2-Ezra8:14). (And considering that three of the men whom Ezra summoned were named Elnathan, which shows that several individuals could share the same name, it is quite possible that this is a different Zechariah from either the one named in [8:3](https://ref.ly/Ezra8:3) or the one named in [8:11](https://ref.ly/Ezra8:11).) Alternate translation: “who were all leaders”

See: Metaphor

* **Associated Passages:** Ezra 8:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Elnathan (ID: `person:Elnathan.2`); Joiarib (ID: `person:Joiarib.2`); Elnathan (ID: `person:Elnathan.4`); Ariel (ID: `person:Ariel`); Nathan (ID: `person:Nathan.5`); Elnathan (ID: `person:Elnathan.3`); Shemaiah (ID: `person:Shemaiah.15`); Zechariah (ID: `person:Zechariah.20`); Jarib (ID: `person:Jarib.2`); Meshullam (ID: `person:Meshullam.9`); Eliezer (ID: `person:Eliezer.8`); Perceive (ID: `keyterm:Perceive`)

## Ezra 8:16 (#4) (id: 463383)

וּ⁠לְ⁠יוֹיָרִ֥יב וּ⁠לְ⁠אֶלְנָתָ֖ן

**"and for Joiarib, and for Elnathan"**

Here Ezra lists the names of two more men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:16
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Elnathan (ID: `person:Elnathan.2`); Joiarib (ID: `person:Joiarib.2`); Elnathan (ID: `person:Elnathan.4`); Ariel (ID: `person:Ariel`); Nathan (ID: `person:Nathan.5`); Elnathan (ID: `person:Elnathan.3`); Shemaiah (ID: `person:Shemaiah.15`); Zechariah (ID: `person:Zechariah.20`); Jarib (ID: `person:Jarib.2`); Meshullam (ID: `person:Meshullam.9`); Eliezer (ID: `person:Eliezer.8`); Perceive (ID: `keyterm:Perceive`)

## Ezra 8:16 (#5) (id: 463384)

מְבִינִֽים

**"men of understanding"**

This phrase suggests that while Joiarib and Elnathan were not community leaders like the other nine men, Ezra sent them anyway to help recruit more temple personnel because they were wise and reasonable and could be persuasive. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “because, even though they were not leaders, they were discerning”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Elnathan (ID: `person:Elnathan.2`); Joiarib (ID: `person:Joiarib.2`); Elnathan (ID: `person:Elnathan.4`); Ariel (ID: `person:Ariel`); Nathan (ID: `person:Nathan.5`); Elnathan (ID: `person:Elnathan.3`); Shemaiah (ID: `person:Shemaiah.15`); Zechariah (ID: `person:Zechariah.20`); Jarib (ID: `person:Jarib.2`); Meshullam (ID: `person:Meshullam.9`); Eliezer (ID: `person:Eliezer.8`); Perceive (ID: `keyterm:Perceive`)

## Ezra 8:17 (#1) (id: 463385)

אוֹתָ⁠ם֙ עַל־אִדּ֣וֹ הָ⁠רֹ֔אשׁ בְּ⁠כָסִפְיָ֖א הַ⁠מָּק֑וֹם

**"them to go to Iddo, the head at the place Kasiphia"**

The implication, since Ezra is trying to recruit Levites, is that Iddo was the leader of the Levites in that place. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “them to speak to a man named Iddo, who was the leader of the Levites at a place called Kasiphia”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Iddo (ID: `person:Iddo`); Casiphia (ID: `place:Casiphia`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:17 (#2) (id: 463386)

אִדּ֣וֹ

**"Iddo"**

**Iddo** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 8:17
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Iddo (ID: `person:Iddo`); Casiphia (ID: `place:Casiphia`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:17 (#3) (id: 463387)

בְּ⁠כָסִפְיָ֖א

**"at"** \- **"Kasiphia"**

**Kasiphia** is the name of a place. Its exact location can no longer be identified.

See: How to Translate Names

* **Associated Passages:** Ezra 8:17
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Iddo (ID: `person:Iddo`); Casiphia (ID: `place:Casiphia`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:17 (#4) (id: 463388)

וָ⁠אָשִׂימָ⁠ה֩ בְּ⁠פִי⁠הֶ֨ם דְּבָרִ֜ים לְ֠⁠דַבֵּר אֶל־אִדּ֨וֹ

**"And I put in their mouth words to speak to Iddo"**

Putting words in someone’s mouth means telling them what to say. Alternate translation: “And I told them what to say to Iddo”

See: Metaphor

* **Associated Passages:** Ezra 8:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Iddo (ID: `person:Iddo`); Casiphia (ID: `place:Casiphia`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:17 (#5) (id: 463389)

אִדּ֨וֹ אָחִ֤י⁠ו הַנְּתִינִים֙ בְּ⁠כָסִפְיָ֣א הַ⁠מָּק֔וֹם

**"Iddo and his brothers, the temple servants in the place Kasiphia"**

Here, **brothers** is a figurative way of referring to people who belonged to groups that could serve in the temple, as Iddo did. (It is possible that some of his biological brothers are also included in this term.) In context, Iddo is a Levite leader, while the **brothers** are said to include **temple servants**, so the term indicates both groups. Alternate translation: “Iddo and his fellow Levites and the temple servants who were also living in Kasiphia”

See: Metaphor

* **Associated Passages:** Ezra 8:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Iddo (ID: `person:Iddo`); Casiphia (ID: `place:Casiphia`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:17 (#6) (id: 463390)

לְ⁠הָֽבִיא־לָ֥⁠נוּ מְשָׁרְתִ֖ים לְ⁠בֵ֥ית אֱלֹהֵֽי⁠נוּ

**"to bring to us those who minister for the house of our God"**

This is the purpose for which Ezra sent the messengers to Iddo. If it would be helpful in your language, you could indicate that in your translation, or you could present this as a direct quotation. Alternate translation: “so that he would send us more people who could serve in the temple of our God” or, as a direct quotation, “to say, ‘Please send us some people who can serve in the temple of our God’”

See: Direct and Indirect Quotations

* **Associated Passages:** Ezra 8:17
* **Associated Articles:** Direct and Indirect Quotations (ID: `33305@UWTranslationManual`)

* **Associated ACAI Concepts:** Iddo (ID: `person:Iddo`); Casiphia (ID: `place:Casiphia`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:17 (#7) (id: 463391)

לָ֥⁠נוּ

**"to us"**

As Ezra tells his story, he uses **us** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:17
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Iddo (ID: `person:Iddo`); Casiphia (ID: `place:Casiphia`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:18 (#1) (id: 463392)

וַ⁠יָּבִ֨יאּוּ

**"And"** \- **"they brought"**

This word indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result … they brought”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:18
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Sherebiah (ID: `person:Sherebiah`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Mahli (ID: `person:Mahli`); Levi (ID: `person:Levi.3`); Insight (ID: `keyterm:Insight`); LORD (ID: `deity:Lord`)

## Ezra 8:18 (#2) (id: 463393)

כְּ⁠יַד־אֱלֹהֵ֨י⁠נוּ הַ⁠טּוֹבָ֤ה עָלֵ֨י⁠נוּ֙

**"according to the good hand of our God upon us"**

As in the expression **according to the good hand of his God upon him** in [7:6](https://ref.ly/Ezra7:6), **hand** represents action, and the expression indicates that God showed Ezra favor and helped him on this occasion. Alternate translation: “because our God was helping us”

See: Metaphor

* **Associated Passages:** Ezra 8:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Sherebiah (ID: `person:Sherebiah`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Mahli (ID: `person:Mahli`); Levi (ID: `person:Levi.3`); Insight (ID: `keyterm:Insight`); LORD (ID: `deity:Lord`)

## Ezra 8:18 (#3) (id: 463394)

לָ֜⁠נוּ \& עָלֵ֨י⁠נוּ֙

**"upon us,"** \- **"to us"**

As Ezra tells his story, he uses **us** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:18
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Sherebiah (ID: `person:Sherebiah`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Mahli (ID: `person:Mahli`); Levi (ID: `person:Levi.3`); Insight (ID: `keyterm:Insight`); LORD (ID: `deity:Lord`)

## Ezra 8:18 (#4) (id: 463395)

אִ֣ישׁ שֶׂ֔כֶל

**"a man of insight"**

Here, **insight**, that is, the ability to “see into” things, means to have good judgment. Alternate translation: “a very prudent man”

See: Metaphor

* **Associated Passages:** Ezra 8:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Sherebiah (ID: `person:Sherebiah`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Mahli (ID: `person:Mahli`); Levi (ID: `person:Levi.3`); Insight (ID: `keyterm:Insight`); LORD (ID: `deity:Lord`)

## Ezra 8:18 (#5) (id: 463396)

מִ⁠בְּנֵ֣י מַחְלִ֔י בֶּן־לֵוִ֖י בֶּן־יִשְׂרָאֵ֑ל וְ⁠שֵׁרֵֽבְיָ֛ה

**"from the sons of Mahli, the son of Levi, the son of Israel, even Sherebiah"**

**Sons** means “descendants” in reference to Mahli, since he was a distant ancestor of the group that Sherebiah belonged to. The word **son** means “descendant” when applied to Levi, since Mahli was actually his grandson. But Levi actually was the biological son of Israel. Alternate translation: “named Sherebiah, one of the descendants of Mahli, the grandson of Levi, the son of Israel”

See: Metaphor

* **Associated Passages:** Ezra 8:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Sherebiah (ID: `person:Sherebiah`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Mahli (ID: `person:Mahli`); Levi (ID: `person:Levi.3`); Insight (ID: `keyterm:Insight`); LORD (ID: `deity:Lord`)

## Ezra 8:18 (#6) (id: 463397)

מַחְלִ֔י \& לֵוִ֖י \& יִשְׂרָאֵ֑ל וְ⁠שֵׁרֵֽבְיָ֛ה

**"the"** \- **"Mahli,"** \- **"Levi,"** \- **"Israel, even Sherebiah"**

These are the names of four men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:18
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Sherebiah (ID: `person:Sherebiah`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Mahli (ID: `person:Mahli`); Levi (ID: `person:Levi.3`); Insight (ID: `keyterm:Insight`); LORD (ID: `deity:Lord`)

## Ezra 8:18 (#7) (id: 463398)

וּ⁠בָנָ֥י⁠ו וְ⁠אֶחָ֖י⁠ו שְׁמֹנָ֥ה עָשָֽׂר

**"and his sons and his brothers were 18"**

Since this was a small group of related people, it is quite possible that it consisted largely, if not entirely, of Sherebiah’s actual sons and brothers. So you could, in your translation, decide to treat the terms as literal. On the other hand, they could also be figurative, with **sons** meaning “descendants” and **brothers** meaning “relatives.” Alternate translation: “along with 18 of his sons and brothers” or “along with 18 of his descendants and relatives”

See: Metaphor

* **Associated Passages:** Ezra 8:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Sherebiah (ID: `person:Sherebiah`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Mahli (ID: `person:Mahli`); Levi (ID: `person:Levi.3`); Insight (ID: `keyterm:Insight`); LORD (ID: `deity:Lord`)

## Ezra 8:19 (#1) (id: 463399)

וְ⁠אֶת־חֲשַׁבְיָ֔ה וְ⁠אִתּ֥⁠וֹ יְשַֽׁעְיָ֖ה מִ⁠בְּנֵ֣י מְרָרִ֑י

**"and Hashabiah, and with him, Jeshaiah, from the sons of Merari"**

Here, **sons** means “descendants.” Alternate translation: “and they also sent us Hashabiah and Jeshaiah, from the descendants of Merari”

See: Metaphor

* **Associated Passages:** Ezra 8:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Jeshaiah (ID: `person:Jeshaiah.5`); Merari (ID: `person:Merari`)

## Ezra 8:19 (#2) (id: 463400)

חֲשַׁבְיָ֔ה \& יְשַֽׁעְיָ֖ה \& מְרָרִ֑י

**"Hashabiah,"** \- **"Jeshaiah,"** \- **"Merari"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 8:19
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Jeshaiah (ID: `person:Jeshaiah.5`); Merari (ID: `person:Merari`)

## Ezra 8:19 (#3) (id: 463401)

אֶחָ֥י⁠ו וּ⁠בְנֵי⁠הֶ֖ם

**"whose brothers and their sons"**

This most likely means the brothers of Jeshaiah and the sons of Jeshaiah and of his brothers. Since, as in [8:18](https://ref.ly/Ezra8:18), this is a small group of related people, you could decide to treat the terms **brothers** and **sons** here as either literal or figurative. Alternate translation: “along with his brothers and their sons” or “along with his relatives and their descendants”

See: Metaphor

* **Associated Passages:** Ezra 8:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Jeshaiah (ID: `person:Jeshaiah.5`); Merari (ID: `person:Merari`)

## Ezra 8:20 (#1) (id: 463402)

וּ⁠מִן־הַ⁠נְּתִינִ֗ים שֶׁ⁠נָּתַ֨ן דָּוִ֤יד וְ⁠הַ⁠שָּׂרִים֙ לַ⁠עֲבֹדַ֣ת הַ⁠לְוִיִּ֔ם נְתִינִ֖ים מָאתַ֣יִם וְ⁠עֶשְׂרִ֑ים

**"and 220 temple servants from the temple servants whom David with the officials had given for the service of the Levites"**

Here Ezra provides some background information to help identify who these **temple servants** were. Alternate translation: “and they also brought 220 men who were from the families that David and his officials had assigned to help the Levites long ago”

See: Background Information

* **Associated Passages:** Ezra 8:20
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Work (ID: `keyterm:Work`); David (ID: `person:David`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:20 (#2) (id: 463403)

כֻּלָּ֖⁠ם נִקְּב֥וּ בְ⁠שֵׁמֽוֹת

**"All of them were designated by names"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “I recorded the name of each one of them.”

See: Active or Passive

* **Associated Passages:** Ezra 8:20
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Work (ID: `keyterm:Work`); David (ID: `person:David`); Levi (ID: `person:Levi.3`); Tribe of Levi (ID: `group:Levi`)

## Ezra 8:21 (#1) (id: 463404)

וָ⁠אֶקְרָ֨א

**"Then I proclaimed"**

The word **then** indicates that the event the story will now relate came after the event it has just described. In your translation, you can use the expression in your language that would best indicate this relationship.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:21 (#2) (id: 463405)

צוֹם֙ \& לְ⁠הִתְעַנּ֖וֹת

**"a fast"** \- **"to afflict ourselves"**

The travelers would be afflicting themselves, that is, making themselves suffer, by denying themselves food. This was a gesture of humility and an expression that seeking God’s help was more important to them even than eating. Alternate translation: “a time of going without eating … to humble ourselves”

See: Symbolic Action

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:21 (#3) (id: 463406)

**"the river Ahava"**

See how you translated this name in [8:15](https://ref.ly/Ezra8:15). Alternate translation: “the Ahava River”

See: How to Translate Names

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:21 (#4) (id: 463407)

לְ⁠הִתְעַנּ֖וֹת

**"to afflict ourselves"**

Here Ezra is using **ourselves** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:21 (#5) (id: 463408)

לִ⁠פְנֵ֣י אֱלֹהֵ֑י⁠נוּ

**"before the face of our God"**

Here, **face** represents the presence of a person. Alternate translation: “in the presence of our God”

See: Metonymy

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:21 (#6) (id: 463409)

אֱלֹהֵ֑י⁠נוּ

**"our God"**

In this case, Ezra is using **our** in the inclusive sense, that is, it includes the reader, since he is reporting his trip to fellow believers in God. Alternate translation: “The God to whom we all belong”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:21 (#7) (id: 463410)

לָ֥⁠נוּ וּ⁠לְ⁠טַפֵּ֖⁠נוּ וּ⁠לְ⁠כָל־רְכוּשֵֽׁ⁠נוּ

**"for us, and for our children, and for all our property"**

As Ezra tells his story, he uses **us** and **our** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:21 (#8) (id: 463411)

לְ⁠בַקֵּ֤שׁ מִמֶּ֨⁠נּוּ֙ דֶּ֣רֶךְ יְשָׁרָ֔ה לָ֥⁠נוּ וּ⁠לְ⁠טַפֵּ֖⁠נוּ וּ⁠לְ⁠כָל־רְכוּשֵֽׁ⁠נוּ

**"to seek from him a straight way for us, and for our children, and for all our property"**

The word **seek** represents asking God to do something for them. A **straight way** represents safety during travel. Alternate translation: “and to ask God to protect us while we traveled and also protect our children and all of our belongings”

See: Metaphor

* **Associated Passages:** Ezra 8:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); LORD (ID: `deity:Lord`)

## Ezra 8:22 (#1) (id: 463412)

כִּ֣י

**"For"**

Here the word **For** indicates that the sentence it introduces explains the reasons for what the previous sentence described. Alternate translation: “I did this because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#2) (id: 463413)

בֹ֗שְׁתִּי לִ⁠שְׁא֤וֹל מִן־הַ⁠מֶּ֨לֶךְ֙ חַ֣יִל וּ⁠פָרָשִׁ֔ים לְ⁠עָזְרֵ֥⁠נוּ מֵ⁠אוֹיֵ֖ב בַּ⁠דָּ֑רֶךְ

**"I was ashamed to ask from the king an army and horsemen to help us from an enemy on the way"**

**An enemy** means “any enemy,” and **on the way** means “during our journey.” Alternate translation: “I would have been embarrassed to ask the king for soldiers and horsemen to protect us from anyone who might try to harm or rob us during our journey”

* **Associated Passages:** Ezra 8:22

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#3) (id: 463414)

לְ⁠עָזְרֵ֥⁠נוּ \& אָמַ֨רְנוּ

**"to help us"** \- **"we had spoken"**

As Ezra tells his story, he uses **us** and **we** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#4) (id: 463415)

כִּֽי

**"For"**

Here the word **For** indicates that the sentence it introduces explains the reasons for what the previous sentence described. Alternate translation: “Because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#5) (id: 463416)

**"we had spoken to the king, saying, “The hand of our God is for good upon all those who seek him, but his might and his nose are against all those who forsake him"**

This sentence gives the reason for the results that the previous sentence in the verse describes. If it would be helpful in your language, you could put this sentence first in the verse, before the other sentence. Then you could show the connection between the sentences by using a phrase such as “and so” after this sentence. Alternate translation: “We had told the king, ‘Our God protects everyone who obeys him, but he punishes everyone who refuses to serve him.’ And so”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#6) (id: 463417)

יַד־אֱלֹהֵ֤י⁠נוּ עַל־כָּל־מְבַקְשָׁי⁠ו֙ לְ⁠טוֹבָ֔ה וְ⁠עֻזּ֣⁠וֹ וְ⁠אַפּ֔⁠וֹ עַ֖ל כָּל־עֹזְבָֽי⁠ו

**"The hand of our God is for good upon all those who seek him, but his might and his nose are against all those who forsake him"**

This is a direct quotation. Ezra is quoting what he and his fellow travelers had told King Artaxerxes. It may be helpful to your readers to indicate this by setting off these words within quotation marks or by following whatever other punctuation or convention your language uses to indicate a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#7) (id: 463418)

יַד־אֱלֹהֵ֤י⁠נוּ עַל־כָּל־מְבַקְשָׁי⁠ו֙ לְ⁠טוֹבָ֔ה וְ⁠עֻזּ֣⁠וֹ וְ⁠אַפּ֔⁠וֹ עַ֖ל כָּל־עֹזְבָֽי⁠ו

**"The hand of our God is for good upon all those who seek him, but his might and his nose are against all those who forsake him"**

This is a direct quotation. If it would be more natural in your language, you could make this an indirect quotation, as in the UST.

See: Direct and Indirect Quotations

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Direct and Indirect Quotations (ID: `33305@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#8) (id: 463419)

יַד־אֱלֹהֵ֤י⁠נוּ \& לְ⁠טוֹבָ֔ה

**"The hand of our God is for good"**

This expression has the same meaning as **the good hand of God** in [7:6](https://ref.ly/Ezra7:6). Here, the word **hand** represents action, and the phrase **the hand of our God is for good** means the care, protection, and favor of God. Alternate translation: “The favor of our God is”

See: Metaphor

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#9) (id: 463420)

אֱלֹהֵ֤י⁠נוּ

**"our God"**

In this case, Ezra is using **our** in the inclusive sense, that is, it includes the reader, since he is reporting his trip to fellow believers in God. Alternate translation: “the God to whom we all belong”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#10) (id: 463421)

כָּל־מְבַקְשָׁי⁠ו֙

**"all those who seek him"**

To **seek him** is a metaphor for worshiping, serving, and obeying him. Alternate translation: “everyone who loves and obeys him”

See: Metaphor

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#11) (id: 463422)

וְ⁠עֻזּ֣⁠וֹ וְ⁠אַפּ֔⁠וֹ עַ֖ל

**"but his might and his nose are against"**

God’s power and anger being on people is a metonym for him punishing them. Alternate translation: “but he punishes”

See: Metonymy

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#12) (id: 463423)

וְ⁠עֻזּ֣⁠וֹ וְ⁠אַפּ֔⁠וֹ

**"but his might and his nose"**

Here, the **nose** represents anger. Alternate translation: “his power and his wrath”

See: Metaphor

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#13) (id: 463424)

וְ⁠עֻזּ֣⁠וֹ וְ⁠אַפּ֔⁠וֹ

**"but his might and his nose"**

This phrase expresses a single idea by using two words connected with **and**. The word **nose**, a figure for anger, tells how God punishes. If it would be helpful in your language, you could express the meaning with a single phrase. Alternate translation: “he wrathfully punishes”

See: Hendiadys

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:22 (#14) (id: 463425)

כָּל־עֹזְבָֽי⁠ו

**"all those who forsake him"**

Forsaking or abandoning God is a metaphor for refusing to serve him. Alternate translation: “all who refuse to serve him”

See: Metaphor

* **Associated Passages:** Ezra 8:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:23 (#1) (id: 463426)

וַ⁠נָּצ֛וּמָ⁠ה

**"So we fasted"**

The word **So** indicates that the sentence it introduces explains the results of what the previous sentence described. Because Ezra was embarrassed to ask the king for protection, he and his fellow travelers earnestly prayed and fasted for God’s protection. Alternate translation: “For this reason, we fasted”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:23
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:23 (#2) (id: 463427)

וַ⁠נָּצ֛וּמָ⁠ה וַ⁠נְּבַקְשָׁ֥⁠ה מֵ⁠אֱלֹהֵ֖י⁠נוּ עַל־זֹ֑את וַ⁠יֵּעָתֵ֖ר לָֽ⁠נוּ

**"So we fasted and sought from our God concerning this, and he was entreated by us"**

If it would be helpful in your language, you could use an active form for the last phrase. Alternate translation: “For this reason, we fasted and prayed to God about this, and he answered our prayer”

See: Active or Passive

* **Associated Passages:** Ezra 8:23
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:23 (#3) (id: 463428)

וַ⁠נָּצ֛וּמָ⁠ה \& לָֽ⁠נוּ

**"So we fasted"** \- **"by us"**

As Ezra tells his story, he uses **we** and **us** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:23
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:23 (#4) (id: 463429)

מֵ⁠אֱלֹהֵ֖י⁠נוּ

**"from our God"**

In this case, Ezra is using **our** in the inclusive sense, that is, it includes the reader, since he is reporting his trip to fellow believers in God. Alternate translation: “from the God to whom we all belong”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:23
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:23 (#5) (id: 463430)

וַ⁠יֵּעָתֵ֖ר לָֽ⁠נוּ

**"and he was entreated by us"**

As is clear from the way this expression is used elsewhere in the Bible, it means that God answered a prayer. For example, Genesis 25:21 says, “Isaac entreated Yahweh for his wife because she could not have children, and Yahweh was entreated by him, and Rebekah his wife conceived.” However, since Ezra does not describe until [8:32](https://ref.ly/Ezra8:32) how the group arrived safely in Jerusalem, the prayer had not yet been answered at this point in the story. So an alternative meaning in context would be that the group received assurance that God would answer their prayer. Alternate translation: “and he answered our prayer” or “and we felt assured that he would answer our prayer”

See: Idiom

* **Associated Passages:** Ezra 8:23
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 8:24 (#1) (id: 463431)

וָ⁠אַבְדִּ֛ילָ⁠ה

**"Then I separated"**

The word **Then** indicates that the event the story will now relate came after the event it has just described. In your translation, you can use the expression in your language that would best indicate this relationship.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 8:24
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Sherebiah (ID: `person:Sherebiah`)

## Ezra 8:24 (#2) (id: 463432)

וָ⁠אַבְדִּ֛ילָ⁠ה מִ⁠שָּׂרֵ֥י הַ⁠כֹּהֲנִ֖ים שְׁנֵ֣ים עָשָׂ֑ר לְ⁠שֵׁרֵֽבְיָ֣ה חֲשַׁבְיָ֔ה וְ⁠עִמָּ⁠הֶ֥ם מֵ⁠אֲחֵי⁠הֶ֖ם עֲשָׂרָֽה

**"Then I separated 12 from the leaders of the priests to Sherebiah, Hashabiah, and with them ten from their brothers"**

The Hebrew text is difficult here. One way to read it is: “Then I separated 12 from the leaders of the priests: Sherebiah, Hashabiah, and with them ten from their brothers.” But since [8:18](https://ref.ly/Ezra8:18) and [19](https://ref.ly/Ezra8:19) specifically identify Sherebiah and Hashabiah as Levites, they could not have been leaders of the priests. An ancient Greek version of the book of Ezra says “and” before Sherebiah, so that this is a list of 24 men in total. Another way to read the Hebrew is as in the ULT, which also results in a list of 24 men. Both of these translations agree with [8:30](https://ref.ly/Ezra8:30), which says that “the priests and the Levites” received the silver and gold and objects that Ezra weighed out to them. It will likely be clearest for your readers if you translate the verse in one of the latter two ways. Alternate translation: “Then I selected 12 leaders of the priests, along with Sherebiah, Hashabiah, and ten of their fellow Levites.”

See: Textual Variants

* **Associated Passages:** Ezra 8:24
* **Associated Articles:** Textual Variants (ID: `33405@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Sherebiah (ID: `person:Sherebiah`)

## Ezra 8:24 (#3) (id: 463433)

וָ⁠אַבְדִּ֛ילָ⁠ה

**"Then I separated"**

This expression means to set someone or something apart for a special purpose or assignment. Alternate translation: “Then I selected”

See: Idiom

* **Associated Passages:** Ezra 8:24
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Sherebiah (ID: `person:Sherebiah`)

## Ezra 8:24 (#4) (id: 463434)

**"to Sherebiah, Hashabiah"**

These are the names of two men. See how you translated them in [8:18](https://ref.ly/Ezra8:18) and [8:19](https://ref.ly/Ezra8:19).

See: How to Translate Names

* **Associated Passages:** Ezra 8:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Sherebiah (ID: `person:Sherebiah`)

## Ezra 8:24 (#5) (id: 463435)

מֵ⁠אֲחֵי⁠הֶ֖ם

**"from their brothers"**

Here, **brothers** is a figurative way of saying “fellow Levites,” although it is possible that some of the biological brothers of Sherebiah and Hashabiah were included in this group. Alternate translation: “of their fellow Levites”

See: Metaphor

* **Associated Passages:** Ezra 8:24
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hashabiah (ID: `person:Hashabiah.7`); Sherebiah (ID: `person:Sherebiah`)

## Ezra 8:25 (#1) (id: 463436)

ו⁠אשקול⁠ה לָ⁠הֶ֔ם אֶת־הַ⁠כֶּ֥סֶף וְ⁠אֶת־הַ⁠זָּהָ֖ב וְ⁠אֶת־הַ⁠כֵּלִ֑ים

**"And I weighed out to them the silver and the gold and the vessels"**

Ezra says he **weighed out** the money to the priests and Levites because ancient units of money such as the kikkar and the daric were actually units of the weight of precious metals. The value of the gold and silver objects would similarly have depended on their weight. Alternate translation: “I divided up the silver and gold and objects into 24 portions of equal value and gave one to each of the priests and Levites whom I had selected to transport them.”

See: Biblical Money

* **Associated Passages:** Ezra 8:25
* **Associated Articles:** Biblical Money (ID: `33372@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:25 (#2) (id: 463437)

ו⁠אשקול⁠ה לָ⁠הֶ֔ם אֶת־הַ⁠כֶּ֥סֶף וְ⁠אֶת־הַ⁠זָּהָ֖ב וְ⁠אֶת־הַ⁠כֵּלִ֑ים

**"And I weighed out to them the silver and the gold and the vessels"**

The implication is that Ezra divided up the money and the objects into 24 portions of roughly equal value and assigned one portion to each priest and Levite to transport. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “I divided up the silver and gold and objects into 24 portions of equal value and gave one to each of the priests and Levites I had selected to transport them”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:25
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:25 (#3) (id: 463438)

תְּרוּמַ֣ת בֵּית־אֱלֹהֵ֗י⁠נוּ

**"the offering of the house of our God"**

Alternate translation: “the gifts for the temple of our God”

See: Metaphor

* **Associated Passages:** Ezra 8:25
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:25 (#4) (id: 463439)

אֱלֹהֵ֗י⁠נוּ

**"our God"**

In this case, Ezra is using **our** in the inclusive sense, that is, it includes the reader, since he is reporting his trip to fellow believers in God. Alternate translation: “from the God to whom we all belong”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:25
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:25 (#5) (id: 463440)

וְ⁠כָל־יִשְׂרָאֵ֖ל הַ⁠נִּמְצָאִֽים

**"and all Israel (the ones who were found"**

The implication is that this refers to all of the Israelites who were living specifically in the province of Babylon, since Artaxerxes had authorized Ezra to invite contributions from people living throughout that province. Alternate translation: “and all the Israelites whom we could find in the province of Babylon” or “and all the Israelites who were living in the province of Babylon”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:25
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:25 (#6) (id: 463441)

וְ⁠כָל־יִשְׂרָאֵ֖ל הַ⁠נִּמְצָאִֽים

**"and all Israel (the ones who were found"**

In this context, **were found** is an idiom that means “could be found” or “were there.” Alternate translation: “and all the Israelites whom we could find in the province of Babylon” or “and all the Israelites who were living in the province of Babylon”

See: Idiom

* **Associated Passages:** Ezra 8:25
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:25 (#7) (id: 463442)

וְ⁠כָל־יִשְׂרָאֵ֖ל

**"and all Israel"**

Here the story refers to all of the Israelites as if they were a single person, their ancestor, Israel. Alternate translation: “and all the Israelites”

See: Personification

* **Associated Passages:** Ezra 8:25
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:26 (#1) (id: 463443)

וָ⁠אֶשְׁקֲלָ֨⁠ה עַל־יָדָ֜⁠ם

**"And I weighed out to their hand"**

While Ezra likely did literally place the money and the objects in the hands of the priests and Levites, **hand** here represents control and action. Alternate translation: “I entrusted to them for safe delivery”

See: Metaphor

* **Associated Passages:** Ezra 8:26
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Ezra 8:26 (#2) (id: 463444)

וָ⁠אֶשְׁקֲלָ֨⁠ה עַל־יָדָ֜⁠ם

**"And I weighed out to their hand"**

The implication is that the list that follows is the total amount of money and objects that Ezra entrusted to the priests and Levites. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “Here are the totals of the silver, gold, and objects that I entrusted to these priests and Levites for safe delivery:”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:26
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Ezra 8:26 (#3) (id: 463445)

כֶּ֗סֶף כִּכָּרִים֙ שֵֽׁשׁ־מֵא֣וֹת וַ⁠חֲמִשִּׁ֔ים וּ⁠כְלֵי־כֶ֥סֶף מֵאָ֖ה לְ⁠כִכָּרִ֑ים זָהָ֖ב מֵאָ֥ה כִכָּֽר

**"650 kikkars of silver, and vessels of silver of 100 kikkars, 100 kikkars of gold"**

Since prices of precious metals vary over time, if you tried to express the value here in modern money, that could cause your Bible translation to become outdated and inaccurate. Instead you could express the amount using the ancient measurement, the kikkar, and explain in a note that a kikkar was equivalent to about 30 kilograms of weight. Alternatively, you could specify the weight of the silver in the text, as UST does. Alternate translation: “650 kikkars of silver, silver objects weighing 100 kikkars, and 100 kikkars of gold”

See: Biblical Money

* **Associated Passages:** Ezra 8:26
* **Associated Articles:** Biblical Money (ID: `33372@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Ezra 8:27 (#1) (id: 463446)

וּ⁠כְפֹרֵ֤י זָהָב֙ עֶשְׂרִ֔ים לַ⁠אֲדַרְכֹנִ֖ים אָ֑לֶף

**"and 20 bowls of gold of 1000 darics"**

1,000 darics was the total value of the 20 bowls, not the value of each one. Here again you could express the amount using the ancient measurement, the daric, and explain in a note that it was equivalent to about 8 or 8\.5 grams, or about a quarter of an ounce. Alternatively, you could specify the weight of the gold in the text, as UST does. Alternate translation: “20 golden bowls worth a total of 1,000 darics”

See: Biblical Money

* **Associated Passages:** Ezra 8:27
* **Associated Articles:** Biblical Money (ID: `33372@UWTranslationManual`)

* **Associated ACAI Concepts:** Daric (ID: `realia:Daric`)

## Ezra 8:28 (#1) (id: 463447)

וָ⁠אֹמְרָ֣⁠ה

**"And I said"**

This phrase indicates that the event the story will now relate came after the event it has just described. Ezra first weighed out the silver and gold and objects to the Levites, then he spoke the following words to them. In your translation, you can use the expression in your language that would best indicate this time sequence.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 8:28
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`); Money (ID: `realia:Money`)

## Ezra 8:28 (#2) (id: 463448)

וָ⁠אֹמְרָ֣⁠ה אֲלֵ⁠הֶ֗ם

**"And I said to them"**

**Them** refers to the selected priests and Levites. In the rest of this verse and in [8:29](https://ref.ly/Ezra8:29), Ezra is quoting what he told them on this occasion. It may be helpful to your readers to indicate this by setting off the words that follow with quotation marks or with whatever other punctuation or convention your language uses to indicate a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 8:28
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`); Money (ID: `realia:Money`)

## Ezra 8:28 (#3) (id: 463449)

אַתֶּ֥ם קֹ֨דֶשׁ֙ לַ⁠יהוָ֔ה וְ⁠הַ⁠כֵּלִ֖ים קֹ֑דֶשׁ

**"You are holy to Yahweh, and the vessels are holy"**

Here, **holy** means “set apart for a special purpose.” Alternate translation: “I have selected you for a special mission. People also donated these objects for a special purpose. They are only to be used in the temple.”

See: Idiom

* **Associated Passages:** Ezra 8:28
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`); Money (ID: `realia:Money`)

## Ezra 8:28 (#4) (id: 463450)

אֱלֹהֵ֥י אֲבֹתֵי⁠כֶֽם

**"the God of your fathers"**

Here, **fathers** means “ancestors.” Alternate translation: “the God of your ancestors” or “the God whom your ancestors worshiped”

See: Metaphor

* **Associated Passages:** Ezra 8:28
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`); Money (ID: `realia:Money`)

## Ezra 8:29 (#1) (id: 463451)

שִׁקְד֣וּ וְ⁠שִׁמְר֗וּ

**"Watch and keep"**

Here Ezra leaves out some of the words that a sentence would ordinarily need in order to be complete. He is describing what the priests and Levites must do with the objects, the silver, and the gold that were devoted to Yahweh. You could indicate that explicitly if it would be helpful to your readers. Alternate translation: “Take good care of those things”

See: Ellipsis

* **Associated Passages:** Ezra 8:29
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:29 (#2) (id: 463452)

שִׁקְד֣וּ וְ⁠שִׁמְר֗וּ

**"Watch and keep"**

These two words work together to give one idea. Ezra uses them together to emphasize how carefully the priests and Levites need to guard the silver and gold and objects. Alternate translation: “Guard diligently”

See: Hendiadys

* **Associated Passages:** Ezra 8:29
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:29 (#3) (id: 463453)

עַֽד־תִּשְׁקְל֡וּ

**"until you weigh them out"**

The implication is that when the priests and Levites arrived in Jerusalem, they would weigh out the silver, gold, and bronze to show the authorities there that they had not lost or taken any of it. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “until you could weigh them out to prove that all of it is there”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:29
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:29 (#4) (id: 463454)

לִ⁠פְנֵי֩

**"before the face of"**

Here, **face** represents the presence of a person. Alternate translation: “in the presence of”

See: Metonymy

* **Associated Passages:** Ezra 8:29
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:29 (#5) (id: 463455)

וְ⁠שָׂרֵֽי־הָ⁠אָב֥וֹת לְ⁠יִשְׂרָאֵ֖ל

**"and the leaders of the fathers of Israel"**

The phrase **the leaders of the fathers** seems to be another version of the expression “the heads of the fathers,” which is an abbreviated way of saying “the heads of father’s houses.” Review the note to [1:5](https://ref.ly/Ezra1:5) if that would be helpful. Alternate translation: “and the clan leaders of Israel”

See: Ellipsis

* **Associated Passages:** Ezra 8:29
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:29 (#6) (id: 463456)

הַ⁠לִּשְׁכ֖וֹת בֵּ֥ית יְהוָֽה

**"in the chambers of the house of Yahweh"**

The book assumes that readers will know that these **chambers** or rooms are specifically the storerooms in the temple, where money and objects for the temple worship were safely kept. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “in the storerooms of the temple of Yahweh”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:29
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:29 (#7) (id: 463457)

בֵּ֥ית יְהוָֽה

**"the house of Yahweh"**

This is the end of Ezra’s quotation of what he told the selected priests and Levites. If you decided in [8:28](https://ref.ly/Ezra8:28) to mark his words to them as a direct quotation, you should indicate the end of it here with a closing primary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 8:29
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:30 (#1) (id: 463458)

וְ⁠קִבְּלוּ֙ הַ⁠כֹּהֲנִ֣ים וְ⁠הַ⁠לְוִיִּ֔ם

**"So the priests and the Levites received"**

The word **So** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “In keeping with what Ezra told them, the priests and the Levites took”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:30
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:30 (#2) (id: 463459)

מִשְׁקַ֛ל הַ⁠כֶּ֥סֶף וְ⁠הַ⁠זָּהָ֖ב וְ⁠הַ⁠כֵּלִ֑ים

**"the weight of the silver, and the gold, and the vessels"**

Here Ezra is describing the money and the objects by reference to something associated with them, their weight, which was what determined their value. Alternate translation: “the silver and the gold and the objects”

See: Metonymy

* **Associated Passages:** Ezra 8:30
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:30 (#3) (id: 463460)

וְ⁠קִבְּלוּ֙ הַ⁠כֹּהֲנִ֣ים וְ⁠הַ⁠לְוִיִּ֔ם מִשְׁקַ֛ל הַ⁠כֶּ֥סֶף וְ⁠הַ⁠זָּהָ֖ב וְ⁠הַ⁠כֵּלִ֑ים

**"So the priests and the Levites received the weight of the silver, and the gold, and the vessels"**

The phrase **received the weight** means that these men, after observing the silver and gold and objects that Ezra weighed out to them ([8:26–27](https://ref.ly/Ezra8:26-Ezra8:27)), accepted responsibility for the full amount of it according to the charge that Ezra had just given them. Alternate translation: “So the priests and the Levites accepted full responsibility for all of the silver and the gold and the objects”

See: Idiom

* **Associated Passages:** Ezra 8:30
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:30 (#4) (id: 463461)

אֱלֹהֵֽי⁠נוּ

**"our God"**

In this case, Ezra is using **our** in the inclusive sense, that is, it includes the reader, since he is reporting his trip to fellow believers in God. Alternate translation: “the God to whom we all belong”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:30
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Levi (ID: `group:Levi`); Jerusalem (ID: `place:Jerusalem`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:31 (#1) (id: 463462)

וַֽ⁠נִּסְעָ֞⁠ה

**"Then we set out"**

As Ezra tells his story, he uses **we** and **us** in the exclusive sense, that is, it excludes the reader. Alternate translation: “Then we started traveling”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#2) (id: 463463)

מִ⁠נְּהַ֣ר אַֽהֲוָ֗א

**"from the river Ahava"**

See how you translated this name in [8:15](https://ref.ly/Ezra8:15). Alternate translation: “from the Ahava River”

See: How to Translate Names

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#3) (id: 463464)

בִּ⁠שְׁנֵ֤ים עָשָׂר֙ לַ⁠חֹ֣דֶשׁ הָ⁠רִאשׁ֔וֹן

**"on day 12 of the first month"**

The **first month** means the first month in the Jewish calendar. You could convert the Hebrew month into an equivalent on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the equivalency will be different every year and the translation will not be entirely accurate. So you may just want to use the number of the Hebrew month. Alternate translation: “on the twelfth day of the first month of that year”

See: Hebrew Months

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#4) (id: 463465)

בִּ⁠שְׁנֵ֤ים עָשָׂר֙ לַ⁠חֹ֣דֶשׁ הָ⁠רִאשׁ֔וֹן

**"on day 12 of the first month"**

The Hebrew uses a cardinal number here, **12**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, “twelfth,” in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of days, you can do that here in your translation. Alternate translation: “on the twelfth day of the first month”

See: Ordinal Numbers

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#5) (id: 463466)

וְ⁠יַד־אֱלֹהֵ֨י⁠נוּ֙ הָיְתָ֣ה עָלֵ֔י⁠נוּ

**"And the hand of our God was upon us"**

As in the expression **hand of our God upon us** in [8:18](https://ref.ly/Ezra8:18), **hand** here represents power and control. The expression indicates that God protected Ezra and the rest of the travelers on their journey. Alternate translation: “God protected us”

See: Metaphor

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#6) (id: 463467)

אֱלֹהֵ֨י⁠נוּ֙

**"our God"**

In this case, Ezra is using **our** in the inclusive sense, that is, it includes the reader, since he is reporting his trip to fellow believers in God. Alternate translation: “the God to whom we all belong”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#7) (id: 463468)

וַ⁠יַּ֨צִּילֵ֔⁠נוּ מִ⁠כַּ֥ף אוֹיֵ֛ב וְ⁠אוֹרֵ֖ב עַל־הַ⁠דָּֽרֶךְ

**"and he delivered us from the palm of the enemy and ambush on the way"**

These two phrases mean similar things. Ezra essentially says the same thing twice to emphasize how carefully God protected this group that was transporting so much treasure. If saying nearly the same thing twice might be confusing for your readers, you can combine the phrases into one. Alternate translation: “and God protected us from any robbers along the way” However, you could also choose to bring out the slight difference in meaning. The first phrase is more general, and the second phrase describes more specifically how someone might have robbed the group. Alternate translation: “and God protected us and prevented any bandits who were hiding along the roadways from robbing us”

See: Parallelism

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#8) (id: 463469)

מִ⁠כַּ֥ף אוֹיֵ֛ב

**"from the palm of the enemy"**

**Palm** here is another way of saying “hand,” which represents strength and power. Alternate translation: “from hostile attacks”

See: Metaphor

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:31 (#9) (id: 463470)

וְ⁠אוֹרֵ֖ב עַל־הַ⁠דָּֽרֶךְ

**"and ambush on the way"**

The abstract noun **ambush** refers to the way bandits will lie in wait along a roadside to rob travelers. If it would be helpful in your language, you could translate the idea behind this word with a noun phrase. Alternate translation: “and from bandits who lie in wait along the roadside”

See: Abstract Nouns

* **Associated Passages:** Ezra 8:31
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahava (ID: `place:Ahava`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`)

## Ezra 8:32 (#1) (id: 463471)

וַ⁠נָּב֖וֹא

**"So we came"**

The word **so** indicates that the sentence it introduces explains the results of what the previous sentence described. The group reached Jerusalem safely because God protected them. Alternate translation: “As a result, we came”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 8:32
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Ezra 8:32 (#2) (id: 463472)

וַ⁠נָּב֖וֹא \& וַ⁠נֵּ֥שֶׁב

**"So we came"** \- **"and we stayed"**

As Ezra tells his story, he uses **we** in the exclusive sense, that is, it excludes the reader.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:32
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Ezra 8:32 (#3) (id: 463473)

וַ⁠נָּב֖וֹא יְרוּשָׁלִָ֑ם וַ⁠נֵּ֥שֶׁב שָׁ֖ם יָמִ֥ים שְׁלֹשָֽׁה

**"So we came to Jerusalem, and we stayed there three days"**

Ezra does not say specifically why the group waited for three days before delivering the silver and gold and objects to the temple. But the implication is that this gave them a chance to rest after their journey and arrange for the delivery. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “We reached Jerusalem safely, and after we got there, we rested for three days”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:32
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jerusalem (ID: `place:Jerusalem`)

## Ezra 8:33 (#1) (id: 463474)

נִשְׁקַ֣ל הַ⁠כֶּסֶף֩ וְ⁠הַ⁠זָּהָ֨ב וְ⁠הַ⁠כֵּלִ֜ים

**"the silver and the gold and the vessels were weighed out"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “each of the priests and Levites weighed out the silver and gold and objects that they had transported”

See: Active or Passive

* **Associated Passages:** Ezra 8:33
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.5`); Uriah (ID: `person:Uriah.4`); Binnui (ID: `person:Binnui.2`); Meremoth (ID: `person:Meremoth`); Jozabad (ID: `person:Jozabad.6`); Phinehas (ID: `person:Phinehas.3`); Noadiah (ID: `person:Noadiah`); Eleazar (ID: `person:Eleazar.6`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:33 (#2) (id: 463475)

אֱלֹהֵ֗י⁠נוּ

**"our God"**

In this case, Ezra is using **our** in the inclusive sense, that is, it includes the reader, since he is reporting his trip to fellow believers in God. Alternate translation: “the God to whom we all belong”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 8:33
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.5`); Uriah (ID: `person:Uriah.4`); Binnui (ID: `person:Binnui.2`); Meremoth (ID: `person:Meremoth`); Jozabad (ID: `person:Jozabad.6`); Phinehas (ID: `person:Phinehas.3`); Noadiah (ID: `person:Noadiah`); Eleazar (ID: `person:Eleazar.6`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:33 (#3) (id: 463476)

עַ֠ל יַד

**"into the hand of"**

As in [8:26](https://ref.ly/Ezra8:26), while the priests and Levites likely did place the money and the objects in the hands of the temple leaders, **hand** here represents control and responsibility. Alternate translation: “into the custody of”

See: Metaphor

* **Associated Passages:** Ezra 8:33
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.5`); Uriah (ID: `person:Uriah.4`); Binnui (ID: `person:Binnui.2`); Meremoth (ID: `person:Meremoth`); Jozabad (ID: `person:Jozabad.6`); Phinehas (ID: `person:Phinehas.3`); Noadiah (ID: `person:Noadiah`); Eleazar (ID: `person:Eleazar.6`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:33 (#4) (id: 463477)

מְרֵמ֤וֹת בֶּן־אֽוּרִיָּה֙

**"Meremoth, the son of Uriah"**

**Meremoth** is the name of a man, and **Uriah** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 8:33
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.5`); Uriah (ID: `person:Uriah.4`); Binnui (ID: `person:Binnui.2`); Meremoth (ID: `person:Meremoth`); Jozabad (ID: `person:Jozabad.6`); Phinehas (ID: `person:Phinehas.3`); Noadiah (ID: `person:Noadiah`); Eleazar (ID: `person:Eleazar.6`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:33 (#5) (id: 463478)

אֶלְעָזָ֣ר בֶּן־פִּֽינְחָ֑ס

**"Eleazar, the son of Phinehas"**

**Eleazar** is the name of a man, and **Phinehas** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 8:33
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.5`); Uriah (ID: `person:Uriah.4`); Binnui (ID: `person:Binnui.2`); Meremoth (ID: `person:Meremoth`); Jozabad (ID: `person:Jozabad.6`); Phinehas (ID: `person:Phinehas.3`); Noadiah (ID: `person:Noadiah`); Eleazar (ID: `person:Eleazar.6`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:33 (#6) (id: 463479)

יוֹזָבָ֧ד בֶּן־יֵשׁ֛וּעַ

**"Jozabad, the son of Jeshua"**

**Jozabad** is the name of a man, and **Jeshua** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 8:33
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.5`); Uriah (ID: `person:Uriah.4`); Binnui (ID: `person:Binnui.2`); Meremoth (ID: `person:Meremoth`); Jozabad (ID: `person:Jozabad.6`); Phinehas (ID: `person:Phinehas.3`); Noadiah (ID: `person:Noadiah`); Eleazar (ID: `person:Eleazar.6`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:33 (#7) (id: 463480)

וְ⁠נֽוֹעַדְיָ֥ה בֶן־בִּנּ֖וּי

**"and Noadiah, the son of Binnui"**

**Noadiah** is the name of a man, and **Binnui** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 8:33
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeshua (ID: `person:Jeshua.5`); Uriah (ID: `person:Uriah.4`); Binnui (ID: `person:Binnui.2`); Meremoth (ID: `person:Meremoth`); Jozabad (ID: `person:Jozabad.6`); Phinehas (ID: `person:Phinehas.3`); Noadiah (ID: `person:Noadiah`); Eleazar (ID: `person:Eleazar.6`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`); LORD (ID: `deity:Lord`); House (ID: `realia:House`); Money (ID: `realia:Money`)

## Ezra 8:34 (#1) (id: 463481)

בְּ⁠מִסְפָּ֥ר בְּ⁠מִשְׁקָ֖ל לַ⁠כֹּ֑ל

**"by number and weight of everything"**

This means that the objects and the bars or coins of silver and gold were both counted and weighed. The implication is that this was a double confirmation of the honesty of the priests and Levites who had transported these donations. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “everything was both counted and weighed as proof that the men who had carried it had not lost or stolen anything”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:34
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 8:34 (#2) (id: 463482)

וַ⁠יִּכָּתֵ֥ב כָּֽל־הַ⁠מִּשְׁקָ֖ל

**"And all the weight was written"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “The priests and Levites in the temple wrote down the weight of all of the silver and gold and of the objects”

See: Active or Passive

* **Associated Passages:** Ezra 8:34
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Ezra 8:35 (#1) (id: 463483)

הַ֠⁠בָּאִים מֵֽ⁠הַ⁠שְּׁבִ֨י בְנֵֽי־הַ⁠גּוֹלָ֜ה

**"The ones who had come from the captivity, the sons of the exile"**

These two phrases mean the same thing. If saying the same thing twice might be confusing for your readers, you can combine the phrases into one. Alternate translation: “The Jews who had returned from exile”

See: Parallelism

* **Associated Passages:** Ezra 8:35
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); To Sin (ID: `keyterm:Sin`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`); Cattle (ID: `fauna:Bull`)

## Ezra 8:35 (#2) (id: 463484)

הַ֠⁠בָּאִים מֵֽ⁠הַ⁠שְּׁבִ֨י בְנֵֽי־הַ⁠גּוֹלָ֜ה

**"The ones who had come from the captivity, the sons of the exile"**

As indicated in [2:1](https://ref.ly/Ezra2:1) and [8:35](https://ref.ly/Ezra8:35), the phrase **the sons of the exile** refers specifically to the group of Jewish people who returned to the land of Judah from Babylon after King Nebuchadnezzar had conquered Jerusalem and had taken many Jews as captives to Babylon. Alternate translation: “The Jews who had returned from exile” or “The Jews who had returned to their homeland”

See: Idiom

* **Associated Passages:** Ezra 8:35
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); To Sin (ID: `keyterm:Sin`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`); Cattle (ID: `fauna:Bull`)

## Ezra 8:35 (#3) (id: 463485)

הַ֠⁠בָּאִים מֵֽ⁠הַ⁠שְּׁבִ֨י בְנֵֽי־הַ⁠גּוֹלָ֜ה

**"The ones who had come from the captivity, the sons of the exile"**

Here the story shifts from Ezra's first\-person account to a third\-person account. If that is confusing in your language, you can continue the story in first person. Alternate translation: “We who had gone into exile in Babylonia but had now returned from captivity”

See: First, Second or Third Person

* **Associated Passages:** Ezra 8:35
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); To Sin (ID: `keyterm:Sin`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`); Cattle (ID: `fauna:Bull`)

## Ezra 8:35 (#4) (id: 463486)

הִקְרִ֥יבוּ עֹל֣וֹת ׀ לֵ⁠אלֹהֵ֣י יִשְׂרָאֵ֗ל פָּרִ֨ים שְׁנֵים־עָשָׂ֤ר עַל־כָּל־יִשְׂרָאֵל֙ אֵילִ֣ים ׀ תִּשְׁעִ֣ים וְ⁠שִׁשָּׁ֗ה כְּבָשִׂים֙ שִׁבְעִ֣ים וְ⁠שִׁבְעָ֔ה צְפִירֵ֥י חַטָּ֖את שְׁנֵ֣ים עָשָׂ֑ר הַ⁠כֹּ֖ל עוֹלָ֥ה לַ⁠יהוָֽה

**"offered burnt up offerings to the God of Israel: 12 bulls for all Israel, 96 rams, 77 lambs, 12 male goats of a sin offering. The whole was a burnt up offering to Yahweh"**

The book assumes that readers will recognize that there are two types of offerings here. The bulls, rams, and lambs are offerings of the first type. They were offered in order to express a desire to be in good standing with God by creating an aroma, the smell of roasting meat, that was considered to be pleasing to God. These were completely burned up. The goats are the second type of offering. As in [6:17](https://ref.ly/Ezra6:17), the goats were a **sin offering**. These were offered as a request for forgiveness from God. As explained in Leviticus 4:22–26 and 6:24–30, only the fat of these goats was burned up. The meat of these goats was for the priests to eat. The phrase **The whole was a burnt up offering** is a summary that includes the two types of offerings, one that was completely burned up and the other that was partially burned up. Therefore the final sentence means, “All of this was properly subjected to fire on the altar as sacrifices to Yahweh.” The book assumes that readers know this. If it would be helpful in your language, you could say this explicitly. Alternate translation: “offered as burnt offerings to the God of Israel 12 bulls for all Israel, 96 rams, and 77 lambs, and as a sin offering 12 male goats. All of these sacrifices were offered to Yahweh through fire on the altar”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:35
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); To Sin (ID: `keyterm:Sin`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`); Cattle (ID: `fauna:Bull`)

## Ezra 8:35 (#5) (id: 463487)

הִקְרִ֥יבוּ עֹל֣וֹת ׀ לֵ⁠אלֹהֵ֣י יִשְׂרָאֵ֗ל פָּרִ֨ים שְׁנֵים־עָשָׂ֤ר עַל־כָּל־יִשְׂרָאֵל֙ אֵילִ֣ים ׀ תִּשְׁעִ֣ים וְ⁠שִׁשָּׁ֗ה כְּבָשִׂים֙ שִׁבְעִ֣ים וְ⁠שִׁבְעָ֔ה צְפִירֵ֥י חַטָּ֖את שְׁנֵ֣ים עָשָׂ֑ר

**"offered burnt up offerings to the God of Israel: 12 bulls for all Israel, 96 rams, 77 lambs, 12 male goats of a sin offering"**

The numbers of animals offered are symbolic, as the book explains in the case of the bulls. There were 12 bulls **for all Israel** because there were twelve tribes of Israel. There were 12 goats for the same reason. The same symbolism seems to lie behind the 96 rams, since that number is eight times 12, although the significance of the number eight is no longer apparent. The number 77 is an intensive form of the number seven, which symbolizes completeness. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “offered 12 bulls, one for each of the tribes of Israel, 96 rams, eight for each of the tribes of Israel, and 77 lambs, expressing completeness, as burnt offerings to the God of Israel, and 12 male goats as a sin offering, one for each of the tribes of Israel”

See: Symbolic Action

* **Associated Passages:** Ezra 8:35
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); To Sin (ID: `keyterm:Sin`); Offering (ID: `keyterm:Offering`); LORD (ID: `deity:Lord`); Goat (ID: `fauna:Goat`); Sheep (ID: `fauna:Lamb`); Cattle (ID: `fauna:Bull`)

## Ezra 8:36 (#1) (id: 463488)

וַֽ⁠יִּתְּנ֣וּ ׀ אֶת־דָּתֵ֣י הַ⁠מֶּ֗לֶךְ לַ⁠אֲחַשְׁדַּרְפְּנֵי֙ הַ⁠מֶּ֔לֶךְ וּ⁠פַחֲו֖וֹת עֵ֣בֶר הַ⁠נָּהָ֑ר

**"And they gave the laws of the king to the satraps of the king and the governors of Beyond\-the\-River"**

Here, **the laws of the king** seem to mean the decrees that Artaxerxes issued in the letter he gave to Ezra, and specifically the provisions that allowed Ezra to ask for support for the temple, exempted temple personnel from taxation, and allowed Ezra to appoint judges. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “Ezra and the Jewish leaders informed the royal officials and governors in Beyond\-the\-River province of the decrees that Artaxerxes had issued in his letter to Ezra”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 8:36
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:36 (#2) (id: 463489)

וַֽ⁠יִּתְּנ֣וּ

**"And they gave"**

Here the story uses a third\-person account rather than Ezra's first\-person account. If that is confusing in your language, you can continue the story in first person. Alternate translation: “We also gave”

See: First, Second or Third Person

* **Associated Passages:** Ezra 8:36
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:36 (#3) (id: 463490)

לַ⁠אֲחַשְׁדַּרְפְּנֵי֙ הַ⁠מֶּ֔לֶךְ וּ⁠פַחֲו֖וֹת

**"to the satraps of the king and the governors of"**

The word **satraps** refers to rulers appointed by the king to rule over his provinces, such as the province of Beyond\-the\-River in which Judah was located. The **governors** were lower officials who worked in the province under the direction of the satrap. Use the words that communicate this best in your language. Alternate translation: “to those who rule over the king's provinces and to the authorities in the province of”

See: Translate Unknowns

* **Associated Passages:** Ezra 8:36
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 8:36 (#4) (id: 463491)

וְ⁠נִשְּׂא֥וּ אֶת־הָ⁠עָ֖ם וְ⁠אֶת־בֵּֽית־הָ⁠אֱלֹהִֽים

**"And they lifted the people and the house of God"**

Here, **lifted** is a metaphor that means supported and sustained. Alternate translation: “The group that had returned with Ezra supported the people of Judah and Jerusalem and the temple of God.”

See: Metaphor

* **Associated Passages:** Ezra 8:36
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Euphrates River (ID: `place:Euphrates`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:1 (#1) (id: 463492)

וּ⁠כְ⁠כַלּ֣וֹת אֵ֗לֶּה

**"Now as soon as these things were finished"**

This phrase indicates that the events the story will now relate came right after the events it has just described. In your translation, you can use the expression in your language that would best indicate this relationship.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:1 (#2) (id: 463493)

וּ⁠כְ⁠כַלּ֣וֹת

**"Now as soon as"** \- **"were finished"**

The word **now** introduces a new event in the story. If your language has a word or expression for introducing a new event, use that here.

See: Introduction of a New Event

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:1 (#3) (id: 463494)

הַ⁠שָּׂרִים֙

**"the leaders"**

These appear to be the same **leaders** as in [8:29](https://ref.ly/Ezra8:29), that is, clan leaders. But this is not a reference to all of them, since the ones who come to Ezra say that some of their fellow leaders have married foreign women. Alternate translation: “some of the clan leaders”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:1 (#4) (id: 463495)

נִגְּשׁ֨וּ אֵלַ֤⁠י \& לֵ⁠אמֹ֔ר

**"came to me, saying"**

After this phrase Ezra begins to quote what these clan leaders told him. It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:1 (#5) (id: 463496)

לֹֽא־נִבְדְּל֞וּ \& מֵ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"have not separated themselves from the peoples of the lands"**

Separation here is a spatial metaphor for behavior. As in [6:21](https://ref.ly/Ezra6:21), it means to refuse to do the same things as another group. Alternate translation: “have not refused to do the things that the other people groups living in the land do”

See: Metaphor

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:1 (#6) (id: 463497)

מֵ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת כְּ֠⁠תוֹעֲבֹֽתֵי⁠הֶם

**"from the peoples of the lands according to their abominations"**

While the similar phrase in [6:21](https://ref.ly/Ezra6:21) spoke of “uncleanness,” that is, things that God does not accept, the phrase here speaks of **abominations**, that is, things that are very displeasing to God. Alternate translation: “from the things that are very displeasing to God that the other people groups living in the land do”

See: Idiom

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:1 (#7) (id: 463498)

מֵ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"from the peoples of the lands"**

See how you translated this expression in [3:3](https://ref.ly/Ezra3:3). It refers to members of other people groups who were living in and around the province of Judah. Alternate translation: “from the other people groups living in the land”

See: Idiom

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:1 (#8) (id: 463499)

לַ⁠כְּנַעֲנִ֨י הַ⁠חִתִּ֜י הַ⁠פְּרִזִּ֣י הַ⁠יְבוּסִ֗י הָֽ⁠עַמֹּנִי֙ הַ⁠מֹּ֣אָבִ֔י הַ⁠מִּצְרִ֖י וְ⁠הָ⁠אֱמֹרִֽי

**"of the Canaanite, the Hittite, the Perizzite, the Jebusite, the Ammonite, the Moabite, the Egyptian, and the Amorite"**

These are the names of eight people groups.

See: How to Translate Names

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hittites (ID: `group:Hittite`); Perizzites (ID: `group:Perizzites`); Jebusites (ID: `group:Jebusites`); Tribe of Levi (ID: `group:Levi`); Heth (ID: `person:Heth`); Israel (ID: `person:Israel`); Canaan (ID: `place:Canaan`); Levi (ID: `person:Levi.3`); Canaanites (ID: `group:Canaan`); Egyptians (ID: `group:Egypt`); Amorites (ID: `group:Amorites`); Ammon (ID: `place:Ammon`); Moab (ID: `place:Moab`); Egypt (ID: `place:Egypt`); Ammonites (ID: `group:Ammon`)

## Ezra 9:2 (#1) (id: 463500)

כִּֽי

**"For"**

Here, **For** indicates that the sentence it introduces explains the reasons for the statement that the leaders make in the previous sentence. Alternate translation: “You see,”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#2) (id: 463501)

נָשְׂא֣וּ מִ⁠בְּנֹֽתֵי⁠הֶ֗ם לָ⁠הֶם֙ וְ⁠לִ⁠בְנֵי⁠הֶ֔ם

**"they have lifted from their daughters for themselves and for their sons"**

The phrase **lifted from their daughters** is an idiom that means “married some of their women.” Alternate translation: “Israelite men have married women from these other groups, and they have also gotten wives for their sons from those groups.”

See: Idiom

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#3) (id: 463502)

וְ⁠הִתְעָֽרְבוּ֙ זֶ֣רַע הַ⁠קֹּ֔דֶשׁ בְּ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"so they have mixed the seed of holiness with the peoples of the lands"**

Here, **mixed** is a metaphor for blending one thing into another. Alternate translation: “so they are blending the Jewish people right in with the other people groups living in the land”

See: Metaphor

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#4) (id: 463503)

וְ⁠הִתְעָֽרְבוּ֙ זֶ֣רַע הַ⁠קֹּ֔דֶשׁ בְּ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"so they have mixed the seed of holiness with the peoples of the lands"**

The implication is that as a result of these marriages, not only was a distinct Jewish lineage being lost, the Jewish people were also beginning to follow the practices of these other groups rather than the commandments of God. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “so they are blending the Jewish people right in with the other people groups living in the land, and they are starting to follow their practices rather than the law of God”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#5) (id: 463504)

זֶ֣רַע הַ⁠קֹּ֔דֶשׁ

**"the seed of holiness"**

As in [8:28](https://ref.ly/Ezra8:28), **holiness** means “that which is set apart for a special purpose.” Here, the word refers specifically to the fact that God had set apart the Israelites to be a model community of God’s followers. Alternate translation: “the Jewish people, who belong to God,”

See: Idiom

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#6) (id: 463505)

זֶ֣רַע הַ⁠קֹּ֔דֶשׁ

**"the seed of holiness"**

Here, **seed** means “descendants.” The reference here is specifically to the descendants of Jacob, who was also known as Israel, and specifically to his descendants who were living in Judah and Jerusalem, that is, the Jews who had returned from exile. Alternate translation: “the Jewish people, who belong to God,”

See: Idiom

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#7) (id: 463506)

בְּ⁠עַמֵּ֖י הָ⁠אֲרָצ֑וֹת

**"with the peoples of the lands"**

As in [9:1](https://ref.ly/Ezra9:1), this expression refers to members of other people groups who were living in and around the province of Judah. Alternate translation: “with the other people groups living in this land”

See: Idiom

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#8) (id: 463507)

וְ⁠יַ֧ד הַ⁠שָּׂרִ֣ים וְ⁠הַ⁠סְּגָנִ֗ים הָֽיְתָ֛ה בַּ⁠מַּ֥עַל הַ⁠זֶּ֖ה רִאשׁוֹנָֽה

**"And the hand of the leaders and the rulers has been first in this unfaithfulness"**

Here, **hand** represents control and action. Alternate translation: “And our clan leaders and provincial officials were actually the first ones to disobey in this way”

See: Metaphor

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#9) (id: 463508)

בַּ⁠מַּ֥עַל הַ⁠זֶּ֖ה

**"in this unfaithfulness"**

Here, **unfaithfulness** means disobedience to God. Alternate translation: “to disobey God in this way”

See: Metaphor

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#10) (id: 463509)

הַ⁠שָּׂרִ֣ים וְ⁠הַ⁠סְּגָנִ֗ים

**"the leaders and the rulers"**

These terms mean similar things, but it seems that **leaders** may refer to the heads of the clans and that **rulers** may refer to official leaders of government. If your language uses one word for both of those, you can combine them and use that word here. Alternate translation: “our chiefs”

See: Doublet

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:2 (#11) (id: 463510)

בַּ⁠מַּ֥עַל הַ⁠זֶּ֖ה

**"in this unfaithfulness"**

This is the end of Ezra’s quotation of what the clan leaders told him. If you decided in [9:1](https://ref.ly/Ezra9:1) to mark their words as a direct quotation, you should indicate that here with a closing primary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 9:2
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 9:3 (#1) (id: 463511)

קָרַ֥עְתִּי אֶת־בִּגְדִ֖⁠י וּ⁠מְעִילִ֑⁠י וָ⁠אֶמְרְטָ֞⁠ה מִ⁠שְּׂעַ֤ר רֹאשִׁ⁠י֙ וּ⁠זְקָנִ֔⁠י וָ⁠אֵשְׁבָ֖⁠ה מְשׁוֹמֵֽם

**"I tore my garment and my robe. And I pulled out some from the hair of my head and my beard, and I sat down appalled"**

By performing these actions, Ezra was demonstrating symbolically how serious a wrong the Israelites had committed. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “to show how wrong it was for the Israelites to have married foreign women, I tore my tunic and my cloak, I pulled out some hair from my head and my beard, and I sat down stunned”

See: Symbolic Action

* **Associated Passages:** Ezra 9:3
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Robe (ID: `realia:Robe`)

## Ezra 9:3 (#2) (id: 463512)

מְשׁוֹמֵֽם

**"appalled"**

The sense of this word is that Ezra was so overwhelmed with shock and horror that he did not move or speak. In your translation, you can use the expression in your language that would best indicate this. Alternate translation: “stunned” or “devastated”

* **Associated Passages:** Ezra 9:3

* **Associated ACAI Concepts:** Robe (ID: `realia:Robe`)

## Ezra 9:4 (#1) (id: 463513)

וְ⁠אֵלַ֣⁠י יֵאָסְפ֗וּ כֹּ֤ל חָרֵד֙ בְּ⁠דִבְרֵ֣י אֱלֹהֵֽי־יִשְׂרָאֵ֔ל עַ֖ל מַ֣עַל הַ⁠גּוֹלָ֑ה

**"And all who trembled at the words of the God of Israel on account of the unfaithfulness of the exiles were gathered to me"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “Everyone came and joined me who was just as distressed as I was about the way the Jews who had returned from exile had disobeyed the commandments of the God of Israel”

See: Active or Passive

* **Associated Passages:** Ezra 9:4
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 9:4 (#2) (id: 463514)

כֹּ֤ל חָרֵד֙

**"all who trembled"**

Often in the Old Testament, **trembled** is a metaphor meaning to regard someone or something with respect and reverential fear. That is likely included in the meaning here, but in this context, the expression also seems to indicate an actual physical response to the situation, just as Ezra did not move or speak. Alternate translation: “everyone who was distressed”

See: Metaphor

* **Associated Passages:** Ezra 9:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 9:4 (#3) (id: 463515)

בְּ⁠דִבְרֵ֣י אֱלֹהֵֽי־יִשְׂרָאֵ֔ל

**"at the words of the God of Israel"**

This phrase does not refer to something that God said on this occasion. Rather, it refers to the commandment that God had given to the Israelites earlier not to intermarry with foreign groups, which Ezra cites in his prayer in [9:10](https://ref.ly/Ezra9:10) and [9:11](https://ref.ly/Ezra9:11). Alternate translation: “at the commandments of the God of Israel”

See: Metaphor

* **Associated Passages:** Ezra 9:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 9:4 (#4) (id: 463516)

עַ֖ל מַ֣עַל הַ⁠גּוֹלָ֑ה

**"on account of the unfaithfulness of the exiles"**

The abstract noun **exiles** refers in this context to the Jews who had returned to their homeland from Babylon. Here, **the exiles** seems to be equivalent to the longer phrase **the sons of the exile** in several other places in the book. If it would be helpful in your language, you could translate the idea behind this term with an equivalent phrase. Alternate translation: “because of the way the Jews who had returned to their homeland had disobeyed”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 9:4 (#5) (id: 463517)

עַ֖ל מַ֣עַל הַ⁠גּוֹלָ֑ה

**"on account of the unfaithfulness of the exiles"**

As in [9:2](https://ref.ly/Ezra9:2), **unfaithfulness** means “disobedience.” Alternate translation: “because of the disobedience of the Jews who had returned to their homeland”

See: Metaphor

* **Associated Passages:** Ezra 9:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 9:4 (#6) (id: 463518)

וַ⁠אֲנִי֙ יֹשֵׁ֣ב מְשׁוֹמֵ֔ם עַ֖ד לְ⁠מִנְחַ֥ת הָ⁠עָֽרֶב

**"And I was sitting appalled until the offering of the evening"**

The **offering of the evening** was a sacrifice that the priests would offer around the time that the sun was going down. Alternate translation: “I continued to sit without moving or speaking until the time of the evening sacrifice” or “I continued to sit without moving or speaking for the rest of that day”

See: Translate Unknowns

* **Associated Passages:** Ezra 9:4
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`)

## Ezra 9:5 (#1) (id: 463519)

קַ֚מְתִּי מִ⁠תַּֽעֲנִיתִ֔⁠י וּ⁠בְ⁠קָרְעִ֥⁠י בִגְדִ֖⁠י וּ⁠מְעִילִ֑⁠י

**"I arose from my humilation, and when I tore my garment and my robe"**

Even though Ezra had been sitting down and he now got up, in this context the word **arise** means more than that. As in [1:5](https://ref.ly/Ezra1:5), [3:2](https://ref.ly/Ezra3:2), and [5:2](https://ref.ly/Ezra5:2), it means to take action to get an enterprise under way. Ezra had been sitting motionless to show how ashamed and upset he was. Now he began to take action to address the situation that the clan leaders had told him about. Alternate translation: “I stopped sitting motionless in my torn clothes and began to do something about the situation”

See: Idiom

* **Associated Passages:** Ezra 9:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`); Robe (ID: `realia:Robe`)

## Ezra 9:5 (#2) (id: 463520)

וּ⁠בְ⁠קָרְעִ֥⁠י בִגְדִ֖⁠י וּ⁠מְעִילִ֑⁠י

**"and when I tore my garment and my robe"**

As the note to [9:3](https://ref.ly/Ezra9:3) explains, Ezra had torn his clothes to show how wrong it was for the Israelites to have married foreign women. By not changing out of his torn clothes, Ezra was continuing to show that symbolically. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “but I kept wearing my torn clothes to show how wrong it was for the Israelites to have married foreign women”

See: Symbolic Action

* **Associated Passages:** Ezra 9:5
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`); Robe (ID: `realia:Robe`)

## Ezra 9:5 (#3) (id: 463521)

וָֽ⁠אֶכְרְעָ⁠ה֙ עַל־בִּרְכַּ֔⁠י וָ⁠אֶפְרְשָׂ֥⁠ה כַפַּ֖⁠י אֶל־יְהוָ֥ה אֱלֹהָֽ⁠י

**"And I knelt down on my knees, and spread out my palms to Yahweh my God"**

These two physical gestures showed publicly that Ezra was going to pray. Alternate translation: “I knelt down and spread out my hands in a gesture of prayer to Yahweh my God.”

See: Symbolic Action

* **Associated Passages:** Ezra 9:5
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Offering (ID: `keyterm:Offering`); Robe (ID: `realia:Robe`)

## Ezra 9:6 (#1) (id: 463522)

וָ⁠אֹמְרָ֗⁠ה

**"And I said"**

After this phrase Ezra begins to quote what he said out loud in prayer on this occasion. The quotation continues through [9:15](https://ref.ly/Ezra9:15). It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#2) (id: 463523)

אֱלֹהַ⁠י֙ \& אֱלֹהַ֛⁠י

**"My God,"** \- **"my God"**

Here, **My God** means that Ezra belongs to God, not that God belongs to Ezra. If that is not clear in your language, then use a different phrase. Alternate translation: “O God, to whom I belong” or “God, whom I worship”

See: Possession

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#3) (id: 463524)

בֹּ֣שְׁתִּי וְ⁠נִכְלַ֔מְתִּי

**"I am ashamed and humiliated"**

**Ashamed** and **humiliated** mean very similar things. Ezra uses them together to emphasize how disgraced he feels by what the Israelites have done. If it would be helpful in your language, you could combine these terms into a single expression. Alternate translation: “I am deeply disgraced”

See: Doublet

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#4) (id: 463525)

לְ⁠הָרִ֧ים \& פָּנַ֖⁠י אֵלֶ֑י⁠ךָ

**"to raise my face to you"**

This means “to look at you directly” or “to look at you face to face,” and since Ezra could not do that literally with God, it is a figurative way of saying “\*to address you.” Alternate translation: “even to speak to you.”

See: Metaphor

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#5) (id: 463526)

כִּ֣י עֲוֺנֹתֵ֤י⁠נוּ רָבוּ֙ לְ⁠מַ֣עְלָ⁠ה רֹּ֔אשׁ וְ⁠אַשְׁמָתֵ֥⁠נוּ גָדְלָ֖ה עַ֥ד לַ⁠שָּׁמָֽיִם

**"For our iniquities have multiplied to above the head, and our guilt has grown up as far as to the heavens"**

These two phrases mean similar things. Ezra uses the repetition to emphasize how guilty the people of Israel are. If repeating the same thing twice would be confusing for your readers, you could combine the phrases into one and show the emphasis in another way. Alternate translation: “We have committed such great sins that our guilt because of them is completely overwhelming us” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The first phrase speaks of the effects of these sins on the Israelites, while the second phrase speaks of their effects on their standing with God. Alternate translation: “We have committed such great sins that they are overwhelming us, and we know that they are making us guilty before you”

See: Parallelism

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#6) (id: 463527)

כִּ֣י עֲוֺנֹתֵ֤י⁠נוּ רָבוּ֙ לְ⁠מַ֣עְלָ⁠ה רֹּ֔אשׁ וְ⁠אַשְׁמָתֵ֥⁠נוּ גָדְלָ֖ה עַ֥ד לַ⁠שָּׁמָֽיִם

**"For our iniquities have multiplied to above the head, and our guilt has grown up as far as to the heavens"**

If it would be helpful in your language, you could put this sentence first in the prayer (after **My God**), since it gives the reason for the result of Ezra feeling ashamed. You could show the connection by using a word like “so” after this phrase. Alternate translation: “we have committed such great sins that we are completely guilty, and we know that we are very guilty before you. And so”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#7) (id: 463528)

עֲוֺנֹתֵ֤י⁠נוּ רָבוּ֙ לְ⁠מַ֣עְלָ⁠ה רֹּ֔אשׁ

**"our iniquities have multiplied to above the head"**

Here, **multiplied** means to become much greater in number. Ezra uses that expression to describe how serious the sins of the Israelites are. Alternate translation: “we have committed so many sins that it is like they have piled up higher than our heads”

See: Metaphor

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#8) (id: 463529)

עֲוֺנֹתֵ֤י⁠נוּ רָבוּ֙

**"our iniquities have multiplied"**

If it would be helpful in your language, you could express the idea behind the abstract noun **iniquities** with a phrase such as “sins that we have committed.” Alternate translation: “the great sins that we have committed”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#9) (id: 463530)

לְ⁠מַ֣עְלָ⁠ה רֹּ֔אשׁ

**"to above the head"**

Ezra uses a spatial metaphor to describe the degree to which the sins have become greater in number. Alternate translation: “to the point that they are obvious to everyone”

See: Metaphor

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#10) (id: 463531)

וְ⁠אַשְׁמָתֵ֥⁠נוּ גָדְלָ֖ה

**"and our guilt has grown up"**

If it would be helpful in your language, you could express the idea behind the abstract noun **guilt** with a phrase such as “make guilty.” Alternate translation: “and our sins are making us guilty”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:6 (#11) (id: 463532)

וְ⁠אַשְׁמָתֵ֥⁠נוּ גָדְלָ֖ה עַ֥ד לַ⁠שָּׁמָֽיִם

**"and our guilt has grown up as far as to the heavens"**

Here, Ezra speaks of God by reference to something associated with him, **the heavens**, which was considered to be the place where God lived. The expression **and our guilt has grown up as far as to the heavens** means that the sins have become so numerous that the pile of them has reached to God in heaven. In other words, their sin is against God himself. Alternate translation: “and our many sins have made us very guilty before you”

See: Metonymy

* **Associated Passages:** Ezra 9:6
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:7 (#1) (id: 463533)

מִ⁠ימֵ֣י אֲבֹתֵ֗י⁠נוּ

**"From the days of our fathers"**

The term **days** is used here to refer a particular period of time. Alternate translation: “From the time when our ancestors lived”

See: Idiom

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#2) (id: 463534)

אֲבֹתֵ֗י⁠נוּ

**"our fathers"**

**Fathers** here, means “ancestors.” Alternate translation: “our ancestors”

See: Metaphor

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#3) (id: 463535)

עַ֖ד הַ⁠יּ֣וֹם הַ⁠זֶּ֑ה

**"until this day"**

In this context, **day** does not refer to one specific day, but rather, to a more general time. Alternate translation: “until this time”

See: Idiom

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#4) (id: 463536)

אֲנַ֨חְנוּ֙ בְּ⁠אַשְׁמָ֣ה גְדֹלָ֔ה

**"we are in great guilt"**

Here, Ezra speaks of the disobedience of the Israelites by reference to something associated with it, the guilt that it has caused. Alternate translation: “we have consistently disobeyed you”

See: Metonymy

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#5) (id: 463537)

אֲנַ֨חְנוּ֙

**"we"**

By **we** here, Ezra is referring to all of the Jewish people. See the UST.

See: Metonymy

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#6) (id: 463538)

בְּ⁠אַשְׁמָ֣ה גְדֹלָ֔ה

**"are in great guilt"**

If it would be helpful in your language, you could express the idea behind the abstract noun **guilt** with a verb such as “disobey.” Alternate translation: “consistently disobeyed you”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#7) (id: 463539)

נִתַּ֡נּוּ אֲנַחְנוּ֩ מְלָכֵ֨י⁠נוּ כֹהֲנֵ֜י⁠נוּ בְּ⁠יַ֣ד ׀ מַלְכֵ֣י הָ⁠אֲרָצ֗וֹת

**"we ourselves, our kings, and our priests have been given into the hand of the kings of the lands"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “you put us, our kings, and our priests into the hands of foreign rulers”

See: Active or Passive

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#8) (id: 463540)

נִתַּ֡נּוּ אֲנַחְנוּ֩ מְלָכֵ֨י⁠נוּ כֹהֲנֵ֜י⁠נוּ בְּ⁠יַ֣ד ׀ מַלְכֵ֣י הָ⁠אֲרָצ֗וֹת

**"we ourselves, our kings, and our priests have been given into the hand of the kings of the lands"**

Here, **hand** represents power and control. Alternate translation: “you allowed foreign rulers to conquer us, our kings, and our priests”

See: Metaphor

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#9) (id: 463541)

מַלְכֵ֣י הָ⁠אֲרָצ֗וֹת

**"the kings of the lands"**

The **kings of the lands** would be the rulers of the “people of the lands” whom Ezra mentions in [9:1](https://ref.ly/Ezra9:1) and [9:2](https://ref.ly/Ezra9:2), and of similar foreign people groups. Alternate translation: “foreign rulers”

See: Idiom

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#10) (id: 463542)

בַּ⁠חֶ֜רֶב בַּ⁠שְּׁבִ֧י וּ⁠בַ⁠בִּזָּ֛ה וּ⁠בְ⁠בֹ֥שֶׁת פָּנִ֖ים

**"by sword, by captivity, and by plunder, and by shame of face"**

Other than **by sword**, these are not the means by which the foreign rulers conquered the Israelites. Rather, they are primarily things that happened to the Israelites after they were conquered. It may be helpful to make this a separate sentence. Alternate translation: “They killed some of us, they took others of us away from our homeland, they stole the things that belonged to us, and all of this has made us greatly ashamed”

See: Idiom

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#11) (id: 463543)

בַּ⁠חֶ֜רֶב

**"by sword"**

The sword represents killing, by reference to one kind of weapon that can kill a person. Alternate translation: “who killed some of us”

See: Metonymy

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#12) (id: 463544)

בַּ⁠שְּׁבִ֧י

**"by captivity"**

If it would be helpful in your language, you could express the idea behind the abstract noun **captivity** with a verb such as “take away.” Alternate translation: “who took others away from our homeland”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#13) (id: 463545)

וּ⁠בַ⁠בִּזָּ֛ה

**"and by plunder"**

If it would be helpful in your language, you could express the idea behind the abstract noun **plunder** with a verb such as “steal.” Alternate translation: “who stole the things that belonged to us”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#14) (id: 463546)

וּ⁠בְ⁠בֹ֥שֶׁת פָּנִ֖ים

**"and by shame of face"**

If it would be helpful in your language, you could express the idea behind the abstract noun **shame** with a word such as “ashamed.” Alternate translation: “and all of this has made us greatly ashamed”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#15) (id: 463547)

וּ⁠בְ⁠בֹ֥שֶׁת פָּנִ֖ים

**"and by shame of face"**

Here, Ezra uses the **face** to describe the whole person figuratively, by reference to one part of a person, their face, which would show their shame by its expression. Alternate translation: “and made us greatly ashamed”

See: Synecdoche

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:7 (#16) (id: 463548)

כְּ⁠הַ⁠יּ֥וֹם הַ⁠זֶּֽה

**"as this day"**

In this context, **day** does not refer to one specific day, but rather to a more general time that includes the day that Ezra is praying to God. Alternate translation: “and that is still our situation now”

See: Idiom

* **Associated Passages:** Ezra 9:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`)

## Ezra 9:8 (#1) (id: 463549)

וְ⁠עַתָּ֡ה

**"But now"**

The phrase **But now** indicates that the sentence that it introduces draws a contrast between what the Israelites had been experiencing in the past and what God was now doing. Use a natural way in your language to indicate this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#2) (id: 463550)

כִּ⁠מְעַט־רֶגַע֩

**"according to a little moment"**

Alternate translation: “for just a short time”

See: Idiom

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#3) (id: 463551)

הָיְתָ֨ה תְחִנָּ֜ה מֵ⁠אֵ֣ת ׀ יְהוָ֣ה אֱלֹהֵ֗י⁠נוּ

**"favor has come from with Yahweh our God"**

The abstract noun **favor** refers to one person helping another person even if that person does not deserve it. If it would be helpful in your language, you could translate the idea behind this word with a phrase such as “be merciful.” Alternate translation: “Yahweh our God has been merciful to us” or “Yahweh our God has been gracious to us”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#4) (id: 463552)

הָיְתָ֨ה תְחִנָּ֜ה מֵ⁠אֵ֣ת ׀ יְהוָ֣ה אֱלֹהֵ֗י⁠נוּ

**"favor has come from with Yahweh our God"**

Ezra speaks here of **favor** or mercy as if it could travel from Yahweh to the Jews. Alternate translation: “Yahweh our God has been merciful to us”

See: Personification

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#5) (id: 463553)

הָיְתָ֨ה תְחִנָּ֜ה מֵ⁠אֵ֣ת ׀ יְהוָ֣ה אֱלֹהֵ֗י⁠נוּ

**"favor has come from with Yahweh our God"**

Ezra is speaking to Yahweh in prayer, and accordingly, he addresses him in the second person in [9:6](https://ref.ly/Ezra9:6) and in [9:10–15](https://ref.ly/Ezra9:10-Ezra9:15). But here he speaks of God in the third person to show humility and respect, as people of this time did when speaking to superiors. If it would be helpful in your language, you could translate his words in the second person. Alternate translation: “you, Yahweh our God, have been merciful to us”

See: First, Second or Third Person

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#6) (id: 463554)

וְ⁠לָ⁠תֶת־לָ֥⁠נוּ יָתֵ֖ד

**"and to give to us a peg"**

Ezra speaks of the Jews’ current situation of safety and security in their homeland as if it were a **peg** or “stake,” such as one would pound securely into the ground in order to fasten the cords of a tent to it, or such as one would fasten to a wall in order to hang utensils on it safely and securely. If it would be helpful in your language, you could express this with a different metaphor that conveys this sense of stability and security, or express this plainly. Alternate translation: “and to bring us into a safe situation”

See: Metaphor

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#7) (id: 463555)

בִּ⁠מְק֣וֹם קָדְשׁ֑⁠וֹ

**"in his holy place"**

Here the word **holy** means “set apart for a special purpose.” The temple was God’s **holy place** because it was set apart for his worship. Alternate translation: “in his temple”

See: Idiom

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#8) (id: 463556)

בִּ⁠מְק֣וֹם קָדְשׁ֑⁠וֹ

**"in his holy place"**

Here again Ezra speaks of God in the third person. If it would be helpful in your language, you could translate his words in the second person. Alternate translation: “in your temple.”

See: First, Second or Third Person

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#9) (id: 463557)

בִּ⁠מְק֣וֹם קָדְשׁ֑⁠וֹ

**"in his holy place"**

Ezra speaks of the **peg** as if it were in God’s temple, likely because God was considered to be present there. Alternate translation: “in your presence”

* **Associated Passages:** Ezra 9:8

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#10) (id: 463558)

בִּ⁠מְק֣וֹם קָדְשׁ֑⁠וֹ

**"in his holy place"**

Ezra may be speaking of the presence of God by association to mean the protection of God. Alternate translation: “under your protection”

See: Metonymy

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#11) (id: 463559)

לְ⁠הַשְׁאִ֥יר לָ֨⁠נוּ֙ פְּלֵיטָ֔ה

**"to leave for us survivors"**

This phrase starts with **to**, which indicates here that the phrase that follows is a result of the phrase that came before. In fact, there are four results in this verse of receiving favor from God. Each result begins with the word **to**. Use a natural way in your language to indicate that these are results of receiving favor from God.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#12) (id: 463560)

לְ⁠הָאִ֤יר \& אֱלֹהֵ֔י⁠נוּ

**"for our God to brighten"**

Here again Ezra speaks of God in the third person. If it would be helpful in your language, you could translate his words in the second person. Alternate translation: “for you, our God, to bring light to”

See: First, Second or Third Person

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#13) (id: 463561)

לְ⁠הָאִ֤יר עֵינֵ֨י⁠נוּ֙ אֱלֹהֵ֔י⁠נוּ וּ⁠לְ⁠תִתֵּ֛⁠נוּ מִֽחְיָ֥ה מְעַ֖ט בְּ⁠עַבְדֻתֵֽ⁠נוּ

**"for our God to brighten our eyes and to give us a little reviving in our slavery"**

These two phrases mean similar things. Ezra uses the repetition to emphasize how merciful God has been to the community of returned exiles. If it would be helpful in your language, you could combine these phrases. Alternate translation: “and for our God to enable our community to thrive a little even under foreign domination” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase says more specifically how God has accomplished what the first phrase describes. Alternate translation: “for our God to give us new vitality by giving us greater freedom even though we are under foreign domination”

See: Parallelism

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#14) (id: 463562)

לְ⁠הָאִ֤יר עֵינֵ֨י⁠נוּ֙ אֱלֹהֵ֔י⁠נוּ

**"for our God to brighten our eyes"**

To **brighten** the **eyes** of a person means to give them new life and vitality in the sense of restoring someone's happiness or well\-being. Alternate translation: “for our God to give us new vitality”

See: Metaphor

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:8 (#15) (id: 463563)

וּ⁠לְ⁠תִתֵּ֛⁠נוּ מִֽחְיָ֥ה מְעַ֖ט בְּ⁠עַבְדֻתֵֽ⁠נוּ

**"and to give us a little reviving in our slavery"**

Since Ezra says that God has made the community more alive **in our slavery**, he seems to be referring implicitly to the greater measure of freedom that King Artaxerxes has allowed them. If it would be helpful to your readers, you could say this explicitly. Alternate translation: “by giving us greater freedom even though we are under foreign domination”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 9:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Plea for Mercy (ID: `keyterm:Supplication`); LORD (ID: `deity:Lord`); Peg (ID: `realia:Peg`); Peg (ID: `realia:Peg.2`)

## Ezra 9:9 (#1) (id: 463564)

כִּֽי־עֲבָדִ֣ים אֲנַ֔חְנוּ

**"For we are slaves"**

Ezra describes the Jewish people as **slaves**, which is an exaggeration that he uses to express humility before God. It is true that their ancestors were taken as captives to Babylon where they were forced to serve the king, and that even now they must do as the king commands, but those who have returned to Judah are allowed to manage their own daily affairs. Alternate translation: “The Babylonians made our people to be slaves”

See: Hyperbole

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#2) (id: 463565)

לֹ֥א עֲזָבָ֖⁠נוּ אֱלֹהֵ֑י⁠נוּ

**"our God has not forsaken us"**

Ezra continues to speak of God in the third person. If it would be helpful in your language, you could translate his words in the second person. Alternate translation: “you, our God, have not forsaken us”

See: First, Second or Third Person

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#3) (id: 463566)

וַֽ⁠יַּט

**"But he has extended"**

This word **But** indicates that the sentence it introduces draws a contrast between what the Israelites might have expected God to do, since they were being punished for their disobedience, and what God actually did. You could begin the sentence with a phrase such as “on the contrary” or other method in your language to indicate this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#4) (id: 463567)

וַֽ⁠יַּט־עָלֵ֣י⁠נוּ חֶ֡סֶד

**"But he has extended to us covenant faithfulness"**

As in [7:28](https://ref.ly/Ezra7:28), Ezra uses a spatial metaphor to depict help stretching out to him from Yahweh. Alternate translation: “On the contrary, he has helped us, as he promised”

See: Metaphor

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#5) (id: 463568)

וַֽ⁠יַּט־עָלֵ֣י⁠נוּ חֶ֡סֶד לִ⁠פְנֵי֩ מַלְכֵ֨י פָרַ֜ס

**"But he has extended to us covenant faithfulness before the face of the kings of Persia"**

Here, the abstract noun **faithfulness** refers to a person wanting to do everything that they can to help another person. As in [3:11](https://ref.ly/Ezra3:11) and [7:28](https://ref.ly/Ezra7:28), **covenant faithfulness** refers specifically to Yahweh doing that for the people of Israel because of his covenant with them, which contained the promises that he had made to them. If it would be helpful in your language, you could translate the idea behind this phrase with verbs such as “help” and “promise.” Alternate translation: “But he has helped us, as he promised, by leading the king of Persia to regard us favorably”

See: Abstract Nouns

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#6) (id: 463569)

לִ⁠פְנֵי֩ מַלְכֵ֨י פָרַ֜ס

**"before the face of the kings of Persia"**

Here, **face** refers to the way that a person regards things, by association with the way that they use the eyes in their face to see things and the way that their facial expression shows what they think of those things. Alternate translation: “by leading the kings of Persia to regard us favorably”

See: Metonymy

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#7) (id: 463570)

מַלְכֵ֨י פָרַ֜ס

**"the kings of Persia"**

Ezra probably uses the plural **kings** to acknowledge implicitly that not just Artaxerxes, but also Cyrus and Darius before him, treated the Jews favorably by allowing them to rebuild the temple and by supporting the worship there. If it would be helpful to your readers, you could name these kings explicitly. Alternate translation: “King Cyrus, King Darius, and King Artaxerxes of Persia”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#8) (id: 463571)

לָֽ⁠תֶת־לָ֣⁠נוּ מִֽחְיָ֗ה

**"to give to us reviving"**

As in [9:8](https://ref.ly/Ezra9:8), **reviving** or renewed life in this context seems to be a figurative expression for greater freedom. Alternate translation: “so that they gave us greater freedom”

See: Metaphor

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#9) (id: 463572)

לְ⁠רוֹמֵ֞ם אֶת־בֵּ֤ית אֱלֹהֵ֨י⁠נוּ֙ וּ⁠לְ⁠הַעֲמִ֣יד אֶת־חָרְבֹתָ֔י⁠ו

**"to raise up the house of our God and to cause its ruins to stand"**

These two phrases mean similar things. Ezra says essentially the same thing twice, probably to emphasize how remarkable this was after all the opposition that there had been to it. If repeating both phrases in your translation might be confusing for your readers, you can combine the phrases. Alternate translation: “and allowed us to rebuild the temple of our God” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase is stronger because it indicates that the temple had previously been destroyed but now it was being restored. Alternate translation: “and allowed us to rebuild the temple of our God by repairing its ruins”

See: Parallelism

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#10) (id: 463573)

לְ⁠רוֹמֵ֞ם

**"to raise up"**

**To raise up** is a figurative expression for building it. Alternate translation: “to rebuild”

See: Metaphor

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#11) (id: 463574)

וּ⁠לְ⁠הַעֲמִ֣יד אֶת־חָרְבֹתָ֔י⁠ו

**"and to cause its ruins to stand"**

Like **raise up**, **cause … to stand** is a figurative expression for building. Alternate translation: “by repairing its ruins”

See: Metaphor

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:9 (#12) (id: 463575)

וְ⁠לָֽ⁠תֶת־לָ֣⁠נוּ גָדֵ֔ר בִּֽ⁠יהוּדָ֖ה וּ⁠בִ⁠ירוּשָׁלִָֽם

**"and to give to us a wall in Judah and in Jerusalem"**

Ezra is not referring to a literal wall. (The walls of Jerusalem were still broken down at this time. They would not be repaired until Nehemiah came to the city over a dozen years later.) Rather, he means that the Persian kings authorized the returned exiles to re\-establish their community in Judah and Jerusalem, and so the king would protect their right to live there. It is as if the Jews are already protected by this as though by a wall. Alternate translation: “and has given us a safe place to live in Judah and Jerusalem.”

See: Metaphor

* **Associated Passages:** Ezra 9:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Judea (ID: `place:Judea`); Persia (ID: `place:Persia`); Kindness (ID: `keyterm:Kindness`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 9:10 (#1) (id: 463576)

וְ⁠עַתָּ֛ה מַה־נֹּאמַ֥ר אֱלֹהֵ֖י⁠נוּ אַֽחֲרֵי־זֹ֑את

**"So now, our God, what can we say after this"**

Ezra is making a statement, not really asking a question. He does not expect God to tell him what excuses he can give for the disobedience of the Israelites. Instead, he is using the question form to emphasize that the Israelites really have no excuses. If it would be helpful in your language, you could translate these words as a statement. Alternate translation: “So now, our God, we really have no excuses, after all of this.”

See: Rhetorical Question

* **Associated Passages:** Ezra 9:10
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 9:10 (#2) (id: 463577)

אַֽחֲרֵי־זֹ֑את

**"after this"**

If it would be helpful in your language, you could explain what **this** refers to. There are two possibilities. (1\) It probably refers to all of the sinning that the Jews did by disobeying God. Alternate translation: “after we have continued to sin against you” or (2\) It could refer to the good things that God did for the Jews even though they continually disobeyed him. Alternate translation: “after you have been much more kind to us than we deserved”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 9:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 9:10 (#3) (id: 463578)

עָזַ֖בְנוּ מִצְוֺתֶֽי⁠ךָ

**"we have forsaken your commandments"**

As in [8:22](https://ref.ly/Ezra8:22), forsaking or abandoning God is a metaphor for disobeying him or refusing to serve him. Alternate translation: “we have disobeyed your commandments”

See: Metaphor

* **Associated Passages:** Ezra 9:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 9:11 (#1) (id: 463579)

אֲשֶׁ֣ר צִוִּ֗יתָ בְּ⁠יַ֨ד עֲבָדֶ֣י⁠ךָ הַ⁠נְּבִיאִים֮

**"which you commanded by the hand of your servants, the prophets"**

Here, **hand** represents control and action. This expression means that God gave these commandments by having the prophets speak them to the people of Israel. Alternate translation: “which you told your servants, the prophets, to speak to us”

See: Metaphor

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:11 (#2) (id: 463580)

לֵ⁠אמֹר֒

**"saying"**

After this phrase Ezra begins to quote what the prophets commanded the people on Yahweh’s behalf. So this is a quotation within a quotation. That is, Ezra is quoting the prayer that he prayed on this occasion, and within that prayer, he is quoting what the prophets said. It may be helpful to your readers to indicate this by setting off these words within secondary quotation marks or with whatever other punctuation or convention your language uses to indicate a quotation within a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:11 (#3) (id: 463581)

אֶ֤רֶץ נִדָּה֙ הִ֔יא בְּ⁠נִדַּ֖ת עַמֵּ֣י הָ⁠אֲרָצ֑וֹת

**"is a land of impurity by the impurity of the peoples of the lands"**

The prophets speak of the land of Canaan as if it were itself impure, because it was a place where the people did things that were very displeasing to God. Alternate translation: “is a land whose inhabitants do things that are displeasing to God”

See: Metaphor

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:11 (#4) (id: 463582)

בְּ⁠נִדַּ֖ת

**"by the impurity of"**

The prophets speak of these displeasing things as if they were contaminated. Alternate translation: “because of the detestable actions of”

See: Metaphor

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:11 (#5) (id: 463583)

עַמֵּ֣י הָ⁠אֲרָצ֑וֹת

**"the peoples of the lands"**

See how you translated this expression in [3:3](https://ref.ly/Ezra3:3). Here it refers to members of other people groups who were living in the land of Canaan. Alternate translation: “the inhabitants”

See: Idiom

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:11 (#6) (id: 463584)

בְּ⁠תוֹעֲבֹֽתֵי⁠הֶ֗ם אֲשֶׁ֥ר מִלְא֛וּ⁠הָ מִ⁠פֶּ֥ה אֶל־פֶּ֖ה בְּ⁠טֻמְאָתָֽ⁠ם

**"by their abominations that have filled it from mouth to mouth with their uncleanness"**

The term **abominations**, like the term **impurity**, refers to things that people do that are very displeasing to God. As in [9:1](https://ref.ly/Ezra9:1), the term indicates that God must judge these actions as wrong, that is, as not in accordance with his own character. The prophets speak of these things as if they were accumulating physically and filling up the land. It may be helpful to begin a new sentence here. Alternate translation: “Throughout the land, from one end to the other, they have been doing things that are very displeasing to God”

See: Metaphor

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:11 (#7) (id: 463585)

מִ⁠פֶּ֥ה אֶל־פֶּ֖ה

**"from mouth to mouth"**

Here, **mouth** is a figurative way of saying “edge,” “end,” or “extremity,” probably by analogy to the way that the mouth of an animal is typically at one end of the animal. Alternate translation: “from one end to the other”

See: Metaphor

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:11 (#8) (id: 463586)

בְּ⁠טֻמְאָתָֽ⁠ם

**"with their uncleanness"**

The prophets also speak of these displeasing things as if they were dirty. See how you translated **uncleanness** in verse [6:21](https://ref.ly/Ezra6:21). Alternate translation: “through the things they do that are displeasing to God”

See: Metaphor

* **Associated Passages:** Ezra 9:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Dispossess (ID: `keyterm:Dispossess`); Defilement (ID: `keyterm:Defilement`)

## Ezra 9:12 (#1) (id: 463587)

וְ֠⁠עַתָּה

**"So now"**

In this verse, Ezra continues quoting the prophets. The phrase **So now** introduces the results of what the previous sentence described. Use a connector in your language that can introduce a resulting action. Alternate translation: “Therefore” or “As a result”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#2) (id: 463588)

בְּֽנוֹתֵי⁠כֶ֞ם אַל־תִּתְּנ֣וּ לִ⁠בְנֵי⁠הֶ֗ם

**"neither give your daughters to their sons"**

This expression specifically refers to giving daughters in marriage. Alternate translation: “do not arrange marriages between your daughters and their sons”

See: Idiom

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#3) (id: 463589)

וּ⁠בְנֹֽתֵי⁠הֶם֙ אַל־תִּשְׂא֣וּ לִ⁠בְנֵי⁠כֶ֔ם

**"nor lift their daughters for your sons"**

As in [9:2](https://ref.ly/Ezra9:2), to **lift their daughters for your sons** is an idiom that means “to get wives for your sons from among their daughters.” Alternate translation: “and do not get wives for your sons from their daughters”

See: Idiom

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#4) (id: 463590)

וְ⁠לֹֽא־תִדְרְשׁ֧וּ שְׁלֹמָ֛⁠ם וְ⁠טוֹבָתָ֖⁠ם עַד־עוֹלָ֑ם

**"nor shall you seek their peace or their good until eternity"**

**Until eternity** is a Hebrew expression that means **for as long as anyone can anticipate into the future**. Alternate translation: “and never do anything that contributes to their welfare or prosperity”

See: Idiom

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#5) (id: 463591)

וְ⁠לֹֽא־תִדְרְשׁ֧וּ שְׁלֹמָ֛⁠ם וְ⁠טוֹבָתָ֖⁠ם

**"nor shall you seek their peace or their good"**

The terms **peace** and **good** mean similar things. The prophets use them together to emphasize the importance of this commandment. If it would be helpful in your language, you could combine these terms into a single expression, as in the UST.

See: Doublet

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#6) (id: 463592)

לְמַ֣עַן תֶּחֶזְק֗וּ וַ⁠אֲכַלְתֶּם֙ אֶת־ט֣וּב הָ⁠אָ֔רֶץ

**"so that you may be strong and eat the good things of the land"** \- **"it"**

These two phrases mean similar things. The prophets say essentially the same thing twice, to emphasize the blessings that the Israelites would enjoy if they obeyed these commandments. If repeating both phrases might be confusing for your readers, you could combine them. Alternate translation: “so that you will become a prosperous nation in that place” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase says more specifically how the nation would become strong, as the first phrase describes. Alternate translation: “so that you will become a strong nation by growing and enjoying abundant harvests”

See: Parallelism

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#7) (id: 463593)

לְמַ֣עַן תֶּחֶזְק֗וּ

**"so that you may be strong"**

The forms of **you** and **your** in this verse are plural, referring to all the Jewish people. Therefore **you may be strong** does not refer to a person's physical strength or health, but rather to the strength of the Jewish nation. Alternate translation: “so that your nation will be strong”

See: Forms of ‘You’ — Singular

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Forms of ‘You’ — Singular (ID: `33321@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#8) (id: 463594)

וְ⁠הוֹרַשְׁתֶּ֥ם לִ⁠בְנֵי⁠כֶ֖ם עַד־עוֹלָֽם

**"and cause your sons to inherit"** \- **"until eternity"**

As earlier in the sentence, **until eternity** means “for as long as anyone can anticipate into the future.” Alternate translation: “so that your descendants will always be able to live there”

See: Idiom

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#9) (id: 463595)

לִ⁠בְנֵי⁠כֶ֔ם

**"your sons"**

Here, **sons** means “descendants.” Alternate translation: “your descendants”

See: Metaphor

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:12 (#10) (id: 463596)

עַד־עוֹלָ֑ם

**"until eternity"**

After this phrase Ezra ends his quotation of what Yahweh commanded through the prophets. If you decided in [9:11](https://ref.ly/Ezra9:11) to mark their words as a secondary quotation, you should indicate that ending here with a closing secondary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation within a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 9:12
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cause to Possess (ID: `keyterm:Possess`); Peace (ID: `keyterm:IntactState`)

## Ezra 9:13 (#1) (id: 463597)

כָּל־הַ⁠בָּ֣א עָלֵ֔י⁠נוּ

**"all that has come upon us"**

The phrase **all that has come upon us** refers to the punishments that Ezra mentioned in [9:7](https://ref.ly/Ezra9:7). If it would be helpful in your language, you could make that information explicit. Alternate translation: “all of the things with which you punished us” or “everything that you allowed to happen to us”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 9:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:13 (#2) (id: 463598)

בְּ⁠מַעֲשֵׂ֨י⁠נוּ֙ הָ⁠רָעִ֔ים וּ⁠בְ⁠אַשְׁמָתֵ֖⁠נוּ הַ⁠גְּדֹלָ֑ה

**"because of our evil deeds and because of our great guilt"**

These two phrases mean similar things. Ezra uses the repetition for emphasis. If it would be helpful in your language, you could combine these phrases. Alternate translation: “since we are so guilty because of the wrong things that we have done” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. Alternate translation: “since we have committed such great sins, and since we are so guilty as a result”

See: Parallelism

* **Associated Passages:** Ezra 9:13
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:13 (#3) (id: 463599)

חָשַׂ֤כְתָּֽ לְ⁠מַ֨טָּה֙ מֵֽ⁠עֲוֺנֵ֔⁠נוּ

**"you have restrained it to lower than our iniquities"**

This is a Hebrew idiom. Alternate translation: “have not punished us as fully as our sins deserve”

See: Idiom

* **Associated Passages:** Ezra 9:13
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:13 (#4) (id: 463600)

וְ⁠נָתַ֥תָּה לָּ֛⁠נוּ פְּלֵיטָ֖ה כָּ⁠זֹֽאת

**"and you have given to us a survival such as this"**

Ezra is referring implicitly to himself and to the other Jews who have returned from exile. Alternate translation: “and you have allowed some of us to return to our homeland”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 9:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Passion (ID: `keyterm:Ardour`); LORD (ID: `deity:Lord`)

## Ezra 9:14 (#1) (id: 463601)

הֲ⁠נָשׁוּב֙ לְ⁠הָפֵ֣ר מִצְוֺתֶ֔י⁠ךָ וּ֨⁠לְ⁠הִתְחַתֵּ֔ן בְּ⁠עַמֵּ֥י הַ⁠תֹּעֵב֖וֹת הָ⁠אֵ֑לֶּה

**"should we return to breaking your commandments and to intermarrying with the peoples of these abominations"**

Ezra is making a statement, not really asking a question. He does not expect God to tell him whether the people should once again disobey. Instead, he is using the question form to emphasize how important it is for them to stop intermarrying with the other people groups. If it would be helpful in your language, you could translate these words as a statement or as an exclamation. Alternate translation: “we certainly should not break your commandments again by intermarrying with peoples who do things that you cannot stand!”

See: Rhetorical Question

* **Associated Passages:** Ezra 9:14
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)


## Ezra 9:14 (#2) (id: 463602)

בְּ⁠עַמֵּ֥י הַ⁠תֹּעֵב֖וֹת הָ⁠אֵ֑לֶּה

**"with the peoples of these abominations"**

As in [9:1](https://ref.ly/Ezra9:1) and [9:11](https://ref.ly/Ezra9:11), the term **abominations** refers to things that God cannot allow to have in his presence. Alternate translation: “with the people groups who do these things that God detests”

See: Idiom

* **Associated Passages:** Ezra 9:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 9:14 (#3) (id: 463603)

הֲ⁠ל֤וֹא תֶֽאֱנַף־בָּ֨⁠נוּ֙ עַד־כַּלֵּ֔ה לְ⁠אֵ֥ין שְׁאֵרִ֖ית וּ⁠פְלֵיטָֽה

**"Would you not be angry with us as far as completion, so that there is neither remnant nor survivors"**

Once again Ezra is making a statement, not really asking a question. He does not expect God to tell him whether he would be angry with the Jews. Instead, he is using the question form to emphasize how angry God would become. If it would be helpful in your language, you could translate these words as a statement or as an exclamation. Alternate translation: “If we did that, you would be totally angry with us, and you would not leave any of us alive.”

See: Rhetorical Question

* **Associated Passages:** Ezra 9:14
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)


## Ezra 9:14 (#4) (id: 463604)

הֲ⁠ל֤וֹא תֶֽאֱנַף־בָּ֨⁠נוּ֙ עַד־כַּלֵּ֔ה

**"Would you not be angry with us as far as completion"**

This is a Hebrew idiom. Alternate translation: “Would you not be totally angry with us”

See: Idiom

* **Associated Passages:** Ezra 9:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 9:14 (#5) (id: 463605)

לְ⁠אֵ֥ין שְׁאֵרִ֖ית וּ⁠פְלֵיטָֽה

**"so that there is neither remnant nor survivors"**

In this specific context, the terms **remnant** and **escape** mean the same thing. Ezra uses the terms together for emphasis. If it would be helpful in your language, you could combine these terms. Alternate translation: “you would not leave any of us alive.”

See: Doublet

* **Associated Passages:** Ezra 9:14
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)


## Ezra 9:15 (#1) (id: 463606)

יְהוָ֞ה אֱלֹהֵ֤י יִשְׂרָאֵל֙ צַדִּ֣יק אַ֔תָּה כִּֽי־נִשְׁאַ֥רְנוּ פְלֵיטָ֖ה כְּ⁠הַ⁠יּ֣וֹם הַ⁠זֶּ֑ה

**"Yahweh, the God of Israel, you are righteous, for we are left as survivors, as this day"**

If it would be helpful in your language, you could reverse the order of these phrases since the second phrase gives the reason for what the first phrase says. Alternate translation: “But you have ensured that a group of us would survive, just as we are right now, so we acknowledge that you, Yahweh, the God of Israel, are righteous”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 9:15
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 9:15 (#2) (id: 463607)

כְּ⁠הַ⁠יּ֣וֹם הַ⁠זֶּ֑ה

**"as this day"**

In this context, **day** does not refer to one specific day, but rather to a more general time. Alternate translation: “as we are now”

See: Idiom

* **Associated Passages:** Ezra 9:15
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 9:15 (#3) (id: 463608)

הִנְ⁠נ֤וּ לְ⁠פָנֶ֨י⁠ךָ֙ בְּ⁠אַשְׁמָתֵ֔י⁠נוּ כִּ֣י אֵ֥ין לַ⁠עֲמ֛וֹד לְ⁠פָנֶ֖י⁠ךָ עַל־זֹֽאת

**"Behold us, before your face in our guilt, for there is none to stand before your face on account of this"**

These two phrases mean similar things. Ezra uses the repetition for emphasis as he makes this confession of guilt on behalf of the people. If it would be helpful in your language, you could combine these phrases. Alternate translation: “We have come into your presence to admit that we are guilty.” However, there is a slight difference in meaning, and you could also choose to bring that out in your translation. The second phrase is more intensive. Alternate translation: “We have come into your presence to admit that we are guilty, and none of us have any excuse for what we have done”

See: Parallelism

* **Associated Passages:** Ezra 9:15
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 9:15 (#4) (id: 463609)

לְ⁠פָנֶ֨י⁠ךָ֙

**"before your face"**

Here, **face** represents the presence of a person. Alternate translation: “standing in your presence”

See: Metonymy

* **Associated Passages:** Ezra 9:15
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 9:15 (#5) (id: 463610)

אֵ֥ין לַ⁠עֲמ֛וֹד לְ⁠פָנֶ֖י⁠ךָ

**"there is none to stand before your face"**

To **stand** is a Hebrew idiom meaning to be acquitted. (For example, in Psalm 130:3, “If you, Yahweh, would mark iniquities, Lord, who could stand?” and Psalm 1:5, “The wicked will not stand in the judgment.”) Alternate translation: “none of us can plead any grounds for acquittal” or “none of us have any excuse for what we have done”

See: Idiom

* **Associated Passages:** Ezra 9:15
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 9:15 (#6) (id: 463611)

עַל־זֹֽאת

**"on account of this"**

This is the end of Ezra’s quotation of the prayer that he prayed on this occasion. If you decided in [9:6](https://ref.ly/Ezra9:6) to mark his words as a direct quotation, you should indicate that ending here with a closing primary quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 9:15
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Israel (ID: `person:Israel`); LORD (ID: `deity:Lord`)

## Ezra 10:1 (#1) (id: 463612)

וּ⁠כְ⁠הִתְפַּלֵּ֤ל עֶזְרָא֙

**"And when Ezra prayed"**

Previously in the book, Ezra was a character in a story that someone else was telling. Then starting from [7:27](https://ref.ly/Ezra7:27) he is the person telling the story, and he continues to tell it through to the end of chapter 9\. Now from here to the end of the book, the story changes back to someone else talking about Ezra as a character in the story. If it would be helpful to your readers, you could continue to translate this chapter as though Ezra were telling it. Alternate translation: “And when I prayed”

See: Introduction of New and Old Participants

* **Associated Passages:** Ezra 10:1
* **Associated Articles:** Introduction of New and Old Participants (ID: `33421@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:1 (#2) (id: 463613)

וּ⁠כְ⁠הִתְפַּלֵּ֤ל עֶזְרָא֙ וּ⁠כְ֨⁠הִתְוַדֹּת֔⁠וֹ

**"And when Ezra prayed, and when he confessed"**

These two phrases mean similar things. If it would be helpful to your readers, you could combine them. Alternate translation: “While Ezra was praying this prayer of confession”

See: Parallelism

* **Associated Passages:** Ezra 10:1
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:1 (#3) (id: 463614)

וּ⁠מִתְנַפֵּ֔ל לִ⁠פְנֵ֖י בֵּ֣ית הָ⁠אֱלֹהִ֑ים

**"and casting himself down before the face of the house of God"**

By **casting himself down** onto the ground in front of the temple, Ezra was symbolically pleading with God, who was considered to be present in the temple, the way a supplicant in this culture would get down with his face on the ground to plead with a powerful person. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “and pleading with God by falling down onto the ground in front of the temple”

See: Symbolic Action

* **Associated Passages:** Ezra 10:1
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:1 (#4) (id: 463615)

לִ⁠פְנֵ֖י בֵּ֣ית הָ⁠אֱלֹהִ֑ים

**"before the face of the house of God"**

Here, **face** means the front of a place. Alternate translation: “in front of the temple”

See: Metaphor

* **Associated Passages:** Ezra 10:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:1 (#5) (id: 463616)

בָכ֥וּ הָ⁠עָ֖ם הַרְבֵּה־בֶֽכֶה

**"the people wept an abundant weeping"**

This is an idiom that means that the people wept a lot. This could refer to either loudness or length, or both. Alternate translation: “the people cried very bitterly”

See: Idiom

* **Associated Passages:** Ezra 10:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:1 (#6) (id: 463617)

בָכ֥וּ הָ⁠עָ֖ם הַרְבֵּה־בֶֽכֶה

**"the people wept an abundant weeping"**

The implication is that the people were weeping because they realized what a wrong thing they had done and how guilty they were because of it. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “the people began to weep very bitterly because they realized that they had sinned and were guilty”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:2 (#1) (id: 463618)

וַ⁠יַּעַן֩

**"Then"** \- **"answered"**

The word **then** indicates that the events the story will now relate came after the events it has just described. In your translation, you can use the expression in your language that would best indicate this relationship.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#2) (id: 463619)

שְׁכַנְיָ֨ה בֶן־יְחִיאֵ֜ל

**"Shekaniah, the son of Jehiel"**

**Shekaniah** is the name of a man, and **Jehiel** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#3) (id: 463620)

מִ⁠בְּנֵ֤י עֵילָם֙

**"from the sons of Elam"**

**Sons** here means “descendants.” Alternate translation: “one of the descendants of Elam”

See: Metaphor

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#4) (id: 463621)

עֵילָם֙

**"Elam"**

**Elam** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#5) (id: 463622)

וַ⁠יַּעַן֩ \& וַ⁠יֹּ֣אמֶר

**"Then"** \- **"answered and said"**

Here the book expresses a single idea by using two words connected with **and**. Together the two words mean that Shekaniah responded to the situation that the Jews were facing. Alternate translation: “responded”

See: Hendiadys

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#6) (id: 463623)

וַ⁠יֹּ֣אמֶר לְ⁠עֶזְרָ֔א

**"and said to Ezra"**

After this phrase, the book begins to quote what Shekaniah said to Ezra. It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#7) (id: 463624)

**"We ourselves have acted unfaithfully against our God"**

As in [9:2](https://ref.ly/Ezra9:2) and [9:4](https://ref.ly/Ezra9:4), **acted unfaithfully** means “acted in disobedience.” Alternate translation: “We ourselves have disobeyed our God”

See: Metaphor

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#8) (id: 463625)

וַ⁠נֹּ֛שֶׁב נָשִׁ֥ים נָכְרִיּ֖וֹת מֵ⁠עַמֵּ֣י הָ⁠אָ֑רֶץ

**"and have caused foreign women from the peoples of the land to dwell"**

To **cause to dwell** is an idiom that means “to marry.” Alternate translation: “and have married foreign women from the other people groups living in the land”

See: Idiom

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#9) (id: 463626)

מֵ⁠עַמֵּ֣י הָ⁠אָ֑רֶץ

**"from the peoples of the land"**

This expression refers to the other people groups who were living in and around the province of Judah. Alternate translation: “the other people groups living in the land”

See: Idiom

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#10) (id: 463627)

וְ⁠עַתָּ֛ה

**"But now"**

The words **But now** indicate that the sentence it introduces draws a contrast between it and the previous sentence. The situation in the previous sentence appeared to be hopeless, but Shekaniah wants to say that the Jews can still do something about it. Indicate this contrast in a natural way in your language. Alternate translation: “nevertheless”

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#11) (id: 463628)

וְ⁠עַתָּ֛ה יֵשׁ־מִקְוֶ֥ה לְ⁠יִשְׂרָאֵ֖ל עַל־זֹֽאת

**"But now, there is hope for Israel concerning this"**

Here Shekaniah refers to the nation of Israel as if it were a person, their ancestor, Israel. Alternate translation: “but there is still hope for the Israelites in this matter”

See: Personification

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#12) (id: 463629)

וְ⁠עַתָּ֛ה יֵשׁ־מִקְוֶ֥ה לְ⁠יִשְׂרָאֵ֖ל עַל־זֹֽאת

**"But now, there is hope for Israel concerning this"**

Though Shekaniah speaks of Israel, meaning the Israelites, in the third person, the term includes himself. If it would be helpful in your language, you could translate this phrase in the first person. Alternate translation: “But there is still hope for us in this matter” or “But there is still something that we can do about this”

See: First, Second or Third Person

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:2 (#13) (id: 463630)

וְ⁠עַתָּ֛ה יֵשׁ־מִקְוֶ֥ה לְ⁠יִשְׂרָאֵ֖ל עַל־זֹֽאת

**"But now, there is hope for Israel concerning this"**

Since Shekaniah is speaking to Ezra and the term **Israel** applies to Ezra as well, if you choose to translate this phrase in the first person, the term “us” or “we” should include the addressee, if your language makes that distinction. Alternate translation: “But there is still hope for us in this matter” or “But there is still something that we can do about this”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** Ezra 10:2
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Shecaniah (ID: `person:Shecaniah.7`); Jehiel (ID: `person:Jehiel.8`); Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`); Elam (ID: `person:Elam`); LORD (ID: `deity:Lord`)

## Ezra 10:3 (#1) (id: 463631)

נִֽכְרָת־בְּרִ֣ית

**"let us cut a covenant"**

This is a Hebrew idiom that means “to make or enter into a covenant,” which is a solemn agreement between two parties. Alternate translation: “let us make a solemn agreement”

See: Idiom

* **Associated Passages:** Ezra 10:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Instruction (ID: `keyterm:Instruction`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Ezra 10:3 (#2) (id: 463632)

לְ⁠הוֹצִ֨יא כָל־נָשִׁ֜ים וְ⁠הַ⁠נּוֹלָ֤ד מֵ⁠הֶם֙

**"to cause all the women to go out, and the ones who were born from them"**

**To cause to go out** is a Hebrew idiom that means “to divorce and send away.” Alternate translation: “to divorce and send away all of these foreign wives and their children”

See: Idiom

* **Associated Passages:** Ezra 10:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Instruction (ID: `keyterm:Instruction`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Ezra 10:3 (#3) (id: 463633)

וְ⁠הַ⁠נּוֹלָ֤ד מֵ⁠הֶם֙

**"and the ones who were born from them"**

If it would be helpful in your language, you could use a noun to represent this phrase that uses a passive verb form. Alternate translation: “and their children”

See: Active or Passive

* **Associated Passages:** Ezra 10:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Instruction (ID: `keyterm:Instruction`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Ezra 10:3 (#4) (id: 463634)

בַּ⁠עֲצַ֣ת אֲדֹנָ֔⁠י

**"by the counsel of my lord"**

Shekaniah addresses Ezra in the third person as a sign of respect. If it would be helpful in your language, you could translate this with the second person. Alternate translation: “doing it in the way that you, my lord, advise”

See: First, Second or Third Person

* **Associated Passages:** Ezra 10:3
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Instruction (ID: `keyterm:Instruction`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Ezra 10:3 (#5) (id: 463635)

וְ⁠הַ⁠חֲרֵדִ֖ים בְּ⁠מִצְוַ֣ת אֱלֹהֵ֑י⁠נוּ

**"and the ones who tremble at the commandment of our God"**

As in [9:4](https://ref.ly/Ezra9:4), **tremble** here is a metaphor meaning to regard someone or something with respect and reverential fear. Alternate translation: “and in the way that seems right to everyone who respects the commandment of our God”

See: Metaphor

* **Associated Passages:** Ezra 10:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Instruction (ID: `keyterm:Instruction`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Ezra 10:3 (#6) (id: 463636)

וְ⁠כַ⁠תּוֹרָ֖ה יֵעָשֶֽׂה

**"And let it be done according to the law"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “And let us make sure that we are following what the law commands”

See: Active or Passive

* **Associated Passages:** Ezra 10:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Instruction (ID: `keyterm:Instruction`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Ezra 10:3 (#7) (id: 463637)

וְ⁠כַ⁠תּוֹרָ֖ה יֵעָשֶֽׂה

**"And let it be done according to the law"**

If it would be helpful in your language, you could put this more general statement before the specific one about cutting a covenant, as in the UST.

See: Information Structure

* **Associated Passages:** Ezra 10:3
* **Associated Articles:** Information Structure (ID: `33286@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Instruction (ID: `keyterm:Instruction`); Covenant (ID: `keyterm:Covenant`); LORD (ID: `deity:Lord`)

## Ezra 10:4 (#1) (id: 463638)

ק֛וּם

**"Arise"**

Here, as in several other places in the book, **arise** means to take action to get an enterprise under way. It could also mean to physically arise, since Ezra was lying prostrate on the ground. Alternate translation: “Get up and do something about this situation”

See: Idiom

* **Associated Passages:** Ezra 10:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 10:4 (#2) (id: 463639)

כִּֽי־עָלֶ֥י⁠ךָ הַ⁠דָּבָ֖ר

**"for the matter is on you"**

This is a Hebrew idiom. Alternate translation: “because this is your responsibility”

See: Idiom

* **Associated Passages:** Ezra 10:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 10:4 (#3) (id: 463640)

כִּֽי־עָלֶ֥י⁠ךָ הַ⁠דָּבָ֖ר

**"for the matter is on you"**

The implication is that this was Ezra’s responsibility because King Artaxerxes had given him the mandate to teach the Jews to follow the commandments of Yahweh and he also gave him the authority to ensure that they did follow those commandments. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “because this is your responsibility, since the king told you to teach us Yahweh’s commandments and to make sure that we follow them”

See: Idiom

* **Associated Passages:** Ezra 10:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 10:4 (#4) (id: 463641)

וַ⁠אֲנַ֣חְנוּ עִמָּ֑⁠ךְ

**"and we are with you"**

Alternate translation: “and we will support you”

* **Associated Passages:** Ezra 10:4


## Ezra 10:4 (#5) (id: 463642)

חֲזַ֖ק וַ⁠עֲשֵֽׂה

**"Be strong and do"**

Here, Shekaniah leaves out some of the words that a sentence would ordinarily need in order to be complete. Alternate translation: “Be strong and take action to address this matter”

See: Ellipsis

* **Associated Passages:** Ezra 10:4
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)


## Ezra 10:4 (#6) (id: 463643)

חֲזַ֖ק וַ⁠עֲשֵֽׂה

**"Be strong and do"**

After this phrase, the book ends its quotation of what Shekaniah said to Ezra on this occasion. If you decided in [10:2](https://ref.ly/Ezra10:2) to mark his words as a quotation, you should indicate their ending here with a closing quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 10:4
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)


## Ezra 10:5 (#1) (id: 463644)

וַ⁠יָּ֣קָם

**"And"** \- **"arose"**

Here the word **And** indicates that the sentence it introduces explains the results of what the previous sentences described. Alternate translation: “As a result”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 10:5
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Swear (ID: `keyterm:Swear`)

## Ezra 10:5 (#2) (id: 463645)

וַ⁠יָּ֣קָם עֶזְרָ֡א

**"And Ezra arose"**

Here also, **arose** means “took action to get an enterprise under way.” It could also mean to physically arise, since Ezra had been lying prostrate on the ground. Alternate translation: “So Ezra got up, took initiative,”

See: Idiom

* **Associated Passages:** Ezra 10:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Swear (ID: `keyterm:Swear`)

## Ezra 10:5 (#3) (id: 463646)

וְ⁠כָל־יִשְׂרָאֵ֗ל

**"and all Israel"**

Here the book refers to the Israelites by the name of their nation. Alternate translation: “all of the Israelites”

See: Metonymy

* **Associated Passages:** Ezra 10:5
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Swear (ID: `keyterm:Swear`)

## Ezra 10:5 (#4) (id: 463647)

וְ⁠כָל־יִשְׂרָאֵ֗ל

**"and all Israel"**

Here, **all Israel** is an exaggeration that is meant to express the idea that Ezra made every Israelite who was there swear an oath. If this is not clear in your language, you can state it without the exaggeration. Alternate translation: “and every Israelite who was there”

See: Hyperbole

* **Associated Passages:** Ezra 10:5
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Swear (ID: `keyterm:Swear`)

## Ezra 10:5 (#5) (id: 463648)

לַ⁠עֲשׂ֛וֹת כַּ⁠דָּבָ֥ר הַ⁠זֶּ֖ה

**"to do according to this word"**

The implication in context is that **this word** refers to Shekaniah’s recommendation that the Jews send away all the foreign wives and their children. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “to do what Shekaniah had recommended and make the Israelites send away all the foreign wives and their children”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Tribe of Levi (ID: `group:Levi`); Israel (ID: `person:Israel`); Levi (ID: `person:Levi.3`); Swear (ID: `keyterm:Swear`)

## Ezra 10:6 (#1) (id: 463649)

מִ⁠לִּ⁠פְנֵי֙ בֵּ֣ית הָֽ⁠אֱלֹהִ֔ים

**"from before the face of the house of God"**

As in [10:1](https://ref.ly/Ezra10:1), **face** here means the front of a place. Alternate translation: “from in front of the temple”

See: Metaphor

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#2) (id: 463650)

וַ⁠יֵּ֕לֶךְ אֶל־לִשְׁכַּ֖ת יְהוֹחָנָ֣ן בֶּן־אֶלְיָשִׁ֑יב

**"and he went to the chamber of Jehohanan, the son of Eliashib"**

As in [8:29](https://ref.ly/Ezra8:29), **chamber** means a room, and in context here it seems to refer similarly to one of the rooms in the temple. Eliashib was the high priest, and so it is probable that one of his family members would have had a room there. The book does not say why Ezra went to one of the rooms in the temple, but the implication is that he did so in order to meet further with the community leaders and make plans for the assembly that the following verses describe. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “and he went to one of the rooms in the temple, the one that belonged to Jehohanan the grandson of Eliashib, to meet further with the leaders to make plans to call an assembly of all the returned exiles”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#3) (id: 463651)

יְהוֹחָנָ֣ן בֶּן־אֶלְיָשִׁ֑יב

**"Jehohanan, the son of Eliashib"**

Jehohanan was actually the grandson of Eliashib, so **son** means “descendant.” Alternate translation: “Jehohanan, the grandson of Eliashib”

See: Metaphor

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#4) (id: 463652)

יְהוֹחָנָ֣ן \& אֶלְיָשִׁ֑יב

**"Jehohanan,"** \- **"Eliashib"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#5) (id: 463653)

וַ⁠יֵּ֣לֶךְ שָׁ֗ם לֶ֤חֶם לֹֽא־אָכַל֙ וּ⁠מַ֣יִם לֹֽא־שָׁתָ֔ה

**"And he went there. He did not eat bread and he did not drink water"**

Here the book leaves out some of the words that would ordinarily be needed in order to express a complete meaning. Alternate translation: “And when he went there, he did not eat bread or drink water”

See: Ellipsis

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#6) (id: 463654)

לֶ֤חֶם לֹֽא־אָכַל֙ וּ⁠מַ֣יִם לֹֽא־שָׁתָ֔ה כִּ֥י מִתְאַבֵּ֖ל עַל־מַ֥עַל הַ⁠גּוֹלָֽה

**"He did not eat bread and he did not drink water, but was mourning on account of the unfaithfulness of the exiles"**

If it would be helpful in your language, you could reverse the order of these phrases, since the second phrase gives the reason for the action that the first phrase describes. Alternate translation: “Because he was still grieving over the way the Jews who had returned to their homeland had disobeyed, he did not eat bread or drink water.”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#7) (id: 463655)

לֶ֤חֶם לֹֽא־אָכַל֙ וּ⁠מַ֣יִם לֹֽא־שָׁתָ֔ה כִּ֥י מִתְאַבֵּ֖ל עַל־מַ֥עַל הַ⁠גּוֹלָֽה

**"He did not eat bread and he did not drink water, but was mourning on account of the unfaithfulness of the exiles"**

Not eating or drinking was a symbolic action whose purpose was to show that Ezra was grieving over the disobedience of the exiles. If it would be helpful in your language, you could say that explicitly. Alternate translation: “To show that he was still grieving over the way that the Jews who had returned to their homeland had disobeyed, he did not eat or drink anything”

See: Symbolic Action

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#8) (id: 463656)

לֶ֤חֶם לֹֽא־אָכַל֙ וּ⁠מַ֣יִם לֹֽא־שָׁתָ֔ה

**"He did not eat bread and he did not drink water"**

The book is using **bread**, one kind of food, to represent all food, and it is using **water**, one kind of drink, to represent all drink. Alternate translation: “He did not eat or drink anything”

See: Synecdoche

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#9) (id: 463657)

מַ֥עַל הַ⁠גּוֹלָֽה

**"the unfaithfulness of the exiles"**

Here, **unfaithfulness** means disobedience. Alternate translation: “the way that the Jews who had returned to their homeland had disobeyed God”

See: Metaphor

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:6 (#10) (id: 463658)

מַ֥עַל הַ⁠גּוֹלָֽה

**"the unfaithfulness of the exiles"**

The abstract noun **exiles** describes the Jews who had returned to their homeland from Babylon. (The term seems to be equivalent to the longer phrase “the sons of the exile” that is used in several places in the book, including in the next verse.) If it would be helpful in your language, you could translate the idea behind this term with an equivalent phrase. Alternate translation: “the way the Jews who had returned to their homeland had disobeyed God”

See: Abstract Nouns

* **Associated Passages:** Ezra 10:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.3`); Jehohanan (ID: `person:Jehohanan.4`); Ezra (ID: `person:Ezra`); Be Unfaithful (ID: `keyterm:Unfaithful`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:7 (#1) (id: 463659)

וַ⁠יַּעֲבִ֨ירוּ ק֜וֹל בִּ⁠יהוּדָ֣ה וִ⁠ירֽוּשָׁלִַ֗ם

**"And they caused a sound to pass throughout Judah and Jerusalem"**

As in [1:1](https://ref.ly/Ezra1:1), this **sound** is the voice of messengers who speak a message, and it represents the message that the messengers speak. Refer to whichever is natural in your language: the message, the messenger, or the messenger's voice. Alternate translation: “They sent messengers throughout Judah and Jerusalem”

See: Metonymy

* **Associated Passages:** Ezra 10:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 10:7 (#2) (id: 463660)

בְּנֵ֣י הַ⁠גּוֹלָ֔ה

**"the sons of the exile"**

As indicated in [2:1](https://ref.ly/Ezra2:1) and [8:35](https://ref.ly/Ezra8:35), the phrase **the sons of the exile** refers specifically to the group of Jewish people who returned to the land of Judah from Babylon where King Nebuchadnezzar had taken them (or their ancestors) as captives many years earlier when he conquered Jerusalem. Alternate translation: “the Jews who had returned from exile” or “the Jews who had returned to their homeland”

See: Idiom

* **Associated Passages:** Ezra 10:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 10:7 (#3) (id: 463661)

בְּנֵ֣י הַ⁠גּוֹלָ֔ה

**"the sons of the exile"**

The abstract noun **exile** refers to the way the Babylonians had taken the Jews as captives and transported them away from their homeland. If it would be helpful in your language, you could translate the idea behind this term with a verb such as “return,” which would describe the present situation of some of these Jews and their descendants. Alternate translation: “the Jews who had returned to their homeland”

See: Abstract Nouns

* **Associated Passages:** Ezra 10:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Judah (ID: `person:Judah.4`)

## Ezra 10:8 (#1) (id: 463662)

כַּ⁠עֲצַ֤ת הַ⁠שָּׂרִים֙ וְ⁠הַ⁠זְּקֵנִ֔ים

**"according to the counsel of the leaders and the elders"**

Alternate translation: “as the leaders and elders had specified that they must”

* **Associated Passages:** Ezra 10:8

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Set Apart for God (ID: `keyterm:Proscribe`)

## Ezra 10:8 (#2) (id: 463663)

יָחֳרַ֖ם כָּל־רְכוּשׁ֑⁠וֹ

**"all his property would be devoted to the ban"**

**Devoted to the ban** is an idiom that refers to the complete loss of property. While in earlier times this would have meant that the property would be totally destroyed, in this context it likely meant that it would be confiscated and sold and that the proceeds would be put in the temple treasury. Alternate translation: “all of his property would be confiscated”

See: Idiom

* **Associated Passages:** Ezra 10:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Set Apart for God (ID: `keyterm:Proscribe`)

## Ezra 10:8 (#3) (id: 463664)

יָחֳרַ֖ם כָּל־רְכוּשׁ֑⁠וֹ

**"all his property would be devoted to the ban"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “the leaders would take away all of his property”

See: Active or Passive

* **Associated Passages:** Ezra 10:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Set Apart for God (ID: `keyterm:Proscribe`)

## Ezra 10:8 (#4) (id: 463665)

וְ⁠ה֥וּא יִבָּדֵ֖ל מִ⁠קְּהַ֥ל הַ⁠גּוֹלָֽה

**"And he himself would be separated from the assembly of the exiles"**

**Separated from the assembly** is a Hebrew idiom that means to be expelled from the community and no longer considered a Jew. Alternate translation: “He himself would be expelled from the Jewish community”

See: Idiom

* **Associated Passages:** Ezra 10:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Set Apart for God (ID: `keyterm:Proscribe`)

## Ezra 10:8 (#5) (id: 463666)

וְ⁠ה֥וּא יִבָּדֵ֖ל מִ⁠קְּהַ֥ל הַ⁠גּוֹלָֽה

**"And he himself would be separated from the assembly of the exiles"**

If it would be helpful in your language, you could say this with an active form, and you could say who would do the action. Alternate translation: “the leaders would remove him from belonging to the Jewish community”

See: Active or Passive

* **Associated Passages:** Ezra 10:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Set Apart for God (ID: `keyterm:Proscribe`)

## Ezra 10:8 (#6) (id: 463667)

מִ⁠קְּהַ֥ל הַ⁠גּוֹלָֽה

**"from the assembly of the exiles"**

As often in the book, the abstract noun **exiles** here refers to the group of Jews who had been transported away from their homeland but had since returned. If it would be helpful in your language, you could translate the idea behind this term with a verb such as “return.” Alternate translation: “from the community of Jews who had returned to their homeland”

See: Abstract Nouns

* **Associated Passages:** Ezra 10:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Counsel (ID: `keyterm:Counsel`); Set Apart for God (ID: `keyterm:Proscribe`)

## Ezra 10:9 (#1) (id: 463668)

וַ⁠יִּקָּבְצ֣וּ

**"So"** \- **"gathered"**

The word **so** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result … gathered”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:9 (#2) (id: 463669)

כָל־אַנְשֵֽׁי־יְהוּדָה֩ וּ⁠בִנְיָמִ֨ן

**"all the men of Judah and Benjamin"**

Since the assembly included Israelites from other tribes (for example, the priests and the Levites, who were both from the tribe of Levi), the book is using the tribes of Judah and Benjamin (or their territory), to represent all of the Israelite tribes. Alternate translation: “all of the Israelite men”

See: Synecdoche

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:9 (#3) (id: 463670)

כָל־אַנְשֵֽׁי־יְהוּדָה֩ וּ⁠בִנְיָמִ֨ן

**"all the men of Judah and Benjamin"**

The book may be describing all of the Israelites by reference to the area in which all of them were now living, which had previously been the territory of the tribes of Judah and Benjamin. Alternate translation: “all the Israelite men”

See: Metonymy

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:9 (#4) (id: 463671)

ה֛וּא חֹ֥דֶשׁ הַ⁠תְּשִׁיעִ֖י בְּ⁠עֶשְׂרִ֣ים בַּ⁠חֹ֑דֶשׁ

**"It was the ninth month, on 20 in the month"**

You could convert the Hebrew day and month into an approximate date on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the date will be different every year and the translation will not be entirely accurate. You may just want to use the numbers of the Hebrew day and month. Alternate translation: “It was the twentieth day of the ninth month”

See: Hebrew Months

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:9 (#5) (id: 463672)

ה֛וּא חֹ֥דֶשׁ הַ⁠תְּשִׁיעִ֖י בְּ⁠עֶשְׂרִ֣ים בַּ⁠חֹ֑דֶשׁ

**"It was the ninth month, on 20 in the month"**

Since the story does not say otherwise, the implication is that this was still the same year in which Ezra returned to Jerusalem, that is, the seventh year of Artaxerxes. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “It was the twentieth day of the ninth month in the seventh year of Artaxerxes as king of Persia”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:9 (#6) (id: 463673)

בְּ⁠עֶשְׂרִ֣ים בַּ⁠חֹ֑דֶשׁ

**"on 20 in the month"**

The Hebrew uses a cardinal number here, **20**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, “twentieth” in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of days, you can do that here in your translation. Alternate translation: “on the twentieth day of month”

See: Ordinal Numbers

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:9 (#7) (id: 463674)

בִּ⁠רְחוֹב֙ בֵּ֣ית הָ⁠אֱלֹהִ֔ים

**"in the open place of the house of God"**

The phrase **open place** means a “plaza” or a “square” in the city where there were no buildings and so where large gatherings like this could be held. Alternate translation: “in the temple square”

See: Translate Unknowns

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:9 (#8) (id: 463675)

מַרְעִידִ֥ים עַל־הַ⁠דָּבָ֖ר וּ⁠מֵ⁠הַ⁠גְּשָׁמִֽים

**"trembling on account of the matter and because of the rains"**

As elsewhere in the book, **trembling** here may be a metaphor for regarding someone or something with respect and reverential fear. However, the people were likely trembling literally because of the rain, that is, shaking with cold. So the book is either mixing figurative and literal uses of the same term, or the people were literally shaking because they were all being called to account for a serious disobedience. Alternate translation: “deeply fearful about the issue of disobedience and shaking with cold from the rain” or “shaking with fear because they were being called to account for disobedience and shaking with cold because of the rain”

See: Metaphor

* **Associated Passages:** Ezra 10:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Tribe of Benjamin (ID: `group:Benjamin`); Judah (ID: `person:Judah.4`); Tribe of Judah (ID: `group:Judah`); Benjamin (ID: `person:Benjamin`); Jerusalem (ID: `place:Jerusalem`); LORD (ID: `deity:Lord`); House (ID: `realia:House`)

## Ezra 10:10 (#1) (id: 463676)

וַ⁠יָּ֨קָם עֶזְרָ֤א הַ⁠כֹּהֵן֙ וַ⁠יֹּ֣אמֶר אֲלֵ⁠הֶ֔ם

**"And Ezra the priest rose up and said to them"**

By standing up in front of the assembly, Ezra was indicating that the meeting was starting and that he was leading it. Alternate translation: “Then Ezra stood up in front of the assembly as its leader and said to the people”

See: Symbolic Action

* **Associated Passages:** Ezra 10:10
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 10:10 (#2) (id: 463677)

עֶזְרָ֤א הַ⁠כֹּהֵן֙

**"Ezra the priest"**

Here the book repeats some background information to remind readers who Ezra was. Alternate translation: “Ezra, who was a priest,”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 10:10
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 10:10 (#3) (id: 463678)

וַ⁠יֹּ֣אמֶר אֲלֵ⁠הֶ֔ם

**"and said to them"**

After this phrase, the book begins to quote what Ezra said to the assembly. It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 10:10
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 10:10 (#4) (id: 463679)

מְעַלְתֶּ֔ם וַ⁠תֹּשִׁ֖יבוּ נָשִׁ֣ים נָכְרִיּ֑וֹת

**"You"** \- **"have acted unfaithfully and have caused foreign wives to dwell"**

The phrase **acted unfaithfully** means “committed disobedience.” Alternate translation: “disobeyed by marrying foreign women”

See: Metaphor

* **Associated Passages:** Ezra 10:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 10:10 (#5) (id: 463680)

מְעַלְתֶּ֔ם וַ⁠תֹּשִׁ֖יבוּ נָשִׁ֣ים נָכְרִיּ֑וֹת

**"You"** \- **"have acted unfaithfully and have caused foreign wives to dwell"**

The implication is that this was disobedience to the commandment of Yahweh. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “disobeyed the commandment of Yahweh by marrying foreign women”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 10:10 (#6) (id: 463681)

וַ⁠תֹּשִׁ֖יבוּ נָשִׁ֣ים נָכְרִיּ֑וֹת

**"and have caused foreign wives to dwell"**

See how you translated this expression in [10:2](https://ref.ly/Ezra10:2). Alternate translation: “and have married foreign women”

See: Idiom

* **Associated Passages:** Ezra 10:10
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`); Israel (ID: `person:Israel`); Be Unfaithful (ID: `keyterm:Unfaithful`)

## Ezra 10:11 (#1) (id: 463682)

וְ⁠עַתָּ֗ה תְּנ֥וּ תוֹדָ֛ה לַ⁠יהוָ֥ה אֱלֹהֵֽי־אֲבֹתֵי⁠כֶ֖ם

**"But now, give praise to Yahweh, the God of your fathers"**

In this context, **give praise** is an idiom that means “make confession.” The two ideas are closely related because, as one praises God for his holiness, at the same time one acknowledges one's own lack of holiness. Alternate translation: “confess to Yahweh, the God of your ancestors, that you have sinned”

See: Idiom

* **Associated Passages:** Ezra 10:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Praise (ID: `keyterm:Praise`); LORD (ID: `deity:Lord`); Delight (ID: `keyterm:Delight`)

## Ezra 10:11 (#2) (id: 463683)

אֱלֹהֵֽי־אֲבֹתֵי⁠כֶ֖ם

**"the God of your fathers"**

**Fathers** here means “ancestors.” Alternate translation: “the God of your ancestors”

See: Metaphor

* **Associated Passages:** Ezra 10:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Praise (ID: `keyterm:Praise`); LORD (ID: `deity:Lord`); Delight (ID: `keyterm:Delight`)

## Ezra 10:11 (#3) (id: 463684)

וַ⁠עֲשׂ֣וּ רְצוֹנ֑⁠וֹ וְ⁠הִבָּֽדְלוּ֙ מֵ⁠עַמֵּ֣י הָ⁠אָ֔רֶץ וּ⁠מִן־הַ⁠נָּשִׁ֖ים הַ⁠נָּכְרִיּֽוֹת

**"and do his will, and separate yourselves from the peoples of the land, and from the foreign women"**

With regard to the other people groups, the word **separate** means ending the alliances that had been formed with them by marriage. With regard to the foreign wives, it is a figurative term for divorce. Alternate translation: “and obey him by divorcing and sending away your foreign wives, and thereby breaking the alliances you have formed with the other people groups living in the land”

See: Metaphor

* **Associated Passages:** Ezra 10:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Praise (ID: `keyterm:Praise`); LORD (ID: `deity:Lord`); Delight (ID: `keyterm:Delight`)

## Ezra 10:11 (#4) (id: 463685)

מֵ⁠עַמֵּ֣י הָ⁠אָ֔רֶץ

**"from the peoples of the land"**

This expression refers to the other people groups who were living in and around the province of Judah. Alternate translation: “from the other people groups that are living in the land”

See: Idiom

* **Associated Passages:** Ezra 10:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Praise (ID: `keyterm:Praise`); LORD (ID: `deity:Lord`); Delight (ID: `keyterm:Delight`)

## Ezra 10:11 (#5) (id: 463686)

הַ⁠נָּשִׁ֖ים הַ⁠נָּכְרִיּֽוֹת

**"the foreign women"**

After this phrase, the book ends its quotation of what Ezra said to the assembly on this occasion. If you decided in [10:10](https://ref.ly/Ezra10:10) to mark his words as a quotation, you should indicate their ending here with a closing quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 10:11
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** Praise (ID: `keyterm:Praise`); LORD (ID: `deity:Lord`); Delight (ID: `keyterm:Delight`)

## Ezra 10:12 (#1) (id: 463687)

וַ⁠יַּֽעְנ֧וּ \& וַ⁠יֹּאמְר֖וּ

**"Then"** \- **"answered and said"**

See how you translated this expression in [10:2](https://ref.ly/Ezra10:2). Alternate translation: “Then … responded”

See: Hendiadys

* **Associated Passages:** Ezra 10:12
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)


## Ezra 10:12 (#2) (id: 463688)

ק֣וֹל גָּד֑וֹל

**"in a loud voice"**

After this phrase the book begins to quote what the assembly replied to Ezra. It may be helpful to your readers to indicate this with an opening quotation mark or with whatever other punctuation or convention your language uses to indicate the beginning of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 10:12
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)


## Ezra 10:12 (#3) (id: 463689)

כֵּ֛ן כִּדְבָרְךָ֥ עָלֵ֖י⁠נוּ לַ⁠עֲשֽׂוֹת

**"Thus, according to your word, it is on us to do"**

Like the similar expression “the matter is on you” in [10:4](https://ref.ly/Ezra10:4), this is a Hebrew idiom. Alternate translation: “We agree that it is our responsibility to do what you have said”

See: Idiom

* **Associated Passages:** Ezra 10:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 10:13 (#1) (id: 463690)

אֲבָ֞ל

**"However"**

The people were all shouting their agreement with Ezra in verse 12, but now in verses 13 and 14 it seems that one person speaks on behalf of the group, rather than all of them shouting this together. If it would be helpful in your language, you could indicate this change of speaker, as in the UST. If you do this, you may need to add a mark that indicates a closing quotation at the end of verse 12 and another mark that indicates an opening quotation before this word.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Rebel (ID: `keyterm:Rebel`); Strength (ID: `keyterm:Strength`)

## Ezra 10:13 (#2) (id: 463691)

אֲבָ֞ל

**"However"**

Here, **However** indicates a contrast between the previous verse and this verse. In the previous verse, the people agree that they need to fix this problem, but here they object that they cannot do it immediately. Use whatever form is most natural in your language to show this contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Ezra 10:13
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Rebel (ID: `keyterm:Rebel`); Strength (ID: `keyterm:Strength`)

## Ezra 10:13 (#3) (id: 463692)

הָ⁠עָ֥ם רָב֙

**"the people are many"**

The implication is that there are too many people potentially involved with the problem to deal with all at once. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “there are too many people to deal with all at once”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Rebel (ID: `keyterm:Rebel`); Strength (ID: `keyterm:Strength`)

## Ezra 10:13 (#4) (id: 463693)

וְ⁠הָ⁠עֵ֣ת גְּשָׁמִ֔ים וְ⁠אֵ֥ין כֹּ֖חַ לַ⁠עֲמ֣וֹד בַּ⁠ח֑וּץ

**"and the time is rain showers and there is no strength to stand in the outdoors"**

To say that there is **no strength** for something is an idiom in Hebrew meaning that it is too difficult. Alternate translation: “it is too much of a hardship to make everyone stand outside during the rainy season”

See: Idiom

* **Associated Passages:** Ezra 10:13
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Rebel (ID: `keyterm:Rebel`); Strength (ID: `keyterm:Strength`)

## Ezra 10:13 (#5) (id: 463694)

וְ⁠הַ⁠מְּלָאכָ֗ה לֹֽא־לְ⁠י֤וֹם אֶחָד֙ וְ⁠לֹ֣א לִ⁠שְׁנַ֔יִם כִּֽי־הִרְבִּ֥ינוּ לִ⁠פְשֹׁ֖עַ בַּ⁠דָּבָ֥ר הַ⁠זֶּֽה

**"And the work is neither for one day nor for two, for we have done much to rebel in this matter"**

If it would be helpful in your language, you could reverse the order of these phrases, since the second phrase gives the reason for the action that the first phrase describes. Alternate translation: “Because so many of us have disobeyed Yahweh and married foreign women, it will take a long time to resolve this problem”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 10:13
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Rebel (ID: `keyterm:Rebel`); Strength (ID: `keyterm:Strength`)

## Ezra 10:13 (#6) (id: 463695)

לֹֽא־לְ⁠י֤וֹם אֶחָד֙ וְ⁠לֹ֣א לִ⁠שְׁנַ֔יִם

**"neither for one day nor for two"**

The phrase **neither for one day nor for two** is a figurative way of saying “not a matter of a short time.” Alternate translation: “something that will take a long time”

See: Idiom

* **Associated Passages:** Ezra 10:13
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Rebel (ID: `keyterm:Rebel`); Strength (ID: `keyterm:Strength`)

## Ezra 10:14 (#1) (id: 463696)

יַֽעֲמְדוּ־נָ֣א שָׂ֠רֵי⁠נוּ לְֽ⁠כָל־הַ⁠קָּהָ֞ל

**"Let our leaders stand for all the assembly"**

Here, **stand for** is an idiom that means “represent.” (In [10:16](https://ref.ly/Ezra10:16) it explains specifically how the community arranged for its leaders to be its representatives.) Alternate translation: “Let our leaders represent all of us”

See: Idiom

* **Associated Passages:** Ezra 10:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 10:14 (#2) (id: 463697)

הַ⁠הֹשִׁ֞יב נָשִׁ֤ים נָכְרִיּוֹת֙

**"the ones who have caused foreign women to dwell"**

See how you translated this expression in [10:2](https://ref.ly/Ezra10:2). Alternate translation: “those who have married foreign women”

See: Idiom

* **Associated Passages:** Ezra 10:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 10:14 (#3) (id: 463698)

לְ⁠עִתִּ֣ים מְזֻמָּנִ֔ים

**"at appointed times"**

If your language does not use passive verbal forms, you can use an active form to express this same idea. Alternate translation: “at times that you set for each one” or “when you have arranged for them to come”

See: Active or Passive

* **Associated Passages:** Ezra 10:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 10:14 (#4) (id: 463699)

וְ⁠עִמָּ⁠הֶ֛ם זִקְנֵי־עִ֥יר וָ⁠עִ֖יר וְ⁠שֹׁפְטֶ֑י⁠הָ

**"and with them the elders of city by city and its magistrates"**

The phrase **city by city** is an idiom that means “each city.” Alternate translation: “with the elders and magistrates of each city”

See: Idiom

* **Associated Passages:** Ezra 10:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 10:14 (#5) (id: 463700)

חֲר֤וֹן אַף־אֱלֹהֵ֨י⁠נוּ֙

**"the burning of the nose of our God"**

The idiom **burning of the nose** refers to being angry. Alternate translation: “our God's anger”

See: Idiom

* **Associated Passages:** Ezra 10:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 10:14 (#6) (id: 463701)

עַ֠ד לְ⁠הָשִׁ֞יב חֲר֤וֹן אַף־אֱלֹהֵ֨י⁠נוּ֙ מִמֶּ֔⁠נּוּ עַ֖ד לַ⁠דָּבָ֥ר הַ⁠זֶּֽה

**"until the burning of the nose of our God is turned back from us as far as it concerns this matter"**

When someone’s anger is **turned back**, that means that the person is no longer angry. Alternate translation: “until our God is no longer angry with us for disobeying in this way”

See: Metaphor

* **Associated Passages:** Ezra 10:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 10:14 (#7) (id: 463702)

לַ⁠דָּבָ֥ר הַ⁠זֶּֽה

**"it concerns this matter"**

After this phrase, the book ends its quotation of what the assembly replied to Ezra on this occasion. If you decided in [10:12](https://ref.ly/Ezra10:12) to mark their words as a quotation, you should indicate their ending here with a closing quotation mark or with whatever other punctuation or convention your language uses to indicate the end of a quotation.

See: Quote Markings

* **Associated Passages:** Ezra 10:14
* **Associated Articles:** Quote Markings (ID: `33306@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`)

## Ezra 10:15 (#1) (id: 463703)

אַ֣ךְ

**"Only"**

The word **only** indicates that the sentence it introduces makes an exception from the larger group that was mentioned before. In [10:12](https://ref.ly/Ezra10:12) it appeared that the whole group was in agreement, but now we find out that there were four men who did not agree. Use a natural way to indicate this contrast that makes it clear that there were two men who opposed the idea, then two more joined them in opposing the idea. Alternate translation: “But a few, namely,”

See: Connect — Exception Clauses

* **Associated Passages:** Ezra 10:15
* **Associated Articles:** Connect — Exception Clauses (ID: `33328@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.10`); Asahel (ID: `person:Asahel.4`); Jahzeiah (ID: `person:Jahzeiah`); Shabbethai (ID: `person:Shabbethai`); Tikvah (ID: `person:Tikvah.2`); Meshullam (ID: `person:Meshullam.10`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:15 (#2) (id: 463704)

יוֹנָתָ֧ן \& וְ⁠יַחְזְיָ֥ה \& עָמְד֣וּ עַל־זֹ֑את וּ⁠מְשֻׁלָּ֛ם וְ⁠שַׁבְּתַ֥י \& עֲזָרֻֽ⁠ם

**"Jonathan,"** \- **"and Jahzeiah,"** \- **"stood against this. And Meshullam and Shabbethai"** \- **"supported them"**

The implication is that while these four men spoke up in the assembly against investigating who among the Israelites had married foreign wives, their protests were unsuccessful. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “Jonathan … and Jahzeiah … did not want the community to investigate who had married foreign women. And Meshullam and Shabbethai … supported them, but they were not able to convince the assembly” Alternatively, you could express this idea at the beginning of verse 16\.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.10`); Asahel (ID: `person:Asahel.4`); Jahzeiah (ID: `person:Jahzeiah`); Shabbethai (ID: `person:Shabbethai`); Tikvah (ID: `person:Tikvah.2`); Meshullam (ID: `person:Meshullam.10`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:15 (#3) (id: 463705)

יוֹנָתָ֧ן בֶּן־עֲשָׂהאֵ֛ל

**"Jonathan, the son of Asahel"**

**Jonathan** is the name of a man, and **Asahel** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 10:15
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.10`); Asahel (ID: `person:Asahel.4`); Jahzeiah (ID: `person:Jahzeiah`); Shabbethai (ID: `person:Shabbethai`); Tikvah (ID: `person:Tikvah.2`); Meshullam (ID: `person:Meshullam.10`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:15 (#4) (id: 463706)

וְ⁠יַחְזְיָ֥ה בֶן־תִּקְוָ֖ה

**"and Jahzeiah, the son of Tikvah"**

**Jahzeiah** is the name of a man, and **Tikvah** is the name of his father.

See: How to Translate Names

* **Associated Passages:** Ezra 10:15
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.10`); Asahel (ID: `person:Asahel.4`); Jahzeiah (ID: `person:Jahzeiah`); Shabbethai (ID: `person:Shabbethai`); Tikvah (ID: `person:Tikvah.2`); Meshullam (ID: `person:Meshullam.10`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:15 (#5) (id: 463707)

עָמְד֣וּ עַל־זֹ֑את

**"stood against this"**

Here, **stood against** something means to have opposed it. Alternate translation: “did not want the community to investigate who had married foreign women”

See: Metaphor

* **Associated Passages:** Ezra 10:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.10`); Asahel (ID: `person:Asahel.4`); Jahzeiah (ID: `person:Jahzeiah`); Shabbethai (ID: `person:Shabbethai`); Tikvah (ID: `person:Tikvah.2`); Meshullam (ID: `person:Meshullam.10`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:15 (#6) (id: 463708)

וּ⁠מְשֻׁלָּ֛ם וְ⁠שַׁבְּתַ֥י

**"And Meshullam and Shabbethai"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:15
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jonathan (ID: `person:Jonathan.10`); Asahel (ID: `person:Asahel.4`); Jahzeiah (ID: `person:Jahzeiah`); Shabbethai (ID: `person:Shabbethai`); Tikvah (ID: `person:Tikvah.2`); Meshullam (ID: `person:Meshullam.10`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:16 (#1) (id: 463709)

וַ⁠יַּֽעֲשׂוּ־כֵן֮ בְּנֵ֣י הַ⁠גּוֹלָה֒

**"So the sons of the exile did thus"**

See how you translated the expression **the sons of the exile** in [10:7](https://ref.ly/Ezra10:7). Alternate translation: “So the Jews who had returned to their homeland did what their assembly had decided”

See: Abstract Nouns

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#2) (id: 463710)

וַ⁠יַּֽעֲשׂוּ־כֵן֮ בְּנֵ֣י הַ⁠גּוֹלָה֒

**"So the sons of the exile did thus"**

Since only a few were opposed to the plan, the larger group of Israelites agreed to do it. If it would be helpful in your language, you might want to express this implied information here. Alternate translation: “But since only these few opposed the plan, the group of Israelites who had returned from exile decided to do it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#3) (id: 463711)

וַ⁠יִּבָּדְלוּ֩ עֶזְרָ֨א הַ⁠כֹּהֵ֜ן אֲנָשִׁ֨ים רָאשֵׁ֧י הָ⁠אָב֛וֹת לְ⁠בֵ֥ית אֲבֹתָ֖⁠ם וְ⁠כֻלָּ֣⁠ם בְּ⁠שֵׁמ֑וֹת

**"and men (the heads of the fathers by the house of their fathers, and all of them by names) were separated to Ezra the priest"**

If it would be helpful in your language, you could say this with an active form. Alternate translation: “Ezra the priest summoned the leaders of each clan by name”

See: Active or Passive

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#4) (id: 463712)

רָאשֵׁ֧י הָ⁠אָב֛וֹת לְ⁠בֵ֥ית אֲבֹתָ֖⁠ם

**"the heads of the fathers by the house of their fathers"**

As in [1:5](https://ref.ly/Ezra1:5) and many other places in the book, **the heads of the fathers** is an abbreviated way of saying “the heads of father’s houses,” and the **house of the father** or “father’s house” means a clan. Alternate translation: “the leaders of each clan”

See: Ellipsis

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#5) (id: 463713)

עֶזְרָ֨א הַ⁠כֹּהֵ֜ן

**"to Ezra the priest"**

Here the book repeats some background information to remind readers who Ezra was. Alternate translation: “Ezra, who was a priest”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#6) (id: 463714)

וַ⁠יֵּשְׁב֗וּ

**"And they sat down"**

While it is likely that these leaders conducted their inquiry while sitting down, **sat down** here is a figurative way of saying “convened” or “met to do business.” Alternate translation: “They came back together”

See: Metaphor

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#7) (id: 463715)

בְּ⁠י֤וֹם אֶחָד֙ לַ⁠חֹ֣דֶשׁ הָ⁠עֲשִׂירִ֔י

**"on day one of the tenth month"**

You could convert the Hebrew day and month into an approximate date on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the date will be different every year and the translation will not be entirely accurate. So you may just want to use the numbers of the Hebrew day and month. Alternate translation: “on the first day of the tenth month”

See: Hebrew Months

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#8) (id: 463716)

בְּ⁠י֤וֹם אֶחָד֙ לַ⁠חֹ֣דֶשׁ הָ⁠עֲשִׂירִ֔י

**"on day one of the tenth month"**

The Hebrew uses a cardinal number here, **one**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, “first,” in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of days, you can do that here in your translation. Alternate translation: “on the first day of the tenth month”

See: Ordinal Numbers

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:16 (#9) (id: 463717)

לְ⁠דַרְי֖וֹשׁ הַ⁠דָּבָֽר

**"to seek out the matter"**

Here, **seek out** means to “investigate.” Alternate translation: “to make a list of who had married foreign women”

See: Metaphor

* **Associated Passages:** Ezra 10:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ezra (ID: `person:Ezra`)

## Ezra 10:17 (#1) (id: 463718)

וַ⁠יְכַלּ֣וּ בַ⁠כֹּ֔ל אֲנָשִׁ֕ים

**"And they finished with all the men"**

**Finished with** is an idiom here. Alternate translation: “they succeeded in identifying all the men”

See: Idiom

* **Associated Passages:** Ezra 10:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 10:17 (#2) (id: 463719)

הַ⁠הֹשִׁ֖יבוּ נָשִׁ֣ים נָכְרִיּ֑וֹת

**"the ones who had caused foreign women to dwell"**

See how you translated this expression in [10:2](https://ref.ly/Ezra10:2). Alternate translation: “who had married foreign women”

See: Idiom

* **Associated Passages:** Ezra 10:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 10:17 (#3) (id: 463720)

עַ֛ד י֥וֹם אֶחָ֖ד לַ⁠חֹ֥דֶשׁ הָ⁠רִאשֽׁוֹן

**"by day one of the first month"**

You could convert the Hebrew day and month into an approximate date on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the date will be different every year and the translation will not be entirely accurate. So you may just want to use the numbers of the Hebrew day and month. Alternate translation: “by the first day of the first month”

See: Hebrew Months

* **Associated Passages:** Ezra 10:17
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)


## Ezra 10:17 (#4) (id: 463721)

עַ֛ד י֥וֹם אֶחָ֖ד לַ⁠חֹ֥דֶשׁ הָ⁠רִאשֽׁוֹן

**"by day one of the first month"**

The Hebrew uses a cardinal number here, **one**, but there is not a significant difference in meaning between that and the way the Hebrew uses an ordinal number, “first,” in similar contexts elsewhere. If your language customarily uses ordinals for the numbers of days, you can do that here in your translation. Alternate translation: “on the first day of the tenth month”

See: Ordinal Numbers

* **Associated Passages:** Ezra 10:17
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)


## Ezra 10:17 (#5) (id: 463722)

עַ֛ד י֥וֹם אֶחָ֖ד לַ⁠חֹ֥דֶשׁ הָ⁠רִאשֽׁוֹן

**"by day one of the first month"**

If it is more natural in your language, you can put this time phrase first in the verse, as in the UST.

See: Information Structure

* **Associated Passages:** Ezra 10:17
* **Associated Articles:** Information Structure (ID: `33286@UWTranslationManual`)


## Ezra 10:18 (#1) (id: 463723)

וַ⁠יִּמָּצֵא֙ מִ⁠בְּנֵ֣י

**"And some were found from the sons of"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. “The clan leaders identified some men from among” Alternate translation:

See: Active or Passive

* **Associated Passages:** Ezra 10:18
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jarib (ID: `person:Jarib.3`); Maaseiah (ID: `person:Maaseiah.6`); Gedaliah (ID: `person:Gedaliah`); Jozadak (ID: `person:Jozadak.2`); Eliezer (ID: `person:Eliezer.9`); Jeshua (ID: `person:Jeshua`)

## Ezra 10:18 (#2) (id: 463724)

מִ⁠בְּנֵ֣י הַ⁠כֹּהֲנִ֔ים

**"some"** \- **"from the sons of the priests"**

Here, **sons of** is a figurative expression that means “descendants of.” These men are all descendants of men who had been priests in Jerusalem. Alternate translation: “among the descendants of the priests”

See: Idiom

* **Associated Passages:** Ezra 10:18
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jarib (ID: `person:Jarib.3`); Maaseiah (ID: `person:Maaseiah.6`); Gedaliah (ID: `person:Gedaliah`); Jozadak (ID: `person:Jozadak.2`); Eliezer (ID: `person:Eliezer.9`); Jeshua (ID: `person:Jeshua`)

## Ezra 10:18 (#3) (id: 463725)

אֲשֶׁ֥ר הֹשִׁ֖יבוּ נָשִׁ֣ים נָכְרִיּ֑וֹת

**"who had caused foreign women to dwell"**

See how you translated this expression in [10:2](https://ref.ly/Ezra10:2). Alternate translation: “who had married foreign women”

See: Idiom

* **Associated Passages:** Ezra 10:18
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jarib (ID: `person:Jarib.3`); Maaseiah (ID: `person:Maaseiah.6`); Gedaliah (ID: `person:Gedaliah`); Jozadak (ID: `person:Jozadak.2`); Eliezer (ID: `person:Eliezer.9`); Jeshua (ID: `person:Jeshua`)

## Ezra 10:18 (#4) (id: 463726)

מִ⁠בְּנֵ֨י יֵשׁ֤וּעַ בֶּן־יֽוֹצָדָק֙ וְ⁠אֶחָ֔י⁠ו

**"from the sons of Jeshua, the son of Jozadak, and his brothers"**

Here, **the sons of Jeshua** means “the descendants of Jeshua,” since Jeshua lived many years before this time, but **son of Jozadak** is literal because Jeshua was the biological son of Jozadak. **Brothers** could mean biological brothers, but it is more likely that it refers to relatives. Alternate translation: “from the descendants of Jeshua the son of Jozadak and his relatives”

See: Metaphor

* **Associated Passages:** Ezra 10:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jarib (ID: `person:Jarib.3`); Maaseiah (ID: `person:Maaseiah.6`); Gedaliah (ID: `person:Gedaliah`); Jozadak (ID: `person:Jozadak.2`); Eliezer (ID: `person:Eliezer.9`); Jeshua (ID: `person:Jeshua`)

## Ezra 10:18 (#5) (id: 463727)

מַֽעֲשֵׂיָה֙ וֶֽ⁠אֱלִיעֶ֔זֶר וְ⁠יָרִ֖יב וּ⁠גְדַלְיָֽה

**"Maaseiah, and Eliezer, and Jarib, and Gedaliah"**

These are the names of four men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:18
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jarib (ID: `person:Jarib.3`); Maaseiah (ID: `person:Maaseiah.6`); Gedaliah (ID: `person:Gedaliah`); Jozadak (ID: `person:Jozadak.2`); Eliezer (ID: `person:Eliezer.9`); Jeshua (ID: `person:Jeshua`)

## Ezra 10:19 (#1) (id: 463728)

וַ⁠יִּתְּנ֥וּ

**"So they gave"**

Here the word **So** indicates that the sentence it introduces explains the results of what the previous sentence described. Alternate translation: “As a result, they gave”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Ezra 10:19
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Flock (ID: `fauna:Flock`); Sheep (ID: `fauna:Lamb`); Goat (ID: `fauna:Goat`)

## Ezra 10:19 (#2) (id: 463729)

וַ⁠יִּתְּנ֥וּ יָדָ֖⁠ם

**"So they gave their hand"**

This means that the men shook hands with the clan leaders to signify that they were making a promise. Alternate translation: “So they shook hands in pledge” or “So they grasped hands with their leaders as a sign of promise”

See: Symbolic Action

* **Associated Passages:** Ezra 10:19
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Flock (ID: `fauna:Flock`); Sheep (ID: `fauna:Lamb`); Goat (ID: `fauna:Goat`)

## Ezra 10:19 (#3) (id: 463730)

לְ⁠הוֹצִ֣יא נְשֵׁי⁠הֶ֑ם

**"to cause their wives to go out"**

As in [10:3](https://ref.ly/Ezra10:3), **to cause to go out** is a Hebrew idiom that means to divorce and send away. Alternate translation: “that they would divorce their wives and send them away”

See: Idiom

* **Associated Passages:** Ezra 10:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Flock (ID: `fauna:Flock`); Sheep (ID: `fauna:Lamb`); Goat (ID: `fauna:Goat`)

## Ezra 10:19 (#4) (id: 463731)

וַ⁠אֲשֵׁמִ֥ים אֵֽיל־צֹ֖אן עַל־אַשְׁמָתָֽ⁠ם

**"and, being guilty ones, a ram of the flock for their guilt"**

Here the book leaves out some of the words that a sentence would ordinarily need in order to be complete. Alternate translation: “and that they would sacrifice a ram from their flocks as a guilt offering, since the clan leaders had found them guilty”

See: Ellipsis

* **Associated Passages:** Ezra 10:19
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Flock (ID: `fauna:Flock`); Sheep (ID: `fauna:Lamb`); Goat (ID: `fauna:Goat`)

## Ezra 10:19 (#5) (id: 463732)

אֵֽיל־צֹ֖אן

**"a ram of the flock"**

A **flock** is a group of small animals such as sheep or goats. If it is unnecessary or unnatural to say that the **ram** was from a flock, then you can just say that it was a ram, as in the UST.

See: When to Keep Information Implicit

* **Associated Passages:** Ezra 10:19
* **Associated Articles:** When to Keep Information Implicit (ID: `33275@UWTranslationManual`)

* **Associated ACAI Concepts:** Flock (ID: `fauna:Flock`); Sheep (ID: `fauna:Lamb`); Goat (ID: `fauna:Goat`)

## Ezra 10:20 (#1) (id: 463733)

וּ⁠מִ⁠בְּנֵ֣י אִמֵּ֔ר

**"And from the sons of Immer"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Immer”

See: Metaphor

* **Associated Passages:** Ezra 10:20
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hanani (ID: `person:Hanani.3`); Zebadiah (ID: `person:Zebadiah.9`); Immer (ID: `person:Immer.2`)

## Ezra 10:20 (#2) (id: 463734)

חֲנָ֖נִי וּ⁠זְבַדְיָֽה

**"Hanani and Zebadiah"**

These are the names of two men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:20
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hanani (ID: `person:Hanani.3`); Zebadiah (ID: `person:Zebadiah.9`); Immer (ID: `person:Immer.2`)

## Ezra 10:21 (#1) (id: 463735)

וּ⁠מִ⁠בְּנֵ֖י חָרִ֑ם

**"And from the sons of Harim"**

Here, **sons** means “descendants.” Alternate translation: “And from the descendants of Harim”

See: Metaphor

* **Associated Passages:** Ezra 10:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Uzziah (ID: `person:Uzziah.4`); Maaseiah (ID: `person:Maaseiah.7`); Jehiel (ID: `person:Jehiel.9`); Shemaiah (ID: `person:Shemaiah.16`); Harim (ID: `person:Harim`); Elijah (ID: `person:Elijah.3`)

## Ezra 10:21 (#2) (id: 463736)

מַעֲשֵׂיָ֤ה וְ⁠אֵֽלִיָּה֙ וּֽ⁠שְׁמַֽעְיָ֔ה וִ⁠יחִיאֵ֖ל וְ⁠עֻזִיָּֽה

**"Maaseiah, and Elijah, and Shemaiah, and Jehiel, and Uzziah"**

These are the names of five men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:21
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Uzziah (ID: `person:Uzziah.4`); Maaseiah (ID: `person:Maaseiah.7`); Jehiel (ID: `person:Jehiel.9`); Shemaiah (ID: `person:Shemaiah.16`); Harim (ID: `person:Harim`); Elijah (ID: `person:Elijah.3`)

## Ezra 10:22 (#1) (id: 463737)

וּ⁠מִ⁠בְּנֵ֖י פַּשְׁח֑וּר

**"And from the sons of Pashhur"**

Here, **sons** means “descendants.” Alternate translation: “And from the descendants of Pashhur”

See: Metaphor

* **Associated Passages:** Ezra 10:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Elasah (ID: `person:Elasah`); Ishmael (ID: `person:Ishmael.6`); Elioenai (ID: `person:Elioenai.4`); Jozabad (ID: `person:Jozabad.7`); Maaseiah (ID: `person:Maaseiah.8`); Nethanel (ID: `person:Nethanel.8`); Pashhur (ID: `person:Pashhur`)

## Ezra 10:22 (#2) (id: 463738)

אֶלְיוֹעֵינַ֤י מַֽעֲשֵׂיָה֙ יִשְׁמָעֵ֣אל נְתַנְאֵ֔ל יוֹזָבָ֖ד וְ⁠אֶלְעָשָֽׂה

**"Elioenai, Maaseiah, Ishmael, Nethanel, Jozabad, and Elasah"**

These are the names of six men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:22
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Elasah (ID: `person:Elasah`); Ishmael (ID: `person:Ishmael.6`); Elioenai (ID: `person:Elioenai.4`); Jozabad (ID: `person:Jozabad.7`); Maaseiah (ID: `person:Maaseiah.8`); Nethanel (ID: `person:Nethanel.8`); Pashhur (ID: `person:Pashhur`)

## Ezra 10:23 (#1) (id: 463739)

הַ⁠לְוִיִּ֑ם

**"the Levites"**

See how you translated **Levites** in [2:40](https://ref.ly/Ezra2:40). Review the explanation in the note there if that would be helpful.

See: How to Translate Names

* **Associated Passages:** Ezra 10:23
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Kelaiah (ID: `person:Kelaiah`); Jozabad (ID: `person:Jozabad.8`); Shimei (ID: `person:Shimei.15`); Eliezer (ID: `person:Eliezer.10`); Judah (ID: `person:Judah.5`); Pethahiah (ID: `person:Pethahiah.2`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:23 (#2) (id: 463740)

יוֹזָבָ֣ד וְ⁠שִׁמְעִ֗י וְ⁠קֵֽלָיָה֙ ה֣וּא קְלִיטָ֔א פְּתַֽחְיָ֥ה יְהוּדָ֖ה וֶ⁠אֱלִיעֶֽזֶר

**"Jozabad, and Shimei, and Kelaiah (that is, Kelita), Pethahiah, Judah, and Eliezer"**

These are the names of six men and an alternate name for one of them.

See: How to Translate Names

* **Associated Passages:** Ezra 10:23
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Kelaiah (ID: `person:Kelaiah`); Jozabad (ID: `person:Jozabad.8`); Shimei (ID: `person:Shimei.15`); Eliezer (ID: `person:Eliezer.10`); Judah (ID: `person:Judah.5`); Pethahiah (ID: `person:Pethahiah.2`); Tribe of Levi (ID: `group:Levi`); Levi (ID: `person:Levi.3`)

## Ezra 10:24 (#1) (id: 463741)

הַ⁠מְשֹׁרְרִ֖ים

**"the singers"**

The term **singers** refers to the same occupational group as in [2:41](https://ref.ly/Ezra2:41). Review the explanation in the note there if that would be helpful.

* **Associated Passages:** Ezra 10:24

* **Associated ACAI Concepts:** Telem (ID: `person:Telem`); Shallum (ID: `person:Shallum.10`); Uri (ID: `person:Uri.3`); Eliashib (ID: `person:Eliashib.4`)

## Ezra 10:24 (#2) (id: 463742)

אֶלְיָשִׁ֑יב

**"Eliashib"**

**Eliashib** is the name of a man.

See: How to Translate Names

* **Associated Passages:** Ezra 10:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Telem (ID: `person:Telem`); Shallum (ID: `person:Shallum.10`); Uri (ID: `person:Uri.3`); Eliashib (ID: `person:Eliashib.4`)

## Ezra 10:24 (#3) (id: 463743)

הַ⁠שֹּׁ֣עֲרִ֔ים

**"the gatekeepers"**

The term **gatekeepers** refers to the same occupational group as in [2:42](https://ref.ly/Ezra2:42). Review the explanation in the note there if that would be helpful.

* **Associated Passages:** Ezra 10:24

* **Associated ACAI Concepts:** Telem (ID: `person:Telem`); Shallum (ID: `person:Shallum.10`); Uri (ID: `person:Uri.3`); Eliashib (ID: `person:Eliashib.4`)

## Ezra 10:24 (#4) (id: 463744)

שַׁלֻּ֥ם וָ⁠טֶ֖לֶם וְ⁠אוּרִֽי

**"Shallum, and Telem, and Uri"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Telem (ID: `person:Telem`); Shallum (ID: `person:Shallum.10`); Uri (ID: `person:Uri.3`); Eliashib (ID: `person:Eliashib.4`)

## Ezra 10:25 (#1) (id: 463745)

וּ⁠מִֽ⁠יִּשְׂרָאֵ֑ל

**"And these were from Israel"**

All of the men in the lists in this chapter were Israelites. So the phrase **from Israel** refers to the other Israelite men who did not belong to the clans or groups mentioned in the previous lists. If it would be helpful for your readers, you could make this clear in your translation. Alternate translation: “These were from the rest of the Israelites”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:25
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Izziah (ID: `person:Izziah`); Malchijah (ID: `person:Malchijah.4`); Parosh (ID: `person:Parosh`); Ramiah (ID: `person:Ramiah`); Mijamin (ID: `person:Mijamin.2`); Benaiah (ID: `person:Benaiah.8`); Eleazar (ID: `person:Eleazar.7`); Israel (ID: `person:Israel`)

## Ezra 10:25 (#2) (id: 463746)

מִ⁠בְּנֵ֣י פַרְעֹ֡שׁ

**"From the sons of Parosh"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Parosh”

See: Metaphor

* **Associated Passages:** Ezra 10:25
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Izziah (ID: `person:Izziah`); Malchijah (ID: `person:Malchijah.4`); Parosh (ID: `person:Parosh`); Ramiah (ID: `person:Ramiah`); Mijamin (ID: `person:Mijamin.2`); Benaiah (ID: `person:Benaiah.8`); Eleazar (ID: `person:Eleazar.7`); Israel (ID: `person:Israel`)

## Ezra 10:25 (#3) (id: 463747)

רַ֠מְיָה וְ⁠יִזִּיָּ֤ה וּ⁠מַלְכִּיָּה֙ וּ⁠מִיָּמִ֣ן וְ⁠אֶלְעָזָ֔ר וּ⁠מַלְכִּיָּ֖ה וּ⁠בְנָיָֽה

**"Ramiah, and Izziah, and Malkijah, and Mijamin, and Eleazar, and Malkijah, and Benaiah"**

These are the names of seven men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:25
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Izziah (ID: `person:Izziah`); Malchijah (ID: `person:Malchijah.4`); Parosh (ID: `person:Parosh`); Ramiah (ID: `person:Ramiah`); Mijamin (ID: `person:Mijamin.2`); Benaiah (ID: `person:Benaiah.8`); Eleazar (ID: `person:Eleazar.7`); Israel (ID: `person:Israel`)

## Ezra 10:26 (#1) (id: 463748)

וּ⁠מִ⁠בְּנֵ֖י עֵילָ֑ם

**"And from the sons of Elam"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Elam”

See: Metaphor

* **Associated Passages:** Ezra 10:26
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeremoth (ID: `person:Jeremoth.3`); Mattaniah (ID: `person:Mattaniah.5`); Abdi (ID: `person:Abdi.3`); Zechariah (ID: `person:Zechariah.21`); Elijah (ID: `person:Elijah.4`); Jehiel (ID: `person:Jehiel.10`); Elam (ID: `person:Elam`)

## Ezra 10:26 (#2) (id: 463749)

מַתַּנְיָ֤ה זְכַרְיָה֙ וִ⁠יחִיאֵ֣ל וְ⁠עַבְדִּ֔י וִ⁠ירֵמ֖וֹת וְ⁠אֵלִיָּֽה

**"Mattaniah, Zechariah, and Jehiel, and Abdi, and Jeremoth, and Elijah"**

These are the names of six men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:26
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Jeremoth (ID: `person:Jeremoth.3`); Mattaniah (ID: `person:Mattaniah.5`); Abdi (ID: `person:Abdi.3`); Zechariah (ID: `person:Zechariah.21`); Elijah (ID: `person:Elijah.4`); Jehiel (ID: `person:Jehiel.10`); Elam (ID: `person:Elam`)

## Ezra 10:27 (#1) (id: 463750)

וּ⁠מִ⁠בְּנֵ֖י זַתּ֑וּא

**"And from the sons of Zattu"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Zattu”

See: Metaphor

* **Associated Passages:** Ezra 10:27
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattaniah (ID: `person:Mattaniah.6`); Elioenai (ID: `person:Elioenai.5`); Eliashib (ID: `person:Eliashib.5`); Jeremoth (ID: `person:Jeremoth.4`); Zabad (ID: `person:Zabad.5`); Aziza (ID: `person:Aziza`); Zattu (ID: `person:Zattu.2`)

## Ezra 10:27 (#2) (id: 463751)

אֶלְיוֹעֵנַ֤י אֶלְיָשִׁיב֙ מַתַּנְיָ֣ה וִֽ⁠ירֵמ֔וֹת וְ⁠זָבָ֖ד וַ⁠עֲזִיזָֽא

**"Elioenai, Eliashib, Mattaniah, and Jeremoth, and Zabad, and Aziza"**

These are the names of six men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:27
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattaniah (ID: `person:Mattaniah.6`); Elioenai (ID: `person:Elioenai.5`); Eliashib (ID: `person:Eliashib.5`); Jeremoth (ID: `person:Jeremoth.4`); Zabad (ID: `person:Zabad.5`); Aziza (ID: `person:Aziza`); Zattu (ID: `person:Zattu.2`)

## Ezra 10:28 (#1) (id: 463752)

וּ⁠מִ⁠בְּנֵ֖י בֵּבָ֑י

**"And from the sons of Bebai"**

Here, **sons** means “descendants.” Alternate translation: “from the descendants of Bebai”

See: Metaphor

* **Associated Passages:** Ezra 10:28
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zabbai (ID: `person:Zabbai`); Hananiah (ID: `person:Hananiah.5`); Jehohanan (ID: `person:Jehohanan.5`); Athlai (ID: `person:Athlai`); Bebai (ID: `person:Bebai`)

## Ezra 10:28 (#2) (id: 463753)

יְהוֹחָנָ֥ן חֲנַנְיָ֖ה זַבַּ֥י עַתְלָֽי

**"Jehohanan, Hananiah, Zabbai, and Athlai"**

These are the names of four men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:28
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zabbai (ID: `person:Zabbai`); Hananiah (ID: `person:Hananiah.5`); Jehohanan (ID: `person:Jehohanan.5`); Athlai (ID: `person:Athlai`); Bebai (ID: `person:Bebai`)

## Ezra 10:29 (#1) (id: 463754)

וּ⁠מִ⁠בְּנֵ֖י בָּנִ֑י

**"And from the sons of Bani"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Bani”

See: Metaphor

* **Associated Passages:** Ezra 10:29
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Adaiah (ID: `person:Adaiah.5`); Sheal (ID: `person:Sheal`); Jashub (ID: `person:Jashub.2`); Jeremoth (ID: `person:Jeremoth.5`); Meshullam (ID: `person:Meshullam.11`); Malluch (ID: `person:Malluch.2`); Bani (ID: `person:Bani`)

## Ezra 10:29 (#2) (id: 463755)

מְשֻׁלָּ֤ם מַלּוּךְ֙ וַ⁠עֲדָיָ֔ה יָשׁ֖וּב וּ⁠שְׁאָ֥ל ירמות

**"Meshullam, Malluk, and Adaiah, Jashub, and Sheal, Jeremoth"**

These are the names of six men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:29
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Adaiah (ID: `person:Adaiah.5`); Sheal (ID: `person:Sheal`); Jashub (ID: `person:Jashub.2`); Jeremoth (ID: `person:Jeremoth.5`); Meshullam (ID: `person:Meshullam.11`); Malluch (ID: `person:Malluch.2`); Bani (ID: `person:Bani`)

## Ezra 10:30 (#1) (id: 463756)

וּ⁠מִ⁠בְּנֵ֛י פַּחַ֥ת מוֹאָ֖ב

**"And from the sons of Pahath\-Moab"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Pahath\-Moab”

See: Metaphor

* **Associated Passages:** Ezra 10:30
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattaniah (ID: `person:Mattaniah.7`); Benaiah (ID: `person:Benaiah.9`); Manasseh (ID: `person:Manasseh.3`); Maaseiah (ID: `person:Maaseiah.9`); Binnui (ID: `person:Binnui.3`); Chelal (ID: `person:Chelal`); Adna (ID: `person:Adna`); Bezalel (ID: `person:Bezalel.2`); Pahath-moab (ID: `person:Pahath-moab`)

## Ezra 10:30 (#2) (id: 463757)

עַדְנָ֣א וּ⁠כְלָ֑ל בְּנָיָ֤ה מַעֲשֵׂיָה֙ מַתַּנְיָ֣ה בְצַלְאֵ֔ל וּ⁠בִנּ֖וּי וּ⁠מְנַשֶּֽׁה

**"Adna, and Kelal, Benaiah, Maaseiah, Mattaniah, Bezalel, and Binnui, and Manasseh"**

These are the names of eight men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:30
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattaniah (ID: `person:Mattaniah.7`); Benaiah (ID: `person:Benaiah.9`); Manasseh (ID: `person:Manasseh.3`); Maaseiah (ID: `person:Maaseiah.9`); Binnui (ID: `person:Binnui.3`); Chelal (ID: `person:Chelal`); Adna (ID: `person:Adna`); Bezalel (ID: `person:Bezalel.2`); Pahath-moab (ID: `person:Pahath-moab`)

## Ezra 10:31 (#1) (id: 463758)

וּ⁠בְנֵ֖י חָרִ֑ם

**"And from the sons of Harim"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Harim”

See: Metaphor

* **Associated Passages:** Ezra 10:31
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliezer (ID: `person:Eliezer.11`); Malchijah (ID: `person:Malchijah.5`); Shimeon (ID: `person:Shimeon`); Shemaiah (ID: `person:Shemaiah.17`); Isshijah (ID: `person:Isshijah`); Harim (ID: `person:Harim`)

## Ezra 10:31 (#2) (id: 463759)

אֱלִיעֶ֧זֶר יִשִּׁיָּ֛ה מַלְכִּיָּ֖ה שְׁמַֽעְיָ֥ה שִׁמְעֽוֹן

**"Eliezer, Ishijah, Malkijah, Shemaiah, Shimeon"**

These are the names of five men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:31
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliezer (ID: `person:Eliezer.11`); Malchijah (ID: `person:Malchijah.5`); Shimeon (ID: `person:Shimeon`); Shemaiah (ID: `person:Shemaiah.17`); Isshijah (ID: `person:Isshijah`); Harim (ID: `person:Harim`)

## Ezra 10:32 (#1) (id: 463760)

בְּנְיָמִ֥ן מַלּ֖וּךְ שְׁמַרְיָֽה

**"Benjamin, Malluk, Shemariah"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:32
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Malluch (ID: `person:Malluch.3`); Benjamin (ID: `person:Benjamin.3`); Shemariah (ID: `person:Shemariah.3`)

## Ezra 10:33 (#1) (id: 463761)

מִ⁠בְּנֵ֖י חָשֻׁ֑ם

**"From the sons of Hashum"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Hashum”

See: Metaphor

* **Associated Passages:** Ezra 10:33
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattenai (ID: `person:Mattenai`); Manasseh (ID: `person:Manasseh.4`); Mattattah (ID: `person:Mattattah`); Shimei (ID: `person:Shimei.16`); Zabad (ID: `person:Zabad.6`); Eliphelet (ID: `person:Eliphelet.6`); Hashum (ID: `person:Hashum`); Jeremai (ID: `person:Jeremai`)

## Ezra 10:33 (#2) (id: 463762)

חָשֻׁ֑ם מַתְּנַ֤י מַתַּתָּה֙ זָבָ֣ד אֱלִיפֶ֔לֶט יְרֵמַ֥י מְנַשֶּׁ֖ה שִׁמְעִֽי

**"Hashum: Mattenai, Mattattah, Zabad, Eliphelet, Jeremai, Manasseh, and Shimei"**

These are the names of eight men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:33
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattenai (ID: `person:Mattenai`); Manasseh (ID: `person:Manasseh.4`); Mattattah (ID: `person:Mattattah`); Shimei (ID: `person:Shimei.16`); Zabad (ID: `person:Zabad.6`); Eliphelet (ID: `person:Eliphelet.6`); Hashum (ID: `person:Hashum`); Jeremai (ID: `person:Jeremai`)

## Ezra 10:34 (#1) (id: 463763)

מִ⁠בְּנֵ֣י בָנִ֔י

**"From the sons of Bani"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Bani”

See: Metaphor

* **Associated Passages:** Ezra 10:34
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Amram (ID: `person:Amram.2`); Bani (ID: `person:Bani.5`); Uel (ID: `person:Uel`); Maadai (ID: `person:Maadai`)

## Ezra 10:34 (#2) (id: 463764)

מַעֲדַ֥י עַמְרָ֖ם וְ⁠אוּאֵֽל

**"Maadai, Amram, and Uel"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:34
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Amram (ID: `person:Amram.2`); Bani (ID: `person:Bani.5`); Uel (ID: `person:Uel`); Maadai (ID: `person:Maadai`)

## Ezra 10:35 (#1) (id: 463765)

בְּנָיָ֥ה בֵדְיָ֖ה כלהי

**"Benaiah, Bedeiah, Keluhi"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:35
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bedeiah (ID: `person:Bedeiah`); Cheluhi (ID: `person:Cheluhi`); Benaiah (ID: `person:Benaiah.10`)

## Ezra 10:36 (#1) (id: 463766)

וַנְיָ֥ה מְרֵמ֖וֹת אֶלְיָשִֽׁיב

**"Vaniah, Meremoth, Eliashib"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:36
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Eliashib (ID: `person:Eliashib.6`); Meremoth (ID: `person:Meremoth.2`); Vaniah (ID: `person:Vaniah`)

## Ezra 10:37 (#1) (id: 463767)

מַתַּנְיָ֥ה מַתְּנַ֖י ו⁠יעשו

**"Mattaniah, Mattenai, and Jaasu"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:37
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattenai (ID: `person:Mattenai.2`); Jaasu (ID: `person:Jaasu`); Mattaniah (ID: `person:Mattaniah.8`)

## Ezra 10:38 (#1) (id: 463768)

וּ⁠בָנִ֥י וּ⁠בִנּ֖וּי שִׁמְעִֽי

**"and Bani, and Binnui, Shimei"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:38
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bani (ID: `person:Bani.6`); Shimei (ID: `person:Shimei.17`); Binnui (ID: `person:Binnui.4`)

## Ezra 10:38 (#2) (id: 463769)

וּ⁠בָנִ֥י וּ⁠בִנּ֖וּי שִׁמְעִֽי

**"and Bani, and Binnui, Shimei"**

The ULT represents the reading of the Hebrew text, but many scholars believe that the correct reading of this verse is found in ancient Greek translations. This is represented in the UST. You may choose to follow the reading of the preferred Bible translation used in your area, and include the other reading in a footnote.

See: Textual Variants

* **Associated Passages:** Ezra 10:38
* **Associated Articles:** Textual Variants (ID: `33405@UWTranslationManual`)

* **Associated ACAI Concepts:** Bani (ID: `person:Bani.6`); Shimei (ID: `person:Shimei.17`); Binnui (ID: `person:Binnui.4`)

## Ezra 10:39 (#1) (id: 463770)

וְ⁠שֶֽׁלֶמְיָ֥ה וְ⁠נָתָ֖ן וַ⁠עֲדָיָֽה

**"and Shelemiah, and Nathan, and Adaiah"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:39
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Shelemiah (ID: `person:Shelemiah.2`); Adaiah (ID: `person:Adaiah.6`); Nathan (ID: `person:Nathan.6`)

## Ezra 10:40 (#1) (id: 463771)

מַכְנַדְבַ֥י שָׁשַׁ֖י שָׁרָֽי

**"Maknadebai, Shashai, Sharai"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:40
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Sharai (ID: `person:Sharai`); Machnadebai (ID: `person:Machnadebai`); Shashai (ID: `person:Shashai`)

## Ezra 10:41 (#1) (id: 463772)

עֲזַרְאֵ֥ל וְ⁠שֶׁלֶמְיָ֖הוּ שְׁמַרְיָֽה

**"Azarel, and Shelemiah, Shemariah"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:41
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Azarel (ID: `person:Azarel.4`); Shelemiah (ID: `person:Shelemiah.3`); Shemariah (ID: `person:Shemariah.4`)

## Ezra 10:42 (#1) (id: 463773)

שַׁלּ֥וּם אֲמַרְיָ֖ה יוֹסֵֽף

**"Shallum, Amariah, Joseph"**

These are the names of three men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:42
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Amariah (ID: `person:Amariah.6`); Shallum (ID: `person:Shallum.11`); Joseph (ID: `person:Joseph.13`)

## Ezra 10:43 (#1) (id: 463774)

מִ⁠בְּנֵ֖י נְב֑וֹ

**"From the sons of Nebo"**

Here, **sons** means “descendants.” Alternate translation: “From the descendants of Nebo”

See: Metaphor

* **Associated Passages:** Ezra 10:43
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattithiah (ID: `person:Mattithiah.3`); Jeiel (ID: `person:Jeiel.8`); Zabad (ID: `person:Zabad.7`); Zebina (ID: `person:Zebina`); Joel (ID: `person:Joel.12`); Jaddai (ID: `person:Jaddai`); Benaiah (ID: `person:Benaiah.11`); Nebo (ID: `place:Nebo.2`)

## Ezra 10:43 (#2) (id: 463775)

יְעִיאֵ֤ל מַתִּתְיָה֙ זָבָ֣ד זְבִינָ֔א יַדַּ֥י וְ⁠יוֹאֵ֖ל בְּנָיָֽה

**"Jeiel, Mattithiah, Zabad, Zebina, Jaddai, and Joel, Benaiah"**

These are the names of seven men.

See: How to Translate Names

* **Associated Passages:** Ezra 10:43
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mattithiah (ID: `person:Mattithiah.3`); Jeiel (ID: `person:Jeiel.8`); Zabad (ID: `person:Zabad.7`); Zebina (ID: `person:Zebina`); Joel (ID: `person:Joel.12`); Jaddai (ID: `person:Jaddai`); Benaiah (ID: `person:Benaiah.11`); Nebo (ID: `place:Nebo.2`)

## Ezra 10:44 (#1) (id: 463776)

כָּל־אֵ֕לֶּה נָשְׂא֖וּ נָשִׁ֣ים נָכְרִיּ֑וֹת

**"All of these had lifted foreign women"**

As in [9:2](https://ref.ly/Ezra9:2), **lifted** is an idiom that means “married.” Alternate translation: “All of these men had married foreign women”

See: Idiom

* **Associated Passages:** Ezra 10:44
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Ezra 10:44 (#2) (id: 463777)

וְ⁠יֵ֣שׁ מֵ⁠הֶ֣ם נָשִׁ֔ים וַ⁠יָּשִׂ֖ימוּ בָּנִֽים

**"And there were women among them, and they bore sons"**

The implication is that if a foreign wife had borne children, the Israelite husband would send both the wife and her children away, as [10:3](https://ref.ly/Ezra10:3) describes. If it would be helpful to your readers, you could say that explicitly. Alternate translation: “Some of the foreign wives had borne children, and the men who had married them divorced them and sent away both them and their children.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Ezra 10:44
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Ezra 10:44 (#3) (id: 463778)

בָּנִֽים

**"sons"**

The term **sons** here means “children” and includes both boys and girls. Alternate translation: “children”

See: When Masculine Words Include Women

* **Associated Passages:** Ezra 10:44
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)


## Ezra - Chapter 1 Introduction (id: 718614)

Ezra 1 General Notes
====================

Structure and formatting
------------------------

The chapter records the story of the first Jews as they return from Persia to Judea.

Special concepts in this chapter
--------------------------------

### King Cyrus

King Cyrus allowed the Jews to return because he wanted them to rebuild the temple. Those who stayed behind gave gifts to those who left to help them on their journey and resettlement. This practice was common under the reign of Cyrus and was used as a way to maintain peace throughout his kingdom. (See: Temple)

Possible translation difficulties in this chapter
-------------------------------------------------

### Jews

After they returned to Judea, the focus of the rest of the Old Testament is on the Jewish people.

* **Associated Passages:** Ezra 1:1
* **Associated Articles:** Temple (ID: `27142@UWTranslationWords`)


## Ezra - Chapter 2 Introduction (id: 718615)

Ezra 2 General Notes
====================

Special concepts in this chapter
--------------------------------

### Genealogy

People had to prove through their genealogies that they were priests or that they were even Jews. (See: Priest)

* **Associated Passages:** Ezra 2:1
* **Associated Articles:** Priest (ID: `26979@UWTranslationWords`)


## Ezra - Chapter 3 Introduction (id: 718616)

Ezra 3 General Notes
====================

Structure and formatting
------------------------

This chapter begins the story of the building of the temple and re\-establishment of worship in the new temple. (See: Temple)

Special concepts in this chapter
--------------------------------

### Rebuilding the temple

They immediately began the temple worship even though the temple had not yet been built because they feared the people of the surrounding nations.

* **Associated Passages:** Ezra 3:1
* **Associated Articles:** Temple (ID: `27142@UWTranslationWords`)


## Ezra - Chapter 4 Introduction (id: 718617)

Ezra 4 General Notes
====================

Structure and formatting
------------------------

The story of the building of the temple and re\-establishment of the temple worship continues in this chapter. (See: Temple)

Special concepts in this chapter
--------------------------------

### Helping to build the temple

The people of the surrounding nations offered to help to build the temple. It is unknown why the Jews refused their help, although it seems likely that the Jews considered these people to not truly belong to the people of God. These other people became their enemies and tried to hinder the work. They even persuaded the king of Persia to stop the Jews from building.

* **Associated Passages:** Ezra 4:1
* **Associated Articles:** Temple (ID: `27142@UWTranslationWords`)


## Ezra - Chapter 5 Introduction (id: 718618)

Ezra 5 General Notes
====================

Structure and formatting
------------------------

The story of the building of the temple and re\-establishment of the temple worship continues in this chapter from where it left off in [4:5](https://ref.ly/Ezra4:5). (See: Temple)

Special concepts in this chapter
--------------------------------

### Rebuilding the temple

The prophets Haggai and Zechariah encouraged the Jews to begin again to build the temple. This was very important to life in Judah. (See: Prophet)

* **Associated Passages:** Ezra 5:1
* **Associated Articles:** Prophet (ID: `26987@UWTranslationWords`); Temple (ID: `27142@UWTranslationWords`)


## Ezra - Chapter 6 Introduction (id: 718619)

Ezra 6 General Notes
====================

Structure and formatting
------------------------

The completion of the story of the building of the temple and re\-establishment of the temple worship occurs in this chapter. (See: Temple)

Special concepts in this chapter
--------------------------------

### Temple taxes

The king said that the Jews were right and ordered money from his taxes to be used to help them with their sacrifices.

Other possible translation difficulties in this chapter
-------------------------------------------------------

### Darius

In this chapter, Darius is called the king of Assyria. In reality, besides ruling over the former Assyrian Empire, Darius was also king of Persia. Persia had conquered Babylon, which had previously conquered Assyria. This made the king of Persia to be the king of Assyria as well. It was unusual to refer to Darius as the king or ruler of Assyria. Ezra may have referred to him in this way to contrast Darius’ actions with those of the former rulers of Assyria, who had treated the Jews very cruelly. Those earlier Assyrian rulers were the ones who had conquered the northern tribes of Israel and deported them to other lands. It was for this reason that the northern tribes lost their identity and were no longer a distinct people group.

* **Associated Passages:** Ezra 6:1
* **Associated Articles:** Temple (ID: `27142@UWTranslationWords`)


## Ezra - Chapter 7 Introduction (id: 718620)

Ezra 7 General Notes
====================

Structure and formatting
------------------------

### Possible title: Ezra begins his religious reforms

### Ezra's geneology

Ezra’s genealogy in [7:1–5](https://ref.ly/Ezra7:1-Ezra7:5) traces Ezra back to Aaron in a list of 16 ancestors. As is common in ancient genealogies, this list almost certainly does not include all of the generations between Aaron and Ezra. Aaron ([7:5](https://ref.ly/Ezra7:5)) was the first high priest under the law of Moses and the first to serve in the tabernacle. Azariah ([7:3](https://ref.ly/Ezra7:3)) was the first high priest to serve in the temple that Solomon built, which this book describes in [5:11](https://ref.ly/Ezra5:11). Seraiah was the last high priest to serve in that temple. The Babylonians executed him when they conquered Jerusalem and destroyed the temple. The names in the list are arranged so that there will be seven generations between Aaron and Azariah, and seven generations between Azariah and Ezra, with Seraiah listed as the last generation before Ezra. If you have the freedom to use formatting creatively in your translation, you could make the implicit information about Aaron, Azariah, and Seraiah explicit and format this genealogy in a way that will highlight its purpose and design. Alternate translation and formatting for verse 1 through the first 2 words of verse 6:

Ezra—

the descendant of Seraiah, the last high priest in Solomon’s temple, the son of Azariah, the son of Hilkiah, the son of Shallum, the son of Zadok, the descendant of Ahitub, the descendant of Amariah,

the son of Azariah, the first high priest in Solomon’s temple,

the descendant of Meraioth, the son of Zerahiah, the son of Uzzi, the son of Bukki, the son of Abishua, the son of Phinehas, the son of Eleazar,

the son of Aaron, the first high priest in the tabernacle

—this Ezra

Special concepts in this chapter
--------------------------------

### God’s Law

The people no longer know the law of Moses. Therefore, the king allows Ezra to return to Judea to teach the people about God’s law. Many people go with him. (See: Law of Moses)

* **Associated Passages:** Ezra 7:1
* **Associated Articles:** Law of Moses (ID: `26817@UWTranslationWords`)


## Ezra - Chapter 8 Introduction (id: 718621)

Ezra 8 General Notes
====================

Special concepts in this chapter
--------------------------------

### The people’s return to Judah

Many people went back to Judah with Ezra. They trusted God to protect them and the precious items that they carried with them, which had been given for the temple. (See: Trust)

* **Associated Passages:** Ezra 8:1
* **Associated Articles:** Trust (ID: `27180@UWTranslationWords`)


## Ezra - Chapter 9 Introduction (id: 718622)

Ezra 9 General Notes
====================

Special concepts in this chapter
--------------------------------

### Intermarriage

When Ezra found out that many Jews who had returned from exile had married Gentile wives, he prayed to God and confessed this sin of his people. He acknowledged that God had been good to them much more than they deserved by letting these few people return from captivity and then they sinned by marrying Gentile wives. The Jewish people had done this before and God had punished them for it. God forbade this type of marriage because it caused the people to worship other gods. (See: Sin and False God)

### Exclusive forms

Ezra uses many first person plural pronouns as he prays to God. Since Ezra is speaking to God, these forms of “we,” “us,” and “our” would all be the exclusive forms. (See: Exclusive and Inclusive ‘We’)

* **Associated Passages:** Ezra 9:1
* **Associated Articles:** False God (ID: `26586@UWTranslationWords`); Sin (ID: `27098@UWTranslationWords`); Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)


## Ezra - Chapter 10 Introduction (id: 718623)

Ezra 10 General Notes
=====================

Special concepts in this chapter
--------------------------------

### Intermarriages

In this chapter, Jewish men agree to divorce their Gentile wives. Divorce should not have been a common practice for the Jews, but they did it at this time because marriage between Jewish men and non\-Jewish women was not allowed.

* **Associated Passages:** Ezra 10:1

