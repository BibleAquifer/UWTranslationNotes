# Translation Notes (unfoldingWord)

**unfoldingWord® Translation Notes** © 2022 unfoldingWord. Released under CC BY\-SA 4\.0 license. *unfoldingWord® Translation Notes* has been adapted in the following languages: Tok Pisin, Arabic (عربي), French (Français), Hindi (हिंदी), Indonesian (Bahasa Indonesia), Portuguese (Português), Russian (Русский), Spanish (Español), Swahili (Kiswahili), and Simplified Chinese (简体中文) from *unfoldingWord® Translation Notes* © 2022 unfoldingWord. Released under CC BY\-SA 4\.0 license by Mission Mutual



--------------------------------


## Esther - Introduction (id: 718561)

Introduction to Esther
======================

Part 1: General Introduction
----------------------------

### Outline of Esther

1. King Ahasuerus sends away his wife, the queen (1:1–22\)
2. Ahasuerus chooses Esther as the new queen (2:1–23\)
3. Haman plots to destroy the Jews (3:1–15\)
4. Mordecai asks Esther to help her people (4:1–17\)
5. Esther pleads with the king for the Jews (5:1–7:10\)
6. The result of Haman’s plot to destroy the Jews (8:1–9:16\)
7. The Feast of Purim (9:17–32\)
8. Conclusion (10:1–3\)

### What is the Book of Esther about?

The Book of Esther tells how a young Jewish woman named Esther became the queen of Persia. As queen, she worked to save all the Jews in the Persian Empire from being destroyed.

This book ends by telling why the Jews celebrate the festival of Purim. The name “Purim” comes from the word “pur,” which means “lots” or “dice.” Haman, the enemy of Jews, threw dice to choose when to attack and destroy the Jews. The Jews celebrate Purim to remember how Yahweh rescued his people from being destroyed.

### How should the title of this book be translated?

Translators can use the traditional title of this book, which is “The Book of Esther” or just “Esther.” Or they can choose a clearer title, such as “The Book About Esther.”

Part 2: Important Religious and Cultural Concepts
-------------------------------------------------

### What was the Persian Empire?

King Cyrus the Great conquered and ruled over many kingdoms. This was in the part of the world called Persia, which is now Iran. So people named his kingdom the Persian Empire. When Cyrus conquered Babylonia in 539 B.C., he then controlled the Jews whom the Babylonians had earlier taken into exile.

### Why were there Jews in Babylonia when the Persians conquered it?

In 586 B.C., the Babylonians conquered and took into exile the people of Judah. These Jews and their descendants were still in Babylon when the Persians conquered it.

### What was meant by “the laws of the Medes and Persians”?

The phrase “the laws of the Medes and Persians” is found in Esther 1:19 and Daniel 6:12\. It referred to laws and decrees that could not be changed or removed once they were issued. In the book of Esther, the king made a decree that the people could attack the Jews. Later he regretted that decision but he was not able to change the decree.

The term “Medes” refers to a people group that had formed its own nation, but the Persians conquered them.

Part 3: Important Translation Issues
------------------------------------

### What different levels of language are in the Book of Esther?

In the Book of Esther, people talk to each other in many different situations. There is the polite and stately talk in the Persian court and the words of royal decrees. Friends and close relatives also talk to each other. There are even the words that one uses in speaking to oneself. Translators should use all the ways their own language has to express these different situations in a way that their readers will identify and understand.

* **Associated Passages:** Esther 0:1


## Esther 1:1 (#1) (id: 261875)

וַ⁠יְהִ֖י

**"Now it happened"**

In the Old Testament, this is a standard way of beginning a historical story. Many languages have similar story\-opening formulas, and if your language has one, you can use it. But do not use it if it would suggest that the story is not real, only made up. Alternate translation: “this account happened”

See: Introduction of a New Event

* **Associated Passages:** Esther 1:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:1 (#2) (id: 261876)

בִּ⁠ימֵ֣י אֲחַשְׁוֵר֑וֹשׁ

**"in the days of Ahasuerus"**

The term **day** is used here to refer to a longer period of time. You could just say “during the reign of Ahasuerus,” as UST does. But as an alternative, you could say, “in the time of Ahasuerus”

See: Idiom

* **Associated Passages:** Esther 1:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:1 (#3) (id: 261877)

אֲחַשְׁוֵר֑וֹשׁ

**"Ahasuerus"**

This is a man’s name. It occurs many times in the story. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 1:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:1 (#4) (id: 261878)

ה֣וּא אֲחַשְׁוֵר֗וֹשׁ הַ⁠מֹּלֵךְ֙ מֵ⁠הֹ֣דּוּ וְ⁠עַד־כּ֔וּשׁ שֶׁ֛בַע וְ⁠עֶשְׂרִ֥ים וּ⁠מֵאָ֖ה מְדִינָֽה

**"he was Ahasuerus, the one who ruled from India even as far as Ethiopia, 127 provinces"**

This is background information to help the reader identify Ahasuerus.

See: Background Information

* **Associated Passages:** Esther 1:1
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:1 (#5) (id: 261879)

ה֣וּא אֲחַשְׁוֵר֗וֹשׁ הַ⁠מֹּלֵךְ֙

**"he was Ahasuerus, the one who ruled"**

Alternative translation: “This king named Ahasuerus ruled”

* **Associated Passages:** Esther 1:1

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:1 (#6) (id: 261880)

מֵ⁠הֹ֣דּוּ וְ⁠עַד־כּ֔וּשׁ

**"from India even as far as Ethiopia"**

In case your audience might not know where these places are, you could say, “extending from India in the east to Ethiopia in the west.”

See: Background Information

* **Associated Passages:** Esther 1:1
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:1 (#7) (id: 261881)

שֶׁ֛בַע וְ⁠עֶשְׂרִ֥ים וּ⁠מֵאָ֖ה מְדִינָֽה

**"127 provinces"**

The number is given to show what a large empire this was. You could say that explicitly by saying, “Ahasuerus ruled a very large empire that had 127 provinces.”

See: Background Information

* **Associated Passages:** Esther 1:1
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:1 (#8) (id: 261882)

שֶׁ֛בַע וְ⁠עֶשְׂרִ֥ים וּ⁠מֵאָ֖ה מְדִינָֽה

**"127 provinces"**

Alternate translation: “one hundred twenty\-seven provinces”

See: Numbers

* **Associated Passages:** Esther 1:1
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ahasuerus (ID: `person:Ahasuerus`); Ethiopia (ID: `place:Ethiopia`)

## Esther 1:2 (#1) (id: 261883)

כְּ⁠שֶׁ֣בֶת \& עַ֚ל כִּסֵּ֣א מַלְכוּת֔⁠וֹ

**"as"** \- **"sat on the throne of his royalty"**

Here, the action of **sitting** on a throne is used to mean ruling over a kingdom. You could just say “ruled,” as UST does. But as an alternative, you could say, “ruled his empire from his royal throne”

See: Metonymy

* **Associated Passages:** Esther 1:2
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:2 (#2) (id: 261884)

עַ֚ל כִּסֵּ֣א מַלְכוּת֔⁠וֹ

**"on the throne of his royalty"**

**Royalty** is an abstract noun that refers to the royal authority that the king exercised. You can translate this idea with a verb by saying, “the throne he ruled from,” or use an adjective, “his royal throne.”

See: Abstract Nouns

* **Associated Passages:** Esther 1:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:2 (#3) (id: 261885)

בְּ⁠שׁוּשַׁ֥ן

**"was in Susa"**

This was the name of a royal city of the Persian kings. It occurs many times in the story. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 1:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:2 (#4) (id: 261886)

הַ⁠בִּירָֽה

**"the citadel"**

This means a castle or palace where a king would live. But since the city of Susa itself is being called a **citadel** here, it’s likely that the term means royal city or capital city. The person telling this story is identifying Susa as the capital by calling it by the name of something closely associated with it, the royal palace within it. A good translation might be “the capital city of Persia.”

See: Translate Unknowns

* **Associated Passages:** Esther 1:2
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:3 (#1) (id: 261887)

בִּ⁠שְׁנַ֤ת שָׁלוֹשׁ֙ לְ⁠מָלְכ֔⁠וֹ

**"in year three of his reign"**

This means that Ahasuerus had already ruled for two full years and that these events took place in the year that followed them. Alternate translation: “during the third year that Ahasuerus ruled his empire” or “after he had ruled for two years”

See: Ordinal Numbers

* **Associated Passages:** Esther 1:3
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Media (ID: `place:Media`); Persia (ID: `place:Persia`)

## Esther 1:3 (#2) (id: 261888)

בִּ⁠שְׁנַ֤ת שָׁלוֹשׁ֙ לְ⁠מָלְכ֔⁠וֹ

**"in year three of his reign"**

**Reign** is an abstract noun that refers to the royal authority that the king exercised. You can translate it with a verb by saying, “during the third year that Ahasuerus ruled.”

See: Abstract Nouns

* **Associated Passages:** Esther 1:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Media (ID: `place:Media`); Persia (ID: `place:Persia`)

## Esther 1:3 (#3) (id: 261889)

עָשָׂ֣ה מִשְׁתֶּ֔ה

**"he made a feast"**

Alternative translation: “he hosted a feast”

* **Associated Passages:** Esther 1:3

* **Associated ACAI Concepts:** Media (ID: `place:Media`); Persia (ID: `place:Persia`)

## Esther 1:3 (#4) (id: 261890)

חֵ֣יל ׀ פָּרַ֣ס וּ⁠מָדַ֗י

**"the army of Persia and Media"**

This likely refers to the leaders of the army. It is describing a part of the army by referring to the whole army. Alternate translation: “the officers of the army”

See: Synecdoche

* **Associated Passages:** Esther 1:3
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Media (ID: `place:Media`); Persia (ID: `place:Persia`)

## Esther 1:3 (#5) (id: 261891)

הַֽ⁠פַּרְתְּמִ֛ים

**"the noblemen"**

This probably means something like “the wealthy landowners.”

* **Associated Passages:** Esther 1:3

* **Associated ACAI Concepts:** Media (ID: `place:Media`); Persia (ID: `place:Persia`)

## Esther 1:3 (#6) (id: 261892)

לְ⁠פָנָֽי⁠ו

**"before his face"**

**Face** stands for the presence of a person, so this phrase means “in his presence.” The invitation was to come to the royal capital to attend a banquet where the king would be present in person. You could say, as UST does, that the king was present in person to host the feast. Or you could say as an alternative, “all of them came to Susa for the feast”

See: Metonymy

* **Associated Passages:** Esther 1:3
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Media (ID: `place:Media`); Persia (ID: `place:Persia`)

## Esther 1:4 (#1) (id: 261893)

בְּ⁠הַרְאֹת֗⁠וֹ אֶת־עֹ֨שֶׁר֙ כְּב֣וֹד מַלְכוּת֔⁠וֹ

**"when he displayed the wealth of the glory of his kingdom"**

You can translate the abstract noun with an emphasized adjective. Alternate translation: “Ahasuerus … wanted to demonstrate … the great wealth of his kingdom”

See: Abstract Nouns

* **Associated Passages:** Esther 1:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`)

## Esther 1:4 (#2) (id: 261894)

בְּ⁠הַרְאֹת֗⁠וֹ

**"when he displayed"**

**He** refers to the king. The implication is that he held this banquet to show all of his officials how wealthy and powerful he was. The invited officials would then go back to all the different parts of the empire and tell everyone this. Alternate translation: “Ahasuerus wanted to demonstrate to his guests”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`)

## Esther 1:4 (#3) (id: 261895)

אֶת־עֹ֨שֶׁר֙ כְּב֣וֹד מַלְכוּת֔⁠וֹ

**"the wealth of the glory of his kingdom"**

The words **wealth** and **glory** have similar meanings and they are used together to emphasize how great his kingdom was. Alternate translation: “the great wealth of his kingdom”

See: Doublet

* **Associated Passages:** Esther 1:4
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`)

## Esther 1:4 (#4) (id: 261896)

בְּ⁠הַרְאֹת֗⁠וֹ \& וְ⁠אֶ֨ת־יְקָ֔ר תִּפְאֶ֖רֶת גְּדוּלָּת֑⁠וֹ

**"when he displayed"** \- **"and the splendor of the beauty of his greatness"**

You can translate the abstract nouns **splendor**, **beauty**, and **greatness** as adjectives. Alternate translation: “Ahasuerus … wanted to demonstrate … that he was a very powerful king”

See: Abstract Nouns

* **Associated Passages:** Esther 1:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`)

## Esther 1:4 (#5) (id: 261897)

יְקָ֔ר תִּפְאֶ֖רֶת גְּדוּלָּת֑⁠וֹ

**"the splendor of the beauty of his greatness"**

The words **splendor** and **beauty** have similar meaning and emphasize how great he was. Alternate translation: “the splendor of his greatness”

See: Doublet

* **Associated Passages:** Esther 1:4
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`)

## Esther 1:4 (#6) (id: 261898)

יָמִ֣ים רַבִּ֔ים שְׁמוֹנִ֥ים וּ⁠מְאַ֖ת יֽוֹם

**"for many days, 180 days"**

This was a very long time for a royal feast to last. The earlier part of the verse provides the reason for this. To make the connection explicit, you could add some explanatory words to the verse like this: “Ahasuerus entertained his guests fabulously because he wanted to demonstrate that his empire was extremely wealthy and that he was a very powerful king. He did so many things for them that the festivities lasted for six months.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`)

## Esther 1:4 (#7) (id: 261899)

שְׁמוֹנִ֥ים וּ⁠מְאַ֖ת יֽוֹם

**"180 days"**

Six months is about **180 days** or about half a year. You can use whatever expression will best express this time period in your language.

See: Numbers

* **Associated Passages:** Esther 1:4
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`)

## Esther 1:5 (#1) (id: 261900)

וּ⁠בִ⁠מְל֣וֹאת ׀ הַ⁠יָּמִ֣ים הָ⁠אֵ֗לֶּה

**"And when those days were fulfilled"**

This implies “at the end of that feast.” You could say that explicitly. Alternate translation: “after those 180 days were over” or “at the end of those six months”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 1:5 (#2) (id: 261901)

עָשָׂ֣ה הַ⁠מֶּ֡לֶךְ \& מִשְׁתֶּ֖ה

**"the king made a feast"**

This was a second feast that the king hosted. He likely did this to thank the people who had worked so hard in hosting the first feast. You could say that explicitly.

See: Numbers

* **Associated Passages:** Esther 1:5
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 1:5 (#3) (id: 261902)

בְּ⁠שׁוּשַׁ֨ן הַ⁠בִּירָ֜ה

**"in Susa the citadel"**

The expression in this case seems to refer specifically to the royal **citadel** from which the city got its name. This second feast would not have been for everyone in the entire city. It was specifically for the people who served the king in the citadel and who had worked so hard over the previous six months hosting the first feast. Alternate translation: “for every man who worked for him in the palace at Susa”

See: Translate Unknowns

* **Associated Passages:** Esther 1:5
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 1:5 (#4) (id: 261903)

הַ⁠נִּמְצְאִים֩

**"who were found"**

Here, **found** is an idiom that means could be found or were there.

See: Idiom

* **Associated Passages:** Esther 1:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 1:5 (#5) (id: 261904)

הַ⁠נִּמְצְאִים֩

**"who were found"**

You can say this with an active form. Alternate translation: “who worked for him in”

See: Active or Passive

* **Associated Passages:** Esther 1:5
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 1:5 (#6) (id: 261905)

לְ⁠מִ⁠גָּ֧דוֹל וְ⁠עַד־קָטָ֛ן

**"from the greatest even to the least"**

The person telling this story is referring to the staff of the palace by speaking of two extreme parts of it, the most important and the least important people who worked there, in order to include everyone in between. You could just explain this meaning, as UST does. Alternate translation: “from the most important official to the least important servant”

See: Merism

* **Associated Passages:** Esther 1:5
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 1:5 (#7) (id: 261906)

שִׁבְעַ֣ת יָמִ֑ים

**"for seven days"**

The story does not suggest that this was an unusually long time by saying, “for many days.” But you could also say something like “a whole week” to show that this was a generous gesture of thanks on the part of the king. Alternate translation: “another feast that lasted seven days”

* **Associated Passages:** Esther 1:5

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 1:6 (#1) (id: 261907)

ח֣וּר ׀ כַּרְפַּ֣ס וּ⁠תְכֵ֗לֶת

**"Linens cotton and blue"**

These **linens** were curtains that were hung in the courtyard. **Cotton** refers to the white color of some of them. So this means white and blue curtains.

See: Translate Unknowns

* **Associated Passages:** Esther 1:6
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:6 (#2) (id: 261908)

ח֣וּר ׀ כַּרְפַּ֣ס וּ⁠תְכֵ֗לֶת

**"Linens cotton and blue"**

You can specify at the beginning of this verse that the location being described is the same as in the previous verse: “In the courtyard, white and blue curtains.”

* **Associated Passages:** Esther 1:6

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:6 (#3) (id: 261909)

בְּ⁠חַבְלֵי־ב֣וּץ וְ⁠אַרְגָּמָ֔ן

**"by cords of byssus and purple"**

**Byssus** means “white linen” and it is used to describe the white color of some of these cords. So this means white and purple cords.

See: Translate Unknowns

* **Associated Passages:** Esther 1:6
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:6 (#4) (id: 261910)

מִטּ֣וֹת ׀ זָהָ֣ב וָ⁠כֶ֗סֶף

**"Couches of gold and silver"**

You can specify that these couches were there for the guests to recline on while they ate.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:6 (#5) (id: 261911)

רִֽצְפַ֥ת

**"the pavement of"**

This word probably refers to a mosaic floor made of inlaid pieces of colorful precious stones.

See: Translate Unknowns

* **Associated Passages:** Esther 1:6
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:6 (#6) (id: 261912)

בַּהַט

**"porphyry"**

This is a kind of red and purple stone that contains pieces of crystal. You could call it “feldspar” or something descriptive like “red marble.”

See: Translate Unknowns

* **Associated Passages:** Esther 1:6
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:6 (#7) (id: 261913)

וָ⁠שֵׁ֖שׁ

**"and alabaster"**

This is a white precious stone. You could identify it as “alabaster.” Alternatively, while it is not exactly the same thing as marble, your readers would get the right idea if you called it “white marble.”

See: Translate Unknowns

* **Associated Passages:** Esther 1:6
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:6 (#8) (id: 261914)

וְ⁠סֹחָֽרֶת

**"and precious stone"**

This word probably refers to a black marble that was used to create borders around mosaics. You could call this “black marble,” or you could just say that the floor contained “another precious stone.”

See: Translate Unknowns

* **Associated Passages:** Esther 1:6
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Cotton (ID: `flora:Cotton`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`); Rope (ID: `realia:Rope`); Money (ID: `realia:Money`); Bed (ID: `realia:Bed`)

## Esther 1:7 (#1) (id: 261915)

וְ⁠הַשְׁקוֹת֙

**"And the serving was"**

The abstract noun **serving** refers to how the drinks were served at the banquet. You can translate it with a verb by saying, “The attendants served wine.”

See: Abstract Nouns

* **Associated Passages:** Esther 1:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 1:7 (#2) (id: 261916)

וְ⁠כֵלִ֖ים מִ⁠כֵּלִ֣ים שׁוֹנִ֑ים

**"with vessels differing from other vessels"**

This could mean that “no two of them were alike.” But you could also just say that the wine was served in “various kinds of golden cups.”

* **Associated Passages:** Esther 1:7

* **Associated ACAI Concepts:** Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 1:7 (#3) (id: 261917)

וְ⁠יֵ֥ין מַלְכ֛וּת רָ֖ב כְּ⁠יַ֥ד הַ⁠מֶּֽלֶךְ

**"and the wine of royalty was abundant according to the hand of the king"**

This means that King Ahasuerus himself paid for all the wine that the guests drank at his seven\-day feast in Susa, and the wine came from his personal supply. Alternate translation: “the king was very generous with the royal wine” or “the king showed a great willingness to give” or “the king provided all of it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 1:7 (#4) (id: 261918)

וְ⁠יֵ֥ין מַלְכ֛וּת רָ֖ב כְּ⁠יַ֥ד הַ⁠מֶּֽלֶךְ

**"and the wine of royalty was abundant according to the hand of the king"**

This probably means special fine wine that only the king could acquire and afford. You can translate the abstract noun **royalty** with an adjective. Alternate translation: “royal wine”

See: Abstract Nouns

* **Associated Passages:** Esther 1:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 1:7 (#5) (id: 261919)

כְּ⁠יַ֥ד הַ⁠מֶּֽלֶךְ

**"according to the hand of the king"**

Here, **hand** refers to the king himself, viewed through his action of giving.

See: Metonymy

* **Associated Passages:** Esther 1:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 1:8 (#1) (id: 261920)

וְ⁠הַ⁠שְּׁתִיָּ֥ה כַ⁠דָּ֖ת

**"And the drinking was according to the law"**

The abstract noun **drinking** refers here not to the action of drinking, but to the guidelines for serving drinks that the king had established for the banquet. Alternate translation: “Ahasuerus had established this rule for all his household attendants who served the wine”

See: Abstract Nouns

* **Associated Passages:** Esther 1:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`)

## Esther 1:8 (#2) (id: 261921)

אֵ֣ין אֹנֵ֑ס

**"There is no compulsion"**

This could mean one of two things: (1\) No one would be stopped from drinking even if the attendants thought they had already had enough. Alternate translation: “there was to be no restriction on drinking” (2\) There would be no requirement to drink. Alternate translation: “no one must be forced to drink” Either way, this was another sign of the generosity that the king showed as he hosted this banquet to thank the people who worked for him. Either he was: (1\) allowing them to drink as much as they wanted to, or (2\) not requiring them to eat and drink everything that was served at a banquet as guests of the king would usually be expected to do. You could say explicitly at the beginning of the verse that the king was giving his guests a special privilege.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`)

## Esther 1:8 (#3) (id: 261922)

כִּי־כֵ֣ן ׀ יִסַּ֣ד הַ⁠מֶּ֗לֶךְ עַ֚ל כָּל־רַ֣ב בֵּית֔⁠וֹ לַ⁠עֲשׂ֖וֹת כִּ⁠רְצ֥וֹן אִישׁ־וָ⁠אִֽישׁ

**"for thus the king had established for every overseer of his house to do according to the desire of man by man"**

This explains why no one had to drink if they did not want to. If it would be helpful in your language, you could give this explanation (the reason) before the result that it accounts for, using a connecting word like “so.” You could say, “The king had established for every overseer of his house to do according to the desire of man by man, so the drinking was according to the law, ‘There is no compulsion.’” Alternate translation: “the king made the attendants who served the wine follow this rule”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 1:8
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`)

## Esther 1:8 (#4) (id: 261923)

לַ⁠עֲשׂ֖וֹת כִּ⁠רְצ֥וֹן אִישׁ־וָ⁠אִֽישׁ

**"to do according to the desire of man by man"**

Alternate translation: “the king wanted every guest to be able to drink as much as he wanted” or “all the guests could drink as little or as much as they wanted”

* **Associated Passages:** Esther 1:8

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`)

## Esther 1:8 (#5) (id: 261924)

אִישׁ־וָ⁠אִֽישׁ

**"man by man"**

This is an idiom that means “everyone.”

See: Idiom

* **Associated Passages:** Esther 1:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`)

## Esther 1:9 (#1) (id: 261925)

גַּ֚ם

**"Also"**

This introduces something else that was happening at the same time. You can indicate this by saying something like “during this time.”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Esther 1:9
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Vashti (ID: `person:Vashti`); House (ID: `realia:House`)

## Esther 1:9 (#2) (id: 261926)

וַשְׁתִּ֣י הַ⁠מַּלְכָּ֔ה

**"Vashti the queen"**

You can explain that she was the wife of Ahasuerus by saying, “Queen Vashti\*\*,\*\* the king’s wife” or “his wife.”

See: Introduction of New and Old Participants

* **Associated Passages:** Esther 1:9
* **Associated Articles:** Introduction of New and Old Participants (ID: `33421@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Vashti (ID: `person:Vashti`); House (ID: `realia:House`)

## Esther 1:9 (#3) (id: 261927)

וַשְׁתִּ֣י הַ⁠מַּלְכָּ֔ה

**"Vashti the queen"**

This is a woman’s name. It occurs several times in this chapter and the next one. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 1:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Vashti (ID: `person:Vashti`); House (ID: `realia:House`)

## Esther 1:9 (#4) (id: 261928)

מִשְׁתֵּ֣ה נָשִׁ֑ים

**"a feast of women"**

This could mean the women who served in the palace, the wives of the men who were officers and servants, or both. You could say, “a feast for the wives of the men who attended” or “for the women who worked in the palace.”

* **Associated Passages:** Esther 1:9

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Vashti (ID: `person:Vashti`); House (ID: `realia:House`)

## Esther 1:9 (#5) (id: 261929)

בֵּ֚ית הַ⁠מַּלְכ֔וּת אֲשֶׁ֖ר לַ⁠מֶּ֥לֶךְ אֲחַשְׁוֵרֽוֹשׁ

**"in the palace of royalty that belonged to the king Ahasuerus"**

Alternate translation: “the royal palace where King Ahasuerus lived”

* **Associated Passages:** Esther 1:9

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Vashti (ID: `person:Vashti`); House (ID: `realia:House`)

## Esther 1:9 (#6) (id: 261930)

בֵּ֚ית הַ⁠מַּלְכ֔וּת

**"in the palace of royalty"**

You can translate the abstract noun **royalty** with the adjective royal. Alternate translation: “royal palace”

See: Abstract Nouns

* **Associated Passages:** Esther 1:9
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Vashti (ID: `person:Vashti`); House (ID: `realia:House`)

## Esther 1:10 (#1) (id: 261931)

בַּ⁠יּוֹם֙ הַ⁠שְּׁבִיעִ֔י

**"On the seventh day"**

Alternate translation: “after six days”

See: Ordinal Numbers

* **Associated Passages:** Esther 1:10
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Carkas (ID: `person:Carkas`); Abagtha (ID: `person:Abagtha`); Biztha (ID: `person:Biztha`); Bigtha (ID: `person:Bigtha`); Harbona (ID: `person:Harbona`); Zethar (ID: `person:Zethar`); Mehuman (ID: `person:Mehuman`); Ahasuerus (ID: `person:Ahasuerus`); Wine (ID: `realia:Wine`); Vine (ID: `flora:Vine`)

## Esther 1:10 (#2) (id: 261932)

כְּ⁠ט֥וֹב לֵב־הַ⁠מֶּ֖לֶךְ בַּ⁠יָּ֑יִן

**"when the heart of the king was pleased by the wine"**

Here, **heart** means the action of thinking or feeling. Alternate translation: “when King Ahasuerus was in a good mood from drinking wine” or “when the king was drunk with wine”

See: Idiom

* **Associated Passages:** Esther 1:10
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Carkas (ID: `person:Carkas`); Abagtha (ID: `person:Abagtha`); Biztha (ID: `person:Biztha`); Bigtha (ID: `person:Bigtha`); Harbona (ID: `person:Harbona`); Zethar (ID: `person:Zethar`); Mehuman (ID: `person:Mehuman`); Ahasuerus (ID: `person:Ahasuerus`); Wine (ID: `realia:Wine`); Vine (ID: `flora:Vine`)

## Esther 1:10 (#3) (id: 261933)

לִ֠⁠מְהוּמָן בִּזְּתָ֨א חַרְבוֹנָ֜א בִּגְתָ֤א וַ⁠אֲבַגְתָא֙ זֵתַ֣ר וְ⁠כַרְכַּ֔ס

**"to Mehuman, Biztha, Harbona, Bigtha and Abagtha, Zethar and Karkas"**

These are names of seven men.

See: How to Translate Names

* **Associated Passages:** Esther 1:10
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Carkas (ID: `person:Carkas`); Abagtha (ID: `person:Abagtha`); Biztha (ID: `person:Biztha`); Bigtha (ID: `person:Bigtha`); Harbona (ID: `person:Harbona`); Zethar (ID: `person:Zethar`); Mehuman (ID: `person:Mehuman`); Ahasuerus (ID: `person:Ahasuerus`); Wine (ID: `realia:Wine`); Vine (ID: `flora:Vine`)

## Esther 1:10 (#4) (id: 261934)

הַ⁠סָּ֣רִיסִ֔ים

**"the"** \- **"eunuchs"**

This term occurs a dozen times in the story. It describes male royal officials who served as guardians for the women who lived in the palace. They were both guardians of the door, to keep unwanted people out of the women’s quarters, and guardians of the women inside, to take care of them and look after their needs. (As we learn in 2:21, some of these officials also protected the king’s private quarters.) As we see here, and as will also be seen in 2:14 and 2:16, their duties included escorting women from place to place. The term likely indicates that, in keeping with the practices of the time, these men were castrated because their work brought them into such close contact with the king’s wives and concubines. If your language has a term for such an official and you think your readers would recognize it, you could use it. Otherwise, you could use a term that describes the role that these officials played in the royal court. Alternate translation: “guardians” or “officials” or “castrated servants”

See: Translate Unknowns

* **Associated Passages:** Esther 1:10
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Carkas (ID: `person:Carkas`); Abagtha (ID: `person:Abagtha`); Biztha (ID: `person:Biztha`); Bigtha (ID: `person:Bigtha`); Harbona (ID: `person:Harbona`); Zethar (ID: `person:Zethar`); Mehuman (ID: `person:Mehuman`); Ahasuerus (ID: `person:Ahasuerus`); Wine (ID: `realia:Wine`); Vine (ID: `flora:Vine`)

## Esther 1:10 (#5) (id: 261935)

הַ⁠מְשָׁ֣רְתִ֔ים אֶת־פְּנֵ֖י הַ⁠מֶּ֥לֶךְ אֲחַשְׁוֵרֽוֹשׁ

**"who served before the face of the king Ahasuerus"**

This is background information to explain who these men were. Alternate translation: “who attended him personally”

See: Background Information

* **Associated Passages:** Esther 1:10
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Carkas (ID: `person:Carkas`); Abagtha (ID: `person:Abagtha`); Biztha (ID: `person:Biztha`); Bigtha (ID: `person:Bigtha`); Harbona (ID: `person:Harbona`); Zethar (ID: `person:Zethar`); Mehuman (ID: `person:Mehuman`); Ahasuerus (ID: `person:Ahasuerus`); Wine (ID: `realia:Wine`); Vine (ID: `flora:Vine`)

## Esther 1:10 (#6) (id: 261936)

אֶת־פְּנֵ֖י הַ⁠מֶּ֥לֶךְ

**"before the face of the king"**

Here, **face** means the presence of a person. The phrase means that these seven men served King Ahasuerus personally.

See: Metonymy

* **Associated Passages:** Esther 1:10
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Carkas (ID: `person:Carkas`); Abagtha (ID: `person:Abagtha`); Biztha (ID: `person:Biztha`); Bigtha (ID: `person:Bigtha`); Harbona (ID: `person:Harbona`); Zethar (ID: `person:Zethar`); Mehuman (ID: `person:Mehuman`); Ahasuerus (ID: `person:Ahasuerus`); Wine (ID: `realia:Wine`); Vine (ID: `flora:Vine`)

## Esther 1:11 (#1) (id: 261937)

לְ֠⁠הָבִיא

**"to bring"**

This is the beginning of the instructions that King Ahasuerus gave to the seven eunuchs he summoned in verse 10\. You can make this explicit by saying, “He told them to bring.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:11 (#2) (id: 261938)

לִ⁠פְנֵ֥י הַ⁠מֶּ֖לֶךְ

**"to the face of the king"**

Here, **face** means the presence of a person. The phrase means that King Ahasuerus wanted Queen Vashti to come personally into his presence.

See: Metonymy

* **Associated Passages:** Esther 1:11
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:11 (#3) (id: 261939)

בְּ⁠כֶ֣תֶר מַלְכ֑וּת

**"in a crown of royalty"**

You can translate the abstract noun **royalty** with an adjective by saying, “wearing her royal crown.”

See: Abstract Nouns

* **Associated Passages:** Esther 1:11
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:11 (#4) (id: 261940)

בְּ⁠כֶ֣תֶר מַלְכ֑וּת

**"in a crown of royalty"**

Ahasuerus likely wanted Vashti to wear her queen’s crown so that everyone would know that she was his wife. You could say that explicitly. (For the possible reasons why he wanted them to know this, see the next note.)

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:11 (#5) (id: 261941)

לְ⁠הַרְא֨וֹת הָֽ⁠עַמִּ֤ים וְ⁠הַ⁠שָּׂרִים֙ אֶת־יָפְיָ֔⁠הּ

**"in order to show the people and the officials her beauty"**

Everything Ahasuerus did at his banquets was to show how rich and powerful he was. He seems to have believed that having a very beautiful wife was one more thing that proved he was a great man. So he wanted everyone to see how beautiful Vashti was. You can put this second in the verse, after explaining that Vashti was a very beautiful woman, because it accounts for what happens afterwards, when the king sends his seven personal servants to bring her in to his banquet.

See: Background Information

* **Associated Passages:** Esther 1:11
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:11 (#6) (id: 261942)

הָֽ⁠עַמִּ֤ים וְ⁠הַ⁠שָּׂרִים֙

**"the people and the officials"**

This might mean “his guests, especially the officials.”

* **Associated Passages:** Esther 1:11

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:11 (#7) (id: 261943)

כִּֽי־טוֹבַ֥ת מַרְאֶ֖ה הִֽיא

**"for she was pleasing of appearance"**

If it would be helpful in your language, you could place this first in the verse because it is the reason that explains everything else that happens afterward in the verse.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 1:11
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:11 (#8) (id: 261944)

**"for she was pleasing of appearance"**

Alternate translation: “she was very beautiful”

See: Idiom

* **Associated Passages:** Esther 1:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Crown (ID: `realia:Crown`); Vashti (ID: `person:Vashti`)

## Esther 1:12 (#1) (id: 261945)

וַ⁠תְּמָאֵ֞ן הַ⁠מַּלְכָּ֣ה וַשְׁתִּ֗י לָ⁠בוֹא֙

**"But the queen Vashti refused to come"**

You can put the events in chronological order and say first that the eunuchs told Vashti what the king had commanded and then say that she refused to come\*\*.\*\*

See: Order of Events

* **Associated Passages:** Esther 1:12
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 1:12 (#2) (id: 261946)

וַ⁠תְּמָאֵ֞ן הַ⁠מַּלְכָּ֣ה וַשְׁתִּ֗י לָ⁠בוֹא֙

**"But the queen Vashti refused to come"**

If your language needs to give a reason why she refused, it is most likely because she did not want a group of drunken men staring at her lustfully.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 1:12 (#3) (id: 261947)

בִּ⁠דְבַ֣ר הַ⁠מֶּ֔לֶךְ

**"at the word of the king"**

Alternate translation: “at the king’s command” or “what the king wanted”

* **Associated Passages:** Esther 1:12

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 1:12 (#4) (id: 261948)

בְּ⁠יַ֣ד הַ⁠סָּרִיסִ֑ים

**"by the hand of the eunuchs"**

Here, **hand** refers to the eunuchs themselves, viewed through their action of telling Vashti what the king had commanded. Alternation translation: “when those servants told Queen Vashti what the king wanted”

See: Metonymy

* **Associated Passages:** Esther 1:12
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 1:12 (#5) (id: 261949)

**"the eunuchs"**

See how you translated this term in [1:10](https://ref.ly/Esth1:10).

See: Translate Unknowns

* **Associated Passages:** Esther 1:12
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 1:12 (#6) (id: 261950)

וַ⁠יִּקְצֹ֤ף הַ⁠מֶּ֨לֶךְ֙ מְאֹ֔ד וַ⁠חֲמָת֖⁠וֹ בָּעֲרָ֥ה בֽ⁠וֹ

**"Then the king became very angry, and his rage burned within him"**

These two phrases mean similar things. The repetition is used to emphasize the idea that they are expressing. You could combine them as UST does by saying, “The king became so angry that he could barely contain himself.”

See: Parallelism

* **Associated Passages:** Esther 1:12
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 1:12 (#7) (id: 261951)

וַ⁠חֲמָת֖⁠וֹ בָּעֲרָ֥ה בֽ⁠וֹ

**"and his rage burned within him"**

Here the story uses a metaphor that pictures the king’s anger as **a fire that burned inside of him**. If your language has a different word picture that it uses to describe extreme anger, you can use that here. If not, you can translate plainly, “his anger continued to increase.”

See: Metaphor

* **Associated Passages:** Esther 1:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 1:13 (#1) (id: 261952)

לַ⁠חֲכָמִ֖ים

**"to the wise men"**

This means the king’s advisors. You could call them “his advisors” or “the royal advisors.”

* **Associated Passages:** Esther 1:13


## Esther 1:13 (#2) (id: 261953)

יֹדְעֵ֣י הָֽ⁠עִתִּ֑ים

**"who were knowers of the times"**

This means something like “who knew the right way to do things” or “who understood the culture.”

* **Associated Passages:** Esther 1:13


## Esther 1:13 (#3) (id: 261954)

כִּי־כֵן֙ דְּבַ֣ר הַ⁠מֶּ֔לֶךְ

**"for thus was the manner of the king"**

This means that it was the king’s habit to consult his advisors on important questions. This is background information that explains why the king called upon these men.

See: Background Information

* **Associated Passages:** Esther 1:13
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)


## Esther 1:13 (#4) (id: 261955)

כִּי־כֵן֙ דְּבַ֣ר הַ⁠מֶּ֔לֶךְ

**"for thus was the manner of the king"**

If it would be helpful in your language, you could place this first in the verse to put the reason before the result because it explains what happens next.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 1:13
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)


## Esther 1:13 (#5) (id: 261956)

לִ⁠פְנֵ֕י

**"before the face of"**

Here, **face** refers to the presence of a person. The phrase means that King Ahasuerus would personally ask his wise men for advice in matters like these. Alternate translation: “to consult personally with”

See: Metonymy

* **Associated Passages:** Esther 1:13
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)


## Esther 1:13 (#6) (id: 261957)

יֹדְעֵ֖י דָּ֥ת וָ⁠דִֽין

**"those who were knowers of law and judgment"**

This means that these advisors knew the law and knew how to make good decisions in light of it.

* **Associated Passages:** Esther 1:13


## Esther 1:14 (#1) (id: 261958)

**"and the ones near to him were"**

Alternate translation: “the king’s closest advisors”

* **Associated Passages:** Esther 1:14

* **Associated ACAI Concepts:** Carshena (ID: `person:Carshena`); Admatha (ID: `person:Admatha`); Marsena (ID: `person:Marsena`); Shethar (ID: `person:Shethar`); Meres (ID: `person:Meres`); Memucan (ID: `person:Memucan`); Tarshish (ID: `place:Tarshish`); Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:14 (#2) (id: 261959)

כַּרְשְׁנָ֤א שֵׁתָר֙ אַדְמָ֣תָא תַרְשִׁ֔ישׁ מֶ֥רֶס מַרְסְנָ֖א מְמוּכָ֑ן

**"Karshena, Shethar, Admatha, Tarshish, Meres, Mersena, Memukan"**

These are the names of seven men.

See: How to Translate Names

* **Associated Passages:** Esther 1:14
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Carshena (ID: `person:Carshena`); Admatha (ID: `person:Admatha`); Marsena (ID: `person:Marsena`); Shethar (ID: `person:Shethar`); Meres (ID: `person:Meres`); Memucan (ID: `person:Memucan`); Tarshish (ID: `place:Tarshish`); Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:14 (#3) (id: 261960)

רֹאֵי֙ פְּנֵ֣י הַ⁠מֶּ֔לֶךְ

**"who were seers of the face of the king"**

**To see the face of** is an idiom that means to be in the presence of a person. The phrase means that King Ahasuerus would regularly and personally consult with these seven advisors.

See: Idiom

* **Associated Passages:** Esther 1:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Carshena (ID: `person:Carshena`); Admatha (ID: `person:Admatha`); Marsena (ID: `person:Marsena`); Shethar (ID: `person:Shethar`); Meres (ID: `person:Meres`); Memucan (ID: `person:Memucan`); Tarshish (ID: `place:Tarshish`); Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:14 (#4) (id: 261961)

**"the ones who sat first in the kingdom"**

Here, **sitting** represents ruling or judging. The expression here means that these were the most powerful officials in the empire.

See: Metaphor

* **Associated Passages:** Esther 1:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Carshena (ID: `person:Carshena`); Admatha (ID: `person:Admatha`); Marsena (ID: `person:Marsena`); Shethar (ID: `person:Shethar`); Meres (ID: `person:Meres`); Memucan (ID: `person:Memucan`); Tarshish (ID: `place:Tarshish`); Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:15 (#1) (id: 261962)

כְּ⁠דָת֙ מַֽה־לַּ⁠עֲשׂ֔וֹת בַּ⁠מַּלְכָּ֖ה וַשְׁתִּ֑י עַ֣ל ׀ אֲשֶׁ֣ר לֹֽא־עָשְׂתָ֗ה אֶֽת־מַאֲמַר֙ הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֔וֹשׁ בְּ⁠יַ֖ד הַ⁠סָּרִיסִֽים

**"According to law, what is to be done with the queen Vashti on account of this, that she did not perform the command of the king Ahasuerus by the hand of the eunuchs"**

This verse presents the question that the king asked the wise men. It may be helpful to indicate this by saying something like “The king asked them.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:15 (#2) (id: 261963)

כְּ⁠דָת֙ \& עַ֣ל ׀ אֲשֶׁ֣ר

**"According to law,"** \- **"on account of this, that"**

Alternate translation: “What does the law say we should do … because … ?”

* **Associated Passages:** Esther 1:15

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:15 (#3) (id: 261964)

בְּ⁠יַ֖ד הַ⁠סָּרִיסִֽים

**"by the hand of the eunuchs"**

To present the events in chronological order, you can begin with the information that these men brought the command. Alternate translation: “Queen Vashti did not do what I told my servants to command her to do”

See: Order of Events

* **Associated Passages:** Esther 1:15
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:15 (#4) (id: 261965)

בְּ⁠יַ֖ד הַ⁠סָּרִיסִֽים

**"by the hand of the eunuchs"**

Here, **hand** represents the action of doing or giving. The phrase means that the eunuchs were the ones who told Queen Vashti what King Ahasuerus had commanded her.

See: Metonymy

* **Associated Passages:** Esther 1:15
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:15 (#5) (id: 261966)

**"the eunuchs"**

See how you translated this term in [1:10](https://ref.ly/Esth1:10).

See: Translate Unknowns

* **Associated Passages:** Esther 1:15
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:16 (#1) (id: 261967)

וַ⁠יֹּ֣אמֶר מְמוּכָ֗ן לִ⁠פְנֵ֤י הַ⁠מֶּ֨לֶךְ֙

**"And Memukan replied before the face of the king"**

Here, **face** represents the presence of a person. The phrase means that Memukan was speaking in the presence of the king and of the other officials. Alternate translation: “then Memukan spoke so that both the king and the officials could hear”

See: Metonymy

* **Associated Passages:** Esther 1:16
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`); Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:16 (#2) (id: 261968)

כָּל־הָ֣⁠עַמִּ֔ים אֲשֶׁ֕ר בְּ⁠כָל־מְדִינ֖וֹת הַ⁠מֶּ֥לֶךְ

**"all the peoples who are in all the provinces of the king"**

This means all the different people groups that were living in the empire. You could say, “all the people groups in all the provinces that King Ahasuerus rules” or more generally, “every person who lives in the entire empire of King Ahasuerus.”

See: Hyperbole

* **Associated Passages:** Esther 1:16
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`); Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:16 (#3) (id: 261969)

**"Memukan"**

See how you translated this man’s name in [1:14](https://ref.ly/Esth1:14).

See: How to Translate Names

* **Associated Passages:** Esther 1:16
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`); Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:16 (#4) (id: 261970)

הַ⁠מֶּ֥לֶךְ אֲחַשְׁוֵרֽוֹשׁ

**"the king Ahasuerus"**

Memukan speaks of **the king** in third person as a form of respect. If you want to portray him as speaking primarily to the king because he is answering the king’s question, you could have him say, “in all the provinces that you rule” or “every person who lives in your entire empire.”

See: First, Second or Third Person

* **Associated Passages:** Esther 1:16
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`); Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:17 (#1) (id: 261971)

כִּֽי

**"For"**

This introduces the reason why Memukan says that Queen Vashti has done wrong against all the men in the kingdom and not only against King Ahasuerus. To show that, you can begin with “This is what will happen.”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 1:17
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:17 (#2) (id: 261972)

יֵצֵ֤א דְבַר־הַ⁠מַּלְכָּה֙ עַל־כָּל־הַ⁠נָּשִׁ֔ים

**"the matter of the queen will go out to all the women"**

To emphasize his point, Memukan exaggerates and says that every single woman in the empire will hear about Queen Vashti refusing to obey King Ahasuerus. You could say, as UST does, that “women all over the empire” will hear about what the queen did. Or you could preserve Memukan’s manner of speaking by saying, “every woman” will hear.

See: Hyperbole

* **Associated Passages:** Esther 1:17
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:17 (#3) (id: 261973)

לְ⁠הַבְז֥וֹת בַּעְלֵי⁠הֶ֖ן בְּ⁠עֵינֵי⁠הֶ֑ן

**"in order to make their husbands despised in their eyes"**

Here, **eyes** stand for seeing, and seeing means knowledge, notice, attention, or judgment. The phrase means that the women will treat their husbands with disrespect and not obey them. Alternate translation: “women will despise their husbands”

See: Metaphor

* **Associated Passages:** Esther 1:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:17 (#4) (id: 261974)

בְּ⁠אָמְרָ֗⁠ם הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֡וֹשׁ אָמַ֞ר לְ⁠הָבִ֨יא אֶת־וַשְׁתִּ֧י הַ⁠מַּלְכָּ֛ה לְ⁠פָנָ֖י⁠ו וְ⁠לֹא־בָֽאָה

**"when they say, ‘The king Ahasuerus said to bring Vashti the queen before his face, but she did not come"**

The implication is that they will go on to say, “So if even the queen can disobey the king, why should I have to obey my husband?” If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:17 (#5) (id: 261975)

הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֡וֹשׁ אָמַ֞ר לְ⁠הָבִ֨יא אֶת־וַשְׁתִּ֧י הַ⁠מַּלְכָּ֛ה לְ⁠פָנָ֖י⁠ו וְ⁠לֹא־בָֽאָה

**"The king Ahasuerus said to bring Vashti the queen before his face, but she did not come"**

The women will stop respecting their husbands after they hear about this. You can put this report about what Queen Vashti did first, before “women will stop respecting their husbands,” because it is the reason that explains that result.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 1:17
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:17 (#6) (id: 261976)

לְ⁠פָנָ֖י⁠ו

**"before his face"**

Here, **face** means the presence of a person. The phrase means the Queen Vashti refused to come into the presence of King Ahasuerus when he summoned her during the feast. See how you translated this in verse 11\.

See: Metonymy

* **Associated Passages:** Esther 1:17
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:18 (#1) (id: 261977)

וְֽ⁠הַ⁠יּ֨וֹם הַ⁠זֶּ֜ה

**"Now this day"**

Alternate translation: “even today” or “even this very day”

* **Associated Passages:** Esther 1:18

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:18 (#2) (id: 261978)

שָׂר֣וֹת פָּֽרַס־וּ⁠מָדַ֗י

**"the noblewomen of Persia and Media"**

It becomes clear later in the verse that Memukan is speaking of the wives of the king’s officials, and you could communicate that here by saying, “the wives of the officials who govern Persia and Media.” But the term “noblewomen” indicates that they also had high status of their own, so you could also call them “the leading women of Persia and Media.”

* **Associated Passages:** Esther 1:18

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:18 (#3) (id: 261979)

אֶת־דְּבַ֣ר הַ⁠מַּלְכָּ֔ה

**"the matter of the queen"**

Alternate translation: “what the queen has done”

* **Associated Passages:** Esther 1:18

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:18 (#4) (id: 261980)

תֹּאמַ֣רְנָה

**"will speak"**

This means that they will “talk back” or “disobey.” Alternate translation: “refuse to obey”

* **Associated Passages:** Esther 1:18

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:18 (#5) (id: 261981)

לְ⁠כֹ֖ל שָׂרֵ֣י הַ⁠מֶּ֑לֶךְ

**"to all the officials of the king"**

This means the husbands of the noblewomen. The implication may be, “They will disobey their husbands even though they are officials of the king.” You could say that explicitly, as UST does.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:18 (#6) (id: 261982)

וּ⁠כְ⁠דַ֖י בִּזָּי֥וֹן וָ⁠קָֽצֶף

**"And there will be contempt and wrath enough"**

This could mean either that they will treat their husbands with disrespect, and this will make their “husbands angry with them” or that “they will be angry with their husbands and treat them with contempt.”

* **Associated Passages:** Esther 1:18

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:18 (#7) (id: 261983)

וּ⁠כְ⁠דַ֖י

**"And there will be"** \- **"enough"**

This could mean that the husbands will be as furious “as much as they can bear.” But you could also say, “That will be bad enough by itself, even if the news does not spread any farther.” If you think that is the meaning, you can say that explicitly, as UST does.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`)

## Esther 1:19 (#1) (id: 261984)

אִם־עַל־הַ⁠מֶּ֣לֶךְ ט֗וֹב

**"If it is good to the king"**

This is an idiom that means “if the king thinks this is a good idea” or “if this advice is pleasing to the king.”

See: Idiom

* **Associated Passages:** Esther 1:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#2) (id: 261985)

אִם־עַל־הַ⁠מֶּ֣לֶךְ ט֗וֹב

**"If it is good to the king"**

Memukan speaks to the king in third person as a form of respect. Alternate translation: “If it pleases you”

See: First, Second or Third Person

* **Associated Passages:** Esther 1:19
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#3) (id: 261986)

יֵצֵ֤א דְבַר־מַלְכוּת֙ מִ⁠לְּ⁠פָנָ֔י⁠ו

**"let a decree of royalty go out from before his face"**

Here, **face** represents the presence of a person. The phrase means that the decree will come from the king himself. Alternate translation: “let a decree go out in your name” or “let a decree go out with your authority”

See: Metonymy

* **Associated Passages:** Esther 1:19
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#4) (id: 261987)

דְבַר־מַלְכוּת֙

**"a decree of royalty"**

You can translate the abstract noun **royalty** with the adjective “royal.” Alternate translation: “royal decree”

See: Abstract Nouns

* **Associated Passages:** Esther 1:19
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#5) (id: 261988)

מִ⁠לְּ⁠פָנָ֔י⁠ו

**"from before his face"**

Memukan speaks to the king in third person as a form of respect. Alternate translation: “personally”

See: First, Second or Third Person

* **Associated Passages:** Esther 1:19
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#6) (id: 261989)

וְ⁠יִכָּתֵ֛ב

**"and let it be written"**

You can say this with an active form, and you can say who will do the action. Alternate translation: “you should write a decree yourself” or “command your scribes to write a decree”

See: Active or Passive

* **Associated Passages:** Esther 1:19
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#7) (id: 261990)

וְ⁠לֹ֣א יַעֲב֑וֹר

**"which do not pass away"**

Alternate translation: “these laws never become invalid” or “can never be changed”

* **Associated Passages:** Esther 1:19

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#8) (id: 261991)

תָב֜וֹא \& לִ⁠פְנֵי֙ הַ⁠מֶּ֣לֶךְ

**"shall"** \- **"come before the face of the king"**

Memukan speaks to the king in third person as a form of respect. Alternate translation: “never come into your presence again”

See: First, Second or Third Person

* **Associated Passages:** Esther 1:19
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#9) (id: 261992)

תָב֜וֹא \& לִ⁠פְנֵי֙ הַ⁠מֶּ֣לֶךְ

**"shall"** \- **"come before the face of the king"**

Here, **face** represents the presence of a person. The phrase means that Queen Vashti will never again come into the king’s presence. Alternate translation: “never again come before King Ahasuerus”

See: Metonymy

* **Associated Passages:** Esther 1:19
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#10) (id: 261993)

וּ⁠מַלְכוּתָ⁠הּ֙ יִתֵּ֣ן הַ⁠מֶּ֔לֶךְ לִ⁠רְעוּתָ֖⁠הּ הַ⁠טּוֹבָ֥ה מִמֶּֽ⁠נָּה

**"And let the king give her royalty to her female neighbor, the woman who is better than she"**

Memukan speaks to the king in third person as a form of respect. You can show the same meaning with a verb that addresses a singular “you” and indicates that Memukan is offering advice, not giving a command when he say, “choose another woman to be queen.” Alternate translation: “the king can give her position as queen to some other woman”

See: First, Second or Third Person

* **Associated Passages:** Esther 1:19
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#11) (id: 261994)

וּ⁠מַלְכוּתָ⁠הּ֙

**"And"** \- **"her royalty"**

You can translate the abstract noun **royalty** with an expression such as “her royal position” or “her position as queen.”

See: Abstract Nouns

* **Associated Passages:** Esther 1:19
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:19 (#12) (id: 261995)

הַ⁠טּוֹבָ֥ה מִמֶּֽ⁠נָּה

**"the woman who is better than she"**

Memukan means that the next queen should be “better than” Vashti by obeying all of the king’s commands. You could say this explicitly, as UST does.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Medes (ID: `group:Mede`); Persians (ID: `group:Persia`); Persia (ID: `place:Persia`); Medan (ID: `person:Medan`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 1:20 (#1) (id: 261996)

וְ⁠נִשְׁמַע֩ פִּתְגָ֨ם הַ⁠מֶּ֤לֶךְ

**"And the decree of the king will be heard"**

You can say this with an active form, and you can say who will do the action. Alternate translation: “when everyone … hears”

See: Active or Passive

* **Associated Passages:** Esther 1:20
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Esther 1:20 (#2) (id: 261997)

פִּתְגָ֨ם הַ⁠מֶּ֤לֶךְ

**"the decree of the king"**

Memukan speaks to the king in third person as a form of respect. You could express the same meaning in the second person: “When they hear what you have commanded.” Alternate translation: “when they hear the king’s decree”

See: First, Second or Third Person

* **Associated Passages:** Esther 1:20
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Esther 1:20 (#3) (id: 261998)

וְ⁠כָל־הַ⁠נָּשִׁ֗ים יִתְּנ֤וּ יְקָר֙ לְ⁠בַעְלֵי⁠הֶ֔ן

**"that he will make for all his kingdom"**

Even though the decree of Ahasuerus applied only to Vashti directly, it was published throughout the empire so that everyone would know about it. Thus, what happened to the queen would discourage women from imitating her. If you need to clarify this in your language, you can explain it in a note: “The women will honor their husbands because they will know that if a woman despises her husband, she can be punished like Vashti.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:20
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Esther 1:20 (#4) (id: 261999)

מַלְכוּת֔⁠וֹ

**"his kingdom"**

Memukan speaks to the king in third person as a form of respect. Alternate translation: “for all your kingdom”

See: First, Second or Third Person

* **Associated Passages:** Esther 1:20
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Esther 1:20 (#5) (id: 262000)

כִּ֥י רַבָּ֖ה הִ֑יא

**"though it is great"**

Alternate translation: “even though your empire is very large”

* **Associated Passages:** Esther 1:20


## Esther 1:20 (#6) (id: 262001)

הַ⁠נָּשִׁ֗ים יִתְּנ֤וּ יְקָר֙ לְ⁠בַעְלֵי⁠הֶ֔ן

**"the women will give honor to their husbands"**

Alternate translation: “women will respect and obey their husbands”

* **Associated Passages:** Esther 1:20


## Esther 1:20 (#7) (id: 262002)

לְ⁠מִ⁠גָּד֖וֹל וְ⁠עַד־קָטָֽן

**"from the greatest to the least"**

This expression refers to all of the husbands in the empire. By naming both extremes, it includes everyone in between. You could just give the meaning, as UST does: “every husband in the empire.” Alternate translation: “this will be true for both the most important man and the least important man in the empire, and everyone in between”

See: Merism

* **Associated Passages:** Esther 1:20
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)


## Esther 1:21 (#1) (id: 262003)

וַ⁠יִּיטַב֙ הַ⁠דָּבָ֔ר בְּ⁠עֵינֵ֥י הַ⁠מֶּ֖לֶךְ

**"And the word seemed good in the eyes of the king"**

Like “if it is good to the king” in verse 19, this is an idiom that means the king thought this was a good idea or this advice pleased the king.

See: Idiom

* **Associated Passages:** Esther 1:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`)

## Esther 1:21 (#2) (id: 262004)

בְּ⁠עֵינֵ֥י הַ⁠מֶּ֖לֶךְ וְ⁠הַ⁠שָּׂרִ֑ים

**"in the eyes of the king and the officials"**

Here, **eyes** stand for seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. The phrase means that King Ahasuerus and all the officials who heard Memukan’s advice thought that his suggestion was a good idea.

See: Metaphor

* **Associated Passages:** Esther 1:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`)

## Esther 1:21 (#3) (id: 262005)

וַ⁠יַּ֥עַשׂ הַ⁠מֶּ֖לֶךְ כִּ⁠דְבַ֥ר מְמוּכָֽן

**"And the king acted according to the word of Memukan"**

This means that King Ahasuerus wrote a law proclaiming what Memukan had suggested.

* **Associated Passages:** Esther 1:21

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`)

## Esther 1:21 (#4) (id: 262006)

**"Memukan"**

Translate his name as in [1:14](https://ref.ly/Esth1:14).

See: How to Translate Names

* **Associated Passages:** Esther 1:21
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Memucan (ID: `person:Memucan`)

## Esther 1:22 (#1) (id: 262007)

וַ⁠יִּשְׁלַ֤ח סְפָרִים֙ אֶל־כָּל־מְדִינ֣וֹת הַ⁠מֶּ֔לֶךְ

**"And he sent letters to all the provinces of the king"**

**He** means King Ahasuerus. This is saying that the king sent letters to every province in his empire.

* **Associated Passages:** Esther 1:22

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 1:22 (#2) (id: 262008)

מְדִינָ֤ה וּ⁠מְדִינָה֙

**"province by province"**

This is an idiom that means “every province” or every single province.

See: Idiom

* **Associated Passages:** Esther 1:22
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 1:22 (#3) (id: 262009)

כִּ⁠כְתָבָ֔⁠הּ

**"according to its own writing"**

Alternate translation: “using its own alphabet” or “written in its own script”

* **Associated Passages:** Esther 1:22

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 1:22 (#4) (id: 262010)

עַ֥ם וָ⁠עָ֖ם

**"people by people"**

This is an idiom that means “every people group” or every single people group.

See: Idiom

* **Associated Passages:** Esther 1:22
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 1:22 (#5) (id: 262011)

כִּ⁠לְשׁוֹנ֑⁠וֹ

**"according to its own tongue"**

Here, **tongue** means the language spoken by a person or a group of people. Alternate translation: “in its own language”

See: Metonymy

* **Associated Passages:** Esther 1:22
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 1:22 (#6) (id: 262012)

לִ⁠הְי֤וֹת כָּל־אִישׁ֙ שֹׂרֵ֣ר בְּ⁠בֵית֔⁠וֹ

**"that every man should be ruling in his house"**

This meant that all men should have complete authority over their wives and their children.

* **Associated Passages:** Esther 1:22

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 1:22 (#7) (id: 262013)

כִּ⁠לְשׁ֥וֹן עַמּֽ⁠וֹ

**"according to the tongue of his people"**

Here, **tongue** means the language spoken by a person or a group of people.

See: Metonymy

* **Associated Passages:** Esther 1:22
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 1:22 (#8) (id: 262014)

כִּ⁠לְשׁ֥וֹן עַמּֽ⁠וֹ

**"according to the tongue of his people"**

The implication is that the husband ought to be able to give orders to his wife in his own native language and that she should understand and obey him, even if she had to learn his language to do so. If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 1:22
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 2:1 (#1) (id: 262015)

אַחַר֙ הַ⁠דְּבָרִ֣ים הָ⁠אֵ֔לֶּה

**"After these things"**

This introduces a new event that happened some time later, but we do not know how much later. You can show this with a phrase like “After some time had passed.”

See: Introduction of a New Event

* **Associated Passages:** Esther 2:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 2:1 (#2) (id: 262016)

כְּ⁠שֹׁ֕ךְ חֲמַ֖ת הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֑וֹשׁ

**"when the rage of the king Ahasuerus subsided"**

Alternate translation: “when King Ahasuerus was not angry any more” or “when King Ahasuerus became less angry”

* **Associated Passages:** Esther 2:1

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 2:1 (#3) (id: 262017)

זָכַ֤ר אֶת־וַשְׁתִּי֙

**"he remembered Vashti"**

This means that he started to think about Vashti. He probably thought about the things that he liked about her, meaning that he missed her.

* **Associated Passages:** Esther 2:1

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 2:1 (#4) (id: 262018)

אֲשֶׁר־עָשָׂ֔תָה וְ⁠אֵ֥ת אֲשֶׁר־נִגְזַ֖ר עָלֶֽי⁠הָ

**"what she had done, and what had been decided concerning her"**

This refers to Vashti’s refusal to obey Ahasuerus, and to the decision he made in [1:21](https://ref.ly/Esth1:21) that she could never come into his presence again. This could actually mean that when he thought about Vashti and what she had done, he regretted the decision that he had made about her. But there was nothing he could do to bring her back because the laws of the empire could not be changed. You could explain all or some of this in your translation, as in the UST.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 2:1 (#5) (id: 262019)

אֲשֶׁר־נִגְזַ֖ר עָלֶֽי⁠הָ

**"what had been decided concerning her"**

You can say this with an active form, and you can say that the king performed the action. Alternate translation: “what he had decided”

See: Active or Passive

* **Associated Passages:** Esther 2:1
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 2:2 (#1) (id: 262020)

נַעֲרֵֽי־הַ⁠מֶּ֖לֶךְ מְשָׁרְתָ֑י⁠ו

**"the young men of the king who served him"**

These were not the older, wiser royal advisors whom Ahasuerus consulted in [1:13](https://ref.ly/Esth1:13), but younger men who stood nearby the king ready to help with any practical needs. You could call them his personal servants.

See: Translate Unknowns

* **Associated Passages:** Esther 2:2
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)


## Esther 2:2 (#2) (id: 262021)

יְבַקְשׁ֥וּ לַ⁠מֶּ֛לֶךְ נְעָר֥וֹת

**"Let them seek for the king young women"**

Implicit in this suggestion is the idea, “You should get a new wife for yourself.” You can say that explicitly at the beginning of the advice that the young men gave to the king.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Esther 2:2 (#3) (id: 262022)

יְבַקְשׁ֥וּ

**"Let them seek"**

**Them** refers to the servants that the king would send to do this. If this is unclear, you could say “servants” or “your servants.” Be sure to express this in a form that shows that it is a not a command but a respectful suggestion such as “You could send some men.” Alternate translation: “to look for”

See: Introduction of New and Old Participants

* **Associated Passages:** Esther 2:2
* **Associated Articles:** Introduction of New and Old Participants (ID: `33421@UWTranslationManual`)


## Esther 2:2 (#4) (id: 262023)

לַ⁠מֶּ֛לֶךְ

**"for the king"**

The young men speak to the king in the third person as a sign of respect. Alternatively, you could have them say “for yourself” or “on your behalf” if that would be clear but also respectful in your language

See: First, Second or Third Person

* **Associated Passages:** Esther 2:2
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Esther 2:2 (#5) (id: 262024)

**"and pleasing of appearance"**

As in 1:11, this is an idiom that means very beautiful.

See: Idiom

* **Associated Passages:** Esther 2:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Esther 2:3 (#1) (id: 262025)

וְ⁠יַפְקֵ֨ד הַ⁠מֶּ֣לֶךְ

**"And let the king appoint"**

The young men speak to the king in third person as a sign of respect. Alternatively, you could have them say “you could appoint” in a respectful way

See: First, Second or Third Person

* **Associated Passages:** Esther 2:3
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#2) (id: 262026)

**"and pleasing of appearance"**

As in 1:11, this is an idiom that means very beautiful.

See: Idiom

* **Associated Passages:** Esther 2:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#3) (id: 262027)

שׁוּשַׁ֤ן הַ⁠בִּירָה֙

**"Susa the citadel"**

In this context, this expression probably means “to the palace here in Susa.”

See: Translate Unknowns

* **Associated Passages:** Esther 2:3
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#4) (id: 262028)

**"the house of women"**

This means the harem for virgins, the place where the young women would live and receive beauty treatments before they were taken to the king (See: verses 13 and 14\).

* **Associated Passages:** Esther 2:3

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#5) (id: 262029)

**"into the hand of Hegai (the eunuch of the king who is overseer of the women"**

If your language does not use the passive form in this way, you can state this in active form or in another way that is natural in your language. Alternate translation: “let Hegai, the king’s official, who is in charge of the women, take care of them”

See: Active or Passive

* **Associated Passages:** Esther 2:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#6) (id: 262030)

אֶל־יַ֥ד הֵגֶ֛א

**"the hand of Hegai"**

Here, **hand** is a metaphor meaning power, control, or authority. As the overseer of the women, Hegai was responsible for the young women who lived in the harem for virgins. Alternate translation: “under the custody of Hegai”

See: Metaphor

* **Associated Passages:** Esther 2:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#7) (id: 262031)

הֵגֶ֛א

**"the"** \- **"Hegai"**

This is a man’s name. It occurs several times in this chapter. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 2:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#8) (id: 262032)

סְרִ֥יס הַ⁠מֶּ֖לֶךְ

**"the eunuch of the king"**

The servants spoke to the king in third person as a sign of respect. Alternate translation: “your eunuch”

See: First, Second or Third Person

* **Associated Passages:** Esther 2:3
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#9) (id: 262033)

סְרִ֥יס

**"the eunuch of"**

See how you translated this term in 1:10\. Alternate translation: “the official of” or “the administrator of”

See: Translate Unknowns

* **Associated Passages:** Esther 2:3
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#10) (id: 262034)

שֹׁמֵ֣ר הַ⁠נָּשִׁ֑ים

**"who is overseer of the women"**

Alternate translation: “who is in charge of the women who live there.”

* **Associated Passages:** Esther 2:3

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:3 (#11) (id: 262035)

תַּמְרוּקֵי⁠הֶֽן

**"their ointments"**

This term seems to refer to a specific substance that women would put on their face or other parts of their body to make themselves more beautiful. From verse 12, it appears to mean lotions that were used along with oils and perfumes. But here, this one aspect of a woman’s beauty treatment seems to be used to refer to all of it, so you could call these “beauty treatments.”

See: Translate Unknowns

* **Associated Passages:** Esther 2:3
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Appoint (ID: `keyterm:Appoint`); House (ID: `realia:House`)

## Esther 2:4 (#1) (id: 262036)

וְ⁠הַֽ⁠נַּעֲרָ֗ה אֲשֶׁ֤ר תִּיטַב֙ בְּ⁠עֵינֵ֣י הַ⁠מֶּ֔לֶךְ

**"And"** \- **"the young woman who is pleasing in the eyes of the king"**

The servants spoke to the king in third person as a sign of respect. You could express the same meaning in the second person by saying something such as “the young woman you like the best.”

See: First, Second or Third Person

* **Associated Passages:** Esther 2:4
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 2:4 (#2) (id: 262037)

בְּ⁠עֵינֵ֣י

**"in the eyes of"**

Here, **eyes** represent seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. The phrase means that King Ahasuerus could decide which young woman he liked the best and make her queen.

See: Metaphor

* **Associated Passages:** Esther 2:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 2:4 (#3) (id: 262038)

וַ⁠יִּיטַ֧ב הַ⁠דָּבָ֛ר בְּ⁠עֵינֵ֥י הַ⁠מֶּ֖לֶךְ

**"And the word was pleasing in the eyes of the king"**

Here, **eyes** have the same figurative meaning. The phrase indicates that King Ahasuerus thought that this suggestion was a good idea. Alternate translation: “The king found their suggestion appealing”

See: Metaphor

* **Associated Passages:** Esther 2:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 2:4 (#4) (id: 262039)

וַ⁠יַּ֥עַשׂ כֵּֽן

**"so he did thus"**

Alternate translation: “so he followed the suggestion”

* **Associated Passages:** Esther 2:4

* **Associated ACAI Concepts:** Vashti (ID: `person:Vashti`)

## Esther 2:5 (#1) (id: 262040)

אִ֣ישׁ יְהוּדִ֔י

**"A man, a Jew"**

This introduces Mordecai as a new character in the story. Use your language’s way of introducing a new character. This expression means a Jewish man.

See: Introduction of New and Old Participants

* **Associated Passages:** Esther 2:5
* **Associated Articles:** Introduction of New and Old Participants (ID: `33421@UWTranslationManual`)

* **Associated ACAI Concepts:** Kish (ID: `person:Kish.4`); Mordecai (ID: `person:Mordecai.2`); Jair (ID: `person:Jair.4`); Shimei (ID: `person:Shimei.18`); Tribe of Benjamin (ID: `group:Benjamin`); Benjamin (ID: `person:Benjamin`); Jews (ID: `group:Jews`); Susa (ID: `place:Susa`)

## Esther 2:5 (#2) (id: 262041)

בְּ⁠שׁוּשַׁ֣ן הַ⁠בִּירָ֑ה

**"in Susa the citadel"**

Here the phrase likely indicates the city rather than the palace, that is, the capital city of Susa.

See: Translate Unknowns

* **Associated Passages:** Esther 2:5
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Kish (ID: `person:Kish.4`); Mordecai (ID: `person:Mordecai.2`); Jair (ID: `person:Jair.4`); Shimei (ID: `person:Shimei.18`); Tribe of Benjamin (ID: `group:Benjamin`); Benjamin (ID: `person:Benjamin`); Jews (ID: `group:Jews`); Susa (ID: `place:Susa`)

## Esther 2:5 (#3) (id: 262042)

מָרְדֳּכַ֗י

**"Mordecai"**

This is a man’s name. It occurs many times throughout the story. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 2:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Kish (ID: `person:Kish.4`); Mordecai (ID: `person:Mordecai.2`); Jair (ID: `person:Jair.4`); Shimei (ID: `person:Shimei.18`); Tribe of Benjamin (ID: `group:Benjamin`); Benjamin (ID: `person:Benjamin`); Jews (ID: `group:Jews`); Susa (ID: `place:Susa`)

## Esther 2:5 (#4) (id: 262043)

בֶּ֣ן יָאִ֧יר בֶּן־שִׁמְעִ֛י בֶּן־קִ֖ישׁ

**"the son of Jair, the son of Shimei, the son of Kish"**

These are men’s names. Jair, Shimei, and Kish are Mordecai’s male ancestors. You could say, “Mordecai was the son of Jair, who was the son of Shimei, who was the son of Kish,” or see the UST.

See: How to Translate Names

* **Associated Passages:** Esther 2:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Kish (ID: `person:Kish.4`); Mordecai (ID: `person:Mordecai.2`); Jair (ID: `person:Jair.4`); Shimei (ID: `person:Shimei.18`); Tribe of Benjamin (ID: `group:Benjamin`); Benjamin (ID: `person:Benjamin`); Jews (ID: `group:Jews`); Susa (ID: `place:Susa`)

## Esther 2:5 (#5) (id: 262044)

אִ֥ישׁ יְמִינִֽי

**"a man, a Benjamite"**

While this likely refers to Kish, it is necessarily also true of Mordecai, and you could put it second in the verse. That way Mordecai’s nationality would come first, then his tribe, and then his family and clan. Alternate translation: “from the tribe of Benjamin”

* **Associated Passages:** Esther 2:5

* **Associated ACAI Concepts:** Kish (ID: `person:Kish.4`); Mordecai (ID: `person:Mordecai.2`); Jair (ID: `person:Jair.4`); Shimei (ID: `person:Shimei.18`); Tribe of Benjamin (ID: `group:Benjamin`); Benjamin (ID: `person:Benjamin`); Jews (ID: `group:Jews`); Susa (ID: `place:Susa`)

## Esther 2:6 (#1) (id: 262045)

אֲשֶׁ֤ר הָגְלָה֙ מִ⁠יר֣וּשָׁלַ֔יִם עִם־הַ⁠גֹּלָה֙ אֲשֶׁ֣ר הָגְלְתָ֔ה עִ֖ם יְכָנְיָ֣ה מֶֽלֶךְ־יְהוּדָ֑ה אֲשֶׁ֣ר הֶגְלָ֔ה נְבוּכַדְנֶאצַּ֖ר מֶ֥לֶךְ בָּבֶֽל

**"who had been exiled from Jerusalem with the exiles who had been exiled along with Jeconiah, the king of Judah, who Nebuchadnezzar the king of Babylon had exiled"**

The story now tells about something that happened many years before. This is background information that explains how this Jewish family came to live in Susa. You can introduce this by saying something like, “Many years earlier.”

See: Background Information

* **Associated Passages:** Esther 2:6
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Jehoiachin (ID: `person:Jehoiachin`)

## Esther 2:6 (#2) (id: 262046)

אֲשֶׁ֤ר הָגְלָה֙ מִ⁠יר֣וּשָׁלַ֔יִם

**"who had been exiled from Jerusalem"**

If it would be helpful in your language, you could say this with an active form, and you could say who did the action. Alternate translation: “King Nebuchadnezzar of Babylon had taken Kish away from Jerusalem along with some other captives”

See: Active or Passive

* **Associated Passages:** Esther 2:6
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Jehoiachin (ID: `person:Jehoiachin`)

## Esther 2:6 (#3) (id: 262047)

אֲשֶׁ֤ר הָגְלָה֙ מִ⁠יר֣וּשָׁלַ֔יִם

**"who had been exiled from Jerusalem"**

You can bring this information forward from later in the verse because it provides background information that helps identify Mordecai.

See: Background Information

* **Associated Passages:** Esther 2:6
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Jehoiachin (ID: `person:Jehoiachin`)

## Esther 2:6 (#4) (id: 262048)

אֲשֶׁ֤ר הָגְלָה֙ מִ⁠יר֣וּשָׁלַ֔יִם

**"who had been exiled from Jerusalem"**

If it would be helpful in your language, you could say where King Nebuchadnezzar brought the people that he took away from Jerusalem. Alternate translation: “had taken Kish away from Jerusalem … and brought him to Babylon.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Jehoiachin (ID: `person:Jehoiachin`)

## Esther 2:6 (#5) (id: 262049)

עִ֖ם

**"along with"**

This means that the two things happened at the same time. You can indicate this with a phrase like “at the same time that he took.”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Esther 2:6
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Jehoiachin (ID: `person:Jehoiachin`)

## Esther 2:6 (#6) (id: 262050)

יְכָנְיָ֣ה מֶֽלֶךְ־יְהוּדָ֑ה

**"Jeconiah, the king of Judah"**

In the historical passage where this event is described (2 Kings 24:8–17\), this king is called Jehoiachin. That was another name by which he was known. You can call him Jehoiachin here in Esther if you think that would help your readers recognize him better.

See: How to Translate Names

* **Associated Passages:** Esther 2:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Jehoiachin (ID: `person:Jehoiachin`)

## Esther 2:6 (#7) (id: 262051)

אֲשֶׁ֣ר הֶגְלָ֔ה נְבוּכַדְנֶאצַּ֖ר מֶ֥לֶךְ בָּבֶֽל

**"who Nebuchadnezzar the king of Babylon had exiled"**

You can say where King Nebuchadnezzar brought this king when he took him away from Jerusalem. For example, you can say, “Nebuchadnezzar … took King Jehoiachin of Judah away from Jerusalem and brought him to Babylon.”

See: How to Translate Names

* **Associated Passages:** Esther 2:6
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Nebuchadnezzar (ID: `person:Nebuchadnezzar`); Judah (ID: `person:Judah.4`); Babylon (ID: `place:Babylon`); Tribe of Judah (ID: `group:Judah`); Jerusalem (ID: `place:Jerusalem`); Jehoiachin (ID: `person:Jehoiachin`)

## Esther 2:7 (#1) (id: 262052)

וַ⁠יְהִ֨י

**"And he was"**

The story now returns to talking about Mordecai. If it is helpful for your readers, you can specify his name, saying, “Now, Mordecai.”

* **Associated Passages:** Esther 2:7

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:7 (#2) (id: 262053)

בַּת־דֹּד֔⁠וֹ

**"the daughter of his uncle"**

If your language has a specific term for this relationship, you can use it. For example, you can say, “his cousin.” Alternate translation: “his uncle’s daughter”

* **Associated Passages:** Esther 2:7

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:7 (#3) (id: 262054)

אֶת־הֲדַסָּ֗ה הִ֤יא אֶסְתֵּר֙

**"Hadassah (she is Esther"**

Here the story is indicating that this woman’s Hebrew name was Hadassah and her Persian name was Esther. You could say that explicitly.

See: How to Translate Names

* **Associated Passages:** Esther 2:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:7 (#4) (id: 262055)

אֶסְתֵּר֙

**"Esther"**

This is a woman’s name. It occurs many times throughout the story. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 2:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:7 (#5) (id: 262056)

אֵ֥ין לָ֖⁠הּ אָ֣ב וָ⁠אֵ֑ם

**"there was not for her a father or a mother"**

You could say simply that “she was an orphan,” as UST does. Alternate translation: “both her father and mother had died”

* **Associated Passages:** Esther 2:7

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:7 (#6) (id: 262057)

וְ⁠הַ⁠נַּעֲרָ֤ה יְפַת־תֹּ֨אַר֙ וְ⁠טוֹבַ֣ת מַרְאֶ֔ה \& אָבִ֨י⁠הָ֙

**"Now the young woman was beautiful of form and pleasing of appearance."** \- **"her"**

These two phrases mean something similar. Separately, they could be saying that Esther was both sexually mature and very attractive. But taken together, they mean that, overall, she was very pleasant to look at. You could combine them, as UST does, and say that she was “exceptionally attractive” Alternate translation: “she had a lovely body and a beautiful face”

See: Parallelism

* **Associated Passages:** Esther 2:7
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:7 (#7) (id: 262058)

וּ⁠בְ⁠מ֤וֹת אָבִ֨י⁠הָ֙ וְ⁠אִמָּ֔⁠הּ לְקָחָ֧⁠הּ מָרְדֳּכַ֛י ל֖⁠וֹ לְ⁠בַֽת

**"And at the death of her father and her mother, Mordecai had taken"** \- **"for him as a daughter"**

This is background information explaining the relationship between Mordecai and Esther. You can move it to right after the place where Esther is introduced by name because it describes what happened in the past, before Esther became a beautiful young woman. Alternate translation: “after her father and her mother had died, Mordecai had adopted her as his own daughter”

See: Background Information

* **Associated Passages:** Esther 2:7
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:8 (#1) (id: 262059)

וַ⁠יְהִ֗י

**"And it happened"**

After providing this background information about Mordecai and Esther in verses 5–7, the story now tells what happened as a result of these facts, when the provincial officers described in verse 3 began sending young women to the harem. That is, the story tells what happened to Esther because her family was living in the Persian empire and she was very attractive. Use your language’s way of showing that the events are now moving forward after giving background information. Alternate translation: “and so this is what happened”

See: Introduction of a New Event

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#2) (id: 262060)

בְּ⁠הִשָּׁמַ֤ע דְּבַר־הַ⁠מֶּ֨לֶךְ֙ וְ⁠דָת֔⁠וֹ וּֽ⁠בְ⁠הִקָּבֵ֞ץ נְעָר֥וֹת רַבּ֛וֹת

**"when the decree of the king and his law were heard, and when many young women were gathered"**

This indicates that two actions took place at the same time. You should indicate this with a phrase like “at the same time.”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#3) (id: 262061)

בְּ⁠הִשָּׁמַ֤ע דְּבַר־הַ⁠מֶּ֨לֶךְ֙ וְ⁠דָת֔⁠וֹ וּֽ⁠בְ⁠הִקָּבֵ֞ץ נְעָר֥וֹת רַבּ֛וֹת

**"when the decree of the king and his law were heard"**

The expressions **were heard** and **were gathered** are passive verbal forms. If your language would not use that form, you could express the ideas in active form or in another way that is natural in your language. Alternate translation: "when the messengers proclaimed the king’s decree and his law, and his servants had gathered many young women"

See: Active or Passive

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#4) (id: 262062)

דְּבַר־הַ⁠מֶּ֨לֶךְ֙

**"the decree of the king"**

This refers to the new decree of King Ahasuerus to gather young women to Susa.

* **Associated Passages:** Esther 2:8

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#5) (id: 262063)

**"and his law"**

This refers to King Ahasuerus commanding that men would have complete authority over their wives.

* **Associated Passages:** Esther 2:8

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#6) (id: 262064)

בְּ⁠הִשָּׁמַ֤ע

**"when"** \- **"were heard"**

You could also say where they did this action. Alternate translation: “Messengers went throughout the empire and proclaimed”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#7) (id: 262065)

וּֽ⁠בְ⁠הִקָּבֵ֞ץ נְעָר֥וֹת רַבּ֛וֹת

**"and when many young women were gathered"**

You can say this with an active form, and you can say who did the action. See the UST.

See: Active or Passive

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#8) (id: 262066)

שׁוּשַׁ֥ן הַ⁠בִּירָ֖ה

**"Susa the citadel"**

Here again, this likely means “the capital city of Susa.”

See: Translate Unknowns

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#9) (id: 262067)

אֶל־יַ֣ד הֵגָ֑י

**"into the hand of Hegai"**

Here, **hand** is a metaphor meaning power, control, or authority. As the overseer of the women, Hegai was responsible for the women in the harem for virgins. You could say that the officers “put them under the custody of Hegai” or that “Hegai began to take care of them.”

See: Metaphor

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#10) (id: 262068)

וַ⁠יְהִ֗י בְּ⁠הִשָּׁמַ֤ע \& וּֽ⁠בְ⁠הִקָּבֵ֞ץ \& וַ⁠תִּלָּקַ֤ח

**"And it happened that, when"** \- **"were heard, and when"** \- **"were gathered"** \- **"was taken"**

This means that Esther was brought to the harem of King Ahaserus at the same time as other young women were also being brought to the harem. You can say, “they brought Esther at the same time” or “they also brought Esther.”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#11) (id: 262069)

וַ⁠תִּלָּקַ֤ח

**"that,"** \- **"was taken"**

You can say this with an active form. Alternate translation: “they brought Esther”

See: Active or Passive

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#12) (id: 262070)

וַ⁠תִּלָּקַ֤ח

**"that,"** \- **"was taken"**

You can explain why this happened. For example, you can say, “Because Esther was exceptionally attractive, the officers also brought her”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#13) (id: 262071)

בֵּ֣ית הַ⁠מֶּ֔לֶךְ

**"the palace of the king"**

Alternate translation: “the king’s palace”

* **Associated Passages:** Esther 2:8

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#14) (id: 262072)

אֶל־יַ֥ד הֵגַ֖י שֹׁמֵ֥ר הַ⁠נָּשִֽׁים

**"into the hand of Hegai (who was overseer of the women"**

As earlier in this verse, this means under the custody of Hegai or that Hegai also began to take care of her.

See: Metaphor

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:8 (#15) (id: 262073)

שֹׁמֵ֥ר הַ⁠נָּשִֽׁים

**"who was overseer of the women"**

This means that Hegai took care of the young women who lived in the harem for virgins. If it would be helpful in your language, you could move this information to earlier in the verse when Hegai is first mentioned because it explains why the young women were brought to him.

See: Background Information

* **Associated Passages:** Esther 2:8
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Hegai (ID: `person:Hegai`); Susa (ID: `place:Susa`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:9 (#1) (id: 262074)

וַ⁠תִּיטַ֨ב הַ⁠נַּעֲרָ֣ה בְ⁠עֵינָי⁠ו֮ וַ⁠תִּשָּׂ֣א חֶ֣סֶד לְ⁠פָנָי⁠ו֒

**"And the young woman was pleasing in his eyes, and she lifted kindness before his face"**

This means that Hegai found Esther attractive, and she obtained kindness from him or that Hegai was very impressed with Esther, and she won his favor. In other words, because he was so impressed with her, this made him want to do everything he could to help her. This is background information that explains why Hegai took the actions that are listed next.

* **Associated Passages:** Esther 2:9

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#2) (id: 262075)

וַ⁠תִּיטַ֨ב הַ⁠נַּעֲרָ֣ה בְ⁠עֵינָי⁠ו֮

**"And the young woman was pleasing in his eyes"**

Here, **eyes** represent seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. In this context, the phrase probably means that Hegai thought that Esther was either an attractive woman or a pleasant person, or both.

See: Metonymy

* **Associated Passages:** Esther 2:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#3) (id: 262076)

הַ⁠נַּעֲרָ֣ה

**"the young woman"**

This means Esther. You should make sure that this is clear in your translation.

* **Associated Passages:** Esther 2:9

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#4) (id: 262077)

בְ⁠עֵינָי⁠ו֮

**"in his eyes"**

Here, **his** refers to Hegai. You should make sure that this clear in your translation.

* **Associated Passages:** Esther 2:9

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#5) (id: 262078)

וַ⁠תִּשָּׂ֣א חֶ֣סֶד לְ⁠פָנָי⁠ו֒

**"and she lifted kindness before his face"**

Here, **face** represents the presence of a person. To say that she “lifted kindness before his face” is a rare expression in Hebrew, and the exact meaning is uncertain. In this context, it could even suggest that Esther and Hegai became friends. Alternate translation: “she won his favor”

See: Metonymy

* **Associated Passages:** Esther 2:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#6) (id: 262079)

לְ⁠פָנָי⁠ו֒

**"before his face"**

Here, **his** refers to Hegai. You should make sure that this clear in your translation.

* **Associated Passages:** Esther 2:9

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#7) (id: 262080)

**"gave her ointments"**

Alternate translation: “her beauty treatments,” as in [2:3](https://ref.ly/Esth2:3).

See: Translate Unknowns

* **Associated Passages:** Esther 2:9
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#8) (id: 262081)

וְ⁠אֶת־מָנוֹתֶ֨⁠הָ֙

**"portions of food"**

In context, this likely means that Hegai made sure that Esther was served good food that would keep her healthy.

* **Associated Passages:** Esther 2:9

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#9) (id: 262082)

שֶׁ֣בַע הַ⁠נְּעָר֔וֹת הָ⁠רְאֻי֥וֹת לָֽ⁠תֶת־לָ֖⁠הּ מִ⁠בֵּ֣ית הַ⁠מֶּ֑לֶךְ

**"gave her seven young women chosen from the house of the king"**

This means that Hegai chose seven female servants from the king’s palace and assigned them to take care of Esther.

* **Associated Passages:** Esther 2:9

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:9 (#10) (id: 262083)

וַ⁠יְשַׁנֶּ֧⁠הָ וְ⁠אֶת־נַעֲרוֹתֶ֛י⁠הָ

**"And he transferred her and her young women"**

**Her** means Esther, and **her young women** means the female attendants Hegai assigned to her.

* **Associated Passages:** Esther 2:9

* **Associated ACAI Concepts:** Kindness (ID: `keyterm:Kindness`); House (ID: `realia:House`)

## Esther 2:10 (#1) (id: 262084)

לֹא־הִגִּ֣ידָה אֶסְתֵּ֔ר אֶת־עַמָּ֖⁠הּ וְ⁠אֶת־מֽוֹלַדְתָּ֑⁠הּ

**"Esther had not declared her people or her ancestry"**

This means that Esther had not told anyone this information about herself.

* **Associated Passages:** Esther 2:10

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:10 (#2) (id: 262085)

אֶת־עַמָּ֖⁠הּ וְ⁠אֶת־מֽוֹלַדְתָּ֑⁠הּ

**"her people or her ancestry"**

These two short phrases mean almost the same thing. They are used together to emphasize that Esther followed Mordecai’s instructions exactly. You could combine them, as UST does, and say something like “what people group she was from.” Alternate translation: “Esther did not tell anyone that she was a Jew, and she did not tell anyone who her relatives were”

See: Doublet

* **Associated Passages:** Esther 2:10
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:10 (#3) (id: 262086)

כִּ֧י מָרְדֳּכַ֛י צִוָּ֥ה עָלֶ֖י⁠הָ

**"for Mordecai had laid a charge upon her"**

This is background information that explains why Esther did not tell anyone about her family or nationality. It might be helpful to your readers to state this at the beginning of the verse.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 2:10
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:10 (#4) (id: 262087)

צִוָּ֥ה עָלֶ֖י⁠הָ

**"had laid a charge upon her"**

This means that Mordecai had gotten her to promise not to do it.

* **Associated Passages:** Esther 2:10

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:11 (#1) (id: 262088)

וּ⁠בְ⁠כָל־י֣וֹם וָ⁠י֔וֹם

**"And always, day by day"**

Alternate translation: “every single day”

See: Idiom

* **Associated Passages:** Esther 2:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:11 (#2) (id: 262089)

מָרְדֳּכַי֙ מִתְהַלֵּ֔ךְ

**"Mordecai was walking about"**

Express this in the way your language describes an action that someone repeats over and over. For example, you can say, “Mordecai would walk around.”

* **Associated Passages:** Esther 2:11

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:11 (#3) (id: 262090)

לִ⁠פְנֵ֖י חֲצַ֣ר

**"before the face of the courtyard of"**

Here, **face** means the front of a place. So you could say, “in front of the courtyard.”

See: Metonymy

* **Associated Passages:** Esther 2:11
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:11 (#4) (id: 262091)

בֵּית־הַ⁠נָּשִׁ֑ים

**"the house of women"**

Alternate translation: “the harem for virgins”

* **Associated Passages:** Esther 2:11

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:11 (#5) (id: 262092)

לָ⁠דַ֨עַת֙ אֶת־שְׁל֣וֹם אֶסְתֵּ֔ר וּ⁠מַה־יֵּעָשֶׂ֖ה בָּֽ⁠הּ

**"in order to know the peace of Esther and what was being done with her"**

This is background information that explains why Mordecai would walk around in front of the courtyard. It was so that he could ask people who were going into or coming out of the courtyard how Esther was doing. If it would be helpful in your language, you could place this first in the verse because it explains the rest of what is said.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 2:11
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:11 (#6) (id: 262093)

אֶת־שְׁל֣וֹם אֶסְתֵּ֔ר

**"the peace of Esther"**

Alternate translation: “how Esther was doing” or “about Esther’s well\-being”

* **Associated Passages:** Esther 2:11

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:11 (#7) (id: 262094)

אֶת־שְׁל֣וֹם אֶסְתֵּ֔ר וּ⁠מַה־יֵּעָשֶׂ֖ה בָּֽ⁠הּ

**"the peace of Esther and what was being done with her"**

These two phrases mean similar things. The story is using the repetition to emphasize how concerned Mordecai was for Esther. You could combine them and say, “how Esther was doing” or “if Esther was all right.”

See: Parallelism

* **Associated Passages:** Esther 2:11
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 2:12 (#1) (id: 262095)

וּ⁠בְ⁠הַגִּ֡יעַ

**"Now when"** \- **"came"**

Verses 12 to 14 are background information about how the virgins who were gathered for the king became his concubines. These verses are not specifically about Esther, but about the women in general. Use your language’s way of letting your readers know that this is background information by using a connecting word or a phrase such as, “This is how virgins became concubines for the king.”

See: Background Information

* **Associated Passages:** Esther 2:12
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:12 (#2) (id: 262096)

נַעֲרָ֨ה וְ⁠נַעֲרָ֜ה

**"young woman by young woman"**

Alternate translation: “each one of the young women”

See: Idiom

* **Associated Passages:** Esther 2:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:12 (#3) (id: 262097)

וּ⁠בְ⁠הַגִּ֡יעַ תֹּר֩ \& לָ⁠ב֣וֹא ׀ אֶל־הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֗וֹשׁ

**"Now when the turn came for"** \- **"to go to the king Ahasuerus"**

Each of these young women was going to have sexual relations with the king and, as a result, legally become one of his concubines, that is, one of his secondary wives. As the story has already explained, he would then choose one of them to become his queen and primary wife. But the king would still provide for each of the concubines for the rest of her life; they would not be free to marry anyone else. This is something that the story’s original audience would have understood implicitly. you could say it explicitly if your readers need this information to understand the story: “Each young woman in the harem, one at a time, was going to have sexual relations with King Ahasuerus and become one of his concubines” or “one of his secondary wives.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:12 (#4) (id: 262098)

כְּ⁠דָ֤ת הַ⁠נָּשִׁים֙

**"according to the law of women"**

Here, **law** probably does not mean a legal decree, but rather a standard regimen that had been developed.

See: Translate Unknowns

* **Associated Passages:** Esther 2:12
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:12 (#5) (id: 262099)

כִּ֛י כֵּ֥ן יִמְלְא֖וּ יְמֵ֣י מְרוּקֵי⁠הֶ֑ן

**"For thus the days of their beautification would be fulfilled"**

This phrase indicates that the story is now going to provide background information on how these treatments were done. It means, “This is how these women would spend the time preparing to go to the king.”

See: Background Information

* **Associated Passages:** Esther 2:12
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:12 (#6) (id: 262100)

שִׁשָּׁ֤ה חֳדָשִׁים֙ בְּ⁠שֶׁ֣מֶן הַ⁠מֹּ֔ר

**"six months by oil of myrrh"**

This means that a woman’s attendants would rub her body with olive oil mixed with myrrh every day for six months. you could explain this in more detail if your readers need this information to understand the story.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:12 (#7) (id: 262101)

בְּ⁠שֶׁ֣מֶן הַ⁠מֹּ֔ר

**"by oil of myrrh"**

This likely means “olive oil mixed with myrrh.”

* **Associated Passages:** Esther 2:12

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:12 (#8) (id: 262102)

וּ⁠בְ⁠תַמְרוּקֵ֖י הַ⁠נָּשִֽׁים

**"and by the ointments of women"**

This means lotions designed for women. In 2:3 and 2:9, this one specific part of the program seems to be used to mean all of it, and so in those verses it can be translated “beauty treatments.” But it should be translated with a specific term or phrase here, such as “women’s lotions” or “feminine ointments” because it is named along with oils and perfumes as one component of the treatment.

See: Translate Unknowns

* **Associated Passages:** Esther 2:12
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Balsam Tree (ID: `flora:BalsamTree`); Ahasuerus (ID: `person:Ahasuerus`); Olive Oil (ID: `realia:OliveOil`); Anointing Oil (ID: `realia:AnointingOil`); Myrrh (ID: `flora:Myrrh`)

## Esther 2:13 (#1) (id: 262103)

וּ⁠בָ⁠זֶ֕ה

**"Then at this"**

Alternate translation: “after a woman’s year of beauty treatments was completed”

* **Associated Passages:** Esther 2:13

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Esther 2:13 (#2) (id: 262104)

הַֽ⁠נַּעֲרָ֖ה בָּאָ֣ה אֶל־הַ⁠מֶּ֑לֶךְ

**"the young woman would go to the king"**

This means that the young woman would go and have sexual relations with the king and thereby become one of his concubines. If your readers need this information to understand the story, you could explain that here, if you did not do so in verse 12\.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Esther 2:13 (#3) (id: 262105)

אֵת֩ כָּל־אֲשֶׁ֨ר תֹּאמַ֜ר יִנָּ֤תֵֽן לָ⁠הּ֙

**"Anything that she said would be given to her"**

**Anything** most likely refers to clothing and jewelry. This phrase probably means that a woman could take any of those that she wanted from the harem for virgins and wear them when she went to the king’s palace. You could say that explicitly by saying, “A young woman could take whatever clothing and jewelry she wanted from the harem for virgins and wear them when she went to the king’s palace.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Esther 2:13 (#4) (id: 262106)

**"Anything that she said would be given to her"**

You can say this with an active form. Alternate translation: “she could take whatever she wanted”

See: Active or Passive

* **Associated Passages:** Esther 2:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Esther 2:13 (#5) (id: 262107)

**"the house of the king"**

Alternate translation: “the king’s rooms” or “the king’s house” or “the king’s palace”

* **Associated Passages:** Esther 2:13

* **Associated ACAI Concepts:** House (ID: `realia:House`)

## Esther 2:14 (#1) (id: 262109)

הִ֣יא בָאָ֗ה

**"she would go"**

She would not walk over on her own. Rather, the king’s servants would come and bring her to the king’s private quarters, in the way that they were told to bring Vashti to the banquet in [1:11](https://ref.ly/Esth1:11). you could say this explicitly if it would help your readers to understand the story.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#2) (id: 262110)

וּ֠⁠בַ⁠בֹּקֶר הִ֣יא שָׁבָ֞ה

**"and in the morning she would return"**

It is implied that this is the following morning. This information can be made clear. Once again, she would not go on her own. Alternate translation: “the next morning the king’s servants would bring her to”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#3) (id: 262111)

בֵּ֤ית הַ⁠נָּשִׁים֙ שֵׁנִ֔י

**"the second house of women"**

Alternate translations: “a different house for women” or “the harem for concubines”

See: Ordinal Numbers

* **Associated Passages:** Esther 2:14
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#4) (id: 262112)

אֶל־יַ֧ד שַֽׁעֲשְׁגַ֛ז סְרִ֥יס הַ⁠מֶּ֖לֶךְ שֹׁמֵ֣ר הַ⁠פִּֽילַגְשִׁ֑ים

**"into the hand of Shaashgaz (the eunuch of the king who was overseer of the concubines"**

Alternate translation: “where Shaashgaz, the king’s servant in charge of the concubines, would take care of her”

* **Associated Passages:** Esther 2:14

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#5) (id: 262113)

אֶל־יַ֧ד שַֽׁעֲשְׁגַ֛ז סְרִ֥יס הַ⁠מֶּ֖לֶךְ שֹׁמֵ֣ר הַ⁠פִּֽילַגְשִׁ֑ים

**"into the hand of Shaashgaz (the eunuch of the king who was overseer of the concubines"**

Here, **hand** is a metaphor meaning power, control, or authority. Just as Hegai was the overseer of the virgins, Shaashgaz was the overseer of the concubines, so he would be the one responsible for Esther while she was in the harem for concubines. Alternate translation: “under the custody of Shaashgaz”

See: Metaphor

* **Associated Passages:** Esther 2:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#6) (id: 262114)

שַֽׁעֲשְׁגַ֛ז

**"the"** \- **"Shaashgaz"**

This is a man’s name.

See: How to Translate Names

* **Associated Passages:** Esther 2:14
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#7) (id: 262115)

סְרִ֥יס הַ⁠מֶּ֖לֶךְ

**"the eunuch of the king"**

See how you translated this term, **eunuch**, in [1:10](https://ref.ly/Esth1:10). You could say “royal” rather than “of the king.”

See: Translate Unknowns

* **Associated Passages:** Esther 2:14
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#8) (id: 262116)

שֹׁמֵ֣ר הַ⁠פִּֽילַגְשִׁ֑ים

**"who was overseer of the concubines"**

This means that Shaashgaz was the official who *took care of the concubines* or who was responsible for the concubines.

* **Associated Passages:** Esther 2:14

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#9) (id: 262117)

הַ⁠פִּֽילַגְשִׁ֑ים

**"the concubines"**

As a **concubine**, the young woman was one of the king’s secondary wives. She would live in this harem for the rest of her life. She was not free to return to her family or to marry anybody else. If it would help your readers to understand the story, you could explain all or part of this to your readers here, if you did not do so in 2:12 or 2:13\.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#10) (id: 262118)

לֹא־תָב֥וֹא עוֹד֙ אֶל־הַ⁠מֶּ֔לֶךְ

**"She would not go to the king again"**

In verse 12, the Note mentioned that “go to the king” meant “have sexual relations with the king.” But for this occurrence of the expression, it is appropriate to use a more general phrase such as “visit the king” because there could be other, more social reasons for a further visit. The woman would now be a secondary wife, and the king would only send for her if he decided that he enjoyed being with her. Alternate translation: “She would not go and see the king again”

* **Associated Passages:** Esther 2:14

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#11) (id: 262119)

חָפֵ֥ץ בָּ֛⁠הּ

**"had delighted in her"**

This means that he “had enjoyed being with her” or that she “had pleased the king very much.”

* **Associated Passages:** Esther 2:14

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:14 (#12) (id: 262120)

וְ⁠נִקְרְאָ֥ה בְ⁠שֵֽׁם

**"and she was called by name"**

You can say this with an active form. For example, you can say, “and the king asked for her by name” or “and called for her by name.”

See: Active or Passive

* **Associated Passages:** Esther 2:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Shaashgaz (ID: `person:Shaashgaz`); House (ID: `realia:House`)

## Esther 2:15 (#1) (id: 262121)

וּ⁠בְ⁠הַגִּ֣יעַ תֹּר־אֶסְתֵּ֣ר \& לָ⁠ב֣וֹא אֶל־הַ⁠מֶּ֗לֶךְ

**"Now when the turn of Esther ("** \- **"her"** \- **"came to go to the king"**

The story resumes here after the background information that was provided in verses 12–14 about what the women in the harem did when they became concubines of the king. A contrast is being drawn. A young woman could take any clothing and jewelry she wanted from the harem. But Esther only asked for what Hegai recommended. So it would be good to introduce this episode with a word such as “but,” which draws a contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#2) (id: 262122)

בַּת־אֲבִיחַ֣יִל דֹּ֣ד מָרְדֳּכַ֡י אֲשֶׁר֩ לָקַֽח־ל֨⁠וֹ לְ⁠בַ֜ת

**"the daughter of Abihail, the uncle of Mordecai, who had taken"** \- **"for him as a daughter"**

This background information reminds the reader who Esther was by describing her relationship to Mordecai. You could make this a separate sentence and move it later in the verse to keep from breaking up the flow of the first sentence about what Esther did.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#3) (id: 262123)

אֲבִיחַ֣יִל

**"Abihail"**

This man was Esther’s father and Mordecai’s uncle.

See: How to Translate Names

* **Associated Passages:** Esther 2:15
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#4) (id: 262124)

לֹ֤א בִקְשָׁה֙ דָּבָ֔ר כִּ֠י אִ֣ם אֶת־אֲשֶׁ֥ר יֹאמַ֛ר הֵגַ֥י

**"she did not seek anything except what Hegai ("** \- **"said"**

You can state this in a positive form by saying, “she asked only for.” Alternate translation: “she did not ask for anything else to wear except”

See: Double Negatives

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Double Negatives (ID: `33265@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#5) (id: 262125)

אֶת־אֲשֶׁ֥ר יֹאמַ֛ר הֵגַ֥י

**"what Hegai ("** \- **"said"**

Alternate translation: “what Hegai recommended” or “what Hegai suggested”

* **Associated Passages:** Esther 2:15

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#6) (id: 262126)

אֶת־אֲשֶׁ֥ר יֹאמַ֛ר הֵגַ֥י

**"what Hegai ("** \- **"said"**

As the king’s personal servant, **Hegai** would have known what kind of clothing and jewelry Ahasuerus found attractive on a woman. By following his advice, Esther showed that she trusted Hegai. This is implicit information that you could make explicit if it would help the reader to understand the story. You might say, for example, “Hegai knew the king’s preferences because he was his personal servant. Esther had come to trust Hegai and so she followed his advice.” This could go at the end of the verse or in a footnote.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#7) (id: 262127)

סְרִיס־הַ⁠מֶּ֖לֶךְ שֹׁמֵ֣ר הַ⁠נָּשִׁ֑ים

**"the eunuch of the king who was overseer of the women"**

This is saying that Hegai was the royal official responsible for the women in the harem for virgins. This is background information that reminds the reader who Hegai was by describing his responsibilities. You could also move this to the end of the verse and put it in parentheses.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#8) (id: 262128)

סְרִיס־הַ⁠מֶּ֖לֶךְ

**"the eunuch of the king"**

See how you decided to translate the term **eunuch** in [1:10](https://ref.ly/Esth1:10). You could say “royal” for “of the king.”

See: Translate Unknowns

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#9) (id: 262129)

נֹשֵׂ֣את חֵ֔ן בְּ⁠עֵינֵ֖י כָּל־רֹאֶֽי⁠הָ

**"a lifter of favor in the eyes of everyone who saw her"**

This is a rare expression in Hebrew, and the exact meaning is uncertain. In this context, the expression probably means that everyone who saw Esther dressed in the clothes she would wear for her visit to the king was very impressed with the way she looked. Alternate translation: “pleased all who saw her”

See: Idiom

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#10) (id: 262130)

בְּ⁠עֵינֵ֖י

**"in the eyes of"**

Here, **eyes** is a metonym for seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. In this context, the phrase probably means that everyone who saw Esther thought that she was either an attractive woman or a pleasant person, or both. Alternate translation: “in the judgment of”

See: Metaphor

* **Associated Passages:** Esther 2:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:15 (#11) (id: 262131)

כָּל־רֹאֶֽי⁠הָ

**"everyone who saw her"**

This phrase can mean either **every man or every person who saw her**. Alternate translation: “every man who saw her”

See: When Masculine Words Include Women

* **Associated Passages:** Esther 2:15
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Hegai (ID: `person:Hegai`); Favor (ID: `keyterm:Favor`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:16 (#1) (id: 262132)

וַ⁠תִּלָּקַ֨ח אֶסְתֵּ֜ר

**"And Esther was taken"**

You can say this with an active form by saying, for example, “The king’s servants brought Esther.”

See: Active or Passive

* **Associated Passages:** Esther 2:16
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); House (ID: `realia:House`)

## Esther 2:16 (#2) (id: 262133)

אֶל־בֵּ֣ית מַלְכוּת֔⁠וֹ

**"to the palace of his royalty"**

You can use an adjective to translate the abstract noun **royalty.** For example, you can say, “in his royal palace”

See: Abstract Nouns

* **Associated Passages:** Esther 2:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); House (ID: `realia:House`)

## Esther 2:16 (#3) (id: 262134)

בַּ⁠חֹ֥דֶשׁ הָ⁠עֲשִׂירִ֖י

**"in the tenth month"**

Alternate translation: “in month ten”

See: Ordinal Numbers

* **Associated Passages:** Esther 2:16
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); House (ID: `realia:House`)

## Esther 2:16 (#4) (id: 262135)

הוּא־חֹ֣דֶשׁ טֵבֵ֑ת

**"which is the month of Tebeth"**

**Tebeth** is the name of the tenth month of the Hebrew calendar. Alternate translation: “that is, the month of Tebeth”

See: Hebrew Months

* **Associated Passages:** Esther 2:16
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); House (ID: `realia:House`)

## Esther 2:16 (#5) (id: 262136)

חֹ֣דֶשׁ טֵבֵ֑ת

**"the month of Tebeth"**

The story is being recorded from the perspective of the Persian court. But for the benefit of its intended Jewish audience, the Hebrew name of the month is given.

* **Associated Passages:** Esther 2:16

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); House (ID: `realia:House`)

## Esther 2:16 (#6) (id: 262137)

חֹ֣דֶשׁ טֵבֵ֑ת

**"the month of Tebeth"**

You could convert the Hebrew days and months into approximate dates on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the dates will be different every year and the translation will not be quite accurate. So you may just want to provide the Hebrew days and months.

See: Translate Unknowns

* **Associated Passages:** Esther 2:16
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); House (ID: `realia:House`)

## Esther 2:16 (#7) (id: 262138)

בִּ⁠שְׁנַת־שֶׁ֖בַע לְ⁠מַלְכוּתֽ⁠וֹ

**"in year seven of his reign"**

Alternate translation: “during the seventh year of the reign of Ahasuerus as king of Persia”

See: Ordinal Numbers

* **Associated Passages:** Esther 2:16
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); House (ID: `realia:House`)

## Esther 2:17 (#1) (id: 262139)

וַ⁠יֶּאֱהַ֨ב הַ⁠מֶּ֤לֶךְ אֶת־אֶסְתֵּר֙ מִ⁠כָּל־הַ⁠נָּשִׁ֔ים וַ⁠תִּשָּׂא־חֵ֥ן וָ⁠חֶ֛סֶד לְ⁠פָנָ֖י⁠ו מִ⁠כָּל־הַ⁠בְּתוּלֹ֑ת

**"And the king loved Esther more than all the women and she lifted favor and kindness before his face more than all the virgins"**

These two phrases mean almost the same thing. The information is given twice, in slightly different ways, to emphasize that Ahasuerus was completely convinced that Esther was the one he should make his queen\*\*.\*\* “More than all the virgins” basically duplicates the meaning of “more than all the women,” so, if this duplication would be confusing in your language, you don’t need to include both phrases, or you could say, “more than all of them” for the second phrase.

See: Parallelism

* **Associated Passages:** Esther 2:17
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Love (ID: `keyterm:Love`); Vashti (ID: `person:Vashti`); Favor (ID: `keyterm:Favor`); Kindness (ID: `keyterm:Kindness`); Crown (ID: `realia:Crown`)

## Esther 2:17 (#2) (id: 262140)

מִ⁠כָּל־הַ⁠נָּשִׁ֔ים

**"more than all the women"**

This means all the other women who had been brought to the king and who became his concubines.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Love (ID: `keyterm:Love`); Vashti (ID: `person:Vashti`); Favor (ID: `keyterm:Favor`); Kindness (ID: `keyterm:Kindness`); Crown (ID: `realia:Crown`)

## Esther 2:17 (#3) (id: 262141)

וַ⁠תִּשָּׂא־חֵ֥ן וָ⁠חֶ֛סֶד לְ⁠פָנָ֖י⁠ו

**"and she lifted favor and kindness before his face"**

This is a rare expression in Hebrew, and the exact meaning is uncertain. See how you translated similar phrases in [2:9](https://ref.ly/Esth2:9) and [2:15](https://ref.ly/Esth2:15). Review the explanation there if that would be helpful. In this context, it probably means that King Ahasuerus was very impressed with both Esther’s appearance and her personality.

See: Idiom

* **Associated Passages:** Esther 2:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Love (ID: `keyterm:Love`); Vashti (ID: `person:Vashti`); Favor (ID: `keyterm:Favor`); Kindness (ID: `keyterm:Kindness`); Crown (ID: `realia:Crown`)

## Esther 2:17 (#4) (id: 262142)

וַ⁠תִּשָּׂא־חֵ֥ן וָ⁠חֶ֛סֶד לְ⁠פָנָ֖י⁠ו

**"and she lifted favor and kindness before his face"**

**Favor** and **kindness** mean similar things and are probably used together here to strengthen a single idea. Alternate translation: “he was very pleased with her”

See: Doublet

* **Associated Passages:** Esther 2:17
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Love (ID: `keyterm:Love`); Vashti (ID: `person:Vashti`); Favor (ID: `keyterm:Favor`); Kindness (ID: `keyterm:Kindness`); Crown (ID: `realia:Crown`)

## Esther 2:17 (#5) (id: 262143)

לְ⁠פָנָ֖י⁠ו

**"before his face"**

Here, **face** represents the presence of a person.

See: Metonymy

* **Associated Passages:** Esther 2:17
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Love (ID: `keyterm:Love`); Vashti (ID: `person:Vashti`); Favor (ID: `keyterm:Favor`); Kindness (ID: `keyterm:Kindness`); Crown (ID: `realia:Crown`)

## Esther 2:17 (#6) (id: 262144)

וַ⁠יָּ֤שֶׂם כֶּֽתֶר־מַלְכוּת֙ בְּ⁠רֹאשָׁ֔⁠הּ

**"And he set a crown of royalty on her head"**

Ahasuerus did this to show that he was making Esther his queen.

See: Symbolic Action

* **Associated Passages:** Esther 2:17
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Love (ID: `keyterm:Love`); Vashti (ID: `person:Vashti`); Favor (ID: `keyterm:Favor`); Kindness (ID: `keyterm:Kindness`); Crown (ID: `realia:Crown`)

## Esther 2:17 (#7) (id: 262145)

כֶּֽתֶר־מַלְכוּת֙

**"a crown of royalty"**

To translate the abstract noun **royalty**, you can use an adjective such as a “royal” crown.

See: Abstract Nouns

* **Associated Passages:** Esther 2:17
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Love (ID: `keyterm:Love`); Vashti (ID: `person:Vashti`); Favor (ID: `keyterm:Favor`); Kindness (ID: `keyterm:Kindness`); Crown (ID: `realia:Crown`)

## Esther 2:18 (#1) (id: 262146)

וַ⁠יַּ֨עַשׂ הַ⁠מֶּ֜לֶךְ מִשְׁתֶּ֣ה גָד֗וֹל

**"And the king made a great feast"**

The king did this to celebrate Esther becoming the queen. You can say this explicitly. For example, you can say, “Then, to celebrate her becoming the queen, the king hosted a great feast.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 2:18 (#2) (id: 262147)

אֵ֖ת מִשְׁתֵּ֣ה אֶסְתֵּ֑ר

**"the feast of Esther"**

You could indicate to your readers this is the name that Ahasuerus gave to the feast, for example, “He called it ‘Esther’s Feast.’”

See: How to Translate Names

* **Associated Passages:** Esther 2:18
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 2:18 (#3) (id: 262148)

וַ⁠הֲנָחָ֤ה לַ⁠מְּדִינוֹת֙ עָשָׂ֔ה

**"And he made a holiday for the provinces"**

This was probably a “tax holiday,” that is, a temporary elimination of taxes. For a time, the provinces did not have to send in the usual taxes to the king. You could say something like “he proclaimed a temporary reprieve from taxes for all the provinces of his empire.”

See: Translate Unknowns

* **Associated Passages:** Esther 2:18
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 2:18 (#4) (id: 262149)

כְּ⁠יַ֥ד הַ⁠מֶּֽלֶךְ

**"according to the hand of the king"**

Here, **hand** represents the action of doing or giving. The phrase means that King Ahasuerus personally gave lavish gifts to the guests at this feast. You could say something like “with royal generosity” or “with great willingness to give” or “with generosity that only a king can give.”

See: Metonymy

* **Associated Passages:** Esther 2:18
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 2:19 (#1) (id: 262150)

וּ⁠בְ⁠הִקָּבֵ֥ץ בְּתוּל֖וֹת שֵׁנִ֑ית

**"Now when virgins were being gathered a second time"**

Since this happened some time after Esther’s Feast, you can show this by using a connecting phrase such as “some time later.”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Esther 2:19
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 2:19 (#2) (id: 262151)

וּ⁠בְ⁠הִקָּבֵ֥ץ בְּתוּל֖וֹת שֵׁנִ֑ית

**"Now when virgins were being gathered a second time"**

The most likely explanation for this is that Ahasuerus decided he would like to have more concubines. you could say this explicitly if it will help the readers to understand the story. He probably believed that having a large number of concubines was another thing that would show he was a great king. To do so, you could add: “because Ahasuerus decided that he would like to have more concubines.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 2:19 (#3) (id: 262152)

וּ⁠בְ⁠הִקָּבֵ֥ץ בְּתוּל֖וֹת שֵׁנִ֑ית

**"Now when virgins were being gathered a second time"**

You can say this with an active form, and you can say who did the action. For example, you can say, “at the time when the king’s officers were gathering virgins for a second time.”

See: Active or Passive

* **Associated Passages:** Esther 2:19
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 2:19 (#4) (id: 262153)

וּ⁠בְ⁠הִקָּבֵ֥ץ בְּתוּל֖וֹת שֵׁנִ֑ית וּ⁠מָרְדֳּכַ֖י

**"Now when virgins were being gathered a second time, Mordecai"** \- **"then"**

This indicates that the information about Mordecai explains what he was doing at this same time. You can use a phrase such as “during this time” to indicate this.

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Esther 2:19
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 2:19 (#5) (id: 262154)

וּ⁠מָרְדֳּכַ֖י יֹשֵׁ֥ב בְּ⁠שַֽׁעַר־הַ⁠מֶּֽלֶךְ

**"Mordecai was then sitting at the gate of the king"**

Possible meanings are: (1\) **Sitting at the king’s gate** is an idiom meaning that Mordecai worked for the king in some capacity and was stationed at the gate. (2\) Mordecai sat there so he could hear about how Esther was doing from the many people who passed through the gate.

See: Idiom

* **Associated Passages:** Esther 2:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 2:19 (#6) (id: 262155)

בְּ⁠שַֽׁעַר־הַ⁠מֶּֽלֶךְ

**"at the gate of the king"**

Alternate translation: “the gate to the king’s house” or “the gate to the king’s palace”

* **Associated Passages:** Esther 2:19

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 2:20 (#1) (id: 262156)

מֽוֹלַדְתָּ⁠הּ֙ וְ⁠אֶת־עַמָּ֔⁠הּ

**"her ancestry (that is, her people"**

These two short phrases mean almost the same thing. They are used together to emphasize that Esther followed Mordecai’s instructions exactly. You could combine them, as UST does. Alternate translation: “who her relatives were or that she was a Jew”

See: Doublet

* **Associated Passages:** Esther 2:20
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:20 (#2) (id: 262157)

כַּ⁠אֲשֶׁ֛ר צִוָּ֥ה עָלֶ֖י⁠הָ מָרְדֳּכָ֑י

**"according to the charge that Mordecai had laid upon her"**

Alternate translation: “just as Mordecai had instructed her” or “as Mordecai had made her promise” or “because Mordecai had warned her not to tell anyone”

* **Associated Passages:** Esther 2:20

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:21 (#1) (id: 262158)

בַּ⁠יָּמִ֣ים הָ⁠הֵ֔ם

**"In those days"**

This resumes the story after the background information that is provided in verse 20\. Indicate this in the way that your language resumes a story after giving background information, such as with a connecting phrase like “during that time.”

See: Connect — Simultaneous Time Relationship

* **Associated Passages:** Esther 2:21
* **Associated Articles:** Connect — Simultaneous Time Relationship (ID: `33334@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:21 (#2) (id: 262159)

וּ⁠מָרְדֳּכַ֖י יֹשֵׁ֣ב בְּ⁠שַֽׁעַר־הַ⁠מֶּ֑לֶךְ

**"when Mordecai was sitting at the gate of the king"**

It is implicit in the story that Mordecai heard the two men talking who are named right afterwards. You can say this explicitly if it helps the readers to understand this part of the story. To do so, add: “he heard two men talking.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 2:21
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:21 (#3) (id: 262160)

יֹשֵׁ֣ב בְּ⁠שַֽׁעַר־הַ⁠מֶּ֑לֶךְ

**"was sitting at the gate of the king"**

As in verse 19, this is likely an idiom that means that Mordecai worked for the king in some capacity and was stationed at the gate where officials met to make important decisions.

See: Idiom

* **Associated Passages:** Esther 2:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:21 (#4) (id: 262161)

בִּגְתָ֨ן וָ⁠תֶ֜רֶשׁ

**"Bigthan and Teresh"**

These are the names of men.

See: How to Translate Names

* **Associated Passages:** Esther 2:21
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:21 (#5) (id: 262162)

שְׁנֵֽי־סָרִיסֵ֤י הַ⁠מֶּ֨לֶךְ֙

**"two eunuchs of the king"**

See how you translated this term in [1:10](https://ref.ly/Esth1:10). For the phrase **of the king**, you can say “royal.”

See: Translate Unknowns

* **Associated Passages:** Esther 2:21
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:21 (#6) (id: 262163)

מִ⁠שֹּׁמְרֵ֣י הַ⁠סַּ֔ף

**"who were guardians of the doorway"**

In this case the men were not guardians of one of the harems but guardians of the king’s private rooms. You could say, “they guarded the doorway to the king’s private rooms” or “they protected the doorway to the king’s private quarters.”

* **Associated Passages:** Esther 2:21

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:21 (#7) (id: 262164)

וַ⁠יְבַקְשׁוּ֙

**"and they sought"**

Here, **seeking** means actively trying to do something. Alternate translation: “they were plotting” or “they wanted to find a way”

See: Metaphor

* **Associated Passages:** Esther 2:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:21 (#8) (id: 262165)

לִ⁠שְׁלֹ֣חַ יָ֔ד

**"to stretch out a hand"**

Here the expression **to stretch out a hand** means to cause someone physical harm with the intention of killing him. You could say, “they were planning to assassinate him.”

See: Idiom

* **Associated Passages:** Esther 2:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 2:22 (#1) (id: 262166)

וַ⁠יִּוָּדַ֤ע הַ⁠דָּבָר֙ לְ⁠מָרְדֳּכַ֔י

**"And the matter was made known to Mordecai"**

You can say this with an active form, for example by saying, “Mordecai found out what they were planning.”

See: Active or Passive

* **Associated Passages:** Esther 2:22
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:22 (#2) (id: 262167)

וַ⁠יַּגֵּ֖ד לְ⁠אֶסְתֵּ֣ר הַ⁠מַּלְכָּ֑ה

**"and he declared it to Esther the queen"**

This means that he told Queen Esther about it somehow.

* **Associated Passages:** Esther 2:22

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:22 (#3) (id: 262168)

בְּ⁠שֵׁ֥ם מָרְדֳּכָֽי

**"in the name of Mordecai"**

This expression means that Esther passed the information along to King Ahasuerus on Mordecai’s behalf or that she told the king the information had come from Mordecai.

See: Idiom

* **Associated Passages:** Esther 2:22
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 2:23 (#1) (id: 262169)

וַ⁠יְבֻקַּ֤שׁ הַ⁠דָּבָר֙ וַ⁠יִּמָּצֵ֔א וַ⁠יִּתָּל֥וּ שְׁנֵי⁠הֶ֖ם עַל־עֵ֑ץ

**"And the matter was sought out and was found out, and the two of them were hanged on a pole"**

This verse tells what happened next after Esther gave the information to the king. You can use a word such as “then” to indicate this.

See: Connect — Sequential Time Relationship

* **Associated Passages:** Esther 2:23
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 2:23 (#2) (id: 262170)

וַ⁠יְבֻקַּ֤שׁ הַ⁠דָּבָר֙ וַ⁠יִּמָּצֵ֔א

**"And the matter was sought out and was found out"**

You can say this with an active form, and you can say who did the action. For example, you can say, “Then the king’s servants investigated Mordecai’s report and found out that it was true.”

See: Active or Passive

* **Associated Passages:** Esther 2:23
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 2:23 (#3) (id: 262171)

וַ⁠יִּתָּל֥וּ שְׁנֵי⁠הֶ֖ם עַל־עֵ֑ץ

**"and the two of them were hanged on a pole"**

You can say this with an active form, and you can say who did the action. Alternate translation: “the king ordered his servants to impale those two men on wooden poles” or “the king ordered his servants to hang those two men on a gallows”

See: Active or Passive

* **Associated Passages:** Esther 2:23
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 2:23 (#4) (id: 262172)

עֵ֑ץ

**"a pole"**

This seems to refer specifically to a pole or wooden structure that would be used to suspend a person above the ground and that could be used as a means of killing him or to display his body after he was dead. This might mean: (1\) by hanging him from it with a rope tied around his neck, or (2\) by impaling him on it, that is, by sticking a sharp point on one end of a pole through his body. Alternate translation: “wooden structure” or “wooden pole” or “gallows”

See: Translate Unknowns

* **Associated Passages:** Esther 2:23
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 2:23 (#5) (id: 262173)

וַ⁠יִּכָּתֵ֗ב

**"And it was written"**

You can say this with an active form, and you can say who did the action. You can say, for example, “The king’s scribes recorded an account of this.”

See: Active or Passive

* **Associated Passages:** Esther 2:23
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 2:23 (#6) (id: 262174)

בְּ⁠סֵ֛פֶר דִּבְרֵ֥י הַ⁠יָּמִ֖ים

**"in the book of the events of days"**

This is an idiom that describes a regular record of the events in a king’s reign. Alternate translation: “the record book”

See: Idiom

* **Associated Passages:** Esther 2:23
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 2:23 (#7) (id: 262175)

לִ⁠פְנֵ֥י הַ⁠מֶּֽלֶךְ

**"before the face of the king"**

Here, **face** refers to the presence of a person. The phrase means that the scribes wrote this account while King Ahasuerus was personally present. You could say that they did this in the king’s presence. Alternate translation: “the king watched a scribe write this down”

See: Metonymy

* **Associated Passages:** Esther 2:23
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 3:1 (#1) (id: 262176)

אַחַ֣ר ׀ הַ⁠דְּבָרִ֣ים הָ⁠אֵ֗לֶּה

**"After these things"**

This introduces a new event in the story. Use a connecting phrase that introduces a new event in your language.

See: Introduction of a New Event

* **Associated Passages:** Esther 3:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#2) (id: 262177)

אַחַ֣ר ׀ הַ⁠דְּבָרִ֣ים הָ⁠אֵ֗לֶּה

**"After these things"**

This event happened some time after Esther became queen, and you can show this by using a connecting phrase such as “some time later.”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Esther 3:1
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#3) (id: 262178)

אֶת־הָמָ֧ן

**"Haman"**

This is a man’s name. It occurs many times in the story. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 3:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#4) (id: 262179)

בֶּֽן־הַמְּדָ֛תָא הָ⁠אֲגָגִ֖י

**"the son of Hammedatha, the Agagite"**

These are the names of Haman’s father and Haman’s people group.

See: How to Translate Names

* **Associated Passages:** Esther 3:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#5) (id: 262180)

גִּדַּל֩ \& אֲחַשְׁוֵר֜וֹשׁ אֶת־הָמָ֧ן

**"Ahasuerus made Haman,"** \- **"great"**

This means that Ahasuerus **promoted** Haman. It’s implied in the story that Haman was already one of Ahasuerus’s officials, but that he was now being promoted to a higher position. You could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#6) (id: 262181)

וַֽ⁠יְנַשְּׂאֵ֑⁠הוּ וַ⁠יָּ֨שֶׂם֙ אֶת־כִּסְא֔⁠וֹ מֵ⁠עַ֕ל כָּל־הַ⁠שָּׂרִ֖ים

**"And he lifted him up and placed his seat of authority from over all the officials"**

These two phrases mean the same thing. They are used together to emphasize that the king was making Haman very powerful. You could combine them and say something like, “the king gave Haman a very high position that was over all of his other officials.”

See: Doublet

* **Associated Passages:** Esther 3:1
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#7) (id: 262182)

וַֽ⁠יְנַשְּׂאֵ֑⁠הוּ

**"And he lifted him up"**

Here, **lifting** is a metaphor meaning to advance or promote a person to a higher, more important position than they held previously. Alternate translation: “he gave him a more important position”

See: Metaphor

* **Associated Passages:** Esther 3:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#8) (id: 262183)

אֶת־כִּסְא֔⁠וֹ

**"his seat of authority"**

Here, **seat of authority** means Haman’s position or status in the Persian government.

See: Idiom

* **Associated Passages:** Esther 3:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#9) (id: 262184)

אֶת־כִּסְא֔⁠וֹ

**"his seat of authority"**

If your language does not use an abstract noun for the idea behind the word **authority**, you can express the same idea with a concrete noun for this whole phrase. For example, you can say, “his position.”

See: Abstract Nouns

* **Associated Passages:** Esther 3:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:1 (#10) (id: 262185)

מֵ⁠עַ֕ל כָּל־הַ⁠שָּׂרִ֖ים אֲשֶׁ֥ר אִתּֽ⁠וֹ

**"from over all the officials who were with him"**

**Him** in this phrase may refer to the king. You could say that “the king promoted him above his other officials” or “gave him more authority than all of his other officials” or that “Haman became more important than all of his other officials.”

* **Associated Passages:** Esther 3:1

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`); Agag (ID: `person:Agag.2`)

## Esther 3:2 (#1) (id: 262186)

וְ⁠כָל־עַבְדֵ֨י הַ⁠מֶּ֜לֶךְ אֲשֶׁר־בְּ⁠שַׁ֣עַר הַ⁠מֶּ֗לֶךְ

**"And all the servants of the king who were at the gate of the king"**

This is probably a reference to the specific group of officials who were stationed at the gate, as Mordecai was. Alternate translation: "And the royal officials who were stationed at the palace gate"

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#2) (id: 262187)

וְ⁠כָל־עַבְדֵ֨י הַ⁠מֶּ֜לֶךְ \& כֹּרְעִ֤ים וּ⁠מִֽשְׁתַּחֲוִים֙ לְ⁠הָמָ֔ן

**"And all the servants of the king"** \- **"were bowing down and prostrating themselves to Haman"**

The servants were required to do this as a way of honoring him whenever Haman walked by. If it would be helpful in your language, you could say this explicitly, as in the UST.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#3) (id: 262188)

כֹּרְעִ֤ים וּ⁠מִֽשְׁתַּחֲוִים֙

**"were bowing down and prostrating themselves"**

This means that the other officials humbled themselves and lay flat on the ground before Haman. These actions represented recognition and respect for the very high position that the king had given him in the Persian government.

See: Symbolic Action

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#4) (id: 262189)

כֹּרְעִ֤ים וּ⁠מִֽשְׁתַּחֲוִים֙

**"were bowing down and prostrating themselves"**

These two phrases mean basically the same thing. The repetition is used to emphasize that the officials knew how important it was to obey the king’s command and honor Haman in this way. If it would be clearer for your readers, you could express the emphasis with a single phrase. Alternate translation: "bowing down on the ground"

See: Doublet

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#5) (id: 262190)

וּ⁠מִֽשְׁתַּחֲוִים֙

**"and prostrating themselves"**

This means bowing down all the way to the ground.

* **Associated Passages:** Esther 3:2

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#6) (id: 262191)

כִּי־כֵ֖ן צִוָּה־ל֣⁠וֹ

**"for thus"** \- **"had commanded concerning him"**

If it would be helpful in your language, you could put this information first in the verse because it explains the reason why all the other officials were bowing to Haman. See the UST.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#7) (id: 262192)

כִּי־כֵ֖ן צִוָּה־ל֣⁠וֹ

**"for thus"** \- **"had commanded concerning him"**

The king did this as a sign of Haman’s new position. You can state this here or in a separate sentence, as in the UST. Alternate translation: “the king had commanded them to do this in order to show that he had given Haman such a high position”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#8) (id: 262193)

וּ⁠מָ֨רְדֳּכַ֔י לֹ֥א יִכְרַ֖ע וְ⁠לֹ֥א יִֽשְׁתַּחֲוֶֽה

**"But Mordecai would neither bow down nor would he prostrate himself"**

See how you translated the similar phrases earlier in this verse. Alternate translation: "Mordecai would not bow down to the ground"

See: Doublet

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:2 (#9) (id: 262194)

וּ⁠מָ֨רְדֳּכַ֔י לֹ֥א יִכְרַ֖ע וְ⁠לֹ֥א יִֽשְׁתַּחֲוֶֽה

**"But Mordecai would neither bow down nor would he prostrate himself"**

It appears that as a Jew, Mordecai considered it wrong to worship anyone other than Yahweh, and that he believed he would be worshiping Haman if he bowed down low to the ground to honor him. So, on religious grounds, Mordecai refused to bow down. If it would be helpful in your language, you could say this explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:3 (#1) (id: 262195)

**"the servants of the king who were at the gate of the king"**

See how you translated this expression at the beginning of [3:2](https://ref.ly/Esth3:2).

* **Associated Passages:** Esther 3:3

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Mordecai (ID: `person:Mordecai.2`)

## Esther 3:3 (#2) (id: 262196)

וַ⁠יֹּ֨אמְר֜וּ \& לְ⁠מָרְדֳּכָ֑י

**"And"** \- **"said to Mordecai"**

They spoke to Mordecai because they saw that he did not bow down. If it would be helpful in your language, you could say this explicitly. You could say, for example, “The other officials saw that, and they asked Mordecai.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Mordecai (ID: `person:Mordecai.2`)

## Esther 3:3 (#3) (id: 262197)

מַדּ֨וּעַ֙ אַתָּ֣ה עוֹבֵ֔ר אֵ֖ת מִצְוַ֥ת הַ⁠מֶּֽלֶךְ

**"Why are you trespassing the command of the king"**

Alternative translation: “why are you disobeying the king’s command” or “why are you not obeying what the king commanded”

* **Associated Passages:** Esther 3:3

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Mordecai (ID: `person:Mordecai.2`)

## Esther 3:4 (#1) (id: 262198)

ב⁠אמר⁠ם אֵלָי⁠ו֙

**"when they spoke to him"**

This means that they spoke to him about not bowing down. Specifically, they warned him that he would be severely punished if he kept disobeying the king and not honoring his highest official. If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:4 (#2) (id: 262199)

י֣וֹם וָ⁠י֔וֹם

**"day after day"**

This expression means “every day.”

See: Idiom

* **Associated Passages:** Esther 3:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:4 (#3) (id: 262200)

וַ⁠יְהִ֗י ב⁠אמר⁠ם אֵלָי⁠ו֙ \& וְ⁠לֹ֥א שָׁמַ֖ע אֲלֵי⁠הֶ֑ם

**"And it happened that, when they spoke to him"** \- **"he did not listen to them"**

Here, **he did not listen** means he did not heed their warning or he did not do what they said. Mordecai heard them, but he did not do what they advised him to do. This means that even though the other officials questioned him and probably warned him, he still refused to bow down.

See: Idiom

* **Associated Passages:** Esther 3:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:4 (#4) (id: 262201)

וַ⁠יַּגִּ֣ידוּ לְ⁠הָמָ֗ן לִ⁠רְאוֹת֙ הֲ⁠יַֽעַמְדוּ֙ דִּבְרֵ֣י מָרְדֳּכַ֔י

**"So they told Haman to see if the words of Mordecai would stand"**

In this context, **standing** means to survive a challenge, and **words** means the reasons that Mordecai gave for his actions. This phrase means that the other officials told Haman about it to see if he would tolerate it.

See: Idiom

* **Associated Passages:** Esther 3:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:4 (#5) (id: 262202)

כִּֽי־הִגִּ֥יד לָ⁠הֶ֖ם אֲשֶׁר־ה֥וּא יְהוּדִֽי

**"for he had told them that he was a Jew"**

This is the answer that Mordecai gave the other officials when they asked him why he refused to bow down to Haman. If it would be helpful in your language, you could put it first in the verse because it explains everything that happens afterwards. Alternate translation: “Mordecai told them that he was a Jew, and that Jews would bow down only to God”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 3:4
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:4 (#6) (id: 262203)

כִּֽי־הִגִּ֥יד לָ⁠הֶ֖ם אֲשֶׁר־ה֥וּא יְהוּדִֽי

**"for he had told them that he was a Jew"**

Mordecai’s explanation would have included the fact that Jews would only worship Yahweh. If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:5 (#1) (id: 262204)

וַ⁠יַּ֣רְא הָמָ֔ן

**"And Haman saw"**

Here, **seeing** means knowledge, notice, attention, or judgment. This phrase means that Haman learned about this.

See: Metaphor

* **Associated Passages:** Esther 3:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:5 (#2) (id: 262205)

אֵ֣ין מָרְדֳּכַ֔י כֹּרֵ֥עַ וּ⁠מִֽשְׁתַּחֲוֶ֖ה ל֑⁠וֹ

**"Mordecai would neither bow down nor prostrate himself to him"**

These two phrases mean basically the same thing. The repetition is used to emphasize how serious an offense this was. You could combine them as UST does and say, “Mordecai was refusing to bow down to him.” Alternate translation: “Mordecai would not bow down or lie on the ground”

See: Parallelism

* **Associated Passages:** Esther 3:5
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:5 (#3) (id: 262206)

וַ⁠יִּמָּלֵ֥א הָמָ֖ן חֵמָֽה

**"And Haman was filled with rage"**

Here, **Haman’s rage** is spoken of as something that could fill him up. You could say that he was furious or extremely angry.

See: Metaphor

* **Associated Passages:** Esther 3:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 3:6 (#1) (id: 262207)

וַ⁠יִּ֣בֶז בְּ⁠עֵינָ֗י⁠ו

**"And it was despised in his eyes"**

You can say this with an active form, and you can say who did the action. Alternate translation: “Haman considered that it was not enough”

See: Active or Passive

* **Associated Passages:** Esther 3:6
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#2) (id: 262208)

וַ⁠יִּ֣בֶז בְּ⁠עֵינָ֗י⁠ו

**"And it was despised in his eyes"**

Haman probably decided to go to such extremes because he was so angry. If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#3) (id: 262209)

וַ⁠יִּ֣בֶז

**"And it was despised"**

**Despise** in this context means not to hate but to think little of or think something is too small. This expression means that Haman decided it would not be enough just to kill Mordecai alone.

* **Associated Passages:** Esther 3:6

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#4) (id: 262210)

בְּ⁠עֵינָ֗י⁠ו

**"in his eyes"**

Here, **eyes** stand for “seeing,” and in this case “seeing” means judgment. This phrase means in Haman’s judgment. Alternate translation: “as he saw it”

See: Metaphor

* **Associated Passages:** Esther 3:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#5) (id: 262211)

לִ⁠שְׁלֹ֤ח יָד֙ בְּ⁠מָרְדֳּכַ֣י לְ⁠בַדּ֔⁠וֹ

**"to stretch out a hand against Mordecai alone"**

Alternate translation: “he rejected the idea of killing only Mordecai” or “he decided to kill more than just Mordecai” or “he decided that it would not be enough to get rid of only Mordecai”

* **Associated Passages:** Esther 3:6

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#6) (id: 262212)

**"to stretch out a hand"**

As in [2:21](https://ref.ly/Esth2:21), the expression **to stretch out a hand** means to cause someone physical harm with the intention of killing them.

See: Idiom

* **Associated Passages:** Esther 3:6
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#7) (id: 262213)

כִּֽי־הִגִּ֥ידוּ ל֖⁠וֹ אֶת־עַ֣ם מָרְדֳּכָ֑י

**"for they had told him the people of Mordecai"**

The pronoun **they** refers to the other “servants of the king” from verse 3, and **him** refers to Haman. The phrase **the people of Mordecai** is referring to the Jews. Mordecai had told these servants that he would not bow down to Haman because Jews worshiped only God. So this means that the other servants had told Haman that Mordecai was not bowing down to him because he was a Jew.

* **Associated Passages:** Esther 3:6

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#8) (id: 262214)

וַ⁠יְבַקֵּ֣שׁ הָמָ֗ן לְ⁠הַשְׁמִ֧יד

**"And Haman sought to annihilate"**

Here, **seeking** means trying to do something. In this context, it means that Haman decided that he would try to kill all the Jews.

See: Metaphor

* **Associated Passages:** Esther 3:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#9) (id: 262215)

בְּ⁠כָל־מַלְכ֥וּת אֲחַשְׁוֵר֖וֹשׁ

**"who were in all the kingdom of Ahasuerus"**

This means in the entire empire that King Ahasuerus ruled.

* **Associated Passages:** Esther 3:6

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:6 (#10) (id: 262216)

הַ⁠יְּהוּדִ֛ים \& עַ֥ם מָרְדֳּכָֽי

**"the Jews, the people of Mordecai"**

These two phrases refer to the same group of people. The repetition emphasizes how great the threat was to their existence. You could combine them as UST does and say something like “all of the Jews.” Alternate translation: “Mordecai’s people, the Jews”

See: Doublet

* **Associated Passages:** Esther 3:6
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:7 (#1) (id: 262217)

בַּ⁠חֹ֤דֶשׁ הָ⁠רִאשׁוֹן֙

**"In the first month"**

Alternate translation: “In month one”

See: Ordinal Numbers

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#2) (id: 262218)

הוּא־חֹ֣דֶשׁ נִיסָ֔ן

**"which is the month of Nisan"**

**Nisan** is the name of the first month of the Hebrew calendar.

See: Hebrew Months

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#3) (id: 262219)

חֹ֣דֶשׁ נִיסָ֔ן

**"the month of Nisan"**

As in [2:16](https://ref.ly/Esth2:16), you could convert the Hebrew month into an approximate time on the calendar that your culture uses. However, the Jews used a lunar calendar, so if you use a solar calendar, the dates will be different every year and the translation will not be quite accurate. So you may just want to give the Hebrew month.

See: Translate Unknowns

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#4) (id: 262220)

בִּ⁠שְׁנַת֙ שְׁתֵּ֣ים עֶשְׂרֵ֔ה לַ⁠מֶּ֖לֶךְ אֲחַשְׁוֵר֑וֹשׁ

**"in year 12 of the king Ahasuerus"**

Alternate translations: “during the twelfth year of the reign of Ahasuerus as king of Persia” or “when King Ahasuerus had reigned for about twelve years”

See: Ordinal Numbers

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#5) (id: 262221)

הִפִּ֣יל פּוּר֩ ה֨וּא הַ⁠גּוֹרָ֜ל

**"a Pur (which is “the lot”) was cast"**

The storyteller is giving both the Persian and the Hebrew name because this is the story behind the Festival of Purim, which takes its name from “Pur.” So this is not repetition for emphasis. You can put in the Persian name and the word for “lot” in your own language to show that the storyteller is doing this.

See: How to Translate Names

* **Associated Passages:** Esther 3:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#6) (id: 262222)

הַ⁠גּוֹרָ֜ל

**"the lot"**

A **lot** was an object with different markings on various sides that was used, most likely by dropping it on the ground, to select a specific time for doing something. The belief was that God or the gods would control which way the object fell, thus guiding the selection process and granting favor to the action if it were done on the date chosen.

See: Translate Unknowns

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#7) (id: 262223)

הִפִּ֣יל פּוּר֩

**"a Pur (which is 'the lot') was cast"**

You can say this with an active form, and you can say who did the action. You can say, for example, “Haman had his servants cast a Pur.”

See: Active or Passive

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#8) (id: 262224)

הִפִּ֣יל פּוּר֩

**"a Pur (which is 'the lot') was cast"**

If it would be helpful in your language, you could put this information first because it explains the rest of what happens in this verse.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#9) (id: 262225)

לִ⁠פְנֵ֣י הָמָ֗ן

**"before the face of Haman"**

Here, **face** represents the presence of a person. In this context, the phrase means that Haman was physically present when the lot was cast. Alternate translation: “as Haman watched”

See: Metonymy

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#10) (id: 262226)

מִ⁠יּ֧וֹם ׀ לְ⁠י֛וֹם וּ⁠מֵ⁠חֹ֛דֶשׁ לְ⁠חֹ֥דֶשׁ

**"from day to day and from month to month"**

This expression means that the lot was cast to select a particular day of the month and a particular month of the year.

* **Associated Passages:** Esther 3:7

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#11) (id: 262227)

מִ⁠יּ֧וֹם ׀ לְ⁠י֛וֹם וּ⁠מֵ⁠חֹ֛דֶשׁ לְ⁠חֹ֥דֶשׁ

**"from day to day and from month to month"**

Haman was casting the lot to determine the best day and the best month to kill the Jews. If it would be helpful in your language, you could say this explicitly. Alternate translation: “to determine the best day and the best month to kill the Jews”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#12) (id: 262228)

שְׁנֵים־עָשָׂ֖ר הוּא־חֹ֥דֶשׁ אֲדָֽר

**"the twelfth, which is the month of Adar"**

**Adar** is the name of the twelfth and last month of the Hebrew calendar. This name occurs several times in the story. Be sure to translate it consistently.

See: Hebrew Months

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#13) (id: 262229)

שְׁנֵים־עָשָׂ֖ר הוּא־חֹ֥דֶשׁ אֲדָֽר

**"the twelfth, which is the month of Adar"**

See how you decided in [2:16](https://ref.ly/Esth2:16) and the first part of this verse to represent the dates that are given in the Hebrew calendar in the book of Esther. Be consistent.

* **Associated Passages:** Esther 3:7

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#14) (id: 262230)

שְׁנֵים־עָשָׂ֖ר הוּא־חֹ֥דֶשׁ אֲדָֽר

**"the twelfth, which is the month of Adar"**

This means that the lot indicated the twelfth month of that year as the time for Haman to carry out his plan. If it would be helpful in your language, you could say this explicitly. (The day that was chosen is reported in verse 13\.) Alternate translation: “the lot indicated the twelfth month, the month of Adar, as the best month to kill the Jews”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:7 (#15) (id: 262231)

שְׁנֵים־עָשָׂ֖ר

**"the twelfth"**

Alternative translation: “month twelve” or “the twelfth month”

See: Ordinal Numbers

* **Associated Passages:** Esther 3:7
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Lot (ID: `keyterm:Lot`); Ahasuerus (ID: `person:Ahasuerus`); Lot (ID: `realia:Lot`)

## Esther 3:8 (#1) (id: 262232)

וַ⁠יֹּ֤אמֶר הָמָן֙ לַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֔וֹשׁ

**"And Haman said to the king Ahasuerus"**

The implication is that Haman then went to see the king so that he could speak with him. You could say that explicitly. Do not give your readers the impression that the king was present with Haman while he was casting the lot.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:8 (#2) (id: 262233)

יֶשְׁנ֣⁠וֹ עַם־אֶחָ֗ד

**"He has one people"**

**He** refers to the king, and **one people** refers to the Jews as a people group. So this means there is a people group or a group of people.

See: Numbers

* **Associated Passages:** Esther 3:8
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:8 (#3) (id: 262234)

מְפֻזָּ֤ר וּ⁠מְפֹרָד֙

**"scattered and dispersed"**

These two words mean almost the same thing. Haman uses the repetition to emphasize that the Jews are everywhere throughout the empire and so their influence, which he says is bad, will affect everyone. You could combine these words as UST does.

See: Doublet

* **Associated Passages:** Esther 3:8
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:8 (#4) (id: 262235)

בֵּ֣ין הָֽ⁠עַמִּ֔ים בְּ⁠כֹ֖ל מְדִינ֣וֹת מַלְכוּתֶ֑⁠ךָ

**"among the peoples in all the provinces of your kingdom"**

Haman is saying that the Jews are mixed in with all the other peoples, and they live in every province.

* **Associated Passages:** Esther 3:8

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:8 (#5) (id: 262236)

וְ⁠דָתֵי⁠הֶ֞ם שֹׁנ֣וֹת מִ⁠כָּל־עָ֗ם וְ⁠אֶת־דָּתֵ֤י הַ⁠מֶּ֨לֶךְ֙ אֵינָ֣⁠ם עֹשִׂ֔ים

**"And their laws are different from every people, and the laws of the king they do not obey"**

Haman is saying that the Jews have their own set of laws, and he is accusing them of not obeying the king’s laws as a result.

* **Associated Passages:** Esther 3:8

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:8 (#6) (id: 262237)

דָּתֵ֤י הַ⁠מֶּ֨לֶךְ֙

**"the laws of the king"**

Haman speaks to the king in the third person as a sign of respect. You could have him say, “your laws.” Or you could indicate this respect another way by having Haman begin by saying, “Your Majesty.”

See: First, Second or Third Person

* **Associated Passages:** Esther 3:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:8 (#7) (id: 262238)

וְ⁠לַ⁠מֶּ֥לֶךְ אֵין־שֹׁוֶ֖ה לְ⁠הַנִּיחָֽ⁠ם

**"And there is no profit to the king to leave them untouched"**

Haman is saying, “It is not good for you to allow them to live in your empire.” This can also be stated without the negative. Alternate translation: “the king should remove them” or “you should remove them”

* **Associated Passages:** Esther 3:8

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:8 (#8) (id: 262239)

וְ⁠לַ⁠מֶּ֥לֶךְ

**"And"** \- **"to the king"**

Haman speaks to the king in the third person as a sign of respect. You could express the same meaning in the second person by saying, “not good for you.”

See: First, Second or Third Person

* **Associated Passages:** Esther 3:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:9 (#1) (id: 262240)

אִם־עַל־הַ⁠מֶּ֣לֶךְ ט֔וֹב

**"If it is good to the king"**

Haman speaks to the king in third person as a form of respect.

See: First, Second or Third Person

* **Associated Passages:** Esther 3:9
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#2) (id: 262241)

אִם־עַל־הַ⁠מֶּ֣לֶךְ ט֔וֹב

**"If it is good to the king"**

See how you decided to translate this idiom in [1:19](https://ref.ly/Esth1:19). Alternate translation: “if it seems like a good idea to you, O king” or “if the king thinks this is a good idea” or “if this advice is pleasing to the king”

See: Idiom

* **Associated Passages:** Esther 3:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#3) (id: 262242)

יִכָּתֵ֖ב

**"let it be written"**

You can say this with an active form, and you can say who would do the action. For example, you could say, “the king could write a decree” or “you could write a decree.”

See: Active or Passive

* **Associated Passages:** Esther 3:9
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#4) (id: 262243)

לְ⁠אַבְּדָ֑⁠ם

**"to destroy them"**

This means that the decree would say that all the Jews should be killed.

* **Associated Passages:** Esther 3:9

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#5) (id: 262244)

אֶשְׁקוֹל֙ עַל־יְדֵי֙ עֹשֵׂ֣י הַ⁠מְּלָאכָ֔ה

**"I will weigh out"** \- **"into the hands of the doers of the work"**

This could mean either of two possibilities: (1\) Haman could mean that the king’s men will be able to collect a huge amount of money from the plunder that they would get from the Jews. Referring to the weight would be a way of saying, “I guarantee that it will be at least that much.” This reasoning agrees with 3:13, where the king’s letter specifies that those who kill the Jews can take their plunder. Alternate translation: “we will get so much plunder that the officers you send will be able to bring back” (Also see UST) (2\) “**Weigh out into the hands**” could be an idiom that means Haman himself will pay the officers who will put the plan into effect. Haman will not do this personally, but it means that he will take responsibility for paying them from his own resources. Alternate translation: “From my own resources I will pay … to the men who do the work”

See: Idiom

* **Associated Passages:** Esther 3:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#6) (id: 262245)

וַ⁠עֲשֶׂ֨רֶת אֲלָפִ֜ים כִּכַּר־כֶּ֗סֶף

**"and"** \- **"10,000 talents of silver"**

This would be worth over $300 million at current prices today. But since prices vary over time, if you expressed the value in modern measurements, that could cause your Bible translation to become outdated and inaccurate. Instead you could express the amount using the ancient measurement, the talent, and explain in a note that a talent was equivalent to about 30 kilograms. Alternatively, you could specify the weight of the silver in the text, as UST does in metric tons.

See: Biblical Money

* **Associated Passages:** Esther 3:9
* **Associated Articles:** Biblical Money (ID: `33372@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#7) (id: 262246)

וַ⁠עֲשֶׂ֨רֶת אֲלָפִ֜ים כִּכַּר־כֶּ֗סֶף

**"and"** \- **"10,000 talents of silver"**

The **talent** was the largest unit of money and **10,000** was the largest number in the counting system of the time. It is possible that Haman used these terms to represent a very large, but not precise, amount. Alternate translation: “a huge amount of silver” or “enormous amounts of silver”

See: Hyperbole

* **Associated Passages:** Esther 3:9
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#8) (id: 262247)

עֹשֵׂ֣י הַ⁠מְּלָאכָ֔ה

**"the doers of the work"**

This could mean: (1\) those who work for the king in general (administrators) or (2\) more specifically the officers whom Ahasuerus would send around the empire to organize the killing of the Jews.

* **Associated Passages:** Esther 3:9

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:9 (#9) (id: 262248)

גִּנְזֵ֥י הַ⁠מֶּֽלֶךְ

**"the treasuries of the king"**

Haman speaks to the king in third person as a form of respect. You could also have him speak of “your royal treasuries.”

See: First, Second or Third Person

* **Associated Passages:** Esther 3:9
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Talent (ID: `realia:Talent`); Money (ID: `realia:Money`)

## Esther 3:10 (#1) (id: 262249)

וַ⁠יָּ֧סַר הַ⁠מֶּ֛לֶךְ אֶת־טַבַּעְתּ֖⁠וֹ מֵ⁠עַ֣ל יָד֑⁠וֹ וַֽ⁠יִּתְּנָ֗⁠הּ לְ⁠הָמָ֧ן

**"And the king removed his signet ring from on his hand, and he gave it to Haman"**

The implication is that Ahasuerus thought that Haman’s suggestion was a good idea and he agreed to it. You could say that explicitly. Alternate translation: “the king agreed that this was a good idea, so”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`)

## Esther 3:10 (#2) (id: 262250)

וַ⁠יָּ֧סַר הַ⁠מֶּ֛לֶךְ אֶת־טַבַּעְתּ֖⁠וֹ מֵ⁠עַ֣ל יָד֑⁠וֹ וַֽ⁠יִּתְּנָ֗⁠הּ לְ⁠הָמָ֧ן

**"And the king removed his signet ring from on his hand, and he gave it to Haman"**

**Giving the ring to Haman** showed that Haman could act on the king’s own authority. If it would be helpful in your language, you could say this explicitly. Alternate translation: “the king gave Haman the power to act on his authority to carry out the plan. To show that, he gave Haman the ring he wore that had his official seal on it”

See: Symbolic Action

* **Associated Passages:** Esther 3:10
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`)

## Esther 3:10 (#3) (id: 262251)

אֶת־טַבַּעְתּ֖⁠וֹ

**"his signet ring"**

This was a special ring that could be used to imprint the king’s official seal on a law or decree. This ring had the king’s name or mark on it. When he put a wax seal on important papers, he would press the mark onto the seal. If a paper had this mark on its seal, people would know that what was written on the paper was written with the king’s authority and had to be obeyed. You could describe this as “the ring he wore that had his official seal on it.”

See: Translate Unknowns

* **Associated Passages:** Esther 3:10
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`)

## Esther 3:10 (#4) (id: 262252)

בֶּֽן־הַמְּדָ֛תָא הָ⁠אֲגָגִ֖י

**"the son of Hammedatha, the Agagite"**

This information about Haman’s identity is repeated to stress that the king giving him the signet ring was an official legal act that transferred power to him. (In the same way, legal documents today often use a person’s full name and other identifying information.) If you repeat this information in your own translation, your readers should see this significance.

* **Associated Passages:** Esther 3:10

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`)

## Esther 3:10 (#5) (id: 262253)

צֹרֵ֥ר הַ⁠יְּהוּדִֽים

**"the adversary of the Jews"**

This phrase gives clarifying information about Haman. It means that he had become “the enemy of the Jews.”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 3:10
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Agagites (ID: `group:Agag`); Hammedatha (ID: `person:Hammedatha`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`)

## Esther 3:11 (#1) (id: 262254)

הַ⁠כֶּ֖סֶף נָת֣וּן לָ֑⁠ךְ

**"The silver is given to you"**

You can say this with an active form. This could mean one of the following things: (1\) “You can keep the money from plundering the Jews for yourself.” (2\) “I give you permission to take the money from the Jews and to give it to the men just as you have said.” (3\) “You do not need to pay for the expenses of the plan yourself.” (4\) This statement by the king may also be a formal and cultural way of expressing gratitude for the promised money without actually releasing Haman from paying the money into the treasury.

See: Active or Passive

* **Associated Passages:** Esther 3:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Money (ID: `realia:Money`)

## Esther 3:11 (#2) (id: 262255)

וְ⁠הָ⁠עָ֕ם לַ⁠עֲשׂ֥וֹת בּ֖⁠וֹ כַּ⁠טּ֥וֹב בְּ⁠עֵינֶֽי⁠ךָ

**"and the people, to do with them as is good in your eyes"**

Here, **eyes** represent seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. The king is telling Haman that **he can do whatever he wants to the Jews**. Alternate translation: “you can do to the Jews as you see fit”

See: Metaphor

* **Associated Passages:** Esther 3:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Money (ID: `realia:Money`)

## Esther 3:12 (#1) (id: 262256)

וַ⁠יִּקָּרְאוּ֩ סֹפְרֵ֨י הַ⁠מֶּ֜לֶךְ

**"And the scribes of the king were called"**

You can say this with an active form. You can also say who did the action. It was most likely Haman, since the king had given him authority to act on his behalf. So you could say, “Haman called in the royal scribes.” Alternate translation: “the king summoned his scribes”

See: Active or Passive

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#2) (id: 262257)

בַּ⁠חֹ֣דֶשׁ הָ⁠רִאשׁ֗וֹן בִּ⁠שְׁלוֹשָׁ֨ה עָשָׂ֣ר יוֹם֮ בּ⁠וֹ֒

**"in the first month, on day 13 of it"**

You can add “of that same year” to show that Haman did this right after Ahasuerus agreed to his plan. Alternate translation: “on the thirteenth day of the first month”

See: Hebrew Months

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#3) (id: 262258)

בַּ⁠חֹ֣דֶשׁ הָ⁠רִאשׁ֗וֹן בִּ⁠שְׁלוֹשָׁ֨ה עָשָׂ֣ר יוֹם֮ בּ⁠וֹ֒

**"in the first month, on day 13 of it"**

You can put this information first because it places the event within the timeline of the story.

* **Associated Passages:** Esther 3:12

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#4) (id: 262259)

וַ⁠יִּכָּתֵ֣ב כְּֽ⁠כָל־אֲשֶׁר־צִוָּ֣ה הָמָ֡ן

**"and it was written according to all that Haman commanded"**

You can say this with an active form. You can also say who did the action. Alternate translation: “and Haman dictated a letter to them” or “they wrote a decree containing all that Haman had commanded”

See: Active or Passive

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#5) (id: 262260)

אֲחַשְׁדַּרְפְּנֵֽי־הַ֠⁠מֶּלֶךְ

**"the satraps of the king"**

The satraps of the king were the provincial governors of the empire, distinct from the governors over smaller districts and the leaders of local peoples.

See: Translate Unknowns

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#6) (id: 262261)

מְדִינָ֣ה וּ⁠מְדִינָ֗ה

**"province by province"**

This expression means every province\*\*.\*\* See how you translated this in 1:22\.

See: Idiom

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#7) (id: 262262)

שָׂ֤רֵי עַם֙ וָ⁠עָ֔ם

**"the officials of people by people"**

Alternate translation: “the leaders of each people group”

* **Associated Passages:** Esther 3:12

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#8) (id: 262263)

עַם֙ וָ⁠עָ֔ם

**"people by people"**

This expression means every people group\*\*.\*\* See how you translated this in [1:22](https://ref.ly/Esth1:22).

See: Idiom

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#9) (id: 262264)

מְדִינָ֤ה וּ⁠מְדִינָה֙ כִּ⁠כְתָבָ֔⁠הּ

**"province by province according to its writing"**

See how you translated this in [1:22](https://ref.ly/Esth1:22). Alternate translation: “to each province using its own alphabet” or “written in its own script”

See: Idiom

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#10) (id: 262265)

מְדִינָ֤ה וּ⁠מְדִינָה֙ כִּ⁠כְתָבָ֔⁠הּ

**"province by province according to its writing"**

The implication is that the scribes translated the letter and sent it to each province using its own alphabet. If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#11) (id: 262266)

וְ⁠עַ֥ם וָ⁠עָ֖ם כִּ⁠לְשׁוֹנ֑⁠וֹ

**"and people by people according to its tongue"**

See how you translated this expression in [1:22](https://ref.ly/Esth1:22). Alternate translation: “and to each people group in its own language”

See: Idiom

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#12) (id: 262267)

כִּ⁠לְשׁוֹנ֑⁠וֹ

**"according to its tongue"**

Here, **tongue** is a metonym meaning the language spoken by a person or a group of people. Alternate translation: “according to its language”

See: Metonymy

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#13) (id: 262268)

בְּ⁠שֵׁ֨ם הַ⁠מֶּ֤לֶךְ אֲחַשְׁוֵרֹשׁ֙ נִכְתָּ֔ב

**"In the name of the king Ahasuerus it was written"**

There are two This could mean: (1\) Here, **name** could mean the authority of the king. This would be saying that Haman’s letter had the same authority as it would have had if the king had sent it himself. Alternate translation: “Haman sent the letter under the king’s own authority” (2\) However, this could also mean that Haman actually had the scribes sign the king’s name to the letter. Alternate translation: “they signed the king’s name at the end of the letters”

See: Metaphor

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:12 (#14) (id: 262269)

נִכְתָּ֔ב וְ⁠נֶחְתָּ֖ם בְּ⁠טַבַּ֥עַת הַ⁠מֶּֽלֶךְ

**"it was written, and it was sealed with the signet ring of the king"**

You can say this with an active form, and you can say who did the action. Alternate translation: “to show that he was sending the letter under the king’s own authority, Haman sealed each copy of the letter with the ring that had the king’s official seal on it,” or “the scribes signed the king’s name at the end of the letters. Then they sealed the letters with wax, and stamped the wax by using the king’s ring”

See: Active or Passive

* **Associated Passages:** Esther 3:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 3:13 (#1) (id: 262270)

וְ⁠נִשְׁל֨וֹחַ סְפָרִ֜ים

**"And letters were sent"**

The expression **were sent** is a passive verbal form. If your language would not use that form, you could express the idea in active form or in another way that is natural in your language. Alternate translation: "Haman sent the letters"

See: Active or Passive

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#2) (id: 262271)

בְּ⁠יַ֣ד הָ⁠רָצִים֮

**"by the hand of runners"**

Here, **hand** could mean two different things: (1\) It could literally mean “hand,” meaning that the couriers carried the letters in their hands. Alternate translation: “couriers hand\-delivered the documents” (2\) “Hand” could also be a metaphor for power, control, or authority, meaning that couriers were the ones who delivered the letters to all the provinces throughout the empire. Alternate translation: “couriers gave the documents directly”

See: Metaphor

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#3) (id: 262272)

אֶל־כָּל־מְדִינ֣וֹת הַ⁠מֶּלֶךְ֒

**"to all of the provinces of the king"**

Haman did not send the letters to the provinces (the geographical territories) themselves. Rather, he sent them to the officials in every province in the empire. The story is using the word “province” to refer to these officials by something associated with them, the territories they ruled. Your translation should make clear that the letters were sent to people, not places.

See: Metonymy

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#4) (id: 262273)

לְ⁠הַשְׁמִ֡יד לַ⁠הֲרֹ֣ג וּ⁠לְ⁠אַבֵּ֣ד

**"to annihilate, to slaughter, and to destroy"**

These words mean the same thing. They are used together to emphasize the completeness of the destruction that is envisioned. You could combine them as UST does and say something like “completely destroy.” (A “doublet” can use more than two words.

See: Doublet

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#5) (id: 262274)

מִ⁠נַּ֨עַר וְ⁠עַד־זָקֵ֜ן

**"from young even to old"**

This is a figure of speech that describes all of something by naming two extreme parts of it in order to include everything in between. You could express its meaning as UST does and say something like “every single one of them.” Alternate translation: “including young people and old people”

See: Merism

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#6) (id: 262275)

טַ֤ף וְ⁠נָשִׁים֙

**"children and women"**

Alternate translation: “including the children and the women”

* **Associated Passages:** Esther 3:13

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#7) (id: 262276)

בְּ⁠י֣וֹם אֶחָ֔ד

**"in one day"**

Alternate translation: “on a single day”

See: Numbers

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#8) (id: 262277)

בִּ⁠שְׁלוֹשָׁ֥ה עָשָׂ֛ר

**"on the thirteenth"**

Implicitly, this means “the thirteenth day of the twelfth month of that same year.” If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#9) (id: 262278)

בִּ⁠שְׁלוֹשָׁ֥ה עָשָׂ֛ר

**"on the thirteenth"**

Alternate translation: “on day thirteen”

See: Ordinal Numbers

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#10) (id: 262279)

לְ⁠חֹ֥דֶשׁ שְׁנֵים־עָשָׂ֖ר

**"of month 12"**

Alternate translation: “of month twelve”

See: Ordinal Numbers

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#11) (id: 262280)

**"which is the month of Adar"**

This is the name of the twelfth month of the Hebrew calendar. See how you decided in 2:16 and 3:7 to express dates that the story gives according to the Hebrew calendar.

See: Hebrew Months

* **Associated Passages:** Esther 3:13
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:13 (#12) (id: 262281)

וּ⁠שְׁלָלָ֖⁠ם לָ⁠בֽוֹז

**"and to plunder their spoils"**

This means that the letters also said that those who killed the Jews could take everything that belonged to them.

* **Associated Passages:** Esther 3:13

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 3:14 (#1) (id: 262282)

הַ⁠כְּתָ֗ב לְ⁠הִנָּ֤תֵֽן דָּת֙ בְּ⁠כָל־מְדִינָ֣ה וּ⁠מְדִינָ֔ה

**"the writing, to be given as a law in every province by province"**

You can say this with an active form. For example, you can say, “The officials in every province presented a copy of the letter as the law.”

See: Active or Passive

* **Associated Passages:** Esther 3:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Esther 3:14 (#2) (id: 262283)

בְּ⁠כָל־מְדִינָ֣ה וּ⁠מְדִינָ֔ה

**"in every province by province"**

This expression means each and every province. See how you translated the term “province” in 1:1\.

See: Idiom

* **Associated Passages:** Esther 3:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Esther 3:14 (#3) (id: 262284)

הַ⁠כְּתָ֗ב \& גָּל֖וּי לְ⁠כָל־הָֽ⁠עַמִּ֑ים

**"the writing,"** \- **"was uncovered for all the peoples"**

You can say this with an active form, and you can say who did the action. Alternate translation: “the letter told the officials to post copies where everyone could see them” or “the king commanded the officials to display copies of these letters where all the people could see them”

See: Active or Passive

* **Associated Passages:** Esther 3:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Esther 3:14 (#4) (id: 262285)

לִ⁠הְי֥וֹת עֲתִדִ֖ים לַ⁠יּ֥וֹם הַ⁠זֶּֽה

**"to be ready for this day"**

Alternate translations: “so that people would be ready to do on that day what the king had written in the letter” or “so that they would get ready to do what the letter said to do when the day came”

* **Associated Passages:** Esther 3:14


## Esther 3:15 (#1) (id: 262286)

הָֽ⁠רָצִ֞ים יָצְא֤וּ דְחוּפִים֙ בִּ⁠דְבַ֣ר הַ⁠מֶּ֔לֶךְ

**"The runners went out, hurrying by decree of the king"**

This means that the king had commanded the couriers to deliver the letters as quickly as possible.

* **Associated Passages:** Esther 3:15

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#2) (id: 262287)

הָֽ⁠רָצִ֞ים יָצְא֤וּ דְחוּפִים֙ בִּ⁠דְבַ֣ר הַ⁠מֶּ֔לֶךְ

**"The runners went out, hurrying by decree of the king"**

You can state explicitly where the couriers brought the letters. Alternate translation: “as the king had commanded, couriers rushed the letters to every province in the empire” or “then, according to what the king commanded, men riding horses took these letters quickly to every province in the empire”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#3) (id: 262288)

**"and the law was given"**

You can say this with an active form, and you can say who did the action. For example, you can say, “People read the letter out loud”

See: Active or Passive

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#4) (id: 262289)

בְּ⁠שׁוּשַׁ֣ן הַ⁠בִּירָ֑ה

**"in Susa the citadel"**

Here this likely means in the “capital city of Susa.”

See: Translate Unknowns

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#5) (id: 262290)

יָשְׁב֣וּ לִ⁠שְׁתּ֔וֹת

**"sat down to drink"**

This probably means that Ahasuerus and Haman celebrated with an entire banquet that is being described here by reference to one part of it, the drinks. The phrase **sat down** would simply be describing how the men would have reclined at a table to eat and drink. Alternate translation: "had a celebration feast together"

See: Synecdoche

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#6) (id: 262291)

נָבֽוֹכָה

**"was in confusion"**

The implication is that the people in Susa were very upset about what was going to happen and they did not know what to do about it. If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#7) (id: 262292)

וְ⁠הָ⁠עִ֥יר

**"but the city of"**

The author wants us to see the contrast between the king and Haman sitting down to relax and drink and the people in the city agitated and in an uproar over what the king and Haman had proclaimed. Use a connecting word or other way that your language uses to show a contrast.

See: Connect — Contrast Relationship

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#8) (id: 262293)

וְ⁠הָ⁠עִ֥יר שׁוּשָׁ֖ן

**"but the city of Susa"**

This does not mean the city itself, but the people who live there. This is a figure of speech in which something is called not by its own name, but by the name of something closely associated with it. So this means “everyone who lived in Susa” or the people in Susa

See: Metonymy

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 3:15 (#9) (id: 262294)

נָבֽוֹכָה

**"was in confusion"**

You can translate the abstract noun **confusion** with a verbal phrase such as “was very confused.”

See: Abstract Nouns

* **Associated Passages:** Esther 3:15
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 4:1 (#1) (id: 262295)

וּ⁠מָרְדֳּכַ֗י יָדַע֙

**"Now when Mordecai knew about"**

Here the story re\-introduces Mordecai as the main character in focus. Alternate translation: “when Mordecai found out”

See: Introduction of New and Old Participants

* **Associated Passages:** Esther 4:1
* **Associated Articles:** Introduction of New and Old Participants (ID: `33421@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:1 (#2) (id: 262296)

אֶת־כָּל־אֲשֶׁ֣ר נַעֲשָׂ֔ה

**"all that had been done"**

Alternate translation: “about Haman’s plan to kill all the Jews” or “about those letters”

* **Associated Passages:** Esther 4:1

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:1 (#3) (id: 262297)

וַ⁠יִּקְרַ֤ע מָרְדֳּכַי֙ אֶת־בְּגָדָ֔י⁠ו וַ⁠יִּלְבַּ֥שׁ שַׂ֖ק וָ⁠אֵ֑פֶר

**"Mordecai tore his garments and put on sackcloth and ashes"**

These actions demonstrate extreme sadness.

See: Symbolic Action

* **Associated Passages:** Esther 4:1
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:1 (#4) (id: 262298)

וַ⁠יִּקְרַ֤ע מָרְדֳּכַי֙ אֶת־בְּגָדָ֔י⁠ו וַ⁠יִּלְבַּ֥שׁ שַׂ֖ק וָ⁠אֵ֑פֶר

**"Mordecai tore his garments and put on sackcloth and ashes"**

You can explain why Mordecai did this. Alternate translation: “he was so anguished that he tore his clothes and put on rough sackcloth and threw ashes over himself”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:1 (#5) (id: 262299)

וַ⁠יֵּצֵא֙ בְּ⁠ת֣וֹךְ הָ⁠עִ֔יר

**"And he went out into the midst of the city"**

The next verse shows that Mordecai was heading towards the king’s palace. You could say that explicitly here. Mordecai might have wanted to call attention to the danger that the Jews were in by going to a public location where people were used to seeing him. Or he might have wanted to contact Esther and ask her to help with the trouble. Or he might have wanted both. Alternate translation: “he started walking towards the city center, towards the king’s palace”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:1 (#6) (id: 262300)

וַ⁠יִּזְעַ֛ק זְעָקָ֥ה גְדֹלָ֖ה וּ⁠מָרָֽה

**"and cried out a great and bitter outcry"**

**Great** and **bitter** are two words that refer to the same thing and that work together. They indicate that Mordecai’s cry was so loud because it was so anguished. Alternate translation: “cried out in anguish”

See: Doublet

* **Associated Passages:** Esther 4:1
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:2 (#1) (id: 262301)

וַ⁠יָּב֕וֹא

**"And he came"**

**He** means Mordecai. Alternate translation: “Mordecai came”

* **Associated Passages:** Esther 4:2

* **Associated ACAI Concepts:** Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:2 (#2) (id: 262302)

עַ֖ד לִ⁠פְנֵ֣י שַֽׁעַר־הַ⁠מֶּ֑לֶךְ

**"only as far as before the face of the gate of the king"**

Here, **face** represents the front of an object. The phrase means that Mordecai could not come inside the gate to the king’s palace, but had to wait just outside of it.

See: Metonymy

* **Associated Passages:** Esther 4:2
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:2 (#3) (id: 262303)

שַֽׁעַר־הַ⁠מֶּ֑לֶךְ

**"the gate of the king"**

See how you translated this in [2:19](https://ref.ly/Esth2:19). Alternate translation: “the gate to the king’s palace”

* **Associated Passages:** Esther 4:2

* **Associated ACAI Concepts:** Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:2 (#4) (id: 262304)

אֵ֥ין לָ⁠ב֛וֹא אֶל־שַׁ֥עַר הַ⁠מֶּ֖לֶךְ בִּ⁠לְב֥וּשׁ שָֽׂק

**"no one was to come into the gate of the king while wearing sackcloth"**

If it would be helpful in your language, you could put this information first in the verse because it explains why Mordecai waited outside. See the UST. Alternate translation: “But no one who was wearing sackcloth was allowed inside the palace gates”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 4:2
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:3 (#1) (id: 262305)

וּ⁠בְ⁠כָל־מְדִינָ֣ה וּ⁠מְדִינָ֗ה

**"And in every province by province"**

This expression means in every single province. See how you translated the term “province” in 1:1\. Alternate translation: “in each and every province” or “in every province of the empire”

See: Idiom

* **Associated Passages:** Esther 4:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:3 (#2) (id: 262306)

מְקוֹם֙ אֲשֶׁ֨ר דְּבַר־הַ⁠מֶּ֤לֶךְ וְ⁠דָת⁠וֹ֙ מַגִּ֔יעַ

**"any place where the decree of the king and his law reached"**

Alternate translation: “anywhere that the letter that said to destroy the Jews was announced in public”

* **Associated Passages:** Esther 4:3

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:3 (#3) (id: 262307)

דְּבַר־הַ⁠מֶּ֤לֶךְ וְ⁠דָת⁠וֹ֙

**"the decree of the king and his law"**

These two phrases are referring to the same thing, that is, the information that the letter conveyed. The repetition is used to emphasize how serious a situation this was. Alternate translation: “the letter from the king” or “the law that said to destroy the Jews”

See: Doublet

* **Associated Passages:** Esther 4:3
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:3 (#4) (id: 262308)

אֵ֤בֶל גָּדוֹל֙ לַ⁠יְּהוּדִ֔ים

**"there was great mourning by the Jews"**

Alternate translation: “the Jews mourned greatly” or “when the Jews heard about it, they mourned greatly”

* **Associated Passages:** Esther 4:3

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:3 (#5) (id: 262309)

וְ⁠צ֥וֹם

**"and fasting"**

**Fasting** is a sign of mourning. If your readers would not understand this, you could state it explicitly. Alternate translation: “they went without food because they were so upset”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:3 (#6) (id: 262310)

וּ⁠בְכִ֖י וּ⁠מִסְפֵּ֑ד

**"and weeping and lamenting"**

These two words mean basically the same thing. The repetition is used to emphasize how distressed the Jews were. Alternate translation: “wailed loudly”

See: Doublet

* **Associated Passages:** Esther 4:3
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:3 (#7) (id: 262311)

שַׂ֣ק וָ⁠אֵ֔פֶר יֻצַּ֖ע לָֽ⁠רַבִּֽים

**"sackcloth and ashes were laid out by the multitudes"**

These were actions that demonstrated deep grief. “Many of them lay on the ground, dressed in sackcloth and sitting in ashes”

See: Symbolic Action

* **Associated Passages:** Esther 4:3
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:4 (#1) (id: 262312)

נַעֲר֨וֹת אֶסְתֵּ֤ר

**"the young female attendants of Esther"**

Alternate translation: “Esther’s female servants” of “Esther’s maids”

* **Associated Passages:** Esther 4:4

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:4 (#2) (id: 262313)

וְ⁠סָרִיסֶ֨י⁠הָ֙

**"with her eunuchs and"**

See how you translated this term in [1:10](https://ref.ly/Esth1:10). Alternate translation: “her male servants” or “her other officials”

See: Translate Unknowns

* **Associated Passages:** Esther 4:4
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:4 (#3) (id: 262314)

וַ⁠יַּגִּ֣ידוּ לָ֔⁠הּ

**"they told her"**

That is, they told her that Mordecai was sitting outside the gate wearing sackcloth. If it would be helpful in your language, you could say that explicitly, as in the UST.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:4 (#4) (id: 262315)

וַ⁠תִּתְחַלְחַ֥ל הַ⁠מַּלְכָּ֖ה מְאֹ֑ד

**"even the queen was seized with extreme fear"**

**The queen** means Esther. You can say this with an active form. Alternate translation: “Esther herself became very afraid” or “this made the queen very distressed”

See: Active or Passive

* **Associated Passages:** Esther 4:4
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:4 (#5) (id: 262316)

וַ⁠תִּתְחַלְחַ֥ל הַ⁠מַּלְכָּ֖ה מְאֹ֑ד

**"even the queen was seized with extreme fear"**

The implication is that this happened when she heard what Mordecai was doing. If it would be helpful in your language, you could say that explicitly. Alternate translation: “When she heard about this, Esther herself became very afraid”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:4 (#6) (id: 262317)

וַ⁠תִּשְׁלַ֨ח בְּגָדִ֜ים לְ⁠הַלְבִּ֣ישׁ אֶֽת־מָרְדֳּכַ֗י

**"And she sent garments to clothe Mordecai"**

**She** means Esther. This was her way of encouraging Mordecai to be hopeful and act publicly as if whatever situation he was worried about were not completely desperate. Alternate translation: “she sent servants to take to Mordecai some good clothes to wear”

See: Symbolic Action

* **Associated Passages:** Esther 4:4
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:4 (#7) (id: 262318)

וְ⁠לֹ֥א קִבֵּֽל

**"but he did not accept"**

This was Mordecai’s way of replying to Esther that the situation truly was desperate. Alternate translation: “he refused to put them on”

See: Symbolic Action

* **Associated Passages:** Esther 4:4
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); Sackcloth (ID: `realia:Sackcloth`)

## Esther 4:5 (#1) (id: 262319)

וַ⁠תִּקְרָא֩ אֶסְתֵּ֨ר לַ⁠הֲתָ֜ךְ מִ⁠סָּרִיסֵ֤י הַ⁠מֶּ֨לֶךְ֙

**"And Esther called for Hathak, from the eunuchs of the king"** \- **"he"**

Alternative translation: “then Esther summoned Hathak, one of the king’s officials”

* **Associated Passages:** Esther 4:5

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:5 (#2) (id: 262320)

לַ⁠הֲתָ֜ךְ

**"for Hathak"**

This is a man’s name. It occurs several times in this chapter. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 4:5
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:5 (#3) (id: 262321)

מִ⁠סָּרִיסֵ֤י הַ⁠מֶּ֨לֶךְ֙ אֲשֶׁ֣ר הֶעֱמִ֣יד לְ⁠פָנֶ֔י⁠הָ

**"from the eunuchs of the king whom he had caused to stand before her face"**

Alternate translation: “who was one of the royal guardians whom the king had assigned to serve Esther personally” You can put this information first because it provides background information.

See: Background Information

* **Associated Passages:** Esther 4:5
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:5 (#4) (id: 262322)

**"from the eunuchs of"**

See how you translated this term **eunuch** in [1:10](https://ref.ly/Esth1:10).

See: Translate Unknowns

* **Associated Passages:** Esther 4:5
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:5 (#5) (id: 262323)

לְ⁠פָנֶ֔י⁠הָ

**"before her face"**

Here, **face** represents the presence of a person. This phrase means that the guardians served Esther personally.

See: Metonymy

* **Associated Passages:** Esther 4:5
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:5 (#6) (id: 262324)

לָ⁠דַ֥עַת מַה־זֶּ֖ה וְ⁠עַל־מַה־זֶּֽה

**"to know what this was and on account of what this was"**

These two phrases mean similar things. In both cases, “this” refers to what Mordecai was doing. The repetition is used to emphasize that Esther now knew there must be a very serious reason why Mordecai was wearing sackcloth in such a public place. Alternate translation: “to find out why he was so distressed that he was sitting at the palace gate wearing sackcloth”

See: Parallelism

* **Associated Passages:** Esther 4:5
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:6 (#1) (id: 262325)

וַ⁠יֵּצֵ֥א הֲתָ֖ךְ אֶֽל־מָרְדֳּכָ֑י

**"And Hathak went out to Mordecai"**

Hathak went out specifically to speak with Mordecai and find out why he was so distressed, as Esther had asked him to do. If it would be helpful in your language, you could say that explicitly. Alternate translation: “Hathak went out to speak with Mordecai”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`)

## Esther 4:6 (#2) (id: 262326)

רְח֣וֹב הָ⁠עִ֔יר

**"the open place of the city"**

Alternate translation: “the central plaza”

* **Associated Passages:** Esther 4:6

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`)

## Esther 4:6 (#3) (id: 262327)

לִ⁠פְנֵ֥י

**"before the face of"**

Here, **face** represents the front of an object. The phrase means that the open square was in front of the palace gate.

See: Metonymy

* **Associated Passages:** Esther 4:6
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`)

## Esther 4:6 (#4) (id: 262328)

שַֽׁעַר־הַ⁠מֶּֽלֶךְ

**"the gate of the king"**

The citadel is being described by reference to something associated with it, the gate that leads into it. Alternate translation: “the citadel gate”

See: Metonymy

* **Associated Passages:** Esther 4:6
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`)

## Esther 4:7 (#1) (id: 262329)

אֵ֖ת כָּל־אֲשֶׁ֣ר קָרָ֑⁠הוּ

**"all that had happened to him"**

Here, **him** refers to Mordecai, but here Mordecai represents the entire group of which he is a member. Alternate translation: “everything that Haman was planning to do to the Jews”

See: Synecdoche

* **Associated Passages:** Esther 4:7
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Money (ID: `realia:Money`)

## Esther 4:7 (#2) (id: 262330)

פָּרָשַׁ֣ת הַ⁠כֶּ֗סֶף אֲשֶׁ֨ר אָמַ֤ר הָמָן֙ לִ֠⁠שְׁקוֹל עַל־גִּנְזֵ֥י הַ⁠מֶּ֛לֶךְ בַּיְּהוּדִ֖ים לְ⁠אַבְּדָֽ⁠ם

**"the exact amount of silver that Haman had said to weigh out into the treasuries of the king against the Jews to destroy them"**

This could mean either of two possibilities: (1\) the amount of silver that Haman said the king would get for his treasury from the plunder that the people who destroyed the Jews would take from them, or (2\) the amount of silver that Haman had said he would contribute to pay for the expenses of the plan to destroy the Jews. You could say either one as an alternate translation, but it should agree with your interpretation of [3:9](https://ref.ly/Esth3:9).

* **Associated Passages:** Esther 4:7

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Money (ID: `realia:Money`)

## Esther 4:8 (#1) (id: 262331)

פַּתְשֶׁ֣גֶן כְּתָֽב־הַ֠⁠דָּת

**"a copy of the writing of the law"**

Alternate translation: “a copy of the letter that Haman had sent out” or “a copy of the decree”

* **Associated Passages:** Esther 4:8

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:8 (#2) (id: 262332)

הַ֠⁠דָּת אֲשֶׁר־נִתַּ֨ן בְּ⁠שׁוּשָׁ֤ן

**"the law that had been given in Susa"**

If it would help your readers to understand, you could say this with an active form, and you could say who did the action. Alternate translation: “the law that the heralds had read out loud in Susa” or “the decree that the king’s servants had posted in Susa”

See: Active or Passive

* **Associated Passages:** Esther 4:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:8 (#3) (id: 262333)

לְ⁠הַשְׁמִידָ⁠ם֙

**"to"** \- **"annihilate them"**

Alternative translation: “that said that all the Jews must be killed”

* **Associated Passages:** Esther 4:8

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:8 (#4) (id: 262334)

לְ⁠הַרְא֥וֹת אֶת־אֶסְתֵּ֖ר וּ⁠לְ⁠הַגִּ֣יד לָ֑⁠הּ

**"in order to show Esther and to inform her"**

These two phrases mean similar things. They are repeated to emphasize how important Mordecai knew it was for Esther to understand just what Haman was planning. “so that Esther could see exactly what it said.”

See: Parallelism

* **Associated Passages:** Esther 4:8
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:8 (#5) (id: 262335)

וּ⁠לְ⁠צַוּ֣וֹת עָלֶ֗י⁠הָ

**"and to lay a charge upon her"**

Alternate translation: “and to urge her”

* **Associated Passages:** Esther 4:8

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:8 (#6) (id: 262336)

לְ⁠הִֽתְחַנֶּן־ל֛⁠וֹ וּ⁠לְ⁠בַקֵּ֥שׁ

**"to implore his favor and to seek"**

These two phrases mean similar things. They are repeated to emphasize the force of the action that Mordecai felt was needed. Alternate translation: “to plead with him”

See: Doublet

* **Associated Passages:** Esther 4:8
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:8 (#7) (id: 262337)

מִ⁠לְּ⁠פָנָ֖י⁠ו

**"from before his face"**

Here, **face** represents the presence of a person. In this context, the phrase means that Esther should go into the presence of the king to ask personally for the king’s intervention. Alternate translation: “to go to the king personally”

See: Metonymy

* **Associated Passages:** Esther 4:8
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:8 (#8) (id: 262338)

עַל־עַמָּֽ⁠הּ

**"concerning her people"**

Alternate translation: “to act mercifully toward her people”

* **Associated Passages:** Esther 4:8

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Esther (ID: `person:Esther`)

## Esther 4:10 (#1) (id: 262339)

וַ⁠תֹּ֤אמֶר אֶסְתֵּר֙ לַ⁠הֲתָ֔ךְ וַ⁠תְּצַוֵּ֖⁠הוּ אֶֽל־מָרְדֳּכָֽי

**"And Esther spoke to Hathak and commanded him to say to Mordecai"**

Alternate translation: “then Esther told Hathak to go back to Mordecai and tell him this”

* **Associated Passages:** Esther 4:10

* **Associated ACAI Concepts:** Hathach (ID: `person:Hathach`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:11 (#1) (id: 262340)

כָּל־עַבְדֵ֣י הַ⁠מֶּ֡לֶךְ וְ⁠עַם־מְדִינ֨וֹת הַ⁠מֶּ֜לֶךְ יֽוֹדְעִ֗ים

**"All the servants of the king and the people of the provinces of the king are knowing"**

**All the servants of the king** refers to the royal officials in the palace. **The people of the provinces** refers to those living out in the empire. So this phrase means something like “those both near and far.” Esther is using a figure of speech to refer to something by speaking of two extreme parts of it in order to include everything in between those parts. If it would be helpful in your language, you could combine these phrases and say something like, “everyone in the whole empire knows.” Alternate translation: “all the king’s officials and all the people in the empire know this law”

See: Merism

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)


## Esther 4:11 (#2) (id: 262341)

כָּל־עַבְדֵ֣י הַ⁠מֶּ֡לֶךְ וְ⁠עַם־מְדִינ֨וֹת הַ⁠מֶּ֜לֶךְ יֽוֹדְעִ֗ים

**"All the servants of the king and the people of the provinces of the king are knowing"**

Esther says this to show that there is no uncertainty about this being the law. You can put this information after the description of the law itself, If it would be helpful in your language, so that it can serve to emphasize how definite the law is that was just described.

* **Associated Passages:** Esther 4:11


## Esther 4:11 (#3) (id: 262342)

כָּל־אִ֣ישׁ וְ⁠אִשָּׁ֡ה אֲשֶׁ֣ר

**"for any man or woman who"**

Here, Esther once again refers to a group by describing two parts of it. She means all of the people whom Ahasuerus rules as king. If it would be helpful in your language, you could combine these phrases and say something like “anyone”.

See: Merism

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)


## Esther 4:11 (#4) (id: 262343)

יָבֽוֹא־אֶל־הַ⁠מֶּלֶךְ֩ אֶל־הֶ⁠חָצֵ֨ר הַ⁠פְּנִימִ֜ית

**"goes to the king, to the inner court"**

As [5:1](https://ref.ly/Esth5:1) makes clear, this refers to a courtyard right outside the room where King Ahasuerus sat on his throne. He could see out of the entrance to the room into the courtyard and tell if anyone came and stood there. Anyone who did that was asking to speak to the king. Alternate translation: “who goes into the inner courtyard of the palace, where the king can see them”

See: Symbolic Action

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)


## Esther 4:11 (#5) (id: 262344)

אֲשֶׁ֣ר לֹֽא־יִקָּרֵ֗א

**"who has not been called"**

If it would help your readers to understand, you could say this with an active form, and you could say who did the action. Alternate translation: “when the king has not summoned that person”

See: Active or Passive

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Esther 4:11 (#6) (id: 262345)

אַחַ֤ת דָּת⁠וֹ֙

**"his law is one"**

If it would be helpful in your language, you could put this after the explanation of the law itself. Alternate translation: “this law applies to everyone in the kingdom”

* **Associated Passages:** Esther 4:11


## Esther 4:11 (#7) (id: 262346)

לְ⁠הָמִ֔ית

**"to cause to die"**

Alternate translation: “that person must be executed” or “the guards will kill that person”

* **Associated Passages:** Esther 4:11


## Esther 4:11 (#8) (id: 262347)

לְ֠⁠בַד מֵ⁠אֲשֶׁ֨ר יֽוֹשִׁיט־ל֥⁠וֹ הַ⁠מֶּ֛לֶךְ אֶת־שַׁרְבִ֥יט הַ⁠זָּהָ֖ב

**"apart from when the king holds out to him the scepter of gold"**

If the king pointed his scepter toward a person, that meant that the king was accepting him. Alternate translation: “unless the king extends his golden scepter towards him”

See: Symbolic Action

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)


## Esther 4:11 (#9) (id: 262348)

אֶת־שַׁרְבִ֥יט הַ⁠זָּהָ֖ב

**"the scepter of gold"**

A scepter was an ornamental staff or wand that rulers carried or held as a symbol of their authority. According to this, the scepter of King Ahasuerus was made of gold. Alternate translation: “golden scepter”

See: Translate Unknowns

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)


## Esther 4:11 (#10) (id: 262349)

וְ⁠חָיָ֑ה

**"then he will live"**

This means that the guards will not kill this person and he can proceed to speak to the king. If it would be helpful in your language, you could say this explicitly. Alternate translation: “then the guards will not kill him and he can speak to the king”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Esther 4:11 (#11) (id: 262350)

וַ⁠אֲנִ֗י לֹ֤א נִקְרֵ֨אתי֙ לָ⁠ב֣וֹא אֶל־הַ⁠מֶּ֔לֶךְ זֶ֖ה שְׁלוֹשִׁ֥ים יֽוֹם

**"But as for me, I have not been called to come to the king these 30 days"**

Esther is saying by implication that she cannot speak to the king as Mordecai has requested because the king has not been calling for her, which would have given her an opportunity to speak to him. If she goes without being summoned, she could be put to death. If it would be helpful in your language, you could say this explicitly, as in the UST.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Esther 4:11 (#12) (id: 262351)

וַ⁠אֲנִ֗י

**"But as for me"**

This expression indicates a contrast between Esther’s present situation and the situation she has just described.

See: Connect — Contrast Relationship

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)


## Esther 4:11 (#13) (id: 262352)

זֶ֖ה שְׁלוֹשִׁ֥ים יֽוֹם

**"these 30 days"**

The phrase means that Esther has not been called in the king’s presence for a period of at least 30 days. You could use an equivalent expression in your language that would indicate this is a long enough time that Esther has reason to believe the king is not interested in seeing her. Alternate translation: “in over a month”

* **Associated Passages:** Esther 4:11


## Esther 4:11 (#14) (id: 262353)

זֶ֖ה שְׁלוֹשִׁ֥ים יֽוֹם

**"these 30 days"**

Alternate translation: “thirty days”

See: Numbers

* **Associated Passages:** Esther 4:11
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)


## Esther 4:12 (#1) (id: 262354)

וַ⁠יַּגִּ֣ידוּ לְ⁠מָרְדֳּכָ֔י

**"And they told to Mordecai"**

From this point on, the author focuses only on Mordecai and Esther and does not specify who the messenger is. You can use a general phrase such as, “they told Mordecai” or “Mordecai was told” or “messengers told Mordecai.” If it would be helpful to your readers, you could continue to specify that the messenger was Hathak. The UST models a way to do that.

* **Associated Passages:** Esther 4:12

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:13 (#1) (id: 262355)

וַ⁠יֹּ֥אמֶר מָרְדֳּכַ֖י לְ⁠הָשִׁ֣יב אֶל־אֶסְתֵּ֑ר

**"And Mordecai said to return to Esther"**

Alternate translation: “then Mordecai sent back this message to Esther”

* **Associated Passages:** Esther 4:13

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`); House (ID: `realia:House`)

## Esther 4:14 (#1) (id: 262356)

רֶ֣וַח וְ⁠הַצָּלָ֞ה יַעֲמ֤וֹד לַ⁠יְּהוּדִים֙ מִ⁠מָּק֣וֹם אַחֵ֔ר

**"relief and deliverance will arise for the Jews from another place"**

Here, **relief** and **deliverance** are spoken of as if they are living things that can rise up. Alternate translation: “someone else will rise up from another place and rescue the Jews”

See: Personification

* **Associated Passages:** Esther 4:14
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 4:14 (#2) (id: 262357)

רֶ֣וַח וְ⁠הַצָּלָ֞ה

**"relief and deliverance"**

Here, **relief** and **deliverance** mean very similar things. They are used together to emphasize the great emotion behind being delivered from this great evil. If it works better in your language, you can use one word instead of two, as in the UST.

See: Doublet

* **Associated Passages:** Esther 4:14
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 4:14 (#3) (id: 262358)

וְ⁠אַ֥תְּ וּ⁠בֵית־אָבִ֖י⁠ךְ תֹּאבֵ֑דוּ

**"but you and the house of your father will perish"**

The implication is that the Jews will be rescued starting in that other place, but the ones living in Susa will still be in danger because no one who could have helped them there (such as Esther) would have done anything. Alternate translation: “you and your relatives will be killed”

* **Associated Passages:** Esther 4:14

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 4:14 (#4) (id: 262359)

וּ⁠מִ֣י יוֹדֵ֔עַ אִם־לְ⁠עֵ֣ת כָּ⁠זֹ֔את הִגַּ֖עַתְּ לַ⁠מַּלְכֽוּת

**"And who knows if you have arrived at royalty for such a time as this"**

This is really a statement, and it is about what is happening right then. Mordecai asks it in question form so that Esther will think deeply about her role in this situation. Alternate translation: “who knows, perhaps it was to intervene in this very situation that you became queen”

See: Rhetorical Question

* **Associated Passages:** Esther 4:14
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 4:16 (#1) (id: 262360)

אֶת־כָּל־הַ⁠יְּהוּדִ֜ים הַֽ⁠נִּמְצְאִ֣ים בְּ⁠שׁוּשָׁ֗ן

**"all the Jews who are found in Susa"**

You can say this with an active form. Alternate translation: “all the Jews who live here in Susa”

See: Active or Passive

* **Associated Passages:** Esther 4:16
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 4:16 (#2) (id: 262361)

וְ⁠צ֣וּמוּ עָ֠לַ⁠י

**"And fast on account of me"**

The imperative verb **fast** here is plural, directed to Mordecai and **all the Jews**. Fasting (that is, going without eating) was a symbolic act that the Jews did when they were praying intensely. Since the author of Esther was careful not to mention God or prayer in the book, rather than adding an explanation in the text, you might want to explain the significance of fasting in a footnote. Suggested footnote: "For the Jews, the purpose of fasting was so that they could pray more intensely"

See: Symbolic Action

* **Associated Passages:** Esther 4:16
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 4:16 (#3) (id: 262362)

וְ⁠אַל־תֹּאכְל֨וּ וְ⁠אַל־תִּשְׁתּ֜וּ שְׁלֹ֤שֶׁת יָמִים֙ לַ֣יְלָה וָ⁠י֔וֹם

**"neither eating nor drinking for three days, both night and day"**

This expression means that Esther was asking the Jews in Susa not to eat or drink anything during the day or the night for a period of three days. Alternate translation: “tell them to not eat or drink anything for three days and three nights”

See: Numbers

* **Associated Passages:** Esther 4:16
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 4:16 (#4) (id: 262363)

גַּם־אֲנִ֥י וְ⁠נַעֲרֹתַ֖⁠י אָצ֣וּם כֵּ֑ן

**"Also, I myself and my young female attendants will fast likewise"**

Alternate translation: “my maids and I will also fast”

* **Associated Passages:** Esther 4:16

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 4:16 (#5) (id: 262364)

וּ⁠בְ⁠כֵ֞ן

**"Then, in such circumstances"**

Alternate translation: “after we have all done that, and while still fasting”

* **Associated Passages:** Esther 4:16

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 4:16 (#6) (id: 262365)

אֲשֶׁ֣ר לֹֽא־כַ⁠דָּ֔ת

**"which is not according to the law"**

Alternate translation: “even though there is a law against going without being summoned”

* **Associated Passages:** Esther 4:16

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 4:16 (#7) (id: 262366)

וְ⁠כַ⁠אֲשֶׁ֥ר אָבַ֖דְתִּי אָבָֽדְתִּי

**"And if I perish, I perish"**

Alternate translation: “then if they kill me, they kill me”

* **Associated Passages:** Esther 4:16

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 4:17 (#1) (id: 262367)

וַֽ⁠יַּעֲבֹ֖ר מָרְדֳּכָ֑י

**"And Mordecai went"**

The implication is that Mordecai did this after Hathak brought Esther’s reply back to him. If it would be helpful in your language, you could say this explicitly. Alternate translation: “so after Hathak told this to Mordecai, he went”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 4:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 4:17 (#2) (id: 262368)

כְּ⁠כֹ֛ל אֲשֶׁר־צִוְּתָ֥ה עָלָ֖י⁠ו אֶסְתֵּֽר

**"according to the whole charge that Esther had laid upon him"**

Alternate translation: “everything that Esther had told him to do”

* **Associated Passages:** Esther 4:17

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 5:1 (#1) (id: 262369)

וַ⁠יְהִ֣י ׀ בַּ⁠יּ֣וֹם הַ⁠שְּׁלִישִׁ֗י

**"And it happened on the third day"**

This introduces a new event in the story. Alternate translation: “three days later” or “when Esther had been fasting for three days”

See: Introduction of a New Event

* **Associated Passages:** Esther 5:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Throne (ID: `realia:Throne`); House (ID: `realia:House`)

## Esther 5:1 (#2) (id: 262370)

וַ⁠תִּלְבַּ֤שׁ אֶסְתֵּר֙ מַלְכ֔וּת

**"that Esther put on royalty"**

If your language does not use an abstract noun for the idea behind the word **royalty**, you can express the same idea with an adjective and a concrete noun. Alternate translation: “Esther put on the robes that showed that she was the queen”

See: Abstract Nouns

* **Associated Passages:** Esther 5:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Throne (ID: `realia:Throne`); House (ID: `realia:House`)

## Esther 5:1 (#3) (id: 262371)

וַ⁠תִּלְבַּ֤שׁ אֶסְתֵּר֙ מַלְכ֔וּת

**"that Esther put on royalty"**

It is clear from the story that before she went to see the king, Esther got a banquet ready so that she could invite him and Haman to come to it right away. If it is confusing to leave that out, you can explain that here. Alternate translation, add: “Esther prepared a grand banquet and then put on her royal robes”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Throne (ID: `realia:Throne`); House (ID: `realia:House`)

## Esther 5:1 (#4) (id: 262372)

בַּ⁠חֲצַ֤ר בֵּית־הַ⁠מֶּ֨לֶךְ֙ הַ⁠פְּנִימִ֔ית

**"in the inner court of the palace of the king"**

See how you translated this in [4:11](https://ref.ly/Esth4:11). Alternate translation: “the inner courtyard of the palace”

* **Associated Passages:** Esther 5:1

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Throne (ID: `realia:Throne`); House (ID: `realia:House`)

## Esther 5:1 (#5) (id: 262373)

נֹ֖כַח בֵּ֣ית הַ⁠מֶּ֑לֶךְ

**"opposite to the house of the king"**

Alternate translation: “across from the room where the king was”

* **Associated Passages:** Esther 5:1

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Throne (ID: `realia:Throne`); House (ID: `realia:House`)

## Esther 5:1 (#6) (id: 262374)

וְ֠⁠הַ⁠מֶּלֶךְ יוֹשֵׁ֞ב עַל־כִּסֵּ֤א מַלְכוּת⁠וֹ֙ בְּ⁠בֵ֣ית הַ⁠מַּלְכ֔וּת

**"Now the king was sitting on the throne of his royalty in the palace of royalty"**

**Royalty** is an abstract noun that refers to those things that belonged only to the king. You can translate this idea with a verb, “the throne he ruled from” and “the palace where he reigned,” or with an adjective, “his royal throne” and “his royal palace.” Alternate translation: “in the throne room of the palace, sitting on the king’s throne”

See: Abstract Nouns

* **Associated Passages:** Esther 5:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Throne (ID: `realia:Throne`); House (ID: `realia:House`)

## Esther 5:1 (#7) (id: 262375)

נֹ֖כַח פֶּ֥תַח הַ⁠בָּֽיִת

**"opposite to the entrance of the palace"**

Alternate translation: “facing the entrance of the room”

* **Associated Passages:** Esther 5:1

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Throne (ID: `realia:Throne`); House (ID: `realia:House`)

## Esther 5:2 (#1) (id: 262376)

וַ⁠יְהִי֩

**"And it happened that"**

This expression introduces what happened next. Alternate translation: “Now” or “and”

See: Connect — Sequential Time Relationship

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Connect — Sequential Time Relationship (ID: `33333@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#2) (id: 262377)

כִ⁠רְא֨וֹת

**"as soon as"** \- **"saw"**

Alternate translation: “when … noticed”

* **Associated Passages:** Esther 5:2

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#3) (id: 262378)

נָשְׂאָ֥ה חֵ֖ן בְּ⁠עֵינָ֑י⁠ו

**"she lifted favor in his eyes"**

See how you translated similar expressions in 2:9, 2:15, and 2:17\. Review the explanations there if that would be helpful. In this context, it probably means that King Ahasuerus **thought Esther looked very attractive** as she stood in the court. It could also mean that King Ahasuerus was very pleased to see her. It could mean both. Alternate translation: “he was very pleased to see her” or “he approved of her”

See: Idiom

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#4) (id: 262379)

נָשְׂאָ֥ה חֵ֖ן בְּ⁠עֵינָ֑י⁠ו

**"she lifted favor in his eyes"**

Here, **eyes** represent seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. This phrase means that King Ahasuerus decided not to kill Esther for coming into the inner court without being summoned. Alternate translation: “he was very pleased to see her”

See: Metaphor

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#5) (id: 262380)

וַ⁠יּ֨וֹשֶׁט הַ⁠מֶּ֜לֶךְ לְ⁠אֶסְתֵּ֗ר אֶת־שַׁרְבִ֤יט הַ⁠זָּהָב֙ אֲשֶׁ֣ר בְּ⁠יָד֔⁠וֹ

**"And the king held out to Esther the scepter of gold that was in his hand"**

King Ahasuerus did this to show that he would not enforce the law that said that Esther should be executed for coming into the king’s inner court without being summoned. See how you translated this in 4:11\. Alternate translation: “he extended the gold scepter toward her.”

See: Symbolic Action

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#6) (id: 262381)

וַ⁠יּ֨וֹשֶׁט הַ⁠מֶּ֜לֶךְ לְ⁠אֶסְתֵּ֗ר אֶת־שַׁרְבִ֤יט הַ⁠זָּהָב֙ אֲשֶׁ֣ר בְּ⁠יָד֔⁠וֹ

**"And the king held out to Esther the scepter of gold that was in his hand"**

King Ahasuerus did this to show that he would not enforce the law that said Esther should be executed. If it would be helpful in your language, you could say this explicitly. Alternate translation: “he extended the gold scepter toward her to signal that he would be glad to talk to her.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#7) (id: 262382)

**"so"** \- **"approached"**

Alternate translation; “so … came close” or “so … came up to the throne”

* **Associated Passages:** Esther 5:2

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#8) (id: 262383)

וַ⁠תִּגַּ֖ע בְּ⁠רֹ֥אשׁ הַ⁠שַּׁרְבִֽיט

**"and touched the head of the scepter"**

This action was probably customary for a king’s subjects in a Persian court, when the king extended the royal scepter.

See: Symbolic Action

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#9) (id: 262384)

וַ⁠תִּגַּ֖ע בְּ⁠רֹ֥אשׁ הַ⁠שַּׁרְבִֽיט

**"and touched the head of the scepter"**

This action demonstrated that Esther respected the authority of King Ahasuerus and was thankful for his kindness to her. If it would be helpful for your readers, you could include that information here, such as: “ … to show that she respected his authority.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:2 (#10) (id: 262385)

בְּ⁠רֹ֥אשׁ הַ⁠שַּׁרְבִֽיט

**"the head of the scepter"**

Here, **head** is a metonym meaning the top (or uppermost part) of an object or location. This phrase means that Esther touched the top of the king’s scepter on the end that was extended toward her. Alternate translation: “the top of the scepter” or “the tip of the scepter”

See: Metonymy

* **Associated Passages:** Esther 5:2
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 5:3 (#1) (id: 262386)

מַה־לָּ֖⁠ךְ אֶסְתֵּ֣ר הַ⁠מַּלְכָּ֑ה

**"What is to you, Esther the queen"**

Alternate translation: “What do you want, Queen Esther?”

* **Associated Passages:** Esther 5:3

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 5:3 (#2) (id: 262387)

מַה־לָּ֖⁠ךְ אֶסְתֵּ֣ר הַ⁠מַּלְכָּ֑ה וּ⁠מַה־בַּקָּשָׁתֵ֛⁠ךְ

**"What is to you, Esther the queen? And what is your request"**

These two phrases mean almost the same thing. Ahasuerus says the same thing twice, in slightly different ways, to show that he is very interested in hearing why Esther has come to see him. You do not need to repeat both phrases in your translation if it would be more natural in your language to combine them into one. Alternate translation: “What do you want, Queen Esther?”

See: Parallelism

* **Associated Passages:** Esther 5:3
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 5:3 (#3) (id: 262388)

עַד־חֲצִ֥י הַ⁠מַּלְכ֖וּת וְ⁠יִנָּ֥תֵֽן לָֽ⁠ךְ

**"As much as half of the kingdom, and let it be given to you"**

This was probably a formal and customary expression in the Persian royal court, meant to show honor to the person making a request of the king and to indicate the king’s willingness to be very generous when answering the request. King Ahasuerus was almost certainly not offering to give Esther half of his kingdom. If it would be helpful in your language, you could express the meaning: “I will give you anything you ask for, no matter how great it is.” Alternate translation: “I will give you what you want, even if you ask me to give you half of my kingdom!”

See: Hyperbole

* **Associated Passages:** Esther 5:3
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 5:3 (#4) (id: 262389)

**"and let it be given to you"**

You can say this with an active form, and you can say who will do the action. Alternate translation: “I will give you anything you ask for” or “I will give you what you want”

See: Active or Passive

* **Associated Passages:** Esther 5:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 5:4 (#1) (id: 262390)

**"If it is good to the king"**

This is an idiom. See how you translated it in 1:19 and 3:9\. Alternate translation: “If it seems like a good idea to you, O king”

See: Idiom

* **Associated Passages:** Esther 5:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:4 (#2) (id: 262391)

אִם־עַל־הַ⁠מֶּ֖לֶךְ ט֑וֹב

**"If it is good to the king"**

Esther speaks to the king in the third person as a sign of respect. Alternate translation: “if it seems like a good idea to you, O king”

See: First, Second or Third Person

* **Associated Passages:** Esther 5:4
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:4 (#3) (id: 262392)

יָב֨וֹא הַ⁠מֶּ֤לֶךְ וְ⁠הָמָן֙

**"let the king come with Haman"**

Esther speaks to the king in the third person as a sign of respect. Alternate translation: “O king, please come with Haman” or “you and Haman come”

See: First, Second or Third Person

* **Associated Passages:** Esther 5:4
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:4 (#4) (id: 262393)

**"the banquet that I have made for him"**

Esther speaks to the king in the third person as a sign of respect. Alternate translation: “the banquet that I have prepared for you”

See: First, Second or Third Person

* **Associated Passages:** Esther 5:4
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:4 (#5) (id: 262394)

הַ⁠מִּשְׁתֶּ֖ה אֲשֶׁר־עָשִׂ֥יתִי לֽ⁠וֹ

**"the banquet that I have made for him"**

If you decide to translate this as “the banquet that I have prepared for you,” then the word “you” should be singular, since here it refers just to the king.

See: Forms of ‘You’ — Singular

* **Associated Passages:** Esther 5:4
* **Associated Articles:** Forms of ‘You’ — Singular (ID: `33321@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:5 (#1) (id: 262395)

וַ⁠יֹּ֣אמֶר הַ⁠מֶּ֔לֶךְ

**"And the king said"**

The king would have said this to his servants, not to Esther. If it would be helpful in your language, you could say that explicitly. Alternate translation: “The king said to his servants”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:5 (#2) (id: 262396)

מַהֲרוּ֙ אֶת־הָמָ֔ן לַ⁠עֲשׂ֖וֹת אֶת־דְּבַ֣ר אֶסְתֵּ֑ר

**"Bring Haman quickly in order to do the word of Esther"**

The **word of Esther** means her invitation to the banquet. Alternate translation: “go and get Haman and bring him quickly to the banquet that Esther has prepared”

* **Associated Passages:** Esther 5:5

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:5 (#3) (id: 262397)

**"had made"**

Alternate translation: “had prepared for them”

* **Associated Passages:** Esther 5:5

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:6 (#1) (id: 262398)

מַה־שְּׁאֵלָתֵ֖⁠ךְ וְ⁠יִנָּ֣תֵֽן לָ֑⁠ךְ וּ⁠מַה־בַּקָּשָׁתֵ֛⁠ךְ עַד־חֲצִ֥י הַ⁠מַּלְכ֖וּת וְ⁠תֵעָֽשׂ

**"What is your petition? And let it be given to you. And what is your request? As much as half of the kingdom, and let it be done"**

These two question\-and\-statement pairs mean almost the same thing. Ahasuerus says the same thing twice in slightly different ways to show that he is truly disposed to give Esther whatever she wants. If this is confusing, you do not need to repeat both phrases in your translation. you can combine them into one or replace the repetition with a statement of what the repetition means, as in the UST. Alternate translation: “now please tell me what you really want. I will do it for you”

See: Parallelism

* **Associated Passages:** Esther 5:6
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 5:6 (#2) (id: 262399)

מַה־שְּׁאֵלָתֵ֖⁠ךְ וְ⁠יִנָּ֣תֵֽן לָ֑⁠ךְ וּ⁠מַה־בַּקָּשָׁתֵ֛⁠ךְ עַד־חֲצִ֥י הַ⁠מַּלְכ֖וּת וְ⁠תֵעָֽשׂ

**"What is your petition? And let it be given to you. And what is your request? As much as half of the kingdom, and let it be done"**

When Ahasuerus asked Esther in the throne room what she wanted, she said that she wanted him and Haman to come to a banquet she had prepared. But since the king asks her once again at the banquet what she wants, it is clear that he knew this was not her real request. Instead, he knew that she wanted to show him hospitality first, as a gesture of respect for him and their relationship. You can indicate this by translating, “Now please tell me what you really want.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:6
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 5:6 (#3) (id: 262400)

שְּׁאֵלָתֵ֖⁠ךְ \& בַּקָּשָׁתֵ֛⁠ךְ

**"is your petition?"** \- **"is your request"**

The two abstract nouns **petition** and **request** can be expressed with a single phrase using the verb “want.” Alternate translation: “what you really want”

See: Abstract Nouns

* **Associated Passages:** Esther 5:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 5:6 (#4) (id: 262401)

וְ⁠יִנָּ֣תֵֽן \& וְ⁠תֵעָֽשׂ

**"And let it be given"** \- **"and let it be done"**

You can capture these parallel passive statements with one statement using an active form. Alternate translation: “I will give you anything you ask for”

See: Active or Passive

* **Associated Passages:** Esther 5:6
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 5:6 (#5) (id: 262402)

עַד־חֲצִ֥י הַ⁠מַּלְכ֖וּת

**"As much as half of the kingdom"**

See how you translated this expression in [5:3](https://ref.ly/Esth5:3). Review the explanation there if that would be helpful. Alternate translation: “I will give you anything you ask for, no matter how great it is”

* **Associated Passages:** Esther 5:6

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 5:7 (#1) (id: 262403)

וַ⁠תַּ֥עַן \& וַ⁠תֹּאמַ֑ר

**"And"** \- **"answered and said"**

Here a single idea is being expressed by using two words connected with “and.” Together the two words mean that Esther responded to the question that the king asked her. Alternate translation: “answered” or “replied”

See: Hendiadys

* **Associated Passages:** Esther 5:7
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 5:7 (#2) (id: 262404)

שְׁאֵלָתִ֖⁠י וּ⁠בַקָּשָׁתִֽ⁠י

**"My petition and my request"**

The words **petition** and **request** mean the same thing. Esther probably used these words together as a way of speaking very formally and respectfully to the king. Alternate translation: “this is what I want”

See: Doublet

* **Associated Passages:** Esther 5:7
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 5:7 (#3) (id: 262405)

שְׁאֵלָתִ֖⁠י וּ⁠בַקָּשָׁתִֽ⁠י

**"My petition and my request"**

The two abstract nouns **petition** and **request** can be expressed with a single phrase using the verb “want.” Alternate translation: “this is what I want”

See: Abstract Nouns

* **Associated Passages:** Esther 5:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 5:8 (#1) (id: 262406)

אִם־מָצָ֨אתִי חֵ֜ן בְּ⁠עֵינֵ֣י הַ⁠מֶּ֗לֶךְ

**"if I have found favor in the eyes of the king"**

The expression **find favor** means to gain the approval or acceptance of another person. Alternate translation: “if you are pleased with me”

See: Idiom

* **Associated Passages:** Esther 5:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#2) (id: 262407)

אִם־מָצָ֨אתִי חֵ֜ן בְּ⁠עֵינֵ֣י הַ⁠מֶּ֗לֶךְ

**"if I have found favor in the eyes of the king"**

Here, **eyes** represent seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. The phrase means the King Ahasuerus is pleased with Esther and willing to grant her request. Alternate translation: “if the king evaluates me and approves” or “if the king is pleased with me”

See: Metaphor

* **Associated Passages:** Esther 5:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#3) (id: 262408)

אִם־מָצָ֨אתִי חֵ֜ן בְּ⁠עֵינֵ֣י הַ⁠מֶּ֗לֶךְ

**"if I have found favor in the eyes of the king"**

Esther speaks to the king in the third person as a sign of respect. Alternate translation: “if you evaluate me and approve” or “if you are pleased with me”

See: First, Second or Third Person

* **Associated Passages:** Esther 5:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#4) (id: 262409)

**"and if it is good to the king"**

This is an idiom. See how you translated it in [1:19](https://ref.ly/Esth1:19), [3:9](https://ref.ly/Esth3:9), and [5:4](https://ref.ly/Esth5:4). Alternate translation: “If it seems like a good idea to you, O king”

See: Idiom

* **Associated Passages:** Esther 5:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#5) (id: 262410)

לָ⁠תֵת֙ אֶת־שְׁאֵ֣לָתִ֔⁠י וְ⁠לַ⁠עֲשׂ֖וֹת אֶת־בַּקָּשָׁתִ֑⁠י

**"to grant my petition and to perform my request"**

The phrases **to grant my petition** and **to perform my request** mean the same thing. Alternate translation: “to do what I want”

See: Doublet

* **Associated Passages:** Esther 5:8
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#6) (id: 262411)

יָב֧וֹא הַ⁠מֶּ֣לֶךְ וְ⁠הָמָ֗ן אֶל־הַ⁠מִּשְׁתֶּה֙ אֲשֶׁ֣ר אֶֽעֱשֶׂ֣ה לָ⁠הֶ֔ם

**"let the king come with Haman to the banquet that I will make for them"**

Esther speaks to the king in the third person as a sign of respect. Alternate translation: “please come with Haman to another banquet that I will prepare for you”

See: First, Second or Third Person

* **Associated Passages:** Esther 5:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#7) (id: 262412)

יָב֧וֹא הַ⁠מֶּ֣לֶךְ וְ⁠הָמָ֗ן אֶל־הַ⁠מִּשְׁתֶּה֙ אֲשֶׁ֣ר אֶֽעֱשֶׂ֣ה לָ⁠הֶ֔ם

**"let the king come with Haman to the banquet that I will make for them"**

If you decide to translate this as “the banquet that I have prepared for you,” then the word “you” should be dual if your language uses that form, since it refers to the king and Haman, otherwise it should be plural.

See: Forms of ‘You’ — Dual/Plural

* **Associated Passages:** Esther 5:8
* **Associated Articles:** Forms of ‘You’ — Dual/Plural (ID: `33319@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#8) (id: 262413)

וּ⁠מָחָ֥ר אֶֽעֱשֶׂ֖ה כִּ⁠דְבַ֥ר הַ⁠מֶּֽלֶךְ

**"and tomorrow I will do according to the word of the king"**

The **word of the king** means his request for Esther to tell him what she wants. Alternate translation: “I will answer your question then”

* **Associated Passages:** Esther 5:8

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:8 (#9) (id: 262414)

כִּ⁠דְבַ֥ר הַ⁠מֶּֽלֶךְ

**"according to the word of the king"**

Esther speaks to the king in the third person as a sign of respect. Alternate translation: “according to your word”

See: First, Second or Third Person

* **Associated Passages:** Esther 5:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Haman (ID: `person:Haman`)

## Esther 5:9 (#1) (id: 262415)

וַ⁠יֵּצֵ֤א

**"And"** \- **"went out"**

Alternate translation: “and … left the banquet”

* **Associated Passages:** Esther 5:9

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:9 (#2) (id: 262416)

שָׂמֵ֖חַ וְ⁠ט֣וֹב לֵ֑ב

**"joyful and pleased of heart"**

The word **joyful** and the phrase **good of heart** mean the same thing. These terms are used together to show that Haman was very happy after attending Esther’s banquet with King Ahasuerus. Alternate translation: “feeling very good”

See: Doublet

* **Associated Passages:** Esther 5:9
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:9 (#3) (id: 262417)

וְ⁠ט֣וֹב לֵ֑ב

**"and pleased of heart"**

Here, **heart** represents the action of thinking or feeling. The phrase means that Haman felt happy.

See: Metaphor

* **Associated Passages:** Esther 5:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:9 (#4) (id: 262418)

וְ⁠לֹא־קָם֙

**"that he neither rose up"**

Under other circumstances, officials were expected to bow low to the ground to show respect for Haman. See [3:2](https://ref.ly/Esth3:2). But since Mordecai was already sitting on the ground as a sign of mourning, he would have been expected to stand up to show respect. If it would be helpful in your language, you could say this explicitly. Alternate translation: “Mordecai did not stand up to show respect for Haman”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:9 (#5) (id: 262419)

וְ⁠לֹא־זָ֣ע מִמֶּ֔⁠נּוּ

**"nor trembled from him"**

The phrase means that Mordecai was not afraid of Haman or of what Haman might do to him for refusing to show the respect that the king had commanded. Alternate translation: "nor showed any fear"

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:9 (#6) (id: 262420)

וַ⁠יִּמָּלֵ֥א הָמָ֛ן עַֽל־מָרְדֳּכַ֖י חֵמָֽה

**"then Haman was filled with rage on account of Mordecai"**

Here, **Haman’s rage** is described as something that could fill him up. See how you translated this expression in [3:5](https://ref.ly/Esth3:5). Alternate translation: “Haman became very angry because of Mordecai”

See: Metaphor

* **Associated Passages:** Esther 5:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:10 (#1) (id: 262421)

וַ⁠יִּתְאַפַּ֣ק הָמָ֔ן

**"But Haman restrained himself"**

There is a contrast between this sentence and the previous one. Haman felt very much like doing one thing, but he decided it would be better to do something else. The contrast can be indicated with a phrase such as “but even though.” Alternate translation: “but even though Haman was so angry, he kept himself from losing his temper” or “Haman refused to show how angry he was”

See: Connect — Contrast Relationship

* **Associated Passages:** Esther 5:10
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 5:10 (#2) (id: 262422)

זֶ֥רֶשׁ

**"Zeresh"**

This is a woman’s name. It occurs several times in this chapter and the next. Be sure to translate it consistently.

See: How to Translate Names

* **Associated Passages:** Esther 5:10
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 5:11 (#1) (id: 262423)

וַ⁠יְסַפֵּ֨ר לָ⁠הֶ֥ם

**"And"** \- **"recounted to them"**

Alternate translation: “then … made a long speech to them about”

* **Associated Passages:** Esther 5:11

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:11 (#2) (id: 262424)

אֶת־כְּב֥וֹד עָשְׁר֖⁠וֹ

**"the glory of his wealth"**

**Glory** and **wealth** are both abstract nouns. Alternate translation: “Haman told them about how great his wealth was” or “Haman told them about the many great things he owned”

See: Abstract Nouns

* **Associated Passages:** Esther 5:11
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:11 (#3) (id: 262425)

וְ⁠רֹ֣ב בָּנָ֑י⁠ו

**"and the multitude of his sons"**

Having many sons was probably seen as a mark of honor and status in Persian culture. This phrase probably means that Haman was boasting about the status he enjoyed in Persian society because he had many sons. Alternate translation: “how many sons he had”

See: Symbolic Action

* **Associated Passages:** Esther 5:11
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:11 (#4) (id: 262426)

בָּנָ֑י⁠ו

**"his sons"**

This word can sometimes be used in Hebrew to mean both male and female children. However, here it clearly refers to male children, so you should translate it that way.

See: When Masculine Words Include Women

* **Associated Passages:** Esther 5:11
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:11 (#5) (id: 262427)

גִּדְּל֤⁠וֹ

**"had made him great"**

Alternate translation: “how the king had continually given him more authority” or “how the king had honored him many times by giving him more important work”

* **Associated Passages:** Esther 5:11

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:11 (#6) (id: 262428)

נִשְּׂא֔⁠וֹ עַל

**"he had lifted him over"**

Here, **lifting** is a metaphor meaning to “advance” or “promote” a person to a higher, more important position than they had previously. Alternate translation: “given him a position more important than”

See: Metaphor

* **Associated Passages:** Esther 5:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:11 (#7) (id: 262429)

הַ⁠שָּׂרִ֖ים וְ⁠עַבְדֵ֥י הַ⁠מֶּֽלֶךְ

**"the officials and administrators of the king"**

This means all of the others, as [3:1](https://ref.ly/Esth3:1) makes clear. Alternate translation: “all of his other officials and administrators”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:11 (#8) (id: 262430)

הַ⁠שָּׂרִ֖ים וְ⁠עַבְדֵ֥י

**"the officials and administrators of the king"**

The word translated as **officials** refers to leaders in a high position and the word translated as **administrators** is more general, referring to everyone who serves the king. Alternate translation: "the leaders and other assistants of"

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Haman (ID: `person:Haman`)

## Esther 5:12 (#1) (id: 262431)

אַ֣ף

**"In addition"**

You can translate it as an exclamation because Haman believes he has saved the best for last and is now introducing what he considers to be his most recent significant honor. Alternate translation: “and that is not all”

See: Exclamations

* **Associated Passages:** Esther 5:12
* **Associated Articles:** Exclamations (ID: `33270@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:12 (#2) (id: 262432)

לֹא־הֵבִיאָה֩ אֶסְתֵּ֨ר הַ⁠מַּלְכָּ֧ה עִם־הַ⁠מֶּ֛לֶךְ \& כִּ֣י אִם־אוֹתִ֑⁠י

**"Esther the queen did not bring anyone with the king"** \- **"except me"**

This can be expressed positively. Alternate translation: “I was the only one Queen Esther invited besides the king” or “Queen Esther invited just two of us, the king and me”

See: Double Negatives

* **Associated Passages:** Esther 5:12
* **Associated Articles:** Double Negatives (ID: `33265@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:12 (#3) (id: 262433)

עָשָׂ֖תָה

**"she had made"**

Alternate translation: “she had prepared”

* **Associated Passages:** Esther 5:12

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:12 (#4) (id: 262434)

וְ⁠גַם־לְ⁠מָחָ֛ר אֲנִ֥י קָֽרוּא־לָ֖⁠הּ עִם־הַ⁠מֶּֽלֶךְ

**"And also for tomorrow, I am called by her with the king"**

You can say this with an active form. Alternate translation: “and she has invited me to attend another banquet with the king again tomorrow”

See: Active or Passive

* **Associated Passages:** Esther 5:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 5:13 (#1) (id: 262435)

וְ⁠כָל־זֶ֕ה אֵינֶ֥⁠נּוּ שֹׁוֶ֖ה לִ֑⁠י בְּ⁠כָל־עֵ֗ת אֲשֶׁ֨ר אֲנִ֤י רֹאֶה֙

**"But all of this is not equal for me to every time that I see"**

This is a figurative way of speaking. It is as if Haman is weighing his happiness on one side of a scale and his unhappiness on the other side. Haman is saying that his happiness over all the honors he has received is still not greater than his unhappiness over seeing Mordecai fail to honor him, so on balance, he is unhappy. Alternate translation: “I still cannot be happy as long as I keep seeing” or “all of this does not make me happy” or “ … does not satisfy me”

See: Metaphor

* **Associated Passages:** Esther 5:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:13 (#2) (id: 262436)

וְ⁠כָל־זֶ֕ה

**"But all of this"**

Haman is continuing to speak here. You can show this by beginning with “Then Haman said.”

* **Associated Passages:** Esther 5:13

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:13 (#3) (id: 262437)

הַ⁠יְּהוּדִ֔י

**"the Jew"**

This phrase gives clarifying information about Mordecai.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 5:13
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:13 (#4) (id: 262438)

יוֹשֵׁ֖ב בְּ⁠שַׁ֥עַר הַ⁠מֶּֽלֶךְ

**"sitting at the gate of the king"**

Haman could be referring to the way Mordecai has been refusing to stand up to honor him, or he could be referring generally to the way Mordecai has never honored him (by either bowing or standing up) ever since he was appointed to a position in the king’s service and stationed at the gate. Either way, being reminded of this dishonor is the reason why Haman continues to be unhappy. If it would be helpful in your language, you could say this explicitly. Alternate translation: “sitting at the citadel gate and refusing to honor me”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:13 (#5) (id: 262439)

בְּ⁠שַׁ֥עַר הַ⁠מֶּֽלֶךְ

**"at the gate of the king"**

Alternate translation: “the citadel gate”

* **Associated Passages:** Esther 5:13

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 5:14 (#1) (id: 262440)

וַ⁠תֹּ֣אמֶר ל⁠וֹ֩ זֶ֨רֶשׁ אִשְׁתּ֜⁠וֹ וְ⁠כָל־אֹֽהֲבָ֗י⁠ו

**"And Zeresh his wife, with all of his friends, said to him"**

The verb is feminine, so it is Zeresh who is speaking here. By mentioning the friends, it is implied that they are in agreement with what she says. If it would be helpful in your language, you could say, “Then Haman’s wife Zeresh suggested, and his friends agreed.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#2) (id: 262441)

וְ⁠כָל־אֹֽהֲבָ֗י⁠ו

**"with all of his friends"**

This means the friends whom Haman had invited to his home that day. Alternate translation: “his friends who were there”

* **Associated Passages:** Esther 5:14

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#3) (id: 262442)

יַֽעֲשׂוּ

**"Let them make"**

You can say who would do the action. Alternate translation: “have your servants set up”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#4) (id: 262443)

עֵץ֮

**"a pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful. Be sure your translation is consistent in the chapters that follow.

See: Translate Unknowns

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#5) (id: 262444)

גָּבֹ֣הַּ חֲמִשִּׁ֣ים אַמָּה֒

**"50 cubits high"**

You can convert this to a modern measure if that is the style of translation that you are using. Alternate translation: “seventy\-five feet high” or “twenty\-five meters high”

See: Biblical Distance

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Biblical Distance (ID: `33369@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#6) (id: 262445)

אֱמֹ֣ר לַ⁠מֶּ֗לֶךְ וְ⁠יִתְל֤וּ אֶֽת־מָרְדֳּכַי֙ עָלָ֔י⁠ו

**"speak to the king, and let them hang Mordecai on it"**

If it would be helpful in your language, you could make explicit what Haman would say to the king. Alternate translation: “speak to the king and tell him that you would like to hang Mordecai on it” or “tell the king that you intend to hang Mordecai on it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#7) (id: 262446)

וְ⁠יִתְל֤וּ אֶֽת־מָרְדֳּכַי֙ עָלָ֔י⁠ו

**"and let them hang Mordecai on it"**

If it would be helpful in your language, you could make explicit who does the action. Probably, Haman’s servants would carry out the action. But since it would happen under Haman’s orders and perhaps under his supervision, you could also describe him as the one who does the action if that would be natural in your language. Alternate translation: “and have your servants hang Mordecai on it” or “and hang Mordecai on it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#8) (id: 262447)

וּ⁠בֹֽא \& אֶל הַ⁠מִּשְׁתֶּ֖ה שָׂמֵ֑חַ

**"Then go"** \- **"to the banquet joyful"**

**Then** indicates more than a time sequence here. Zeresh and the friends are saying what the result would be. Alternate translation: “after he is dead, you will be able to go to the banquet … in a good mood”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#9) (id: 262448)

וַ⁠יִּיטַ֧ב הַ⁠דָּבָ֛ר לִ⁠פְנֵ֥י הָמָ֖ן

**"And the word was good before the face of Haman"**

The **word** was the suggestion that Zeresh and the friends made. Here, **face** represents Haman himself by something associated with him, his face, which showed what he was thinking and feeling. Alternate translation: “Haman decided that he liked this idea” or “that suggestion pleased Haman”

See: Metonymy

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#10) (id: 262449)

וַ⁠יַּ֥עַשׂ הָ⁠עֵֽץ

**"and he made the pole"**

Haman most likely did not do this personally. He ordered it to be done. Alternate translation: “he gave his servants orders to set up the pole” or “he told his servants to construct the gallows”

* **Associated Passages:** Esther 5:14

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 5:14 (#11) (id: 262450)

הָ⁠עֵֽץ

**"the pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful. Be sure your translation is consistent in the chapters that follow.

See: Translate Unknowns

* **Associated Passages:** Esther 5:14
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`)

## Esther 6:1 (#1) (id: 262451)

בַּ⁠לַּ֣יְלָה הַ⁠ה֔וּא

**"That night"**

This introduces a new event in the story. Use a natural way of introducing a new event in your language.

See: Introduction of a New Event

* **Associated Passages:** Esther 6:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 6:1 (#2) (id: 262452)

נָדְדָ֖ה שְׁנַ֣ת הַ⁠מֶּ֑לֶךְ

**"the sleep of the king fled away"**

Here, **sleep** is spoken of as if it were a living thing that could flee away. Alternate translation: “the king could not sleep”

See: Personification

* **Associated Passages:** Esther 6:1
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 6:1 (#3) (id: 262453)

וַ⁠יֹּ֗אמֶר לְ⁠הָבִ֞יא

**"And he said to bring"**

**He** means the king. The implication is that he told the young men who attended him to do this. Verses 3 and 5 indicate this. If it would be helpful in your language, you could say this explicitly. Alternate translation: “so he commanded his servants to fetch”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 6:1 (#4) (id: 262454)

אֶת־סֵ֤פֶר הַ⁠זִּכְרֹנוֹת֙ דִּבְרֵ֣י הַ⁠יָּמִ֔ים

**"the book of records of the events of days"**

This is equivalent to the expression “the book of the events of days” in [2:23](https://ref.ly/Esth2:23). See how you translated that expression. Alternate translation: “the royal chronicles” or “the records of what had happened during all the time that he had been king”

* **Associated Passages:** Esther 6:1

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 6:1 (#5) (id: 262455)

וַ⁠יִּהְי֥וּ נִקְרָאִ֖ים

**"and they were being called out"**

This means that the chronicles were read out loud. You can say this with an active form, and you can say who did the action. Alternate translation: “the young men got the chronicles and began to read them out loud”

See: Active or Passive

* **Associated Passages:** Esther 6:1
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 6:1 (#6) (id: 262456)

לִ⁠פְנֵ֥י הַ⁠מֶּֽלֶךְ

**"before the face of the king"**

Here, **face** represents the presence of a person. This phrase means that the book was being read in the king’s presence. Alternate translation: “to the king”

See: Metonymy

* **Associated Passages:** Esther 6:1
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Scroll (ID: `realia:Scroll`)

## Esther 6:2 (#1) (id: 262457)

וַ⁠יִּמָּצֵ֣א כָת֗וּב

**"And it was found written"**

Both terms **found** and **recorded** can be expressed in active form. Alternate translation: “they found that the writers had recorded there” or “they read the part where the writers had written”

See: Active or Passive

* **Associated Passages:** Esther 6:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:2 (#2) (id: 262458)

הִגִּ֨יד מָרְדֳּכַ֜י עַל

**"Mordecai had told about"**

The implication is that Mordecai had saved the king’s life by discovering the plot and telling him about it. If it would be helpful in your language, you could say this explicitly. Alternate translation: “Mordecai had saved the king’s life by discovering a plot against the king and telling him about it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:2 (#3) (id: 262459)

הִגִּ֨יד מָרְדֳּכַ֜י עַל

**"Mordecai had told about"**

To relate the events in chronological order, you can put this at the end of the verse. Alternate translation: “but Mordecai had saved the king’s life by discovering their plot and telling him about it”

See: Order of Events

* **Associated Passages:** Esther 6:2
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:2 (#4) (id: 262460)

בִּגְתָ֣נָא וָ⁠תֶ֗רֶשׁ

**"Bigthana and Teresh"**

These are the same two men who are named in [2:21](https://ref.ly/Esth2:21). However, there the first man’s name is said to be “Bigthan.” Decide on a consistent way of translating these names so your readers will know that they are the same men.

See: How to Translate Names

* **Associated Passages:** Esther 6:2
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:2 (#5) (id: 262461)

שְׁנֵי֙ סָרִיסֵ֣י הַ⁠מֶּ֔לֶךְ

**"two of the eunuchs of the king"**

See how you translated the term **eunuch** in [1:10](https://ref.ly/Esth1:10). Alternate translation: “two of the royal guardians”

See: Translate Unknowns

* **Associated Passages:** Esther 6:2
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:2 (#6) (id: 262462)

מִ⁠שֹּׁמְרֵ֖י הַ⁠סַּ֑ף

**"who were from the guardians of the doorway"**

See how you translated this expression in [2:23](https://ref.ly/Esth2:23). Alternate translation: “who protected the doorway to the king’s private quarters”

* **Associated Passages:** Esther 6:2

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:2 (#7) (id: 262463)

בִּקְשׁוּ֙

**"they had sought"**

Here, **seeking** means actively trying to do something. Alternate translation: “they had planned” or “they had tried”

See: Metaphor

* **Associated Passages:** Esther 6:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:2 (#8) (id: 262464)

לִ⁠שְׁלֹ֣חַ יָ֔ד

**"to stretch out a hand"**

As in [2:21](https://ref.ly/Esth2:21), the expression **to stretch out a hand** means to cause someone physical harm with the intention of killing them. Alternate translation: “to assassinate” or “to kill”

See: Idiom

* **Associated Passages:** Esther 6:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Bigthan (ID: `person:Bigthan`); Teresh (ID: `person:Teresh`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Doorstep (ID: `realia:Doorstep`)

## Esther 6:3 (#1) (id: 262465)

מַֽה־נַּעֲשָׂ֞ה יְקָ֧ר וּ⁠גְדוּלָּ֛ה לְ⁠מָרְדֳּכַ֖י

**"What honor or greatness was done for Mordecai"**

**Honor** and **greatness** have similar meanings and they are used together to emphasize that Mordecai should certainly have been honored in some grand way for saving the king’s life. If it would be helpful in your language, you could express this by putting the terms together. For example, you could say, “What great honor was done for Mordecai?” Alternate translation: “what did we do to reward Mordecai or to show that we appreciated what he did”

See: Hendiadys

* **Associated Passages:** Esther 6:3
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Mordecai (ID: `person:Mordecai.2`)

## Esther 6:3 (#2) (id: 262466)

נַּעֲשָׂ֞ה

**"was done"**

You can say this with an active form. Alternate translation: “what did I do” or “what did we do”

See: Active or Passive

* **Associated Passages:** Esther 6:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Mordecai (ID: `person:Mordecai.2`)

## Esther 6:3 (#3) (id: 262467)

לֹא־נַעֲשָׂ֥ה עִמּ֖⁠וֹ דָּבָֽר

**"Not a thing has been done with him"**

You can say this with an active form. However, it would be good to say this in a way that does not create the impression that the young men are accusing the king. Alternate translation: “no one did anything for him”

See: Active or Passive

* **Associated Passages:** Esther 6:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Mordecai (ID: `person:Mordecai.2`)

## Esther 6:3 (#4) (id: 262468)

עַל־זֶ֑ה

**"on account of this"**

Alternate translation: “for saving my life”

* **Associated Passages:** Esther 6:3

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Mordecai (ID: `person:Mordecai.2`)

## Esther 6:4 (#1) (id: 262469)

מִ֣י בֶ⁠חָצֵ֑ר

**"Who is in the court"**

This could mean either: (1\) The king knows that there is someone in the courtyard, maybe because he heard someone walking there; he wants to know who it is. (2\) Since there are always people around the palace, the king assumes that there must be someone out in the courtyard. In either case, the king wants to know if there is someone there with whom he could consult about the best way to honor Mordecai. As the story has already said in [1:13](https://ref.ly/Esth1:13), it was the king’s habit to consult his advisors on important questions. If it would be helpful in your language, you could say this explicitly. Alternate translation: “the king wanted to consult someone about the best way to honor Mordecai, so he asked, ‘who is in the courtyard’”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:4 (#2) (id: 262470)

מִ֣י בֶ⁠חָצֵ֑ר

**"Who is in the court"**

In order to present the events in logical and chronological order, you can put the king’s question last in the verse if that would be more natural in your language. Haman had already come in the courtyard by the time the king asked this question.

* **Associated Passages:** Esther 6:4

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:4 (#3) (id: 262471)

וְ⁠הָמָ֣ן בָּ֗א לַ⁠חֲצַ֤ר בֵּית־הַ⁠מֶּ֨לֶךְ֙ הַ⁠חִ֣יצוֹנָ֔ה לֵ⁠אמֹ֣ר לַ⁠מֶּ֔לֶךְ לִ⁠תְלוֹת֙ אֶֽת־מָרְדֳּכַ֔י עַל־הָ⁠עֵ֖ץ אֲשֶׁר־הֵכִ֥ין לֽ⁠וֹ

**"Now Haman had come into the outer court of the house of the king to say to the king to hang Mordecai on the pole that he had prepared for him"**

This sentence indicates that Haman had already entered the outer court when King Ahasuerus asked his question. You could indicate this with a phrase such as “at that moment” or “just then” or “while they were talking”

See: Connect — Background Information

* **Associated Passages:** Esther 6:4
* **Associated Articles:** Connect — Background Information (ID: `33332@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:4 (#4) (id: 262472)

לַ⁠חֲצַ֤ר בֵּית־הַ⁠מֶּ֨לֶךְ֙ הַ⁠חִ֣יצוֹנָ֔ה

**"into the outer court of the house of the king"**

This phrase is describing the first courtyard a person would come to after entering the palace from the outside. Alternate translation: “the outer courtyard of the palace”

* **Associated Passages:** Esther 6:4

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:4 (#5) (id: 262473)

לֵ⁠אמֹ֣ר לַ⁠מֶּ֔לֶךְ לִ⁠תְלוֹת֙ אֶֽת־מָרְדֳּכַ֔י

**"to say to the king to hang Mordecai"**

Alternate translation: “to tell the king that he wanted to hang Mordecai” or “to tell the king that he wanted to impale Mordecai”

* **Associated Passages:** Esther 6:4

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:4 (#6) (id: 262474)

הָ⁠עֵ֖ץ אֲשֶׁר־הֵכִ֥ין לֽ⁠וֹ

**"the pole that he had prepared for him"**

Alternate translation: “on the pole that he had set up” or “on the gallows that he had set up for Mordecai”

* **Associated Passages:** Esther 6:4

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:4 (#7) (id: 262475)

הָ⁠עֵ֖ץ

**"the pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful. Be sure your translation is consistent in the chapters that follow.

See: Translate Unknowns

* **Associated Passages:** Esther 6:4
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:5 (#1) (id: 262476)

וַ⁠יֹּ֨אמְר֜וּ נַעֲרֵ֤י הַ⁠מֶּ֨לֶךְ֙

**"And the young men of the king said"**

It is implied that the young men went and looked, perhaps through a window or a doorway, to see who might be in the courtyard before they answered the king. If it would be helpful in your language, you can say so. Alternate translation: “the young men who served the king looked and saw that Haman was in the courtyard. They said”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Haman (ID: `person:Haman`)

## Esther 6:5 (#2) (id: 262477)

הִנֵּ֥ה

**"Behold"**

This is a term meant to focus the attention of the listener on what the speaker is about to say. If there is not a good way to translate this term in your language, this term can be omitted from the translation. You could also have the young men address the king directly to get his attention. Alternate translation: “O king”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Haman (ID: `person:Haman`)

## Esther 6:5 (#3) (id: 262478)

יָבֽוֹא

**"Let him come"**

Alternate translation: “he may come in”

* **Associated Passages:** Esther 6:5

* **Associated ACAI Concepts:** Servants (ID: `group:GenericServants`); Haman (ID: `person:Haman`)

## Esther 6:6 (#1) (id: 262479)

וַ⁠יָּבוֹא֮ הָמָן֒ וַ⁠יֹּ֤אמֶר ל⁠וֹ֙ הַ⁠מֶּ֔לֶךְ

**"And Haman came, and the king said to him"**

Alternate translation: “when Haman came in, the king said”

* **Associated Passages:** Esther 6:6

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`)

## Esther 6:6 (#2) (id: 262480)

מַה־לַ⁠עֲשׂ֕וֹת בָּ⁠אִ֕ישׁ אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֣ץ בִּ⁠יקָר֑⁠וֹ

**"What is one to do for the man in whose honor the king is delighted"**

Here the king speaks of himself in the third person. If it would be helpful in your language, you could use the first person instead. Alternate translation: “what should I do for the man whom I would really like to honor”

See: First, Second or Third Person

* **Associated Passages:** Esther 6:6
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`)

## Esther 6:6 (#3) (id: 262481)

אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֣ץ בִּ⁠יקָר֑⁠וֹ

**"in whose honor the king is delighted"**

**To delight** in doing something means to be glad to do it and to really want to do it. Alternate translation: “whom I am glad to honor” or “whom I want to honor”

See: Idiom

* **Associated Passages:** Esther 6:6
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`)

## Esther 6:6 (#4) (id: 262482)

וַ⁠יֹּ֤אמֶר הָמָן֙ בְּ⁠לִבּ֔⁠וֹ

**"Then Haman said in his heart"**

Here, **heart** could mean two different things: (1\) Heart could represent the action of thinking or feeling. In that case, the phrase would mean “then Haman said in his thoughts.” (2\) Heart could also be a metaphor meaning to be inside someone or something. In that case, the phrase would mean “then Haman said inside himself.” Either way, the phrase means that Haman was not saying this out loud, but saying it to himself in his thoughts. Alternate translation: “Haman thought to himself”

See: Metaphor

* **Associated Passages:** Esther 6:6
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`)

## Esther 6:6 (#5) (id: 262483)

לְ⁠מִ֞י יַחְפֹּ֥ץ הַ⁠מֶּ֛לֶךְ לַ⁠עֲשׂ֥וֹת יְקָ֖ר יוֹתֵ֥ר מִמֶּֽ⁠נִּי

**"For whom would the king delight to do honor more than me"**

Haman is actually making a statement to himself rather than asking a question and trying to figure out the answer. He uses a question form to emphasize how true he believes the statement to be. If it would be helpful in your language, instead of a question, you could use a statement such as, “Surely there is no one whom the king would take pleasure in honoring more than me!” Alternate translation: “whom would the king like to honor more than me”

See: Rhetorical Question

* **Associated Passages:** Esther 6:6
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`)

## Esther 6:7 (#1) (id: 262484)

אִ֕ישׁ אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֥ץ בִּ⁠יקָרֽ⁠וֹ

**"For a man whom the king delights to honor"**

**To delight** in doing something means to be glad to do it and to really want to do it. Alternate translation: “for a man the king is glad to honor” or “for a man the king wants to honor”

See: Idiom

* **Associated Passages:** Esther 6:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`)

## Esther 6:7 (#2) (id: 262485)

אִ֕ישׁ אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֥ץ בִּ⁠יקָרֽ⁠וֹ

**"For a man whom the king delights to honor"**

Haman speaks to the king in the third person as a sign of respect. You can have him address the king in the second person. Alternate translation: “if you really want to honor someone”

See: First, Second or Third Person

* **Associated Passages:** Esther 6:7
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`)

## Esther 6:8 (#1) (id: 262486)

יָבִ֨יאוּ֙

**"let them bring"**

The implication is that the king’s servants would do this. If it would be helpful in your language, you could say this explicitly. Alternate translation: “let someone bring” or “tell your servants to bring”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`); Crown (ID: `realia:Crown`)

## Esther 6:8 (#2) (id: 262487)

לְב֣וּשׁ מַלְכ֔וּת אֲשֶׁ֥ר לָֽבַשׁ־בּ֖⁠וֹ הַ⁠מֶּ֑לֶךְ

**"a garment of royalty with which the king has clothed himself"**

**Royalty** is an abstract noun that refers to anything that belongs to the king. You can translate this idea with an adjective, “a royal garment.” Alternate translation: “one of your own royal robes that you have already worn yourself” or “one of the robes you wear to show that you are the king”

See: Abstract Nouns

* **Associated Passages:** Esther 6:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`); Crown (ID: `realia:Crown`)

## Esther 6:8 (#3) (id: 262488)

אֲשֶׁ֥ר לָֽבַשׁ־בּ֖⁠וֹ הַ⁠מֶּ֑לֶךְ

**"with which the king has clothed himself"**

Haman speaks to the king in the third person as a sign of respect. You can have him address the king in the second person. Alternate translation: “that you have already worn yourself”

See: First, Second or Third Person

* **Associated Passages:** Esther 6:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`); Crown (ID: `realia:Crown`)

## Esther 6:8 (#4) (id: 262489)

וְ⁠ס֗וּס אֲשֶׁ֨ר רָכַ֤ב עָלָי⁠ו֙

**"and a horse on which"** \- **"has ridden"**

Haman speaks to the king in the third person as a sign of respect. You can have him address the king in the second person. Alternate translation: “a horse that you have already ridden yourself”

See: First, Second or Third Person

* **Associated Passages:** Esther 6:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`); Crown (ID: `realia:Crown`)

## Esther 6:8 (#5) (id: 262490)

וַ⁠אֲשֶׁ֥ר נִתַּ֛ן כֶּ֥תֶר מַלְכ֖וּת בְּ⁠רֹאשֽׁ⁠וֹ

**"and which has been given a crown of royalty for its head"**

This whole phrase refers to the horse. You can say this with an active form. Alternate translation: “and that has a royal crown on its head”

See: Active or Passive

* **Associated Passages:** Esther 6:8
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`); Crown (ID: `realia:Crown`)

## Esther 6:8 (#6) (id: 262491)

וַ⁠אֲשֶׁ֥ר נִתַּ֛ן כֶּ֥תֶר מַלְכ֖וּת בְּ⁠רֹאשֽׁ⁠וֹ

**"and which has been given a crown of royalty for its head"**

The implication is that this would be done to show that this was one of the king’s own horses. If it would be helpful in your language, you could say that explicitly. Alternate translation: “and that has a royal crown on its head to show that it belongs to you”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`); Crown (ID: `realia:Crown`)

## Esther 6:8 (#7) (id: 262492)

כֶּ֥תֶר מַלְכ֖וּת

**"a crown of royalty"**

**Royalty** is an abstract noun that refers to the royal authority that the king exercised. You can translate this idea with an adjective, “a royal crown.”

See: Abstract Nouns

* **Associated Passages:** Esther 6:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`); Crown (ID: `realia:Crown`)

## Esther 6:9 (#1) (id: 262493)

וְ⁠נָת֨וֹן הַ⁠לְּב֜וּשׁ וְ⁠הַ⁠סּ֗וּס עַל־יַד־אִ֞ישׁ מִ⁠שָּׂרֵ֤י הַ⁠מֶּ֨לֶךְ֙

**"And give this garment and this horse into the hand of a man from the"** \- **"officials of the king"**

Here, **hand** is a metaphor meaning power, control, or authority. It appears that the king himself would not perform such acts of service to one of his subjects even if he really wanted to honor that person. So Haman is saying that on behalf of the king, representing his authority and power, one of the king’s most noble officials should present the man with the robe and the horse. Alternate translation: “then, on your behalf, have one of your most noble officials present the man with the robe and the horse”

See: Metaphor

* **Associated Passages:** Esther 6:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#2) (id: 262494)

אִ֞ישׁ מִ⁠שָּׂרֵ֤י הַ⁠מֶּ֨לֶךְ֙ הַֽ⁠פַּרְתְּמִ֔ים

**"a man from the most noble officials of the king"**

Haman speaks to the king in the third person as a sign of respect. If it would be helpful in your language, you could use the second person. Alternate translation: “one of your most noble officials”

See: First, Second or Third Person

* **Associated Passages:** Esther 6:9
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#3) (id: 262495)

וְ⁠הִלְבִּ֨ישׁוּ֙

**"And let them clothe"**

Since **them** is plural, it likely refers to the king’s servants. If it would be helpful in your language, you can say this explicitly. Alternate translation: “have your servants clothe … with the robe”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#4) (id: 262496)

אֶת־הָ⁠אִ֔ישׁ אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֣ץ בִּֽ⁠יקָר֑⁠וֹ

**"the man whom the king delights to honor"**

**To delight** in doing something means to be glad to do something or to want to do something. Alternate translation: “the man whom you really want to honor”

See: Idiom

* **Associated Passages:** Esther 6:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#5) (id: 262497)

אֶת־הָ⁠אִ֔ישׁ אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֣ץ בִּֽ⁠יקָר֑⁠וֹ

**"the man whom the king delights to honor"**

Haman speaks to the king in the third person as a sign of respect. If it would be helpful in your language, you could use the second person. Alternate translation: “the man whom you really want to honor”

See: First, Second or Third Person

* **Associated Passages:** Esther 6:9
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#6) (id: 262498)

בִּ⁠רְח֣וֹב הָ⁠עִ֔יר

**"in the open square of the city"**

This refers to a large, open space in the city, either the space that was in front of the palace (as in [4:6](https://ref.ly/Esth4:6)), or a space inside one of the city gates. In either case, this would have been a public area where people could gather, and so it was a place where a great number of people would have seen how the king was honoring Mordecai. Alternate translation: “through the public square of the city”

* **Associated Passages:** Esther 6:9

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#7) (id: 262499)

וְ⁠קָרְא֣וּ

**"and proclaim"**

Alternate translation: “and tell the noble official and servants to proclaim” or “and have them shout out”

* **Associated Passages:** Esther 6:9

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#8) (id: 262500)

לְ⁠פָנָ֔י⁠ו

**"before his face"**

Here, **face** represents the front of a person. The phrase means that the servants were to shout out to the people in front of the man on the horse, that is, to the people who were in the path of his horse as it approached. Alternate translation: “to whoever is in front of them”

See: Metonymy

* **Associated Passages:** Esther 6:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#9) (id: 262501)

כָּ֚כָה יֵעָשֶׂ֣ה לָ⁠אִ֔ישׁ אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֥ץ בִּ⁠יקָרֽ⁠וֹ

**"Thus shall it be done for the man whom the king delights to honor"**

**To delight** in doing something means to be glad to do something or to want to do something. Alternate translation: “the king is doing this because he really wants to honor this man” or “this is what the king does when he especially wants to honor someone”

See: Idiom

* **Associated Passages:** Esther 6:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:9 (#10) (id: 262502)

כָּ֚כָה יֵעָשֶׂ֣ה

**"Thus shall it be done"**

You can say this with an active form, and you can say who is doing the action. Alternate translation: “the king is doing this” or “this is what the king does”

See: Active or Passive

* **Associated Passages:** Esther 6:9
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Horse (ID: `fauna:Horse`)

## Esther 6:10 (#1) (id: 262503)

וַ⁠יֹּ֨אמֶר הַ⁠מֶּ֜לֶךְ

**"And the king said"**

The implication is that the king liked Haman’s suggestion. If it would be helpful in your language, you could say this explicitly. Alternate translation: “the king liked this idea, so he replied”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:10 (#2) (id: 262504)

כַּ⁠אֲשֶׁ֣ר דִּבַּ֔רְתָּ וַֽ⁠עֲשֵׂה־כֵן֙

**"according to how you have spoken, and do thus"**

Alternate translation: “do everything that you have said” or “do just what you have suggested”

* **Associated Passages:** Esther 6:10

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:10 (#3) (id: 262505)

הַ⁠יּוֹשֵׁ֖ב בְּ⁠שַׁ֣עַר הַ⁠מֶּ֑לֶךְ

**"the man who sits at the gate of the king"**

As in [2:19](https://ref.ly/Esth2:19), this is likely an idiom which means that Mordecai worked for the king in some capacity and was stationed at the gate. Alternate translation: “one of my servants, who is stationed at the king’s gate”

See: Idiom

* **Associated Passages:** Esther 6:10
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:10 (#4) (id: 262506)

הַ⁠יּוֹשֵׁ֖ב בְּ⁠שַׁ֣עַר הַ⁠מֶּ֑לֶךְ

**"the man who sits at the gate of the king"**

Here the king speaks of himself in the third person. If it would be helpful in your language, you could use the first person. Alternate translation: “one of my servants, who is stationed at the gate to the palace”

See: First, Second or Third Person

* **Associated Passages:** Esther 6:10
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:10 (#5) (id: 262507)

אַל־תַּפֵּ֣ל דָּבָ֔ר

**"Do not allow a word to fall"**

Here the action of letting **fall** is a metaphor meaning to leave out or leave unfulfilled. The phrase means that Haman must do absolutely everything that he said. Alternate translation: “do not leave out anything that you suggested”

See: Metaphor

* **Associated Passages:** Esther 6:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:10 (#6) (id: 262508)

אַל־תַּפֵּ֣ל דָּבָ֔ר

**"Do not allow a word to fall"**

This can be stated positively. Alternate translation: “Be sure to do absolutely everything that you have said”

See: Double Negatives

* **Associated Passages:** Esther 6:10
* **Associated Articles:** Double Negatives (ID: `33265@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:11 (#1) (id: 262509)

וַ⁠יַּרְכִּיבֵ֨⁠הוּ֙ בִּ⁠רְח֣וֹב הָ⁠עִ֔יר

**"and caused him to ride in the open place of the city"**

See how you translated these expressions in verse 9\. Alternate translation: “seated him on the horse and then led the horse through the public square of the city”

* **Associated Passages:** Esther 6:11

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:11 (#2) (id: 262510)

וַ⁠יִּקְרָ֣א לְ⁠פָנָ֔י⁠ו

**"and called out before his face"**

Here, **face** represents the front of a person. The phrase means that Haman shouted out to the people in front of Mordecai, that is, the people who were in the path of his horse as it approached. Alternate translation: “called out to all of the people in front of him”

See: Metonymy

* **Associated Passages:** Esther 6:11
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:11 (#3) (id: 262511)

אֲשֶׁ֥ר הַ⁠מֶּ֖לֶךְ חָפֵ֥ץ בִּ⁠יקָרֽ⁠וֹ

**"whom the king delights to honor"**

**To delight** in doing something means to be glad to do something or to want to do something. Alternate translation: “the king is doing this because he really wants to honor this man”

See: Idiom

* **Associated Passages:** Esther 6:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Horse (ID: `fauna:Horse`)

## Esther 6:12 (#1) (id: 262512)

אָבֵ֖ל וַ⁠חֲפ֥וּי רֹֽאשׁ

**"mourning with a covering of head"**

People often covered their heads to show that they were either extremely sad or ashamed. Alternate translation: “covering his head because he felt completely disgraced”

See: Symbolic Action

* **Associated Passages:** Esther 6:12
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 6:13 (#1) (id: 262513)

וַ⁠יְסַפֵּ֨ר הָמָ֜ן

**"And Haman recounted"**

The implication is that, just as Haman gathered his friends together in [5:10](https://ref.ly/Esth5:10) to boast of his status, now he gathered them together once again to talk about what had just happened. If it would be helpful in your language, you could say this explicitly. Alternate translation: “Haman brought all his friends together again. He told … ”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#2) (id: 262514)

אֵ֖ת כָּל־אֲשֶׁ֣ר קָרָ֑⁠הוּ

**"everything that had happened to him"**

The implication is that he told about the things that had happened to him “that day.” If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#3) (id: 262515)

וַ⁠יֹּ֩אמְרוּ֩ ל֨⁠וֹ חֲכָמָ֜י⁠ו וְ⁠זֶ֣רֶשׁ

**"Then his wise men and Zeresh"** \- **"said to him"**

The implication is that, just as King Ahasuerus had royal advisors, Haman had advisors of his own. They would have been included among the “friends” he gathered for this occasion. If it would be helpful in your language, you could say this explicitly. Alternate translation: “some of his friends were also his advisors, and they and his wife Zeresh told him”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#4) (id: 262516)

חֲכָמָ֜י⁠ו

**"his wise men"**

See how you translated this expression in [1:13](https://ref.ly/Esth1:13). Alternate translation: “his advisors”

* **Associated Passages:** Esther 6:13

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#5) (id: 262517)

אִ֣ם \& מָרְדֳּכַ֞י אֲשֶׁר֩ הַחִלּ֨וֹתָ לִ⁠נְפֹּ֤ל לְ⁠פָנָי⁠ו֙

**"If Mordecai, before whose face you have begun to fall,"** \- **"then"**

This could mean either of two possibilities: (1\) The action of falling could refer to bowing down and it could be a metaphor meaning to be dishonored and defeated. Haman wanted Mordecai to bow down in front of him. But instead, the story would be saying that Haman is starting to bow down (in a metaphorical sense) in front of Mordecai. Alternate translation: “Mordecai has begun to humiliate you. If he … ” (2\) “Fall” could be a figurative way of saying “be killed in battle.” In that case, Zeresh would be comparing Haman and Mordecai to two soldiers who are fighting. She is saying that Haman has already “begun to fall,” that is, he is losing the battle, and he is likely to be killed. Alternate translation: “Mordecai has begun to defeat you. Since he … ”

See: Metaphor

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#6) (id: 262518)

לְ⁠פָנָי⁠ו֙

**"before"** \- **"face"**

Here, **face** represents the front of a person. This could mean: (1\) that Haman has already begun to fall down (bow down) in front of Mordecai figuratively, or (2\) that Haman and Mordecai are facing one another in hand\-to\-hand combat. Alternate translation: “before whom” or “in whose presence”

See: Metonymy

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#7) (id: 262519)

מִ⁠זֶּ֣רַע הַ⁠יְּהוּדִ֡ים

**"is from the seed of the Jews"**

"Here, **seed** means ""descendants."" If it would be helpful in your language, you could use an equivalent expression or express the meaning plainly. Alternate translation: “is among the descendants of the Jews”"

See: Idiom

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#8) (id: 262520)

לֹא־תוּכַ֣ל ל֔⁠וֹ

**"you will not prevail against him"**

Here Haman’s wife and friends are again speaking as if Haman is in a battle with Mordecai. Alternate translation: “you will not win against him” or “you will not have greater honor than he has”

See: Metaphor

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#9) (id: 262521)

כִּֽי

**"but"**

This conjunction indicates a contrast between the actions of prevailing (in the previous clause) and falling.

See: Connect — Contrast Relationship

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#10) (id: 262522)

נָפ֥וֹל תִּפּ֖וֹל

**"you will surely fall"**

Here the action of **falling**: (1\) either refers once again to prostrating oneself, and is a metaphor meaning to be dishonored and defeated, or (2\) Haman’s wife and friends are saying that for Haman, defeat will be the outcome of the figurative battle. Alternate translation: “you will certainly lose to him”

See: Metaphor

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:13 (#11) (id: 262523)

לְ⁠פָנָֽי⁠ו

**"before his face"**

Here, **face** represents the front of a person. The phrase means that Haman will lose in his struggle with Mordecai. Alternate translation: “before him” or “in his presence”

See: Metonymy

* **Associated Passages:** Esther 6:13
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Zeresh (ID: `person:Zeresh`); Love (ID: `keyterm:Love`); Mordecai (ID: `person:Mordecai.2`); A Group of People (ID: `group:GenericGroup`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 6:14 (#1) (id: 262524)

עוֹדָ⁠ם֙ מְדַבְּרִ֣ים עִמּ֔⁠וֹ

**"They were still speaking with him"**

This phrase indicates something that happened after Haman’s wife and friends started talking with Haman and while they were still talking. You should indicate this with a phrase such as “while.” Alternate translation: “while they were still talking together, the king’s eunuchs arrived”

See: Connect — Background Information

* **Associated Passages:** Esther 6:14
* **Associated Articles:** Connect — Background Information (ID: `33332@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 6:14 (#2) (id: 262525)

וְ⁠סָרִיסֵ֥י הַ⁠מֶּ֖לֶךְ

**"when the eunuchs of the king"**

See how you decided to translate this term in [1:10](https://ref.ly/Esth1:10). Not all of the king’s eunuchs came. This can be stated explicitly. Alternate translation: “some of the king’s servants”

See: Translate Unknowns

* **Associated Passages:** Esther 6:14
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 6:14 (#3) (id: 262526)

וַ⁠יַּבְהִ֨לוּ֙ לְ⁠הָבִ֣יא אֶת־הָמָ֔ן

**"And they hurried to bring Haman"**

Alternative translation: “they wanted to bring Haman quickly”

* **Associated Passages:** Esther 6:14

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 6:14 (#4) (id: 262527)

אֲשֶׁר־עָשְׂתָ֥ה אֶסְתֵּֽר

**"that Esther had made"**

Esther’s servants would have done most of the work of preparing the banquet. This can be stated explicitly. Alternate translation: “that Esther and her servants had prepared”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 6:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 7:1 (#1) (id: 262528)

וַ⁠יָּבֹ֤א הַ⁠מֶּ֨לֶךְ֙ וְ⁠הָמָ֔ן לִ⁠שְׁתּ֖וֹת עִם־אֶסְתֵּ֥ר הַ⁠מַּלְכָּֽה

**"And the king came with Haman to drink with Esther the queen"**

Esther hosted Ahasuerus and Haman for more than just drinks. This is a figure of speech in which a part of something is used to mean the whole thing. An entire banquet is being described here by reference to one part of it, the drinks. Alternate translation: “so the king and Haman went to the second banquet that Queen Esther had prepared”

See: Synecdoche

* **Associated Passages:** Esther 7:1
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 7:2 (#1) (id: 262529)

בַּ⁠יּ֤וֹם הַ⁠שֵּׁנִי֙

**"on the second day"**

This refers to the day after the first banquet, as mentioned in [5:8](https://ref.ly/Esth5:8). Alternate translation: “at that second banquet” or “on that second day of feasting”

See: Ordinal Numbers

* **Associated Passages:** Esther 7:2
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:2 (#2) (id: 262530)

בְּ⁠מִשְׁתֵּ֣ה הַ⁠יַּ֔יִן

**"during the banquet of wine"**

Alternate translation: “while they were drinking wine” or “while the servants were pouring the wine and giving it to them”

* **Associated Passages:** Esther 7:2

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:2 (#3) (id: 262531)

מַה־שְּׁאֵלָתֵ֛⁠ךְ \& וְ⁠תִנָּ֣תֵֽן לָ֑⁠ךְ וּ⁠מַה־בַּקָּשָׁתֵ֛⁠ךְ עַד־חֲצִ֥י הַ⁠מַּלְכ֖וּת וְ⁠תֵעָֽשׂ

**"What is your petition,"** \- **"And let it be given to you. And what is your request? As much as half of the kingdom, and let it be done"**

These two statements mean almost the same thing. Ahasuerus says the same thing twice, in slightly different ways, to show that he is truly disposed to giving Esther what she wants. You do not need to repeat both phrases in your translation if that would be confusing for your readers. Alternate translation: “now please tell me what you really want. Tell me, and I will do it for you”

See: Parallelism

* **Associated Passages:** Esther 7:2
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:2 (#4) (id: 262532)

מַה־שְּׁאֵלָתֵ֛⁠ךְ

**"What is your petition"**

Ahasuerus has now allowed Esther to show her hospitality at two banquets. But he knows that she has done this as a gesture of respect for him and their relationship, and that this signals that she has an important and urgent request to make. You can indicate this by having the king ask, “Now please tell me what you really want.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 7:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:2 (#5) (id: 262533)

וְ⁠תִנָּ֣תֵֽן \& וְ⁠תֵעָֽשׂ

**"And let it be given"** \- **"and let it be done"**

You can capture these parallel passive statements with one statement using an active form. Alternate translation: “I will give you what you ask for” or “I will do for you what you ask”

See: Active or Passive

* **Associated Passages:** Esther 7:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:2 (#6) (id: 262534)

שְּׁאֵלָתֵ֛⁠ךְ \& בַּקָּשָׁתֵ֛⁠ךְ

**"is your petition,"** \- **"is your request"**

The two abstract nouns **petition** and **request** can be expressed with a single phrase using the verb “want.” Alternate translation: “what you really want”

See: Abstract Nouns

* **Associated Passages:** Esther 7:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:2 (#7) (id: 262535)

**"As much as half of the kingdom"**

See how you translated this expression in [5:3](https://ref.ly/Esth5:3) and [5:6](https://ref.ly/Esth5:6). Alternate translation: “I will give you anything you ask for, no matter how great it is”

See: Hyperbole

* **Associated Passages:** Esther 7:2
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:3 (#1) (id: 262536)

**"And"** \- **"answered and said"**

Here a single idea is being expressed by using two words connected with “and.” Together the two words mean that Esther responded to the question that the king asked her. Alternate translation: “answered”

See: Hendiadys

* **Associated Passages:** Esther 7:3

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#2) (id: 262537)

אִם־מָצָ֨אתִי חֵ֤ן בְּ⁠עֵינֶ֨י⁠ךָ֙

**"If I have found favor in your eyes"**

**Favor** means the approval or acceptance of another person. Alternate translation: “if you evaluate me and approve” or “if you are pleased with me”

See: Idiom

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#3) (id: 262538)

אִם־מָצָ֨אתִי חֵ֤ן בְּ⁠עֵינֶ֨י⁠ךָ֙

**"If I have found favor in your eyes"**

Here, **eyes** represent seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. Alternate translation: “if you evaluate me and approve” or “if you are pleased with me”

See: Metonymy

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#4) (id: 262539)

הַ⁠מֶּ֔לֶךְ

**"O king"**

In this case, Esther is addressing Ahasuerus directly as “O king”; she is not speaking about him in the third person as a sign of respect, as she and others usually do elsewhere in the story. This may indicate a special urgency to what she is saying, so it would be good to include this difference in your translation or indicate the urgency in some other way.

* **Associated Passages:** Esther 7:3

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#5) (id: 262540)

**"and if it is good to the king"**

This is an idiom that also appears in [1:19](https://ref.ly/Esth1:19), [3:9](https://ref.ly/Esth3:9), [5:4](https://ref.ly/Esth5:4), and [5:8](https://ref.ly/Esth5:8). The suggested translation in those cases was, “If it seems like a good plan to you, O king.” That was suitable for the deferential tone the speakers were using. But here, Esther has just addressed the king directly with “O king” and she is pleading urgently for the survival of her whole people. So an alternate translation here could be something more like, “I hope you will be willing to do what I ask”

See: Idiom

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#6) (id: 262541)

תִּנָּֽתֶן־לִ֤⁠י נַפְשִׁ⁠י֙ בִּ⁠שְׁאֵ֣לָתִ֔⁠י וְ⁠עַמִּ֖⁠י בְּ⁠בַקָּשָׁתִֽ⁠י

**"let my life be given to me at my petition and my people at my request"**

The phrases **at my petition** and **at my request** mean the same thing. The repetition is used to emphasize the urgency of the request. If it would be more natural in your language, you could combine the two requests. Alternate translation: “please spare my life and save my people” or “my request is that you spare my life and the lives of my people”

See: Parallelism

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#7) (id: 262542)

תִּנָּֽתֶן־לִ֤⁠י נַפְשִׁ⁠י֙

**"let my life be given to me"**

You can say this with an active form. Alternate translation: “please spare my life”

See: Active or Passive

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#8) (id: 262543)

בִּ⁠שְׁאֵ֣לָתִ֔⁠י

**"at my petition"**

The abstract noun **petition** can be expressed with the verb “ask for.” Alternate translation: “that is what I am asking for” or “that is what I want”

See: Abstract Nouns

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#9) (id: 262544)

וְ⁠עַמִּ֖⁠י

**"and my people"**

If this is unclear in your language, you can repeat from the previous clause the words that have been left out. Alternate translation: “and let my people be given to me”

See: Ellipsis

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:3 (#10) (id: 262545)

**"at my request"**

The abstract noun **request** can be expressed with the verb “ask for.” Alternate translation: “that is what I am asking for” or “that is what I want”

See: Abstract Nouns

* **Associated Passages:** Esther 7:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Favor (ID: `keyterm:Favor`); Esther (ID: `person:Esther`)

## Esther 7:4 (#1) (id: 262546)

כִּ֤י נִמְכַּ֨רְנוּ֙

**"For we have been sold"**

As Esther points out later in the verse, the Jews actually have not been exchanged for money. Rather, **sell** is a figurative way of saying “turn over to.” If it would be helpful in your language, you could express this meaning by saying something like “For someone has turned us over to our enemies.” Alternatively, you could use the same figure, but show that it is a comparison. Alternate translation: “it is as though I and my people are cattle that have been sold to be slaughtered”

See: Metaphor

* **Associated Passages:** Esther 7:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## Esther 7:4 (#2) (id: 262547)

כִּ֤י נִמְכַּ֨רְנוּ֙

**"For we have been sold"**

You can say this with an active form. Alternate translation: “for someone has sold us” or “for someone has put us in danger of our enemies”

See: Active or Passive

* **Associated Passages:** Esther 7:4
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## Esther 7:4 (#3) (id: 262548)

לְ⁠הַשְׁמִ֖יד לַ⁠הֲר֣וֹג וּ⁠לְ⁠אַבֵּ֑ד

**"for annihilation, for slaughter, and for destruction"**

As in [3:13](https://ref.ly/Esth3:13), these words mean the same thing and are used together to emphasize the completeness of the destruction. If using three words would be difficult or confusing in your language, you could use one word for this and indicate that the destruction is extreme. Alternate translation: “and they are going to kill every one of us” (A “doublet” can involve the use of more than two words.)

See: Doublet

* **Associated Passages:** Esther 7:4
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)


## Esther 7:4 (#4) (id: 262549)

לְ⁠הַשְׁמִ֖יד לַ⁠הֲר֣וֹג וּ⁠לְ⁠אַבֵּ֑ד

**"for annihilation, for slaughter, and for destruction"**

If your language does not use abstract nouns such as these, you can replace them with one or more verbs. Alternate translation: “and they are going to kill every one of us”

See: Abstract Nouns

* **Associated Passages:** Esther 7:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## Esther 7:4 (#5) (id: 262550)

וְ֠⁠אִלּוּ לַ⁠עֲבָדִ֨ים וְ⁠לִ⁠שְׁפָח֤וֹת נִמְכַּ֨רְנוּ֙

**"Now if we had been sold as slaves and female servants"**

Alternate translation: “if the men and even the women had been sold into slavery”

* **Associated Passages:** Esther 7:4


## Esther 7:4 (#6) (id: 262551)

אֵ֥ין הַ⁠צָּ֛ר שֹׁוֶ֖ה בְּ⁠נֵ֥זֶק הַ⁠מֶּֽלֶךְ

**"the distress would not have been equal to a burden of the king"**

Esther is speaking very deferentially and with some exaggeration in order to impress on the king how important this is to her. She is saying that it would have been worth less to her to see her people rescued from slavery than it would have been to see the king spared the trouble of intervening on their behalf. (Haman uses the expression of “not equal” in this same sense in [5:13](https://ref.ly/Esth5:13).) Alternate translation: “that would have been a matter too small to bother you with” or “that would be a small matter to a king like you, but this is very much worse”

See: Hyperbole

* **Associated Passages:** Esther 7:4
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)


## Esther 7:4 (#7) (id: 262552)

הַ⁠מֶּֽלֶךְ

**"the king"**

Esther addresses King Ahasuerus in the third person as a way of showing respect. Alternate translation: “you”

See: First, Second or Third Person

* **Associated Passages:** Esther 7:4
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)


## Esther 7:5 (#1) (id: 262553)

**"And"** \- **"spoke and said"**

These two words mean the same thing. Alternate translation: “responded”

See: Doublet

* **Associated Passages:** Esther 7:5
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 7:5 (#2) (id: 262554)

מִ֣י ה֥וּא זֶה֙ וְ⁠אֵֽי־זֶ֣ה ה֔וּא

**"Who is he, this man? Where is this man, he"**

These two phrases mean similar things. They both ask about the identity of the man who is trying to destroy the Jews. King Ahasuerus uses the repetition to emphasize how outraged he is. If the repetition would be confusing in your language, you could combine the two phrases. However, from another perspective, both the man’s identity and his location are important to know so that his plot can be stopped. So, for that reason, you could also decide to include both phrases in your translation.

See: Parallelism

* **Associated Passages:** Esther 7:5
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 7:5 (#3) (id: 262555)

מִ֣י ה֥וּא זֶה֙ וְ⁠אֵֽי־זֶ֣ה ה֔וּא

**"Who is he, this man? Where is this man, he"**

Alternate translation: “who has done this”

* **Associated Passages:** Esther 7:5

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 7:5 (#4) (id: 262556)

וְ⁠אֵֽי־זֶ֣ה ה֔וּא אֲשֶׁר־מְלָא֥⁠וֹ לִבּ֖⁠וֹ לַ⁠עֲשׂ֥וֹת כֵּֽן

**"Where is this man, he whose heart is full to do thus"**

Here the concept of a **full heart** uses two metaphors at once. “Heart” represents the action of thinking or feeling. Also, saying that the “heart” is “full” is a figurative way of saying that someone is fully intending to do something, usually something that should not be done. Alternate translation: “where is the man who is intending to do such a thing”

See: Metaphor

* **Associated Passages:** Esther 7:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 7:6 (#1) (id: 262557)

צַ֣ר וְ⁠אוֹיֵ֔ב

**"an adversary, and an enemy"**

These words mean the same thing. Together, they emphasize Haman’s complete hostility towards the Jews. If it is more natural in your language, you can combine them and indicate the emphasis in a different way. Alternate translation: “the one who is our absolute nemesis”

See: Doublet

* **Associated Passages:** Esther 7:6
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 7:6 (#2) (id: 262558)

נִבְעַ֔ת

**"was terrified"**

Alternative translation: “was extremely afraid”

* **Associated Passages:** Esther 7:6

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 7:6 (#3) (id: 262559)

נִבְעַ֔ת מִ⁠לִּ⁠פְנֵ֥י

**"was terrified from before the face of"**

Here, **face** means the presence of a person. The phrase means that Haman was now extremely afraid to be in the presence of King Ahasuerus and Queen Esther. Alternate translation: “this made Haman very afraid to be in the presence of the king and the queen”

See: Metonymy

* **Associated Passages:** Esther 7:6
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 7:7 (#1) (id: 262560)

וְ⁠הַ⁠מֶּ֜לֶךְ קָ֤ם בַּ⁠חֲמָת⁠וֹ֙ מִ⁠מִּשְׁתֵּ֣ה הַ⁠יַּ֔יִן

**"And the king rose up in his rage from the banquet of wine"**

Here, **in his rage** is an idiom meaning to be extremely angry. Alternate translation: “the king was extremely angry and got up”

See: Idiom

* **Associated Passages:** Esther 7:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:7 (#2) (id: 262561)

אֶל־גִּנַּ֖ת הַ⁠בִּיתָ֑ן

**"to the garden of the palace"**

The implication is that Ahasuerus went outside to absorb the shock that a man he had promoted to the highest position in his empire was plotting to destroy his queen and her whole people. Ahasuerus needed to sort out his conflicting loyalties and decide what to do about this. If it would be misunderstood, you could say explicitly why he went outside. Alternate translation: “he went outside to the palace garden to decide what to do.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 7:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:7 (#3) (id: 262562)

לְ⁠בַקֵּ֤שׁ עַל־נַפְשׁ⁠וֹ֙

**"in order to seek for his life"**

Here, **seeking** means to beg or plead urgently for something. This phrase means that Haman wanted to beg Esther to persuade the king not to order his execution. Alternate translation: “to plead with Queen Esther to spare his life”

See: Metaphor

* **Associated Passages:** Esther 7:7
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:7 (#4) (id: 262563)

כִּ֣י רָאָ֔ה

**"for he saw"**

Here, **seeing** is a metaphor for knowledge, notice, attention, or judgment. Alternate translation: “he realized” or “he understood”

See: Metaphor

* **Associated Passages:** Esther 7:7
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:7 (#5) (id: 262564)

כָלְתָ֥ה אֵלָ֛י⁠ו הָ⁠רָעָ֖ה מֵ⁠אֵ֥ת הַ⁠מֶּֽלֶךְ

**"evil was determined against him by the king"**

You can say this with an active form. Alternate translation: “the king had decided to cause a disaster against him” or “the king was decided to destroy him” or “the king had decided to kill him”

See: Active or Passive

* **Associated Passages:** Esther 7:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:7 (#6) (id: 262565)

כָלְתָ֥ה אֵלָ֛י⁠ו הָ⁠רָעָ֖ה מֵ⁠אֵ֥ת הַ⁠מֶּֽלֶךְ

**"evil was determined against him by the king"**

Here the abstract noun **evil** means “harm,” not something that is morally wrong as in the previous verse. In this context, it can be expressed with a verb such as “execute.” Alternate translation: “the king was likely to execute him”

See: Abstract Nouns

* **Associated Passages:** Esther 7:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Vine (ID: `flora:Vine`); Wine (ID: `realia:Wine`)

## Esther 7:8 (#1) (id: 262566)

וְ⁠הַ⁠מֶּ֡לֶךְ שָׁב֩ \& וְ⁠הָמָן֙ נֹפֵ֔ל

**"Now when the king returned"** \- **"Haman was fallen"**

The implication is that when the king returned to the room, he saw what this verse describes next. If it would be helpful in your language, you could say that explicitly. Alternate translation: “when the king returned … , he saw that Haman had fallen … ”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#2) (id: 262567)

וְ⁠הָמָן֙ נֹפֵ֔ל עַל־הַ⁠מִּטָּה֙ אֲשֶׁ֣ר אֶסְתֵּ֣ר עָלֶ֔י⁠הָ

**"Haman was fallen on the couch on which Esther was"**

The implication is that Haman was doing this as he pleaded for his life. If it would be helpful in your language, you could add that information to the end of this phrase: “ … as he was begging Esther to spare his life”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#3) (id: 262568)

וְ⁠הָמָן֙ נֹפֵ֔ל עַל־הַ⁠מִּטָּה֙ אֲשֶׁ֣ר אֶסְתֵּ֣ר עָלֶ֔י⁠הָ

**"Haman was fallen on the couch on which Esther was"**

If you prefer to present the events in chronological order, you can put this information first in the verse. See the UST.

See: Order of Events

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#4) (id: 262569)

וְ⁠הָמָן֙ נֹפֵ֔ל עַל־הַ⁠מִּטָּה֙ אֲשֶׁ֣ר אֶסְתֵּ֣ר עָלֶ֔י⁠הָ

**"Haman was fallen on the couch on which Esther was"**

The expression **was fallen** means that as Haman was begging for his life, he was not standing up; he had suddenly come down very close to Esther as she was reclining on a couch. (From the impression that the king got when he returned, it is possible that Haman was even grabbing her arms or shoulders as he pleaded with Esther.) Alternate translation: “Haman had come down very close to Esther as she was reclining on a banqueting couch” or “Haman had thrown himself down on the couch where Esther was reclining”

See: Idiom

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#5) (id: 262570)

הַ⁠מִּטָּה֙ אֲשֶׁ֣ר אֶסְתֵּ֣ר עָלֶ֔י⁠הָ

**"the couch on which Esther was"**

In this culture, especially at a banquet like this one, wealthy people would recline on couches as they ate and drank. Alternate translation: “Esther’s banqueting couch”

* **Associated Passages:** Esther 7:8

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#6) (id: 262571)

הֲ֠⁠גַם לִ⁠כְבּ֧וֹשׁ אֶת־הַ⁠מַּלְכָּ֛ה עִמִּ֖⁠י בַּ⁠בָּ֑יִת

**"Is it also to subdue the queen with me in the house"**

The king phrases this as a question to show his shock and anger at what he thinks Haman is doing. If it would be helpful in your language, you could phrase it as a statement. Alternate translation: “he is even trying to rape the queen”

See: Rhetorical Question

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#7) (id: 262572)

לִ⁠כְבּ֧וֹשׁ אֶת־הַ⁠מַּלְכָּ֛ה

**"to subdue the queen"**

The story is using a mild expression to refer to something that is disturbing. In this context, “subdue” means “rape.” The king thought that Haman was trying to rape Esther, and he likely said just that. You can show this in your translation if you want to make this clear. Alternate translation: “trying to rape the queen”

See: Euphemism

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Euphemism (ID: `33268@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#8) (id: 262573)

עִמִּ֖⁠י בַּ⁠בָּ֑יִת

**"with me in the house"**

These two phrases mean similar things. The repetition is used to emphasize the idea that they are expressing. If it would be helpful in your language, you could combine the phrases. However, each one does indicate something slightly different about what a serious offense it would be if Haman actually were trying to rape Esther. It would be a violation of the king’s personal trust and a violation of the trust implicit in hospitality. So you could also include both phrases. Alternate translation: “in my presence and in my own house”

See: Parallelism

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#9) (id: 262574)

הַ⁠דָּבָ֗ר יָצָא֙ מִ⁠פִּ֣י הַ⁠מֶּ֔לֶךְ

**"The word was going out from the mouth of the king"**

This expression describes the action of speaking. Alternate translation: “as soon as the king said this”

See: Idiom

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#10) (id: 262575)

הַ⁠דָּבָ֗ר \& וּ⁠פְנֵ֥י הָמָ֖ן חָפֽוּ

**"The word"** \- **"when they covered the face of Haman"**

The king spoke, and then the servants covered Haman’s face. One action followed upon another. However, this happened so quickly that almost no time seemed to pass in between. You can indicate this with a phrase like “as soon as.” Alternate translation: “when the king said this, immediately they covered Haman’s face”

See: Connect — Background Information

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Connect — Background Information (ID: `33332@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#11) (id: 262576)

וּ⁠פְנֵ֥י הָמָ֖ן חָפֽוּ

**"when they covered the face of Haman"**

The implication is that some the king’s servants who were present did this, and they did it because they understood that the king wanted Haman to be executed. Alternate translation: “some of his servants covered Haman’s face because he was going to be executed”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:8 (#12) (id: 262577)

וּ⁠פְנֵ֥י הָמָ֖ן חָפֽוּ

**"when they covered the face of Haman"**

**Covering the face** in this culture, as in many others, was a sign that a person had been condemned to death. Alternate translation: “some of his servants covered Haman’s face to show that he would be executed”

See: Symbolic Action

* **Associated Passages:** Esther 7:8
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); Wine (ID: `realia:Wine`); House (ID: `realia:House`); Vine (ID: `flora:Vine`); Bed (ID: `realia:Bed`)

## Esther 7:9 (#1) (id: 262578)

**"Harbona"**

This is a man’s name. See how you translated it in [1:10](https://ref.ly/Esth1:10).

See: How to Translate Names

* **Associated Passages:** Esther 7:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#2) (id: 262579)

אֶחָ֨ד מִן־הַ⁠סָּרִיסִ֜ים לִ⁠פְנֵ֣י הַ⁠מֶּ֗לֶךְ

**"one from the eunuchs before the face of the king"**

Here, **face** represents the presence of a person. This phrase means that Harbona was one of the eunuchs who served King Ahasuerus personally.

See: Metonymy

* **Associated Passages:** Esther 7:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#3) (id: 262580)

הַ⁠סָּרִיסִ֜ים לִ⁠פְנֵ֣י הַ⁠מֶּ֗לֶךְ

**"the eunuchs before the face of the king"**

See how you translated this term and this phrase in [1:10](https://ref.ly/Esth1:10). Alternate translation: “the guardians who served the king personally”

See: Translate Unknowns

* **Associated Passages:** Esther 7:9
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#4) (id: 262581)

הִנֵּה

**"behold"**

**Behold** is a term meant to focus the attention of the listener on what the speaker is about to say. If there is not a good way to translate this term in your language, it can be omitted from your translation. But consider using an expression that is natural in your language for someone to address the king directly to get his attention. For example, you could say, “O king!”

* **Associated Passages:** Esther 7:9

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#5) (id: 262582)

גַּ֣ם \& הָ⁠עֵ֣ץ אֲשֶׁר־עָשָׂ֪ה הָמָ֟ן לְֽ⁠מָרְדֳּכַ֞י \& עֹמֵד֙ בְּ⁠בֵ֣ית הָמָ֔ן

**"Also,"** \- **"the pole that Haman made for Mordecai,"** \- **"is standing at the house of Haman"**

Alternate translation: “Haman has also set up a pole \[or gallows] at his house because he wants to impale \[or hang] Mordecai on it”

* **Associated Passages:** Esther 7:9

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#6) (id: 262583)

הָ⁠עֵ֣ץ

**"the pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful. Be sure your translation is consistent in the chapters that follow.

See: Translate Unknowns

* **Associated Passages:** Esther 7:9
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#7) (id: 262584)

לְֽ⁠מָרְדֳּכַ֞י אֲשֶׁ֧ר דִּבֶּר־ט֣וֹב עַל־הַ⁠מֶּ֗לֶךְ

**"for Mordecai, who spoke good for the king"**

This expression means that Mordecai said something that benefitted the king. Specifically, Mordecai reported an assassination plot and saved the king’s life. Alternate translation: “Mordecai, who saved the king’s life.”

See: Idiom

* **Associated Passages:** Esther 7:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#8) (id: 262585)

דִּבֶּר־ט֣וֹב עַל־הַ⁠מֶּ֗לֶךְ

**"spoke good for the king"**

Harbona addresses the king in the third person as a way of showing respect. Alternate translation: “saved your life”

See: First, Second or Third Person

* **Associated Passages:** Esther 7:9
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#9) (id: 262586)

עֹמֵד֙

**"is standing"**

Alternate translation: “is set up”

* **Associated Passages:** Esther 7:9

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#10) (id: 262587)

גָּבֹ֖הַּ חֲמִשִּׁ֣ים אַמָּ֑ה

**"50 cubits high"**

If it would be helpful in your language, you could convert this to a modern measure. See how you translated this phrase in [5:14](https://ref.ly/Esth5:14). Alternate translation: “seventy\-five feet high” or “twenty\-five meters high”

See: Biblical Distance

* **Associated Passages:** Esther 7:9
* **Associated Articles:** Biblical Distance (ID: `33369@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:9 (#11) (id: 262588)

תְּלֻ֥⁠הוּ עָלָֽי⁠ו

**"Hang him on it"**

**Him** means Haman. If it would be helpful in your language, you could say that explicitly. Alternate translation: “hang \[or impale] Haman on it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 7:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Harbona (ID: `person:Harbona.2`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`); Cubit (ID: `realia:Cubit`); House (ID: `realia:House`)

## Esther 7:10 (#1) (id: 262589)

הָ⁠עֵ֖ץ

**"the pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful. Be sure your translation is consistent in the chapters that follow.

See: Translate Unknowns

* **Associated Passages:** Esther 7:10
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 7:10 (#2) (id: 262590)

וַ⁠חֲמַ֥ת הַ⁠מֶּ֖לֶךְ שָׁכָֽכָה

**"and the rage of the king subsided"**

Here the story compares the king’s anger to floodwaters, which can cause great destruction. “Subside” means for waters to drain away and for their level to go down. You could express the meaning by saying something like “Then the king’s rage lessened,” or “Then the king was not so angry.” Or you could use a different metaphor that would be meaningful in your language, such as, “Then the king cooled off.”

See: Metaphor

* **Associated Passages:** Esther 7:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Haman (ID: `person:Haman`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:1 (#1) (id: 262591)

בַּ⁠יּ֣וֹם הַ⁠ה֗וּא

**"On that day"**

This could mean one of three things: (1\) A new series of events is now being described, but the story makes clear that they happened on the same day as the second banquet. While it must have been some time later in the day, the idea is that when Haman lost his life, and also his property, Esther gained what her enemy had lost. Alternate translation: “that same day” (2\) The expression could also be an idiom that means “as of that day.” This would refer to the effective date for the transfer of Haman’s property to Esther. Alternate translation: “as of that day” (3\) The term “day” could also be used to mean around the same time.

See: Introduction of a New Event

* **Associated Passages:** Esther 8:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:1 (#2) (id: 262592)

נָתַ֞ן הַ⁠מֶּ֤לֶךְ אֲחַשְׁוֵרוֹשׁ֙ לְ⁠אֶסְתֵּ֣ר הַ⁠מַּלְכָּ֔ה אֶת־בֵּ֥ית הָמָ֖ן

**"the king Ahasuerus gave to Esther the queen the house of Haman"**

Here, **house** represents all the household and property of Haman. The story could be referring to all the property by naming one part of it, the house. Or “house” could be a metonym for “property,” meaning everything a person owns and keeps in their house, and by extension everything they own beyond the house as well. Alternate translation: “King Ahasuerus declared that everything that Haman had owned would now belong to Queen Esther”

See: Synecdoche

* **Associated Passages:** Esther 8:1
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:1 (#3) (id: 262593)

צֹרֵ֣ר הַיְּהוּדִ֑ים

**"the adversary of the Jews"**

This phrase gives clarifying information about Haman. Alternate translation: “the enemy of the Jews”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 8:1
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:1 (#4) (id: 262594)

וּ⁠מָרְדֳּכַ֗י בָּ֚א לִ⁠פְנֵ֣י הַ⁠מֶּ֔לֶךְ

**"And Mordecai came before the face of the king"**

The implication is that when the king learned how Mordecai was related to Esther, he summoned Mordecai into his presence. If it would be helpful in your language, you could say this explicitly. Alternate translation: “when he learned that, the king sent for Mordecai to come into his presence”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:1 (#5) (id: 262595)

לִ⁠פְנֵ֣י הַ⁠מֶּ֔לֶךְ

**"before the face of the king"**

Here, **face** represents the presence of a person. This phrase indicates that Mordecai was allowed to come into the king’s presence. Alternate translation: “into his presence”

See: Metonymy

* **Associated Passages:** Esther 8:1
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:1 (#6) (id: 262596)

הִגִּ֥ידָה אֶסְתֵּ֖ר מַ֥ה הוּא־לָֽ⁠הּ

**"Esther told what he was to her"**

You can say this before saying that the king summoned Mordecai, since it happened first. See the UST. Alternate translation: “Esther told the king how Mordecai was related to her”

See: Order of Events

* **Associated Passages:** Esther 8:1
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:1 (#7) (id: 262597)

הִגִּ֥ידָה אֶסְתֵּ֖ר מַ֥ה הוּא־לָֽ⁠הּ

**"Esther told what he was to her"**

Esther told this to the king. She would likely have explained not just how she and Mordecai were related, but that he had raised her after her parents died. If it would be helpful in your language, you could say this explicitly. Alternate translation: “Esther told the king that Mordecai was her cousin and that he had been like a father to her”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:2 (#1) (id: 262598)

וַ⁠יָּ֨סַר הַ⁠מֶּ֜לֶךְ אֶת־טַבַּעְתּ֗⁠וֹ \& וַֽ⁠יִּתְּנָ֖⁠הּ לְ⁠מָרְדֳּכָ֑י

**"And the king removed his signet ring,"** \- **"and he gave it to Mordecai"**

Giving the ring to Mordecai showed that Mordecai could now act on the king’s own authority, and it enabled him to do that. If it would be helpful in your language, you could say this explicitly. Alternate translation: “the king now gave this ring to Mordecai to show that Mordecai had the power to act on the authority of the king”

See: Symbolic Action

* **Associated Passages:** Esther 8:2
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 8:2 (#2) (id: 262599)

אֶת־טַבַּעְתּ֗⁠וֹ

**"his signet ring"**

See how you translated this in [3:10](https://ref.ly/Esth3:10). Review the explanation there if that would be helpful. Alternate translation: “the ring that had his official seal on it”

See: Translate Unknowns

* **Associated Passages:** Esther 8:2
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 8:2 (#3) (id: 262600)

אֲשֶׁ֤ר הֶֽעֱבִיר֙ מֵֽ⁠הָמָ֔ן

**"which he had caused to pass from Haman"**

This means that the king had taken back his signet ring from Haman. He would have done this when he sentenced Haman to death. If it would be helpful in your language, you could say that explicitly. Alternate translation: “the king had given Haman the ring that had his official seal on it, but he had taken it back from Haman when he sentenced Haman to death, and the king was wearing it again.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 8:2 (#4) (id: 262601)

אֲשֶׁ֤ר הֶֽעֱבִיר֙ מֵֽ⁠הָמָ֔ן

**"which he had caused to pass from Haman"**

If it would be helpful in your language, you could put this information first in the verse because it happened before everything else.

See: Order of Events

* **Associated Passages:** Esther 8:2
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 8:2 (#5) (id: 262602)

וַֽ⁠יִּתְּנָ֖⁠הּ לְ⁠מָרְדֳּכָ֑י

**"and he gave it to Mordecai"**

By giving his signet ring to Mordecai, the king gave Mordecai the authority to write important papers that people would have to obey. If it would be helpful in your language, you could say this explicitly. See the UST.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 8:2 (#6) (id: 262603)

וַ⁠תָּ֧שֶׂם אֶסְתֵּ֛ר אֶֽת־מָרְדֳּכַ֖י עַל־בֵּ֥ית הָמָֽן

**"And Esther placed Mordecai over the house of Haman"**

To place someone over something means to put them in charge of it. Alternate translation: “Esther appointed Mordecai to be in charge of everything that had belonged to Haman”

See: Metaphor

* **Associated Passages:** Esther 8:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 8:2 (#7) (id: 262604)

בֵּ֥ית הָמָֽן

**"the house of Haman"**

See the explanation of this phrase in verse 1\. Alternate translation: “everything that Haman had owned” or “the household of Haman”

See: Metonymy

* **Associated Passages:** Esther 8:2
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Mordecai (ID: `person:Mordecai.2`); Haman (ID: `person:Haman`)

## Esther 8:3 (#1) (id: 262605)

וַ⁠תּ֣וֹסֶף אֶסְתֵּ֗ר וַ⁠תְּדַבֵּר֙ לִ⁠פְנֵ֣י הַ⁠מֶּ֔לֶךְ

**"And Esther repeated her action, and she spoke before the face of the king"**

Here the exact meaning of the progression of verbs is uncertain, but this verse probably begins a new event in the story separate from what happened in the previous verse. Alternate translation: “then Esther again spoke in the king’s presence”

See: Introduction of a New Event

* **Associated Passages:** Esther 8:3
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#2) (id: 262606)

לִ⁠פְנֵ֣י הַ⁠מֶּ֔לֶךְ

**"before the face of the king"**

Here, **face** represents the presence of a person. Alternate translation: “while she was in his presence”

See: Metonymy

* **Associated Passages:** Esther 8:3
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#3) (id: 262607)

וַ⁠תִּפֹּ֖ל לִ⁠פְנֵ֣י רַגְלָ֑י⁠ו

**"She fell before the face of his feet"**

Here, **falling** is an idiom referring to the action of bowing down or prostrating oneself, and **face** represents the front of a person, place, or object. Alternate translation: “and she bowed down” or “and she prostrated herself”

See: Idiom

* **Associated Passages:** Esther 8:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#4) (id: 262608)

וַ⁠תִּפֹּ֖ל לִ⁠פְנֵ֣י רַגְלָ֑י⁠ו

**"She fell before the face of his feet"**

This may actually mean that Esther put her face right on top of the feet of King Ahasuerus. This would have been an act of humility and desperation by which she showed that her need was very great and that she believed the king had great power to help her.

See: Symbolic Action

* **Associated Passages:** Esther 8:3
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#5) (id: 262609)

וַ⁠תִּפֹּ֖ל לִ⁠פְנֵ֣י רַגְלָ֑י⁠ו

**"She fell before the face of his feet"**

Esther’s action was probably a recognized sign of pleading in this culture. However, it would likely still have been remarkable for a queen to fall at the feet of her husband, the king. you could show this by introducing the information with a phrase like “in fact.” Alternate translation: “in fact, to show how desperately she was pleading, Esther got down and put her face right on top of his feet”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#6) (id: 262610)

לִ⁠פְנֵ֣י רַגְלָ֑י⁠ו

**"before the face of his feet"**

Here, **face** is a metonym meaning the front of a person, place, or object. This phrase means that Esther prostrated herself in front of the feet of King Ahasuerus.

See: Metonymy

* **Associated Passages:** Esther 8:3
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#7) (id: 262611)

לְ⁠הַֽעֲבִיר֙ אֶת־רָעַת֙ הָמָ֣ן הָֽ⁠אֲגָגִ֔י

**"to take away the evil of Haman the Agagite"**

Here, **the evil** refers to Haman’s plan. This expression means to prevent a wrong action from happening. Alternate translation: “to stop the evil plan of Haman the Agagite” or “to prevent the evil things from happening that Haman the Agagite had planned”

See: Idiom

* **Associated Passages:** Esther 8:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#8) (id: 262612)

**"the Agagite"**

This is the name of Haman’s people group. See how you translated this in [3:1](https://ref.ly/Esth3:1).

See: How to Translate Names

* **Associated Passages:** Esther 8:3
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:3 (#9) (id: 262613)

וְ⁠אֵת֙ מַֽחֲשַׁבְתּ֔⁠וֹ אֲשֶׁ֥ר חָשַׁ֖ב

**"and his plot that he had plotted"**

Alternate translation: “and the plot that he had invented” or “and the plot that Haman invented”

* **Associated Passages:** Esther 8:3

* **Associated ACAI Concepts:** Agagites (ID: `group:Agag`); To Beg for Mercy (ID: `keyterm:BegForMercy`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 8:4 (#1) (id: 262614)

וַ⁠יּ֤וֹשֶׁט הַ⁠מֶּ֨לֶךְ֙ לְ⁠אֶסְתֵּ֔ר אֵ֖ת שַׁרְבִ֣ט הַ⁠זָּהָ֑ב

**"And the king held out to Esther the scepter of gold"**

As in [5:2](https://ref.ly/Esth5:2), King Ahasuerus did this to show that Esther had his favor. In that earlier episode, this indicated that he would not enforce the law that said Esther should be executed for coming into the inner court without being summoned. But since Esther was apparently already in the king’s presence on this occasion, it seems that the gesture could also be used generally to show that the king was positively disposed towards a person and would grant the person’s request. Alternate translation: “the king held out his golden scepter to Esther”

See: Symbolic Action

* **Associated Passages:** Esther 8:4
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 8:4 (#2) (id: 262615)

אֵ֖ת שַׁרְבִ֣ט הַ⁠זָּהָ֑ב

**"the scepter of gold"**

See how you translated this in [4:11](https://ref.ly/Esth4:11). If it would be helpful, review the explanation of what this object was. Alternate translation: “golden scepter”

See: Translate Unknowns

* **Associated Passages:** Esther 8:4
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 8:4 (#3) (id: 262616)

וַ⁠תָּ֣קָם אֶסְתֵּ֔ר

**"so Esther rose up"**

Alternate translation: “Esther got up off the floor”

* **Associated Passages:** Esther 8:4

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 8:4 (#4) (id: 262617)

לִ⁠פְנֵ֥י הַ⁠מֶּֽלֶךְ

**"before the face of the king"**

Here, **face** represents the front of a person, place, or object. This phrase means the Esther now stood facing King Ahasuerus. Alternate translation: “and stood facing the king”

See: Metonymy

* **Associated Passages:** Esther 8:4
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`)

## Esther 8:5 (#1) (id: 262618)

אִם־עַל־הַ⁠מֶּ֨לֶךְ ט֜וֹב \& וְ⁠כָשֵׁ֤ר הַ⁠דָּבָר֙ לִ⁠פְנֵ֣י הַ⁠מֶּ֔לֶךְ

**"If it is good to the king,"** \- **"and the word is proper before the face of the king"**

These two phrases mean basically the same thing. Esther is saying very deferentially that she hopes that the king will think that her suggestion is a good idea and will, therefore, grant her request. Esther uses the repetition to emphasize the importance of what she is expressing. If it would be helpful in your language, you could combine the phrases. Alternate translation: “if what I am going to suggest seems like a good idea to you”

See: Parallelism

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#2) (id: 262619)

וְ⁠אִם־מָצָ֧אתִי חֵ֣ן לְ⁠פָנָ֗י⁠ו \& וְ⁠טוֹבָ֥ה אֲנִ֖י בְּ⁠עֵינָ֑י⁠ו

**"and if I have found favor before his face,"** \- **"and I am good in his eyes"**

These two phrases mean basically the same thing. Esther is saying very deferentially that she hopes that the king is positively disposed towards her, that he thinks well of her, and will, therefore, grant her request. Esther uses the repetition to emphasize the importance of what she is expressing. If it would be helpful in your language, you could combine the phrases. Alternate translation: “if you are pleased with me”

See: Parallelism

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#3) (id: 262620)

אִם־עַל־הַ⁠מֶּ֨לֶךְ ט֜וֹב וְ⁠אִם־מָצָ֧אתִי חֵ֣ן לְ⁠פָנָ֗י⁠ו

**"If it is good to the king, and if I have found favor before his face"**

Here Esther addresses the king throughout in the third person as a way of showing respect. You could express the same meaning in the second person, as in the UST.

See: First, Second or Third Person

* **Associated Passages:** Esther 8:5
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#4) (id: 262621)

אִם־עַל־הַ⁠מֶּ֨לֶךְ ט֜וֹב

**"If it is good to the king"**

This is an idiom that also appears in [1:19](https://ref.ly/Esth1:19), [3:9](https://ref.ly/Esth3:9), [5:4](https://ref.ly/Esth5:4), and [5:8](https://ref.ly/Esth5:8). A suggested translation in those cases was, “If it seems like a good idea to you, O king.” That was suitable for the deferential tone the speakers were using. But in another place, [7:3](https://ref.ly/Esth7:3), when Esther was pleading urgently for the survival of her whole people, the suggested alternate translation was, “I hope you will be willing to do what I ask” Here, Esther is also pleading, so you could use that phrase here in your translation as well. Alternate translation: “if what I ask for seems right in the king’s evaluation”

See: Idiom

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#5) (id: 262622)

וְ⁠אִם־מָצָ֧אתִי חֵ֣ן

**"and if I have found favor"**

This expression also appears many times in the story. It means to gain the approval or acceptance of another person. Alternate translation: “if you are pleased with me”

See: Idiom

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#6) (id: 262623)

לְ⁠פָנָ֗י⁠ו \& לִ⁠פְנֵ֣י הַ⁠מֶּ֔לֶךְ

**"before his face,"** \- **"before the face of the king"**

Here, **face** means the presence of a person. These phrases could mean “as I stand here before you.” But “face” could also be referring to the king himself by naming one part of him. What he thought and felt about Esther’s request would become evident in his face first, so it would be an appropriate part of him to use to represent all of him. Alternate translation: “you”

See: Metonymy

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#7) (id: 262624)

בְּ⁠עֵינָ֑י⁠ו

**"in his eyes"**

Here, **eyes** represent the action of seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. Alternate translation: “if you evaluate me and I please you” or “if you are pleased with me”

See: Metaphor

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#8) (id: 262625)

יִכָּתֵ֞ב לְ⁠הָשִׁ֣יב אֶת־הַ⁠סְּפָרִ֗ים

**"let it be written to bring back the letters"**

The first letters that told of the plan to destroy the Jews would not be brought back to Susa physically. Rather, **bring back** is an idiom that means “cancel” or “revoke.” Alternate translation: “make a new law to cancel what Haman decreed”

See: Idiom

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:5 (#9) (id: 262626)

יִכָּתֵ֞ב לְ⁠הָשִׁ֣יב אֶת־הַ⁠סְּפָרִ֗ים

**"let it be written to bring back the letters"**

You can say this with an active form. Alternate translation: “write a new letter” or “make a new law”

See: Active or Passive

* **Associated Passages:** Esther 8:5
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Favor (ID: `keyterm:Favor`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Scroll (ID: `realia:Scroll`)

## Esther 8:6 (#1) (id: 262627)

כִּ֠י

**"For"**

Here Esther is giving the reason why she is asking King Ahasuerus to revoke the letters. Alternate translation: “the reason I am asking is that”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 8:6
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)


## Esther 8:6 (#2) (id: 262628)

אֵיכָכָ֤ה אוּכַל֙ וְֽ⁠רָאִ֔יתִי בָּ⁠רָעָ֖ה אֲשֶׁר־יִמְצָ֣א אֶת־עַמִּ֑⁠י וְ⁠אֵֽיכָכָ֤ה אוּכַל֙ וְֽ⁠רָאִ֔יתִי בְּ⁠אָבְדַ֖ן מוֹלַדְתִּֽ⁠י

**"how could I bear to see the evil that my people would find? And how could I bear seeing the destruction of my kindred"**

Esther is actually making a statement, but she expresses herself in question form to show how strongly she feels about what she is saying. She does not actually expect the king to tell her how she could bear seeing her people destroyed. Alternate translation: “I cannot bear to see disaster fall on the Jews” or “I cannot endure watching my relatives be killed”

See: Rhetorical Question

* **Associated Passages:** Esther 8:6
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)


## Esther 8:6 (#3) (id: 262629)

אֵיכָכָ֤ה אוּכַל֙ וְֽ⁠רָאִ֔יתִי בָּ⁠רָעָ֖ה אֲשֶׁר־יִמְצָ֣א אֶת־עַמִּ֑⁠י וְ⁠אֵֽיכָכָ֤ה אוּכַל֙ וְֽ⁠רָאִ֔יתִי בְּ⁠אָבְדַ֖ן מוֹלַדְתִּֽ⁠י

**"how could I bear to see the evil that my people would find? And how could I bear seeing the destruction of my kindred"**

These two sentences mean basically the same thing. Esther uses the repetition to emphasize the importance of what she is saying. If it would be helpful in your language, you could combine the phrases. Alternate translation: “I could not bear to see my relatives and my whole people destroyed”

See: Parallelism

* **Associated Passages:** Esther 8:6
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)


## Esther 8:6 (#4) (id: 262630)

בָּ⁠רָעָ֖ה אֲשֶׁר־יִמְצָ֣א אֶת־עַמִּ֑⁠י

**"the evil that my people would find"**

Even though it was morally very wrong for Haman to want to destroy Mordecai’s whole people, here the abstract noun **evil** likely means “harm,” as in [7:7](https://ref.ly/Esth7:7). It means the same thing as “destruction” in the next sentence. Alternate translation: “my people being destroyed”

See: Abstract Nouns

* **Associated Passages:** Esther 8:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## Esther 8:6 (#5) (id: 262631)

יִמְצָ֣א

**"would find"**

As in [1:5](https://ref.ly/Esth1:5), **find** is an idiom that means something exists in a certain place or time, not that people were looking for it and located it. Esther is speaking of the harm that will exist for the Jews if the effect of Haman’s letters is not undone. Alternate translation: “would be destroyed”

See: Idiom

* **Associated Passages:** Esther 8:6
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Esther 8:6 (#6) (id: 262632)

בְּ⁠אָבְדַ֖ן

**"the destruction"**

This abstract noun can be expressed with a verb such as “destroyed.”

See: Abstract Nouns

* **Associated Passages:** Esther 8:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## Esther 8:6 (#7) (id: 262633)

מוֹלַדְתִּֽ⁠י

**"of my kindred"**

Alternate translation: “my relatives”

* **Associated Passages:** Esther 8:6


## Esther 8:7 (#1) (id: 262634)

הִנֵּ֨ה

**"Behold"**

This is a term meant to focus the attention of the listener on what the speaker is about to say. If there is not a good way to translate this term in your language, this term can be omitted from the translation, or you can use an alternate translation like “as you know.”

* **Associated Passages:** Esther 8:7

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:7 (#2) (id: 262635)

בֵית־הָמָ֜ן נָתַ֣תִּי לְ⁠אֶסְתֵּ֗ר

**"I have given the house of Haman to Esther"**

See the note about this expression in [8:1](https://ref.ly/Esth8:1). Alternate translation: “I have given Esther all the property that belonged to Haman” or “I have given Esther the household of Haman”

* **Associated Passages:** Esther 8:7

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:7 (#3) (id: 262636)

בֵית־הָמָ֜ן

**"the house of Haman"**

Here, **house** is a metonym meaning the entire household of Haman. Alternate translation: “the household of Haman”

See: Metonymy

* **Associated Passages:** Esther 8:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:7 (#4) (id: 262637)

וְ⁠אֹת⁠וֹ֙ תָּל֣וּ עַל־הָ⁠עֵ֔ץ

**"and they have hanged him on the pole"**

You can put this information first because it happened before the king gave Haman’s property to Esther. Alternate translation: “I had my servants impale Haman on a wooden pole” or “I had my servants hang Haman on a gallows”

See: Order of Events

* **Associated Passages:** Esther 8:7
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:7 (#5) (id: 262638)

הָ⁠עֵ֔ץ

**"the pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful. Be sure your translation is consistent in the chapters that follow.

See: Translate Unknowns

* **Associated Passages:** Esther 8:7
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:7 (#6) (id: 262639)

שָׁלַ֥ח יָד֖⁠וֹ

**"he stretched out his hand"**

Here the expression **to stretch out a hand** means to cause someone physical harm with the intention of killing them. Alternate translation: “because he plotted to destroy all the Jews”

See: Idiom

* **Associated Passages:** Esther 8:7
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Mordecai (ID: `person:Mordecai.2`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`); House (ID: `realia:House`)

## Esther 8:8 (#1) (id: 262640)

וְ֠⁠אַתֶּם

**"So,"** \- **"you"**

Alternate translation: “so this is what you should do”

* **Associated Passages:** Esther 8:8

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#2) (id: 262641)

כִּתְב֨וּ עַל־הַ⁠יְּהוּדִ֜ים כַּ⁠טּ֤וֹב בְּ⁠עֵֽינֵי⁠כֶם֙

**"write for the Jews as is good in your eyes"**

Alternate translation: “I am giving you permission to write other letters to save your people”

* **Associated Passages:** Esther 8:8

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#3) (id: 262642)

כַּ⁠טּ֤וֹב בְּ⁠עֵֽינֵי⁠כֶם֙

**"as is good in your eyes"**

Here, **eyes** represent the action of seeing, and seeing is a metaphor for knowledge, notice, attention, or judgment. This phrase means that Esther and Mordecai have permission to write what they think is best. Alternate translation: “as you think is best” or “what you think is best”

See: Metaphor

* **Associated Passages:** Esther 8:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#4) (id: 262643)

בְּ⁠שֵׁ֣ם הַ⁠מֶּ֔לֶךְ

**"in the name of the king"**

Here, **name** is a metaphor meaning authority. This phrase means Esther and Mordecai have permission to write with the authority of the King Ahasuerus.

See: Metaphor

* **Associated Passages:** Esther 8:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#5) (id: 262644)

בְּ⁠שֵׁ֣ם הַ⁠מֶּ֔לֶךְ

**"in the name of the king"**

King Ahasuerus refers to himself in the third person. You can have him say this in the first person. Alternate translation: “I give you permission to write with my own authority”

See: First, Second or Third Person

* **Associated Passages:** Esther 8:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#6) (id: 262645)

בְּ⁠טַבַּ֣עַת הַ⁠מֶּ֑לֶךְ

**"with the signet ring of the king"**

This is a special ring that could be used to imprint the king’s official seal on a law or decree. Alternate translation: “the ring that has my official seal on it”

See: Translate Unknowns

* **Associated Passages:** Esther 8:8
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#7) (id: 262646)

בְּ⁠טַבַּ֣עַת הַ⁠מֶּ֑לֶךְ

**"with the signet ring of the king"**

King Ahasuerus refers to himself in the third person. You can have him say this in the first person. Alternate translation: “the ring that has my official seal on it”

See: First, Second or Third Person

* **Associated Passages:** Esther 8:8
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#8) (id: 262647)

כִּֽי

**"For"**

Here, King Ahasuerus is expressing the reason why he cannot simply revoke the first letter and also why Esther and Mordecai can write another letter that people must obey. Alternate translation: “you see” or “people will obey your letter because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 8:8
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#9) (id: 262648)

אֵ֥ין לְ⁠הָשִֽׁיב

**"there is none to take back"**

As in verse 5, this expression means “revoke.” Alternate translation: “no one can revoke” or “no one can nullify”

* **Associated Passages:** Esther 8:8

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#10) (id: 262649)

כְתָ֞ב \& אֵ֥ין לְ⁠הָשִֽׁיב

**"there is none to take back a writing"**

If it would be helpful in your language, you could put this first in the verse because it explains why Ahasuerus answers Esther and Mordecai in the way that he does. See the UST.

* **Associated Passages:** Esther 8:8

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:8 (#11) (id: 262650)

כְתָ֞ב אֲשֶׁר־נִכְתָּ֣ב בְּ⁠שֵׁם־הַ⁠מֶּ֗לֶךְ וְ⁠נַחְתּ֛וֹם בְּ⁠טַבַּ֥עַת הַ⁠מֶּ֖לֶךְ

**"a writing that has been written in the name of the king and has been sealed with the signet ring of the king"**

The king is describing a general situation, but he has Haman’s letter specifically in mind. If it would be helpful in your language, you could say that explicitly. Alternate translation: “Haman wrote his letter with my authority, and he sealed it with the ring that has my official seal on it. No one can revoke a letter like that” (The story explained in [1:19](https://ref.ly/Esth1:19) that the king’s laws could not be changed once they had been made and proclaimed.)

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:9 (#1) (id: 262651)

וַ⁠יִּקָּרְא֣וּ סֹפְרֵֽי־הַ⁠מֶּ֣לֶךְ

**"And the scribes of the king were called"**

This indicates that what is described in this verse was done because of what the king said in the previous verse. Use a connecting word or phrase to show this in your language.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#2) (id: 262652)

וַ⁠יִּקָּרְא֣וּ סֹפְרֵֽי־הַ⁠מֶּ֣לֶךְ

**"And the scribes of the king were called"**

You can say this with an active form, and you can say who did the action. Alternate translation: “the king called his scribes”

See: Active or Passive

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#3) (id: 262653)

**"And the scribes of the king were called"**

If you use an expression such as “called,” make sure your readers will understand that the king likely sent an official to go and bring the scribes back with him. The king did not call out in a loud voice to get them to come.

* **Associated Passages:** Esther 8:9

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#4) (id: 262654)

בַּ⁠חֹ֨דֶשׁ הַ⁠שְּׁלִישִׁ֜י

**"in the third month"**

Alternate translation: “in month three”

See: Ordinal Numbers

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#5) (id: 262655)

בַּ⁠חֹ֨דֶשׁ הַ⁠שְּׁלִישִׁ֜י

**"in the third month"**

It would still be the same year as in [3:7](https://ref.ly/Esth3:7), the twelfth year that Ahasuerus reigned as king of Persia. If it would be helpful in your language, you could say that explicitly. Alternate translation: “in the third month of the year”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#6) (id: 262656)

הוּא־חֹ֣דֶשׁ סִיוָ֗ן

**"which is the month of Sivan"**

**Sivan** is the name of the third month of the Hebrew calendar. Alternate translation: “the month of Sivan”

See: Hebrew Months

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#7) (id: 262657)

הוּא־חֹ֣דֶשׁ סִיוָ֗ן

**"which is the month of Sivan"**

The story is being recorded from the perspective of the Persian court, but for the benefit of its intended Jewish audience, the Hebrew name of the month is given. This month overlaps with May and June on a Western calendar.

* **Associated Passages:** Esther 8:9

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#8) (id: 262658)

בִּ⁠שְׁלוֹשָׁ֣ה וְ⁠עֶשְׂרִים֮ בּ⁠וֹ֒

**"on the twenty\-third of it"**

Alternation translation: “on day 23” or “on the twenty\-third day of the month” The exact date of the letter helps to establish its legal authority. This would be on June 25th of a Western calendar.

See: Ordinal Numbers

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#9) (id: 262659)

וַ⁠יִּכָּתֵ֣ב כְּֽ⁠כָל־אֲשֶׁר־צִוָּ֣ה מָרְדֳּכַ֣י

**"And according to all that Mordecai commanded, it was written"**

You can say this with an active form. Alternate translation: “and they wrote in a letter everything that Mordecai dictated to them”

See: Active or Passive

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#10) (id: 262660)

אֶל־הַ⁠יְּהוּדִ֡ים

**"to the Jews"**

It appears from verse 11 that the letter would have been addressed specifically to the Jews in the empire, telling them that the king had authorized them to defend themselves. But copies were also sent to all the royal and provincial officials. If it would be helpful in your language, you could say this explicitly. Alternate translation: “the letter addressed the Jews in the empire, but copies were also sent”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#11) (id: 262661)

וְ⁠אֶ֣ל הָ⁠אֲחַשְׁדַּרְפְּנִֽים־וְ⁠הַ⁠פַּחוֹת֩ וְ⁠שָׂרֵ֨י הַ⁠מְּדִינ֜וֹת

**"and to the satraps, and the governors and officials of the provinces"**

See how you translated these terms in [3:12](https://ref.ly/Esth3:12), where they were used to describe who received Haman’s letter. These are all people in government positions. The first one is the governor of the province and the other two are rulers under him who govern cities or smaller areas. If your language uses one term for all of these, you could use that with a descriptive phrase such as “to the government leaders of each province and also to the leaders under him.”

* **Associated Passages:** Esther 8:9

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#12) (id: 262662)

אֲשֶׁ֣ר ׀ מֵ⁠הֹ֣דּוּ וְ⁠עַד־כּ֗וּשׁ שֶׁ֣בַע וְ⁠עֶשְׂרִ֤ים וּ⁠מֵאָה֙ מְדִינָ֔ה

**"that were from India even as far as Ethiopia: 127 provinces"**

This is background information explaining how far this new letter had to be sent. See how you translated this phrase in [1:1](https://ref.ly/Esth1:1). Alternate translation: “the empire of Ahasuerus had 127 provinces, stretching all the way from India in the east to Ethiopia in the west”

* **Associated Passages:** Esther 8:9

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#13) (id: 262663)

שֶׁ֣בַע וְ⁠עֶשְׂרִ֤ים וּ⁠מֵאָה֙ מְדִינָ֔ה

**"127 provinces"**

Alternate translation: “one hundred and twenty\-seven provinces”

See: Numbers

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#14) (id: 262664)

מְדִינָ֤ה וּ⁠מְדִינָה֙

**"province by province"**

This expression means “to people in every province.” Alternate translation: “the scribes wrote to the people in each province”

See: Idiom

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#15) (id: 262665)

כִּ⁠כְתָבָ֔⁠הּ

**"according to its writing"**

Alternate translation: “using its own alphabet” or “written in its own script”

* **Associated Passages:** Esther 8:9

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#16) (id: 262666)

וְ⁠עַ֥ם וָ⁠עָ֖ם

**"and people by people"**

This expression means “every people group.” Alternate translation: “and to each ethnic group”

See: Idiom

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#17) (id: 262667)

כִּ⁠לְשֹׁנ֑⁠וֹ

**"according to its tongue"**

Here, **tongue** means the language spoken by a person or a group of people. Alternate translation: “in its own language”

See: Metonymy

* **Associated Passages:** Esther 8:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:9 (#18) (id: 262668)

וְ⁠אֶ֨ל־הַ⁠יְּהוּדִ֔ים כִּ⁠כְתָבָ֖⁠ם וְ⁠כִ⁠לְשׁוֹנָֽ⁠ם

**"and to the Jews according to their writing and according to their tongue"**

The Jews would have been included among all the people groups in the empire in the phrase “people by people.” So this seems to be saying, “and especially to the Jews.” Verse 11 explains that it was particularly important for the Jews to read the letter because it gave them the right to defend themselves. Alternate translation: “they wrote especially to the Jews in their own script and in their own language”

* **Associated Passages:** Esther 8:9

* **Associated ACAI Concepts:** India (ID: `place:India`); Ethiopia (ID: `place:Ethiopia`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 8:10 (#1) (id: 262669)

וַ⁠יִּכְתֹּ֗ב \& וַ⁠יַּחְתֹּ֖ם

**"And he wrote"** \- **"and he sealed"**

**He** refers to Mordecai. If it would be helpful in your language, you can use his name here.

* **Associated Passages:** Esther 8:10

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Horse (ID: `fauna:Horse`); Scroll (ID: `realia:Scroll`)

## Esther 8:10 (#2) (id: 262670)

בְּ⁠שֵׁם֙ הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵרֹ֔שׁ

**"in the name of the king Ahasuerus"**

Here, **name** is a metaphor meaning authority. Alternate translation: “Mordecai wrote with the authority of King Ahasuerus”

See: Metaphor

* **Associated Passages:** Esther 8:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Horse (ID: `fauna:Horse`); Scroll (ID: `realia:Scroll`)

## Esther 8:10 (#3) (id: 262671)

וַ⁠יַּחְתֹּ֖ם בְּ⁠טַבַּ֣עַת הַ⁠מֶּ֑לֶךְ

**"and he sealed with the signet ring of the king"**

This means that Mordecai sealed the letters with this ring. If it would be helpful in your language, you could say that explicitly. Alternate translation: “and he sealed the letters with the ring that had the king’s official seal on it”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Horse (ID: `fauna:Horse`); Scroll (ID: `realia:Scroll`)

## Esther 8:10 (#4) (id: 262672)

בְּ⁠יַד֩ הָ⁠רָצִ֨ים בַּ⁠סּוּסִ֜ים

**"by the hand of runners on horses"**

As in [3:13](https://ref.ly/Esth3:13), **hand** could mean two different things. (1\) It could literally mean “hand,” meaning that the runners carried the letters in their hands. (2\) It could also be a metaphor for power, control, or authority, meaning that runners were the ones who delivered the letters to all the provinces throughout the empire. Alternate translation: “couriers on horseback delivered the letters”

See: Metaphor

* **Associated Passages:** Esther 8:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Horse (ID: `fauna:Horse`); Scroll (ID: `realia:Scroll`)

## Esther 8:10 (#5) (id: 262673)

רֹכְבֵ֤י הָ⁠רֶ֨כֶשׁ֙ הָֽ⁠אֲחַשְׁתְּרָנִ֔ים בְּנֵ֖י הָֽ⁠רַמָּכִֽים

**"riders of the royal pack horses, sons of the mares"**

Alternate translation: “They rose fast horses that were used in the king’s service. These horses had been bred in the king’s stables.”

* **Associated Passages:** Esther 8:10

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Horse (ID: `fauna:Horse`); Scroll (ID: `realia:Scroll`)

## Esther 8:10 (#6) (id: 262674)

בְּנֵ֖י הָֽ⁠רַמָּכִֽים

**"sons of the mares"**

Here, **sons** is a metonym meaning the offspring of royal livestock. Alternate translation: “the offspring of the king’s horses”

See: Metonymy

* **Associated Passages:** Esther 8:10
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`); Horse (ID: `fauna:Horse`); Scroll (ID: `realia:Scroll`)

## Esther 8:11 (#1) (id: 262675)

בְּ⁠כָל־עִיר־וָ⁠עִ֗יר

**"in every city, city by city"**

This expression means “in each and every city.” It is possibly referring to the entire empire by naming one part of it, its cities. The story says in [9:19](https://ref.ly/Esth9:19) that not just Jews living in cities but Jews living in rural areas also defended themselves. It is likely that the messengers only published the news in the cities and not the entire countryside, but the news was certainly intended for everyone, not only people living in the cities. Alternate translation: “throughout the empire” or “in each and every city” or “in every single city”

See: Idiom

* **Associated Passages:** Esther 8:11
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:11 (#2) (id: 262676)

לְ⁠הִקָּהֵל֮ וְ⁠לַ⁠עֲמֹ֣ד עַל־נַפְשָׁ⁠ם֒

**"to gather and to stand for their life"**

Here, **to stand** is a metaphor meaning to defend oneself and fight back instead of running away from an enemy. Alternate translation: “to join together and fight for their lives” or “to join together and fight back”

See: Metaphor

* **Associated Passages:** Esther 8:11
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:11 (#3) (id: 262677)

לְ⁠הַשְׁמִיד֩ וְ⁠לַ⁠הֲרֹ֨ג וּ⁠לְ⁠אַבֵּ֜ד

**"to annihiliate, and to slaughter, and to destroy"**

These words mean the same thing and are used together to emphasize the completeness of the destruction that is being described. See how you translated this in [3:13](https://ref.ly/Esth3:13) and [7:4](https://ref.ly/Esth7:4). Alternate translation: “completely destroy” (A “doublet” can involve the use of more than two words.)

See: Doublet

* **Associated Passages:** Esther 8:11
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:11 (#4) (id: 262678)

אֶת־כָּל־חֵ֨יל עַ֧ם וּ⁠מְדִינָ֛ה הַ⁠צָּרִ֥ים אֹתָ֖⁠ם

**"any strength of a people or province that would attack them"**

**Strength** is a figurative way of referring to an army or to a person carrying weaponry. Alternate translation: “the army of any people or province that attacked them”

See: Metonymy

* **Associated Passages:** Esther 8:11
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:11 (#5) (id: 262679)

טַ֣ף וְ⁠נָשִׁ֑ים

**"children and women"**

Alternate translation: “they could also kill the wives and children of the armed men”

* **Associated Passages:** Esther 8:11

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:11 (#6) (id: 262680)

וּ⁠שְׁלָלָ֖⁠ם לָ⁠בֽוֹז

**"and plunder their spoil"**

See how you translated this expression in [3:13](https://ref.ly/Esth3:13). Alternate translation: “and take everything that belonged to them”

* **Associated Passages:** Esther 8:11

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:12 (#1) (id: 262681)

בְּ⁠י֣וֹם אֶחָ֔ד

**"on one day"**

This was the day that Haman had set for destroying the Jews in [3:13](https://ref.ly/Esth3:13). If it would be helpful in your language, you could make this explicit. Alternate translation: “on the same day that Haman had set for destroying the Jews”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 8:12 (#2) (id: 262682)

בְּ⁠כָל־מְדִינ֖וֹת הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֑וֹשׁ

**"in all the provinces of the king Ahasuerus"**

Alternate translation: “in every province of the kingdom”

* **Associated Passages:** Esther 8:12

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 8:12 (#3) (id: 262683)

בִּ⁠שְׁלוֹשָׁ֥ה עָשָׂ֛ר

**"on the thirteenth"**

Alternate translation: “on day 13” or “on the thirteenth day”

See: Ordinal Numbers

* **Associated Passages:** Esther 8:12
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 8:12 (#4) (id: 262684)

לְ⁠חֹ֥דֶשׁ שְׁנֵים־עָשָׂ֖ר

**"of month 12"**

Alternate translation: “of month 12” or “of the twelfth month”

See: Ordinal Numbers

* **Associated Passages:** Esther 8:12
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 8:12 (#5) (id: 262685)

לְ⁠חֹ֥דֶשׁ שְׁנֵים־עָשָׂ֖ר

**"of month 12"**

Implicitly, this means “the twelfth month of that same year.” If it would be helpful in your language, you could say that explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:12
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 8:12 (#6) (id: 262686)

**"which is the month of Adar"**

This is the name of the twelfth and last month of the Hebrew calendar. See how you translated this in [3:7](https://ref.ly/Esth3:7) and [3:13](https://ref.ly/Esth3:13).

See: Hebrew Months

* **Associated Passages:** Esther 8:12
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 8:13 (#1) (id: 262687)

פַּתְשֶׁ֣גֶן הַ⁠כְּתָ֗ב לְ⁠הִנָּ֤תֵֽן דָּת֙

**"A copy of the writing was to be given as a law"**

You can say this with an active form. Alternate translation: “the letter told the officials to proclaim this as a law”

See: Active or Passive

* **Associated Passages:** Esther 8:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:13 (#2) (id: 262688)

בְּ⁠כָל־מְדִינָ֣ה וּ⁠מְדִינָ֔ה

**"in every province by province"**

This expression means “in each and every province.” Alternate translation: “in every single province”

See: Idiom

* **Associated Passages:** Esther 8:13
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:13 (#3) (id: 262689)

גָּל֖וּי לְ⁠כָל־הָ⁠עַמִּ֑ים

**"being uncovered for all the peoples"**

You can say this with an active form. Alternate translation: “the letter told the officials in every single province to post copies where everyone could see them”

See: Active or Passive

* **Associated Passages:** Esther 8:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:13 (#4) (id: 262690)

וְ⁠לִ⁠הְי֨וֹת הַיְּהוּדִ֤ים עֲתִידִים֙ לַ⁠יּ֣וֹם הַ⁠זֶּ֔ה

**"and for the Jews to be prepared for that day"**

Alternate translation: “that way the Jews would get ready to do what the letter said when the day came”

* **Associated Passages:** Esther 8:13

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:13 (#5) (id: 262691)

לְ⁠הִנָּקֵ֖ם מֵ⁠אֹיְבֵי⁠הֶֽם

**"to take revenge from their enemies"**

This expression, “to take revenge from” another person, means to correct a wrong they have done. In this context, the phrase means to correct the wrong of the original law that gave people permission to kill the Jews. Alternate translation: “and fight back against their enemies”

See: Idiom

* **Associated Passages:** Esther 8:13
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:14 (#1) (id: 262692)

הָ⁠רָצִ֞ים רֹכְבֵ֤י הָ⁠רֶ֨כֶשׁ֙ הָֽ⁠אֲחַשְׁתְּרָנִ֔ים

**"The runners, the riders of the royal pack horses"**

See how you translated this in [8:10](https://ref.ly/Esth8:10). Alternate translation: “messengers riding on fast horses that were used for the king’s business”

* **Associated Passages:** Esther 8:14

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Horse (ID: `fauna:Horse`)

## Esther 8:14 (#2) (id: 262693)

מְבֹהָלִ֥ים וּ⁠דְחוּפִ֖ים בִּ⁠דְבַ֣ר הַ⁠מֶּ֑לֶךְ

**"hastened and hurried by the word of the king"**

You can say this with an active form. Alternate translation: “the king commanded the couriers to deliver the letters as quickly as possible”

See: Active or Passive

* **Associated Passages:** Esther 8:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Horse (ID: `fauna:Horse`)

## Esther 8:14 (#3) (id: 262694)

מְבֹהָלִ֥ים וּ⁠דְחוּפִ֖ים

**"hastened and hurried"**

These two terms mean almost the same thing and are used together to emphasize the fact that the couriers were told to deliver the letters as quickly as possible. Alternate translation: “they went immediately” or “as quickly as possible”

See: Doublet

* **Associated Passages:** Esther 8:14
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Horse (ID: `fauna:Horse`)

## Esther 8:14 (#4) (id: 262695)

הָ⁠רָצִ֞ים רֹכְבֵ֤י הָ⁠רֶ֨כֶשׁ֙ הָֽ⁠אֲחַשְׁתְּרָנִ֔ים

**"The runners, the riders of the royal pack horses"**

To present the events in chronological order, you can put this after the king’s command, as in the UST.

See: Order of Events

* **Associated Passages:** Esther 8:14
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Horse (ID: `fauna:Horse`)

## Esther 8:14 (#5) (id: 262696)

וְ⁠הַ⁠דָּ֥ת נִתְּנָ֖ה

**"And the law was given"**

You can say this with an active form. Alternate translation: “the king’s officials also posted and read copies of the letter”

See: Active or Passive

* **Associated Passages:** Esther 8:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Horse (ID: `fauna:Horse`)

## Esther 8:15 (#1) (id: 262697)

וּ⁠מָרְדֳּכַ֞י יָצָ֣א

**"And Mordecai went out"**

This introduces a new event in the story. Use a way that is natural in your language to indicate this.

See: Introduction of a New Event

* **Associated Passages:** Esther 8:15
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:15 (#2) (id: 262698)

מִ⁠לִּ⁠פְנֵ֣י הַ⁠מֶּ֗לֶךְ

**"from before the face of the king"**

Here, **face** represents the presence of a person. This phrase means that Mordecai had been in the presence of King Ahasuerus and was now leaving in order to fulfill his duties as a high official in the Persian government. Alternate translation: “from the palace”

See: Metonymy

* **Associated Passages:** Esther 8:15
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:15 (#3) (id: 262699)

בִּ⁠לְב֤וּשׁ מַלְכוּת֙ תְּכֵ֣לֶת וָ⁠ח֔וּר וַ⁠עֲטֶ֤רֶת זָהָב֙ גְּדוֹלָ֔ה וְ⁠תַכְרִ֥יךְ בּ֖וּץ וְ⁠אַרְגָּמָ֑ן

**"in a garment of royalty of blue and white, with a great crown of gold and a robe of fine linen and purple"**

The implication is that the king gave Mordecai these special things to wear to show that he was now his highest officer. If it would be helpful in your language, you could say that explicitly. To put these events in chronological order, you could place this information before the report that Mordecai left the king’s presence to fulfill his duties.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:15 (#4) (id: 262700)

בִּ⁠לְב֤וּשׁ מַלְכוּת֙ תְּכֵ֣לֶת

**"in a garment of royalty of blue"**

Alternate translation: “a blue and white garment that the king had worn”

* **Associated Passages:** Esther 8:15

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:15 (#5) (id: 262701)

וַ⁠עֲטֶ֤רֶת זָהָב֙ גְּדוֹלָ֔ה

**"with a great crown of gold"**

Alternate translation: “a large golden crown”

* **Associated Passages:** Esther 8:15

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:15 (#6) (id: 262702)

וְ⁠הָ⁠עִ֣יר שׁוּשָׁ֔ן צָהֲלָ֖ה וְ⁠שָׂמֵֽחָה

**"and the city of Susa cheered and rejoiced"**

Here, **the city** represents the people living in it. Alternate translation: “the people of Susa”

See: Metonymy

* **Associated Passages:** Esther 8:15
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:15 (#7) (id: 262703)

צָהֲלָ֖ה וְ⁠שָׂמֵֽחָה

**"cheered and rejoiced"**

This phrase expresses a single idea by using two words connected with “and.” The word **rejoiced** tells how they cheered. If it would be helpful in your language, you could express the meaning by saying something like “shouted joyfully.” Alternate translation: “cheered and were happy”

See: Hendiadys

* **Associated Passages:** Esther 8:15
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:15 (#8) (id: 262704)

צָהֲלָ֖ה וְ⁠שָׂמֵֽחָה

**"cheered and rejoiced"**

The implication is that the people did this when they saw Mordecai. If it would be helpful in your language, you could say this explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`); Susa (ID: `place:Susa`); Crown (ID: `realia:Crown`); Linen (ID: `realia:Linen`); Purple Cloth (ID: `realia:PurpleCloth`)

## Esther 8:16 (#1) (id: 262705)

הָֽיְתָ֥ה אוֹרָ֖ה

**"there was light"**

Here, **light** represents happiness. Alternate translation: “the Jews felt happy”

See: Metaphor

* **Associated Passages:** Esther 8:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:16 (#2) (id: 262706)

הָֽיְתָ֥ה אוֹרָ֖ה וְ⁠שִׂמְחָ֑ה

**"there was light and joy"**

The terms **light** and **joy** refer to the same thing here. They are used together to emphasize the extreme happiness that the Jews felt. Alternate translation: “the Jews felt very happy”

See: Doublet

* **Associated Passages:** Esther 8:16
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:16 (#3) (id: 262707)

וְ⁠שָׂשֹׂ֖ן וִ⁠יקָֽר

**"and rejoicing and honor"**

These terms have similar meaning and are used together with the previous doublet to emphasize again the great happiness and joy that the Jews felt.

See: Doublet

* **Associated Passages:** Esther 8:16
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:16 (#4) (id: 262708)

וִ⁠יקָֽר

**"and honor"**

Here, **honor** might have two possible meanings. (1\) Other people honored the Jews. Alternate translation: “other people honored them” (2\) The Jews themselves felt honor instead of shame. Alternate translation: “they felt honored”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#1) (id: 262709)

וּ⁠בְ⁠כָל־מְדִינָ֨ה וּ⁠מְדִינָ֜ה

**"In every province by province"**

This expression means “each and every province.” Alternate translation: “in every single province”

See: Idiom

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#2) (id: 262710)

וּ⁠בְ⁠כָל־עִ֣יר וָ⁠עִ֗יר

**"and in every city by city"**

This expression means “each and every city.” Alternate translation: “in every single city”

See: Idiom

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#3) (id: 262711)

מְקוֹם֙ אֲשֶׁ֨ר דְּבַר־הַ⁠מֶּ֤לֶךְ וְ⁠דָת⁠וֹ֙ מַגִּ֔יעַ

**"any place where the word of the king and his law came"**

This expression describes the king’s message as if it were a person that could travel to a particular place. This phrase refers to all the places that received the king’s letter. Alternate translation: “wherever the king’s couriers took his decree” or “wherever the couriers read the letter announcing the king’s decree”

See: Personification

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#4) (id: 262712)

שִׂמְחָ֤ה וְ⁠שָׂשׂוֹן֙ לַ⁠יְּהוּדִ֔ים

**"there was joy and rejoicing for the Jews"**

The terms **joy** and **rejoicing** have similar meaning and are used together to emphasize the great happiness and joy that the Jews felt. Alternate translation: “the Jews rejoiced greatly”

See: Doublet

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#5) (id: 262713)

מִשְׁתֶּ֖ה וְ⁠י֣וֹם ט֑וֹב

**"a feast and a good day"**

These two terms mean something similar and are used together to emphasize the great happiness and joy that the Jews felt. Alternate translation: “and had festive celebrations”

See: Doublet

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#6) (id: 262714)

וְ⁠י֣וֹם ט֑וֹב

**"and a good day"**

This expression generally means a day of happiness or celebration. Alternate translation: “and a holiday”

See: Idiom

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#7) (id: 262715)

וְ⁠רַבִּ֞ים מֵֽ⁠עַמֵּ֤י הָ⁠אָ֨רֶץ֙ מִֽתְיַהֲדִ֔ים

**"And many from the peoples of the land became Jews"**

The implication is that they did this because they thought that the Jews might attack them when the Jews fought back against their enemies. If it would be helpful in your language, you could say this explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#8) (id: 262716)

וְ⁠רַבִּ֞ים מֵֽ⁠עַמֵּ֤י הָ⁠אָ֨רֶץ֙ מִֽתְיַהֲדִ֔ים

**"And many from the peoples of the land became Jews"**

Alternate translation: “many people from the other ethnic groups in the empire”

* **Associated Passages:** Esther 8:17

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#9) (id: 262717)

מֵֽ⁠עַמֵּ֤י הָ⁠אָ֨רֶץ֙

**"from the peoples of the land"**

The **peoples of the land** were the non\-Jewish people groups within the empire.

See: Idiom

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#10) (id: 262718)

נָפַ֥ל פַּֽחַד־הַ⁠יְּהוּדִ֖ים עֲלֵי⁠הֶֽם

**"dread of the Jews had fallen upon them"**

Here, **falling** is a metaphor meaning to affect someone. Alternate translation: “they had become very afraid of the Jews”

See: Metaphor

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 8:17 (#11) (id: 262719)

נָפַ֥ל פַּֽחַד־הַ⁠יְּהוּדִ֖ים עֲלֵי⁠הֶֽם

**"dread of the Jews had fallen upon them"**

To present the events in chronological order, you can say this before saying that the people from the other groups became Jews themselves.

See: Order of Events

* **Associated Passages:** Esther 8:17
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#1) (id: 262720)

וּ⁠בִ⁠שְׁנֵים֩ עָשָׂ֨ר חֹ֜דֶשׁ

**"Now in month 12"**

This introduces a new event. Use a natural way in your language to indicate a new event.

See: Introduction of a New Event

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#2) (id: 262721)

וּ⁠בִ⁠שְׁנֵים֩ עָשָׂ֨ר חֹ֜דֶשׁ הוּא־חֹ֣דֶשׁ אֲדָ֗ר בִּ⁠שְׁלוֹשָׁ֨ה עָשָׂ֥ר יוֹם֙ בּ֔⁠וֹ

**"Now in month 12, which is the month of Adar, on day 13 of it"**

The implication is that this happened in the twelfth month of the same year that the letters were sent out. If it would be helpful in your language, you could say that explicitly. Alternate translation: “on the thirteenth day of the twelfth month of that year, the month of Adar”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#3) (id: 262722)

וּ⁠בִ⁠שְׁנֵים֩ עָשָׂ֨ר חֹ֜דֶשׁ

**"Now in month 12"**

Alternate translation: “now in the twelfth month”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#4) (id: 262723)

**"which is the month of Adar"**

**Adar** is the name of the twelfth and last month of the Hebrew calendar. See how you translated this in [3:7](https://ref.ly/Esth3:7), [3:13](https://ref.ly/Esth3:13), and [8:12](https://ref.ly/Esth8:12).

See: Hebrew Months

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#5) (id: 262724)

בִּ⁠שְׁלוֹשָׁ֨ה עָשָׂ֥ר יוֹם֙ בּ֔⁠וֹ

**"on day 13 of it"**

Alternate translation: “on the thirteenth day of the month”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#6) (id: 262725)

אֲשֶׁ֨ר \& דְּבַר־הַ⁠מֶּ֛לֶךְ וְ⁠דָת֖⁠וֹ לְ⁠הֵעָשׂ֑וֹת

**"when the word of the king and his law"** \- **"the time to be done"**

Alternate translation: “what the letters said the king had decreed”

* **Associated Passages:** Esther 9:1

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#7) (id: 262726)

אֲשֶׁ֨ר הִגִּ֧יעַ דְּבַר־הַ⁠מֶּ֛לֶךְ וְ⁠דָת֖⁠וֹ לְ⁠הֵעָשׂ֑וֹת

**"when the word of the king and his law had reached the time to be done"**

This expression describes the king’s decree as if it had traveled through time (as a person travels through space) and reached this particular day. This phrase means that it was time for people to obey the decree. Alternate translation: “when the time came for people to obey the king’s law and decree” or “when it was the day established in the king’s letters for people to carry out the king’s law”

See: Personification

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#8) (id: 262727)

בַּ⁠יּ֗וֹם אֲשֶׁ֨ר שִׂבְּר֜וּ אֹיְבֵ֤י הַ⁠יְּהוּדִים֙ לִ⁠שְׁל֣וֹט בָּ⁠הֶ֔ם

**"on the day when the enemies of the Jews hoped to dominate them"**

Alternate translation: “the enemies of the Jews had expected to defeat the Jews on that day”

* **Associated Passages:** Esther 9:1

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#9) (id: 262728)

לִ⁠שְׁל֣וֹט

**"to dominate"**

This word usually means “to rule over,” but here it means “to have power over, to be able to destroy.”

See: Idiom

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#10) (id: 262729)

וְ⁠נַהֲפ֣וֹךְ

**"but being overturned"**

Saying that a situation was turned over is a figurative way of saying that what happened was the opposite of what was expected. Alternate translation: “the situation was reversed” or “the opposite happened”

See: Metaphor

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#11) (id: 262730)

ה֔וּא אֲשֶׁ֨ר יִשְׁלְט֧וּ הַ⁠יְּהוּדִ֛ים הֵ֖מָּה בְּ⁠שֹׂנְאֵי⁠הֶֽם

**"it happened that the Jews themselves dominated those who hated them"**

Alternate translation: “Instead, it was the Jews themselves who destroyed their enemies”

* **Associated Passages:** Esther 9:1

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:1 (#12) (id: 262731)

בְּ⁠שֹׂנְאֵי⁠הֶֽם

**"those who hated them"**

This is an idiom that describes enemies

See: Idiom

* **Associated Passages:** Esther 9:1
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:2 (#1) (id: 262732)

לִ⁠שְׁלֹ֣חַ יָ֔ד

**"to stretch out a hand"**

Here the expression **to stretch out a hand** means to cause someone physical harm with the intention of killing him. Alternate translation: “to defend themselves” or “to fight”

See: Idiom

* **Associated Passages:** Esther 9:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 9:2 (#2) (id: 262733)

בִּ⁠מְבַקְשֵׁ֖י רָֽעָתָ֑⁠ם

**"against those seeking their evil"**

Here the abstract noun **evil** likely means “harm,” as in [7:7](https://ref.ly/Esth7:7) and [8:6](https://ref.ly/Esth8:6). In this context, the term can be expressed with a verb. Alternate translation: “who were trying to destroy them”

See: Abstract Nouns

* **Associated Passages:** Esther 9:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 9:2 (#3) (id: 262734)

וְ⁠אִישׁ֙ לֹא־עָמַ֣ד לִ⁠פְנֵי⁠הֶ֔ם

**"But a man did not stand to their face"**

Alternate translation: “no one stood against them” or “no one was able to defeat them”

See: Idiom

* **Associated Passages:** Esther 9:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 9:2 (#4) (id: 262735)

וְ⁠אִישׁ֙ לֹא־עָמַ֣ד לִ⁠פְנֵי⁠הֶ֔ם

**"But a man did not stand to their face"**

Here, **standing** is a metaphor meaning to defend oneself and to fight back instead of running away from an enemy. Alternate translation: “was able to fight back”

See: Metaphor

* **Associated Passages:** Esther 9:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 9:2 (#5) (id: 262736)

לִ⁠פְנֵי⁠הֶ֔ם

**"to their face"**

Here, **face** stands for the presence of a person, so here it means “when faced with them.” Alternate translation: “against them”

See: Metonymy

* **Associated Passages:** Esther 9:2
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 9:2 (#6) (id: 262737)

נָפַ֥ל פַּחְדָּ֖⁠ם עַל־כָּל־הָ⁠עַמִּֽים

**"sudden fear of them had fallen on all the peoples"**

Here, **falling** is a metaphor meaning to affect someone. Alternate translation: “all the peoples suddenly became very afraid”

See: Metaphor

* **Associated Passages:** Esther 9:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 9:2 (#7) (id: 262738)

נָפַ֥ל פַּחְדָּ֖⁠ם עַל־כָּל־הָ⁠עַמִּֽים

**"sudden fear of them had fallen on all the peoples"**

The implication is that as a result, no one helped anyone who attacked the Jews. If it would be helpful in your language, you could say this explicitly.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 9:3 (#1) (id: 262739)

**"the officials of the provinces, and the satraps and the governors"**

See how you translated these terms in [3:12](https://ref.ly/Esth3:12) and [8:9](https://ref.ly/Esth8:9). Alternate translation: “the government leaders in each province”

* **Associated Passages:** Esther 9:3

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 9:3 (#2) (id: 262740)

וְ⁠עֹשֵׂ֤י הַ⁠מְּלָאכָה֙ אֲשֶׁ֣ר לַ⁠מֶּ֔לֶךְ

**"and those doing the work that was for the king"**

Alternate translation: “anyone the king had trusted with his affairs”

* **Associated Passages:** Esther 9:3

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 9:3 (#3) (id: 262741)

מְנַשְּׂאִ֖ים אֶת־הַ⁠יְּהוּדִ֑ים

**"were lifting up the Jews"**

Here, **lifting up** is a figurative way of saying “helping.” The picture is likely of someone helping a tired or injured person to stand or walk by holding them up. Alternate translation: “helped the Jews”

See: Metaphor

* **Associated Passages:** Esther 9:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 9:3 (#4) (id: 262742)

נָפַ֥ל פַּֽחַד־מָרְדֳּכַ֖י עֲלֵי⁠הֶֽם

**"dread of Mordecai had fallen on them"**

Here, **falling** is a metaphor meaning to affect someone. Alternate translation: “they were afraid of Mordecai”

See: Metaphor

* **Associated Passages:** Esther 9:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 9:4 (#1) (id: 262743)

כִּֽי

**"For"**

This term introduces the reason why the officials and satraps and governors were becoming afraid of Mordecai.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 9:4
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 9:4 (#2) (id: 262744)

גָ֤דוֹל מָרְדֳּכַי֙ בְּ⁠בֵ֣ית הַ⁠מֶּ֔לֶךְ

**"Mordecai was great in the palace of the king"**

The implication is that this is why all the other officials were afraid of Mordecai. If it would be helpful in your language, you could say this explicitly. Alternate translation, add: “They were afraid of him because”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 9:4 (#3) (id: 262745)

גָ֤דוֹל מָרְדֳּכַי֙ בְּ⁠בֵ֣ית הַ⁠מֶּ֔לֶךְ

**"Mordecai was great in the palace of the king"**

**The palace of the king** is a figurative way of describing the king’s administration by referring to the place where it was headquartered. Alternate translation: “was very important in the king’s government”

See: Metonymy

* **Associated Passages:** Esther 9:4
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 9:4 (#4) (id: 262746)

גָ֤דוֹל מָרְדֳּכַי֙ בְּ⁠בֵ֣ית הַ⁠מֶּ֔לֶךְ

**"Mordecai was great in the palace of the king"**

**Great** here is the same term that, as a verb, describes promotion within the king’s service in [3:1](https://ref.ly/Esth3:1) and [5:11](https://ref.ly/Esth5:11). Alternate translation: “Mordecai was a very important royal official”

* **Associated Passages:** Esther 9:4

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 9:4 (#5) (id: 262747)

וְ⁠שָׁמְע֖⁠וֹ הוֹלֵ֣ךְ בְּ⁠כָל־הַ⁠מְּדִינ֑וֹת

**"and the report of him was going out into all the provinces"**

Here the story speaks of the news of Mordecai’s greatness as if it were a living thing that could travel throughout the empire. Alternate translation: “throughout the empire, everyone was hearing the news of his greatness”

See: Personification

* **Associated Passages:** Esther 9:4
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 9:4 (#6) (id: 262748)

מָרְדֳּכַ֖י הוֹלֵ֥ךְ וְ⁠גָדֽוֹל

**"Mordecai was progressing and becoming great"**

This expression means that Mordecai continued to become more powerful and influential. Alternate translation: “Mordecai was becoming more famous because the king was giving him more and more power”

See: Idiom

* **Associated Passages:** Esther 9:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Mordecai (ID: `person:Mordecai.2`)

## Esther 9:5 (#1) (id: 262749)

וַ⁠יַּכּ֤וּ הַ⁠יְּהוּדִים֙ בְּ⁠כָל־אֹ֣יְבֵי⁠הֶ֔ם מַכַּת־חֶ֥רֶב

**"And the Jews struck to all their enemies a strike of sword"**

After the information about Mordecai, the story now returns to tell what happened on the appointed day. You could add a phrase to show this. Alternate translation, add: “on the day when they were allowed to defend themselves”

* **Associated Passages:** Esther 9:5

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`); Jews (ID: `group:Jews`)

## Esther 9:5 (#2) (id: 262750)

וַ⁠יַּכּ֤וּ הַ⁠יְּהוּדִים֙ בְּ⁠כָל־אֹ֣יְבֵי⁠הֶ֔ם מַכַּת־חֶ֥רֶב

**"And the Jews struck to all their enemies a strike of sword"**

This expression means that the Jews defended themselves against their enemies, even to the point of killing people who attacked them. Alternate translation: “the Jews attacked all of their enemies and killed them with their swords”

See: Idiom

* **Associated Passages:** Esther 9:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`); Jews (ID: `group:Jews`)

## Esther 9:5 (#3) (id: 262751)

מַכַּת־חֶ֥רֶב

**"a strike of sword"**

Swords were not necessarily the only weapons the Jews had and used. The sword is used to represent all of their weaponry. If it would be helpful in your language, you could express this more general meaning. Alternate translation: “the Jews took up their weapons and used them against their enemies”

See: Synecdoche

* **Associated Passages:** Esther 9:5
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`); Jews (ID: `group:Jews`)

## Esther 9:5 (#4) (id: 262752)

וְ⁠הֶ֖רֶג וְ⁠אַבְדָ֑ן

**"and slaughter and destruction"**

These two words have the same meaning and are used together for emphasis. Alternate translation: “they destroyed them completely”

See: Doublet

* **Associated Passages:** Esther 9:5
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`); Jews (ID: `group:Jews`)

## Esther 9:5 (#5) (id: 262753)

וַ⁠יַּֽעֲשׂ֥וּ בְ⁠שֹׂנְאֵי⁠הֶ֖ם כִּ⁠רְצוֹנָֽ⁠ם

**"And they did to those who hated them according to their pleasure"**

This expression does not mean that the Jews felt pleasure when they killed their enemies. Rather, it means that the Jews were able to defend themselves against their enemies and were not hindered in any way. Alternate translation: “they were able to do everything they wanted to do against their enemies”

See: Idiom

* **Associated Passages:** Esther 9:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`); Jews (ID: `group:Jews`)

## Esther 9:5 (#6) (id: 262754)

בְ⁠שֹׂנְאֵי⁠הֶ֖ם

**"to those who hated them"**

This is an idiom that describes enemies.

See: Idiom

* **Associated Passages:** Esther 9:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Delight (ID: `keyterm:Delight`); Jews (ID: `group:Jews`)

## Esther 9:6 (#1) (id: 262755)

וּ⁠בְ⁠שׁוּשַׁ֣ן הַ⁠בִּירָ֗ה

**"And in Susa the citadel"**

Alternate translation: “just in Susa, the capital city”

* **Associated Passages:** Esther 9:6

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:6 (#2) (id: 262756)

הָרְג֤וּ \& וְ⁠אַבֵּ֔ד

**"slaughtered and destroyed"**

These two words have the same meaning and are used together for emphasis. Alternate translation: “killed”

See: Doublet

* **Associated Passages:** Esther 9:6
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:6 (#3) (id: 262757)

חֲמֵ֥שׁ מֵא֖וֹת אִֽישׁ

**"500 men"**

Alternate translation: “five hundred men”

See: Numbers

* **Associated Passages:** Esther 9:6
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:7 (#1) (id: 262758)

פַּרְשַׁנְדָּ֛תָא \& דַּֽלְפ֖וֹן \& אַסְפָּֽתָא

**"Parshandatha"** \- **"Dalphon"** \- **"Aspatha"**

The story lists the names of Haman’s sons before explaining that they were his sons and that the Jews killed them. To be clear who these people are, you can add this information from verse 10 explicitly here. Alternate translation, add: “rhe Jews killed the ten sons of their enemy Haman son of Hammedatha. The names of his sons were … ”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Parshandatha (ID: `person:Parshandatha`); Dalphon (ID: `person:Dalphon`); Aspatha (ID: `person:Aspatha`)

## Esther 9:7 (#2) (id: 262759)

פַּרְשַׁנְדָּ֛תָא \& דַּֽלְפ֖וֹן \& אַסְפָּֽתָא

**"Parshandatha"** \- **"Dalphon"** \- **"Aspatha"**

These are the names of men.

See: How to Translate Names

* **Associated Passages:** Esther 9:7
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Parshandatha (ID: `person:Parshandatha`); Dalphon (ID: `person:Dalphon`); Aspatha (ID: `person:Aspatha`)

## Esther 9:8 (#1) (id: 262760)

פּוֹרָ֛תָא \& אֲדַלְיָ֖א \& אֲרִידָֽתָא

**"Poratha"** \- **"Adalia"** \- **"Aridatha"**

These are the names of men.

See: How to Translate Names

* **Associated Passages:** Esther 9:8
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Poratha (ID: `person:Poratha`); Adalia (ID: `person:Adalia`); Aridatha (ID: `person:Aridatha`)

## Esther 9:9 (#1) (id: 262761)

פַּרְמַ֨שְׁתָּא֙ \& אֲרִיסַ֔י \& אֲרִדַ֖י \& וַיְזָֽתָא

**"Parmashta"** \- **"Arisai"** \- **"Aridai"** \- **"Vaizatha"**

These are the names of men.

See: How to Translate Names

* **Associated Passages:** Esther 9:9
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Arisai (ID: `person:Arisai`); Parmashta (ID: `person:Parmashta`); Vaizatha (ID: `person:Vaizatha`); Aridai (ID: `person:Aridai`)

## Esther 9:10 (#1) (id: 262762)

עֲ֠שֶׂרֶת בְּנֵ֨י

**"the ten sons of"**

Alternate translation: “the 10 sons”

See: Numbers

* **Associated Passages:** Esther 9:10
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 9:10 (#2) (id: 262763)

צֹרֵ֥ר הַ⁠יְּהוּדִ֖ים

**"the adversary of the Jews"**

This phrase gives clarifying information about Haman. Alternate translation: “the enemy of the Jews”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 9:10
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 9:10 (#3) (id: 262764)

לֹ֥א שָׁלְח֖וּ אֶת־יָדָֽ⁠ם

**"they did not stretch out their hand"**

Here the expression **to stretch out a hand** means to take something from another person. Alternate translation: “they did not take”

See: Idiom

* **Associated Passages:** Esther 9:10
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 9:10 (#4) (id: 262765)

וּ⁠בַ֨⁠בִּזָּ֔ה

**"But"** \- **"to the plunder"**

Alternate translation: “But … their possessions”

* **Associated Passages:** Esther 9:10

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`)

## Esther 9:11 (#1) (id: 262766)

בַּ⁠יּ֣וֹם הַ⁠ה֗וּא

**"On that day"**

Alternate translation: “at the end of that day”

* **Associated Passages:** Esther 9:11

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 9:11 (#2) (id: 262767)

בָּ֣א מִסְפַּ֧ר \& לִ⁠פְנֵ֥י הַ⁠מֶּֽלֶךְ

**"the report of the number of"** \- **"came to the face of the king"**

Here the story speaks about the report as if it were a living thing that could come into the king’s presence. Alternate translation: “one of the king’s servants told the king the number of”

See: Personification

* **Associated Passages:** Esther 9:11
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 9:11 (#3) (id: 262768)

לִ⁠פְנֵ֥י הַ⁠מֶּֽלֶךְ

**"to the face of the king"**

Here, **face** represents the presence of a person. This phrase likely means that someone came into the king’s presence in order to deliver this report. Alternate translation: “someone came in and reported to the king”

See: Metonymy

* **Associated Passages:** Esther 9:11
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 9:11 (#4) (id: 262769)

מִסְפַּ֧ר הַֽ⁠הֲרוּגִ֛ים

**"the report of the number of those who were killed"**

You can say this with an active form, and you can say who did the action. Alternate translation: “how many people the Jews had killed”

See: Active or Passive

* **Associated Passages:** Esther 9:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`)

## Esther 9:12 (#1) (id: 262770)

הָרְגוּ֩ \& וְ⁠אַבֵּ֜ד

**"have slaughtered and destroyed"**

These terms mean the same thing and are used together for emphasis. Alternate translation: “killed”

See: Doublet

* **Associated Passages:** Esther 9:12
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:12 (#2) (id: 262771)

חֲמֵ֧שׁ מֵא֣וֹת אִ֗ישׁ

**"500 men"**

Alternate translation: “five hundred men”

See: Numbers

* **Associated Passages:** Esther 9:12
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:12 (#3) (id: 262772)

וְ⁠אֵת֙

**"with"**

Alternate translation: “including”

* **Associated Passages:** Esther 9:12

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:12 (#4) (id: 262773)

עֲשֶׂ֣רֶת בְּנֵֽי

**"ten sons of"**

Alternate translation: “10 sons”

See: Numbers

* **Associated Passages:** Esther 9:12
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:12 (#5) (id: 262774)

בִּ⁠שְׁאָ֛ר מְדִינ֥וֹת הַ⁠מֶּ֖לֶךְ מֶ֣ה עָשׂ֑וּ

**"In the rest of the provinces of the king, what have they done"**

The king is making a statement, but he uses a question form to show that he is very convinced that the Jews must have also killed many people in the other provinces. Alternate translation: “what they must have done in the rest of the king’s provinces” or “they must have killed many more in the rest of the king’s provinces”

See: Rhetorical Question

* **Associated Passages:** Esther 9:12
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:12 (#6) (id: 262775)

וּ⁠מַה־שְּׁאֵֽלָתֵ⁠ךְ֙ וְ⁠יִנָּ֣תֵֽן לָ֔⁠ךְ וּ⁠מַה־בַּקָּשָׁתֵ֥⁠ךְ ע֖וֹד וְ⁠תֵעָֽשׂ

**"Now what is your petition? And it will be given to you. And what is your request? Again, and it will be done"**

These two statements mean basically the same thing. Ahasuerus says the same thing twice to show that he is truly disposed to give Esther what she wants. If it would be confusing, you do not need to repeat both phrases in your translation. Alternate translation: “is there anything more that you want? Tell me, and I will do it for you”

See: Parallelism

* **Associated Passages:** Esther 9:12
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:12 (#7) (id: 262776)

וְ⁠יִנָּ֣תֵֽן לָ֔⁠ךְ \& וְ⁠תֵעָֽשׂ

**"And it will be given to you."** \- **"and it will be done"**

You can capture these parallel passive statements with one statement using an active form. You can also say who will do the action. Alternate translation: “I will do it for you”

See: Active or Passive

* **Associated Passages:** Esther 9:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:12 (#8) (id: 262777)

שְּׁאֵֽלָתֵ⁠ךְ֙ \& בַּקָּשָׁתֵ֥⁠ךְ

**"your petition?"** \- **"your request"**

The two abstract nouns **petition** and **request** can be expressed with a single phrase using the verb “want.” Alternate translation: “anything more that you want”

See: Abstract Nouns

* **Associated Passages:** Esther 9:12
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#1) (id: 262778)

אִם־עַל־הַ⁠מֶּ֣לֶךְ ט֔וֹב

**"If it is good to the king"**

This is an idiom that has been used many times in the story. Alternate translation: “if it seems like a good idea to you, O king”

See: Idiom

* **Associated Passages:** Esther 9:13
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#2) (id: 262779)

יִנָּתֵ֣ן

**"let it be given"**

You can say this with an active form, and you can indicate that Esther is asking the king to do it. Alternate translation: “please allow”

See: Active or Passive

* **Associated Passages:** Esther 9:13
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#3) (id: 262780)

גַּם־מָחָ֗ר לַ⁠יְּהוּדִים֙ אֲשֶׁ֣ר בְּ⁠שׁוּשָׁ֔ן לַ⁠עֲשׂ֖וֹת כְּ⁠דָ֣ת הַ⁠יּ֑וֹם

**"also tomorrow to the Jews who are in Susa to do according to the law of today"**

Alternate translation: “all the Jews who live in Susa to obey today’s decree tomorrow also” or “to do tomorrow also what was decreed that they should do today”

* **Associated Passages:** Esther 9:13

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#4) (id: 262781)

בְּ⁠שׁוּשָׁ֔ן

**"are in Susa"**

Alternate translation: “in the capital city of Susa”

* **Associated Passages:** Esther 9:13

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#5) (id: 262782)

וְ⁠אֵ֛ת עֲשֶׂ֥רֶת בְּנֵֽי־הָמָ֖ן יִתְל֥וּ

**"let the ten sons of Haman hang"**

The request is not for the king to allow the bodies to be impaled (or hanged), but instead, for the king to order this. Alternate translation: “and have the bodies of Haman’s ten sons hanged \[or impaled]”

* **Associated Passages:** Esther 9:13

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#6) (id: 262783)

עֲשֶׂ֥רֶת בְּנֵֽי־הָמָ֖ן יִתְל֥וּ

**"let the ten sons of Haman hang"**

The purpose of this would not be to kill the sons, since they are already dead, but to demonstrate publicly that the enemies of the Jews had been completely defeated.

See: Symbolic Action

* **Associated Passages:** Esther 9:13
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#7) (id: 262784)

עֲשֶׂ֥רֶת בְּנֵֽי־הָמָ֖ן יִתְל֥וּ

**"let the ten sons of Haman hang"**

It is implicit that since Haman’s sons are already dead, what Esther is actually asking for is for their bodies to be impaled or hanged. If it would be helpful in your language, you could say this explicitly. Alternate translation: “the bodies of Haman’s ten sons”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:13
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#8) (id: 262785)

עֲשֶׂ֥רֶת בְּנֵֽי

**"ten sons of"**

Alternate translation: “10 sons”

See: Numbers

* **Associated Passages:** Esther 9:13
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:13 (#9) (id: 262786)

**"the pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful.

See: Translate Unknowns

* **Associated Passages:** Esther 9:13
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`); Esther (ID: `person:Esther`); Haman (ID: `person:Haman`)

## Esther 9:14 (#1) (id: 262787)

וַ⁠יֹּ֤אמֶר הַ⁠מֶּ֨לֶךְ֙ לְ⁠הֵֽעָשׂ֣וֹת כֵּ֔ן

**"And the king said for this to be done"**

You can say this with an active form. Alternate translation: “the king granted both of Esther’s requests”

See: Active or Passive

* **Associated Passages:** Esther 9:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 9:14 (#2) (id: 262788)

וַ⁠תִּנָּתֵ֥ן דָּ֖ת בְּ⁠שׁוּשָׁ֑ן

**"And a law was given in Susa"**

This phrase, “in Susa,” seems to indicate that this refers to Esther’s first request. If it would be helpful in your language, you could say that explicitly. Alternate translation: “he issued a decree allowing the Jews in Susa to fight against their enemies again the next day”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 9:14 (#3) (id: 262789)

וַ⁠תִּנָּתֵ֥ן דָּ֖ת בְּ⁠שׁוּשָׁ֑ן

**"And a law was given in Susa"**

This can be stated in active form as in the UST.

See: Active or Passive

* **Associated Passages:** Esther 9:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 9:14 (#4) (id: 262790)

בְּ⁠שׁוּשָׁ֑ן

**"in Susa"**

Alternate translation: “in the capital city of Susa”

* **Associated Passages:** Esther 9:14

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 9:14 (#5) (id: 262791)

תָּלֽוּ

**"they hanged"**

This was Esther’s second request. You can say who did the action. Alternate translation: “he ordered his servants to hang \[or impale] the bodies of Haman’s ten sons”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 9:14 (#6) (id: 262792)

עֲשֶׂ֥רֶת בְּנֵֽי

**"ten sons of"**

Alternate translation: “10 sons”

See: Numbers

* **Associated Passages:** Esther 9:14
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Haman (ID: `person:Haman`)

## Esther 9:15 (#1) (id: 262793)

בְּ⁠שׁוּשָׁ֗ן

**"were in Susa"**

Alternate translation: “who lived in the capital city of Susa”

* **Associated Passages:** Esther 9:15

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:15 (#2) (id: 262794)

וַ⁠יִּֽקָּהֲל֞וּ

**"And"** \- **"assembled themselves"**

Alternate translation: “joined together”

* **Associated Passages:** Esther 9:15

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:15 (#3) (id: 262795)

בְּ⁠י֣וֹם אַרְבָּעָ֤ה עָשָׂר֙

**"on day 14"**

Alternate translation: “on day 14”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:15
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:15 (#4) (id: 262796)

**"of the month of Adar"**

**Adar** is the name of the twelfth and last month of the Hebrew calendar. See how you translated this in [3:7](https://ref.ly/Esth3:7), [3:13](https://ref.ly/Esth3:13), [8:12](https://ref.ly/Esth8:12), and [9:1](https://ref.ly/Esth9:1).

See: Hebrew Months

* **Associated Passages:** Esther 9:15
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:15 (#5) (id: 262797)

בְּ⁠שׁוּשָׁ֗ן

**"in Susa"**

Alternate translation: “in the capital city of Susa”

* **Associated Passages:** Esther 9:15

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:15 (#6) (id: 262798)

שְׁלֹ֥שׁ מֵא֖וֹת אִ֑ישׁ

**"300 men"**

Alternate translation: “three hundred men”

See: Numbers

* **Associated Passages:** Esther 9:15
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:15 (#7) (id: 262799)

וּ⁠בַ֨⁠בִּזָּ֔ה לֹ֥א שָׁלְח֖וּ אֶת־יָדָֽ⁠ם

**"But they did not stretch out their hand to the plunder"**

Here, the expression **to stretch out a hand** means to take something from another person. Alternate translation: “they did not take the things that belonged to them”

See: Idiom

* **Associated Passages:** Esther 9:15
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:16 (#1) (id: 262800)

נִקְהֲל֣וּ ׀ וְ⁠עָמֹ֣ד עַל־נַפְשָׁ֗⁠ם

**"assembled themselves and stood for their lives"**

Here, **standing** is a metaphor meaning to defend oneself and to fight back instead of running away from an enemy. See how you translated this in [8:11](https://ref.ly/Esth8:11). Alternate translation: “joined together to fight back against their enemies”

See: Metaphor

* **Associated Passages:** Esther 9:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:16 (#2) (id: 262801)

נִקְהֲל֣וּ ׀ וְ⁠עָמֹ֣ד עַל־נַפְשָׁ֗⁠ם

**"assembled themselves and stood for their lives"**

It’s implicit here, and stated explicitly in the next verse, that these other Jews fought their enemies only on the thirteenth day. If it would be helpful in your language, you could say that explicitly here. Alternate translation, add: “on the thirteenth day of the month of Adar”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:16 (#3) (id: 262802)

וְ⁠נ֨וֹחַ֙ מֵ⁠אֹ֣יְבֵי⁠הֶ֔ם

**"and they rested from their enemies"**

Here, **resting** is a figurative way of saying that they no longer needed to fight against their enemies because they had won the battle. Alternate translation: “they had no more trouble from their enemies”

See: Metaphor

* **Associated Passages:** Esther 9:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:16 (#4) (id: 262803)

וְ⁠הָרֹג֙ בְּ⁠שֹׂ֣נְאֵי⁠הֶ֔ם חֲמִשָּׁ֥ה וְ⁠שִׁבְעִ֖ים אָ֑לֶף

**"and they slaughtered 75,000 of those who hated them"**

Alternate translation: “and killed 75,000 of them”

* **Associated Passages:** Esther 9:16

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:16 (#5) (id: 262804)

חֲמִשָּׁ֥ה וְ⁠שִׁבְעִ֖ים אָ֑לֶף

**"75,000"**

Alternate translation: “seventy\-five thousand”

See: Numbers

* **Associated Passages:** Esther 9:16
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:16 (#6) (id: 262805)

לֹ֥א שָֽׁלְח֖וּ אֶת־יָדָֽ⁠ם

**"they did not stretch out their hand"**

Alternate translation: “they did not take the things that belonged to them” or “they did not take the valuable things” or “the did not take their possessions”

* **Associated Passages:** Esther 9:16

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:17 (#1) (id: 262806)

בְּ⁠יוֹם־שְׁלֹשָׁ֥ה עָשָׂ֖ר

**"On day 13"**

Alternate translation: “on the thirteenth day”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:17
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)


## Esther 9:17 (#2) (id: 262807)

**"of the month of Adar"**

**Adar** is the name of the twelfth and last month of the Hebrew calendar. See how you translated this in [3:7](https://ref.ly/Esth3:7), [3:13](https://ref.ly/Esth3:13), [8:12](https://ref.ly/Esth8:12), [9:1](https://ref.ly/Esth9:1), and [9:15](https://ref.ly/Esth9:15).

See: Hebrew Months

* **Associated Passages:** Esther 9:17
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)


## Esther 9:17 (#3) (id: 262808)

בְּ⁠אַרְבָּעָ֤ה עָשָׂר֙ בּ֔⁠וֹ וְ⁠עָשֹׂ֣ה אֹת֔⁠וֹ י֖וֹם מִשְׁתֶּ֥ה וְ⁠שִׂמְחָֽה

**"On the fourteenth of it, then they made it a day of feasting and rejoicing"**

Alternate translation: “they devoted the next day, the fourteenth day of the month of Adar, to joyful celebration” or “they feasted joyfully the next day, the fourteenth day of the month of Adar”

* **Associated Passages:** Esther 9:17


## Esther 9:17 (#4) (id: 262809)

בְּ⁠אַרְבָּעָ֤ה עָשָׂר֙ בּ֔⁠וֹ

**"On the fourteenth of it"**

Alternate translation: “on day 14”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:17
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)


## Esther 9:17 (#5) (id: 262810)

מִשְׁתֶּ֥ה וְ⁠שִׂמְחָֽה

**"feasting and rejoicing"**

This phrase expresses a single idea by using two words connected with “and.” The word **rejoicing** tells how they celebrated. Alternate translation: “joyful celebration”

See: Hendiadys

* **Associated Passages:** Esther 9:17
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)


## Esther 9:17 (#6) (id: 262811)

מִשְׁתֶּ֥ה

**"feasting"**

This is a figure of speech in which a part of something is used to mean the whole thing. The celebrations must have included more than just eating special meals together, but the story uses those meals to refer to the entire celebrations.

See: Synecdoche

* **Associated Passages:** Esther 9:17
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)


## Esther 9:18 (#1) (id: 262812)

וְהַיְּהוּדִ֣ים אֲשֶׁר־בְּ⁠שׁוּשָׁ֗ן נִקְהֲלוּ֙

**"But the Jews who were in Susa assembled themselves"**

Alternate translation: “but the Jews who lived in the capital city of Susa”

* **Associated Passages:** Esther 9:18

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:18 (#2) (id: 262813)

נִקְהֲלוּ֙

**"assembled themselves"**

The implication is that they did this to fight against their enemies. This can be stated explicitly. Alternate translation: “the Jews who lived in Susa joined together to fight against their enemies”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:18 (#3) (id: 262814)

בִּ⁠שְׁלֹשָׁ֤ה עָשָׂר֙ בּ֔⁠וֹ וּ⁠בְ⁠אַרְבָּעָ֥ה עָשָׂ֖ר בּ֑⁠וֹ

**"on the thirteenth of it and on the fourteenth of it"**

If it would be helpful in your language, you could say which month this is. Alternate translation: “on the thirteenth and fourteenth days of the month of Adar”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:18 (#4) (id: 262815)

בִּ⁠שְׁלֹשָׁ֤ה עָשָׂר֙ בּ֔⁠וֹ

**"on the thirteenth of it"**

Alternate translation: “on day 13”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:18
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:18 (#5) (id: 262816)

וּ⁠בְ⁠אַרְבָּעָ֥ה עָשָׂ֖ר בּ֑⁠וֹ

**"and on the fourteenth of it"**

Alternate translation: “and on day 14”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:18
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:18 (#6) (id: 262817)

וְ⁠נ֗וֹחַ בַּ⁠חֲמִשָּׁ֤ה עָשָׂר֙ בּ֔⁠וֹ וְ⁠עָשֹׂ֣ה אֹת֔⁠וֹ י֖וֹם מִשְׁתֶּ֥ה וְ⁠שִׂמְחָֽה

**"And they rested on the fifteenth of it, and they made it a day of feasting and rejoicing"**

**Resting** is a figurative way of saying that they no longer needed to fight against their enemies because they had won the battle. Alternate translation: “they defeated them, and there was no fighting on the fifteenth day. They devoted that day to joyful celebration”

See: Metaphor

* **Associated Passages:** Esther 9:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:18 (#7) (id: 262818)

בַּ⁠חֲמִשָּׁ֤ה עָשָׂר֙ בּ֔⁠וֹ

**"on the fifteenth of it"**

Alternate translation: “on day 15”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:18
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:18 (#8) (id: 262819)

מִשְׁתֶּ֥ה וְ⁠שִׂמְחָֽה

**"feasting and rejoicing"**

This phrase means the same thing as in the previous verse. Alternate translation: “for feasting joyfully”

See: Hendiadys

* **Associated Passages:** Esther 9:18
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Susa (ID: `place:Susa`); Jews (ID: `group:Jews`)

## Esther 9:19 (#1) (id: 262820)

עַל־כֵּ֞ן

**"Therefore"**

This term introduces the result of the events that were described in the previous verse. Alternate translation: “for that reason” or “that is why”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#2) (id: 262821)

הַ⁠יְּהוּדִ֣ים הַפְּרָזִ֗ים הַ⁠יֹּשְׁבִים֮ בְּ⁠עָרֵ֣י הַ⁠פְּרָזוֹת֒

**"the Jews, the ones of the open country, the ones who dwell in the cities of the open areas"**

These two phrases mean similar things. They are used together to make the identification of this group clear. The first phrase means that they lived in rural areas. The second phrase means they lived in settlements that did not have walls around them, that is, in villages. You can combine these phrases. Alternate translation: “the Jews who live in villages in the countryside”

See: Parallelism

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#3) (id: 262822)

עֹשִׂ֗ים אֵ֠ת י֣וֹם אַרְבָּעָ֤ה עָשָׂר֙ \& וְ⁠י֣וֹם ט֑וֹב

**"make day 14 of"** \- **"and a good day"**

The implication is that this explanation is being offered for the benefit of city\-dwelling Jews who might wonder why rural Jews celebrate this holiday on a different day. If it would be helpful in your language, you could say this explicitly. Alternate translation, add: “rather than on the fifteenth day.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#4) (id: 262823)

עֹשִׂ֗ים אֵ֠ת י֣וֹם אַרְבָּעָ֤ה עָשָׂר֙ \& וְ⁠י֣וֹם ט֑וֹב

**"make day 14 of"** \- **"and a good day"**

Alternate translation: “observe this holiday on the fourteenth day”

* **Associated Passages:** Esther 9:19

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#5) (id: 262824)

אֵ֠ת י֣וֹם אַרְבָּעָ֤ה עָשָׂר֙

**"day 14 of"**

Alternate translation: “the fourteenth day”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#6) (id: 262825)

**"the month of Adar"**

**Adar** is the name of the twelfth and last month of the Hebrew calendar. See how you translated this in [3:7](https://ref.ly/Esth3:7), [3:13](https://ref.ly/Esth3:13), [8:12](https://ref.ly/Esth8:12), [9:1](https://ref.ly/Esth9:1), [9:15](https://ref.ly/Esth9:15) and [9:17](https://ref.ly/Esth9:17).

See: Hebrew Months

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#7) (id: 262826)

**"for rejoicing and for feasting"**

As in verses 17 and 18, this means “by celebrating joyfully”

See: Hendiadys

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#8) (id: 262827)

וְ⁠י֣וֹם ט֑וֹב

**"and a good day"**

This expression generally means a day of happiness or celebration.

See: Idiom

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#9) (id: 262828)

וּ⁠מִשְׁל֥וֹחַ מָנ֖וֹת אִ֥ישׁ לְ⁠רֵעֵֽ⁠הוּ

**"and for the sending of gifts, a man to his friend"**

Giving gifts, in this culture as in many cultures, was a way of acknowledging a special occasion. Alternate translation: “and by giving gifts to one another”

See: Symbolic Action

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:19 (#10) (id: 262829)

אִ֥ישׁ לְ⁠רֵעֵֽ⁠הוּ

**"a man to his friend"**

Here, **a man** means “a person.” The term **friend** would include family members and neighbors as well as social friends. Alternate translation: “to one another”

See: Idiom

* **Associated Passages:** Esther 9:19
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:20 (#1) (id: 262830)

וַ⁠יִּכְתֹּ֣ב \& אֶת־הַ⁠דְּבָרִ֖ים הָ⁠אֵ֑לֶּה

**"And"** \- **"wrote these things"**

Alternate translation: “wrote an account of all of these events”

* **Associated Passages:** Esther 9:20

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:20 (#2) (id: 262831)

כָּל־הַ⁠יְּהוּדִ֗ים אֲשֶׁר֙ בְּ⁠כָל־מְדִינוֹת֙ הַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֔וֹשׁ

**"all the Jews who were in all the provinces of the king Ahasuerus"**

Alternate translation: “all the Jews everywhere in the kingdom”

* **Associated Passages:** Esther 9:20

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:20 (#3) (id: 262832)

הַ⁠קְּרוֹבִ֖ים וְ⁠הָ⁠רְחוֹקִֽים

**"the near ones and the far ones"**

This is a figurative way of referring to something by speaking of two extreme parts of it in order to include everything in between. This expression means the Jews who lived in or near Susa, those who lived far away, and all Jews in between. Alternate translation: “everywhere they lived”

See: Merism

* **Associated Passages:** Esther 9:20
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:21 (#1) (id: 262833)

לְ⁠קַיֵּם֮ עֲלֵי⁠הֶם֒ לִ⁠הְי֣וֹת עֹשִׂ֗ים

**"to set up for them to be making"**

Here, **to set up** means to establish, and to **make** a day means to observe it as a holiday. Alternate translation: “to establish … as a holiday”

See: Idiom

* **Associated Passages:** Esther 9:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Esther 9:21 (#2) (id: 262834)

אֵ֠ת י֣וֹם אַרְבָּעָ֤ה עָשָׂר֙

**"day 14"**

Alternate translation: “the fourteenth day”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:21
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)


## Esther 9:21 (#3) (id: 262835)

**"of the month of Adar"**

**Adar** is the name of the twelfth and last month of the Hebrew calendar. See how you translated this in [3:7](https://ref.ly/Esth3:7), [3:13](https://ref.ly/Esth3:13), [8:12](https://ref.ly/Esth8:12), [9:1](https://ref.ly/Esth9:1), [9:15](https://ref.ly/Esth9:15), [9:17](https://ref.ly/Esth9:17), and [9:19](https://ref.ly/Esth9:19).

See: Hebrew Months

* **Associated Passages:** Esther 9:21
* **Associated Articles:** Hebrew Months (ID: `33382@UWTranslationManual`)


## Esther 9:21 (#4) (id: 262836)

יוֹם־חֲמִשָּׁ֥ה עָשָׂ֖ר בּ֑⁠וֹ

**"day 15 of it"**

Alternate translation: “the fifteenth day”

See: Ordinal Numbers

* **Associated Passages:** Esther 9:21
* **Associated Articles:** Ordinal Numbers (ID: `33393@UWTranslationManual`)


## Esther 9:21 (#5) (id: 262837)

בְּ⁠כָל־שָׁנָ֖ה וְ⁠שָׁנָֽה

**"every year by year"**

This expression means “every year.” Alternation translation: “each year”

See: Idiom

* **Associated Passages:** Esther 9:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)


## Esther 9:22 (#1) (id: 262838)

כַּ⁠יָּמִ֗ים אֲשֶׁר

**"as the days when"** \- **"as"**

This verse gives the reason for what Mordecai told the Jews to do in the previous verse. Alternate translation: “because those were the days when”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#2) (id: 262839)

**"the Jews rested on them from their enemies"**

As in verses 16, 17, and 18, **resting** here is a figurative way of saying that they no longer needed to fight against their enemies because they had won the battle. Alternate translation: “the Jews stopped fighting because they had defeated their enemies”

See: Metaphor

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#3) (id: 262840)

וְ⁠הַ⁠חֹ֗דֶשׁ אֲשֶׁר֩

**"and"** \- **"the month when"**

Alternate translation: “And that was the month when”

* **Associated Passages:** Esther 9:22

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#4) (id: 262841)

נֶהְפַּ֨ךְ לָ⁠הֶ֤ם מִ⁠יָּגוֹן֙ לְ⁠שִׂמְחָ֔ה וּ⁠מֵ⁠אֵ֖בֶל לְ⁠י֣וֹם ט֑וֹב

**"it had turned for them from sorrow into joy and from mourning into a good day"**

These two phrases mean similar things. The repetition is used to emphasize how dramatic and wonderful the change was. If it would be helpful in your language, you could combine these phrases and say something like “after being very sad, they became very happy.” Alternate translation: “they changed from being very sorrowful and crying to being very joyful and celebrating”

See: Parallelism

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#5) (id: 262842)

נֶהְפַּ֨ךְ לָ⁠הֶ֤ם מִ⁠יָּגוֹן֙ לְ⁠שִׂמְחָ֔ה וּ⁠מֵ⁠אֵ֖בֶל לְ⁠י֣וֹם ט֑וֹב

**"it had turned for them from sorrow into joy and from mourning into a good day"**

It is implicit that the Jews were deeply distressed because they were being threatened with destruction, and they became very happy once they were safe from all their enemies. If it would be helpful in your language, you could say that here. Alternate translation: “everything had changed for them. They had been deeply distressed because they were threatened with destruction. But they became very happy once they were safe from all their enemies”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#6) (id: 262843)

נֶהְפַּ֨ךְ לָ⁠הֶ֤ם מִ⁠יָּגוֹן֙ לְ⁠שִׂמְחָ֔ה

**"it had turned for them from sorrow into joy"**

**Turning** represents changing. Alternate translation: “they changed from being very sad to being joyful”

See: Metaphor

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#7) (id: 262844)

מִ⁠יָּגוֹן֙ לְ⁠שִׂמְחָ֔ה

**"from sorrow into joy"**

The abstract nouns **sorrow** and **joy** can be expressed with adjectives such as “distressed” and “happy.”

See: Abstract Nouns

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#8) (id: 262845)

לַ⁠עֲשׂ֣וֹת אוֹתָ֗⁠ם יְמֵי֙

**"in order to make them days of"**

These are things that Mordecai is telling the Jews to do in his letters. If it would be helpful in your language, you could say that here. Alternate translation: “so Mordecai told them to observe those days with”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#9) (id: 262846)

**"feasting and rejoicing"**

As in verses 17, 18, and 19, this phrase expresses a single idea by using two words connected with “and.” The word “rejoicing” tells how the celebrating was to be done. Alternate translation: “joyful celebration” or “feasting joyfully”

See: Hendiadys

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#10) (id: 262847)

וּ⁠מִשְׁל֤וֹחַ מָנוֹת֙ אִ֣ישׁ לְ⁠רֵעֵ֔⁠הוּ

**"and sending of gifts, a man to his friend"**

See how you translated this in verse 19\. Review the notes there if that would be helpful. Alternate translation: “and by giving gifts to one another”

* **Associated Passages:** Esther 9:22

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#11) (id: 262848)

אִ֣ישׁ לְ⁠רֵעֵ֔⁠הוּ

**"a man to his friend"**

Here, **a man** means “a person.” The term **friend** would include family members and neighbors as well as social friends. Alternate translation: “to one another”

See: Idiom

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#12) (id: 262849)

וּ⁠מַתָּנ֖וֹת לָֽ⁠אֶבְיוֹנִֽים

**"and gifts to the needy"**

In this culture as in many others, helping the poor was also a way of acknowledging a special occasion. The idea was that no one should miss out on the benefits of the wonderful thing that God had done. Alternate translation: “Mordecai also told them that they should help the poor on those days”

See: Symbolic Action

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Symbolic Action (ID: `33402@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:22 (#13) (id: 262850)

וּ⁠מַתָּנ֖וֹת לָֽ⁠אֶבְיוֹנִֽים

**"and gifts to the needy"**

This is also something that Mordecai is telling the Jews to do in his letters. If it would be helpful in your language, you could say that here. Alternate translation: “Mordecai also told them that they should give gifts to the poor on those days”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:22
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:23 (#1) (id: 262851)

וְ⁠קִבֵּל֙ \& אֲשֶׁר־כָּתַ֥ב מָרְדֳּכַ֖י אֲלֵי⁠הֶֽם

**"And"** \- **"accepted"** \- **"what Mordecai had written to them"**

The implication seems to be that the Jews were glad to do what Mordecai had instructed, because they had already been doing it. You can add a word such as “readily” to indicate this.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:23
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 9:23 (#2) (id: 262852)

וְ⁠קִבֵּל֙

**"And"** \- **"accepted"**

Alternate translation: “agreed”

* **Associated Passages:** Esther 9:23

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 9:23 (#3) (id: 262853)

אֵ֥ת אֲשֶׁר־הֵחֵ֖לּוּ לַ⁠עֲשׂ֑וֹת

**"what they had begun to do"**

You can put this information first, to present the events in logical and chronological order. Alternate translation: “the Jews were already celebrating those days that way”

See: Order of Events

* **Associated Passages:** Esther 9:23
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`)

## Esther 9:24 (#1) (id: 262854)

כִּי֩

**"For"**

This introduces the reason for the events previously described. The story will now summarize everything that happened previously. The Jews were to celebrate because they were able to fight back when Haman plotted to destroy all of them. Alternate translation: “they would celebrate these days to remember”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 9:24
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Lot (ID: `keyterm:Lot`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Lot (ID: `realia:Lot`)

## Esther 9:24 (#2) (id: 262855)

כִּי֩

**"For"**

If it would be helpful in your language, you could say here what this reason is explaining. Alternate translation: “they would establish those days as a holiday in order to remember how”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:24
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Lot (ID: `keyterm:Lot`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Lot (ID: `realia:Lot`)

## Esther 9:24 (#3) (id: 262856)

צֹרֵר֙ כָּל־הַ⁠יְּהוּדִ֔ים

**"the adversary of all the Jews"**

This phrase gives clarifying information about Haman. Alternate translation: “the enemy of all the Jews”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 9:24
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Lot (ID: `keyterm:Lot`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Lot (ID: `realia:Lot`)

## Esther 9:24 (#4) (id: 262857)

חָשַׁ֥ב עַל־הַ⁠יְּהוּדִ֖ים לְ⁠אַבְּדָ֑⁠ם

**"had plotted concerning the Jews to annihilate them"**

Alternate translation: “had tried to carry out a plan that would destroy the Jews”

* **Associated Passages:** Esther 9:24

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Lot (ID: `keyterm:Lot`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Lot (ID: `realia:Lot`)

## Esther 9:24 (#5) (id: 262858)

וְ⁠הִפִּ֥יל פּוּר֙ ה֣וּא הַ⁠גּוֹרָ֔ל

**"And he had cast a Pur (which is “the lot"**

You can say explicitly why Haman did this. Alternate translation: “he threw Pur (that is, he threw lots) to find out what would be the best day”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:24
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Lot (ID: `keyterm:Lot`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Lot (ID: `realia:Lot`)

## Esther 9:24 (#6) (id: 262859)

פּוּר֙ ה֣וּא הַ⁠גּוֹרָ֔ל

**"a Pur (which is “the lot"**

As in 3:7, the storyteller is giving both the Persian and the Hebrew name because this is the story behind the Festival of Purim, which takes its name from “Pur.” So this is not repetition for emphasis. You can put in the Persian name and then the name for “lot” in your own language to show that the storyteller is doing this. Alternate translation: “a Pur (that is, a lot)”

See: How to Translate Names

* **Associated Passages:** Esther 9:24
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Lot (ID: `keyterm:Lot`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Lot (ID: `realia:Lot`)

## Esther 9:24 (#7) (id: 262860)

לְ⁠הֻמָּ֖⁠ם וּֽ⁠לְ⁠אַבְּדָֽ⁠ם

**"to vex them and to destroy them"**

The terms **vex** and **destroy** mean basically the same thing. They are used together to emphasize the degree to which Haman’s deadly plans were distressing to the Jews. If it would be helpful in your language, you could combine the terms and express the emphasis with a word like “completely.” Alternate translation: “and completely destroy them”

See: Hendiadys

* **Associated Passages:** Esther 9:24
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Hammedatha (ID: `person:Hammedatha`); Agagites (ID: `group:Agag`); Lot (ID: `keyterm:Lot`); Agag (ID: `person:Agag.2`); Jews (ID: `group:Jews`); Haman (ID: `person:Haman`); Lot (ID: `realia:Lot`)

## Esther 9:25 (#1) (id: 262861)

וּ⁠בְ⁠בֹאָ⁠הּ֮ לִ⁠פְנֵ֣י הַ⁠מֶּלֶךְ֒

**"But when she came to the face of the king"**

In this summary, many details are communicated implicitly. **She** means Esther. Also, this verse continues the account of what this celebration is commemorating. If it would be helpful in your language, you could make these things explicit. Alternate translation: “the celebration also commemorated how Esther dared to come into the king’s presence without being summoned, and she won his favor”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:25
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 9:25 (#2) (id: 262862)

לִ⁠פְנֵ֣י הַ⁠מֶּלֶךְ֒

**"to the face of the king"**

Here, **face** is a metonym meaning the presence of a person. This phrase means that Esther came into the king’s presence. Alternate translation: “into the king’s presence” or “before the king”

See: Metonymy

* **Associated Passages:** Esther 9:25
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 9:25 (#3) (id: 262863)

אָמַ֣ר עִם־הַ⁠סֵּ֔פֶר

**"he said with the letter"**

Once again, there is much information that is implicit here. If it would be helpful in your language, you could make it explicit. Alternate translation: “the king gave Mordecai the authority to send a letter throughout the empire saying that the Jews could defend themselves against their enemies”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:25
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 9:25 (#4) (id: 262864)

יָשׁ֞וּב מַחֲשַׁבְתּ֧⁠וֹ הָ⁠רָעָ֛ה אֲשֶׁר־חָשַׁ֥ב עַל־הַ⁠יְּהוּדִ֖ים עַל־רֹאשׁ֑⁠וֹ

**"Let his evil plot that he plotted concerning the Jews return on his head"**

**His**, **he**, and **him** in this sentence refer to Haman. **Return on his head** is an idiom that means that what a person was planning to do to someone else happened to that person instead. Use an idiom with that meaning in your language. Alternate translation: “the wicked plan that Haman developed against the Jews will be done to him”

See: Idiom

* **Associated Passages:** Esther 9:25
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 9:25 (#5) (id: 262865)

וְ⁠תָל֥וּ אֹת֛⁠וֹ וְ⁠אֶת־בָּנָ֖י⁠ו עַל־הָ⁠עֵֽץ

**"and let them hang him and his sons on the pole"**

The letter itself did not say this. The story is summarizing the events in compressed form. It is speaking of the letter as if it were a person who could take action and give commands like this, but it was the king who gave the command. Alternate translation: “the king also ordered his servants to hang \[or impale] Haman on a wooden pole \[or hang Haman on a gallows]. When the Jews in Susa killed his ten sons, the king had their bodies hanged \[or impaled] as well”

See: Personification

* **Associated Passages:** Esther 9:25
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 9:25 (#6) (id: 262866)

**"the pole"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Review the note there if that would be helpful.

See: Translate Unknowns

* **Associated Passages:** Esther 9:25
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Scroll (ID: `realia:Scroll`)

## Esther 9:26 (#1) (id: 262867)

עַל־כֵּ֡ן

**"Therefore"**

This introduces the reason why the Jews gave the name “Purim” to this celebration. Alternate translation: “for that reason” or “that is why”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#2) (id: 262868)

קָֽרְאוּ֩ לַ⁠יָּמִ֨ים הָ⁠אֵ֤לֶּה פוּרִים֙

**"they called these days “Purim"**

**They** means the Jews. Alternate translation: “the Jews called these days Purim, like the word Pur”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#3) (id: 262869)

פוּרִים֙

**"Purim"**

This is the name of the festival that commemorates the salvation of the Jewish people in ancient Persia from Haman’s plot to destroy and kill all the Jews in a single day.

See: How to Translate Names

* **Associated Passages:** Esther 9:26
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#4) (id: 262870)

עַל־שֵׁ֣ם הַ⁠פּ֔וּר

**"on account of the name of Pur"**

This is information that the original audience needed to understand how this celebration got its name. You can put this first in the verse because it explains what comes next. Alternate translation: “the Persian word for ‘lot’ is ‘Pur’”

See: Background Information

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#5) (id: 262871)

שֵׁ֣ם הַ⁠פּ֔וּר

**"the name of Pur"**

It can be stated clearly what “Pur” means. Alternate translation: “the word Pur, which means ‘lot’”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#6) (id: 262872)

עַל־כֵּ֕ן

**"Therefore"**

The story has just given the reason why the holiday is called Purim. Now it is going to give another reason. It will be explaining why the Jews added this holiday to their calendar, in addition to the festivals that were commanded in the Law of Moses. The next verse describes them adding the holiday. This verse gives the reasons why they did that. Alternate translation: “because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#7) (id: 262873)

עַל־כָּל־דִּבְרֵ֖י הָ⁠אִגֶּ֣רֶת הַ⁠זֹּ֑את

**"on account of all the words of this letter"**

This is a reference to the letter that Mordecai wrote, as described in verses 20–22\. Alternate translation: “because Mordecai wrote to them to tell them to observe this holiday”

* **Associated Passages:** Esther 9:26

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#8) (id: 262874)

וּ⁠מָֽה־רָא֣וּ עַל־כָּ֔כָה וּ⁠מָ֥ה הִגִּ֖יעַ אֲלֵי⁠הֶֽם

**"and what they had seen concerning this and what had come upon them"**

These two phrases mean similar things. The repetition is used to emphasize what a vivid experience it was to live through the events that this story describes. If it would be helpful in your language, you could combine the phrases and express the emphasis with a word like “amazing.” Alternate translation: “the amazing things that had happened to them”

See: Parallelism

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#9) (id: 262875)

וּ⁠מָֽה־רָא֣וּ עַל־כָּ֔כָה וּ⁠מָ֥ה הִגִּ֖יעַ אֲלֵי⁠הֶֽם

**"and what they had seen concerning this and what had come upon them"**

These phrases say generally “this” and “what,” but they are referring to the specific events the story has related. If it would be helpful in your language, you could describe these things specifically. Alternate translation: “because the Jews had been able to destroy the enemies who had wanted to destroy them”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#10) (id: 262876)

וּ⁠מָֽה־רָא֣וּ עַל־כָּ֔כָה וּ⁠מָ֥ה הִגִּ֖יעַ אֲלֵי⁠הֶֽם

**"and what they had seen concerning this and what had come upon them"**

You can put this first, before the information about Mordecai’s letter, because it happened first. You can say “then” when you tell about the letter.

See: Order of Events

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:26 (#11) (id: 262877)

וּ⁠מָ֥ה הִגִּ֖יעַ אֲלֵי⁠הֶֽם

**"and what had come upon them"**

This expression means “what had happened to them.”

See: Idiom

* **Associated Passages:** Esther 9:26
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `realia:Lot`)

## Esther 9:27 (#1) (id: 262878)

קִיְּמ֣וּ וְקִבְּל֣וּ הַ⁠יְּהוּדִים֩ ׀ עֲלֵי⁠הֶ֨ם ׀ וְ⁠עַל־זַרְעָ֜⁠ם וְ⁠עַ֨ל כָּל־הַ⁠נִּלְוִ֤ים עֲלֵי⁠הֶם֙ \& לִ⁠הְי֣וֹת עֹשִׂ֗ים אֵ֣ת שְׁנֵ֤י הַ⁠יָּמִים֙ הָ⁠אֵ֔לֶּה

**"the Jews set up and accepted for themselves and for their seed and for all those who unite themselves to them ("** \- **"to be making these two days"**

Alternate translation: “the Jews said that they would tell their descendants and those people who became Jews to be certain to celebrate this festival”

* **Associated Passages:** Esther 9:27

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:27 (#2) (id: 262879)

קִיְּמ֣וּ וְקִבְּל֣וּ הַ⁠יְּהוּדִים֩

**"the Jews set up and accepted"**

“Set up” and “accepted” mean basically the same thing. The repetition is used to emphasize that the Jews definitely agreed to do this. If it would be helpful in your language, you could combine these words. Alternate translation: “agreed to establish”

See: Doublet

* **Associated Passages:** Esther 9:27
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:27 (#3) (id: 262880)

וְ⁠עַל־זַרְעָ֜⁠ם

**"and for their seed"**

As in [6:13](https://ref.ly/Esth6:13), **seed** means “descendants.” Alternate translation: “and for their descendants”

See: Idiom

* **Associated Passages:** Esther 9:27
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:27 (#4) (id: 262881)

וְ⁠לֹ֣א יַעֲב֔וֹר

**"and it will not pass away"**

This expression means that the Jews would never stop celebrating the feast of Purim every year. you could put this last since it applies to the whole verse. Alternate translation: “always” or “forever”

See: Idiom

* **Associated Passages:** Esther 9:27
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:27 (#5) (id: 262882)

לִ⁠הְי֣וֹת עֹשִׂ֗ים אֵ֣ת שְׁנֵ֤י הַ⁠יָּמִים֙ הָ⁠אֵ֔לֶּה

**"to be making these two days"**

To present things in chronological order, you can put this before the reference to Jewish descendants and converts to Judaism. Alternate translation: “to establish those two days as holidays and to observe them”

See: Order of Events

* **Associated Passages:** Esther 9:27
* **Associated Articles:** Order of Events (ID: `33269@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:27 (#6) (id: 262883)

כִּ⁠כְתָבָ֖⁠ם

**"according to their writing"**

Alternate translation: “in the way that Mordecai had told them to do in the letter”

* **Associated Passages:** Esther 9:27

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:27 (#7) (id: 262884)

וְ⁠כִ⁠זְמַנָּ֑⁠ם

**"and according to their appointed time"**

This means the fourteenth and fifteenth days of the month of Adar, as specified in [9:21](https://ref.ly/Esth9:21). Alternate translation: “on those exact days of the month of Adar”

* **Associated Passages:** Esther 9:27

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:27 (#8) (id: 262885)

בְּ⁠כָל־שָׁנָ֖ה וְ⁠שָׁנָֽה

**"every year by year"**

This expression means “each and every year.” Alternate translation: “every single year”

See: Idiom

* **Associated Passages:** Esther 9:27
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#1) (id: 262886)

**"And these days are remembered and are made"**

This gives the result of the reasons described in verses 26 and 27\. Alternate translation: “therefore” or “that is why”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#2) (id: 262887)

וְ⁠הַ⁠יָּמִ֣ים הָ֠⁠אֵלֶּה נִזְכָּרִ֨ים וְ⁠נַעֲשִׂ֜ים

**"And these days are remembered and are made"**

**Remembered** and **made** mean basically the same thing here. The repetition is used to emphasize that the Jews have been faithful in doing this. If it would be helpful in your language, you could express the general meaning by saying something like “So the Jews have celebrated these days.” Alternate translation: “therefore they said that they would remember and celebrate on those two days”

See: Doublet

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#3) (id: 262888)

וְ⁠הַ⁠יָּמִ֣ים הָ֠⁠אֵלֶּה נִזְכָּרִ֨ים וְ⁠נַעֲשִׂ֜ים

**"And these days are remembered and are made"**

This expression uses two passive forms, but you could say the same thing using active forms. Alternate translation: “so the Jews have celebrated and observed these days”

See: Active or Passive

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#4) (id: 262889)

בְּ⁠כָל־דּ֣וֹר וָ⁠ד֗וֹר

**"in every generation by generation"**

This expression means “in each and every generation.” Alternate translation: “in every single generation”

See: Idiom

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#5) (id: 262890)

מִשְׁפָּחָה֙ וּ⁠מִשְׁפָּחָ֔ה

**"family by family"**

This expression means “every family.” Alternate translation: “every Jewish family”

See: Idiom

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#6) (id: 262891)

מְדִינָ֥ה וּ⁠מְדִינָ֖ה וְ⁠עִ֣יר וָ⁠עִ֑יר

**"province by province, and city by city"**

This could be a figure of speech that refers to something by speaking of two extreme parts of it in order to include everything in between those parts. Generally speaking, a province would be the largest division of the empire that would identify a person’s location, and a city would be the smallest. Particularly, since the Jews would continue to celebrate Purim after the Persian empire and its provinces no longer exist, you might choose to express the meaning of this figure of speech in a more general way. Alternate translation: “everywhere they have lived”

See: Merism

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#7) (id: 262892)

מְדִינָ֥ה וּ⁠מְדִינָ֖ה

**"province by province"**

This expression means “every province.” Alternate translation: “in every single province”

See: Idiom

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#8) (id: 262893)

וְ⁠עִ֣יר וָ⁠עִ֑יר

**"and city by city"**

This expression means “every city.” Alternate translation: “in every single city”

See: Idiom

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#9) (id: 262894)

וִ⁠ימֵ֞י הַ⁠פּוּרִ֣ים הָ⁠אֵ֗לֶּה לֹ֤א יַֽעַבְרוּ֙ מִ⁠תּ֣וֹךְ הַ⁠יְּהוּדִ֔ים וְ⁠זִכְרָ֖⁠ם לֹא־יָס֥וּף מִ⁠זַּרְעָֽ⁠ם

**"And these days of Purim will not pass away from the midst of the Jews, and their remembrance will not come to an end from their seed"**

As in verse 27, this expression means that the Jews will never stop celebrating the feast of Purim. You can say this positively. Alternate translation: “will always observe the Festival of Purim faithfully”

See: Double Negatives

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Double Negatives (ID: `33265@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#10) (id: 262895)

וִ⁠ימֵ֞י הַ⁠פּוּרִ֣ים הָ⁠אֵ֗לֶּה לֹ֤א יַֽעַבְרוּ֙ מִ⁠תּ֣וֹךְ הַ⁠יְּהוּדִ֔ים וְ⁠זִכְרָ֖⁠ם לֹא־יָס֥וּף מִ⁠זַּרְעָֽ⁠ם

**"And these days of Purim will not pass away from the midst of the Jews, and their remembrance will not come to an end from their seed"**

These two phrases mean similar things. The repetition is used to emphasize that the Jews will definitely not stop celebrating Purim each year. If it would be helpful in your language, you could combine the phrases and express the emphasis with a word like “definitely” or “certainly” or “always.” Alternate translation: “the Jews and their descendants will certainly always continue to celebrate this festival of Purim”

See: Parallelism

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#11) (id: 262896)

מִ⁠תּ֣וֹךְ הַ⁠יְּהוּדִ֔ים

**"from the midst of the Jews"**

Alternate translation: “within the Jewish community”

* **Associated Passages:** Esther 9:28

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#12) (id: 262897)

וְ⁠זִכְרָ֖⁠ם לֹא־יָס֥וּף

**"and their remembrance will not come to an end"**

Alternate translation: “will always observe”

* **Associated Passages:** Esther 9:28

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:28 (#13) (id: 262898)

**"from their seed"**

"As in verse 27, **seed** means ""descendants."" Alternate translation: “and for their descendants”"

See: Metaphor

* **Associated Passages:** Esther 9:28
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`)

## Esther 9:29 (#1) (id: 262899)

בַת־אֲבִיחַ֛יִל

**"the daughter of Abihail"**

This information reminds the reader who Esther was.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 9:29
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:29 (#2) (id: 262900)

אֲבִיחַ֛יִל

**"Abihail"**

This man was Esther’s father and Mordecai’s uncle. See how you translated his name in [2:15](https://ref.ly/Esth2:15).

See: How to Translate Names

* **Associated Passages:** Esther 9:29
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:29 (#3) (id: 262901)

וּ⁠מָרְדֳּכַ֥י

**"and Mordecai"**

Since the Hebrew verb wrote is feminine singular, it means that the letter was from Esther. The mention of **Mordecai** here probably means that Mordecai helped her to write the letter. Alternate translation: “with Mordecai”

* **Associated Passages:** Esther 9:29

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:29 (#4) (id: 262902)

הַ⁠יְּהוּדִ֖י

**"the Jew"**

This phrase gives information about Mordecai to remind the reader.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 9:29
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:29 (#5) (id: 262903)

וַ֠⁠תִּכְתֹּב \& אֶת־כָּל־תֹּ֑קֶף

**"And"** \- **"wrote with all power"**

Alternate translation: “using her royal authority” or “using the authority that she had as queen”

* **Associated Passages:** Esther 9:29

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:29 (#6) (id: 262904)

לְ⁠קַיֵּ֗ם אֵ֣ת אִגֶּ֧רֶת הַ⁠פּוּרִ֛ים הַ⁠זֹּ֖את הַ⁠שֵּׁנִֽית

**"to set up this second letter of Purim"**

**This second letter of Purim** could refer to: (1\) this letter that Esther is writing, which is the second letter that the Jews will receive about the festival of Purim. Alternate translation: “an additional letter about Purim with her authority” or (2\) the second letter that Mordecai wrote (See: verses 20–22\), creating the festival of Purim. Alternate translation: “to confirm what Mordecai had written about Purim in his second letter” In either case, this letter from Esther is to add her royal authority to what Mordecai had already written.

See: Numbers

* **Associated Passages:** Esther 9:29
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Abihail (ID: `person:Abihail.5`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:30 (#1) (id: 262905)

וַ⁠יִּשְׁלַ֨ח סְפָרִ֜ים

**"And he sent letters"**

While this says “**he**,” in context it refers to the letter that Esther wrote with Mordecai’s help. Alternate translation: “they sent copies of this second letter” or “Mordecai ordered messengers to take copies of the letter”

See: When Masculine Words Include Women

* **Associated Passages:** Esther 9:30
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Truth (ID: `keyterm:Truth`); Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:30 (#2) (id: 262906)

אֶל־כָּל־הַ⁠יְּהוּדִ֗ים אֶל־שֶׁ֨בַע וְ⁠עֶשְׂרִ֤ים וּ⁠מֵאָה֙ מְדִינָ֔ה מַלְכ֖וּת אֲחַשְׁוֵר֑וֹשׁ

**"to all the Jews, to 127 provinces, the kingdom of Ahasuerus"**

These three phrases all mean the same thing. The repetition emphasizes that Esther and Mordecai sent this second letter out comprehensively throughout the empire. Alternate translation: “to all the Jews throughout the entire empire of Ahasuerus”

See: Parallelism

* **Associated Passages:** Esther 9:30
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Truth (ID: `keyterm:Truth`); Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:30 (#3) (id: 262907)

שֶׁ֨בַע וְ⁠עֶשְׂרִ֤ים וּ⁠מֵאָה֙ מְדִינָ֔ה

**"127 provinces"**

The letter was not sent to the provinces as geographical territories, but to the Jews who lived in them. The Jews are being described by something associated with them, the places where they lived.

See: Metonymy

* **Associated Passages:** Esther 9:30
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Truth (ID: `keyterm:Truth`); Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:30 (#4) (id: 262908)

שֶׁ֨בַע וְ⁠עֶשְׂרִ֤ים וּ⁠מֵאָה֙ מְדִינָ֔ה

**"127 provinces"**

Alternate translation: “one hundred and twenty\-seven provinces”

See: Numbers

* **Associated Passages:** Esther 9:30
* **Associated Articles:** Numbers (ID: `33392@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Truth (ID: `keyterm:Truth`); Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:30 (#5) (id: 262909)

דִּבְרֵ֥י שָׁל֖וֹם וֶ⁠אֱמֶֽת

**"words of peace and truth"**

The abstract nouns **peace** and **truth** can be expressed in other ways. Alternate translation: “encouraging the Jews that they are now safe and can live peacefully”

See: Abstract Nouns

* **Associated Passages:** Esther 9:30
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Truth (ID: `keyterm:Truth`); Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:30 (#6) (id: 262910)

דִּבְרֵ֥י שָׁל֖וֹם וֶ⁠אֱמֶֽת

**"words of peace and truth"**

The Hebrew word translated as **truth** here also refers to things that are “sure” or “secure.” When “peace” and “truth” are paired together, they can refer to a peaceful, stable political environment, as in 2 Kings 20:19 and Jeremiah 33:6\. Alternate translation: “with a message that assured them of their peaceful and stable situation”

* **Associated Passages:** Esther 9:30

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Truth (ID: `keyterm:Truth`); Jews (ID: `group:Jews`); Ahasuerus (ID: `person:Ahasuerus`); Scroll (ID: `realia:Scroll`)

## Esther 9:31 (#1) (id: 262911)

לְ⁠קַיֵּ֡ם אֵת־יְמֵי֩ הַ⁠פֻּרִ֨ים הָ⁠אֵ֜לֶּה בִּ⁠זְמַנֵּי⁠הֶ֗ם כַּ⁠אֲשֶׁר֩ קִיַּ֨ם עֲלֵי⁠הֶ֜ם מָרְדֳּכַ֤י הַ⁠יְּהוּדִי֙ וְ⁠אֶסְתֵּ֣ר הַ⁠מַּלְכָּ֔ה

**"to set up these days of Purim at their appointed times according to what Mordecai the Jew and Esther the queen had set up for them"**

Alternate translation: “in this second letter, Mordecai the Jew and Queen Esther confirmed that Purim should be celebrated on the fourteenth and fifteenth days of the month of Adar”

* **Associated Passages:** Esther 9:31

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:31 (#2) (id: 262912)

הַ⁠יְּהוּדִי֙

**"the Jew"**

This phrase gives clarifying information about Mordecai.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 9:31
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:31 (#3) (id: 262913)

וְ⁠כַ⁠אֲשֶׁ֛ר קִיְּמ֥וּ עַל־נַפְשָׁ֖⁠ם וְ⁠עַל־זַרְעָ֑⁠ם דִּבְרֵ֥י הַ⁠צֹּמ֖וֹת וְ⁠זַעֲקָתָֽ⁠ם

**"and according to what they had set up concerning their lives and concerning their seed, the matters of the fasts and their outcry"**

This is referring to background information that the original audience would have known. They would have known what the fasting was about. It could be either: (1\) fasting as a part of Purim. We know that some Jews fasted on the 13th day of the month of Adar to commemorate the day that Haman intended to destroy them, or (2\) other times of fasting. Ever since the destruction of Jerusalem, the Jews had been fasting and mourning in the fifth month of the year to show their sorrow over what had happened. (The story refers to the Babylonian conquest of Jerusalem in [2:6](https://ref.ly/Esth2:6).) You could put this information in a footnote if it would help your readers to understand this.

* **Associated Passages:** Esther 9:31

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:31 (#4) (id: 262914)

וְ⁠עַל־זַרְעָ֑⁠ם

**"and concerning their seed"**

Here, **seed** is a metaphor meaning the offspring or descendants of the Jews. Alternate translation: “and for their descendants” or “and for their offspring”

See: Idiom

* **Associated Passages:** Esther 9:31
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Esther (ID: `person:Esther`)

## Esther 9:32 (#1) (id: 262915)

וְ⁠נִכְתָּ֖ב בַּ⁠סֵּֽפֶר

**"and it was written in the book"**

You can say this with an active form, and you can say who did the action. Alternate translation: “and the royal scribes made an official record of it”

See: Active or Passive

* **Associated Passages:** Esther 9:32
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Esther (ID: `person:Esther`); Scroll (ID: `realia:Scroll`)

## Esther 10:1 (#1) (id: 262916)

וַ⁠יָּשֶׂם֩ הַ⁠מֶּ֨לֶךְ אֲחַשְׁוֵר֧וֹשׁ

**"And the king Ahasuerus set"**

This introduces a new event in the story. Use a natural way to introduce a new event in your language.

See: Introduction of a New Event

* **Associated Passages:** Esther 10:1
* **Associated Articles:** Introduction of a New Event (ID: `33419@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:1 (#2) (id: 262917)

וַ⁠יָּשֶׂם֩ \& מַ֛ס

**"And"** \- **"set a tribute"**

Alternate translation: “Then … levied a tax”

* **Associated Passages:** Esther 10:1

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:1 (#3) (id: 262918)

עַל־הָ⁠אָ֖רֶץ וְ⁠אִיֵּ֥י הַ⁠יָּֽם

**"on the land and the islands of the sea"**

The purpose of this chapter is to describe the greatness of Mordecai. It does that by showing that he was second in command to a very powerful emperor. Referring to both the land and the sea is a way to include everything that lives in a very large area of the earth. If it would be helpful in your language, you could express the general meaning by saying something like “everyone throughout his empire.” Alternate translation: “on all the people in the empire … even the people who lived on the islands in the Mediterranean Sea” or “on everyone throughout the land and even the far\-away islands”

See: Merism

* **Associated Passages:** Esther 10:1
* **Associated Articles:** Merism (ID: `33291@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:1 (#4) (id: 262919)

וְ⁠אִיֵּ֥י הַ⁠יָּֽם

**"and the islands of the sea"**

The phrase **the islands of the sea** likely refers to the fact that the Persian kings had conquered territories reaching all the way to the Mediterranean Sea. If it would be helpful in your language, you could say that explicitly. Alternate translation: “which reached all the way to the Mediterranean Sea.”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** Esther 10:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:1 (#5) (id: 262920)

עַל־הָ⁠אָ֖רֶץ וְ⁠אִיֵּ֥י הַ⁠יָּֽם

**"on the land and the islands of the sea"**

These geographic features were not expected to pay the tax. The land and coastlands represent the people living there. The story is describing those people by reference to something associated with them, the places where they live.

See: Metonymy

* **Associated Passages:** Esther 10:1
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:2 (#1) (id: 262921)

וְ⁠כָל־מַעֲשֵׂ֤ה תָקְפּ⁠וֹ֙ וּ⁠גְב֣וּרָת֔⁠וֹ

**"And all the deeds of his power and his might"**

**Power** and **might** mean essentially the same thing. They are used together to emphasize how powerful King Ahasuerus was. If it is more natural in your language, you can use one word with that meaning, with another word that gives it emphasis. Alternate translation: “all that he achieved because of how very powerful he was” or “all the great things that he did because of his great power”

See: Doublet

* **Associated Passages:** Esther 10:2
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#2) (id: 262922)

תָקְפּ⁠וֹ֙ וּ⁠גְב֣וּרָת֔⁠וֹ

**"his power and his might"**

The abstract nouns **power** and **might** can be translated with an adjective. See the UST.

See: Abstract Nouns

* **Associated Passages:** Esther 10:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#3) (id: 262923)

וּ⁠פָרָשַׁת֙

**"with the full account of"**

Alternate translation: “They also wrote a full account”

* **Associated Passages:** Esther 10:2

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#4) (id: 262924)

גְּדֻלַּ֣ת מָרְדֳּכַ֔י

**"the greatness of Mordecai"**

Alternate translation: “of how important Mordecai was” or “of how the king had honored Mordecai for the great things he had done”

* **Associated Passages:** Esther 10:2

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#5) (id: 262925)

**"had made him great"**

See how you translated this phrase in 3:1 and 5:11\. Alternate translation: “had promoted him”

* **Associated Passages:** Esther 10:2

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#6) (id: 262926)

הֲ⁠לוֹא־הֵ֣ם כְּתוּבִ֗ים עַל־סֵ֨פֶר֙ דִּבְרֵ֣י הַ⁠יָּמִ֔ים לְ⁠מַלְכֵ֖י מָדַ֥י וּ⁠פָרָֽס

**"are they not written in the book of the events of days for the kings of Media and Persia"**

This is actually a statement. The question form is used to emphasize the certainty of the statement. If questions are not used this way in your language, then use a statement instead, as in the UST.

See: Rhetorical Question

* **Associated Passages:** Esther 10:2
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#7) (id: 262927)

הֲ⁠לוֹא־הֵ֣ם כְּתוּבִ֗ים

**"are they not written"**

You can say this with an active form, and you can say who did the action. Alternate translation: “The king’s scribes made a record … ”

See: Active or Passive

* **Associated Passages:** Esther 10:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#8) (id: 262928)

הֲ⁠לוֹא־הֵ֣ם כְּתוּבִ֗ים

**"are they not written"**

Because it comes first logically, you can put this first in the verse, if it would be helpful in your language.

* **Associated Passages:** Esther 10:2

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:2 (#9) (id: 262929)

סֵ֨פֶר֙ דִּבְרֵ֣י הַ⁠יָּמִ֔ים לְ⁠מַלְכֵ֖י מָדַ֥י וּ⁠פָרָֽס

**"the book of the events of days for the kings of Media and Persia"**

See how you translated this in [2:23](https://ref.ly/Esth2:23). Alternate translation: “the royal record books of Media and Persia”

See: Idiom

* **Associated Passages:** Esther 10:2
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Persia (ID: `place:Persia`); Media (ID: `place:Media`); Mordecai (ID: `person:Mordecai.2`)

## Esther 10:3 (#1) (id: 262930)

כִּ֣י

**"For"**

This word indicates that this verse will give the reason why the scribes made a record about Mordecai.

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** Esther 10:3
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#2) (id: 262931)

הַ⁠יְּהוּדִ֗י

**"the Jew"**

This phrase gives clarifying information about Mordecai.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** Esther 10:3
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#3) (id: 262932)

מִשְׁנֶה֙ לַ⁠מֶּ֣לֶךְ אֲחַשְׁוֵר֔וֹשׁ

**"was second to the king Ahasuerus"**

Alternate translation: “was the second most important person after King Ahasuerus himself”

* **Associated Passages:** Esther 10:3

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#4) (id: 262933)

וְ⁠גָדוֹל֙ לַ⁠יְּהוּדִ֔ים

**"and great among the Jews"**

Alternate translation: “and a great leader of the Jews”

* **Associated Passages:** Esther 10:3

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#5) (id: 262934)

אֶחָ֑י⁠ו

**"his brothers"**

Here, **brothers** is a figurative way of describing fellow members of the same people group. Alternate translation: “fellow Jews”

See: When Masculine Words Include Women

* **Associated Passages:** Esther 10:3
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#6) (id: 262935)

דֹּרֵ֥שׁ טוֹב֙ לְ⁠עַמּ֔⁠וֹ וְ⁠דֹבֵ֥ר שָׁל֖וֹם לְ⁠כָל־זַרְעֽ⁠וֹ

**"seeking good for his people and speaking peace to all its seed"**

These two phrases basically mean the same thing. The repetition is used to emphasize how hard Mordecai worked for the good of his people. If it would be helpful in your language, you could combine these phrases and say something like, “He worked hard so his people and their descendants would prosper.”

See: Parallelism

* **Associated Passages:** Esther 10:3
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#7) (id: 262936)

דֹּרֵ֥שׁ טוֹב֙ לְ⁠עַמּ֔⁠וֹ וְ⁠דֹבֵ֥ר שָׁל֖וֹם לְ⁠כָל־זַרְעֽ⁠וֹ

**"seeking good for his people and speaking peace to all its seed"**

The abstract nouns **good** and **peace** refer in this context to prosperity and security. You could translate these ideas with verbs, for example, you could say, “He worked hard to make sure that his people would prosper and their descendants would be secure.”

See: Abstract Nouns

* **Associated Passages:** Esther 10:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#8) (id: 262937)

דֹּרֵ֥שׁ טוֹב֙ לְ⁠עַמּ֔⁠וֹ

**"seeking good for his people"**

**Seeking** is a figurative way to describe actively trying to do something or work hard for something.

See: Metaphor

* **Associated Passages:** Esther 10:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#9) (id: 262938)

וְ⁠דֹבֵ֥ר שָׁל֖וֹם לְ⁠כָל־זַרְעֽ⁠וֹ

**"and speaking peace to all its seed"**

**Speaking peace** is a figurative way of describing actions that benefit the general welfare of others.

See: Idiom

* **Associated Passages:** Esther 10:3
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther 10:3 (#10) (id: 262939)

לְ⁠כָל־זַרְעֽ⁠וֹ

**"to all its seed"**

"As in verses 27 and 28, **seed** means ""descendants."" **seed** means “descendants.” Even if you combine the two parallel phrases, you can still convey the idea of “down through the generations” with a word such as “always.” Alternate translation: “to all of their descendants"""

See: Metaphor

* **Associated Passages:** Esther 10:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Peace (ID: `keyterm:IntactState`); Jews (ID: `group:Jews`); Mordecai (ID: `person:Mordecai.2`); Ahasuerus (ID: `person:Ahasuerus`)

## Esther - Chapter 1 Introduction (id: 718562)

Esther 1 Chapter Introduction
=============================

**Religious and Cultural Concepts in This Chapter**
---------------------------------------------------

### The king punishes Queen Vashti

When the wise men who were advisors to the king heard that the queen had refused to obey the king by showing her beauty to his guests, they feared that husbands would lose their authority. Therefore, they advised the king to take away from her the title of queen, and that she should no longer be allowed to come into the king's presence.

* **Associated Passages:** Esther 1:1


## Esther - Chapter 2 Introduction (id: 718563)

Esther 2 General Notes
======================

Special concepts in this chapter
--------------------------------

### Esther becomes queen

Esther was humble and took the advice of the royal officials about how to dress for her time with the king. The king chose Esther to be the new queen.

### Mordecai warns the king against a plot

Esther’s cousin, Mordecai, discovered that two men planned to kill the king. He told Esther, who then told the king. She also gave Mordecai credit for telling her.

* **Associated Passages:** Esther 2:1


## Esther - Chapter 3 Introduction (id: 718564)

Esther 3 General Notes
======================

Special concepts in this chapter
--------------------------------

### Haman plots against the Jews

Mordecai was told to prostrate himself before Haman. This would be considered to be worship. But for a Jew, it was wrong to worship someone other than Yahweh. Because of this, he refused to do it. This made Haman angry, so he decided to kill all the Jews in the Persian Empire. Haman was a descendant of Agag, a king of the Amalekite people. The Amalekites became the enemies of the Jews during the exodus (Exodus 17:8–16\). Moses told the people that they would need to destroy them (Deuteronomy 25:17–19\), and later God commanded Saul to destroy them (1 Samuel 15:2–3\). Samuel killed King Agag (1 Samuel 15:33\), but the Amalekites were never completely destroyed (1 Samuel 30:17\). Both Mordecai and Haman were probably very aware that their people were enemies.

* **Associated Passages:** Esther 3:1


## Esther - Chapter 4 Introduction (id: 718565)

Esther 4 General Notes
======================

Special concepts in this chapter
--------------------------------

### Mordecai warns Esther to act

Mordecai tells Esther she must beg the king for the Jews’ lives even if she risks her own death.

Other possible translation difficulties in this chapter
-------------------------------------------------------

### Implicit information

There is implicit information that translators may not understand. The sentence, “Who knows whether you have come to this royal position for such a time as this?” means “maybe God made you the queen so you could save the Jews.” (See: Assumed Knowledge and Implicit Information)

* **Associated Passages:** Esther 4:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Esther - Chapter 5 Introduction (id: 718566)

Esther 5 General Notes
======================

Structure and formatting
------------------------

This chapter begins a section about Haman’s fall (Chapters 5–7\).

Special concepts in this chapter
--------------------------------

### Esther’s respect

Esther approached the king with the utmost of respect. By doing this, her character became respected by the king. (See: Assumed Knowledge and Implicit Information)

* **Associated Passages:** Esther 5:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Esther - Chapter 6 Introduction (id: 718567)

Esther 6 General Notes
======================

Structure and formatting
------------------------

This chapter continues the story of Haman’s fall.

Special concepts in this chapter
--------------------------------

### Approaching the king

It was not possible for a person to easily approach the king. Normally, access to him was very limited. There are several events in this chapter which show the layers of protection surrounding the king.

* **Associated Passages:** Esther 6:1


## Esther - Chapter 7 Introduction (id: 718568)

Esther 7 General Notes
======================

Structure and formatting
------------------------

The story of Haman’s fall concludes in this chapter.

Other possible translation difficulties in this chapter
-------------------------------------------------------

### Covering Haman’s face

When “the servants covered Haman’s face,” they were showing that he had been condemned to be executed. (See: Assumed Knowledge and Implicit Information)

* **Associated Passages:** Esther 7:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## Esther - Chapter 8 Introduction (id: 718569)

Esther 8 General Notes
======================

Special concepts in this chapter
--------------------------------

### God’s protection

Yahweh is at work in this chapter preventing the possible destruction of the Jews. God used Esther and Mordecai to protect their people.

* **Associated Passages:** Esther 8:1


## Esther - Chapter 9 Introduction (id: 718570)

Esther 9 General Notes
======================

Special concepts in this chapter
--------------------------------

### Purim

The events of this chapter were so significant, the Jews celebrated these events every year after this. It is known as “Purim.”

Important stylistic devices in this chapter
-------------------------------------------

### Ironic Situation

The day that was supposed to bring great victory to the enemies of the Jews became a day of great victory for the Jews. This is a type of irony.

### Opening Summary

Verse 1 of chapter 9 provides a summary of everything that happens in this chapter. If using an opening summary is not normally used in your language, be sure to mark this as a summary either in the text or in a footnote. An opening summary like this is a characteristic device of Hebrew storytelling. So even though it describes the final outcome, we do not recommend that you move it to the end of the chapter unless it is very confusing to leave it here. To make it clear that verse 1 is only a summary and not the entire story, you could say something like this at the end of the verse: “This is what happened” or “Here are more details about what happened.”

* **Associated Passages:** Esther 9:1


## Esther - Chapter 10 Introduction (id: 718571)

Esther 10 General Notes
=======================

Special concepts in this chapter
--------------------------------

### Mordecai’s new position

Through the power of Yahweh, Mordecai was given a new position in the Persian Empire. Mordecai was now the second in command in the kingdom of Persia and he used his position to help other Jews.

* **Associated Passages:** Esther 10:1

