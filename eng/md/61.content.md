# Translation Notes (unfoldingWord)

**unfoldingWord® Translation Notes** © 2022 unfoldingWord. Released under CC BY\-SA 4\.0 license. *unfoldingWord® Translation Notes* has been adapted in the following languages: Tok Pisin, Arabic (عربي), French (Français), Hindi (हिंदी), Indonesian (Bahasa Indonesia), Portuguese (Português), Russian (Русский), Spanish (Español), Swahili (Kiswahili), and Simplified Chinese (简体中文) from *unfoldingWord® Translation Notes* © 2022 unfoldingWord. Released under CC BY\-SA 4\.0 license by Mission Mutual



--------------------------------


## 2 Peter 1:1 (#1) (id: 257538)

**Σίμων Πέτρος**

**"Simon Peter"**

In this culture, letter writers would give their own names first, and they would refer to themselves in the third person. If that would be confusing in your language, you could use the first person. If your language has a particular way of introducing the author of a letter, you could also use that. Alternate translation: “I, Simon Peter, am writing this letter” or “From Simon Peter”

See: First, Second or Third Person

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#2) (id: 257539)

**Σίμων Πέτρος**

**"Simon Peter"**

**Simon Peter** is the name of a man, a disciple of Jesus. See the information about him in Part 1 of the Introduction to 2 Peter.

See: How to Translate Names

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#3) (id: 257540)

**δοῦλος καὶ ἀπόστολος Ἰησοῦ Χριστοῦ**

**"a servant and an apostle of Jesus Christ"**

This phrase gives further information about Simon Peter. He describes himself as being both a **servant of Jesus Christ** and one given the position and authority of being Christ’s **apostle**.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#4) (id: 257541)

**τοῖς \& λαχοῦσιν**

**"the ones having received"**

In this culture, after giving their own names, letter writers would then say to whom they were writing, naming those people in the third person. If that would be confusing in your language, you could use the second person. Alternate translation: “to you who have received”

See: First, Second or Third Person

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** First, Second or Third Person (ID: `33256@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#5) (id: 257542)

**τοῖς ἰσότιμον ἡμῖν λαχοῦσιν πίστιν**

**"to the ones having received a faith equal in value with us"**

That these people have **received a faith** implies that God has given that faith to them. Alternate translation: “to those to whom God has given a faith equal in value with us”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#6) (id: 257543)

**τοῖς ἰσότιμον \& λαχοῦσιν πίστιν**

**"to the ones having received a faith equal in value"**

If it would be helpful in your language, you could express the idea behind the abstract noun **faith** with a verb, such as “trust” or “believe.” Alternate translation: “to those whom God has made to trust” or “to those whom God has made to believe”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#7) (id: 257544)

**ἡμῖν**

**"with us"**

Here, the word **us** refers to Peter and the other apostles, but not to those to whom he is writing. Alternate translation: “as we apostles have received”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#8) (id: 257545)

**ἐν δικαιοσύνῃ**

**"by the righteousness"**

The word **by** indicates the means through which they received the faith. Alternate translation: “by means of the righteousness”

* **Associated Passages:** 2 Peter 1:1

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:1 (#9) (id: 257546)

**δικαιοσύνῃ τοῦ Θεοῦ ἡμῶν καὶ Σωτῆρος**

**"the righteousness of our God and Savior"**

If it would be helpful in your language, you could express the abstract noun **righteousness** by translating the idea behind it with an adjective such as “righteous” or “right.” Alternate translation: “the righteous acts of our God and Savior” or “the right way of our God and Savior”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Peter (ID: `person:Peter`); LORD (ID: `deity:Lord`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:2 (#1) (id: 257547)

**χάρις ὑμῖν καὶ εἰρήνη πληθυνθείη**

**"May grace and peace be multiplied to you"**

In this culture, letter writers would offer a good wish for the recipient before introducing the main business of the letter. Use a form in your language that makes it clear that this is a greeting and blessing. Alternate translation: “May God increase his kind acts to you and make you more peaceful”

See: Blessings

* **Associated Passages:** 2 Peter 1:2
* **Associated Articles:** Blessings (ID: `33371@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:2 (#2) (id: 257548)

**χάρις \& καὶ εἰρήνη πληθυνθείη**

**"May grace and peace be multiplied"**

God is the one who will give **grace** and **peace** to believers. If it would be helpful in your language, you could make that information explicit. Alternate translation: “May God increase his grace and peace”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:2 (#3) (id: 257549)

**χάρις ὑμῖν καὶ εἰρήνη πληθυνθείη**

**"May grace and peace be multiplied to you"**

If it would be helpful in your language, you could express the abstract nouns **grace** and **peace** by stating the ideas behind them with equivalent expressions. Alternate translation: “May God multiply his kind acts to you and give you a more peaceful spirit”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:2 (#4) (id: 257550)

**χάρις \& καὶ εἰρήνη πληθυνθείη**

**"May grace and peace be multiplied"**

Peter speaks of **grace and peace** as if they were objects that could increase in size or number. If this is confusing in your language, you can use a different metaphor that means that these things will increase, or use plain language. Alternate translation: “May God increase his grace and peace”

See: Metaphor

* **Associated Passages:** 2 Peter 1:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:2 (#5) (id: 257551)

**ὑμῖν**

**"to you"**

Here the pronoun **you** is plural, since Peter is writing to believers in Jesus as a group. Generally, throughout the letter the pronouns “you” and “your” are plural for this same reason.

See: Forms of You

* **Associated Passages:** 2 Peter 1:2
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:2 (#6) (id: 257552)

**ἐν ἐπιγνώσει τοῦ Θεοῦ, καὶ Ἰησοῦ τοῦ Κυρίου ἡμῶν**

**"in the knowledge of God and of Jesus our Lord"**

If you would not use an abstract noun here, you could translate **knowledge** using a verbal phrase. Alternate translation: “because you know God and Jesus our Lord”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:2 (#7) (id: 257553)

**ἐν ἐπιγνώσει τοῦ Θεοῦ, καὶ Ἰησοῦ τοῦ Κυρίου ἡμῶν**

**"in the knowledge of God and of Jesus our Lord"**

This could mean: (1\) “because of knowing God and Jesus our Lord” or (2\) “through knowing God and Jesus our Lord.”

* **Associated Passages:** 2 Peter 1:2

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:2 (#8) (id: 257554)

**τοῦ Κυρίου ἡμῶν**

**"our Lord"**

Here, **our Lord** means “the person who is lord over us” or “the person who rules over us.”

See: Possession

* **Associated Passages:** 2 Peter 1:2
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Make Peace (ID: `keyterm:Peace`); LORD (ID: `deity:Lord`); Favor (ID: `keyterm:Favor`)

## 2 Peter 1:3 (#1) (id: 257555)

**ὡς \& ἡμῖν τῆς θείας δυνάμεως αὐτοῦ \& δεδωρημένης**

**"As his divine power has given to us"**

Here, **As** indicates that this verse provides the reason for the expected result, which is Peter’s command in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7). Alternate translation: “Since his divine power has given to us”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#2) (id: 257556)

**ἡμῖν**

**"to us"**

Here, **us** refers to Peter and all believers.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#3) (id: 257557)

**τῆς θείας δυνάμεως αὐτοῦ**

**"his divine power"**

The pronoun **his** could refer to: (1\) God. Alternate translation: “God’s divine power” (2\) Jesus. Alternate translation: “Jesus, by his power as God”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#4) (id: 257558)

**τῆς θείας δυνάμεως αὐτοῦ**

**"his divine power"**

If it would be helpful in your language, you could express the idea behind the abstract noun **power** with a verbal phrase. Alternate translation: “God, because he can do anything,”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#5) (id: 257559)

**τῆς θείας δυνάμεως αὐτοῦ \& δεδωρημένης**

**"his divine power has given"**

Peter is speaking of God’s **divine power** as if it were a living thing that could give something to people. God is the one who is giving, and he uses his **divine power** to do so. Alternate translation: “God has used his divine power to give”

See: Personification

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#6) (id: 257560)

**πρὸς ζωὴν καὶ εὐσέβειαν**

**"for life and godliness"**

Here, the word **for** indicates the purpose for which God has given all these things to believers. Alternate translation: “for the purpose of life and godliness”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#7) (id: 257561)

**πρὸς ζωὴν καὶ εὐσέβειαν**

**"for life and godliness"**

Here, **godliness** describes the word **life**. Alternate translation: “for a godly life”

See: Hendiadys

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#8) (id: 257562)

**εὐσέβειαν**

**"godliness"**

If it would be helpful in your language, you could express the idea behind the abstract noun **godliness** with a verbal phrase. Alternate translation: “acting honorably toward God”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#9) (id: 257563)

**διὰ τῆς ἐπιγνώσεως**

**"through the knowledge"**

The word **through** here indicates the means by which God has given us all things for life and godliness. Alternate translation: “by means of the knowledge”

* **Associated Passages:** 2 Peter 1:3

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#10) (id: 257564)

**διὰ τῆς ἐπιγνώσεως τοῦ καλέσαντος ἡμᾶς**

**"through the knowledge of the one having called us"**

If it would be helpful in your language, you could express the abstract noun **knowledge** by translating it with a verbal phrase. Alternate translation: “through our knowing the one who called us”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#11) (id: 257565)

**τοῦ καλέσαντος ἡμᾶς**

**"of the one having called us"**

This phrase could refer to: (1\) God. Alternate translation: “of God, who called us” (2\) Jesus. Alternate translation: “of Jesus, who called us”

* **Associated Passages:** 2 Peter 1:3

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#12) (id: 257566)

**ἡμᾶς**

**"us"**

Here, **us** refers to Peter and his audience, fellow believers.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#13) (id: 257567)

**διὰ δόξης καὶ ἀρετῆς**

**"through his glory and excellence"**

Here, **through** indicates the means by which God called us. Alternate translation: “by means of his glory and excellence”

* **Associated Passages:** 2 Peter 1:3

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:3 (#14) (id: 257568)

**διὰ δόξης καὶ ἀρετῆς**

**"through his glory and excellence"**

If it would be helpful in your language, you could express the abstract nouns **glory** and **excellence** by stating the ideas behind them with a verbal phrase. Alternate translation: “because he is so great and so good”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Divine (ID: `keyterm:Divine`); Glory (ID: `keyterm:Glory`); Ability (ID: `keyterm:Ability`); Life (ID: `keyterm:Life`)

## 2 Peter 1:4 (#1) (id: 257569)

**δι’ ὧν**

**"through which"**

Here, **through** indicates the means by which God gave his promises. Alternate translation: “by means of which”

* **Associated Passages:** 2 Peter 1:4

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#2) (id: 257570)

**δι’ ὧν**

**"through which"**

Here, **which** refers back to words from the previous verse. It could refer to: (1\) “his glory and excellence.” Alternate translation: “through his glory and excellence” (2\) “all the things for life and godliness.” Alternate translation: “by giving us all of these things”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#3) (id: 257571)

**ἡμῖν**

**"to us"**

Here, **us** refers to Peter and his audience, fellow believers.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#4) (id: 257572)

**δεδώρηται**

**"he has given"**

The pronoun **he** could refer to: (1\) God. Alternate translation: “God has given” (2\) Jesus. Alternate translation: “Jesus has given”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#5) (id: 257573)

**τὰ τίμια καὶ μέγιστα ἡμῖν ἐπαγγέλματα δεδώρηται,**

**"he has given to us the precious and great promises"**

If it would be helpful in your language, you could express the idea behind the abstract noun **promises** with a verbal phrase. Alternate translation: “he has promised to us precious and great things”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#6) (id: 257574)

**ἵνα διὰ τούτων γένησθε θείας κοινωνοὶ φύσεως**

**"so that through them you might be sharers of the divine nature"**

This is a purpose clause. Peter is stating the purpose for which God has given to us precious and great promises. In your translation, follow the conventions of your language for purpose clauses. Alternate translation (without a comma preceding): “in order that through them you might be sharers of the divine nature”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#7) (id: 257575)

**διὰ τούτων**

**"through them"**

The word **through** here indicates the means by which you might be sharers of the divine nature. Alternate translation: “by means of them”

* **Associated Passages:** 2 Peter 1:4

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#8) (id: 257576)

**διὰ τούτων**

**"through them"**

Here the pronoun **them** refers back to the precious and great promises of the previous phrase. Alternate translation: “through these promises”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#9) (id: 257577)

**θείας \& φύσεως**

**"of the divine nature"**

The abstract noun **nature** refers to the inherent features of something or what it is like. Alternate translation: “of what God is like”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#10) (id: 257578)

**ἀποφυγόντες τῆς \& φθορᾶς**

**"having escaped the corruption"**

Peter speaks of people not suffering from the **corruption** that wicked desires cause as if they had **escaped** from that corruption. If it would be helpful in your language, you could express this phrase plainly. Alternate translation: “no longer being corrupted”

See: Metaphor

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#11) (id: 257579)

**ἐν τῷ κόσμῳ**

**"in the world"**

Here, **the world** could mean: (1\) the place where we all live, where we are surrounded by sinful people and temptations to sin. Alternate translation: “that is all around us” (2\) the system of values that people share who do not honor God. Alternate translation: “of the world’s ungodly value system”

See: Metonymy

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#12) (id: 257580)

**ἐν ἐπιθυμίᾳ**

**"by lust"**

Here, **by** indicates the means by which the world became corrupt. It does not indicate the means by which Peter’s addressees escaped corruption. Alternate translation: “by means of lust”

* **Associated Passages:** 2 Peter 1:4

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:4 (#13) (id: 257581)

**φθορᾶς**

**"corruption"**

If it would be helpful in your language, you could express the idea behind the abstract noun **corruption** with a verbal phrase. Alternate translation: “things that corrupt you”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Ability (ID: `keyterm:Ability`); Divine (ID: `keyterm:Divine`)

## 2 Peter 1:5 (#1) (id: 257582)

**καὶ αὐτὸ τοῦτο δὲ**

**"and now {for} this same {reason"**

The phrase **{for} this same {reason}** refers to what Peter has just said in the previous verses. If it would be helpful in your language, you could state this phrase explicitly. Alternate translation: “and now because of these things that God has done”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:5
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:5 (#2) (id: 257583)

**σπουδὴν πᾶσαν παρεισενέγκαντες**

**"applying all diligence"**

The phrase **applying all diligence** indicates the means by which one is to do the action of supplying that follows. Alternate translation: “by means of applying all diligence”

* **Associated Passages:** 2 Peter 1:5

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:5 (#3) (id: 257584)

**σπουδὴν πᾶσαν παρεισενέγκαντες**

**"applying all diligence"**

Here, **applying all diligence** is an idiom that means doing one’s best or making the best effort. Alternate translation: “making every effort”

See: Idiom

* **Associated Passages:** 2 Peter 1:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:5 (#4) (id: 257585)

**ἐπιχορηγήσατε ἐν τῇ πίστει ὑμῶν**

**"supply in your faith"**

If it would be helpful in your language, you could express the idea behind the abstract noun **faith** with a verb such as “trust” or “believe.” Alternate translation: “as you trust in Jesus, add”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:5
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:5 (#5) (id: 257586)

**ὑμῶν**

**"your"**

The pronoun **your** is plural here, since Peter is writing to believers in Jesus as a group. Generally, throughout the letter the pronouns “you” and “your” are plural for this same reason.

See: Forms of You

* **Associated Passages:** 2 Peter 1:5
* **Associated Articles:** Forms of You (ID: `33317@UWTranslationManual`)

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:5 (#6) (id: 257587)

**τὴν ἀρετήν \& τῇ ἀρετῇ**

**"goodness;"** \- **"goodness"**

If it would be helpful in your language, you could express the idea behind the abstract noun **goodness** with an adjective phrase in both occurrences in this verse. Alternate translation: “doing what is good … doing what is good”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:5
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:5 (#7) (id: 257588)

**ἐν δὲ τῇ ἀρετῇ τὴν γνῶσιν**

**"and in goodness, knowledge"**

Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from earlier in the sentence. Alternate translation: “and supply in your goodness, knowledge”

See: Ellipsis

* **Associated Passages:** 2 Peter 1:5
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:5 (#8) (id: 257589)

**τὴν γνῶσιν**

**"knowledge"**

If it would be helpful in your language, you could express the idea behind the abstract noun **knowledge** using a verbal phrase. Alternate translation: “knowing more about God”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:5
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Eagerness (ID: `keyterm:Eagerness`); Believe (ID: `keyterm:Believe`)

## 2 Peter 1:6 (#1) (id: 257590)

**ἐν δὲ τῇ γνώσει τὴν ἐνκράτειαν**

**"and in knowledge, self\-control"**

Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from earlier in the sentence. Alternate translation: “and supply in knowledge, self\-control”

See: Ellipsis

* **Associated Passages:** 2 Peter 1:6
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)


## 2 Peter 1:6 (#2) (id: 257591)

**τῇ γνώσει**

**"knowledge"**

If it would be helpful in your language, you could express the abstract noun **knowledge** by translating it using a verbal phrase. Alternate translation: “knowing more about God”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## 2 Peter 1:6 (#3) (id: 257592)

**τὴν ἐνκράτειαν \& τῇ ἐνκρατείᾳ**

**"self\-control;"** \- **"self\-control"**

If it would be helpful in your language, you could express the idea behind the abstract noun **self\-control** with a verbal phrase in both occurrences in this verse. Alternate translation: “controlling yourself … controlling yourself”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## 2 Peter 1:6 (#4) (id: 257593)

**ἐν δὲ τῇ ἐνκρατείᾳ τὴν ὑπομονήν**

**"and in self\-control, endurance"**

Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from earlier in the sentence. Alternate translation: “and supply in self\-control, endurance”

See: Ellipsis

* **Associated Passages:** 2 Peter 1:6
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)


## 2 Peter 1:6 (#5) (id: 257594)

**τὴν ὑπομονήν \& τῇ ὑπομονῇ**

**"endurance;"** \- **"endurance"**

If it would be helpful in your language, you could express the idea behind the abstract noun **endurance** with a verbal phrase in both occurrences in this verse. Alternate translation: “enduring hardship … enduring hardship”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## 2 Peter 1:6 (#6) (id: 257595)

**ἐν δὲ τῇ ὑπομονῇ τὴν εὐσέβειαν,**

**"and in endurance, godliness"**

Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from earlier in the sentence. Alternate translation: “and supply in endurance, godliness”

See: Ellipsis

* **Associated Passages:** 2 Peter 1:6
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)


## 2 Peter 1:6 (#7) (id: 257596)

**τὴν εὐσέβειαν**

**"godliness"**

If it would be helpful in your language, you could express the idea behind the abstract noun **godliness** with a verbal phrase. Alternate translation: “acting honorably toward God”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## 2 Peter 1:7 (#1) (id: 257597)

**ἐν δὲ τῇ εὐσεβείᾳ τὴν φιλαδελφίαν**

**"and in godliness, brotherly affection"**

Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from earlier in the sentence. Alternate translation: “and supply in godliness, brotherly affection”

See: Ellipsis

* **Associated Passages:** 2 Peter 1:7
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Love for One's Fellow Believer (ID: `keyterm:LoveForOnesFellowBeliever`); Love (ID: `keyterm:Love`)

## 2 Peter 1:7 (#2) (id: 257598)

**τὴν φιλαδελφίαν \& τῇ φιλαδελφίᾳ**

**"brotherly affection;"** \- **"brotherly affection"**

If it would be helpful in your language, you could express the idea behind the abstract noun **brotherly affection** with a verbal phrase. Alternate translation: “being concerned for your brothers and sisters … being concerned for your brothers and sisters”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Love for One's Fellow Believer (ID: `keyterm:LoveForOnesFellowBeliever`); Love (ID: `keyterm:Love`)

## 2 Peter 1:7 (#3) (id: 257599)

**ἐν δὲ τῇ φιλαδελφίᾳ τὴν ἀγάπην**

**"and in brotherly affection, love"**

Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from earlier in the sentence. Alternate translation: “and supply in brotherly affection, love”

See: Ellipsis

* **Associated Passages:** 2 Peter 1:7
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Love for One's Fellow Believer (ID: `keyterm:LoveForOnesFellowBeliever`); Love (ID: `keyterm:Love`)

## 2 Peter 1:7 (#4) (id: 257600)

**τὴν ἀγάπην**

**"love"**

If it would be helpful in your language, you could express the idea behind the abstract noun **love** with a verbal phrase. Alternate translation: “loving others”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Love for One's Fellow Believer (ID: `keyterm:LoveForOnesFellowBeliever`); Love (ID: `keyterm:Love`)

## 2 Peter 1:8 (#1) (id: 257601)

**ταῦτα γὰρ ὑμῖν ὑπάρχοντα καὶ πλεονάζοντα**

**"For these things existing and increasing in you"**

The word **For** here indicates that Peter is giving a reason why his audience should obey the command given in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7). Alternate translation: “Because these things existing and increasing in you”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:8
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Useless (ID: `keyterm:Idle`)

## 2 Peter 1:8 (#2) (id: 257602)

**ταῦτα γὰρ ὑμῖν ὑπάρχοντα καὶ πλεονάζοντα, οὐκ ἀργοὺς οὐδὲ ἀκάρπους καθίστησιν**

**"For these things existing and increasing in you cause you to be neither barren nor unfruitful"**

Peter is describing a conditional situation. If it would be helpful in your language, you could express it that way. Alternate translation: “For if these things are existing and increasing in you, then they will cause you to be neither barren nor unfruitful”

See: Connect — Hypothetical Conditions

* **Associated Passages:** 2 Peter 1:8
* **Associated Articles:** Connect — Hypothetical Conditions (ID: `33327@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Useless (ID: `keyterm:Idle`)

## 2 Peter 1:8 (#3) (id: 257603)

**ταῦτα**

**"these things"**

Here, **these things** refers back to the faith, goodness, knowledge, self\-control, endurance, godliness, brotherly affection, and love which Peter mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7).

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Useless (ID: `keyterm:Idle`)

## 2 Peter 1:8 (#4) (id: 257604)

**οὐκ ἀργοὺς οὐδὲ ἀκάρπους καθίστησιν**

**"cause you to be neither barren nor unfruitful"**

Peter speaks of a person who does not possess these qualities as if he were a field that will not produce a crop. If that is confusing in your language, you could use a different metaphor with that meaning or state the meaning plainly. Alternate translation: “cause you to be neither unproductive nor useless”

See: Metaphor

* **Associated Passages:** 2 Peter 1:8
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Useless (ID: `keyterm:Idle`)

## 2 Peter 1:8 (#5) (id: 257605)

**οὐκ ἀργοὺς οὐδὲ ἀκάρπους καθίστησιν**

**"cause you to be neither barren nor unfruitful"**

If your language does not use this passive form, you can state this in positive terms. Alternate translation: “cause you to produce and bear fruit”

See: Double Negatives

* **Associated Passages:** 2 Peter 1:8
* **Associated Articles:** Double Negatives (ID: `33265@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Useless (ID: `keyterm:Idle`)

## 2 Peter 1:8 (#6) (id: 257606)

**οὐκ ἀργοὺς οὐδὲ ἀκάρπους**

**"neither barren nor unfruitful"**

The words **barren** and **unfruitful** mean basically the same thing. In combination with the negatives **neither** and **nor**, they are used together to emphasize that this person will not be unproductive but will experience great benefits from knowing Jesus. If it is confusing in your language to use two words together that mean the same thing, you could just use one word with that meaning. Alternate translation: “not unproductive”

See: Doublet

* **Associated Passages:** 2 Peter 1:8
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Useless (ID: `keyterm:Idle`)

## 2 Peter 1:8 (#7) (id: 257607)

**εἰς τὴν τοῦ Κυρίου ἡμῶν, Ἰησοῦ Χριστοῦ, ἐπίγνωσιν**

**"in the knowledge of our Lord Jesus Christ"**

If it would be helpful in your language, you could express the idea behind the abstract noun **knowledge** using a verbal phrase. Alternate translation: “in your knowing our Lord Jesus Christ”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Useless (ID: `keyterm:Idle`)

## 2 Peter 1:9 (#1) (id: 257608)

**γὰρ**

**"For"**

**For** indicates that Peter is giving another reason why his audience should obey the command given in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7). Peter gave a positive reason in [1:8](https://ref.ly/2Pet1:8) and gives a negative reason here. Alternate translation: “Because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:9 (#2) (id: 257609)

**ᾧ \& μὴ πάρεστιν ταῦτα, τυφλός ἐστιν**

**"he in whom these things are not present is blind"**

Here, **he** does not refer to a specific person, but to any person who does not have these things. Alternate translation: “anyone who does not have these things”

See: Generic Noun Phrases

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Generic Noun Phrases (ID: `33277@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:9 (#3) (id: 257610)

**ταῦτα**

**"these things"**

The phrase **these things** refers back to the faith, goodness, knowledge, self\-control, endurance, godliness, brotherly affection, and love, which Peter mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7).

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:9 (#4) (id: 257611)

**τυφλός ἐστιν μυωπάζων**

**"he"** \- **"is blind, nearsighted"**

In this metaphor, Peter speaks of a person who does not possess these qualities as if the person were **blind** or **nearsighted**. He means this in a spiritual sense, that this person cannot see what is spiritually important. If that is confusing in your language, you could use a different metaphor with that meaning or state the meaning plainly. Alternate translation: “he … is like a blind or shortsighted person who cannot see their importance”

See: Metaphor

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:9 (#5) (id: 257612)

**τυφλός ἐστιν μυωπάζων**

**"he"** \- **"is blind, nearsighted"**

Although the words **blind** and **nearsighted** have similar meanings, **blind** is more extreme than **nearsighted,** and a person cannot be both at the same time. If it is confusing in your language to describe someone using both of these words in this way, you could use a word like “or” between them or show how they might work together. Alternate translation: “he … is blind or nearsighted” or “he … is blindly nearsighted” or “he … is so nearsighted that he is blind to what is spiritually important”

See: Hendiadys

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:9 (#6) (id: 257613)

**λήθην λαβὼν τοῦ καθαρισμοῦ**

**"having received forgetfulness of the cleansing"**

If it would be helpful in your language, you could express the idea behind the abstract noun **forgetfulness** in this phrase with a verb. Alternate translation: “having forgotten the cleansing”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:9 (#7) (id: 257614)

**τοῦ καθαρισμοῦ τῶν πάλαι αὐτοῦ ἁμαρτιῶν**

**"of the cleansing of his past sins"**

If it would be helpful in your language, you could express the idea behind the abstract noun **cleansing** with a verbal phrase. Alternate translation: “that God has cleansed him from his old sins”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:9 (#8) (id: 257615)

**τοῦ καθαρισμοῦ τῶν πάλαι αὐτοῦ ἁμαρτιῶν.**

**"of the cleansing of his past sins"**

Peter is speaking of forgiving sin as if sin were something that made people dirty and thus required **cleansing** from God. Alternate translation: “of the forgiving of his past sins”

See: Metaphor

* **Associated Passages:** 2 Peter 1:9
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** To Cleanse (ID: `keyterm:Cleanse.2`)

## 2 Peter 1:10 (#1) (id: 257616)

**διὸ**

**"Therefore"**

Peter uses **Therefore** to introduce a description of what his readers should do as a result of what he has just said. He is referring specifically to the two reasons for obedience given in [1:8–9](https://ref.ly/2Pet1:8-2Pet1:9). Alternate translation: “Because of these reasons”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:10
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:10 (#2) (id: 257617)

**ἀδελφοί**

**"brothers"**

Peter is using the term **brothers** as a way to directly address his fellow believers in Jesus. See the UST.

See: Metaphor

* **Associated Passages:** 2 Peter 1:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:10 (#3) (id: 257618)

**ἀδελφοί**

**"brothers"**

Peter is using the term **brothers** in a generic sense that includes both men and women. Be sure that this is clear in your translation so that your readers do not get the impression that Peter is addressing only men. If you use a word such as “believers” to translate the metaphor **brothers**, you may need to use both the masculine and the feminine forms of that word in your language. If you retain the metaphor, you could state “my brothers and sisters.”

See: When Masculine Words Include Women

* **Associated Passages:** 2 Peter 1:10
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:10 (#4) (id: 257619)

**βεβαίαν ὑμῶν τὴν κλῆσιν καὶ ἐκλογὴν ποιεῖσθαι**

**"to make your calling and election sure"**

The words **calling** and **election** share similar meanings and both refer to God choosing believers to belong to him. Peter uses them together to emphasize this idea. If it would be helpful in your language, you could use just one word and provide the emphasis in another way. Alternate translation: “make sure that God has really chosen you to belong to him”

See: Doublet

* **Associated Passages:** 2 Peter 1:10
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:10 (#5) (id: 257620)

**ταῦτα γὰρ ποιοῦντες**

**"for doing these things"**

Here, **these things** refers back to the faith, goodness, knowledge, self\-control, endurance, godliness, brotherly affection, and love which Peter mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7).

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:10
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:10 (#6) (id: 257621)

**ταῦτα γὰρ ποιοῦντες οὐ μὴ πταίσητέ ποτε**

**"for doing these things, you will certainly not ever stumble"**

Peter is describing a conditional situation. If it would be helpful in your language, you could express it that way. Alternate translation: “For if you do these things, then you will certainly not ever stumble”

See: Connect — Hypothetical Conditions

* **Associated Passages:** 2 Peter 1:10
* **Associated Articles:** Connect — Hypothetical Conditions (ID: `33327@UWTranslationManual`)

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:10 (#7) (id: 257622)

**οὐ μὴ πταίσητέ ποτε**

**"you will certainly not ever stumble"**

The combination of the words **certainly not ever** here expresses strong emphatic negation. Alternate translation: “you will most certainly never stumble”

* **Associated Passages:** 2 Peter 1:10

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:10 (#8) (id: 257623)

**οὐ μὴ πταίσητέ ποτε**

**"you will certainly not ever stumble"**

Here, **stumble** could mean: (1\) abandoning faith in Christ. Alternate translation: “you will certainly not abandon faith in Christ” (2\) committing sin. Alternate translation: “you will certainly not practice sinful behavior”

See: Metaphor

* **Associated Passages:** 2 Peter 1:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Stumble (ID: `keyterm:Stumble`); Chosen (ID: `keyterm:Chosen`)

## 2 Peter 1:11 (#1) (id: 257624)

**"For"**

**For** indicates that Peter is giving a reason why his readers should want to obey the commands given in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7) and [1:10](https://ref.ly/2Pet1:10).

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:11
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`); Kingdom (ID: `keyterm:Kingdom`)

## 2 Peter 1:11 (#2) (id: 257625)

**οὕτως**

**"in this way"**

Here, **in this way** refers back to the way of living that includes the faith, goodness, knowledge, self\-control, endurance, godliness, brotherly affection, and love, which Peter mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7).

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:11
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`); Kingdom (ID: `keyterm:Kingdom`)

## 2 Peter 1:11 (#3) (id: 257626)

**πλουσίως ἐπιχορηγηθήσεται ὑμῖν ἡ εἴσοδος εἰς τὴν αἰώνιον βασιλείαν**

**"will be richly provided to you the entrance into the eternal kingdom"**

If your language does not use this passive form, you can express this with an active form. Alternate translation: “God will richly provide to you the entrance into the eternal kingdom”

See: Active or Passive

* **Associated Passages:** 2 Peter 1:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`); Kingdom (ID: `keyterm:Kingdom`)

## 2 Peter 1:11 (#4) (id: 257627)

**εἰς τὴν αἰώνιον βασιλείαν τοῦ Κυρίου ἡμῶν καὶ Σωτῆρος, Ἰησοῦ Χριστοῦ**

**"into the eternal kingdom of our Lord and Savior Jesus Christ"**

If it would be helpful in your language, you could express the idea behind the abstract noun **kingdom** with a verbal phrase, such as “where our Lord and Savior Jesus Christ rules.” Alternate translation: “into the eternal place where our Lord and Savior Jesus Christ rules”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:11
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`); Kingdom (ID: `keyterm:Kingdom`)

## 2 Peter 1:12 (#1) (id: 257628)

**διὸ**

**"Therefore"**

Connecting Statement:\\n\\nPeter uses **Therefore** to introduce the purpose of his letter. In order to encourage his readers to do everything he has said in [1:5–10](https://ref.ly/2Pet1:5-2Pet1:10), and especially because of the promise in [1:11](https://ref.ly/2Pet1:11), he wants to keep reminding them about these things. Use a natural form in your language to show that this introduces a result or purpose for saying what came before. Alternate translation: “Because these things are very important”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:12
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`)

## 2 Peter 1:12 (#2) (id: 257629)

**τούτων**

**"these things"**

Here, **these things** refers back to what Peter has stated in the previous verses, specifically to faith, goodness, knowledge, self\-control, endurance, godliness, brotherly affection, and love, which Peter mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7).

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:12
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`)

## 2 Peter 1:12 (#3) (id: 257630)

**ἐστηριγμένους ἐν**

**"have been established in"**

If your language does not use this passive form, you can express this with an active form. Alternate translation: “you learned well”

See: Active or Passive

* **Associated Passages:** 2 Peter 1:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`)

## 2 Peter 1:12 (#4) (id: 257631)

**ἐστηριγμένους ἐν τῇ παρούσῃ ἀληθείᾳ**

**"have been established in the present truth"**

Here, **established** is used to refer to one being firmly committed to something. Alternate translation: “you strongly believe the truth that you now have”

See: Metaphor

* **Associated Passages:** 2 Peter 1:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`)

## 2 Peter 1:12 (#5) (id: 257632)

**ἐν τῇ παρούσῃ ἀληθείᾳ**

**"in the present truth"**

Here, **in** has the meaning of “in reference to” or “with regard to.” Alternate translation: “in reference to the present truth”

* **Associated Passages:** 2 Peter 1:12

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`)

## 2 Peter 1:12 (#6) (id: 257633)

**ἐν τῇ παρούσῃ ἀληθείᾳ**

**"in the present truth"**

Here, **present** is used as if **truth** were an object that could be there with Peter’s audience. Here it does not refer to present time. Alternate translation: “in the truth that you have” or “in the truth that is with you”

See: Metaphor

* **Associated Passages:** 2 Peter 1:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`)

## 2 Peter 1:12 (#7) (id: 257634)

**ἐν τῇ παρούσῃ ἀληθείᾳ**

**"in the present truth"**

If it would be helpful in your language, you could express the idea behind the abstract noun **truth** with an adjective, such as “true.” Alternate translation: “in these true teachings”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:12
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`)

## 2 Peter 1:13 (#1) (id: 257635)

**δὲ**

**"But"**

**But** could mean: (1\) Peter is contrasting what he just said in the previous verse with what he is about to say. His audience already knows the truth, but he wants to remind them again. Alternate translation, as in the UST: “Nevertheless.” (2\) Peter is connecting this statement with what he said at the beginning of the previous verse. Peter is always ready to remind them of the truth, and he thinks it is correct to do so. Alternate translation: “And”

See: Connecting Words and Phrases

* **Associated Passages:** 2 Peter 1:13
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:13 (#2) (id: 257636)

**ἐφ’ ὅσον εἰμὶ ἐν τούτῳ τῷ σκηνώματι**

**"as long as I am in this tent"**

Peter speaks of his body as if it were a **tent** that he is wearing and will take off. Being in his body represents being alive. If it would be helpful in your language, you could indicate this directly. Alternate translation: “as long as I am in this body” or “as long as I live”

See: Metaphor

* **Associated Passages:** 2 Peter 1:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:13 (#3) (id: 257637)

**διεγείρειν ὑμᾶς ἐν ὑπομνήσει**

**"to arouse you in remembrance"**

Peter uses **arouse** to refer to causing his readers to think about these things. If it would be helpful in your language, you could express this usage by translating this plainly. Alternate translation: “to remind you of these things so that you will think about them”

See: Metaphor

* **Associated Passages:** 2 Peter 1:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:13 (#4) (id: 257638)

**διεγείρειν ὑμᾶς ἐν ὑπομνήσει**

**"to arouse you in remembrance"**

If it would be helpful in your language, you could express the idea behind the abstract noun **remembrance** in this phrase with the verb “remind.” Alternate translation: “to remind you”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:13
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:14 (#1) (id: 257639)

**εἰδὼς**

**"knowing"**

In this clause Peter is giving a reason why he will always remind his audience of doctrinal truths in this letter, specifically faith, goodness, knowledge, self\-control, endurance, godliness, brotherly affection, and love, which Peter mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7). Alternate translation: “This is because I know”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:14
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:14 (#2) (id: 257640)

**ταχινή ἐστιν ἡ ἀπόθεσις τοῦ σκηνώματός μου**

**"the putting off of my tent is imminent"**

Peter speaks of his body as if it were a **tent** that he is wearing and will take off. Being in his body represents being alive, and taking it off represents dying. Alternate translation: “I will soon take off this body”

See: Metaphor

* **Associated Passages:** 2 Peter 1:14
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:14 (#3) (id: 257641)

**ταχινή ἐστιν ἡ ἀπόθεσις τοῦ σκηνώματός μου**

**"the putting off of my tent is imminent"**

**Putting off** his **tent** is a nice way to refer to dying. If it would be helpful in your language, you could indicate this directly. Alternate translation: “I will die soon”

See: Euphemism

* **Associated Passages:** 2 Peter 1:14
* **Associated Articles:** Euphemism (ID: `33268@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:14 (#4) (id: 257642)

**"just as also our Lord Jesus Christ revealed this to me"**

Suggested footnote: “Peter may be referring here to what Jesus told him, as recorded in John 21:18–19\.”

* **Associated Passages:** 2 Peter 1:14

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Body (ID: `keyterm:Body.3`); Tent (ID: `realia:Tent`)

## 2 Peter 1:15 (#1) (id: 257643)

**δὲ καὶ**

**"Likewise"**

**Likewise** here could mean that: (1\) this statement is in addition to what Peter just said in the previous verse. Alternate translation: “Moreover” (2\) this statement is contrasting what he just said in the previous verse with what he is about to say. Alternate translation: “But”

See: Connecting Words and Phrases

* **Associated Passages:** 2 Peter 1:15
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)


## 2 Peter 1:15 (#2) (id: 257644)

**ἑκάστοτε, ἔχειν ὑμᾶς \& τὴν τούτων μνήμην ποιεῖσθαι**

**"to cause you to always have the remembrance of these things"**

If it would be helpful in your language, you could express the idea behind the abstract noun **remembrance** in this phrase with the verb “remind.” Alternate translation: “to remind you of these things”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:15
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## 2 Peter 1:15 (#3) (id: 257645)

**τούτων**

**"of these things"**

Here, **these things** refers to what Peter has said in the previous verses, specifically the faith, goodness, knowledge, self\-control, endurance, godliness, brotherly affection, and love which Peter mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7).

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:15
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)


## 2 Peter 1:15 (#4) (id: 257646)

**μετὰ τὴν ἐμὴν ἔξοδον**

**"after my departure"**

Peter uses the word **departure** as a nice way to speak of his death. If it would be helpful in your language, you could use a more common euphemism in your language, or state it directly. Alternate translation: “after my passing” or “after I die”

See: Euphemism

* **Associated Passages:** 2 Peter 1:15
* **Associated Articles:** Euphemism (ID: `33268@UWTranslationManual`)


## 2 Peter 1:16 (#1) (id: 257647)

**γὰρ**

**"For"**

Connecting Statement:\\n\\n**For** indicates that in [1:16–21](https://ref.ly/2Pet1:16-2Pet1:21) Peter explains to the believers why they should remember “these things,” which were mentioned in [1:5–7](https://ref.ly/2Pet1:5-2Pet1:7). Alternate translation: “This is because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:16
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:16 (#2) (id: 257648)

**οὐ \& ἐγνωρίσαμεν**

**"we did not make known"**

Here, **we** refers to Peter and the other apostles. It does not refer to his readers. Alternate translation: “we apostles did not follow”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:16
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:16 (#3) (id: 257649)

**τὴν \& δύναμιν καὶ παρουσίαν**

**"the power and coming"**

The words **power** and **coming** work together to refer to the same thing; they may be translated as a single phrase. Alternate translation: “the powerful coming”

See: Hendiadys

* **Associated Passages:** 2 Peter 1:16
* **Associated Articles:** Hendiadys (ID: `33280@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:16 (#4) (id: 257650)

**"the power and coming of our Lord Jesus Christ"**

In this clause Peter refers to the second coming of the Lord Jesus to earth. This future event was foreshadowed by the powerful appearance of Jesus known as the “transfiguration,” which is described in Matthew 17:1–8, Mark 9:1–8, and Luke 9:28–36\. Peter was an eyewitness of that event.

* **Associated Passages:** 2 Peter 1:16

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:16 (#5) (id: 257651)

**τοῦ Κυρίου ἡμῶν Ἰησοῦ Χριστοῦ**

**"of our Lord Jesus Christ"**

Here, **our** refers to all believers.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:16
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:16 (#6) (id: 257652)

**ἐπόπται γενηθέντες**

**"by} having become eyewitnesses"**

This phrase indicates the means by which the apostles had made known to others the second coming of Jesus. The apostles based their teaching about the return of Jesus partly on their eyewitness experience. Alternate translation: “by means of becoming eyewitnesses”

* **Associated Passages:** 2 Peter 1:16

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:16 (#7) (id: 257653)

**τῆς ἐκείνου μεγαλειότητος**

**"of the majesty of that one"**

The pronoun **that one** refers to Jesus. Alternate translation: “of the majesty of Jesus”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:16
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:16 (#8) (id: 257654)

**τῆς ἐκείνου μεγαλειότητος**

**"of the majesty of that one"**

If it would be helpful in your language, you could express the idea behind the abstract noun **majesty** with an adjective such as “majestic.” Alternate translation: “of his majestic nature”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Ability (ID: `keyterm:Ability`)

## 2 Peter 1:17 (#1) (id: 257655)

**γὰρ**

**"For"**

Here, **For** indicates that what follows in [1:17–18](https://ref.ly/2Pet1:17-2Pet1:18) is the reason that Peter can say in the previous verse that he was an eyewitness to the majesty of Jesus. Use a connector that indicates that this is a reason or explanation. Alternate translation: “I say this because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#2) (id: 257656)

**παρὰ Θεοῦ Πατρὸς**

**"from God the Father"**

**Father** is an important title for God.

See: Translating Son and Father

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Translating Son and Father (ID: `33349@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#3) (id: 257657)

**λαβὼν \& παρὰ Θεοῦ Πατρὸς τιμὴν καὶ δόξαν**

**"he has received honor and glory from God the Father"**

If it would be helpful in your language, you could express the abstract nouns **honor** and **glory** by translating them with equivalent expressions that use verbs. Alternate translation: “and God the Father honored and glorified him”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#4) (id: 257658)

**φωνῆς ἐνεχθείσης αὐτῷ τοιᾶσδε ὑπὸ τῆς Μεγαλοπρεποῦς Δόξης**

**"such a voice having been brought to him by the Majestic Glory"**

If your language does not use this passive form, you can state this in active form. Alternate translation: “when he heard that voice come to him from the Majestic Glory” or “when he heard the voice of the Majestic Glory speak to him” or “when the Majestic Glory spoke to him”

See: Active or Passive

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#5) (id: 257659)

**φωνῆς ἐνεχθείσης αὐτῷ τοιᾶσδε ὑπὸ τῆς Μεγαλοπρεποῦς Δόξης**

**"such a voice having been brought to him by the Majestic Glory"**

Consider natural ways of introducing direct quotations in your language. Alternate translation: “such a voice having been brought to him by the Majestic Glory, and this is what God said”

See: Quotations and Quote Margins

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Quotations and Quote Margins (ID: `33426@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#6) (id: 257660)

**ἐνεχθείσης αὐτῷ**

**"having been brought to him"**

The pronoun **him** refers to Jesus. If it would be helpful in your language, you could state explicitly the idea behind this connection. Alternate translation: “having been brought to Jesus”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#7) (id: 257661)

**τῆς Μεγαλοπρεποῦς Δόξης**

**"the Majestic Glory"**

Peter refers to God in terms of his **Glory**. The glory of God is closely associated with God himself and here substitutes for his name. Alternate translation: “God, the Supreme Glory”

See: Metonymy

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#8) (id: 257662)

**ὁ Υἱός μου**

**"my Son"**

**Son** is an important title for Jesus, the Son of God.

See: Translating Son and Father

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Translating Son and Father (ID: `33349@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:17 (#9) (id: 257663)

**μου \& μου \& ἐγὼ**

**"my"** \- **"my"** \- **"myself"**

The pronouns **my** and **myself** refer to God the Father, who is the one speaking in the quotation.

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:17
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Glory (ID: `keyterm:Glory`)

## 2 Peter 1:18 (#1) (id: 257664)

**ταύτην τὴν φωνὴν ἡμεῖς ἠκούσαμεν ἐξ οὐρανοῦ, ἐνεχθεῖσαν**

**"we ourselves heard this voice, having been brought from heaven"**

With the words **we ourselves**, Peter is referring to himself and to the disciples James and John, who also heard the voice of God. Alternate translation: “we, James, John, and I, heard this voice that came from heaven”

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:18
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Holy (ID: `keyterm:Holy`)

## 2 Peter 1:18 (#2) (id: 257665)

**ἐξ οὐρανοῦ, ἐνεχθεῖσαν**

**"having been brought from heaven"**

If your language does not use this passive form, you can express this with an active form. Alternate translation: “having come from heaven” or “that came from heaven”

See: Active or Passive

* **Associated Passages:** 2 Peter 1:18
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Holy (ID: `keyterm:Holy`)

## 2 Peter 1:18 (#3) (id: 257666)

**σὺν αὐτῷ, ὄντες**

**"being with him"**

This is a temporal phrase that can begin with “when.” Alternate translation: “when we were with him”

* **Associated Passages:** 2 Peter 1:18

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Holy (ID: `keyterm:Holy`)

## 2 Peter 1:18 (#4) (id: 257667)

**σὺν αὐτῷ**

**"with him"**

Here, **him** refers to Jesus, not God the Father. If it would be helpful in your language, you could state this reference explicitly. Alternate translation: “being with Jesus”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 1:18
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Holy (ID: `keyterm:Holy`)

## 2 Peter 1:18 (#5) (id: 257668)

**τῷ ἁγίῳ ὄρει**

**"the holy mountain"**

The **mountain** Peter refers to is the mountain on which Jesus was powerfully transformed in the event known as the “transfiguration.” If it would be helpful to your readers, you could express this information explicitly. Alternate translation: “on the holy mountain on which Jesus was powerfully transformed”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:18
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); Holy (ID: `keyterm:Holy`)

## 2 Peter 1:19 (#1) (id: 257669)

**ἔχομεν βεβαιότερον τὸν προφητικὸν λόγον**

**"we have the very certain prophetic word"**

The word translated as **very certain** could refer to: (1\) something that is extremely reliable. In this case, Peter is saying in [1:18–19](https://ref.ly/2Pet1:18-2Pet1:19) that we have two reliable witnesses to Jesus’ glory: God’s own voice speaking on the mountain of transfiguration and the extremely reliable prophetic scriptures. Alternate translation: “We also have the most reliable prophetic word” (2\) something that is confirmed by something else. In this case, Peter is saying that God’s voice on the mountain confirms, or makes even more trustworthy, the prophetic scripture that we already trusted completely. Alternate translation: “we have the prophetic word confirmed”

* **Associated Passages:** 2 Peter 1:19

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#2) (id: 257670)

**ἔχομεν**

**"we have"**

Here, **we** refers to all believers, including Peter and his readers.

See: Exclusive and Inclusive ‘We’

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Exclusive and Inclusive ‘We’ (ID: `33271@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#3) (id: 257671)

**βεβαιότερον τὸν προφητικὸν λόγον**

**"the very certain prophetic word"**

Peter uses the phrase **prophetic word** to refer to the entire Old Testament. It does not refer only to those Old Testament books called “the Prophets,” nor only to the predictive prophecies within the Old Testament. Alternate translation: “the Scriptures, which the prophets spoke”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#4) (id: 257672)

**ᾧ καλῶς ποιεῖτε προσέχοντες**

**"to which you do well to pay attention"**

Here the relative pronoun **which** refers to the prophetic word mentioned in the previous phrase. Peter instructs the believers to pay close attention to the prophetic message, which is the Old Testament.

See: Pronouns

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Pronouns (ID: `33304@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#5) (id: 257673)

**ᾧ καλῶς ποιεῖτε προσέχοντες**

**"to which you do well to pay attention"**

Peter uses the statement **you do well** to tell his audience that they should pay attention to the Old Testament Scriptures. If it would be helpful in your language, you could express this phrase by translating this phrase as a suggestion or command. Alternate translation: “to which you should pay attention”

See: Statements — Other Uses

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Statements — Other Uses (ID: `33263@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#6) (id: 257674)

**ὡς λύχνῳ φαίνοντι ἐν αὐχμηρῷ τόπῳ**

**"as to a lamp shining in a dark place"**

Peter compares the prophetic word to a **lamp** that gives light in the darkness. Just as a lamp gives light in order for someone to see in **a dark place**, so the prophetic word gives believers guidance for how to live properly in this sin\-filled world. If it would be helpful in your language, you could state this simile plainly. Alternate translation: “a guide for you to know how to live in this world”

See: Simile

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Simile (ID: `33314@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#7) (id: 257675)

**ἕως οὗ ἡμέρα διαυγάσῃ**

**"until the day might dawn"**

Peter speaks of Christ’s second coming by calling it the new **day** that comes in the morning. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “until the day Christ returns”

See: Metaphor

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#8) (id: 257676)

**φωσφόρος ἀνατείλῃ ἐν ταῖς καρδίαις ὑμῶν**

**"the morning star might rise in your hearts"**

Peter speaks of Christ as the **morning star**, which is a star that indicates daybreak and the end of the night. Christ will **rise** by bringing light into the hearts of believers, ending all doubt and bringing full understanding of who he is. If it would be helpful in your language, you could state this metaphor plainly or change the metaphor to a simile. Alternate translation: “Christ brings full understanding to you like the morning star shines its light into the world”

See: Metaphor

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#9) (id: 257677)

**ἐν ταῖς καρδίαις ὑμῶν**

**"in your hearts"**

Here, **hearts** is a metonym for people’s minds. Alternate translation: “in your minds” or “to help you understand”

See: Metonymy

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:19 (#10) (id: 257678)

**φωσφόρος**

**"the morning star"**

The **morning star** refers to the planet Venus, which is sometimes visible in the sky just before the sun rises, thus indicating that daybreak is near. If it would be helpful to your readers, you could make this information more explicit. Alternate translation: “the star that appears just before the sun rises”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 1:19
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Speak as a Prophet (ID: `keyterm:Speak`); Heart (ID: `keyterm:Heart`)

## 2 Peter 1:20 (#1) (id: 257679)

**τοῦτο πρῶτον γινώσκοντες**

**"knowing this first"**

Peter uses **first** here to refer to the degree of importance. It does not to refer to order in time. Alternate translation: “Most importantly, you must understand”

* **Associated Passages:** 2 Peter 1:20

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 1:20 (#2) (id: 257680)

**τοῦτο πρῶτον γινώσκοντες**

**"knowing this first"**

Peter is using a statement to give an instruction. If it would be helpful in your language, you could indicate this by translating it as a command. If you do so, it might be helpful to start a new sentence here. Alternate translation: “Above all, know this”

See: Statements — Other Uses

* **Associated Passages:** 2 Peter 1:20
* **Associated Articles:** Statements — Other Uses (ID: `33263@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 1:20 (#3) (id: 257681)

**πᾶσα προφητεία Γραφῆς ἰδίας ἐπιλύσεως οὐ γίνεται**

**"every prophecy of scripture does not come from one’s own interpretation"**

Here, **one’s own interpretation** could mean: (1\) that the Old Testament prophets did not base any of their prophecies on their own interpretations of what God had said, but only prophesied what God revealed to them. If it would be more natural in your language, you could change the order of information. Alternate translation: “no prophet interpreted his prophecy according to his own interpretation” (2\) that no individual person can interpret scripture on his or her own, but only with the help of the Holy Spirit and the larger community of believers. Alternate translation: “no one is able to explain any prophecy in the Bible through his own ability”

See: Information Structure

* **Associated Passages:** 2 Peter 1:20
* **Associated Articles:** Information Structure (ID: `33286@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 1:20 (#4) (id: 257682)

**ἰδίας ἐπιλύσεως**

**"from one’s own interpretation"**

The word **interpretation** is an abstract noun that represents an action. If it would be helpful in your language, you could state this with a verb. Alternate translation: “no prophet interpreted his prophecy according to what he himself thought”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:20
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 1:21 (#1) (id: 257683)

**γὰρ**

**"For"**

**For** indicates that what follows is the reason that the statement of the previous verse is true. It could mean: (1\) the prophets could not prophesy according to their own interpretations, because true prophecy could only come from the Holy Spirit. (2\) no one can interpret prophecy without help from the Holy Spirit, because the prophecy came from the Holy Spirit. Alternate translation: “The reason for this is that”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 1:21
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`)

## 2 Peter 1:21 (#2) (id: 257684)

**οὐ \& θελήματι ἀνθρώπου ἠνέχθη προφητεία ποτέ**

**"no prophecy was ever brought by the will of man"**

If your language does not use this passive form, you can express this with an active form, and you can state who did the action. Alternate translation: “no prophet ever prophesied by the will of man” or “the will of man never produced any prophecy”

See: Active or Passive

* **Associated Passages:** 2 Peter 1:21
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`)

## 2 Peter 1:21 (#3) (id: 257685)

**οὐ \& θελήματι ἀνθρώπου ἠνέχθη προφητεία ποτέ**

**"no prophecy was ever brought by the will of man"**

If it would be helpful in your language, you could express the idea behind the abstract noun **will** with a verbal phrase, such as “what a man desires.” Alternate translation: “no prophecy was ever made according to what a man desires”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 1:21
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`)

## 2 Peter 1:21 (#4) (id: 257686)

**θελήματι ἀνθρώπου**

**"by the will of man"**

Peter is using the term **man** in a generic sense that includes both men and women. Alternate translation: “by human desire”

See: When Masculine Words Include Women

* **Associated Passages:** 2 Peter 1:21
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`)

## 2 Peter 1:21 (#5) (id: 257687)

**ὑπὸ Πνεύματος Ἁγίου φερόμενοι, ἐλάλησαν ἀπὸ Θεοῦ ἄνθρωποι**

**"men being carried along by the Holy Spirit spoke from God"**

Peter speaks of the **Holy Spirit** helping the prophets to write what God wanted them to write as if the Holy Spirit **carried** them from one place to another. Alternate translation: “men spoke from God by the Holy Spirit directing them”

See: Metaphor

* **Associated Passages:** 2 Peter 1:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`)

## 2 Peter 1:21 (#6) (id: 257688)

**ἐλάλησαν ἀπὸ Θεοῦ**

**"spoke from God"**

In this phrase, Peter is leaving out a word that it would need in many languages in order to be complete. If this word is required in your language, it can be supplied from earlier in the verse. Alternate translation: “spoke prophecy from God”

See: Ellipsis

* **Associated Passages:** 2 Peter 1:21
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** LORD (ID: `deity:Lord`); Holy Spirit (ID: `deity:HolySpirit`)

## 2 Peter 2:1 (#1) (id: 257689)

**δὲ**

**"Now"**

The term translated **Now** could refer to: (1\) a new topic expressed by **Now** in the ULT. (2\) a contrast between the false prophets in this clause and the true Old Testament prophets mentioned in the previous verse. Alternate translation: “But”

See: Connect — Contrast Relationship

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#2) (id: 257690)

**ἐν τῷ λαῷ**

**"among the people"**

Here, **the people** refers specifically to the Israelites. If it would be helpful to your readers, you could indicate this explicitly. Alternate translation: “the people of Israel” or “the Israelites”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#3) (id: 257691)

**αἱρέσεις ἀπωλείας**

**"heresies of destruction"**

Here, **heresies** refers to opinions that are contrary to the teaching of Christ and the apostles. Alternate translation: “opinions of destruction”

See: Translate Unknowns

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#4) (id: 257692)

**αἱρέσεις ἀπωλείας,**

**"heresies of destruction"**

If it would be helpful in your language, you could express the idea behind the abstract noun **destruction** with an equivalent expression. Alternate translation: “destructive heresies” or “heresies that destroy”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#5) (id: 257693)

**αἱρέσεις ἀπωλείας**

**"heresies of destruction"**

Peter is using the possessive form to describe an opinion that is characterized by **destruction**. If it would be helpful in your language, you could use the adjective “destructive” instead of the noun “destruction.” Alternate translation: “destructive heresies”

See: Possession

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#6) (id: 257694)

**αἱρέσεις ἀπωλείας**

**"heresies of destruction"**

Here, **destruction** could refer to: (1\) the eternal damnation of those who teach or accept these **heresies**. Alternate translation: “heresies that result in their eternal damnation” (2\) the destruction of the faith of those who teach or accept these **heresies**. Alternate translation: “heresies that destroy their faith in the Messiah”

* **Associated Passages:** 2 Peter 2:1

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#7) (id: 257695)

**τὸν ἀγοράσαντα αὐτοὺς Δεσπότην**

**"the master who bought them"**

Here, **master** refers to Jesus. If it would be helpful in your language, you could express this explicitly. Alternate translation: “master Jesus who bought them”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#8) (id: 257696)

**τὸν ἀγοράσαντα αὐτοὺς Δεσπότην**

**"the master who bought them"**

Peter uses the phrase **the master who bought** to speak of Jesus as the owner of the people he has saved from damnation by paying the penalty for their sins with his death. If it would be helpful to your readers, you could express this plainly. Alternate translation: “Jesus who saved them”

See: Metaphor

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#9) (id: 257697)

**ἐπάγοντες ἑαυτοῖς ταχινὴν ἀπώλειαν**

**"bringing swift destruction on themselves"**

Here, **bringing** indicates that this clause is the result of the deeds of the false teachers described in the previous clauses. If it would be helpful to your readers, you could make a new sentence here and state this connection plainly. Alternate translation: “As a result, they are bringing swift destruction on themselves.”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#10) (id: 257698)

**ταχινὴν ἀπώλειαν**

**"swift destruction"**

Here,**swift** could mean: (1\) their destruction will come soon. Alternate translation: “destruction that will happen soon” or “imminent destruction” (2\) their destruction will be sudden or quick. Alternate translation: “quick destruction”

* **Associated Passages:** 2 Peter 2:1

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:1 (#11) (id: 257699)

**ἐπάγοντες ἑαυτοῖς ταχινὴν ἀπώλειαν**

**"bringing swift destruction on themselves"**

If it would be helpful in your language, you could express the idea behind the abstract noun **destruction** with a verb, such as “destroy.” Alternate translation: “they are causing themselves to be destroyed soon”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Redeem (ID: `keyterm:Redeem`)

## 2 Peter 2:2 (#1) (id: 257700)

**πολλοὶ**

**"many"**

If it would be helpful to your readers, you could indicate explicitly, as the UST does, that **many** refers to people.

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#2) (id: 257701)

**ἐξακολουθήσουσιν**

**"will follow"**

Here Peter uses the word **follow** to refer to someone imitating the actions of someone else, like someone who walks behind another person in the same direction. If it would be helpful in your language, you could state this metaphor literally. Alternate translation: “will imitate their licentious acts”

See: Metaphor

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#3) (id: 257702)

**αὐτῶν ταῖς ἀσελγείαις**

**"their licentious acts"**

Here the pronoun **their** refers to the false teachers introduced in the previous verse. If it would be helpful in your language, you could express this explicitly. Alternate translation: “the licentious acts of these false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#4) (id: 257703)

**ταῖς ἀσελγείαις**

**"licentious acts"**

Here, **licentious acts** refers to immoral sexual actions that demonstrate a lack of self\-control. Alternate translation: “uncontrolled sensual acts”

* **Associated Passages:** 2 Peter 2:2

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#5) (id: 257704)

**δι’ οὓς**

**"because of whom"**

Here, **whom** refers to the false teachers. It does not refer to the licentious acts in the previous clause. If it would be helpful to your readers, you could indicate explicitly, as the UST does, that this refers to the false teachers. Alternate translation: “through these false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#6) (id: 257705)

**ἡ ὁδὸς τῆς ἀληθείας**

**"the way of truth"**

Peter uses the phrase **the way of truth** here to refer to the Christian faith or how a Christian person lives his or her life. If it would be helpful in your language, you could state this metaphor literally. Alternate translation: “the true Christian manner of living” or “the true Christian faith”

See: Metaphor

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#7) (id: 257706)

**ἡ ὁδὸς τῆς ἀληθείας**

**"the way of truth"**

Peter is using the possessive form to describe **the way** that is characterized by **truth**. If it would be helpful in your language, you could use the adjective “true” instead of the noun “truth.” Alternate translation: “the true way”

See: Possession

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#8) (id: 257707)

**ἡ ὁδὸς τῆς ἀληθείας βλασφημηθήσεται**

**"the way of truth will be slandered"**

If your language does not use this passive form, you can state this in active form and say who will do the action. Alternate translation: “unbelievers will slander the way of truth”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#9) (id: 257708)

**ἡ ὁδὸς τῆς ἀληθείας βλασφημηθήσεται**

**"the way of truth will be slandered"**

Peter speaks of **the way of truth** as if it were a person who could be **slandered** or shown disrespect. Alternate translation: “they will say bad things about the way of truth”

See: Personification

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:2 (#10) (id: 257709)

**ἡ ὁδὸς τῆς ἀληθείας βλασφημηθήσεται**

**"the way of truth will be slandered"**

Peter assumes that his audience will know that unbelievers will be the ones who slander the Christian faith when they see the sensual lives of the false teachers and their followers. If it would be helpful to your readers, you could indicate this explicitly. Alternate translation: “the way of truth will be slandered by unbelievers”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Truth (ID: `keyterm:Truth`); Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:3 (#1) (id: 257710)

**ἐν πλεονεξίᾳ**

**"in greed"**

Here, **in** indicates the reason for what the false teachers do. Alternate translation: “because of greed”

* **Associated Passages:** 2 Peter 2:3

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#2) (id: 257711)

**ἐν πλεονεξίᾳ**

**"in greed"**

If it would be helpful in your language, you could express the abstract noun **greed** by translating the idea behind it with an adjective, such as “greedy.” Alternate translation: “because they are greedy”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#3) (id: 257712)

**πλαστοῖς λόγοις**

**"with false words"**

Here, **false words** are the means by which the false teachers will exploit their victims. Alternate translation: “by means of false words”

* **Associated Passages:** 2 Peter 2:3

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#4) (id: 257713)

**πλαστοῖς λόγοις**

**"with false words"**

Peter is using the term **words** to describe the teachings of the false teachers that were conveyed by using **words**. Alternate translation: “by false teachings”

See: Metonymy

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#5) (id: 257714)

**ἐμπορεύσονται**

**"they will exploit"**

Here, **they** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could state this explicitly. Alternate translation: “these false teachers will exploit you”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#6) (id: 257715)

**οἷς τὸ κρίμα ἔκπαλαι οὐκ ἀργεῖ**

**"for whom condemnation from long ago is not idle"**

Here, **whom** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could state this explicitly. Alternate translation: “condemnation from long ago is not idle for these false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#7) (id: 257716)

**οἷς τὸ κρίμα ἔκπαλαι οὐκ ἀργεῖ**

**"for whom condemnation from long ago is not idle"**

Here, **for whom** indicates that the condemnation is directed again the false teachers. Alternate translation: “against whom condemnation from long ago is not idle”

* **Associated Passages:** 2 Peter 2:3

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#8) (id: 257717)

**οἷς τὸ κρίμα ἔκπαλαι οὐκ ἀργεῖ, καὶ ἡ ἀπώλεια αὐτῶν οὐ νυστάζει**

**"for whom condemnation from long ago is not idle, and their destruction does not sleep"**

These two long phrases mean basically the same thing and emphasize that the false teachers will certainly be condemned. If it would be helpful in your language, you could combine them. Alternate translation: “their destruction from long ago is certain”

See: Parallelism

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Parallelism (ID: `33299@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#9) (id: 257718)

**οἷς τὸ κρίμα ἔκπαλαι οὐκ ἀργεῖ, καὶ ἡ ἀπώλεια αὐτῶν οὐ νυστάζει**

**"for whom condemnation from long ago is not idle, and their destruction does not sleep"**

You can translate these phrases with verbs in positive terms. Alternate translation: “their condemnation from long ago is active, and their destruction is awake”

See: Double Negatives

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Double Negatives (ID: `33265@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#10) (id: 257719)

**τὸ κρίμα ἔκπαλαι οὐκ ἀργεῖ, καὶ ἡ ἀπώλεια αὐτῶν οὐ νυστάζει**

**"condemnation from long ago is not idle, and their destruction does not sleep"**

Peter speaks of **condemnation** and **destruction** as if they are people who can be **idle** or **sleep**. If this might be confusing for your readers, you could express the meaning plainly. Alternate translation: “condemnation from long ago is not ineffective, and their destruction is not delayed”

See: Personification

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:3 (#11) (id: 257720)

**οἷς τὸ κρίμα ἔκπαλαι οὐκ ἀργεῖ, καὶ ἡ ἀπώλεια αὐτῶν οὐ νυστάζει**

**"for whom condemnation from long ago is not idle, and their destruction does not sleep"**

If it would be helpful in your language, you could express the abstract nouns “condemnation” and “destruction” by stating the ideas behind them with verbal forms. Alternate translation: “they are not idly condemned from long ago, and they will not be destroyed too late”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:4 (#1) (id: 257721)

**γὰρ**

**"For"**

**For** indicates here that Peter is giving the reason for the result he implicitly described in the previous verse. He is saying why the destruction of the false teachers is certain. Alternate translation: “This is because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#2) (id: 257722)

**εἰ**

**"if"**

Connecting Statement:\\n\\nHere, **if** indicates the beginning of a conditional sentence that extends from [2:4](https://ref.ly/2Pet2:4) to [2:10](https://ref.ly/2Pet2:10). Peter is speaking as if this were a hypothetical possibility, but he means that it is actually true. If your language does not state something as a condition if it is certain or true, and if your readers might think that what Peter is saying is not certain, then you could translate his words as an affirmative statement. Alternate translation: “Since God did not spare the angels who had sinned”

See: Connect — Factual Conditions

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Connect — Factual Conditions (ID: `33326@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#3) (id: 257723)

**οὐκ ἐφείσατο**

**"did not spare"**

Here, **spare** means “to refrain from punishing.” Alternate translation: “did not refrain from punishing”

* **Associated Passages:** 2 Peter 2:4

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#4) (id: 257724)

**ἀγγέλων ἁμαρτησάντων**

**"angels who had sinned"**

Peter uses **who had sinned** to distinguish the angels who were punished by God from those that were not.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#5) (id: 257725)

**σειροῖς ζόφου**

**"in chains of darkness"**

Some of the best ancient manuscripts read “pits” instead of “chains.” If a translation of the Bible exists in your region, you may wish to use the reading that it has. If a translation of the Bible does not exist in your region, you may wish to use the reading in the ULT.

See: Textual Variants

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Textual Variants (ID: `33405@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#6) (id: 257726)

**σειροῖς ζόφου**

**"in chains of darkness"**

This phrase could refer to: (1\) chains in a very dark place. Alternate translation: “in chains in darkness” (2\) a very deep darkness that imprisons them like chains. Alternate translation: “bound in darkness like chains”

See: Metaphor

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#7) (id: 257727)

**ταρταρώσας**

**"throwing down to Tartarus"**

The word **Tartarus** is a term from Ancient Greek religion that refers to the place where evil spirits and wicked men who have died are punished. Some Ancient Jewish literature written in Greek uses **Tartarus** as a term for the place where God punishes the wicked. Alternate translation: “he cast them into hell”

See: How to Translate Names

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#8) (id: 257728)

**παρέδωκεν**

**"he handed over"**

God is the one who **handed over** the angels who had sinned. If it would be helpful to your readers, you could make that explicit in your translation. Alternate translation: “God handed over”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#9) (id: 257729)

**παρέδωκεν**

**"he handed over"**

Here, Peter speaks of God imprisoning the angels who sinned like someone who has **handed over** a criminal to the prison guards for imprisonment. Alternate translation: “imprisoned”

See: Metaphor

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#10) (id: 257730)

**εἰς κρίσιν**

**"for judgment"**

This phrase gives the purpose or goal for which the sinning angels are being held in captivity. Alternate translation: “for the purpose of judgment”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#11) (id: 257731)

**εἰς κρίσιν**

**"for judgment"**

If it would be helpful in your language, you could express the abstract noun **judgment** by translating it with a verbal phrase. Alternate translation: “to be judged”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#12) (id: 257732)

**εἰς κρίσιν τηρουμένους**

**"those being kept for judgment"**

This phrase refers to the sinful angels mentioned earlier in the verse. If it would be helpful to your readers, you could express this explicitly. Alternate translation: “those sinful angels who are being kept for judgment”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:4 (#13) (id: 257733)

**εἰς κρίσιν τηρουμένους**

**"those being kept for judgment"**

If your language does not use this passive form, you can express this with an active verbal form and you can indicate who is doing the action. Alternate translation: “those whom God is keeping for judgment”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:4
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); To Sin (ID: `keyterm:Sin`); An Angel (ID: `deity:Angel`); LORD (ID: `deity:Lord`); Judgment (ID: `keyterm:Judgment`); To Hold in Tartarus (ID: `keyterm:HoldInTartarus`); Hell (ID: `place:Hell`)

## 2 Peter 2:5 (#1) (id: 257734)

**καὶ**

**"and"**

Here, **and** indicates the beginning of the second condition in a conditional sentence that extends from [2:4](https://ref.ly/2Pet2:4) to [2:10](https://ref.ly/2Pet2:10). Peter is speaking as if this were a hypothetical possibility, but he means that it is actually true. If your language does not state something as a condition if it is certain or true, and if your readers might think that what Peter is saying is not certain, then you could translate his words as an affirmative statement. Alternate translation: “Since”

See: Connect — Factual Conditions

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Connect — Factual Conditions (ID: `33326@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#2) (id: 257735)

**ἀρχαίου κόσμου οὐκ ἐφείσατο**

**"he did not spare the ancient world"**

Here, **world** refers to the people who lived in it. Alternate translation: “he did not spare the people who lived in the ancient times”

See: Metonymy

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#3) (id: 257736)

**οὐκ ἐφείσατο**

**"he did not spare"**

As in [2:4](https://ref.ly/2Pet2:4), the word **spare** here means “to refrain from punishing.” Alternate translation: “did not refrain from punishing”

* **Associated Passages:** 2 Peter 2:5

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#4) (id: 257737)

**οὐκ ἐφείσατο**

**"he did not spare"**

Here, **he** refers to God. If it would be helpful for your readers, you could express this explicitly. Alternate translation: “God did not spare”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#5) (id: 257738)

**ὄγδοον, Νῶε**

**"the eighth, Noah"**

Here, **eighth** is an idiom used to refer to a group of eight people. It means that Noah was one of only eight people in the ancient world whom God did not destroy. If it would be helpful in your language, you could express the literal meaning of the idiom. Alternate translation: “eight people, including Noah” or “with seven others, Noah”

See: Idiom

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#6) (id: 257739)

**Νῶε, δικαιοσύνης κήρυκα**

**"Noah, a preacher of righteousness"**

This phrase gives us more information about Noah. It tells us that Noah proclaimed **righteousness** to the ungodly people of the ancient world. It does not distinguish this Noah from any other person named Noah.

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#7) (id: 257740)

**δικαιοσύνης κήρυκα**

**"a preacher of righteousness"**

If it would be helpful in your language, you could express the idea behind the abstract noun **righteousness** with an equivalent expression. In this context, the term refers righteous deeds. Alternate translation: “a preacher of righteous deeds” or “a preacher of how to act rightly”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#8) (id: 257741)

**δικαιοσύνης κήρυκα**

**"a preacher of righteousness"**

Peter could be using the possessive form to refer to: (1\) a preacher who is characterized by righteousness. Alternate translation: “a preacher who is righteous” (2\) a preacher who tells others to live righteously. Alternate translation: “one who urged others to live righteously”

See: Possession

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#9) (id: 257742)

**κατακλυσμὸν κόσμῳ ἀσεβῶν ἐπάξας**

**"having brought a flood upon the world of the ungodly ones"**

This clause indicates when God protected Noah and his other seven family members, when he **brought a flood upon the world**, as it is translated in the UST.

* **Associated Passages:** 2 Peter 2:5

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:5 (#10) (id: 257743)

**κόσμῳ ἀσεβῶν**

**"upon the world of the ungodly ones"**

Peter could be using the possessive form to refer to: (1\) the human content of the ancient world. Alternate translation: “the world that contained ungodly people” (2\) the world as being characterized by ungodliness. Alternate translation: “the ungodly world”

See: Possession

* **Associated Passages:** 2 Peter 2:5
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Noah (ID: `person:Noah`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Ability (ID: `keyterm:Ability`)

## 2 Peter 2:6 (#1) (id: 257744)

**καὶ**

**"and"**

Here, **and** indicates the beginning of the third condition in a conditional sentence that extends from [2:4](https://ref.ly/2Pet2:4) to [2:10](https://ref.ly/2Pet2:10). Peter is speaking as if this were a hypothetical possibility, but he means that it is actually true. If your language does not state something as a condition if it is certain or true, and if your readers might think that what Peter is saying is not certain, then you could translate his words as an affirmative statement. Alternate translation: “Since”

See: Connect — Factual Conditions

* **Associated Passages:** 2 Peter 2:6
* **Associated Articles:** Connect — Factual Conditions (ID: `33326@UWTranslationManual`)

* **Associated ACAI Concepts:** Sodom (ID: `place:Sodom`); Gomorrah (ID: `place:Gomorrah`); Condemn (ID: `keyterm:Condemn`)

## 2 Peter 2:6 (#2) (id: 257745)

**καὶ πόλεις Σοδόμων καὶ Γομόρρας τεφρώσας καταστροφῇ κατέκρινεν**

**"and the cities of Sodom and Gomorrah, having reduced to ashes, he condemned to destruction"**

If it would be natural in your language, you could reverse the order of these phrases. Alternate translation: “and he condemned the cities of Sodom and Gomorrah to destruction, having reduced them to ashes”

See: Information Structure

* **Associated Passages:** 2 Peter 2:6
* **Associated Articles:** Information Structure (ID: `33286@UWTranslationManual`)

* **Associated ACAI Concepts:** Sodom (ID: `place:Sodom`); Gomorrah (ID: `place:Gomorrah`); Condemn (ID: `keyterm:Condemn`)

## 2 Peter 2:6 (#3) (id: 257746)

**πόλεις Σοδόμων καὶ Γομόρρας τεφρώσας**

**"the cities of Sodom and Gomorrah, having reduced to ashes"**

This phrase indicates the means by which God destroyed **Sodom and Gomorrah**. Alternate translation: “by means of reducing the cities of Sodom and Gomorrah to ashes”

* **Associated Passages:** 2 Peter 2:6

* **Associated ACAI Concepts:** Sodom (ID: `place:Sodom`); Gomorrah (ID: `place:Gomorrah`); Condemn (ID: `keyterm:Condemn`)

## 2 Peter 2:6 (#4) (id: 257747)

**καταστροφῇ κατέκρινεν**

**"he condemned to destruction"**

Here, **he** refers to God. If it would be helpful to your readers, you could state that explicitly. Alternate translation: “God condemned them to destruction”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:6
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Sodom (ID: `place:Sodom`); Gomorrah (ID: `place:Gomorrah`); Condemn (ID: `keyterm:Condemn`)

## 2 Peter 2:6 (#5) (id: 257748)

**καταστροφῇ κατέκρινεν**

**"he condemned to destruction"**

If it would be helpful in your language, you could express the abstract noun **destruction** by translating the idea behind it with an verb, such as “destroy.” Alternate translation: “he condemned them to be destroyed”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:6
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Sodom (ID: `place:Sodom`); Gomorrah (ID: `place:Gomorrah`); Condemn (ID: `keyterm:Condemn`)

## 2 Peter 2:6 (#6) (id: 257749)

**ὑπόδειγμα μελλόντων ἀσεβέσιν τεθεικώς**

**"having set an example {of the things that} are going to happen to the ungodly ones"**

This clause indicates the result of what happened in the previous clauses of the verse. God’s destruction of Sodom and Gomorrah resulted in them being an **example** and a warning of what happens to others who disobey God. Alternate translation: “with the result that God set them as an example of the things that will happen to the ungodly”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 2:6
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Sodom (ID: `place:Sodom`); Gomorrah (ID: `place:Gomorrah`); Condemn (ID: `keyterm:Condemn`)

## 2 Peter 2:6 (#7) (id: 257750)

**ἀσεβέσιν**

**"to the ungodly ones"**

Here, **ungodly** refers to wicked people in general, not to one specific wicked person. Alternate translation: “to an ungodly person” or “to ungodly people”

See: Generic Noun Phrases

* **Associated Passages:** 2 Peter 2:6
* **Associated Articles:** Generic Noun Phrases (ID: `33277@UWTranslationManual`)

* **Associated ACAI Concepts:** Sodom (ID: `place:Sodom`); Gomorrah (ID: `place:Gomorrah`); Condemn (ID: `keyterm:Condemn`)

## 2 Peter 2:7 (#1) (id: 257751)

**καὶ**

**"and"**

Connecting Statement:\\n\\nHere, **and** indicates the beginning of the fourth condition in a conditional sentence that extends from [2:4](https://ref.ly/2Pet2:4) to [2:10](https://ref.ly/2Pet2:10). Peter is speaking as if this were a hypothetical possibility, but he means that it is actually true. If your language does not state something as a condition if it is certain or true, and if your readers might think that what Peter is saying is not certain, then you could translate his words as an affirmative statement. Alternate translation: “since”

See: Connect — Factual Conditions

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Connect — Factual Conditions (ID: `33326@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#2) (id: 257752)

**ἐρύσατο**

**"he rescued"**

Here, **he** refers to God. If it would be helpful in your language, you could express this explicitly. Alternate translation, as in the UST: “God rescued”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#3) (id: 257753)

**καταπονούμενον ὑπὸ τῆς τῶν ἀθέσμων ἐν ἀσελγείᾳ ἀναστροφῆς**

**"being oppressed by the behavior of the lawless ones in licentiousness"**

Connecting Statement:\\n\\nThis clause is giving more information about **Lot**. If it would be helpful to your readers, you could begin a new sentence here to clarify this. Alternate translation: “He was being oppressed by the behavior of the lawless ones in licentiousness”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#4) (id: 257754)

**καταπονούμενον ὑπὸ τῆς τῶν ἀθέσμων ἐν ἀσελγείᾳ ἀναστροφῆς**

**"being oppressed by the behavior of the lawless ones in licentiousness"**

If your language does not use this passive form, you can express this with an active form. Alternate translation: “the behavior of the lawless ones in licentiousness oppressed him”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#5) (id: 257755)

**ὑπὸ τῆς τῶν ἀθέσμων ἐν ἀσελγείᾳ ἀναστροφῆς**

**"by the behavior of the lawless ones in licentiousness"**

Here, **by** could refer to: (1\) the thing that was oppressing Lot, as in the ULT. (2\) the reason why Lot was oppressed. Alternate translation: “because of the behavior of the lawless ones in licentiousness”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#6) (id: 257756)

**ὑπὸ τῆς τῶν ἀθέσμων \& ἀναστροφῆς**

**"by the behavior of the lawless ones"**

If it would be helpful in your language, you could translate the abstract noun **behavior** with an equivalent expression. Alternate translation: “by what the lawless ones did” or “by how the lawless ones acted”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#7) (id: 257757)

**τῆς τῶν ἀθέσμων ἐν ἀσελγείᾳ ἀναστροφῆς**

**"the behavior of the lawless ones in licentiousness"**

Here, **in** indicates the content of what the lawless people were doing. If it would be helpful in your language, you could translate **in licentiousness** with an adjective. Alternate translation: “the licentious behavior of the lawless ones”

* **Associated Passages:** 2 Peter 2:7

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#8) (id: 257758)

**τῆς τῶν ἀθέσμων ἐν ἀσελγείᾳ ἀναστροφῆς**

**"the behavior of the lawless ones in licentiousness"**

If it would be helpful in your language, you could translate the abstract noun **licentiousness** with an adjective. See how you translated the plural form of this term in [2:2](https://ref.ly/2Pet2:2). Alternate translation: “the licentious behavior of the lawless ones” or “the wild sensual behavior of the lawless ones”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:7 (#9) (id: 257759)

**τῶν ἀθέσμων**

**"of the lawless ones"**

Here, **the lawless ones** refers to the people who lived in the city of Sodom, where Lot lived. If it would be helpful in your language, you could state this phrase explicitly. Alternate translation: “of the lawless people of Sodom” or “of the people who act as if there is no law in Sodom”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Lot (ID: `person:Lot`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Lawless (ID: `keyterm:Lawless`); Deliver (ID: `keyterm:Deliver`)

## 2 Peter 2:8 (#1) (id: 257760)

**γὰρ**

**"for"**

Peter uses **for** here to provide background information about Lot’s life in Sodom. This is to help readers understand why Peter called Lot a righteous person in the previous verse. Peter is not using **for** here to indicate result. Use the natural form in your language for introducing background information. Alternate translation: “now”

See: Background Information

* **Associated Passages:** 2 Peter 2:8
* **Associated Articles:** Background Information (ID: `33415@UWTranslationManual`)

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:8 (#2) (id: 257761)

**βλέμματι γὰρ καὶ ἀκοῇ**

**"for, by seeing and by hearing"**

If it would be helpful in your language, you could translate the abstract nouns **seeing** and **hearing** with verbal phrases. Alternate translation: “for by what he saw and what he heard”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:8
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:8 (#3) (id: 257762)

**ὁ δίκαιος**

**"that righteous {man"**

This refers to Lot. If it would be helpful in your language, you could state this phrase explicitly. Alternate translation: “righteous Lot”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:8
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:8 (#4) (id: 257763)

**ἐνκατοικῶν ἐν αὐτοῖς**

**"living among them"**

This clause indicates the time when Lot dwelled in Sodom. Alternate translation: “when he lived among them”

* **Associated Passages:** 2 Peter 2:8

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:8 (#5) (id: 257764)

**αὐτοῖς**

**"them"**

Here, the pronoun **them** refers to the inhabitants of Sodom. If it would be helpful to your readers, you could specify what the pronoun **them** refers to. Alternate translation: “the people of Sodom”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:8
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:8 (#6) (id: 257765)

**ἐνκατοικῶν ἐν αὐτοῖς ἡμέραν ἐξ ἡμέρας**

**"living among them day from day"**

This phrase, **day from day**, is an idiom meaning “day after day” or “every day.” You may need to express this literally in your language. Alternate translation: “living among them day after day” or “living among them every day”

See: Idiom

* **Associated Passages:** 2 Peter 2:8
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:8 (#7) (id: 257766)

**ψυχὴν δικαίαν \& ἐβασάνιζεν**

**"was tormenting his righteous soul"**

Here, **soul** refers to Lot’s thoughts and emotions. The immoral behavior of the citizens of Sodom and Gomorrah disturbed him emotionally. Alternate translation: “was greatly disturbed”

See: Synecdoche

* **Associated Passages:** 2 Peter 2:8
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:8 (#8) (id: 257767)

**ἀνόμοις ἔργοις**

**"by lawless works"**

This phrase could mean: (1\) the **lawless works** were the means by which Lot tormented his soul. Alternate translation: “with lawless works” (2\) the **lawless works** were the reason why Lot tormented his soul. Alternate translation: “because of lawless works”

* **Associated Passages:** 2 Peter 2:8

* **Associated ACAI Concepts:** Lawless (ID: `keyterm:Lawless`); Be (Or Show Oneself) Just (ID: `keyterm:Just`)

## 2 Peter 2:9 (#1) (id: 257768)

**οἶδεν Κύριος**

**"the Lord knows how"**

This verse and the next verse are the end of a conditional sentence that extends from [2:4](https://ref.ly/2Pet2:4) to [2:10](https://ref.ly/2Pet2:10). Peter is giving the result of the previous conditions being true. If you have made [2:4–10](https://ref.ly/2Pet2:4-2Pet2:10) into separate sentences, then you will need to indicate that [2:9](https://ref.ly/2Pet2:9) is the result of the previous conditions being true. Alternate translation: “Therefore, it is true that the Lord knows how” or “Since these things are true, it is also true that the Lord knows how”

See: Connect — Factual Conditions

* **Associated Passages:** 2 Peter 2:9
* **Associated Articles:** Connect — Factual Conditions (ID: `33326@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); LORD (ID: `deity:Lord`); Deliver (ID: `keyterm:Deliver`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:9 (#2) (id: 257769)

**ἀδίκους δὲ εἰς ἡμέραν κρίσεως κολαζομένους τηρεῖν**

**"and to keep the unrighteous ones to be punished in the day of judgment"**

Here, **and** could refer to: (1\) a contrast between the previous clause and what follows, as in the ULT and UST. (2\) a simple connection between the previous clause and what follows. Alternate translation: “and to keep the unrighteous ones to be punished in the day of judgment”

See: Connecting Words and Phrases

* **Associated Passages:** 2 Peter 2:9
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); LORD (ID: `deity:Lord`); Deliver (ID: `keyterm:Deliver`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:9 (#3) (id: 257770)

**πειρασμοῦ \& ἀδίκους δὲ εἰς ἡμέραν κρίσεως κολαζομένους τηρεῖν**

**"a trial, and to keep the unrighteous ones to be punished in the day of judgment"**

In this clause, Peter is leaving out some words that it would need in many languages in order to be complete. These words can be supplied from the previous clause. Alternate translation: “a trial and the Lord knows how to keep the unrighteous ones to be punished in the day of judgment”

See: Ellipsis

* **Associated Passages:** 2 Peter 2:9
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); LORD (ID: `deity:Lord`); Deliver (ID: `keyterm:Deliver`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:9 (#4) (id: 257771)

**κολαζομένους**

**"to be punished"**

This is a purpose clause. Peter is stating the purpose for which God is keeping the unrighteous people. Alternate translation (without a comma preceding): “in order to be punished”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** 2 Peter 2:9
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); LORD (ID: `deity:Lord`); Deliver (ID: `keyterm:Deliver`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:9 (#5) (id: 257772)

**ἀδίκους \& κολαζομένους τηρεῖν**

**"to keep the unrighteous ones to be punished"**

If your language does not use this passive form, you can express this with an active form. Alternate translation: “to keep the unrighteous ones to punish them”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:9
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); LORD (ID: `deity:Lord`); Deliver (ID: `keyterm:Deliver`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:9 (#6) (id: 257773)

**εἰς ἡμέραν κρίσεως**

**"in the day of judgment"**

Here, **in** could refer to: (1\) when the unrighteous ones will be punished. Alternate translation: “on the day of judgment” (2\) the point in time up to which the unrighteous ones will be punished. Alternate translation: “until the day of judgment”

* **Associated Passages:** 2 Peter 2:9

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); LORD (ID: `deity:Lord`); Deliver (ID: `keyterm:Deliver`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:9 (#7) (id: 257774)

**ἡμέραν κρίσεως**

**"the day of judgment"**

Peter is using the possessive form to describe a **day** that is characterized by **judgment**. If it would be helpful in your language, you could use a phrase to explain it. Alternate translation: “the day when God judges mankind”

See: Possession

* **Associated Passages:** 2 Peter 2:9
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); LORD (ID: `deity:Lord`); Deliver (ID: `keyterm:Deliver`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:10 (#1) (id: 257775)

**δὲ**

**"but"**

Connecting Statement:\\n\\nHere, **but** indicates a connection between the last clause of the previous verse and what follows. It does not indicate a contrast between “the unrighteous” in the previous verse and “those going after the flesh” in this verse. Alternate translation: “and especially those going after the flesh”

See: Connecting Words and Phrases

* **Associated Passages:** 2 Peter 2:10
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#2) (id: 257776)

**τοὺς ὀπίσω \& πορευομένους**

**"the ones going after"**

Connecting Statement:\\n\\nPeter uses the phrase **going after** to refer to habitually doing something. This expression is frequently used in the Bible to describe people worshiping false gods or engaging in sexual immorality. If it would be helpful in your language, you could express this plainly. Alternate translation: “those habitually engaging in”

See: Metaphor

* **Associated Passages:** 2 Peter 2:10
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#3) (id: 257777)

**σαρκὸς**

**"the flesh"**

Here, **flesh** is used to refer to person’s sinful nature. If it would be helpful in your language, you could state this figure of speech literally. Alternate translation: “their sinful nature”

See: Metonymy

* **Associated Passages:** 2 Peter 2:10
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#4) (id: 257778)

**ἐν ἐπιθυμίᾳ μιασμοῦ**

**"in {its} lusts of defilement"**

Here, **in** indicates that this phrase shows the means by which the wicked people go after the flesh. Alternate translation: “by means of practicing its lusts of defilement”

* **Associated Passages:** 2 Peter 2:10

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#5) (id: 257779)

**ἐν ἐπιθυμίᾳ μιασμοῦ**

**"in {its} lusts of defilement"**

If it would be helpful in your language, you could express the idea behind the abstract noun **defilement** with a verbal phrase. Alternate translation: “in its lusts that defile”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:10
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#6) (id: 257780)

**καὶ κυριότητος καταφρονοῦντας**

**"and despising authority"**

Here, **and** indicates that this clause provides an additional characteristic of those mentioned in the previous clause. It does not indicate a second group of wicked people. These wicked people not only pursue their sinful desires, but also despise authority. Alternate translation: “and who also despise authority”

See: Connecting Words and Phrases

* **Associated Passages:** 2 Peter 2:10
* **Associated Articles:** Connecting Words and Phrases (ID: `33335@UWTranslationManual`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#7) (id: 257781)

**κυριότητος καταφρονοῦντας**

**"despising authority"**

Here, **authority** could refer to: (1\) God’s authority, as suggested from the examples of rebelling against God in the previous verses. Alternate translation: “despising God’s authority” (2\) angelic authority, as suggested by the insulting of “glorious ones” mentioned in the remainder of the verse. Alternate translation: “despising the authority of angels”

* **Associated Passages:** 2 Peter 2:10

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#8) (id: 257782)

**τολμηταὶ**

**"Audacious ones"**

**Audacious ones** marks the beginning of the second section of this chapter, which continues until the end of [2:22](https://ref.ly/2Pet2:22). In this section Peter describes the wicked character and deeds of the false teachers.

* **Associated Passages:** 2 Peter 2:10

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#9) (id: 257783)

**τολμηταὶ αὐθάδεις**

**"Audacious ones! Self\-willed ones"**

The words **Audacious ones** and **Self\-willed ones** are exclamations that emphasize the bold pride of the false teachers. Alternate translation: “They are so audacious and self\-willed!” or “How audacious and self\-willed they are!”

See: Exclamations

* **Associated Passages:** 2 Peter 2:10
* **Associated Articles:** Exclamations (ID: `33270@UWTranslationManual`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#10) (id: 257784)

**αὐθάδεις**

**"Self\-willed ones"**

**Self\-willed** means “doing whatever one wants to do.” Alternate translation: “Those who do whatever they want”

* **Associated Passages:** 2 Peter 2:10

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#11) (id: 257785)

**οὐ τρέμουσιν**

**"They do not tremble"**

Here, **They** refers to the false teachers Peter introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful to your readers, you could express this explicitly. Alternate translation: “These false teachers do not tremble”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:10
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#12) (id: 257786)

**δόξας \& βλασφημοῦντες**

**"while insulting glorious ones"**

This clause indicates the time when the false teachers do not tremble. Alternate translation: “when insulting glorious ones”

* **Associated Passages:** 2 Peter 2:10

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:10 (#13) (id: 257787)

**δόξας**

**"glorious ones"**

Here, **glorious ones** could refer to: (1\) spiritual beings, such as angels, demons, or both. Alternate translation: “glorious spiritual beings” (2\) important human beings, such as church leaders. Alternate translation: “glorious people”

* **Associated Passages:** 2 Peter 2:10

* **Associated ACAI Concepts:** Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Blaspheme (ID: `keyterm:Blaspheme`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:11 (#1) (id: 257788)

**ἰσχύϊ καὶ δυνάμει μείζονες ὄντες**

**"being greater in strength and power"**

This clause could refer to: (1\) a contrast between the description of the **angels** in this clause with their behavior in the next clause. Alternate translation: “despite being greater in strength and power” (2\) a description of the **angels**. Alternate translation: “who are greater in strength and power”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** 2 Peter 2:11
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Blasphemous (ID: `keyterm:Blasphemous`); Court Case (ID: `keyterm:CourtCase`); Ability (ID: `keyterm:Ability`); An Angel (ID: `deity:Angel`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:11 (#2) (id: 257789)

**ἰσχύϊ καὶ δυνάμει μείζονες ὄντες**

**"being greater in strength and power"**

Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from the surrounding context, which is a description of the false teachers. Alternate translation: “being greater in strength and power than these false teachers”

See: Ellipsis

* **Associated Passages:** 2 Peter 2:11
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Blasphemous (ID: `keyterm:Blasphemous`); Court Case (ID: `keyterm:CourtCase`); Ability (ID: `keyterm:Ability`); An Angel (ID: `deity:Angel`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:11 (#3) (id: 257790)

**ἰσχύϊ καὶ δυνάμει μείζονες ὄντες**

**"being greater in strength and power"**

The words **strength** and **power** mean similar things. Peter is using them together for emphasis. Together, the words describe extreme power. If it would be helpful in your language, you could translate this pair of words with a single expression. Alternate translation: “being much more powerful”

See: Doublet

* **Associated Passages:** 2 Peter 2:11
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Blasphemous (ID: `keyterm:Blasphemous`); Court Case (ID: `keyterm:CourtCase`); Ability (ID: `keyterm:Ability`); An Angel (ID: `deity:Angel`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:11 (#4) (id: 257791)

**οὐ φέρουσιν κατ’ αὐτῶν \& βλάσφημον κρίσιν**

**"do not bring an insulting judgment against them"**

Here, **them** could mean: (1\) the “glorious ones.” Alternate translation: “do not bring an insulting judgment against these glorious ones.” (2\) the false teachers. Alternate translation: “do not bring an insulting judgment against these false teachers.”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:11
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Blasphemous (ID: `keyterm:Blasphemous`); Court Case (ID: `keyterm:CourtCase`); Ability (ID: `keyterm:Ability`); An Angel (ID: `deity:Angel`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 2:12 (#1) (id: 257792)

**οὗτοι**

**"these {men"**

Here, **these {men}** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). Alternate translation: “these false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#2) (id: 257793)

**οὗτοι \& ὡς ἄλογα ζῷα**

**"these {men}, as unreasoning animals"**

Peter describes the false teachers by comparing them to **unreasoning animals**. Just as **animals** cannot think rationally, neither can these people. Alternate translation: “these false teachers are like animals who are not capable of thinking rationally”

See: Metaphor

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#3) (id: 257794)

**γεγεννημένα φυσικὰ εἰς ἅλωσιν καὶ φθοράν**

**"having by nature been born for capture and destruction"**

This clause gives more information about the unreasoning animals and thus, by way of comparison, the false teachers. Alternate translation: “which have by nature been born for capture and destruction”

See: Distinguishing Versus Informing or Reminding

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Distinguishing Versus Informing or Reminding (ID: `33264@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#4) (id: 257795)

**γεγεννημένα φυσικὰ εἰς ἅλωσιν καὶ φθοράν**

**"having by nature been born for capture and destruction"**

Here, **by nature** means that the unreasoning animals (and false teachers by comparison) were destined for capture and destruction due to their very nature as animals born for this purpose. Alternate translation: “according to their nature, these animals are born for capture and destruction”

* **Associated Passages:** 2 Peter 2:12

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#5) (id: 257796)

**εἰς ἅλωσιν καὶ φθοράν**

**"for capture and destruction"**

This is a purpose clause. The word **for** here indicates that what follows it is the purpose for which these animals have been born. Alternate translation: “for the purpose of capture and destruction”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#6) (id: 257797)

**εἰς ἅλωσιν καὶ φθοράν**

**"for capture and destruction"**

If it would be helpful in your language, you could express the abstract nouns **capture** and **destruction** by stating the ideas behind them with verbs. Alternate translation: “in order to capture and destroy them”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#7) (id: 257798)

**ἐν οἷς ἀγνοοῦσιν βλασφημοῦντες**

**"slandering about {those things} in which they are ignorant"**

This clause could refer to: (1\) more information about the false teachers. Alternate translation: “who slander those things in which they are ignorant” (2\) the reason the false teachers will be destroyed. Alternate translation: “because they slander those things in which they are ignorant”

* **Associated Passages:** 2 Peter 2:12

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#8) (id: 257799)

**ἐν οἷς ἀγνοοῦσιν**

**"about {those things} in which they are ignorant"**

Here, **those things** could refer to: (1\) the “glorious ones” of [2:10](https://ref.ly/2Pet2:10). Alternate translation: “who slander those of whom they are ignorant” (2\) the Christian teachings that these false teachers are rejecting. Alternate translation: “who slander the teachings of which they are ignorant”

* **Associated Passages:** 2 Peter 2:12

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#9) (id: 257800)

**ἐν τῇ φθορᾷ αὐτῶν καὶ φθαρήσονται**

**"they will also be destroyed in their destruction"**

Here, the pronouns **they** and **their** refer to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could express this explicitly. Alternate translation: “these false teachers will also be destroyed in their destruction”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#10) (id: 257801)

**καὶ φθαρήσονται**

**"they will also be destroyed"**

If your language does not use this passive form, you can express this with an active form, and you can state who will do the action. Alternate translation: “God will also destroy them”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#11) (id: 257802)

**καὶ φθαρήσονται**

**"they will also be destroyed"**

Here, **also** is used for emphasis and may be translated as “indeed” or “surely.” Alternate translation: “they will indeed be destroyed” or “they will surely be destroyed”

* **Associated Passages:** 2 Peter 2:12

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#12) (id: 257803)

**ἐν τῇ φθορᾷ αὐτῶν**

**"in their destruction"**

This clause indicates the time when the false teachers will be destroyed. Alternate translation: “at the time of their destruction”

* **Associated Passages:** 2 Peter 2:12

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:12 (#13) (id: 257804)

**ἐν τῇ φθορᾷ αὐτῶν**

**"in their destruction"**

If it would be helpful in your language, you could express the idea behind the abstract noun **destruction** with a verb, such as “destroy.” Alternate translation: “when they are destroyed”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:12
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Blaspheme (ID: `keyterm:Blaspheme`)

## 2 Peter 2:13 (#1) (id: 257805)

**ἀδικούμενοι μισθὸν ἀδικίας**

**"being harmed {as} wages of unrighteousness"**

Peter speaks of the punishment that the false teachers will receive as if it were a wage they had earned. If it would be helpful in your language, you could express this plainly. Alternate translation: “receiving the punishment they deserve for their unrighteousness”

See: Metaphor

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#2) (id: 257806)

**ἀδικίας**

**"of unrighteousness"**

If it would be helpful in your language, you could express the abstract noun **unrighteousness** by translating it with an equivalent expression. Alternate translation: “of the wrong things they have done”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#3) (id: 257807)

**ἡδονὴν**

**"pleasure"**

If it would be helpful in your language, you could express the abstract noun **pleasure** by translating it with an equivalent expression. Alternate translation: “something that pleases”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#4) (id: 257808)

**τὴν ἐν ἡμέρᾳ τρυφήν**

**"the reveling in the day"**

Here, **reveling** refers to immoral activity that includes gluttony, drunkenness, and sexual activity. If it would be helpful in your language, you could express this abstract noun by translating it with an equivalent expression. Alternate translation: “their ability to revel in the day”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#5) (id: 257809)

**τὴν ἐν ἡμέρᾳ τρυφήν**

**"the reveling in the day"**

This phrase indicates the time when the false teachers were **reveling**. If it would be helpful in your language, you could translate **in** as “during.” Doing these things “during the day” indicates that these people are not ashamed of this behavior. Alternate translation: “reveling during the day”

* **Associated Passages:** 2 Peter 2:13

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#6) (id: 257810)

**σπίλοι καὶ μῶμοι**

**"stains and blemishes"**

Peter speaks of the false teachers as if they were **stains** or **blemishes** on a garment that cause shame for those who wear it. If it would be helpful in your language, you could express this metaphor by translating it as a simile. Alternate translation: “like stains and blemishes on clothes, which cause disgrace”

See: Metaphor

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#7) (id: 257811)

**σπίλοι καὶ μῶμοι**

**"stains and blemishes"**

The words **stains** and **blemishes** share similar meanings. Peter is using them together for emphasis. If it would be helpful in your language, you could translate them with a single expression. Alternate translation: “unsightly stains”

See: Doublet

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#8) (id: 257812)

**σπίλοι καὶ μῶμοι**

**"stains and blemishes"**

For emphasis, here Peter is leaving out some of the words that a sentence would need in many languages in order to be complete. These words can be supplied from the context. Alternate translation: “They are stains and blemishes”

See: Ellipsis

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:13 (#9) (id: 257813)

**ἐντρυφῶντες ἐν ταῖς ἀπάταις αὐτῶν**

**"reveling in their deceptions"**

If it would be helpful in your language, you could translate the abstract noun **deceptions** with an adjective like “deceptive.” Alternate translation: “reveling in their deceptive deeds”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:13
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Unjust Deed (ID: `keyterm:UnjustDeed`); To Act Wickedly (ID: `keyterm:ActWickedly`)

## 2 Peter 2:14 (#1) (id: 257814)

**ὀφθαλμοὺς ἔχοντες μεστοὺς μοιχαλίδος**

**"having eyes full of an adulteress"**

Here, **eyes** refers to a person’s desires, and **eyes full** means that a person constantly desires one thing. Alternate translation: “constantly desiring an adulteress”

See: Metonymy

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#2) (id: 257815)

**ὀφθαλμοὺς ἔχοντες μεστοὺς μοιχαλίδος**

**"having eyes full of an adulteress"**

This clause could mean: (1\) the false teachers constantly desired to have immoral sexual relations with any woman they saw, thereby viewing every women as a potential adulteress. Alternate translation: “constantly desiring women with whom to commit adultery” (2\) the false teachers were constantly looking for immoral women with whom to have immoral sexual relations. Alternate translation: “constantly seeking sexually immoral women”

* **Associated Passages:** 2 Peter 2:14

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#3) (id: 257816)

**δελεάζοντες ψυχὰς ἀστηρίκτους**

**"enticing unstable souls"**

This clause refers to the actions of the false teachers Peter introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could express this explicitly. Alternate translation: “these false teachers entice unstable souls”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#4) (id: 257817)

**δελεάζοντες ψυχὰς ἀστηρίκτους**

**"enticing unstable souls"**

Here, **souls** refers to persons. Alternate translation: “enticing unstable people”

See: Synecdoche

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Synecdoche (ID: `33315@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#5) (id: 257818)

**καρδίαν γεγυμνασμένην πλεονεξίας ἔχοντες**

**"having hearts trained in covetousness"**

This clause refers to the actions of the false teachers Peter introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could express this explicitly. Alternate translation: “they have hearts trained in covetousness”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#6) (id: 257819)

**καρδίαν γεγυμνασμένην πλεονεξίας ἔχοντες**

**"having hearts trained in covetousness"**

If your language does not use this passive form, you can express this with an active form, and you can state who did the action. Alternate translation: “they trained their hearts to be covetous”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#7) (id: 257820)

**καρδίαν γεγυμνασμένην πλεονεξίας ἔχοντες**

**"having hearts trained in covetousness"**

Peter is using **hearts** to refer to whole persons, including their thoughts, desires, and emotions. The term can thus be translated here with the reflexive pronoun “themselves.” Alternate translation: “having trained themselves to covet”

See: Metonymy

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#8) (id: 257821)

**καρδίαν γεγυμνασμένην πλεονεξίας ἔχοντες**

**"having hearts trained in covetousness"**

If it would be helpful in your language, you could express the idea behind the abstract noun **covetousness** with a verb. Alternate translation: “having training their hearts to covet”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#9) (id: 257822)

**κατάρας τέκνα**

**"children of cursing"**

Peter is using a Hebrew idiom in which a person is said to be a “child” of the thing that characterizes that person. Here, **children of cursing** refers to people who are cursed by God. He is not speaking of people who curse others. Alternate translation: “accursed people”

See: Idiom

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:14 (#10) (id: 257823)

**κατάρας τέκνα**

**"children of cursing"**

These words are an exclamation that emphasizes the wickedness of the false teachers. Alternate translation: “They are such cursed children!” or “What cursed children they are!”

See: Exclamations

* **Associated Passages:** 2 Peter 2:14
* **Associated Articles:** Exclamations (ID: `33270@UWTranslationManual`)

* **Associated ACAI Concepts:** Be Unfaithful (ID: `keyterm:Unfaithful`); Heart (ID: `keyterm:Heart`); To Sin (ID: `keyterm:Sin`)

## 2 Peter 2:15 (#1) (id: 257824)

**καταλειπόντες εὐθεῖαν ὁδὸν**

**"Abandoning the straight way"**

Peter uses the metaphor **Abandoning the straight way** to give an image of walkers leaving a certain pathway. He speaks of the false teachers refusing to live their lives in obedience to the Lord as if they had stopped walking on the Lord’s path. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “refusing to live in obedience to God”

See: Metaphor

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#2) (id: 257825)

**εὐθεῖαν ὁδὸν**

**"the straight way"**

Here, **the straight way** refers to the way of living life that is right and pleasing to the Lord. Peter may also be using it here to refer specifically to the Christian faith, similar to his use of “the way of truth” in [2:2](https://ref.ly/2Pet2:2). Alternate translation: “the right way of the Lord”

See: Idiom

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#3) (id: 257826)

**ἐπλανήθησαν**

**"they have gone astray"**

Here, Peter continues the path metaphor from the previous clause. He describes the wicked lifestyles of the false teachers as if they had been **led astray** from the straight path. If it would be helpful in your language, you could express this metaphor by translating this plainly. Alternate translation: “they have been behaving wickedly”

See: Metaphor

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#4) (id: 257827)

**ἐπλανήθησαν**

**"they have gone astray"**

If your language does not use this passive form, you can express this with an active form. Alternate translation: “they went astray”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#5) (id: 257828)

**ἐξακολουθήσαντες τῇ ὁδῷ τοῦ Βαλαὰμ τοῦ Βοσὸρ, ὃς μισθὸν ἀδικίας ἠγάπησεν**

**"having followed the way of Balaam of Bosor, who loved the wages of unrighteousness"**

In this verse, Peter compares the false teachers to **Balaam**. Peter assumes that his readers will know that he is referring to a story recorded in the Old Testament book of Numbers. In that story, Balaam was hired by wicked kings to curse the Israelites. When God did not allow Balaam to do so, he used wicked women to seduce the Israelites into sexual immorality and idol worship so that God would punish them for their disobedience. Balaam did these wicked things because he wanted to be paid by the wicked kings, but he was eventually killed by the Israelites when they conquered the land of Canaan. You could indicate this explicitly if it would be helpful to your readers, particularly if they would not know the story. Alternate translation, as a statement: “following the way of Balaam son of Bosor, who loved the wages of unrighteousness so much that he led the Israelites into immorality and idolatry in order to receive money”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#6) (id: 257829)

**ἐξακολουθήσαντες τῇ ὁδῷ τοῦ Βαλαὰμ τοῦ Βοσὸρ**

**"having followed the way of Balaam of Bosor"**

Here, Peter uses the word **followed** to refer to someone imitating the actions of someone else, like someone who walks behind another person in the same direction. If it would be helpful in your language, you could state this metaphor literally. Alternate translation: “having imitated the way of Balaam son of Bosor”

See: Metaphor

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#7) (id: 257830)

**Βαλαὰμ \& Βοσὸρ**

**"of Balaam of Bosor"**

**Bosor** is the name of a man.

See: How to Translate Names

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** How to Translate Names (ID: `33391@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#8) (id: 257831)

**τῇ ὁδῷ τοῦ Βαλαὰμ τοῦ Βοσὸρ**

**"the way of Balaam of Bosor"**

Here, Peter uses **the way of Balaam** to refer to how Balaam lived his life. If it would be helpful in your language, you could state this metaphor literally. Alternate translation: “the manner of living of Balaam son of Bosor”

See: Metaphor

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#9) (id: 257832)

**ὃς μισθὸν ἀδικίας ἠγάπησεν**

**"who loved the wages of unrighteousness"**

Here, the pronoun**who** refers to Balaam. It does not refer to Bosor, nor to the false teachers. If it would be helpful in your language, you could express this directly. If you begin a new sentence, you will have to replace the comma with a period. Alternate translation: “Balaam loved the wages of unrighteousness”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#10) (id: 257833)

**ὃς μισθὸν ἀδικίας ἠγάπησεν**

**"who loved the wages of unrighteousness"**

Peter is using the possessive form to describe **wages** that are characterized by **unrighteousness**. If it would be helpful in your language, you could use a phrase to explain it. Alternate translation: “who loved the unrighteous wages”

See: Possession

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:15 (#11) (id: 257834)

**μισθὸν ἀδικίας**

**"the wages of unrighteousness"**

If it would be helpful in your language, you could express the idea behind the abstract noun **unrighteousness** with the adjective “unrighteous.” Alternate translation: “the unrighteous wages” or “the wages for unrighteous acts”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:15
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Balaam (ID: `person:Balaam`); Beor (ID: `person:Beor.2`); Love (ID: `keyterm:Love`); Straightness (ID: `keyterm:Straightness`); Unjust Deed (ID: `keyterm:UnjustDeed`)

## 2 Peter 2:16 (#1) (id: 257835)

**ἔλεγξιν \& ἔσχεν**

**"he had a rebuke"**

If it would be helpful in your language, you could express the abstract noun **rebuke** as a verb and you could state who did the action. Alternate translation: “God rebuked him”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Donkey (ID: `fauna:Donkey`)

## 2 Peter 2:16 (#2) (id: 257836)

**ἔλεγξιν \& ἔσχεν**

**"he had a rebuke"**

If it would be helpful in your language, you could specify who rebuked Balaam. This clause could mean: (1\) the donkey rebuked Balaam. Alternate translation: “a donkey rebuked him” (2\) God rebuked Balaam through the donkey. Alternate translation: “God rebuked him”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Donkey (ID: `fauna:Donkey`)

## 2 Peter 2:16 (#3) (id: 257837)

**ἰδίας παρανομίας**

**"for his own transgression"**

This **transgression** specifically refers to Balaam’s use of wicked women to lead the Israelites into sexual immorality and idolatry. If it would be helpful to your readers, you could express this explicitly. Alternate translation: “for his wicked act of leading the Israelites into immorality”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Donkey (ID: `fauna:Donkey`)

## 2 Peter 2:16 (#4) (id: 257838)

**τὴν τοῦ προφήτου παραφρονίαν**

**"the irrationality of the prophet"**

If it would be helpful in your language, you could express the idea behind the abstract noun **irrationality** with an adjective like “irrational” or “foolish.” Alternate translation: “the irrational action of the prophet” or “the foolish action of the prophet”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Donkey (ID: `fauna:Donkey`)

## 2 Peter 2:16 (#5) (id: 257839)

**ἐκώλυσεν τὴν τοῦ προφήτου παραφρονίαν**

**"restrained the irrationality of the prophet"**

Here, **the prophet** refers to Balaam. If it would be helpful in your language, you could express this explicitly. Alternate translation: “restrained the madness of the prophet Balaam”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 2:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Donkey (ID: `fauna:Donkey`)

## 2 Peter 2:17 (#1) (id: 257840)

**οὗτοί**

**"These {men"**

**These {men}** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could express this explicitly. Alternate translation: “These false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:17
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)


## 2 Peter 2:17 (#2) (id: 257841)

**οὗτοί εἰσιν πηγαὶ ἄνυδροι**

**"These {men} are waterless springs"**

Peter speaks of the false teachers to describe their uselessness. People expect **springs** to provide water to quench thirst, but **waterless springs** leave thirsty people disappointed. In the same way, false teachers, although they promise many things, are unable to do what they promise. If it would be helpful in your language, you could express this metaphor by translating this plainly or turn the metaphor into a simile. Alternate translation: “These men are disappointing, like springs without water”

See: Metaphor

* **Associated Passages:** 2 Peter 2:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## 2 Peter 2:17 (#3) (id: 257842)

**ὁμίχλαι ὑπὸ λαίλαπος ἐλαυνόμεναι**

**"mists driven by a storm"**

Peter gives a second figurative description of the uselessness of the false teachers. When people see storm clouds, they expect rain to fall. When the winds from the **storm** blow the clouds away before the rain can fall, the people are disappointed. In the same way, false teachers, although they promise many things, are unable to do what they promise. If it would be helpful in your language, you could express this metaphor by translating this plainly, or turn the metaphor into a simile. Alternate translation: “they never give what they promise” or “they are disappointing like rain clouds that the storm drives away”

See: Metaphor

* **Associated Passages:** 2 Peter 2:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## 2 Peter 2:17 (#4) (id: 257843)

**οὗτοί εἰσιν πηγαὶ ἄνυδροι, καὶ ὁμίχλαι ὑπὸ λαίλαπος ἐλαυνόμεναι**

**"These {men} are waterless springs and mists driven by a storm"**

These two metaphors mean similar things. Peter is using them together for emphasis. If it would be helpful in your language, you could express this connection by translating them with a single expression. Alternate translation: “These are men who surely never give what they promise” or “These are men who surely disappoint”

See: Doublet

* **Associated Passages:** 2 Peter 2:17
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)


## 2 Peter 2:17 (#5) (id: 257844)

**οἷς ὁ ζόφος τοῦ σκότους τετήρηται**

**"for whom the gloom of darkness has been reserved"**

If your language does not use this passive form, you can express this with an active form, and you can state who did the action. Alternate translation: “for whom God has reserved the gloom of darkness”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:17
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)


## 2 Peter 2:17 (#6) (id: 257845)

**ὁ ζόφος τοῦ σκότους**

**"the gloom of darkness"**

This could mean: (1\) the **gloom** is characterized by **darkness**. Alternate translation: “the dark gloom” (2\) the **gloom** is identical to the **darkness**. Alternate translation: “the gloom, which is darkness.”

* **Associated Passages:** 2 Peter 2:17


## 2 Peter 2:17 (#7) (id: 257846)

**ὁ ζόφος τοῦ σκότους**

**"the gloom of darkness"**

Here, Peter uses **gloom** and **darkness** to refer to hell. If it would be helpful in your language, you could express this directly. Alternate translation: “for whom God has reserved the gloom of darkness of hell”

See: Metaphor

* **Associated Passages:** 2 Peter 2:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## 2 Peter 2:18 (#1) (id: 257847)

**γὰρ**

**"For"**

Here, **For** indicates the reason why the false teachers are reserved for punishment in the gloom of darkness, as mentioned in the previous verse. Alternate translation: “This is so because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 2:18
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:18 (#2) (id: 257848)

**ὑπέρογκα \& ματαιότητος φθεγγόμενοι**

**"speaking arrogant things of vanity"**

This clause indicates the means by which the false teachers entice others to sin. Alternate translation: “by means of speaking arrogant things of vanity”

* **Associated Passages:** 2 Peter 2:18

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:18 (#3) (id: 257849)

**ὑπέρογκα \& ματαιότητος**

**"arrogant things of vanity"**

Peter is using the possessive form to describe **arrogant** speech that is characterized by **vanity**. Alternate translation: “vain, arrogant things” or “vain and arrogant things”

See: Possession

* **Associated Passages:** 2 Peter 2:18
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:18 (#4) (id: 257850)

**ματαιότητος**

**"of vanity"**

If it would be helpful in your language, you could express the idea behind the abstract noun **vanity** with an adjective, such as “vain.”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:18
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:18 (#5) (id: 257851)

**δελεάζουσιν ἐν ἐπιθυμίαις σαρκὸς**

**"they entice by lusts of the flesh"**

Here, the pronoun **they** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could express this explicitly. Alternate translation: “these false teachers entice by the lusts of the flesh”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:18
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:18 (#6) (id: 257852)

**ἐν ἐπιθυμίαις σαρκὸς**

**"by lusts of the flesh"**

Here, **flesh** is used to refer to a person’s sinful nature. If it would be helpful in your language, you could state the literal meaning for the metaphor. Alternate translation: “by the lusts of their sinful natures”

See: Metaphor

* **Associated Passages:** 2 Peter 2:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:18 (#7) (id: 257853)

**ἀσελγείαις**

**"by licentious acts"**

Here, **licentious acts** refers to immoral sexual actions that demonstrate a lack of self\-control. See how you translated this term in [2:2](https://ref.ly/2Pet2:2). Alternate translation: “uncontrolled sensual acts”

* **Associated Passages:** 2 Peter 2:18

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:18 (#8) (id: 257854)

**τοὺς ὀλίγως ἀποφεύγοντας τοὺς ἐν πλάνῃ ἀναστρεφομένους**

**"the ones barely escaping from the ones living in error"**

Here, Peter speaks of people who recently became believers as **those barely escaping** from sinful humanity. He also refers to unbelievers who still live according to their sinful desires as **those living in error**. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “people who recently ceased living sinfully as other people do”

See: Metaphor

* **Associated Passages:** 2 Peter 2:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Worthless (ID: `keyterm:Worthless`); Behavior (ID: `keyterm:Behavior`); Body (ID: `keyterm:Body.3`)

## 2 Peter 2:19 (#1) (id: 257855)

**ἐλευθερίαν αὐτοῖς ἐπαγγελλόμενοι**

**"promising freedom to them"**

This clause refers to another means by which the false teachers enticed their followers, continuing from the previous verse. Alternate translation: “They also entice them by promising freedom to them”

* **Associated Passages:** 2 Peter 2:19

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:19 (#2) (id: 257856)

**ἐλευθερίαν αὐτοῖς ἐπαγγελλόμενοι**

**"promising freedom to them"**

Here, **freedom** is a metaphor for the ability to live exactly as one wants. Alternate translation: “promising to give them the ability to live exactly as they want to live”

See: Metaphor

* **Associated Passages:** 2 Peter 2:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:19 (#3) (id: 257857)

**ἐλευθερίαν αὐτοῖς ἐπαγγελλόμενοι**

**"promising freedom to them"**

Here, the pronoun **them** refers to those people who are deceived by the false teachers. If it would be helpful in your language, you could express this explicitly. Alternate translation: “promising freedom to those whom they deceive”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:19
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:19 (#4) (id: 257858)

**αὐτοὶ δοῦλοι ὑπάρχοντες τῆς φθορᾶς;**

**"while themselves being slaves of destruction"**

Peter uses the word **themselves** here to emphasize the irony of spiritually enslaved people promising spiritual freedom to others. Alternate translation: “while they themselves are slaves of destruction”

See: Reflexive Pronouns

* **Associated Passages:** 2 Peter 2:19
* **Associated Articles:** Reflexive Pronouns (ID: `33309@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:19 (#5) (id: 257859)

**δοῦλοι**

**"slaves"**

Peter speaks of people who live sinfully as if they were **slaves** to sin who need to escape from their captivity. If it would be helpful in your language, you could state this metaphor as a simile. Alternate translation: “like slaves”

See: Metaphor

* **Associated Passages:** 2 Peter 2:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:19 (#6) (id: 257860)

**δοῦλοι \& τῆς φθορᾶς**

**"slaves of destruction"**

Peter is using the possessive form to describe **slaves** that are characterized by **destruction**. Alternate translation: “slaves that will be destroyed”

See: Possession

* **Associated Passages:** 2 Peter 2:19
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:19 (#7) (id: 257861)

**ᾧ γάρ τις ἥττηται, τούτῳ δεδούλωται**

**"For by what someone has been overcome, by this he has been enslaved"**

Peter speaks of a person as being **enslaved** when anything has control over that person, he speaks of that thing as the master of that person. If it would be helpful in your language, you could express this metaphor by translating this plainly or as a simile. Alternate translation: “For if a person is overpowered by something, that person becomes controlled by that thing” or “For if a person is overpowered by something, that person becomes like a slave to that thing”

See: Metaphor

* **Associated Passages:** 2 Peter 2:19
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:19 (#8) (id: 257862)

**ᾧ γάρ τις ἥττηται, τούτῳ δεδούλωται**

**"For by what someone has been overcome, by this he has been enslaved"**

If it would be helpful in your language, you could express this passive sentence with an active form. Alternate translation: “For if something overpowers a person, that thing enslaves that person”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:19
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 2:20 (#1) (id: 257863)

**γὰρ**

**"For"**

Here, **For** could refer to: (1\) another explanation of Peter’s statement “themselves being slaves of destruction” in the previous verse, (2\) a transition from what Peter said in the previous verse to what he is going to say in this verse. Here, **For** does not indicate the reason or result of what was said in the previous verse. Alternate translation, as in the UST: “And”

* **Associated Passages:** 2 Peter 2:20

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#2) (id: 257864)

**εἰ \& ἀποφυγόντες τὰ μιάσματα τοῦ κόσμου, ἐν ἐπιγνώσει τοῦ Κυρίου ἡμῶν καὶ Σωτῆρος, Ἰησοῦ Χριστοῦ, τούτοις δὲ πάλιν ἐμπλακέντες ἡττῶνται, γέγονεν αὐτοῖς τὰ ἔσχατα χείρονα τῶν πρώτων**

**"if, having escaped the defilements of the world through the knowledge of our Lord and Savior Jesus Christ, but, having been entangled again, they have been overcome by these things, for them the last things have become worse than the first things"**

Peter is speaking as if this were a hypothetical possibility, but he means that it is actually true. If your language does not state something as a condition if it is certain or true, and if your readers might think that what Peter is saying is not certain, then you could translate his words as an affirmative statement. Alternate translation: “they have escaped the impurities of the world through the knowledge of our Lord and Savior Jesus Christ, but having become entangled and overcome by them again, the last has become worse for them than the first”

See: Connect — Factual Conditions

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Connect — Factual Conditions (ID: `33326@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#3) (id: 257865)

**εἰ \& ἀποφυγόντες τὰ μιάσματα τοῦ κόσμου**

**"if, having escaped the defilements of the world"**

Using a similar metaphor to that in [2:18](https://ref.ly/2Pet2:18), here Peter speaks of believers as if they had been slaves to the **defilements** of the world and have **escaped** that captivity. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “if they have ceased living in the defiling manner of the world”

See: Metaphor

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#4) (id: 257866)

**τὰ μιάσματα τοῦ κόσμου**

**"the defilements of the world"**

If it would be helpful in your language, you could express the idea behind the abstract noun **defilements** with an equivalent expression. Alternate translation: “the things that sinful human society does to defile itself”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#5) (id: 257867)

**τὰ μιάσματα τοῦ κόσμου**

**"the defilements of the world"**

Here, the **world** refers to human society, which has been corrupted by sin. Alternate translation: “the defiling practices of sinful human society”

See: Metonymy

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#6) (id: 257868)

**ἐν ἐπιγνώσει τοῦ Κυρίου ἡμῶν καὶ Σωτῆρος, Ἰησοῦ Χριστοῦ**

**"through the knowledge of our Lord and Savior Jesus Christ"**

If it would be helpful in your language, you could express the idea behind the abstract noun **knowledge** with a verbal phrase. See how you translated similar phrases in [1:2](https://ref.ly/2Pet1:2). Alternate translation: “by knowing our Lord and Savior Jesus Christ”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#7) (id: 257869)

**τοῦ Κυρίου ἡμῶν καὶ Σωτῆρος**

**"of our Lord and Savior"**

Here, **our Lord** means “the person who is lord over us” or “the person who rules over us.” The conjunction **and** indicates that **our** also applies to **Savior**, meaning “the person who saves us.” Alternate translation: “of the person who rules over us and saves us”

See: Possession

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#8) (id: 257870)

**τούτοις \& πάλιν ἐμπλακέντες ἡττῶνται**

**"having been entangled again, they have been overcome by these things"**

If your language does not use this passive form, you can express this with an active form, and you can supply the doer of the action from earlier in the verse. Alternate translation: “these things having entangled them again; these things have overcome them”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#9) (id: 257871)

**πάλιν ἐμπλακέντες**

**"having been entangled again"**

Here, Peter speaks of people who seem to have been believers but have returned to living sinfully as if they have become **entangled** in a net. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “if they have begun to live sinfully again”

See: Metaphor

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#10) (id: 257872)

**τούτοις**

**"by these things"**

Here, the pronoun **these things** refers to “the defilements of the world.” If it would be helpful to your readers, you could specify this in your translation. Alternate translation: “by these defilements of the world”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#11) (id: 257873)

**αὐτοῖς**

**"for them"**

Connecting Statement:\\n\\nHere, the pronoun **them** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1) and discussed in [2:12–19](https://ref.ly/2Pet2:12-2Pet2:19). If it would be helpful in your language, you could express this explicitly. Alternate translation: “these false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:20 (#12) (id: 257874)

**γέγονεν αὐτοῖς τὰ ἔσχατα χείρονα τῶν πρώτων**

**"for them the last things have become worse than the first things"**

Here, the adjectives **last** and **first** function as nouns. They are plural, and ULT supplies the noun **things** in each case to show that. If your language does not use adjectives in this way, you can supply a more specific singular noun. Alternate translation: “the final condition for them is worse than their original condition”

See: Nominal Adjectives

* **Associated Passages:** 2 Peter 2:20
* **Associated Articles:** Nominal Adjectives (ID: `33294@UWTranslationManual`)

* **Associated ACAI Concepts:** Jesus (ID: `person:Jesus.2`); Defilement (ID: `keyterm:Defilement`); Ability (ID: `keyterm:Ability`); Savior (ID: `keyterm:Savior`)

## 2 Peter 2:21 (#1) (id: 257875)

**γὰρ**

**"For"**

Here, **For** indicates the reason why the last condition of the false teachers was worse than their first condition, as mentioned in the previous verse. Alternate translation: “This is so because”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#2) (id: 257876)

**αὐτοῖς**

**"for them"**

Here, the pronoun **them** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could express this explicitly. Alternate translation: “for these false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#3) (id: 257877)

**τὴν ὁδὸν τῆς δικαιοσύνης**

**"the way of righteousness"**

Peter is using the possessive form to describe **the way** that is characterized by **righteousness**. If it would be helpful in your language, you could use a phrase to explain it. Alternate translation: “the righteous way”

See: Possession

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#4) (id: 257878)

**τὴν ὁδὸν τῆς δικαιοσύνης**

**"the way of righteousness"**

Peter speaks of life as a **way** or path. This phrase refers to the way of living life that is right and pleasing to the Lord. Peter may also be using it here to refer specifically to the Christian faith, similar to his use of “the way of truth” in [2:2](https://ref.ly/2Pet2:2) and “the straight way” in [2:15](https://ref.ly/2Pet2:15). Alternate translation: “the manner of living that pleases the Lord”

See: Idiom

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#5) (id: 257879)

**ἐπιγνοῦσιν**

**"having known {it"**

This clause indicates that the next clause describes an event that happened after the event of this clause. If it would be helpful in your language, you could express this explicitly. Alternate translation: “after having known it”

* **Associated Passages:** 2 Peter 2:21

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#6) (id: 257880)

**ὑποστρέψαι ἐκ τῆς \& ἁγίας ἐντολῆς**

**"to turn away from the holy commandment"**

Here, **to turn away from** is a metaphor that means to stop doing something. Alternate translation: “to stop obeying the holy commandment”

See: Metaphor

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#7) (id: 257881)

**τῆς παραδοθείσης \& ἁγίας ἐντολῆς**

**""**

Peter uses **the holy commandment** to speak of the commands of God in general. He is not referring to one specific **commandment**. These commands were **delivered** by the apostles to the believers. Alternate translation: “the holy commands”

See: Generic Noun Phrases

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Generic Noun Phrases (ID: `33277@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#8) (id: 257882)

**τῆς παραδοθείσης \& ἁγίας ἐντολῆς**

**""**

If it would be helpful in your language, you could express the ideas behind the abstract noun **commandment** with an equivalent expression. Alternate translation: “what God commanded”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:21 (#9) (id: 257883)

**τῆς παραδοθείσης αὐτοῖς ἁγίας ἐντολῆς**

**"the holy commandment that has been delivered to them"**

If your language does not use this passive form, you can express this with an active form, and you can state who did the action. Alternate translation: “the holy commandment that the apostles delivered to them”

See: Active or Passive

* **Associated Passages:** 2 Peter 2:21
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Be (Or Show Oneself) Just (ID: `keyterm:Just`); Commandment (ID: `keyterm:Commandment`); Holy (ID: `keyterm:Holy`)

## 2 Peter 2:22 (#1) (id: 257884)

**συμβέβηκεν αὐτοῖς τὸ τῆς ἀληθοῦς παροιμίας**

**"This of the true proverb has happened to them"**

Here, **This** refers to the **proverb** Peter states later in this verse. It does not refer back to a statement from the previous verse. If it would be helpful in your language, you could use a different expression. Alternate translation: “What this true proverb says has happened to them” or “This true proverb describes what happened to them”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:22
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Pig (ID: `fauna:Pig`); A Person (ID: `person:GenericPerson`); True (ID: `keyterm:True`); Jackal (ID: `fauna:Jackal`); Dog (ID: `fauna:Dog`)

## 2 Peter 2:22 (#2) (id: 257885)

**αὐτοῖς**

**"to them"**

Here, the pronoun **them** refers to the false teachers introduced in [2:1](https://ref.ly/2Pet2:1). If it would be helpful in your language, you could express this explicitly. Alternate translation: “to these false teachers”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 2:22
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Pig (ID: `fauna:Pig`); A Person (ID: `person:GenericPerson`); True (ID: `keyterm:True`); Jackal (ID: `fauna:Jackal`); Dog (ID: `fauna:Dog`)

## 2 Peter 2:22 (#3) (id: 257886)

**κύων ἐπιστρέψας ἐπὶ τὸ ἴδιον ἐξέραμα, καί, ὗς λουσαμένη, εἰς κυλισμὸν βορβόρου**

**"A dog returns to its own vomit, and a washed pig to the rolling of mud"**

Peter uses two proverbs to illustrate what the false teachers have done. These proverbs make a figurative comparison: Just as a dog returns to eat its own vomit and a washed pig rolls in the mud again, so these false teachers, who once stopped living a sinful life, have now gone back to living sinfully. Although they knew “the way of righteousness,” they went back to doing the things that defile them morally and spiritually. If it would be helpful in your language, you could express the meaning by translating these proverbs as similes. Alternate translation: “They are like dogs that eat their own vomit or like clean pigs that go back to rolling in the mud.”

See: Proverbs

* **Associated Passages:** 2 Peter 2:22
* **Associated Articles:** Proverbs (ID: `33425@UWTranslationManual`)

* **Associated ACAI Concepts:** Pig (ID: `fauna:Pig`); A Person (ID: `person:GenericPerson`); True (ID: `keyterm:True`); Jackal (ID: `fauna:Jackal`); Dog (ID: `fauna:Dog`)

## 2 Peter 2:22 (#4) (id: 257887)

**κύων**

**"A dog"**

A **dog** is an animal that is considered to be unclean and disgusting by Jews and many cultures of the Ancient Near East. Therefore, calling someone a **dog** was an insult. If dogs are unfamiliar to your culture and you have a different animal that is considered unclean and disgusting or whose name is used as an insult, you could use the name of this animal instead.

See: Translate Unknowns

* **Associated Passages:** 2 Peter 2:22
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Pig (ID: `fauna:Pig`); A Person (ID: `person:GenericPerson`); True (ID: `keyterm:True`); Jackal (ID: `fauna:Jackal`); Dog (ID: `fauna:Dog`)

## 2 Peter 2:22 (#5) (id: 257888)

**ὗς**

**"a"** \- **"pig"**

A **pig** is an animal that is considered to be unclean and disgusting by Jews and many cultures of the Ancient Near East. Therefore, calling someone a **pig** was an insult. If pigs are unfamiliar to your culture and you have a different animal that is considered unclean and disgusting or whose name is used as an insult, you could use the name of this animal instead.

See: Translate Unknowns

* **Associated Passages:** 2 Peter 2:22
* **Associated Articles:** Translate Unknowns (ID: `33408@UWTranslationManual`)

* **Associated ACAI Concepts:** Pig (ID: `fauna:Pig`); A Person (ID: `person:GenericPerson`); True (ID: `keyterm:True`); Jackal (ID: `fauna:Jackal`); Dog (ID: `fauna:Dog`)

## 2 Peter 3:1 (#1) (id: 257889)

**ἀγαπητοί**

**"Beloved ones"**

**Beloved ones** refers here to those to whom Peter is writing, which can be extended to all believers. If it would be helpful in your language, you could state this term explicitly. Alternate translation: “Beloved fellow believers”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:1
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Beloved (ID: `keyterm:Beloved`); Mind (ID: `keyterm:Mind.2`)

## 2 Peter 3:1 (#2) (id: 257890)

**ἐν αἷς**

**"in which"**

Here, **which** refers to both this letter and the previous letter that Peter had written to this group of believers. If it would be helpful in your language, you could start a new sentence to say this explicitly. Alternate translation: “In both of these letters”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:1
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Beloved (ID: `keyterm:Beloved`); Mind (ID: `keyterm:Mind.2`)

## 2 Peter 3:1 (#3) (id: 257891)

**διεγείρω ὑμῶν ἐν ὑπομνήσει τὴν εἰλικρινῆ διάνοιαν**

**"I arouse your pure mind in remembrance"**

Here, Peter uses **arouse**, as if the minds of his readers are asleep, to refer to causing his readers to think about these things. If it would be helpful in your language, you could express this metaphor by translating this plainly. See how you translated this term in [1:13](https://ref.ly/2Pet1:13). Alternate translation: “to remind your sincere mind of these things so that you will think about them”

See: Metaphor

* **Associated Passages:** 2 Peter 3:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Beloved (ID: `keyterm:Beloved`); Mind (ID: `keyterm:Mind.2`)

## 2 Peter 3:1 (#4) (id: 257892)

**ἐν ὑπομνήσει**

**"in remembrance"**

If it would be helpful in your language, you could express the idea behind the abstract noun **remembrance** in this phrase with a verb. See how you translated this term in [1:13](https://ref.ly/2Pet1:13). Alternate translation: “to remember”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:1
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Beloved (ID: `keyterm:Beloved`); Mind (ID: `keyterm:Mind.2`)

## 2 Peter 3:1 (#5) (id: 257893)

**ὑμῶν \& τὴν εἰλικρινῆ διάνοιαν**

**"your pure mind"**

Although the word **pure** usually refers to something being uncontaminated or not mixed with something else, Peter uses it here to indicate that his readers have minds which have not been deceived by the false teachers. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “your undeceived minds”

See: Metaphor

* **Associated Passages:** 2 Peter 3:1
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Pure (ID: `keyterm:Pure`); Beloved (ID: `keyterm:Beloved`); Mind (ID: `keyterm:Mind.2`)

## 2 Peter 3:2 (#1) (id: 257894)

**μνησθῆναι**

**"to remember"**

Here, Peter is stating a purpose for which he is writing this letter. Alternate translation (without a comma preceding): “that you should remember”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#2) (id: 257895)

**τῶν προειρημένων ῥημάτων, ὑπὸ τῶν ἁγίων προφητῶν**

**"the words spoken previously by the holy prophets"**

If your language does not use this passive form, you can state this in active form. Alternate translation: “the words that the holy prophets spoke previously”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#3) (id: 257896)

**τῶν προειρημένων ῥημάτων**

**"the words spoken previously"**

Peter is using **words** here to describe the prophecies of the Old Testament prophets that were conveyed by using words, especially those prophecies about the future return of Christ. Alternate translation: “the prophecies spoken previously”

See: Metonymy

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#4) (id: 257897)

**ὑπὸ τῶν ἁγίων προφητῶν**

**"by the holy prophets"**

Here, **prophets** refers to the Old Testament prophets whom Peter also referred to in [1:19–21](https://ref.ly/2Pet1:19-2Pet1:21). If it would be helpful in your language, you could express this explicitly. Alternate translation: “by the holy Old Testament prophets”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#5) (id: 257898)

**τῆς τῶν ἀποστόλων ὑμῶν ἐντολῆς τοῦ Κυρίου καὶ Σωτῆρος**

**"the command of the Lord and Savior through your apostles"**

If your language does not use this passive form, you can state this in active form. Alternate translation: “the command of our Lord and Savior, which your apostles gave to you”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#6) (id: 257899)

**τῆς \& ἐντολῆς τοῦ Κυρίου καὶ Σωτῆρος**

**"the command of the Lord and Savior"**

If it would be helpful in your language, you could express the idea behind the abstract noun **command** with an equivalent expression. Alternate translation: “what our Lord and Savior commanded”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#7) (id: 257900)

**τῆς \& ἐντολῆς τοῦ Κυρίου καὶ Σωτῆρος**

**"the command of the Lord and Savior"**

Peter uses **command** here to speak of the commands of Jesus in general. He is not referring to one specific command. These commands were delivered by the apostles to the believers. Alternate translation: “the commands of the Lord and Savior”

See: Generic Noun Phrases

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Generic Noun Phrases (ID: `33277@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#8) (id: 257901)

**τοῦ Κυρίου**

**"of the Lord"**

If it would be helpful in your language, you could express the idea behind the abstract noun **Lord** with an equivalent expression. Alternate translation: “the person who rules”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#9) (id: 257902)

**Σωτῆρος**

**"Savior"**

If it would be helpful in your language, you could express the idea behind the abstract noun **Savior** with an equivalent expression. Alternate translation: “the person who saves”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#10) (id: 257903)

**τῶν ἀποστόλων ὑμῶν**

**"through your apostles"**

This phrase indicates the means by which **the command of the Lord and Savior** was given to Peter’s readers. Alternate translation: “by means of your apostles”

* **Associated Passages:** 2 Peter 3:2

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:2 (#11) (id: 257904)

**τῶν ἀποστόλων ὑμῶν**

**"through your apostles"**

Here, **your apostles** could refer to: (1\) the apostles who proclaimed the teachings of Christ to Peter’s readers or ministered to them in some way. Alternate translation: “the apostles who serve you” (2\) all apostles, who belong to all Christians. Alternate translation: “the apostles of us all”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:2
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`); Commandment (ID: `keyterm:Commandment`); Jesus (ID: `person:Jesus.2`); Savior (ID: `keyterm:Savior`); Word (ID: `keyterm:Word`)

## 2 Peter 3:3 (#1) (id: 257905)

**τοῦτο πρῶτον γινώσκοντες**

**"knowing this first"**

Peter uses **first** here to refer to the degree of importance. It does not to refer to order in time. See how you translated this in [1:20](https://ref.ly/2Pet1:20). Alternate translation: “Most importantly, you must understand”

* **Associated Passages:** 2 Peter 3:3


## 2 Peter 3:3 (#2) (id: 257906)

**τοῦτο πρῶτον γινώσκοντες**

**"knowing this first"**

Peter is using a statement to give an instruction. If it would be helpful in your language, you could indicate this by translating it as a command. If you do so, it might be helpful to start a new sentence here. See how you translated this in [1:20](https://ref.ly/2Pet1:20). Alternate translation: “Above all, know this”

See: Statements — Other Uses

* **Associated Passages:** 2 Peter 3:3
* **Associated Articles:** Statements — Other Uses (ID: `33263@UWTranslationManual`)


## 2 Peter 3:3 (#3) (id: 257907)

**ἐλεύσονται \& ἐν ἐμπαιγμονῇ ἐμπαῖκται**

**"mockers will come with mockery"**

If it would be helpful in your language, you could express the idea behind the abstract noun **mockery** with a verb. Alternate translation: “mockers will come and mock”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:3
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)


## 2 Peter 3:3 (#4) (id: 257908)

**κατὰ τὰς ἰδίας ἐπιθυμίας αὐτῶν πορευόμενοι**

**"going according to their own lusts"**

Here, Peter uses **going** to refer to habitually doing something, like someone who walks toward something. If it would be helpful in your language, you could state this metaphor literally. Alternate translation: “those habitually living according to their own lusts”

See: Metaphor

* **Associated Passages:** 2 Peter 3:3
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)


## 2 Peter 3:3 (#5) (id: 257909)

**κατὰ τὰς ἰδίας ἐπιθυμίας αὐτῶν πορευόμενοι**

**"going according to their own lusts"**

Here, **lusts** refers to sinful desires that are opposed to God’s will. Alternate translation: “living according to their own sinful desires”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:3
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)


## 2 Peter 3:4 (#1) (id: 257910)

**καὶ λέγοντες**

**"and saying"**

Consider natural ways of introducing direct quotations in your language. Alternate translation: “and they will say”

See: Quotations and Quote Margins

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Quotations and Quote Margins (ID: `33426@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#2) (id: 257911)

**ποῦ ἐστιν ἡ ἐπαγγελία τῆς παρουσίας αὐτοῦ?**

**"Where is the promise of his coming"**

The mockers ask this rhetorical question to emphasize that they do not believe that Jesus will return. Alternate translation: “There is no promise of his coming!” or “The promise of his coming is not true!”

See: Rhetorical Question

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#3) (id: 257912)

**ποῦ ἐστιν ἡ ἐπαγγελία τῆς παρουσίας αὐτοῦ?**

**"Where is the promise of his coming"**

Here, **Where** is used to ask what has happened to the promise. The mockers are not asking for the location of something. If it would be helpful in your language, you could express this idiom by translating this plainly. Alternate translation: “What happened to the promise of his coming?” or “What has come about regarding the promise of his coming?”

See: Idiom

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#4) (id: 257913)

**ποῦ ἐστιν ἡ ἐπαγγελία τῆς παρουσίας αὐτοῦ**

**"Where is the promise of his coming"**

Here, **promise** refers to the fulfillment of the promise that Jesus will return. Alternate translation: “Where is the fulfillment of the promise of his coming?”

See: Metonymy

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#5) (id: 257914)

**ἡ ἐπαγγελία τῆς παρουσίας αὐτοῦ**

**"the promise of his coming"**

Here, the pronoun **his** refers to Jesus. If it would be helpful in your language, you could express this explicitly. Alternate translation: “the promise of Jesus’ coming”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#6) (id: 257915)

**τῆς παρουσίας αὐτοῦ**

**"of his coming"**

Here, **his coming** refers to the return of the Lord Jesus to earth. If it would be helpful to your readers, you could express this explicitly. Alternate translation: “the return of Jesus to earth”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#7) (id: 257916)

**ἀφ’ ἧς γὰρ οἱ πατέρες ἐκοιμήθησαν**

**"For since the fathers fell asleep"**

Here, **the fathers** is used. It could refer to: (1\) the Old Testament ancestors of the Israelites, often called “the patriarchs.” Alternate translation: “For since Israel’s patriarchs fell asleep” (2\) those leaders of the first generation of Christians who had died by the time Peter wrote this letter. Alternate translation: “For since the first Christian leaders fell asleep”

See: Metaphor

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#8) (id: 257917)

**οἱ πατέρες ἐκοιμήθησαν**

**"the fathers fell asleep"**

Here, **fell asleep** is a euphemism meaning they died. You could either use a similar euphemism for death in your language or say this plainly. Alternate translation: “the fathers died”

See: Euphemism

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Euphemism (ID: `33268@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#9) (id: 257918)

**πάντα οὕτως διαμένει ἀπ’ ἀρχῆς κτίσεως**

**"all things continue in the same way from the beginning of creation"**

Here, **all things** is an exaggeration the mockers use to argue that nothing in the world has ever changed, so it cannot be true that Jesus will return.

See: Hyperbole

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Hyperbole (ID: `33281@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:4 (#10) (id: 257919)

**ἀπ’ ἀρχῆς κτίσεως**

**"from the beginning of creation"**

If it would be helpful in your language, you could express the idea behind the abstract noun **creation** with a verbal phrase. Alternate translation: “since God created the world”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:4
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`)

## 2 Peter 3:5 (#1) (id: 257920)

**λανθάνει γὰρ αὐτοὺς τοῦτο, θέλοντας**

**"For this is hidden {from} them willingly"**

If your language does not use this passive form, you can state this in active form. Alternate translation: “For they willingly hide this from themselves”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:5
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:5 (#2) (id: 257921)

**θέλοντας ὅτι οὐρανοὶ ἦσαν ἔκπαλαι**

**"willingly, that the heavens existed long ago"**

Peter is leaving out some of the words that this clause would need in many languages in order to be complete. These words can be supplied from the end of the verse. Alternate translation: “that the heavens existed long ago by the word of God”

See: Ellipsis

* **Associated Passages:** 2 Peter 3:5
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:5 (#3) (id: 257922)

**γῆ ἐξ ὕδατος καὶ δι’ ὕδατος συνεστῶσα τῷ τοῦ Θεοῦ λόγῳ**

**"the earth had been formed out of water and through water by the word of God"**

If your language does not use this passive form, you can state this in active form. Alternate translation: “the word of God formed the earth out of water and through water”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:5
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:5 (#4) (id: 257923)

**ἐξ ὕδατος καὶ δι’ ὕδατος συνεστῶσα**

**"had been formed out of water and through water"**

This clause refers to God causing the land to come up **out of** and **through** the **water**, gathering the bodies of water together to make the land appear.

* **Associated Passages:** 2 Peter 3:5

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:5 (#5) (id: 257924)

**τῷ τοῦ Θεοῦ λόγῳ**

**"by the word of God"**

Here, **the word of God** refers to the God’s specific commands by which the earth was created. If it would be helpful in your language, you could express this explicitly. Alternate translation: “by the commands of God”

See: Metonymy

* **Associated Passages:** 2 Peter 3:5
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:6 (#1) (id: 257925)

**δι’ ὧν**

**"through which"**

Here, **which** refers both to God’s word and water. If it would be helpful in your language, you could express this explicitly. Alternate translation: “through the word of God and water”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:6
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Ability (ID: `keyterm:Ability`)

## 2 Peter 3:6 (#2) (id: 257926)

**ὕδατι κατακλυσθεὶς**

**"having been flooded by water"**

If your language does not use this passive form, you can state this in active form and you can state who did the action. You may need to begin a new sentence. Alternate translation: “God flooded the world with water”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:6
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Ability (ID: `keyterm:Ability`)

## 2 Peter 3:6 (#3) (id: 257927)

**ὁ τότε κόσμος**

**"the world at that time"**

Here, **at that time** refers to the time when the world existed before the flood. It does not refer to the exact time when the universe was created. Alternate translation: “the world that existed then”

* **Associated Passages:** 2 Peter 3:6

* **Associated ACAI Concepts:** Ability (ID: `keyterm:Ability`)

## 2 Peter 3:6 (#4) (id: 257928)

**ὕδατι κατακλυσθεὶς**

**"having been flooded by water"**

This phrase indicates the means by which the ancient world was destroyed. Alternate translation: “by having been flooded by water”

* **Associated Passages:** 2 Peter 3:6

* **Associated ACAI Concepts:** Ability (ID: `keyterm:Ability`)

## 2 Peter 3:7 (#1) (id: 257929)

**οἱ δὲ νῦν οὐρανοὶ καὶ ἡ γῆ**

**"But the present heavens and the earth"**

Here, **But** indicates that Peter is contrasting the past destruction of the ancient world he mentioned in the previous verse with the future destruction of the present world. If it would be helpful in your language, you could state this contrast explicitly. Alternate translation: “In contrast, the present heavens and the earth”

See: Connect — Contrast Relationship

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#2) (id: 257930)

**οἱ \& νῦν οὐρανοὶ καὶ ἡ γῆ, τῷ αὐτῷ λόγῳ τεθησαυρισμένοι εἰσὶν, πυρὶ**

**"the present heavens and the earth, by the same word, are having been reserved for fire"**

If your language does not use this passive form, you can state this in active form and say who has done the action. Alternate translation: “God, by that same word, has reserved the present heavens and the earth for fire”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#3) (id: 257931)

**τῷ αὐτῷ λόγῳ**

**"by the same word"**

Here, **word** refers to “the word of God,” which Peter said in [3:5–6](https://ref.ly/2Pet3:5-2Pet3:6) was the means by which the heavens and earth were created and a flood destroyed the world. If it would be helpful in your language, you could express this explicitly. Alternate translation: “by the same word of God”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#4) (id: 257932)

**πυρὶ**

**"for fire"**

Here, **for** indicates the purpose for which God has been reserving the present heavens and earth. Alternate translation: “for the purpose of fire”

See: Connect — Goal (Purpose) Relationship

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** Connect — Goal (Purpose) Relationship (ID: `33330@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#5) (id: 257933)

**πυρὶ**

**"for fire"**

Here, Peter uses **fire** to refer to what fire does, which is burn. Alternate translation: “for burning with fire”

See: Metonymy

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#6) (id: 257934)

**τηρούμενοι εἰς ἡμέραν κρίσεως**

**"being kept for a day of judgment"**

If your language does not use this passive form, you can state this in active form and say who is doing the action. It might also be helpful to begin a new sentence. Alternate translation: “God is keeping them for the day of judgment”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#7) (id: 257935)

**εἰς ἡμέραν κρίσεως**

**"for a day of judgment"**

Here, **for** could refer to: (1\) the purpose for which God is keeping the present heavens and earth. Alternate translation: “for the purpose of the day of judgment” (2\) the point in time up to which God is keeping the present heavens and earth. Alternate translation: “until the day of judgment”

* **Associated Passages:** 2 Peter 3:7

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#8) (id: 257936)

**ἡμέραν κρίσεως καὶ ἀπωλείας τῶν ἀσεβῶν ἀνθρώπων**

**"a day of judgment and destruction of the ungodly men"**

If it would be helpful in your language, you could express the abstract nouns **judgment** and **destruction** by stating the ideas behind them with verbal phrases. Alternate translation: “the day when God judges mankind and destroys the ungodly men”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:7 (#9) (id: 257937)

**τῶν ἀσεβῶν ἀνθρώπων**

**"of the ungodly men"**

Although the term **men** is masculine, Peter is using the word here in a generic sense that includes both men and women. Alternate translation: “of the ungodly people”

See: When Masculine Words Include Women

* **Associated Passages:** 2 Peter 3:7
* **Associated Articles:** When Masculine Words Include Women (ID: `33276@UWTranslationManual`)

* **Associated ACAI Concepts:** Court Case (ID: `keyterm:CourtCase`); Sky (ID: `keyterm:Heaven`); Judgment (ID: `keyterm:Judgment`)

## 2 Peter 3:8 (#1) (id: 257938)

**ἓν \& τοῦτο μὴ λανθανέτω ὑμᾶς**

**"let this one thing not be hidden {from} you"**

Alternate translation: “do not fail to understand this one fact” or “do not ignore this one thing”

* **Associated Passages:** 2 Peter 3:8

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`)

## 2 Peter 3:8 (#2) (id: 257939)

**ὅτι μία ἡμέρα παρὰ Κυρίῳ ὡς χίλια ἔτη**

**"that one day with the Lord {is} like 1,000 years"**

Here, **with the Lord** means “in the judgment of the Lord.” Alternate translation: “that from the Lord’s point of view, one day is like a thousand years”

* **Associated Passages:** 2 Peter 3:8

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`)

## 2 Peter 3:8 (#3) (id: 257940)

**μία ἡμέρα παρὰ Κυρίῳ ὡς χίλια ἔτη, καὶ χίλια ἔτη ὡς ἡμέρα μία**

**"one day with the Lord {is} like 1,000 years, and 1,000 years {are} like one day"**

These two phrases mean basically the same thing. The repetition is used to emphasize that God does not perceive of time the same way that humans do. What might seem to be a short or long time for people would not seem so for God. If it would be helpful in your language, you could combine these phrases. Alternate translation: “one day and 1,000 years are alike for the Lord”

See: Doublet

* **Associated Passages:** 2 Peter 3:8
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`)

## 2 Peter 3:9 (#1) (id: 257941)

**οὐ βραδύνει Κύριος τῆς ἐπαγγελίας**

**"The Lord does not delay {concerning} the promise"**

Here, **promise** refers to the fulfillment of the **promise** that Jesus will return. See how you translated it in [3:4](https://ref.ly/2Pet3:4). Alternate translation: “The Lord does not delay to fulfill his promise”

See: Metonymy

* **Associated Passages:** 2 Peter 3:9
* **Associated Articles:** Metonymy (ID: `33293@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`)

## 2 Peter 3:9 (#2) (id: 257942)

**ὥς τινες βραδύτητα ἡγοῦνται**

**"as some consider delay"**

Here, **some** refers to the “mockers” introduced in [3:3](https://ref.ly/2Pet3:3) and anyone else who believed that the Lord is slow to fulfill his promises, because Jesus had not yet returned. If it would be helpful in your language, you could express this explicitly. Alternate translation: “as some, such as these mockers, consider slowness”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:9
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`)

## 2 Peter 3:9 (#3) (id: 257943)

**ἀλλὰ μακροθυμεῖ εἰς ὑμᾶς**

**"but is patient toward you"**

Peter is leaving out a word that this clause would need in many languages in order to be complete. This word can be supplied from the beginning of the verse. If it would be helpful in your language, you could state explicitly who is the one being **patient** and begin a new sentence. Alternate translation: “But God is patient toward you”

See: Ellipsis

* **Associated Passages:** 2 Peter 3:9
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`)

## 2 Peter 3:9 (#4) (id: 257944)

**μὴ βουλόμενός τινας ἀπολέσθαι**

**"not wanting any to perish"**

This clause indicates the reason why God is delaying the return of Jesus. Alternate translation: “because he does not want any to perish”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 3:9
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`)

## 2 Peter 3:9 (#5) (id: 257945)

**ἀλλὰ πάντας εἰς μετάνοιαν χωρῆσαι**

**"but all to move on to repentance"**

Peter is leaving out words that this clause would need in many languages in order to be complete. These words can be supplied from the previous clause. Alternate translation: “but he wants all to move on to repentance”

See: Ellipsis

* **Associated Passages:** 2 Peter 3:9
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`)

## 2 Peter 3:9 (#6) (id: 257946)

**ἀλλὰ πάντας εἰς μετάνοιαν χωρῆσαι**

**"but all to move on to repentance"**

If it would be helpful in your language, you could express the idea behind the abstract noun **repentance** with a verbal phrase. Alternate translation: “but all to repent”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:9
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Repent (ID: `keyterm:Repent`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#1) (id: 257947)

**δὲ**

**"But"**

Here, **But** indicates a contrast between what the mockers believed about God and what God will really do. Although the Lord is being patient and wants people to repent, he will indeed return and bring judgment.

See: Connect — Contrast Relationship

* **Associated Passages:** 2 Peter 3:10
* **Associated Articles:** Connect — Contrast Relationship (ID: `33329@UWTranslationManual`)

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#2) (id: 257948)

**ἥξει \& ἡμέρα Κυρίου ὡς κλέπτης**

**"the day of the Lord will come as a thief"**

Peter speaks of the **day** when God will judge everyone as if it were a **thief** who will come unexpectedly and take people by surprise. If it would be helpful in your language, you could state this simile plainly. Alternate translation: “the day of the Lord will come unexpectedly”

See: Simile

* **Associated Passages:** 2 Peter 3:10
* **Associated Articles:** Simile (ID: `33314@UWTranslationManual`)

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#3) (id: 257949)

**κλέπτης, ἐν ᾗ**

**"a thief, in which"**

Here, **which** refers to “the day of the Lord”. If it would be helpful for your readers, you could express this explicitly and begin a new sentence. Alternate translation: “a thief. In the day of the Lord”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:10
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#4) (id: 257950)

**στοιχεῖα \& λυθήσεται**

**"the elements will be destroyed"**

If your language does not use this passive form, you can state this in an active form and you can indicate who will do the action. Alternate translation: “God will destroy the elements”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:10
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#5) (id: 257951)

**στοιχεῖα \& λυθήσεται**

**"the elements will be destroyed"**

Here, **elements** could refer to: (1\) the basic elements that make up the natural universe. Alternate translation: “the components of nature will be destroyed” (2\) the heavenly bodies, such as the sun, moon, and stars. Alternate translation: “the heavenly bodies will be destroyed”

* **Associated Passages:** 2 Peter 3:10

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#6) (id: 257952)

**στοιχεῖα δὲ καυσούμενα λυθήσεται**

**"and the elements will be destroyed, being burned up"**

Here, **being burned up** refers to the means by which the elements will be destroyed. If it would be helpful in your language, you could express this explicitly. Alternate translation: “and the elements will be destroyed by means of burning” or “and the elements will be destroyed by means of fire”

* **Associated Passages:** 2 Peter 3:10

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#7) (id: 257953)

**γῆ καὶ τὰ ἐν αὐτῇ ἔργα εὑρεθήσεται**

**"the earth and the deeds in it will be found"**

God will see all the **earth** and all the **deeds** of everyone, and he will then judge everything. If your language does not use this passive form, you can state this in active terms. Alternate translation: “God will find the earth and the deeds in it”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:10
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#8) (id: 257954)

**γῆ καὶ τὰ ἐν αὐτῇ ἔργα εὑρεθήσεται**

**"the earth and the deeds in it will be found"**

Here, **deeds** refers to the actions of people on the earth. If it would be helpful in your language, you could express this explicitly. Alternate translation: “the earth and what people have done in it will be found”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:10
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:10 (#9) (id: 257955)

**γῆ καὶ τὰ ἐν αὐτῇ ἔργα εὑρεθήσεται**

**"the earth and the deeds in it will be found"**

Here, **found** indicates that the removal of the heavens and elements will leave the earth and what has been done on it exposed for God to see and judge. Alternate translation: “the earth and the deeds in it will be exposed” or “the earth and the deeds in it will be revealed”

* **Associated Passages:** 2 Peter 3:10

* **Associated ACAI Concepts:** Fundamental Principles (ID: `keyterm:FundamentalPrinciples`); Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`)

## 2 Peter 3:11 (#1) (id: 257956)

**τούτων οὕτως πάντων λυομένων**

**"All of these things being thus destroyed"**

This clause indicates the reason for the expected result that follows in the rest of the verse. Peter tells his readers that God’s future destruction of heaven and earth should result in them living holy and godly lives. Alternate translation: “Because of all of these things being thus destroyed”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 3:11
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`)

## 2 Peter 3:11 (#2) (id: 257957)

**τούτων οὕτως πάντων λυομένων**

**"All of these things being thus destroyed"**

If your language does not use this passive form, you can state this in active form and you can state who will do the action. Alternate translation: “Since God will thus destroy all of these things”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:11
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`)

## 2 Peter 3:11 (#3) (id: 257958)

**τούτων**

**"of these things"**

Here, **these things** refers to the heavens, the elements, and the earth mentioned in the previous verse. If it would be helpful in your language, you could express this explicitly. Alternate translation: “these things that I have just described”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:11
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`)

## 2 Peter 3:11 (#4) (id: 257959)

**ποταποὺς δεῖ ὑπάρχειν ὑμᾶς?**

**"of what sort is it necessary for you to be"**

Peter is using the question form for emphasis. If it would be helpful in your language, you could express this emphasis by translating his words as a statement or an exclamation and communicate the emphasis in another way. If it you translate these words as a statement, then you will need to change the question mark at the end of the next verse into a period. Alternate translation: “it is certainly necessary for you to be”

See: Rhetorical Question

* **Associated Passages:** 2 Peter 3:11
* **Associated Articles:** Rhetorical Question (ID: `33310@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`)

## 2 Peter 3:11 (#5) (id: 257960)

**ἐν ἁγίαις ἀναστροφαῖς καὶ εὐσεβείαις**

**"in holy behaviors and godliness"**

Peter is leaving out words that this clause would need in many languages in order to be complete. These words can be supplied from the context. Alternate translation: “living with holy behaviors and godly acts”

See: Ellipsis

* **Associated Passages:** 2 Peter 3:11
* **Associated Articles:** Ellipsis (ID: `33267@UWTranslationManual`)

* **Associated ACAI Concepts:** Holy (ID: `keyterm:Holy`)

## 2 Peter 3:12 (#1) (id: 257961)

**προσδοκῶντας καὶ σπεύδοντας**

**"waiting for and hastening"**

Here, **waiting for and hastening** are two things Peter wanted his readers to do while also living holy and godly lives, as stated in the previous verse. Alternate translation: “while expecting and hastening”

* **Associated Passages:** 2 Peter 3:12

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`); Fundamental Principles (ID: `keyterm:FundamentalPrinciples`)

## 2 Peter 3:12 (#2) (id: 257962)

**δι’ ἣν**

**"because of which"**

Here, **which** refers to “the day of God” from the previous clause. If it would be helpful in your language, you could express this explicitly. Alternate translation: “because of that day”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:12
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`); Fundamental Principles (ID: `keyterm:FundamentalPrinciples`)

## 2 Peter 3:12 (#3) (id: 257963)

**πυρούμενοι**

**"being set on fire"**

This phrase indicates the means by which the heavens will be destroyed. Alternate translation: “by means of being set on fire”

* **Associated Passages:** 2 Peter 3:12

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`); Fundamental Principles (ID: `keyterm:FundamentalPrinciples`)

## 2 Peter 3:12 (#4) (id: 257964)

**οὐρανοὶ πυρούμενοι, λυθήσονται**

**"the heavens, being set on fire, will be destroyed"**

If your language does not use this passive form, you can state this in active form and say who will do the action. Alternate translation: “God will destroy the heavens with fire”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:12
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`); Fundamental Principles (ID: `keyterm:FundamentalPrinciples`)

## 2 Peter 3:12 (#5) (id: 257965)

**στοιχεῖα καυσούμενα, τήκεται**

**"the elements, being burned up by heat, will be melted"**

Here, **elements** could refer to: (1\) the basic elements that make up the natural universe. Alternate translation: “the components of nature will be destroyed by heat” (2\) the heavenly bodies, such as the sun, moon, and stars. Alternate translation: “the heavenly bodies will be destroyed by heat” See how you translated this in [3:10](https://ref.ly/2Pet3:10).

* **Associated Passages:** 2 Peter 3:12

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`); Fundamental Principles (ID: `keyterm:FundamentalPrinciples`)

## 2 Peter 3:12 (#6) (id: 257966)

**καυσούμενα**

**"being burned up by heat"**

This phrase indicates the means by which the heavens will be destroyed. Alternate translation: “by means of being burned up by heat”

* **Associated Passages:** 2 Peter 3:12

* **Associated ACAI Concepts:** Sky (ID: `keyterm:Heaven`); LORD (ID: `deity:Lord`); Fundamental Principles (ID: `keyterm:FundamentalPrinciples`)

## 2 Peter 3:13 (#1) (id: 257967)

**καινοὺς \& οὐρανοὺς καὶ γῆν καινὴν, κατὰ τὸ ἐπάγγελμα αὐτοῦ προσδοκῶμεν**

**"new heavens and a new earth, according to his promise, we are waiting for"**

Peter put the direct object of the main verb at the front of this sentence for emphasis. If your language uses a similar construction for emphasis, it would be appropriate to have it here in your translation. But if this construction would be confusing in your language, you could express this emphasis in another way and change the structure of the sentence. Alternate translation: “according to his promise, we are waiting for new heavens and a new earth”

See: Information Structure

* **Associated Passages:** 2 Peter 3:13
* **Associated Articles:** Information Structure (ID: `33286@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Sky (ID: `keyterm:Heaven`)

## 2 Peter 3:13 (#2) (id: 257968)

**κατὰ τὸ ἐπάγγελμα αὐτοῦ**

**"according to his promise"**

Here, the pronoun **his** could refer to: (1\) God. Alternate translation: “according to God’s promise” (2\) Jesus. Alternate translation: “according to Jesus’ promise”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:13
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Sky (ID: `keyterm:Heaven`)

## 2 Peter 3:13 (#3) (id: 257969)

**"his promise"**

Here, **promise** could refer to: (1\) God’s promise to create a new heavens and earth, as promised in Isaiah 65:17 and Isaiah 66:22\. Alternate translation: “his promise of new heavens and a new earth” (2\) the promise of Jesus’ second coming, as in [3:4](https://ref.ly/2Pet3:4). Alternate translation: “his promise of the return of Jesus”

* **Associated Passages:** 2 Peter 3:13

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Sky (ID: `keyterm:Heaven`)

## 2 Peter 3:13 (#4) (id: 257970)

**ἐν οἷς δικαιοσύνη κατοικεῖ**

**"in which righteousness dwells"**

Here, **righteousness** is spoken of as though it were a person who could dwell somewhere. If this might be confusing for your readers, you could express this meaning plainly. Alternate translation: “in which righteousness exists”

See: Personification

* **Associated Passages:** 2 Peter 3:13
* **Associated Articles:** Personification (ID: `33302@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Sky (ID: `keyterm:Heaven`)

## 2 Peter 3:13 (#5) (id: 257971)

**ἐν οἷς δικαιοσύνη κατοικεῖ**

**"in which righteousness dwells"**

If it would be helpful in your language, you could express the abstract noun **righteousness** by translating the idea behind it with an equivalent expression. Alternate translation: “in which everyone is righteous” or “in which everyone does what is right”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:13
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Promise (ID: `keyterm:Promise`); Be (Or Show Oneself) Just (ID: `keyterm:Just`); Sky (ID: `keyterm:Heaven`)

## 2 Peter 3:14 (#1) (id: 257972)

**διό**

**"Therefore"**

Peter uses **Therefore** to introduce a description of what his readers should do as a result of what he has just said. He is referring specifically to the discussion of the coming day of the Lord given in [3:10–13](https://ref.ly/2Pet3:10-2Pet3:13). Alternate translation: “Because of these reasons”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 3:14
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Be Blameless (ID: `keyterm:Blameless`); Above Criticism (ID: `keyterm:AboveCriticism`); Make Peace (ID: `keyterm:Peace`)

## 2 Peter 3:14 (#2) (id: 257973)

**ἀγαπητοί**

**"beloved ones"**

Here, **beloved ones** refers to those whom Peter is writing to, which can be extended to all believers. If it would be helpful in your language, you could express this explicitly. See how you translated this in [3:1](https://ref.ly/2Pet3:1). Alternate translation: “beloved fellow believers”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:14
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Be Blameless (ID: `keyterm:Blameless`); Above Criticism (ID: `keyterm:AboveCriticism`); Make Peace (ID: `keyterm:Peace`)

## 2 Peter 3:14 (#3) (id: 257974)

**ταῦτα**

**"these things"**

Here, **these things** refers back to the events related to the coming day of the Lord, which Peter described in [3:10–13](https://ref.ly/2Pet3:10-2Pet3:13). If it would be helpful in your language, you could express this explicitly. Alternate translation: “these things that will happen on the day of the Lord”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:14
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Be Blameless (ID: `keyterm:Blameless`); Above Criticism (ID: `keyterm:AboveCriticism`); Make Peace (ID: `keyterm:Peace`)

## 2 Peter 3:14 (#4) (id: 257975)

**σπουδάσατε ἄσπιλοι καὶ ἀμώμητοι αὐτῷ εὑρεθῆναι**

**"be diligent to be found spotless and blameless by him"**

If your language does not use this passive form, you can state this in an active form. Alternate translation: “be diligent so that God will find you spotless and blameless”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:14
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Be Blameless (ID: `keyterm:Blameless`); Above Criticism (ID: `keyterm:AboveCriticism`); Make Peace (ID: `keyterm:Peace`)

## 2 Peter 3:14 (#5) (id: 257976)

**ἄσπιλοι καὶ ἀμώμητοι**

**"spotless and blameless"**

The words **spotless** and **blameless** share similar meanings. Peter is using them together for emphasis. If it would be helpful in your language, you could express this connection by translating them with a single expression. Alternate translation: “completely pure”

See: Doublet

* **Associated Passages:** 2 Peter 3:14
* **Associated Articles:** Doublet (ID: `33266@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Be Blameless (ID: `keyterm:Blameless`); Above Criticism (ID: `keyterm:AboveCriticism`); Make Peace (ID: `keyterm:Peace`)

## 2 Peter 3:14 (#6) (id: 257977)

**αὐτῷ**

**"by him"**

Here, the pronoun **him** could refer to: (1\) Jesus. Alternate translation: “by Jesus” (2\) God. Alternate translation: “by God”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:14
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Be Blameless (ID: `keyterm:Blameless`); Above Criticism (ID: `keyterm:AboveCriticism`); Make Peace (ID: `keyterm:Peace`)

## 2 Peter 3:14 (#7) (id: 257978)

**ἐν εἰρήνῃ**

**"in peace"**

Here, **in peace** could refer to: (1\) having peace with God. Alternate translation, as in the UST: “at peace with God” (2\) feeling peace in one’s heart. Alternate translation: “with peace in your heart”

* **Associated Passages:** 2 Peter 3:14

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Be Blameless (ID: `keyterm:Blameless`); Above Criticism (ID: `keyterm:AboveCriticism`); Make Peace (ID: `keyterm:Peace`)

## 2 Peter 3:15 (#1) (id: 257979)

**τὴν τοῦ Κυρίου ἡμῶν μακροθυμίαν, σωτηρίαν ἡγεῖσθε**

**"consider the patience of our Lord {to be} salvation"**

Because the Lord is patient, the day of judgment has not yet happened. This gives people an opportunity to repent and be saved, as Peter explained in [3:9](https://ref.ly/2Pet3:9). If it would be helpful in your language, you could express this explicitly. Alternate translation: “consider the patience of our Lord as an opportunity to repent and be saved”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:15
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Salvation (ID: `keyterm:Salvation`); Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`)

## 2 Peter 3:15 (#2) (id: 257980)

**τὴν τοῦ Κυρίου ἡμῶν μακροθυμίαν, σωτηρίαν ἡγεῖσθε**

**"consider the patience of our Lord {to be} salvation"**

If it would be helpful in your language, you could express the abstract nouns **patience** and **salvation** by translating the ideas behind them with equivalent expressions. Alternate translation: “that our Lord is patient to save people”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:15
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Salvation (ID: `keyterm:Salvation`); Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`)

## 2 Peter 3:15 (#3) (id: 257981)

**σωτηρίαν**

**"to be} salvation"**

If it would be helpful in your language, you could express the abstract noun **salvation** by translating the idea behind it with an equivalent expression. Alternate translation: “to save people”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:15
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Salvation (ID: `keyterm:Salvation`); Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`)

## 2 Peter 3:15 (#4) (id: 257982)

**ὁ ἀγαπητὸς ἡμῶν ἀδελφὸς Παῦλος**

**"our beloved brother Paul"**

Peter is using the term **brother** to refer to Paul as a fellow believer in Jesus. Alternate translation: “our beloved fellow Christian brother Paul”

See: Metaphor

* **Associated Passages:** 2 Peter 3:15
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Salvation (ID: `keyterm:Salvation`); Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`)

## 2 Peter 3:15 (#5) (id: 257983)

**κατὰ τὴν δοθεῖσαν αὐτῷ σοφίαν**

**"according to the wisdom having been given to him"**

If your language does not use this passive form, you can state this in active form and you can state who did the action. Alternate translation: “according to the wisdom that God gave to him”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:15
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Salvation (ID: `keyterm:Salvation`); Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`)

## 2 Peter 3:15 (#6) (id: 257984)

**κατὰ τὴν δοθεῖσαν αὐτῷ σοφίαν**

**"according to the wisdom having been given to him"**

If it would be helpful in your language, you could express the abstract noun **wisdom** by translating the idea behind it with an adjective, such as “wise.” Alternate translation: “according to the wise words having been given to him”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:15
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Salvation (ID: `keyterm:Salvation`); Beloved (ID: `keyterm:Beloved`); LORD (ID: `deity:Lord`); Paul (ID: `person:Paul`)

## 2 Peter 3:16 (#1) (id: 257985)

**ἐν πάσαις ταῖς ἐπιστολαῖς**

**"in all the letters"**

Here, the context suggests that Paul is the author of **the letters**. If it would be helpful in your language, you could express this explicitly. Alternate translation: “in all Paul’s letters”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 3:16 (#2) (id: 257986)

**λαλῶν ἐν αὐταῖς περὶ τούτων**

**"speaking in them about these things"**

Here, **these things** could refer to: (1\) the events related to the day of the Lord discussed in [3:10–13](https://ref.ly/2Pet3:10-2Pet3:13) and called “these things” in [3:14](https://ref.ly/2Pet3:14). Alternate translation: “these things that will happen on the day of the Lord” (2\) the need to live godly lives and consider that God’s patience is for saving people, as discussed in [3:14–15](https://ref.ly/2Pet3:14-2Pet3:15). Alternate translation: “these things I have just said about living blamelessly and about God’s patience”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:16
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 3:16 (#3) (id: 257987)

**ἃ οἱ ἀμαθεῖς καὶ ἀστήρικτοι στρεβλοῦσιν**

**"which the ignorant and unstable distort"**

Here, **which** refers to the things in Paul’s letters that are difficult to understand. If it would be helpful in your language, you could state this phrase explicitly and start a new sentence. Alternate translation: “The ignorant and unstable ones distort these difficult things found in Paul’s letters”

See: Pronouns — When to Use Them

* **Associated Passages:** 2 Peter 3:16
* **Associated Articles:** Pronouns — When to Use Them (ID: `33424@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 3:16 (#4) (id: 257988)

**ἃ οἱ ἀμαθεῖς καὶ ἀστήρικτοι στρεβλοῦσιν**

**"which the ignorant and unstable distort"**

Here, **distort** is used to describe changing the meaning of a statement in order to give a false meaning like twisting something so that it changes shape. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “which the ignorant and unstable ones interpret falsely”

See: Metaphor

* **Associated Passages:** 2 Peter 3:16
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 3:16 (#5) (id: 257989)

**ὡς καὶ τὰς λοιπὰς Γραφὰς**

**"as also the other scriptures"**

Here, **the other scriptures** refers to both the entire Old Testament and the New Testament texts that had been written by the time Peter wrote this letter. If it would be helpful to your readers, you could express this explicitly. Alternate translation: “as also the other authoritative scriptures”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:16
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 3:16 (#6) (id: 257990)

**πρὸς τὴν ἰδίαν αὐτῶν ἀπώλειαν**

**"to their own destruction"**

Here, **to** indicates that this clause provides the result of the “ignorant and unstable ones” falsely interpreting the scriptures. If it would be helpful in your language, you could express this explicitly. Alternate translation: “resulting in their own destruction”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 3:16
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 3:16 (#7) (id: 257991)

**πρὸς τὴν ἰδίαν αὐτῶν ἀπώλειαν**

**"to their own destruction"**

If it would be helpful in your language, you could express the idea behind the abstract noun **destruction** with a verbal phrase. Alternate translation: “so that they will be destroyed”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:16
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Written (ID: `keyterm:Scripture`)

## 2 Peter 3:17 (#1) (id: 257992)

**οὖν**

**"Therefore"**

Connecting Statement:\\n\\nHere, Peter uses **Therefore** to introduce a description of what his readers should do as a result of what he has said, which could be: (1\) the destruction of those who falsely interpret the scriptures mentioned in the previous verse. Alternate translation: “Because those who falsely interpret the Scriptures will be destroyed” (2\) the previous content of the entire letter, especially the sure destruction of the false teachers. Alternate translation: “Because of all these things I have told you”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#2) (id: 257993)

**ἀγαπητοί**

**"beloved ones"**

Here, **beloved ones** refers to those whom Peter is writing to, which can be extended to all believers. If it would be helpful in your language, you could express this explicitly. See how you translated this in [3:1](https://ref.ly/2Pet3:1) and [3:14](https://ref.ly/2Pet3:14). Alternate translation: “beloved fellow believers”

See: Assumed Knowledge and Implicit Information

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Assumed Knowledge and Implicit Information (ID: `33273@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#3) (id: 257994)

**προγινώσκοντες**

**"knowing beforehand"**

Connecting Statement:\\n\\nHere, Peter is giving the reason why his readers should heed his command in the next phrase. Alternate translation: “since you know beforehand”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#4) (id: 257995)

**ἵνα μὴ \& ἐκπέσητε τοῦ ἰδίου στηριγμοῦ**

**"so that you might not lose your own steadfastness"**

Here, Peter speaks of **steadfastness** as if it were a possession that believers could **lose**. If it would be helpful in your language, you could express this metaphor plainly. Alternate translation: “so that you might not stop being steadfast”

See: Metaphor

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#5) (id: 257996)

**ἵνα μὴ \& ἐκπέσητε τοῦ ἰδίου στηριγμοῦ**

**"so that you might not lose your own steadfastness"**

If it would be helpful in your language, you could express the idea behind the abstract noun **steadfastness** with the adjective “steadfast.” Alternate translation: “so that you might not lose your own steadfast faith”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#6) (id: 257997)

**τῇ τῶν ἀθέσμων πλάνῃ συναπαχθέντες**

**"having been led astray by the error of the lawless ones"**

This clause indicates the reason why people might lose their own steadfastness. If it would be helpful in your language, you could express this explicitly. Alternate translation: “because of being led astray by the error of the lawless ones”

See: Connect — Reason\-and\-Result Relationship

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Connect — Reason-and-Result Relationship (ID: `33331@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#7) (id: 257998)

**τῇ τῶν ἀθέσμων πλάνῃ συναπαχθέντες**

**"having been led astray by the error of the lawless ones"**

If your language does not use this passive form, you can state this in active form. Alternate translation: “the error of the lawless ones having led you astray”

See: Active or Passive

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Active or Passive (ID: `33258@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#8) (id: 257999)

**τῇ τῶν ἀθέσμων πλάνῃ συναπαχθέντες**

**"having been led astray by the error of the lawless ones"**

Here, Peter uses **led astray** to describe people being deceived into living wickedly by false teachers as if they had been led away from the straight path. If it would be helpful in your language, you could express this metaphor by translating this plainly. Alternate translation: “having been deceived into living wickedly by the error of the lawless ones”

See: Metaphor

* **Associated Passages:** 2 Peter 3:17
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:17 (#9) (id: 258000)

**τῇ τῶν ἀθέσμων πλάνῃ**

**"by the error of the lawless ones"**

This phrase indicates the means by which one might be led astray. Alternate translation: “by means of the error of the lawless ones”

* **Associated Passages:** 2 Peter 3:17

* **Associated ACAI Concepts:** Beloved (ID: `keyterm:Beloved`); Lawless (ID: `keyterm:Lawless`)

## 2 Peter 3:18 (#1) (id: 258001)

**αὐξάνετε \& ἐν χάριτι, καὶ γνώσει τοῦ Κυρίου ἡμῶν καὶ Σωτῆρος, Ἰησοῦ Χριστοῦ**

**"grow in grace and knowledge of our Lord and Savior Jesus Christ"**

Here, **grow** is used to express experiencing or having something in increasing amounts. If it would be helpful in your language, you could state this metaphor plainly. Alternate translation: “have more and more of the grace and knowledge of our Lord and Savior Jesus Christ”

See: Metaphor

* **Associated Passages:** 2 Peter 3:18
* **Associated Articles:** Metaphor (ID: `33292@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Favor (ID: `keyterm:Favor`); Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`)

## 2 Peter 3:18 (#2) (id: 258002)

**ἐν χάριτι, καὶ γνώσει**

**"in grace and knowledge"**

Here, **in** means “with reference to.” Alternate translation: “with reference to the grace and knowledge”

* **Associated Passages:** 2 Peter 3:18

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Favor (ID: `keyterm:Favor`); Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`)

## 2 Peter 3:18 (#3) (id: 258003)

**χάριτι**

**"grace"**

If it would be helpful in your language, you could express the abstract noun **grace** by translating it with an equivalent expression. Alternate translation: “kind acts”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:18
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Favor (ID: `keyterm:Favor`); Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`)

## 2 Peter 3:18 (#4) (id: 258004)

**γνώσει**

**"knowledge"**

If it would be helpful in your language, you could express the abstract noun **knowledge** by translating it with an equivalent expression. Alternate translation: “what you know”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:18
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Favor (ID: `keyterm:Favor`); Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`)

## 2 Peter 3:18 (#5) (id: 258005)

**τοῦ Κυρίου ἡμῶν καὶ Σωτῆρος**

**"of our Lord and Savior"**

Here, **our Lord** means “the person who is lord over us” or “the person who rules over us.” The conjunction and indicates that our also applies to **Savior**, meaning “the person who saves us.” If you include both of these phrases in your translation, then you will also need to put a comma at the end of the second phrase. Alternate translation: “of the person who rules over us and saves us,”

See: Possession

* **Associated Passages:** 2 Peter 3:18
* **Associated Articles:** Possession (ID: `33303@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Favor (ID: `keyterm:Favor`); Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`)

## 2 Peter 3:18 (#6) (id: 258006)

**αὐτῷ ἡ δόξα**

**"To him {be} the glory"**

If it would be helpful in your language, you could express the abstract noun **glory** by translating it with an equivalent expression. Alternate translation: “May everyone glorify him”

See: Abstract Nouns

* **Associated Passages:** 2 Peter 3:18
* **Associated Articles:** Abstract Nouns (ID: `33257@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Favor (ID: `keyterm:Favor`); Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`)

## 2 Peter 3:18 (#7) (id: 258007)

**εἰς ἡμέραν αἰῶνος**

**"to the day of the age"**

Here, **to the day of the age** is an idiom that means “forever.” If it would be helpful in your language, you could use an equivalent idiom or use plain language. Alternate translation: “to eternity” or “forever”

See: Idiom

* **Associated Passages:** 2 Peter 3:18
* **Associated Articles:** Idiom (ID: `33283@UWTranslationManual`)

* **Associated ACAI Concepts:** Glory (ID: `keyterm:Glory`); Favor (ID: `keyterm:Favor`); Jesus (ID: `person:Jesus.2`); Always (ID: `keyterm:Always`); Savior (ID: `keyterm:Savior`)
